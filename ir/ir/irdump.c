...
static ir_dump_flags_t flags =
    ir_dump_flag_blocks_as_subgraphs | ir_dump_flag_keepalive_edges |
    ir_dump_flag_ld_names | ir_dump_flag_back_edges |
    ir_dump_flag_consts_local | ir_dump_flag_entities_in_hierarchy |
...
void ir_set_dump_flags(ir_dump_flags_t new_flags) { flags = new_flags; }

void ir_add_dump_flags(ir_dump_flags_t new_flags) { flags |= new_flags; }

void ir_remove_dump_flags(ir_dump_flags_t to_remove) { flags &= ~to_remove; }

ir_dump_flags_t ir_get_dump_flags(void) { return flags; }
...
static bool get_opt_dump_const_local(void) {
  return (flags & ir_dump_flag_out_edges) || (flags & ir_dump_flag_loops) ||
         (flags & ir_dump_flag_consts_local) || (flags & ir_dump_flag_iredges);
}
...
const char *ir_get_dump_filter(void) { return dump_filter; }

/** Returns true if dump file filter is not set, or if it is a substring of
 * name. */
 ...
 * to the file F.
 */
static void print_type_type_edge(FILE *F, const ir_type *S, const ir_type *T,
                                 const char *fmt, ...) {
  va_list ap;
 ...
 * to the file F.
 */
static void print_type_ent_edge(FILE *F, const ir_type *tp,
                                const ir_entity *ent, const char *fmt, ...) {
  va_list ap;
...
/**
 * Prints the edge from an entity ent1 to an entity ent2 with additional info
 * fmt, ... to the file F.
 */
static void print_ent_ent_edge(FILE *F, const ir_entity *ent1,
                               const ir_entity *ent2, int backedge,
                               ird_color_t color, const char *fmt, ...) {
  va_list ap;
  ...
  ir_vfprintf(F, fmt, ap);
  fprintf(F, " ");
  if (color != ird_color_none) print_vcg_color(F, color);
  fprintf(F, "}\n");
  va_end(ap);
}
 ...
 * to the file F.
 */
static void print_ent_type_edge(FILE *F, const ir_entity *ent,
                                const ir_type *tp, const char *fmt, ...) {
  va_list ap;
 ...
 * to the file F.
 */
static void print_node_type_edge(FILE *F, const ir_node *irn, ir_type *tp,
                                 const char *fmt, ...) {
  va_list ap;
 ...
 * ... to the file F.
 */
static void print_node_ent_edge(FILE *F, const ir_node *irn,
                                const ir_entity *ent, const char *fmt, ...) {
  va_list ap;
...
static void *ird_get_irn_link(const ir_node *n) {
  if (irdump_link_map == NULL) return NULL;
...
static void ird_set_irn_link(const ir_node *n, void *x) {
  if (irdump_link_map == NULL) init_irdump();
...
static const char *get_ent_dump_name_(const ir_entity *ent, bool dump_ld_name) {
  if (ent == NULL) return "<NULL entity>";
  if (dump_ld_name) {
    /* Don't use get_entity_ld_ident (ent) as it computes the mangled name! */
    if (ent->ld_name != NULL) return get_id_str(ent->ld_name);
  }
 ...
 */
static void ird_walk_graph(ir_graph *irg, irg_walk_func *pre,
...
static void collect_node(ir_node *node, void *env) {
  (void)env;
  if (is_Block(node) || node_floats(node) ||
      (get_op_flags(get_irn_op(node)) & irop_flag_dump_noblock)) {
    ir_node **arr = (ir_node **)ird_get_irg_link(get_irn_irg(node));
    if (arr == NULL) arr = NEW_ARR_F(ir_node *, 0);
    ARR_APP1(ir_node *, arr, node);
    ird_set_irg_link(get_irn_irg(node),
                     arr); /* arr is an l-value, APP_ARR might change it! */
  } else {
...
static ir_node **construct_block_lists(ir_graph *irg) {
  ...
  foreach_irp_irg_r(i, irg) { ird_set_irg_link(irg, NULL); }
...
void dump_node_opcode(FILE *F, const ir_node *n) {
  ...
  switch (get_irn_opcode(n)) {
    ...
    case iro_Block: {
      ...
      char const *const prefix = n == get_irg_start_block(irg) ? "Start "
    ...
    case iro_Div:
      fprintf(F, "%s", name);
      if (get_Div_no_remainder(n)) fprintf(F, "RL");
      fprintf(F, "[%s]", get_mode_name(get_Div_resmode(n)));
      break;

    case iro_Phi:
      fprintf(F, "%s", name);
      if (get_Phi_loop(n)) fprintf(F, "[loop]");
      break;

    case iro_Load: {
      char const *const prefix =
          get_Load_unaligned(n) == align_non_aligned ? "ua" : "";
    ...
    case iro_Store: {
      char const *const prefix =
          get_Store_unaligned(n) == align_non_aligned ? "ua" : "";
...
static void dump_node_mode(FILE *F, const ir_node *n) {
  ...
  if ((mode != mode_ANY && mode != mode_BAD && mode != mode_BB &&
       mode != mode_M && mode != mode_T && mode != mode_X) ||
      is_Bad(n) || is_Proj(n))
  ...
  switch (get_irn_opcode(n)) {
    case iro_Proj: {
      ...
      if (code == iro_Proj && is_Start(get_Proj_pred(pred))) {
        ...
        for (size_t i = 0, n = ir_switch_table_get_n_entries(table); i < n;
             ++i) {
          ir_switch_table_entry const *const entry =
              ir_switch_table_get_entry_const(table, i);
          if (entry->pn != proj_nr) continue;
          ir_fprintf(F, "%s%T", sep, entry->min);
          if (entry->min != entry->max) ir_fprintf(F, "...%T", entry->max);
          sep = ", ";
        }
        fputc(' ', F);
      } else {
        ...
        for (unsigned i = 0; i < ARRAY_SIZE(proj_lut); ++i) {
          if (code != proj_lut[i].code) continue;
          for (unsigned j = 0; j < proj_lut[i].num_data; ++j) {
            if (proj_nr != proj_lut[i].data[j].nr) continue;
        ...
        if (!found) fprintf(F, "%u ", proj_nr);
        if (code == iro_Cond && get_Cond_jmp_pred(pred) != COND_JMP_PRED_NONE) {
          if (proj_nr == pn_Cond_false &&
              get_Cond_jmp_pred(pred) == COND_JMP_PRED_FALSE)
            fprintf(F, "PRED ");
          if (proj_nr == pn_Cond_true &&
              get_Cond_jmp_pred(pred) == COND_JMP_PRED_TRUE)
            fprintf(F, "PRED ");
        }
      }
      break;
 ...
 */
static void dump_node_vcgattr(FILE *F, const ir_node *node,
                              const ir_node *local) {
  if (dump_node_vcgattr_hook != NULL) {
    if (dump_node_vcgattr_hook(F, node, local) != 0) return;
  }
  ...
  if (is_op_constlike(op)) {
    print_vcg_color(F, ird_color_const);
             ...
             (is_op_fragile(op) && ir_throws_exception(n))) {
    print_vcg_color(F, ird_color_controlflow);
...
static void dump_node_info(FILE *F, const ir_node *n) {
  ...
  /* call the dump_node operation if available */
  if (ops->dump_node != NULL) ops->dump_node(F, n, dump_node_info_txt);
...
static void print_constblkid(FILE *F, const ir_node *node,
                             const ir_node *block) {
...
static void dump_const_block_local(FILE *F, const ir_node *n) {
  ...
  if (is_irn_constlike(blk)) {
    ...
    if (dump_edge_vcgattr_hook != NULL && dump_edge_vcgattr_hook(F, n, -1)) {
      fprintf(F, "}\n");
      return;
    }
...
void dump_node(FILE *F, const ir_node *n) {
  if (get_opt_dump_const_local() && is_irn_constlike(n)) return;
...
void dump_ir_data_edges(FILE *F, const ir_node *n) {
  if (dump_node_edge_hook) dump_node_edge_hook(F, n);
...
void dump_begin_block_subgraph(FILE *F, const ir_node *block) {
  ...
  ird_color_t const color =
      !get_Block_matured(block) ? ird_color_error : ird_color_block_background;
...
void dump_block_edges(FILE *F, const ir_node *block) {
  ...
  if (dump_block_edge_hook) dump_block_edge_hook(F, block);
}
  ...
  switch (get_irn_opcode(n)) {
    {
      ir_type *type;
      case iro_Align:
        type = get_Align_type(n);
        goto type;
      case iro_Call:
        type = get_Call_type(n);
        goto type;
      case iro_Size:
        type = get_Size_type(n);
        goto type;
      type:
...
void dump_type_node(FILE *F, ir_type *tp) {
  ...
  print_vcg_color(
      F, is_segment_type(tp) ? ird_color_segment_type : ird_color_type);
  fprintf(F, "}\n");
}
 ...
 */
static void dump_type_info(ir_type *const tp, ir_entity *const ent,
                           void *const env) {
  FILE *F = (FILE *)env;
  ...
  if (ent != NULL) {
    ...
    if (is_Class_type(get_entity_owner(ent))) {
      for (size_t i = get_entity_n_overwrites(ent); i-- > 0;)
        print_ent_ent_edge(F, ent, get_entity_overwrites(ent, i), 0,
                           ird_color_none, ENT_OVERWRITES_EDGE_ATTR);
    }
    /* attached subgraphs */
    ...
    switch (get_type_opcode(tp)) {
      ...
      case tpo_method:
        for (size_t i = get_method_n_params(tp); i-- > 0;) {
          print_type_type_edge(F, tp, get_method_param_type(tp, i),
                               METH_PAR_EDGE_ATTR, i);
        }
        for (size_t i = get_method_n_ress(tp); i-- > 0;) {
          print_type_type_edge(F, tp, get_method_res_type(tp, i),
                               METH_RES_EDGE_ATTR, i);
        }
        return;
      case tpo_array:
        print_type_type_edge(F, tp, get_array_element_type(tp),
                             ARR_ELT_TYPE_EDGE_ATTR);
        return;
      case tpo_pointer:
        print_type_type_edge(F, tp, get_pointer_points_to_type(tp),
 ...
 */
static void dump_class_hierarchy_node(ir_type *const tp, ir_entity *const ent,
                                      void *const ctx) {
  FILE *F = (FILE *)ctx;

  /* dump this type or entity */
  if (ent != NULL) {
    if (get_entity_owner(ent) == get_glob_type()) return;
    if (!is_Method_type(get_entity_type(ent))) return; /* GL */
    if (flags & ir_dump_flag_entities_in_hierarchy &&
      ...
      for (size_t i = get_entity_n_overwrites(ent); i-- > 0;) {
        print_ent_ent_edge(F, get_entity_overwrites(ent, i), ent, 0,
                           ird_color_none, ENT_OVERWRITES_EDGE_ATTR);
      }
    }
  } else {
    if (tp == get_glob_type()) return;
    if (is_Class_type(tp)) {
      dump_type_node(F, tp);
      /* and now the edges */
      for (size_t i = get_class_n_supertypes(tp); i-- > 0;) {
...
#ifdef DEBUG_libfirm
  fprintf(F, "\n The loop was analyzed %ld times.",
          (long int)PTR_TO_INT(get_loop_link(loop)));
#endif
  fprintf(F, "\"");
}
...
static void dump_loops(FILE *F, const ir_loop *loop) {
  ...
  if (get_loop_depth(loop) != 0) {
    for (size_t i = n_elements; i-- > 0;) {
      ...
      if (*element.kind != k_ir_node) continue;
 ...
 */
void dump_vcg_header(FILE *F, const char *name, const char *layout,
                     const char *orientation) {
  const char *label = (flags & ir_dump_flag_disable_edge_labels) ? "no" : "yes";

  if (layout == NULL) layout = "Compilergraph";
  if (orientation == NULL) orientation = "bottom_to_top";
...
static void dump_block_to_cfg(ir_node *block, void *env) {
  ...
  if (is_Block(block)) {
    ...
    print_nodeid(F, block);
    fprintf(F, " label: \"");
    if (block == get_irg_start_block(get_irn_irg(block))) fprintf(F, "Start ");
    if (block == get_irg_end_block(get_irn_irg(block))) fprintf(F, "End ");
    ...
    for (int i = get_Block_n_cfgpreds(block); i-- > 0;) {
      ...
      if (!is_Bad(pred)) pred = get_nodes_block(pred);
      fprintf(F, "edge: { sourcename: ");
      print_nodeid(F, block);
      fprintf(F, " targetname: ");
      print_nodeid(F, pred);
    ...
    if (ir_get_dump_flags() & ir_dump_flag_dominance) {
      if (irg_has_properties(get_irn_irg(block),
                             IR_GRAPH_PROPERTY_CONSISTENT_DOMINANCE) &&
          get_Block_idom(block)) {
...
void dump_callgraph(FILE *F) {
  ...
  foreach_irp_irg_r(i, irg) {
    ...
    for (j = 0; j < n_callees; ++j) {
      ...
      const char *attr = be ? "label:\"recursion %zu\"" : "label:\"calls %zu\"";
...
static void dump_loops_standalone(FILE *F, ir_loop *loop) {
  ...
  for (size_t i = 0, n = get_loop_n_elements(loop); i < n; ++i) {
    ...
    if (get_kind(son) == k_ir_loop) {
      ...
      if (loop_node_started) { /* Close the "firm-nodes" node first if we
                                  started one. */
        fprintf(F, "\" }\n");
        fprintf(F, "edge: {sourcename: ");
        print_loopid(F, loop);
        fprintf(F, " targetname: \"l%ld-%lu-nodes\" label:\"%lu...%lu\"}\n",
                get_loop_loop_nr(loop), (unsigned long)first,
                (unsigned long)first, (unsigned long)i - 1);
      ...
      if (!loop_node_started) {
        ...
        fprintf(
            F, "node: { title: \"l%ld-%lu-nodes\" color: lightyellow label: \"",
            get_loop_loop_nr(loop), (unsigned long)i);
  ...
  if (loop_node_started) {
    ...
    print_loopid(F, loop);
    fprintf(F, "\" targetname: \"l%ld-%lu-nodes\" label:\"%lu...%lu\"}\n",
            get_loop_loop_nr(loop), (unsigned long)first, (unsigned long)first,
...
void dump_loop_tree(FILE *out, ir_graph *irg) {
  ...
  if (get_irg_loop(irg)) dump_loops_standalone(out, get_irg_loop(irg));
...
static void collect_nodeloop(FILE *F, ir_loop *loop, pset *loopnodes) {
  if (flags & ir_dump_flag_loops) dump_loop_node(F, loop);
  ...
  for (size_t i = 0, n = get_loop_n_elements(loop); i < n; ++i) {
    ...
    if (*(le.kind) == k_ir_loop) {
      if (flags & ir_dump_flag_loops) dump_loop_son_edge(F, loop, son_number++);
      /* Recur */
...
void dump_loop(FILE *F, ir_loop *l) {
  ...
  /* build block lists */
  foreach_pset(loopnodes, ir_node, n) { set_irn_link(n, NULL); }
  foreach_pset(extnodes, ir_node, n) { set_irn_link(n, NULL); }
  ...
  foreach_pset(loopnodes, ir_node, b) {
    if (is_Block(b)) {
      ...
      /* dump the nodes that go into the block */
      for (ir_node *n = (ir_node *)get_irn_link(b); n;
...
static void add_dump_path(void) {
  ...
  if (dump_path != NULL) {
    ...
    if (len > 0 && dump_path[len - 1] != '/') obstack_1grow(&obst, '/');
  }
}
...
void dump_ir_graph_ext(ir_graph_dump_func func, ir_graph *graph,
  ...
  if (!ir_should_dump(dump_name)) return;
  ...
  if (suffix != NULL) {
    if (suffix[0] != '.') obstack_1grow(&obst, '-');
...
void dump_all_ir_graphs(const char *suffix) {
  foreach_irp_irg(i, irg) { dump_ir_graph(irg, suffix); }
}
