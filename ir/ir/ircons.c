 ...
 */
static uninitialized_local_variable_func_t *default_initialize_local_variable =
    NULL;
...
static void try_remove_unnecessary_phi(ir_node *phi) {
  ...
  if (have_self_loop && get_irn_mode(phi) == mode_M) return;
...
void mature_immBlock(ir_node *block) {
  if (get_Block_matured(block)) return;
...
ir_node *new_d_Const_long(dbg_info *db, ir_mode *mode, long value) {
  assert(
      irg_is_constrained(current_ir_graph, IR_GRAPH_CONSTRAINT_CONSTRUCTION));
  return new_rd_Const_long(db, current_ir_graph, mode, value);
}

ir_node *new_rd_DivRL(dbg_info *dbgi, ir_node *block, ir_node *irn_mem,
                      ir_node *irn_left, ir_node *irn_right, int pinned) {
  ir_graph *const irg = get_irn_irg(block);
  ir_node *const in[] = {irn_mem, irn_left, irn_right};
  ir_node *res =
      new_ir_node(dbgi, irg, block, op_Div, mode_T, ARRAY_SIZE(in), in);
...
ir_node *new_r_DivRL(ir_node *block, ir_node *irn_mem, ir_node *irn_left,
                     ir_node *irn_right, int pinned) {
  return new_rd_DivRL(NULL, block, irn_mem, irn_left, irn_right, pinned);
}

ir_node *new_d_DivRL(dbg_info *dbgi, ir_node *irn_mem, ir_node *irn_left,
                     ir_node *irn_right, int pinned) {
  assert(
      irg_is_constrained(current_ir_graph, IR_GRAPH_CONSTRAINT_CONSTRUCTION));
  return new_rd_DivRL(dbgi, current_ir_graph->current_block, irn_mem, irn_left,
                      irn_right, pinned);
}

ir_node *new_DivRL(ir_node *irn_mem, ir_node *irn_left, ir_node *irn_right,
                   int pinned) {
...
ir_node *new_rd_Phi_loop(dbg_info *db, ir_node *block, int arity,
  ...
  if (optimized == res) keep_alive(optimized);
  return optimized;
}
...
ir_node *new_rd_immBlock(dbg_info *dbgi, ir_graph *irg) {
  ...
  ir_node *const res =
      new_ir_node(dbgi, irg, NULL, op_Block, mode_BB, -1, NULL);
  ...
  res->attr.block.graph_arr =
      NEW_ARR_DZ(ir_node *, get_irg_obstack(irg), irg->n_loc);
...
ir_node *new_r_immBlock(ir_graph *irg) { return new_rd_immBlock(NULL, irg); }

ir_node *new_d_immBlock(dbg_info *dbgi) {
  return new_rd_immBlock(dbgi, current_ir_graph);
}

ir_node *new_immBlock(void) { return new_d_immBlock(NULL); }
...
ir_node *(get_cur_block)(void) { return get_r_cur_block(current_ir_graph); }
...
ir_node *get_store(void) { return get_r_store(current_ir_graph); }
...
void set_store(ir_node *store) { set_r_store(current_ir_graph, store); }
...
ir_node *new_r_Anchor(ir_graph *irg) {
  ir_node *const res =
      new_ir_node(NULL, irg, NULL, op_Anchor, mode_ANY, 0, NULL);
...
void(verify_new_node)(ir_node *node) { verify_new_node_(node); }

ir_node *new_rd_Const_null(dbg_info *const dbgi, ir_graph *const irg,
                           ir_mode *const mode) {
...
ir_node *new_rd_Const_one(dbg_info *const dbgi, ir_graph *const irg,
                          ir_mode *const mode) {
