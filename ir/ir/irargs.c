...
static int firm_emit(lc_appendable_t *app, const lc_arg_occ_t *occ,
  ...
  if (X == NULL) return lc_arg_append(app, occ, "(null)", 6);
...
#define A(s) occ->flag_hash ? s " " : ""
  ...
  switch (*obj) {
      ...
      switch (occ->conversion) {
        ...
        default:
          if (is_Const(node)) {
            ...
            snprintf(buf, sizeof(buf), "%s%s %s<%s>", A("irn"),
                     get_irn_opname(node), get_mode_name(get_irn_mode(node)),
                     tv_buf);
          } else if (is_Address(node)) {
            const char *entity_name =
                get_entity_name_or_null_str(get_Address_entity(node));
            snprintf(buf, sizeof(buf), "%s%s %s[%s]", A("irn"),
                     get_irn_opname(node), get_mode_name(get_irn_mode(node)),
                     entity_name);
          } else if (is_Member(node)) {
            const char *entity_name =
                get_entity_name_or_null_str(get_Member_entity(node));
            snprintf(buf, sizeof(buf), "%s%s %s[%s]", A("irn"),
                     get_irn_opname(node), get_mode_name(get_irn_mode(node)),
                     entity_name);
          } else if (is_Cmp(node)) {
            ...
            snprintf(buf, sizeof(buf), "%s%s %s", A("irn"),
                     get_irn_opname(node), get_relation_string(relation));
          } else {
            snprintf(buf, sizeof(buf), "%s%s %s", A("irn"),
                     get_irn_opname(node), get_mode_name(get_irn_mode(node)));
          }
          snprintf(add, sizeof(add), "[%ld:%u]", get_irn_node_nr(node),
                   get_irn_idx(node));
      }
      break;
    }
    ...
    case k_ir_loop: {
      ...
      snprintf(buf, sizeof(buf), "loop[%ld:%u]", get_loop_loop_nr(loop),
               get_loop_depth(loop));
      break;
    }
  ...
  if (occ->flag_plus) strncat(buf, add, sizeof(buf) - strlen(buf) - 1);
...
lc_arg_env_t *firm_get_arg_env(void) {
  ...
  static lc_arg_handler_t const ident_handler = {firm_get_arg_type,
                                                 firm_emit_ident};
  static lc_arg_handler_t const indent_handler = {firm_get_arg_type_int,
                                                  firm_emit_indent};
  static lc_arg_handler_t const pnc_handler = {firm_get_arg_type_int,
                                               firm_emit_pnc};
  static lc_arg_handler_t const bitset_handler = {bitset_get_arg_type,
                                                  bitset_emit};
  static lc_arg_handler_t const debug_handler = {firm_get_arg_type,
                                                 firm_emit_dbg};
  ...
  static struct {
    ...
    char letter;
      ...
      {"firm:type", 't'},   {"firm:entity", 'e'}, {"firm:entity_ld", 'E'},
      {"firm:tarval", 'T'}, {"firm:irn", 'n'},    {"firm:op", 'O'},
      {"firm:irn_nr", 'N'}, {"firm:mode", 'm'},   {"firm:block", 'B'},
  };
