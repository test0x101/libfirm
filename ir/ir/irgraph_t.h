...
/**
 * @file
 * @brief    Entry point to the representation of procedure code -- internal
 * header.
...
#define set_irg_additional_properties(irg, m) \
  set_irg_additional_properties_(irg, m)
...
#define irg_is_constrained(irg, constraints) \
  irg_is_constrained_(irg, constraints)
...
static inline irg_callee_info_state get_irg_callee_info_state_(
    const ir_graph *irg) {
  return irg->callee_info_state;
}

static inline void set_irg_callee_info_state_(ir_graph *irg,
  ...
  if ((irp_state == irg_callee_info_consistent) ||
      ((irp_state == irg_callee_info_inconsistent) &&
       (s == irg_callee_info_none)))
...
static inline void *get_irg_link_(const ir_graph *irg) { return irg->link; }
...
/**
 * Allocates a new idx in the irg for the node and adds the irn to the idx ->
 * irn map.
 * @param irg The graph.
 * @param irn The node.
...
static inline void irg_kill_node(ir_graph *irg, ir_node *n) {
  ...
  if (idx + 1 == irg->last_node_idx) --irg->last_node_idx;
