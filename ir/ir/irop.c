...
ir_op *new_ir_op(unsigned code, const char *name, op_pin_state p,
  ...
  if (opcodes[code] != NULL) panic("opcode registered twice");
  opcodes[code] = res;
...
unsigned ir_get_n_opcodes(void) { return ARR_LEN(opcodes); }
...
const char *get_op_name(const ir_op *op) { return op->name; }

unsigned(get_op_code)(const ir_op *op) { return get_op_code_(op); }
...
op_pin_state(get_op_pinned)(const ir_op *op) { return get_op_pinned_(op); }

unsigned get_next_ir_opcode(void) { return next_iro++; }
...
irop_flags get_op_flags(const ir_op *op) { return (irop_flags)op->flags; }
...
static unsigned default_hash_node(const ir_node *node) {
  ...
  bool consider_block =
      is_cfop(node) || (!get_opt_global_cse() && !is_Block(node));
...
static int attrs_equal_ASM(const ir_node *a, const ir_node *b) {
  ...
  if (attr_a->text != attr_b->text) return false;

  int n_inputs = get_ASM_n_inputs(a);
  if (n_inputs != get_ASM_n_inputs(b)) return false;

  size_t const n_constraints = get_ASM_n_constraints(a);
  if (n_constraints != get_ASM_n_constraints(b)) return false;
  ...
  if (n_clobbers != get_ASM_n_clobbers(b)) return false;

  ident **cla = get_ASM_clobbers(a);
  ident **clb = get_ASM_clobbers(b);
  for (size_t i = 0; i < n_clobbers; ++i) {
    if (cla[i] != clb[i]) return false;
  }

  return except_attrs_equal(&attr_a->exc, &attr_b->exc);
}

void default_copy_attr(ir_graph *irg, ir_node const *old_node,
                       ir_node *new_node) {
  (void)irg;
...
static void block_copy_attr(ir_graph *irg, const ir_node *old_node,
  ...
  new_node->attr.block.backedge =
      new_backedge_arr(get_irg_obstack(irg), get_irn_arity(new_node));
...
static void ASM_copy_attr(ir_graph *irg, const ir_node *old_node,
  ...
  new_node->attr.assem.constraints =
      DUP_ARR_D(ir_asm_constraint, obst, old_node->attr.assem.constraints);
  new_node->attr.assem.clobbers =
      DUP_ARR_D(ident *, obst, old_node->attr.assem.clobbers);
}
...
void set_op_hash(ir_op *op, hash_func func) { op->ops.hash = func; }
...
void set_op_dump(ir_op *op, dump_node_func func) { op->ops.dump_node = func; }
