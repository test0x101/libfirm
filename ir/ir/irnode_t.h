...
/* This section MUST come first, so the inline functions get used in this
 * header. */
...
#define set_Block_block_visited(node, visit) \
...
#define ir_switch_table_get_n_entries(table) \
  ir_switch_table_get_n_entries_(table)
 ...
 */
static inline ir_op *get_irn_op_(const ir_node *node) { return node->op; }

/**
 * Sets the opcode struct of the node.
 */
static inline void set_irn_op(ir_node *node, ir_op *op) { node->op = op; }
 ...
 * Intern version for libFirm.
 */
static inline ir_mode *get_irn_mode_(const ir_node *node) { return node->mode; }
...
static inline ir_graph *get_irn_irg_(const ir_node *node) { return node->irg; }
...
static inline void *get_irn_generic_attr_(ir_node *node) { return &node->attr; }

static inline const void *get_irn_generic_attr_const_(const ir_node *node) {
  return &node->attr;
}

static inline dbg_info *get_irn_dbg_info_(const ir_node *n) { return n->dbi; }

static inline void set_irn_dbg_info_(ir_node *n, dbg_info *db) { n->dbi = db; }
...
static inline size_t ir_switch_table_get_n_entries_(
...
#define foreach_irn_in(irn, idx, pred)                             \
  for (bool pred##__b = true; pred##__b;)                          \
    for (ir_node const *const pred##__irn = (irn); pred##__b;      \
         pred##__b = false)                                        \
      for (int idx = 0, pred##__n = get_irn_arity(pred##__irn);    \
           pred##__b && idx != pred##__n; ++idx)                   \
        for (ir_node *const pred =                                 \
                 (pred##__b = false, get_irn_n(pred##__irn, idx)); \
             !pred##__b; pred##__b = true)
