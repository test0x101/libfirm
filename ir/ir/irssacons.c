...
static void (*ssa_cons_walker)(ir_graph *, irg_walk_func *, irg_walk_func *,
                               void *) = irg_block_walk_graph;
...
static void prepare_blocks(ir_node *block, void *env) {
  ...
  /* reset mature flag */
  if (block != get_irg_start_block(irg)) set_Block_matured(block, 0);
  block->attr.block.graph_arr =
      NEW_ARR_DZ(ir_node *, get_irg_obstack(irg), n_loc);
...
static void finish_block(ir_node *block, void *env) {
  ...
  if (block != get_irg_start_block(irg)) mature_immBlock(block);
}
