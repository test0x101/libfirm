...
void set_irp(ir_prog *new_irp) { irp = new_irp; }
...
static void complete_ir_prog(ir_prog *irp, const char *module_name) {
  ...
  irp->segment_types[IR_SEGMENT_GLOBAL] =
      new_type_segment(NEW_IDENT("GlobalType"), tf_none);
  irp->segment_types[IR_SEGMENT_THREAD_LOCAL] =
      new_type_segment(NEW_IDENT("ThreadLocal"), tf_none);
  irp->segment_types[IR_SEGMENT_CONSTRUCTORS] =
      new_type_segment(NEW_IDENT("Constructors"), tf_info);
  irp->segment_types[IR_SEGMENT_DESTRUCTORS] =
      new_type_segment(NEW_IDENT("Destructors"), tf_info);
  irp->segment_types[IR_SEGMENT_JCR] =
      new_type_segment(NEW_IDENT("Java Class Registry"), tf_info);

  irp->const_code_irg = new_const_code_irg();
  irp->globals_entity_usage_state = ir_entity_usage_not_computed;
}

void init_irprog_1(void) { irp = new_incomplete_ir_prog(); }

void init_irprog_2(void) { complete_ir_prog(irp, INITAL_PROG_NAME); }
...
void free_ir_prog(void) {
  ...
  foreach_irp_irg_r(i, irg) { free_ir_graph(irg); }
  ...
  for (size_t i = get_irp_n_types(); i > 0;) free_type(get_irp_type(--i));
...
ir_type *(get_glob_type)(void) { return get_glob_type_(); }

ir_type *(get_tls_type)(void) { return get_tls_type_(); }
...
size_t(get_irp_n_irgs)(void) { return get_irp_n_irgs_(); }

ir_graph *(get_irp_irg)(size_t pos) { return get_irp_irg_(pos); }

size_t get_irp_last_idx(void) { return irp->max_irg_idx; }
...
size_t(get_irp_n_types)(void) { return get_irp_n_types_(); }

ir_type *(get_irp_type)(size_t pos) { return get_irp_type_(pos); }
...
void set_irp_prog_name(ident *name) { irp->name = name; }
int irp_prog_name_is_set(void) {
  return irp->name != new_id_from_str(INITAL_PROG_NAME);
}
ident *get_irp_ident(void) { return irp->name; }
const char *get_irp_name(void) { return get_id_str(irp->name); }

ir_graph *(get_const_code_irg)(void) { return get_const_code_irg_(); }
...
ir_label_t(get_irp_next_label_nr)(void) { return get_irp_next_label_nr_(); }

void add_irp_asm(ident *asm_string) {
  ARR_APP1(ident *, irp->global_asms, asm_string);
}

size_t get_irp_n_asms(void) { return ARR_LEN(irp->global_asms); }
