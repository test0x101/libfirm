...
ir_dump_verbosity_t ir_get_dump_verbosity(void) { return verbosity; }
...
void dump_irnode_to_file(FILE *const F, const ir_node *const n) {
  ...
  switch ((ir_opcode)get_irn_opcode(n)) {
    case iro_Block: {
      ...
      if (irg_has_properties(get_irn_irg(n),
    ...
    case iro_Start: {
      ...
      for (size_t i = 0, n_params = get_method_n_params(tp); i < n_params;
           ++i) {
        ir_fprintf(F, "    param %d type: %+F\n", i,
                   get_method_param_type(tp, i));
      }
      break;
    }
    case iro_Cond:
    ...
    case iro_ASM: {
      ...
      for (size_t i = 0, n_cons = get_ASM_n_constraints(n); i < n_cons;
           sep = ",", ++i) {
        ...
        if (in_pos != -1) fprintf(F, " [in: %d]", n_ASM_max + 1 + in_pos);
        int const out_pos = c->out_pos;
        if (out_pos != -1) fprintf(F, " [out: %d]", out_pos);
      }
  ...
  if (b) {
    ...
    switch (b->state) {
      case BITINFO_INVALID:
        fputs(" (invalid)", F);
        break;
      case BITINFO_VALID: /* nothing */
        break;
      case BITINFO_IN_FLIGHT:
        fputs(" (in flight)", F);
        break;
      case BITINFO_UNSTABLE:
        fputs(" (unstable)", F);
        break;
    }
    fputc('\n', F);
...
static bool is_init_string(ir_initializer_t const *const init,
  ...
  for (size_t i = 0, n = get_initializer_compound_n_entries(init); i != n;
    ...
    if (get_initializer_kind(val) != IR_INITIALIZER_TARVAL) return false;
    ir_tarval *tv = get_initializer_tarval_value(val);

    if (!tarval_is_constant(tv)) return false;

    long v = get_tarval_long(tv);
    if (v != 0 && (v < 0x07 || 0x0D < v) && v != 0x1B &&
        (v < 0x20 || 0x80 <= v) && (v < 0xA0 || 0x100 <= v))
      return false;
  }
 ...
 * Dump initializers.
 */
static void dump_ir_initializers_to_file(
    FILE *const F, const char *const prefix,
    const ir_initializer_t *const initializer, const ir_type *const type) {
  if (need_nl) {
    fprintf(F, "\n%s    ", prefix);
    need_nl = false;
  }
      ...
      if (is_Array_type(type)) {
        ...
        if (is_init_string(initializer, type)) {
          ...
          for (size_t i = 0; i != n; ++i) {
            ir_initializer_t const *const val =
          ...
          for (size_t i = 0; i < n; ++i) {
            ...
            fprintf(F, "[%d]", (int)i);
            dump_ir_initializers_to_file(F, prefix, sub_initializer,
                                         element_type);
          }
        }
      } else {
...
static void dump_entity_to_file_prefix(FILE *const F,
  ...
  if (verbosity & dump_verbosity_onlynames) {
    ir_fprintf(F, "%sentity %s.%+F\n", prefix, get_compound_name(owner), ent);
    return;
  }
  ...
  if (verbosity & dump_verbosity_entattrs) {
    ...
    if (is_Class_type(get_entity_owner(ent))) {
      ...
      if (get_irp_inh_transitive_closure_state() !=
    ...
    if (is_Method_type(type)) fputs("(...)", F);
  ...
  if (verbosity & dump_verbosity_entconsts) {
    if (get_entity_kind(ent) == IR_ENTITY_NORMAL) {
      ...
      if (initializer) {
        ...
        need_nl = true;
        dump_ir_initializers_to_file(F, prefix, initializer,
                                     get_entity_type(ent));
        fputc('\n', F);
      }
    }
  ...
  if (verbosity & dump_verbosity_entattrs) {
    ...
    dump_entity_linkage(F, ent);
    fprintf(F, "\n%s  volatility:  %s", prefix,
            get_volatility_name(get_entity_volatility(ent)));
    fprintf(F, "\n%s  aligned:  %s", prefix,
            get_align_name(get_entity_aligned(ent)));
    fprintf(F, "\n%s  alignment:  %u", prefix, get_entity_alignment(ent));
    fprintf(F, "\n%s  ld_name: %s", prefix,
            ent->ld_name ? get_entity_ld_name(ent) : "no yet set");
    if (is_entity_compound_member(ent)) {
...
static void dump_compound_members(FILE *const F, ir_type const *const type) {
  ...
  for (size_t i = 0, n = get_compound_n_members(type); i < n; ++i) {
    ...
    if (verbosity & (is_method_entity(mem) ? dump_verbosity_methods
  ...
  switch (get_type_opcode(tp)) {
    ...
    case tpo_method:
      if (verbosity & dump_verbosity_typeattrs) {
        ...
        if (is_method_variadic(tp)) fprintf(F, "\n    ...");
        fprintf(F, "\n  properties:");
