...
static bool modes_are_equal(const ir_mode *m, const ir_mode *n) {
  if (m->sort != n->sort) return false;
  if (m->sort == irms_auxiliary || m->sort == irms_data)
    return streq(m->name, n->name);
  return m->arithmetic == n->arithmetic && m->size == n->size &&
         m->sign == n->sign && m->modulo_shift == n->modulo_shift &&
...
ir_mode *new_float_mode(const char *name, ir_mode_arithmetic arithmetic,
  ...
  if (exponent_size >= 256) panic("exponents >= 256 bits not supported");
  if (mantissa_size >= 256) panic("mantissa >= 256 bits not supported");
...
ident *(get_mode_ident)(const ir_mode *mode) { return get_mode_ident_(mode); }
...
int(mode_is_signed)(const ir_mode *mode) { return mode_is_signed_(mode); }

int(mode_is_float)(const ir_mode *mode) { return mode_is_float_(mode); }

int(mode_is_int)(const ir_mode *mode) { return mode_is_int_(mode); }

int(mode_is_reference)(const ir_mode *mode) { return mode_is_reference_(mode); }

int(mode_is_num)(const ir_mode *mode) { return mode_is_num_(mode); }

int(mode_is_data)(const ir_mode *mode) { return mode_is_data_(mode); }
  ...
  switch (get_mode_sort(sm)) {
    case irms_int_number:
      switch (get_mode_sort(lm)) {
        case irms_int_number:
          if (get_mode_arithmetic(sm) != get_mode_arithmetic(lm)) return false;
          ...
          if (mode_is_signed(sm)) {
            if (!mode_is_signed(lm)) return false;
          } else {
            if (mode_is_signed(lm)) return sm_bits < lm_bits;
          }
...
int values_in_mode(const ir_mode *sm, const ir_mode *lm) {
  ...
  if (sm == mode_b) return mode_is_int(lm) || mode_is_float(lm);
...
ir_mode *find_unsigned_mode(const ir_mode *mode) {
  ...
  if (mode->sort == irms_reference) n.sort = irms_int_number;
...
ir_mode *find_double_bits_int_mode(const ir_mode *mode) {
  ...
  ir_mode n = *mode;
  n.size = 2 * mode->size;
  if (n.modulo_shift != 0 && n.modulo_shift < n.size) n.modulo_shift = n.size;
  return find_mode(&n);
}
...
int is_reinterpret_cast(const ir_mode *src, const ir_mode *dst) {
  ...
  if (get_mode_size_bits(src) != get_mode_size_bits(dst)) return false;
  ir_mode_arithmetic ma = get_mode_arithmetic(src);
  if (ma != get_mode_arithmetic(dst)) return false;
...
size_t ir_get_n_modes(void) { return ARR_LEN(mode_list); }
