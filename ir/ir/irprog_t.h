...
/**
 * @file
 * @brief    Entry point to the representation of a whole program 0-- private
 * header.
...
static inline size_t get_irp_n_irgs_(void) { return ARR_LEN(irp->graphs); }
...
static inline size_t get_irp_n_types_(void) { return ARR_LEN(irp->types); }
...
static inline long get_irp_new_node_nr(void) { return irp->max_node_nr++; }

static inline size_t get_irp_new_irg_idx(void) { return irp->max_irg_idx++; }
...
#define foreach_irp_irg(idx, irg)                                      \
  for (bool irg##__b = true; irg##__b; irg##__b = false)               \
    for (size_t idx = 0, irg##__n = get_irp_n_irgs();                  \
         irg##__b && idx != irg##__n; ++idx)                           \
      for (ir_graph *const irg = (irg##__b = false, get_irp_irg(idx)); \
           !irg##__b; irg##__b = true)
