...
void free_ir_graph(ir_graph *irg) {
  ...
  if (irg->loc_descriptions) free(irg->loc_descriptions);
  irg->kind = k_BAD;
  free_graph(irg);
}
...
size_t get_irg_idx(const ir_graph *irg) { return irg->index; }
...
ir_node *(get_irg_start)(const ir_graph *irg) { return get_irg_start_(irg); }

void(set_irg_start)(ir_graph *irg, ir_node *node) { set_irg_start_(irg, node); }
...
ir_node *(get_irg_end)(const ir_graph *irg) { return get_irg_end_(irg); }

void(set_irg_end)(ir_graph *irg, ir_node *node) { set_irg_end_(irg, node); }

ir_node *(get_irg_frame)(const ir_graph *irg) { return get_irg_frame_(irg); }

void(set_irg_frame)(ir_graph *irg, ir_node *node) { set_irg_frame_(irg, node); }
...
ir_node *(get_irg_args)(const ir_graph *irg) { return get_irg_args_(irg); }

void(set_irg_args)(ir_graph *irg, ir_node *node) { set_irg_args_(irg, node); }

ir_node *(get_irg_no_mem)(const ir_graph *irg) { return get_irg_no_mem_(irg); }
...
int get_irg_n_locs(ir_graph *irg) { return irg->n_loc - 1; }
...
void(set_irg_link)(ir_graph *irg, void *thing) { set_irg_link_(irg, thing); }

void *(get_irg_link)(const ir_graph *irg) { return get_irg_link_(irg); }
...
ir_visited_t get_max_irg_visited(void) { return max_irg_visited; }

void set_max_irg_visited(int val) { max_irg_visited = val; }

ir_visited_t inc_max_irg_visited(void) {
#ifndef NDEBUG
  foreach_irp_irg(i, irg) { assert(max_irg_visited >= get_irg_visited(irg)); }
#endif
...
void(inc_irg_block_visited)(ir_graph *irg) { inc_irg_block_visited_(irg); }
...
unsigned get_irg_last_idx(const ir_graph *irg) { return irg->last_node_idx; }
...
void confirm_irg_properties(ir_graph *irg, ir_graph_properties_t props) {
  ...
  if (!(props & IR_GRAPH_PROPERTY_CONSISTENT_OUT_EDGES)) edges_deactivate(irg);
