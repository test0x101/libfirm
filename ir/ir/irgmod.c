...
void exchange(ir_node *old, ir_node *nw) {
  ...
  if (is_Phi(old) && get_Phi_loop(old) && !(is_Phi(nw) && get_Phi_loop(nw)) &&
    ...
    foreach_irn_in(end, i, kept) { assert(kept != old); }
  }
#endif
  ...
  if (edges_activated(irg)) {
    ...
    if (is_irn_dynamic(old)) DEL_ARR_F(old->in);
...
void collect_new_start_block_node(ir_node *node) {
  ...
  /* alreayd in list? */
  if (get_irn_link(node) != NULL) return;
...
void collect_phiprojs_and_start_block_nodes(ir_graph *irg) {
  assert((ir_resources_reserved(irg) &
...
static void move_node(ir_node *node, ir_node *to_bl) {
  ...
  if (get_irn_mode(node) != mode_T || is_Bad(node)) return;
...
static void move_edges(ir_node *node, ir_node *from_bl, ir_node *to_bl) {
  ...
  /* recursion ... */
  foreach_irn_in(node, i, pred) {
    if (get_nodes_block(pred) == from_bl) move_edges(pred, from_bl, to_bl);
  }
}
  ...
  switch (get_irn_opcode(n)) {
    ...
    case iro_Conv:
      return new_rd_Conv(dbg, block,
                         duplicate_subgraph(dbg, get_Conv_op(n), block), mode);
    case iro_Minus:
