...
void ir_arch_lower(ir_settings_arch_dep_t const *const new_settings) {
  ...
  foreach_irp_irg(i, irg) { optimize_graph_df(irg); }
}

/** check, whether a mode allows a Mulh instruction. */
static bool allow_Mulh(ir_mode *mode) {
  if (get_mode_size_bits(mode) > settings.max_bits_for_mulh) return false;
 ...
 * instructions.
 */
static int default_evaluate(insn_kind kind, const ir_mode *mode,
                            ir_tarval *tv) {
  (void)mode;
  (void)tv;
  if (kind == MUL) return 13;
  return 1;
}
 ...
 */
static ir_tarval *condensed_to_value(mul_env *env, const unsigned char *R,
                                     int r) {
  ir_tarval *tv = get_mode_one(env->mode);
  ir_tarval *res = NULL;
  for (int i = 0; i < r; ++i) {
    int j = R[i];
    if (j != 0) tv = tarval_shl_unsigned(tv, j);
...
static instruction *decompose_mul(mul_env *env, unsigned char *R, int r,
                                  ir_tarval *N) {
  if (r <= 2) return decompose_simple_cases(env, R, r);
  ...
  if (N == NULL) N = condensed_to_value(env, R, r);
...
static ir_node *build_graph(mul_env *env, instruction *inst) {
  if (inst->irn != NULL) return inst->irn;
...
static int evaluate_insn(mul_env *env, instruction *inst) {
  ...
  switch (inst->kind) {
    ...
    case SHIFT:
      if (inst->shift_count > settings.highest_shift_amount) env->fail = true;
      if (env->n_shift <= 0)
        env->fail = true;
      else
        --env->n_shift;
 ...
 */
static ir_node *do_decomposition(ir_node *irn, ir_node *operand,
                                 ir_tarval *tv) {
  mul_env env;
  ...
  env.evaluate =
      settings.evaluate != NULL ? settings.evaluate : default_evaluate;
  ...
  if (evaluate_insn(&env, inst) <= mul_costs && !env.fail) {
    ...
    if (env.shf_mode == NULL) env.shf_mode = mode_Iu;
...
ir_node *arch_dep_replace_mul_with_shifts(ir_node *irn) {
  ...
  if (!settings.replace_muls) return irn;

  assert(is_Mul(irn));
  ir_mode *mode = get_irn_mode(irn);
  if (!mode_is_int(mode)) return irn;
  ...
  if (tv == get_mode_null(mode)) return irn;
...
static struct magicu_info compute_unsigned_magic_info(ir_tarval *divisor,
  ...
  for (ir_tarval *tmp = divisor; CMP(tmp, ZERO(mode)) & ir_relation_greater;
       tmp = SHR(tmp, 1))
    ceil_log_2_D++;
  ...
  for (;; ++exponent) {
    ...
    if ((exponent + extra_shift >= ceil_log_2_D) ||
        ...
        (CMP(SUB(divisor, remainder), SHL(ONE(mode), exponent + extra_shift)) &
...
static ir_node *replace_div_by_mulh(ir_node *div, ir_tarval *tv) {
  ...
  if (is_Bad(n) || is_Bad(block)) return div;
...
ir_node *arch_dep_replace_div_by_const(ir_node *irn) {
  /* If the architecture dependent optimizations were not initialized
          or this optimization was not enabled. */
  if (!settings.replace_divs) return irn;
  if (!is_Div(irn)) return irn;

  ir_node *c = get_Div_right(irn);
  if (!is_Const(c)) return irn;

  /* check for division by zero */
  ir_tarval *tv = get_Const_tarval(c);
  if (tarval_is_null(tv)) return irn;
  ...
  if (k > 0) { /* division by 2^k or -2^k */
    ...
    if (mode_is_signed(mode)) {
      ...
      /* create the correction code for signed values only if there might be a
       * remainder */
    ...
    /* other constant */
    if (allow_Mulh(mode)) res = replace_div_by_mulh(irn, tv);
...
ir_node *arch_dep_replace_mod_by_const(ir_node *irn) {
  /* If the architecture dependent optimizations were not initialized
     or this optimization was not enabled. */
  if (!settings.replace_mods) return irn;
  if (!is_Mod(irn)) return irn;
  ...
  if (k == 0) {
    ...
    if (mode_is_signed(mode)) {
      ...
      ir_tarval *k_val = tarval_shl_unsigned(get_mode_all_one(mode), k);
      k_node = new_r_Const(irg, k_val);
      curr = new_rd_And(dbg, block, curr, k_node);
      res = new_rd_Sub(dbg, block, left, curr);
    } else { /* unsigned case */
      ir_tarval *k_val = tarval_shr_unsigned(get_mode_all_one(mode),
