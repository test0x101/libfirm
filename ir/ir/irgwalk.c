...
static void irg_walk_2_pre(ir_node *node, irg_walk_func *pre, void *env) {
  ...
  if (!is_Block(node)) {
    ...
    if (pred->visited < visited) irg_walk_2_pre(pred, pre, env);
  }
...
static void irg_walk_2_post(ir_node *node, irg_walk_func *post, void *env) {
  ...
  if (!is_Block(node)) {
    ir_node *pred = get_nodes_block(node);
    if (pred->visited < visited) irg_walk_2_post(pred, post, env);
  }
  foreach_irn_in_r(node, i, pred) {
    if (pred->visited < visited) irg_walk_2_post(pred, post, env);
  }
...
static void irg_walk_2_both(ir_node *node, irg_walk_func *pre,
  ...
  if (!is_Block(node)) {
    ir_node *pred = get_nodes_block(node);
    if (pred->visited < visited) irg_walk_2_both(pred, pre, post, env);
  }
  foreach_irn_in_r(node, i, pred) {
    if (pred->visited < visited) irg_walk_2_both(pred, pre, post, env);
  }
...
void irg_walk_2(ir_node *node, irg_walk_func *pre, irg_walk_func *post,
  ...
  if (post == NULL)
    irg_walk_2_pre(node, pre, env);
  else if (pre == NULL)
    irg_walk_2_post(node, post, env);
  else
    irg_walk_2_both(node, pre, post, env);
}
...
void all_irg_walk(irg_walk_func *pre, irg_walk_func *post, void *env) {
  foreach_irp_irg(i, irg) { irg_walk_graph(irg, pre, post, env); }
}

/**
 * specialized version of irg_walk_in_or_dep_2, called if only pre callback
 * exists
 */
static void irg_walk_in_or_dep_2_pre(ir_node *node, irg_walk_func *pre,
  ...
  if (!is_Block(node)) {
    ir_node *pred = get_nodes_block(node);
    if (pred->visited < visited) irg_walk_in_or_dep_2_pre(pred, pre, env);
  }
  foreach_irn_in_r(node, i, pred) {
    if (pred->visited < visited) irg_walk_in_or_dep_2_pre(pred, pre, env);
  }
...
/**
 * specialized version of irg_walk_in_or_dep_2, called if only post callback
 * exists
 */
static void irg_walk_in_or_dep_2_post(ir_node *node, irg_walk_func *post,
  ...
  if (!is_Block(node)) {
    ir_node *pred = get_nodes_block(node);
    if (pred->visited < visited) irg_walk_in_or_dep_2_post(pred, post, env);
  }
  foreach_irn_in_r(node, i, pred) {
    if (pred->visited < visited) irg_walk_in_or_dep_2_post(pred, post, env);
  }
...
/**
 * specialized version of irg_walk_in_or_dep_2, called if pre and post callbacks
 * exist
 */
...
static void irg_walk_in_or_dep_2(ir_node *node, irg_walk_func *pre,
  ...
  if (post == NULL)
    irg_walk_in_or_dep_2_pre(node, pre, env);
  else if (pre == NULL)
    irg_walk_in_or_dep_2_post(node, post, env);
  else
    irg_walk_in_or_dep_2_both(node, pre, post, env);
}
...
static void walk_topo_helper(ir_node *irn, ir_nodeset_t *walker_called,
                             irg_walk_func *walker, void *env) {
  if (irn_visited(irn)) {
  ...
  if (is_loop_breaker) mark_irn_visited(irn);
...
static void irg_block_walk_2(ir_node *node, irg_walk_func *pre,
                             irg_walk_func *post, void *env) {
  if (Block_block_visited(node)) return;
...
static void walk_entity(ir_entity *ent, void *env) {
  ...
  if (get_entity_kind(ent) == IR_ENTITY_NORMAL) {
    ...
    if (init) walk_initializer(init, my_env);
  }
}
