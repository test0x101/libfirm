...
static const char *get_segment_name(ir_segment_t segment) {
  switch (segment) {
    case IR_SEGMENT_GLOBAL:
      return "global";
    case IR_SEGMENT_THREAD_LOCAL:
      return "thread_local";
    case IR_SEGMENT_CONSTRUCTORS:
      return "constructors";
    case IR_SEGMENT_DESTRUCTORS:
      return "destructors";
    case IR_SEGMENT_JCR:
      return "jcr";
  }
...
static const char *get_mode_arithmetic_name(ir_mode_arithmetic arithmetic) {
  switch (arithmetic) {
    ...
    case irma_x86_extended_float:
      return "x86_extended_float";
  }
  panic("invalid mode_arithmetic");
}

/** Returns the according symbol value for the given string and tag, or SYMERROR
 * if none was found. */
...
static void write_list_begin(write_env_t *env) { fputs("[", env->file); }

static void write_list_end(write_env_t *env) { fputs("] ", env->file); }

static void write_scope_begin(write_env_t *env) { fputs("{\n", env->file); }

static void write_scope_end(write_env_t *env) { fputs("}\n\n", env->file); }
...
static void write_mode_arithmetic(write_env_t *env,
...
static bool is_internal_mode(ir_mode *mode) {
  return !mode_is_int(mode) && !mode_is_reference(mode) && !mode_is_float(mode);
}
...
static void write_type_primitive(write_env_t *env, ir_type *tp) {
  ...
  if (is_internal_mode(mode) && !is_default_mode(mode)) return;
...
static void write_type(write_env_t *env, ir_type *tp) {
  ...
  switch (get_type_opcode(tp)) {
    ...
    case tpo_primitive:
      write_type_primitive(env, tp);
      return;
    case tpo_method:
      write_type_method(env, tp);
      return;
    case tpo_pointer:
      write_type_pointer(env, tp);
      return;
    case tpo_array:
      write_type_array(env, tp);
      return;
  }
...
static void write_entity(write_env_t *env, ir_entity *ent) {
  ...
  switch ((ir_entity_kind)ent->kind) {
    case IR_ENTITY_ALIAS:
      write_symbol(env, "alias");
      break;
    case IR_ENTITY_NORMAL:
      write_symbol(env, "entity");
      break;
    case IR_ENTITY_METHOD:
      write_symbol(env, "method");
      break;
    case IR_ENTITY_LABEL:
      write_symbol(env, "label");
      break;
    case IR_ENTITY_COMPOUND_MEMBER:
      write_symbol(env, "compound_member");
      break;
    case IR_ENTITY_PARAMETER:
      write_symbol(env, "parameter");
      break;
  ...
  if (linkage & IR_LINKAGE_CONSTANT) write_symbol(env, "constant");
  if (linkage & IR_LINKAGE_WEAK) write_symbol(env, "weak");
  if (linkage & IR_LINKAGE_GARBAGE_COLLECT)
    write_symbol(env, "garbage_collect");
  if (linkage & IR_LINKAGE_MERGE) write_symbol(env, "merge");
  if (linkage & IR_LINKAGE_HIDDEN_USER) write_symbol(env, "hidden_user");
  write_list_end(env);

  write_type_ref(env, type);
  if (ent->kind != IR_ENTITY_LABEL) write_type_ref(env, owner);
...
static void write_node(const ir_node *node, write_env_t *env) {
  ...
  fputc('\t', env->file);
  if (func == NULL) panic("no write_node_func for %+F", node);
  func(env, node);
  fputc('\n', env->file);
}
...
static void write_preds(ir_node *node, write_env_t *env) {
  foreach_irn_in(node, i, pred) { write_node_recursive(pred, env); }
}
...
static void write_modes(write_env_t *env) {
  ...
  for (size_t i = 0, n_modes = ir_get_n_modes(); i < n_modes; i++) {
    ...
    if (is_internal_mode(mode)) continue;
    fputc('\t', env->file);
    write_mode(env, mode);
    fputc('\n', env->file);
  }
...
void ir_export_file(FILE *file) {
  ...
  foreach_irp_irg(i, irg) { write_irg(env, irg); }
...
int read_int(read_env_t *env) { return (int)read_long(env); }

unsigned read_unsigned(read_env_t *env) { return (unsigned)read_long(env); }
...
static void *get_id(read_env_t *env, long id) {
  ...
  id_entry *entry =
      set_find(id_entry, env->idset, &key, sizeof(key), (unsigned)id);
...
static ir_entity *create_error_entity(void) {
  ir_entity *res =
      new_entity(get_glob_type(), new_id_from_str("error"), get_unknown_type());
  return res;
}
...
static const char *get_typetag_name(typetag_t typetag) {
  switch (typetag) {
    case tt_align:
      return "align";
    case tt_builtin_kind:
      return "builtin kind";
    case tt_cond_jmp_predicate:
      return "cond_jmp_predicate";
    case tt_initializer:
    ...
    case tt_loop:
      return "loop";
    case tt_mode_arithmetic:
      return "mode_arithmetic";
    case tt_pin_state:
      return "pin state";
    case tt_segment:
      return "segment";
...
bool read_throws(read_env_t *env) { return (bool)read_enum(env, tt_throws); }

bool read_loop(read_env_t *env) { return (bool)read_enum(env, tt_loop); }
...
static bool type_matches(const ir_type *t, tp_opcode op, unsigned size,
  ...
  return t->opcode == op && t->size == size && t->align == align &&
         t->flags == flags;
}
...
static void read_type(read_env_t *env) {
  ...
  if (maybe_initial_type) {
    ...
    if (candidate &&
        type_matches(candidate, opcode, size, align, state, flags)) {
      type = candidate;
      skip_to(env, '\n');
      goto extend_env;
    } else {
...
finish_type:
  if (align > 0) set_type_alignment(type, align);
  type->flags = flags;

  if (state == layout_fixed) ARR_APP1(ir_type *, env->fixedtypes, type);
...
static void read_entity(read_env_t *env, ir_entity_kind kind) {
  ...
  if (kind != IR_ENTITY_LABEL) owner = read_type_ref(env);
  ...
  switch (kind) {
    ...
    case IR_ENTITY_NORMAL:
      ...
      if (ld_name != NULL) set_entity_ld_ident(entity, ld_name);
      const char *str = read_word(env);
      if (streq(str, "initializer")) {
        ir_initializer_t *initializer = read_initializer(env);
        if (initializer != NULL) set_entity_initializer(entity, initializer);
    ...
    case IR_ENTITY_COMPOUND_MEMBER:
      ...
      for (int i = 0, n = get_compound_n_members(owner); i < n; ++i) {
        ...
        if (e->type == type && streq_null(e->name, name) &&
            streq_null(e->ld_name, ld_name) && get_entity_offset(e) == offset &&
...
static ir_node *read_ASM(read_env_t *env) {
  ...
  if (!read_pinned(env)) flags |= cons_floats;
  if (read_throws(env)) flags |= cons_throws_exception;

  int n_in = read_preds(env);
  ir_node **in = (ir_node **)obstack_finish(&env->preds_obst);

  ir_node *newnode =
      new_r_ASM(block, mem, n_in, in, asm_text, ARR_LEN(constraints),
                constraints, ARR_LEN(clobbers), clobbers, flags);
...
static ir_node *read_Phi(read_env_t *env) {
  ...
  ir_node *res =
      loop ? new_r_Phi_loop(block, 0, NULL) : new_r_Phi(block, 0, NULL, mode);
...
static void read_graph(read_env_t *env, ir_graph *irg) {
  ...
  for (size_t i = 0, n = ARR_LEN(env->delayed_preds); i < n; ++i) {
    ...
    if (is_Anchor(dp->node)) {
      foreach_irn_in(get_irg_anchor(irg), a, old) { exchange(old, ins[a]); }
    }
  next_delayed_pred:;
  }
  ...
  while (true) {
    ...
    switch (kwkind) {
      ...
      case kw_float_mode: {
        ...
        new_float_mode(name, arith, exponent_size, mantissa_size, overflow);
        break;
      }
...
int ir_import_file(FILE *input, const char *inputname) {
  ...
  for (size_t i = 0, n = ARR_LEN(env->delayed_initializers); i < n; ++i) {
    ...
    if (node == NULL) {
      parse_error(env,
                  "node %ld mentioned in an initializer was never defined\n",
                  di->node_nr);
      continue;
    }
