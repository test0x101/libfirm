...
static bool check_input_mode(const ir_node *n, int input, const char *inputname,
  ...
  if (in_mode != mode) {
    ...
    warn(n, "expected mode %+F for input '%s' but found %+F (%+F)", mode,
...
static bool check_input_func(const ir_node *n, int input, const char *inputname,
  ...
  if (!func(mode)) {
    ...
    warn(n, "expected %s mode for input '%s' but found %+F (%+F)", modedescr,
         inputname, mode, in);
    return false;
  }
  return true;
...
static bool check_mode_same_input(const ir_node *n, int input,
  ...
  if (mode != in_mode) {
    ...
    warn(n, "mode of input '%s' different from output mode %+F", inputname,
         mode);
    return false;
  }
  return true;
...
static bool invalid_proj(const ir_node *proj) {
  warn(proj, "invalid proj number %u for predecessor %+F", get_Proj_num(proj),
       get_Proj_pred(proj));
  return false;
}
...
static int verify_node_Proj_Start(const ir_node *p) {
  switch ((pn_Start)get_Proj_num(p)) {
    case pn_Start_M:
      return check_mode(p, mode_M);
    case pn_Start_P_frame_base:
      return check_mode_func(p, mode_is_reference, "reference");
    case pn_Start_T_args:
      return check_mode(p, mode_T);
  }
...
static int verify_node_Proj_Div(const ir_node *p) {
  switch ((pn_Div)get_Proj_num(p)) {
    ...
    case pn_Div_X_except:
      return check_mode(p, mode_X);
    case pn_Div_res:
      return check_mode(p, get_Div_resmode(get_Proj_pred(p)));
  }
  return invalid_proj(p);
}
...
static int verify_node_Proj_Mod(const ir_node *p) {
  switch ((pn_Mod)get_Proj_num(p)) {
    ...
    case pn_Mod_X_except:
      return check_mode(p, mode_X);
    case pn_Mod_res:
      return check_mode(p, get_Mod_resmode(get_Proj_pred(p)));
  }
  return invalid_proj(p);
}
...
static int verify_node_Proj_Load(const ir_node *p) {
  switch ((pn_Load)get_Proj_num(p)) {
    ...
    case pn_Load_X_except:
      return check_mode(p, mode_X);
    case pn_Load_res:
      return check_mode(p, get_Load_mode(get_Proj_pred(p)));
  }
  return invalid_proj(p);
}
...
static int verify_node_Proj_Proj_Start(const ir_node *p) {
  ...
  if (!is_Method_type(mt)) return true;
  unsigned pn = get_Proj_num(p);
...
static int verify_node_Proj_Proj(const ir_node *p) {
  ...
  switch (get_irn_opcode(predpred)) {
    case iro_Start:
      return verify_node_Proj_Proj_Start(p);
    case iro_Call:
      return verify_node_Proj_Proj_Call(p);
    default:
      return true;
  }
}
...
static bool verify_node_Proj_fragile(const ir_node *node) {
  ...
  if (!is_fragile_op(pred)) return true;

  if (!is_x_except_Proj(node) && !is_x_regular_Proj(node)) return true;
  int throws_exception = ir_throws_exception(pred);
  if (!throws_exception) {
    warn(node,
         "exception Proj on %+F which is not marked as throwing exceptions",
         pred);
    return false;
  }
  return true;
...
static int verify_node_Proj(const ir_node *p) {
  ...
  if (op->ops.verify_proj_node) fine &= op->ops.verify_proj_node(p);
...
static int verify_node_Start(const ir_node *n) { return check_mode(n, mode_T); }
...
static int verify_node_End(const ir_node *n) {
  ...
  if (!irg_is_constrained(irg, IR_GRAPH_CONSTRAINT_BACKEND)) {
    foreach_irn_in(n, i, kept) {
      ...
      if (is_Block(kept)) continue;
      if (is_Phi(kept) && get_Phi_loop(kept)) continue;
      /* noreturn calls are currently kept with keep-alive edges */
      if (is_Call(kept)) continue;
      if (is_Bad(kept)) continue;
      warn(n,
           "keep-alive edge only allowed on Block, PhiLoop and Call node, "
           "found %+F",
           kept);
      fine = false;
    }
...
static int verify_node_Jmp(const ir_node *n) { return check_mode(n, mode_X); }

static int verify_node_IJmp(const ir_node *n) {
  bool fine = check_input_func(n, n_IJmp_target, "target", mode_is_reference,
                               "reference");
...
static bool verify_switch_table(const ir_node *n) {
  ...
  for (size_t e = 0, n_entries = ir_switch_table_get_n_entries(table);
    ...
    if (mode_is_int(mode) && (get_tarval_mode(entry->min) != mode ||
      ...
      warn(n, "switch table entry %zu has wrong mode for min or max value", e);
      fine = false;
...
static int verify_node_Switch(const ir_node *n) {
  bool fine = check_mode(n, mode_T);
  fine &=
      check_input_func(n, n_Switch_selector, "selector", mode_is_int, "int");
...
static int verify_node_Return(const ir_node *n) {
  ...
  if ((size_t)get_Return_n_ress(n) != get_method_n_ress(mt)) {
    warn(n,
         "number of inputs does not match method type (%zu inputs, %zu "
         "declared)",
    ...
    for (int i = get_Return_n_ress(n); i-- > 0;) {
      ...
      if (is_atomic_type(expected)) {
        ...
        fine &= check_input_func(n, n_Return_max + 1 + i, NULL,
...
static int verify_node_Raise(const ir_node *n) {
  bool fine = check_mode(n, mode_X);
  fine &= check_input_func(n, n_Raise_exo_ptr, "exo_ptr", mode_is_reference,
                           "reference");
...
static int verify_node_Const(const ir_node *n) {
  ...
  if (fine && tv_mode != mode) {
    warn(n, "tarval mode (%+F) different from Const mode (%+F)", tv_mode, mode);
    fine = false;
  }
  return fine;
}
...
static int verify_node_Member(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Member_ptr, "ptr", mode_is_reference, "reference");
...
static int verify_node_Call(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Call_ptr, "ptr", mode_is_reference, "reference");
...
static int verify_node_Cmp(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Cmp_left, "left", mode_is_data_not_b, "data_not_b");
...
static int verify_node_Shl(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Shl_right, "right", mode_is_uint, "unsigned int");
  return fine;
}
...
static int verify_node_Shr(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Shr_right, "right", mode_is_uint, "unsigned int");
  return fine;
}
...
static int verify_node_Shrs(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Shrs_right, "right", mode_is_uint, "unsigned int");
  return fine;
}
...
static int verify_node_Conv(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Conv_op, "op", mode_is_data_not_b, "data_not_b");
  return fine;
}
...
static int verify_node_Bitcast(const ir_node *n) {
  bool fine = check_mode_func(n, mode_is_data_not_b, "data_not_b");
  fine &=
      check_input_func(n, n_Bitcast_op, "op", mode_is_data_not_b, "data_not_b");
  ...
  if (get_mode_size_bits(src_mode) != get_mode_size_bits(dst_mode) ||
    ...
    warn(n,
         "bitcast only allowed for modes with same size and different "
         "arithmetic");
    fine = false;
  }
  return fine;
...
static int verify_node_Phi(const ir_node *n) {
  ...
  if (get_Phi_loop(n)) check_mode(n, mode_M);
  return fine;
}
...
static int verify_node_Load(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Load_ptr, "ptr", mode_is_reference, "reference");
...
static int verify_node_Store(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Store_ptr, "ptr", mode_is_reference, "reference");
  fine &= check_input_func(n, n_Store_value, "value", mode_is_data_not_b,
                           "data_not_b");
  return fine;
}
...
static int verify_node_Alloc(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Alloc_size, "size", mode_is_uint, "unsigned int");
  return fine;
}
...
static int verify_node_Free(const ir_node *n) {
  ...
  fine &=
      check_input_func(n, n_Free_ptr, "ptr", mode_is_reference, "reference");
  return fine;
}
...
static int verify_node_CopyB(const ir_node *n) {
  bool fine = check_mode(n, mode_M);
  fine &=
      check_input_func(n, n_CopyB_src, "src", mode_is_reference, "reference");
  fine &=
      check_input_func(n, n_CopyB_dst, "dst", mode_is_reference, "reference");
...
static bool check_dominance_for_node(const ir_node *use) {
  ...
  if (is_Block(use) || is_End(use) || is_Anchor(use)) return true;
  ...
  foreach_irn_in_r(use, i, def) {
    ...
    if (!is_Block(def_bl) || get_Block_dom_depth(def_bl) == -1) continue;

    ir_node *use_bl = bl;
    if (is_Phi(use)) {
      use_bl = get_Block_cfgpred_block(bl, i);
      if (use_bl == NULL) continue;
    }
    if (get_Block_dom_depth(use_bl) == -1) continue;
...
int irn_verify(const ir_node *const n) {
  ...
  if (op->ops.verify_node != NULL) fine &= op->ops.verify_node(n);
...
static int check_block_cfg(const ir_node *block, check_cfg_env_t *env) {
  ...
  for (int i = 0, n_cfgpreds = get_Block_n_cfgpreds(block); i < n_cfgpreds;
    ...
    if (former_branch != NULL && former_branch != branch) {
      warn(branch_block, "multiple branching nodes (%+F,%+F) in block", branch,
           former_branch);
      fine = false;
      continue;
    }
    ...
    if (is_Cond(branch)) {
      ...
      if (pn == pn_Cond_true) ir_nodeset_insert(&env->true_projs, branch);
      if (pn == pn_Cond_false) ir_nodeset_insert(&env->false_projs, branch);
    } else if (is_Switch(branch)) {
      unsigned pn = get_Proj_num(branch_proj);
      if (pn == pn_Switch_default) ir_nodeset_insert(&env->true_projs, branch);
    }
  }
  return fine;
}
...
static bool check_has_memory(ir_graph *irg) {
  ...
  foreach_irn_in(end_block, i, ret) {
    if (!is_Bad(ret)) return true;
  }
  /* we can't reliably detect backend calls yet */
  if (irg_is_constrained(irg, IR_GRAPH_CONSTRAINT_BACKEND)) return true;
  ir_node *end = get_irg_end(irg);
  foreach_irn_in(end, i, kept) {
    if (get_irn_mode(kept) == mode_M || is_Call(kept)) return true;
  }
...
static void check_simple_properties(ir_node *node, void *env) {
  ...
  if (is_Block(node)) {
    if (irg_has_properties(irg, IR_GRAPH_PROPERTY_CONSISTENT_DOMINANCE |
      ...
      warn(node,
           "IR_GRAPH_PROPERTY_NO_UNREACHABLE_CODE + "
           "IR_GRAPH_PROPERTY_CONSISTENT_DOMINANCE is set, but block has depth "
           "-1");
      properties_fine = false;
    }
    if (irg_has_properties(irg, IR_GRAPH_PROPERTY_CONSISTENT_POSTDOMINANCE |
      ...
      warn(node,
           "IR_GRAPH_PROPERTY_NO_UNREACHABLE_CODE + "
           "IR_GRAPH_PROPERTY_CONSISTENT_POSTDOMINANCE is set, but block has "
           "depth -1");
      properties_fine = false;
    }
    ...
    if (get_irn_arity(node) > 1 &&
      ...
      foreach_irn_in(node, p, pred) {
        ...
        if (has_multiple_X_succs(skipped)) {
          warn(node,
               "IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES set, but edge %+F->%+F is "
               "critical",
               node, skipped);
          properties_fine = false;
        }
  ...
  if (is_Return(node)) {
    ...
    if (n_returns > 1 &&
      ...
      warn(node,
           "IR_GRAPH_PROPERTY_ONE_RETURN set, but multiple return nodes found");
      properties_fine = false;
    }
  }
}
...
static void assure_in_set(ir_node *node, void *env) {
  ...
  if (!ir_nodeset_contains(nodes, node)) {
    warn(node,
         "IR_GRAPH_PROPERTY_CONSISTENT_OUT_EDGES set, but node cannot be found "
         "with normal edges");
    properties_fine = false;
  }
...
static void check_consistent_out_edges(ir_graph *irg) {
  if (!irg_has_properties(irg, IR_GRAPH_PROPERTY_CONSISTENT_OUT_EDGES)) return;

  if (!edges_verify(irg)) properties_fine = false;
