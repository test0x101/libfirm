 ...
 */
static block_entry_t *block_find_entry(ir_node *block,
                                       blk_collect_data_t *ctx) {
  block_entry_t key;
  key.block = block;
  block_entry_t *elem =
 ...
 */
static void traverse_pre(blk_collect_data_t *blks, irg_walk_func *pre,
                         void *env) {
 ...
 */
static void traverse_dom_blocks_top_down(ir_graph *irg,
                                         blk_collect_data_t *blks,
                                         irg_walk_func *pre,
                                         irg_walk_func *post, void *env) {
  dom_traversal_t ctx;
...
static void collect_walk(ir_node *node, blk_collect_data_t *env) {
  ...
  if (is_Block(node)) {
    ...
    if (!irn_visited(block)) collect_walk(block, env);
 ...
 */
static void do_irg_walk_blk(
    ir_graph *irg, irg_walk_func *pre, irg_walk_func *post, void *env,
    void (*traverse)(ir_graph *irg, blk_collect_data_t *blks,
