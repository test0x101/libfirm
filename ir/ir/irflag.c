...
#define FLAG(name, value, def)    \
  void set_opt_##name(int flag) { \
    if (flag)                     \
      libFIRM_opt |= irf_##name;  \
    else                          \
      libFIRM_opt &= ~irf_##name; \
  }                               \
  int(get_opt_##name)(void) { return (libFIRM_opt & irf_##name) != 0; }
...
void set_optimize(int value) { set_opt_optimize(value); }

int(get_optimize)(void) { return get_opt_optimize(); }
...
void all_optimizations_off(void) { libFIRM_opt = 0; }

static const lc_opt_table_entry_t firm_flags[] = {
#define FLAG(name, val, def) \
  LC_OPT_ENT_BIT(#name, #name, &libFIRM_opt, (1 << val)),
#include "irflag_t.def"
#undef FLAG
    LC_OPT_LAST};
