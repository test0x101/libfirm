...
#define InitData(self, entry, key) \
  do {                             \
    (entry).value = (key);         \
...
#define EntrySetDeleted(entry)          \
  do {                                  \
    (entry).data.value = (ir_node *)-1; \
    list_del(&(entry).data.list);       \
  } while (0)
...
#define ADDITIONAL_INIT             \
  INIT_LIST_HEAD(&self->elem_list); \
  INIT_LIST_HEAD(&self->all_iters);
#define ADDITIONAL_TERM             \
  INIT_LIST_HEAD(&self->elem_list); \
  INIT_LIST_HEAD(&self->all_iters);
...
int ir_valueset_replace(ir_valueset_t *valueset, ir_node *value,
                        ir_node *expr) {
  int res = 0;
  ir_valueset_entry_t *entry = ir_valueset_insert_(valueset, value);
...
void *ir_valueset_lookup(const ir_valueset_t *valueset, const ir_node *value) {
  ...
  if (entry != NULL) return entry->expr;
  return NULL;
}
...
void ir_valueset_remove_iterator(ir_valueset_t *valueset,
  ...
  ir_valueset_entry_t *rem =
      list_entry(iterator->iter->prev, ir_valueset_entry_t, list);
