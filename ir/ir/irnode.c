...
const char *get_relation_string(ir_relation relation) {
  static char const *const relation_names[] = {"false",
                                               "equal",
                                               "less",
                                               "less_equal",
                                               "greater",
...
ir_node *new_ir_node(dbg_info *db, ir_graph *irg, ir_node *block, ir_op *op,
  ...
  ir_node *const res =
      (ir_node *)OALLOCNZ(get_irg_obstack(irg), char, node_size);
  ...
  if (block != NULL) edges_notify_edge(res, -1, block, NULL, irg);
...
ir_node *new_similar_node(ir_node *const old, ir_node *const block,
                          ir_node **const in) {
...
int(get_irn_arity)(const ir_node *node) { return get_irn_arity_(node); }

void set_irn_in(ir_node *const node, int const arity,
...
ir_node *(get_irn_n)(const ir_node *node, int n) { return get_irn_n_(node, n); }
...
void remove_Sync_n(ir_node *n, int i) { remove_irn_n(n, i); }

ir_mode *(get_irn_mode)(const ir_node *node) { return get_irn_mode_(node); }

void(set_irn_mode)(ir_node *node, ir_mode *mode) { set_irn_mode_(node, mode); }

ir_op *(get_irn_op)(const ir_node *node) { return get_irn_op_(node); }

unsigned(get_irn_opcode)(const ir_node *node) { return get_irn_opcode_(node); }
...
void(mark_irn_visited)(ir_node *node) { mark_irn_visited_(node); }

int(irn_visited)(const ir_node *node) { return irn_visited_(node); }

int(irn_visited_else_mark)(ir_node *node) {
  return irn_visited_else_mark_(node);
}

void(set_irn_link)(ir_node *node, void *link) { set_irn_link_(node, link); }

void *(get_irn_link)(const ir_node *node) { return get_irn_link_(node); }

int(get_irn_pinned)(const ir_node *node) { return get_irn_pinned_(node); }
...
void set_End_keepalives(ir_node *end, int n, ir_node *in[]) {
  ...
  for (int i = 0; i < n; ++i) {
    ...
    edges_notify_edge(end, END_KEEPALIVE_OFFSET + i,
                      end->in[1 + END_KEEPALIVE_OFFSET + i], NULL, irg);
  }
...
void remove_End_n(ir_node *n, int idx) { remove_irn_n(n, idx); }
...
void remove_keep_alive(const ir_node *irn) {
  ...
  for (int i = get_End_n_keepalives(end);;) {
    ...
    /* find irn */
    if (old_ka == irn)
      set_irn_n(end, END_KEEPALIVE_OFFSET + i,
                new_r_Bad(irg, get_irn_mode(irn)));
  }
}
...
int(is_Const_null)(const ir_node *node) { return is_Const_null_(node); }

int(is_Const_one)(const ir_node *node) { return is_Const_one_(node); }

int(is_Const_all_one)(const ir_node *node) { return is_Const_all_one_(node); }
...
ir_node *(get_Phi_next)(const ir_node *phi) { return get_Phi_next_(phi); }

void(set_Phi_next)(ir_node *phi, ir_node *next) { set_Phi_next_(phi, next); }
...
int is_x_except_Proj(const ir_node *node) {
  ...
  if (!is_fragile_op(pred)) return false;
...
ir_graph *(get_irn_irg)(const ir_node *node) { return get_irn_irg_(node); }
...
ir_node *skip_Pin(ir_node *node) {
  if (is_Pin(node)) return get_Pin_op(node);
  return node;
}

ir_node *skip_Confirm(ir_node *node) {
  if (is_Confirm(node)) return get_Confirm_value(node);
  return node;
}

ir_node *skip_Id(ir_node *node) {
  /* This should compact Id-cycles to self-cycles. It has the same (or less?)
   * complexity than any other approach, as Id chains are resolved and all point
   * to the real node, or all id's are self loops. */
  if (node->op != op_Id) return node;
  ...
  if (node != pred) { /* not a self referencing Id. Resolve Id chain. */
    ...
    node->in[0 + 1] =
        node; /* turn us into a self referencing Id:  shorten Id cycles. */
    ir_node *res = skip_Id(rem_pred);
    /* self-loop */
    if (is_Id(res)) return node;

    node->in[0 + 1] =
        res; /* Turn Id chain into Ids all referencing the chain end. */
    return res;
  } else {
    return node;
  }
...
int is_cfop(const ir_node *node) {
  if (is_fragile_op(node) && ir_throws_exception(node)) return true;
...
int(is_irn_forking)(const ir_node *node) { return is_irn_forking_(node); }
...
static bool is_call_no_write(const ir_node *node) {
  ...
  if (prop & mtp_property_no_write) return true;
...
int is_irn_const_memory(const ir_node *node) {
  const ir_op *op = get_irn_op(node);
  if (get_op_flags(op) & irop_flag_const_memory) return true;
  if (is_Builtin(node)) {
  ...
  if (is_Call(node)) return is_call_no_write(node);
  return false;
}

void(copy_node_attr)(ir_graph *irg, const ir_node *old_node,
                     ir_node *new_node) {
  copy_node_attr_(irg, old_node, new_node);
}

ir_type *(get_irn_type_attr)(ir_node *node) { return get_irn_type_attr_(node); }

ir_entity *(get_irn_entity_attr)(ir_node *node) {
  return get_irn_entity_attr_(node);
}

int(is_irn_constlike)(const ir_node *node) { return is_irn_constlike_(node); }

int(is_irn_keep)(const ir_node *node) { return is_irn_keep_(node); }
...
void(set_irn_dbg_info)(ir_node *n, dbg_info *db) { set_irn_dbg_info_(n, db); }

dbg_info *(get_irn_dbg_info)(const ir_node *n) { return get_irn_dbg_info_(n); }
...
void ir_switch_table_set(ir_switch_table *table, size_t n, ir_tarval *min,
                         ir_tarval *max, unsigned pn) {
...
ir_node *get_Proj_for_pn(ir_node const *const irn, unsigned const pn) {
  ...
  foreach_out_edge(irn, edge) {
    ...
    if (is_Proj(proj) && get_Proj_num(proj) == pn) return proj;
  }
  return NULL;
}
