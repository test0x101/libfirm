...
#define KeysEqual(this, key1, key2) \
  ((key1->src) == (key2->src) && (key1->pos == key2->pos))
...
#define foreach_tgt(irn, i, n, kind)                \
  for (int i = edge_kind_info[kind].first_idx,      \
           n = edge_kind_info[kind].get_arity(irn); \
       i < n; ++i)
 ...
 */
static inline long edge_get_id(const ir_edge_t *e) { return (long)e; }
...
static inline void verify_list_head(ir_node *irn, ir_edge_kind_t kind) {
  ...
  list_for_each(pos, head) {
    if (pset_find_ptr(lh_set, pos)) {
      ...
      ir_fprintf(
          stderr,
          "EDGE Verifier: edge list broken (self loop not to head) for %+F:\n",
          irn);
...
void edges_dump_kind(ir_graph *irg, ir_edge_kind_t kind) {
  if (!edges_activated_kind(irg, kind)) return;

  irg_edge_info_t *info = get_irg_edge_info(irg, kind);
  ir_edgeset_t *edges = &info->edges;
  ir_edge_t *e;
  ir_edgeset_iterator_t iter;
  foreach_ir_edgeset(edges, e, iter) { ir_printf("%+F %d\n", e->src, e->pos); }
}
...
static void edges_notify_edge_kind(ir_node *src, int pos, ir_node *tgt,
  ...
  /* verify list heads */
  if (edges_dbg) {
    if (tgt) verify_list_head(tgt, kind);
    if (old_tgt) verify_list_head(old_tgt, kind);
  }
#endif
}
  ...
  if (edges_activated_kind(irg, EDGE_KIND_BLOCK)) {
    if (is_Block(src)) {
      ...
      if (old_tgt != NULL && !is_Bad(old_tgt)) {
        ...
        if (is_Bad(bl_old)) bl_old = NULL;
      }
      if (tgt != NULL && !is_Bad(tgt)) {
        bl_tgt = get_nodes_block(tgt);
        if (is_Bad(bl_tgt)) bl_tgt = NULL;
      }
      ...
      foreach_block_succ_safe(old_tgt, edge) {
        ...
        if (block_pred != src) continue;
        if (is_Bad(tgt)) tgt = NULL;
        edges_notify_edge_kind(succ, succ_pos, tgt, old_tgt, EDGE_KIND_BLOCK,
                               irg);
      }
    }
  }
...
void edges_activate_kind(ir_graph *irg, ir_edge_kind_t kind) {
   ...
   *   While this would help for revivaled nodes, it increases memory and
   * runtime.
   ...
   * - Manually iterate over the identities root set. This did not consume more
   * memory but increase the computation time because the |identities| >= |V|
   *
...
int(edges_activated)(const ir_graph *irg) { return edges_activated_(irg); }
...
int edges_verify_kind(ir_graph *irg, ir_edge_kind_t kind) {
  ...
  foreach_ir_edgeset(edges, e, iter) { e->present = false; }
#endif
...
static void count_user(ir_node *irn, void *env) {
  ...
  for (int i = get_irn_arity(irn); i-- > first;) {
    ...
    if (bs) bitset_set(bs, get_irn_idx(irn));
  }
}
...
static void verify_edge_counter(ir_node *irn, void *env) {
  ...
  const struct list_head *pos;
  list_for_each(pos, head) { ++list_cnt; }
  ...
  if (edge_cnt != list_cnt) {
    w->fine = false;
    ir_fprintf(stderr,
               "Edge Verifier: edge count is %d, but %d edge(s) are recorded "
               "in list at %+F\n",
  ...
  if (ref_cnt != list_cnt) {
    w->fine = false;
    ir_fprintf(stderr,
               "Edge Verifier: %+F reachable by %d node(s), but the list "
               "contains %d edge(s)\n",
...
void init_edges(void) { FIRM_DBG_REGISTER(dbg, "firm.ir.edges"); }

void edges_init_dbg(int do_dbg) { edges_dbg = do_dbg; }
...
void assure_edges_kind(ir_graph *irg, ir_edge_kind_t kind) {
  if (!edges_activated_kind(irg, kind)) edges_activate_kind(irg, kind);
}
...
int(get_edge_src_pos)(const ir_edge_t *edge) { return get_edge_src_pos_(edge); }
