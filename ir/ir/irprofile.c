...
uint32_t ir_profile_get_block_execcount(const ir_node *block) {
  ...
  execcount_t *const ec =
      set_find(execcount_t, profile, &query, sizeof(query), query.block);
...
static unsigned int get_irp_n_blocks(void) {
  unsigned int count = 0;
  foreach_irp_irg(i, irg) { count += get_irg_n_blocks(irg); }
  return count;
}
...
static void add_constructor(ir_entity *method) {
  ...
  ir_entity *const ptr =
      new_global_entity(constructors, ide, ptr_type, ir_visibility_private,
...
static ir_entity *get_init_firmprof_ref(void) {
  ...
  ir_type *const init_type =
      new_type_method(3, 0, false, cc_cdecl_set, mtp_no_property);
...
static ir_graph *gen_initializer_irg(ir_entity *ent_filename,
  ...
  ir_entity *const ent = new_global_entity(
      owner, name, type, ir_visibility_local, IR_LINKAGE_DEFAULT);
  ...
  ir_node *const call =
      new_r_Call(bb, init_mem, callee, ARRAY_SIZE(ins), ins, call_type);
 ...
 */
static void instrument_block(ir_node *const bb, ir_node *const address,
                             unsigned int const id) {
  ir_graph *const irg = get_irn_irg(bb);

  /* We can't instrument the end block */
  if (bb == get_irg_end_block(irg)) return;
  ...
  ir_node *const cnst =
      new_r_Const_long(irg, mode_off, get_mode_size_bytes(mode_ctr) * id);
  ir_node *const offset = new_r_Add(bb, address, cnst);
  ir_node *const load =
      new_r_Load(bb, unknown, offset, mode_ctr, type_arr, cons_none);
  ...
  ir_node *const store =
      new_r_Store(bb, lmem, offset, add, type_arr, cons_none);
 ...
 */
static ir_entity *new_array_entity(ident *const name,
                                   ir_mode *const element_mode,
                                   unsigned const length,
                                   ir_linkage const linkage) {
  ...
  ir_type *const owner = get_glob_type();
  return new_global_entity(owner, id, array_type, ir_visibility_private,
                           linkage);
}
 ...
 */
static ir_entity *new_static_string_entity(char const *const name,
                                           char const *const string) {
  /* Create the type for a fixed-length string */
  ir_mode *const mode = mode_Bs;
  size_t const length = strlen(string) + 1;
  ir_entity *const result =
      new_array_entity(name, mode, length, IR_LINKAGE_CONSTANT);
...
ir_graph *ir_profile_instrument(const char *filename) {
   ...
   * complain. */
  if (get_irp_n_irgs() == 0) return NULL;
   ...
   * backend */
  ir_entity *const bblock_counts = new_array_entity(
      "__FIRMPROF__BLOCK_COUNTS", mode_Iu, n_blocks, IR_LINKAGE_DEFAULT);

  ir_entity *const ent_filename =
      new_static_string_entity("__FIRMPROF__FILE_NAME", filename);

  /* initialize block id array and instrument blocks */
  block_id_walker_data_t wd = {.id = 0};
  foreach_irp_irg_r(i, irg) { instrument_irg(irg, bblock_counts, &wd); }

  return gen_initializer_irg(ent_filename, bblock_counts, n_blocks);
}

static unsigned int *parse_profile(const char *filename,
  ...
  for (unsigned i = 0; i < num_blocks; ++i) {
    ...
    if ((ret = fread(bytes, 1, 4, f)) < 1) break;
...
bool ir_profile_read(const char *filename) {
  ...
  block_assoc_t env = {.i = 0, .counters = parse_profile(filename, n_blocks)};
  if (!env.counters) return false;
...
static void initialize_execfreq(ir_node *block, void *data) {
  ...
  if (block == get_irg_start_block(irg) || block == get_irg_end_block(irg)) {
    ...
    if (freq < MIN_EXECFREQ) freq = MIN_EXECFREQ;
  }
...
void ir_create_execfreqs_from_profile(void) {
  foreach_irp_irg_r(i, irg) { ir_set_execfreqs_from_profile(irg); }
}
