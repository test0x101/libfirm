...
/**
 * @file
 * @brief    Representation of opcode of intermediate operation -- private
 * header.
...
#define set_generic_function_ptr(op, func) \
  set_generic_function_ptr_((op), (op_func)(func))
#define get_generic_function_ptr(type, op) \
  ((type *)get_generic_function_ptr_((op)))
...
typedef struct {
  ...
  computed_value_func computed_value_Proj; /**< Evaluates a Proj node into a
                                              tarval if possible. */
  equivalent_node_func equivalent_node; /**< Optimizes the node by returning an
                                           equivalent one. */
  equivalent_node_func equivalent_node_Proj; /**< Optimizes the Proj node by
                                                returning an equivalent one. */
  transform_node_func
      transform_node; /**< Optimizes the node by transforming it. */
  transform_node_func
  ...
  get_type_attr_func
      get_type_attr; /**< Returns the type attribute of a node. */
  get_entity_attr_func
      get_entity_attr;          /**< Returns the entity attribute of a node. */
 ...
 */
static inline size_t get_op_attr_size(const ir_op *op) { return op->attr_size; }
...
static inline unsigned get_op_code_(const ir_op *op) { return op->code; }
...
static inline void set_op_tag_(ir_op *op, unsigned tag) { op->tag = tag; }

static inline unsigned get_op_tag_(const ir_op *op) { return op->tag; }

static inline void set_op_attr_(ir_op *op, void *attr) { op->attr = attr; }

static inline void *get_op_attr_(const ir_op *op) { return op->attr; }
