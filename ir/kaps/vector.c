...
vector_t *vector_alloc(pbqp_t *pbqp, unsigned length) {
  vector_t *vec = (vector_t *)obstack_alloc(
      &pbqp->obstack, sizeof(*vec) + sizeof(*vec->entries) * length);
  assert(length > 0);
...
vector_t *vector_copy(pbqp_t *pbqp, vector_t *v) {
  unsigned len = v->len;
  vector_t *copy = (vector_t *)obstack_copy(
      &pbqp->obstack, v, sizeof(*copy) + sizeof(*copy->entries) * len);
  assert(copy);
...
void vector_add(vector_t *sum, vector_t *summand) {
  ...
  for (unsigned i = 0; i < len; ++i) {
    sum->entries[i].data =
        pbqp_add(sum->entries[i].data, summand->entries[i].data);
  }
}
...
void vector_add_matrix_col(vector_t *vec, pbqp_matrix_t *mat,
  ...
  for (unsigned index = 0; index < len; ++index) {
    vec->entries[index].data = pbqp_add(
