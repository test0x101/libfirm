...
static void normalize_towards_source(pbqp_edge_t *edge) {
  ...
  for (unsigned src_index = 0; src_index < src_len; ++src_index) {
    ...
    if (min != 0) {
      ...
      src_vec->entries[src_index].data =
          pbqp_add(src_vec->entries[src_index].data, min);
...
static void normalize_towards_target(pbqp_edge_t *edge) {
  ...
  for (unsigned tgt_index = 0; tgt_index < tgt_len; ++tgt_index) {
    ...
    if (min != 0) {
      ...
      tgt_vec->entries[tgt_index].data =
          pbqp_add(tgt_vec->entries[tgt_index].data, min);
...
static void merge_source_into_target(pbqp_t *pbqp, pbqp_edge_t *edge) {
  ...
  for (unsigned tgt_index = 0; tgt_index < tgt_len; ++tgt_index) {
    if (tgt_vec->entries[tgt_index].data == INF_COSTS) continue;

    unsigned onlyOneZero = 0;

    for (unsigned src_index = 0; src_index < src_len; ++src_index) {
      if (src_vec->entries[src_index].data == INF_COSTS) continue;

      if (mat->entries[src_index * tgt_len + tgt_index] == INF_COSTS) continue;
    ...
    if (old_edge->tgt == src_node) {
      for (unsigned tgt_index = 0; tgt_index < tgt_len; ++tgt_index) {
        ...
        for (unsigned other_index = 0; other_index < other_len; ++other_index) {
          if (other_vec->entries[other_index].data == INF_COSTS) continue;

          new_matrix->entries[tgt_index * other_len + other_index] =
              old_matrix->entries[other_index * src_len + src_index];
        }
      }
    } else {
      ...
      for (unsigned tgt_index = 0; tgt_index < tgt_len; ++tgt_index) {
        ...
        for (unsigned other_index = 0; other_index < other_len; ++other_index) {
          ...
          new_matrix->entries[tgt_index * other_len + other_index] =
              old_matrix->entries[src_index * other_len + other_index];
        }
      }
    }
    ...
    if (old_edge->tgt == tgt_node) {
      for (unsigned src_index = 0; src_index < src_len; ++src_index) {
        ...
        for (unsigned other_index = 0; other_index < other_len; ++other_index) {
          ...
          new_matrix->entries[src_index * other_len + other_index] =
              old_matrix->entries[other_index * tgt_len + tgt_index];
        }
      }
    } else {
      /* Source node selects the row of the old_matrix. */
      for (unsigned src_index = 0; src_index < src_len; ++src_index) {
        ...
        for (unsigned other_index = 0; other_index < other_len; ++other_index) {
          ...
          new_matrix->entries[src_index * other_len + other_index] =
              old_matrix->entries[tgt_index * other_len + other_index];
        }
      }
    }
...
void simplify_edge(pbqp_t *pbqp, pbqp_edge_t *edge) {
  ...
  if (pbqp->dump_file) {
    char txt[100];
    sprintf(txt, "Simplification of Edge n%u-n%u", src_node->index,
            tgt_node->index);
    pbqp_dump_section(pbqp->dump_file, 3, txt);
  }
#endif
...
static void select_column(pbqp_edge_t *edge, unsigned col_index) {
  ...
  for (unsigned src_index = 0; src_index < src_len; ++src_index) {
    ...
    if (elem != 0) {
      ...
      src_vec->entries[src_index].data =
          pbqp_add(src_vec->entries[src_index].data, elem);
    }
  }
...
static void select_row(pbqp_edge_t *edge, unsigned row_index) {
  ...
  for (unsigned tgt_index = 0; tgt_index < tgt_len; ++tgt_index) {
    ...
    if (elem != 0) {
      ...
      tgt_vec->entries[tgt_index].data =
          pbqp_add(tgt_vec->entries[tgt_index].data, elem);
    }
  }
