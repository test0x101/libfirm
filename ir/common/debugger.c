 ...
 */
static void reset_dbg_buf(void) { firm_dbg_msg_buf[0] = '\0'; }
...
static void dbg_printf(const char *fmt, ...) {
  ...
  if (redir_output) {
    ...
    ir_vsnprintf(firm_dbg_msg_buf + cur, sizeof(firm_dbg_msg_buf) - cur, fmt,
                 args);
  } else {
    ir_vprintf(fmt, args);
  }
...
static void dbg_new_entity(void *ctx, ir_entity *ent) {
  ...
  bp_ident_t *ielem =
      set_find(bp_ident_t, bp_idents, &ikey, sizeof(ikey), HASH_IDENT_BP(ikey));
...
static const char *reason_str(bp_reasons_t reason) {
  switch (reason) {
    case BP_ON_NEW_THING:
      return "node, entity or type creation";
    case BP_ON_REPLACE:
      return "node replace";
    case BP_ON_LOWER:
...
#define CASE_ON(a, hook, handler)              \
  case a:                                      \
    if (!IS_HOOKED(hook)) HOOK(hook, handler); \
    break
#define CASE_OFF(a, hook)              \
  case a:                              \
    if (IS_HOOKED(hook)) UNHOOK(hook); \
    break
  ...
  if (num_active_bp[bp->reason] > 0) {
    ...
    switch (bp->reason) {
      ...
      case BP_ON_NEW_THING:
        if (!IS_HOOKED(hook_new_node)) HOOK(hook_new_node, dbg_new_node);
        if (!IS_HOOKED(hook_new_type)) HOOK(hook_new_type, dbg_new_type);
        if (!IS_HOOKED(hook_new_entity)) HOOK(hook_new_entity, dbg_new_entity);
        break;
      default:
        break;
    }
    ...
    switch (bp->reason) {
      ...
      case BP_ON_NEW_THING:
        if (IS_HOOKED(hook_new_node)) UNHOOK(hook_new_node);
        if (IS_HOOKED(hook_new_type)) UNHOOK(hook_new_type);
        if (IS_HOOKED(hook_new_entity)) UNHOOK(hook_new_entity);
        break;
      default:
        break;
    }
...
static void break_on_nr(long nr, bp_reasons_t reason) {
  ...
  bp_nr_t *elem =
      set_insert(bp_nr_t, bp_numbers, &key, sizeof(key), HASH_NR_BP(key));
...
static void break_on_ident(const char *name, bp_reasons_t reason) {
  ...
  bp_ident_t *elem =
      set_insert(bp_ident_t, bp_idents, &key, sizeof(key), HASH_IDENT_BP(key));
...
static void show_commands(void) {
  dbg_printf(
      ...
      "irgldname ldname      prints address and graph number of a method given "
      "by its ldname\n"
      "irgname name          prints address and graph number of a method given "
      "by its name\n"
  ...
  for (breakpoint *p = bp_list; p != NULL; p = p->next) {
    ...
    switch (p->kind) {
      ...
      case BP_IDENT: {
        ...
        dbg_printf("+%s of ident \"%s\" ", reason_str(p->reason),
                   get_id_str(ident_p->id));
        break;
      }
    }
...
static ir_type *find_type_nr(long nr) {
  for (int i = 0, n = get_irp_n_types(); i < n; ++i) {
    ...
    if (get_type_nr(tp) == nr) return tp;
  }
  return NULL;
}
...
static ir_type *find_type_name(const char *name) {
  for (int i = 0, n = get_irp_n_types(); i < n; ++i) {
    ir_type *tp = get_irp_type(i);
    if (!is_compound_type(tp)) continue;

    if (streq(get_compound_name(tp), name)) return tp;
  }
  return NULL;
}
 ...
 */
static void check_ent_nr(ir_type *const type, ir_entity *const entity,
                         void *const ctx) {
  (void)type;
...
static void check_ent_name(ir_type *const type, ir_entity *const entity,
  ...
  if (streq(name, env->u.name)) env->res = entity;
}
 ...
 */
static void show_by_name(ir_type *const type, ir_entity *const ent,
                         void *const env) {
  (void)type;
 ...
 */
static void show_by_ldname(ir_type *const type, ir_entity *const ent,
                           void *const env) {
  (void)type;
...
static const char *const reserved[] = {
    "bp",      "create",     "disable",       "dumpfilter", "enable",
    "help",    "init",       "initialnodenr", "irgldname",  "irgname",
    "lower",   "newent",     "remirg",        "replace",    "setlvl",
    "setmask", "setoutfile", "showent",       "showtype",
};
...
static char next_char(void) {
  if (lexer.curr_pos >= lexer.end_pos) return '\0';
  return *lexer.curr_pos++;
}

static void unput(void) {
  if (lexer.curr_pos < lexer.end_pos) --lexer.curr_pos;
}
...
static unsigned get_token(void) {
  ...
  if (c == '.' || isalpha((unsigned char)c)) {
    ...
    for (size_t i = ARRAY_SIZE(reserved); i-- != 0;) {
      if (strncmp(tok_start, reserved[i], len) == 0 && reserved[i][len] == '\0')
    ...
    /* we support negative numbers as well, so one can easily set all bits with
     * -1 */
    if (c == '-') {
      sign = 1;
      c = next_char();
...
void firm_debug(const char *cmd) {
  ...
  for (;;) {
    ...
    switch (token) {
      ...
      case tok_create:
        token = get_token();
        if (token != tok_number) goto error;
...
void firm_init_debugger(void) {
  ...
  if (env != NULL) firm_debug(env);

  if (break_on_init) firm_debug_break();
}
...
_firm_only_that_you_can_compile_with_NDEBUG_defined;
 ...
 * an various events.
 * The extension uses a text interface which can be accessed from most
 debuggers.
