...
void ir_timer_free(ir_timer_t *timer) { free(timer); }

#ifdef HAVE_GETTIMEOFDAY

static inline void _time_get(ir_timer_val_t *val) { gettimeofday(val, NULL); }
...
static inline void _time_get(ir_timer_val_t *val) {
  if (!QueryPerformanceCounter(&val->hi_prec)) val->lo_prec = timeGetTime();
}
...
static inline unsigned long _time_to_msec(const ir_timer_val_t *elapsed) {
  ...
  if (!QueryPerformanceFrequency(&freq)) return (unsigned long)elapsed->lo_prec;
...
int ir_timer_leave_high_priority(void) {
  ...
  if (std_sched_param_init) res = sched_setparam(pid, &std_sched_param);
...
int ir_timer_leave_high_priority(void) { return 0; }
...
void ir_timer_start(ir_timer_t *timer) {
  if (timer->running) panic("timer started twice");
...
void ir_timer_stop(ir_timer_t *timer) {
  if (!timer->running) panic("attempting to stop stopped timer");
  if (timer != timer_stack) panic("timer stack error");
  ...
  _time_get(&val);
  timer->running = 0;
  _time_add(&timer->elapsed, &timer->elapsed,
            _time_sub(&tgt, &val, &timer->start));
}
...
void ir_timer_push(ir_timer_t *timer) {
  ...
  if (timer->parent == NULL) panic("pushing timer with unknown parent");

  timer->displaced = timer_stack;
  for (ir_timer_t *t = timer_stack; t != parent; t = t->parent) {
    if (t == NULL) panic("parent timer not on stack");
    ir_timer_stop(t);
  }
  timer_stack = parent;
...
void ir_timer_pop(ir_timer_t *timer) {
  ...
  if (displaced == NULL) panic("timer start/stop/push/pop mismatch");
