...
typedef struct {
  ...
  lc_opt_dump_vals_t
      *dump_vals; /**< A function which is able to format the possible values
                       for this option into a string. May be NULL. */

} lc_opt_table_entry_t;

#define _LC_OPT_ENT(name, desc, type, val_type, value, len, cb, dump,       \
                    dump_vals)                                              \
  {                                                                         \
    name, desc, type,                                                       \
        1 ? (value)                                                         \
          : (val_type *)0 /* Produces a warning, if var has wrong type. */, \
        len, cb, dump, dump_vals                                            \
  }

#define LC_OPT_ENT_INT(name, desc, addr)                                \
  _LC_OPT_ENT(name, desc, lc_opt_type_int, int, addr, 0, lc_opt_int_cb, \
              lc_opt_int_dump, NULL)

#define LC_OPT_ENT_DBL(name, desc, addr)                       \
  _LC_OPT_ENT(name, desc, lc_opt_type_double, double, addr, 0, \
              lc_opt_double_cb, lc_opt_double_dump, NULL)

#define LC_OPT_ENT_BIT(name, desc, addr, mask)                   \
  _LC_OPT_ENT(name, desc, lc_opt_type_bit, unsigned, addr, mask, \
              lc_opt_bit_cb, lc_opt_bit_dump, NULL)

#define LC_OPT_ENT_BOOL(name, desc, addr)                                     \
  _LC_OPT_ENT(name, desc, lc_opt_type_boolean, bool, addr, 0, lc_opt_bool_cb, \
              lc_opt_bool_dump, lc_opt_bool_dump_vals)

typedef char lc_opt_str_t[];
#define LC_OPT_ENT_STR(name, desc, buf)                                        \
  _LC_OPT_ENT(name, desc, lc_opt_type_string, lc_opt_str_t, buf, sizeof(*buf), \
              lc_opt_string_cb, lc_opt_string_dump, NULL)
 ...
 */
lc_opt_entry_t *lc_opt_add_opt(lc_opt_entry_t *grp, const char *name,
