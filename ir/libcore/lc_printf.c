...
int lc_arg_register(lc_arg_env_t *env, const char *name, char letter,
  ...
  lc_arg_t *ent =
      set_insert(lc_arg_t, env->args, &arg, sizeof(arg), hash_str(name));

  if (ent && base != 0) map[letter - base] = ent;
...
int lc_arg_append(lc_appendable_t *app, const lc_arg_occ_t *occ,
                  const char *str, size_t len) {
  char pad = ' ';

  /* Set the padding to zero, if the zero is given and we are not left
   * justified. (A minus overrides the zero). See printf(3). */
  if (!occ->flag_minus && occ->flag_zero) pad = '0';

  return lc_appendable_snwadd(app, str, len, MAX(0, occ->width),
                              occ->flag_minus, pad);
}
...
#define LC_ARG_TYPE(type, name, va_type)          \
  case lc_arg_type_##name:                        \
    res = snprintf(buf, len, fmt, val->v_##name); \
    break;
...
static char *make_fmt(char *buf, size_t len, const lc_arg_occ_t *occ) {
  ...
  if (occ->precision > 0) snprintf(prec, sizeof(prec), ".%d", occ->precision);

  if (occ->width > 0) snprintf(width, sizeof(width), "%d", occ->width);
  ...
  if (occ->modifier_length == 1) {
    if (mod[0] == 'z') {
      if (sizeof(size_t) == sizeof(int)) mod[0] = '\0';
      ...
      if (sizeof(ptrdiff_t) == sizeof(int)) mod[0] = '\0';
      ...
      if (sizeof(intmax_t) == sizeof(int)) mod[0] = '\0';
...
#endif
  snprintf(buf, len, "%%%s%s%s%s%s%s%s%s%c", occ->flag_space ? "#" : "",
           occ->flag_hash ? "#" : "", occ->flag_plus ? "+" : "",
           occ->flag_minus ? "-" : "", occ->flag_zero ? "0" : "", width, prec,
           mod, occ->conversion);
...
static int std_get_lc_arg_type(const lc_arg_occ_t *occ) {
  ...
  if (modlen > 0) {
    ...
    switch (mod[0]) {
      case 'h':
        return modlen > 1 && mod[1] == 'h' ? lc_arg_type_char
                                           : lc_arg_type_short;
      case 'l':
        return modlen > 1 && mod[1] == 'l' ? lc_arg_type_long_long
                                           : lc_arg_type_long;
#define TYPE_CASE(letter, type) \
  case letter:                  \
    return lc_arg_type_##type
...
static int std_emit(lc_appendable_t *app, const lc_arg_occ_t *occ,
                    const lc_arg_value_t *val) {
  char fmt[32];
  int res = 0;
...
static const lc_arg_handler_t std_handler = {std_get_lc_arg_type, std_emit};
...
#define LC_ARG_TYPE(type, name, va_type)  \
  case lc_arg_type_##name:                \
    val.v_##name = va_arg(args, va_type); \
    break;
#include "lc_printf_arg_types.def"
#undef LC_ARG_TYPE
      }
...
int lc_evsnprintf(const lc_arg_env_t *env, char *buf, size_t len,
...
int lc_evfprintf(const lc_arg_env_t *env, FILE *f, const char *fmt,
                 va_list args) {
  lc_appendable_t app;
...
int lc_evoprintf(const lc_arg_env_t *env, struct obstack *obst, const char *fmt,
                 va_list args) {
  lc_appendable_t app;
