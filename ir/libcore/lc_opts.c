...
static lc_opt_entry_t *init_opt(lc_opt_entry_t *ent, lc_opt_type_t type,
                                void *val, size_t length, lc_opt_callback_t *cb,
                                lc_opt_dump_t *dump,
...
lc_opt_entry_t *lc_opt_add_opt(lc_opt_entry_t *parent, const char *name,
                               const char *desc, lc_opt_type_t type,
                               void *value, size_t length,
                               lc_opt_callback_t *cb, lc_opt_dump_t *dump,
                               lc_opt_dump_vals_t *dump_vals) {
  if (!parent->is_grp) return NULL;
...
static lc_opt_entry_t *lc_opt_find_ent(const struct list_head *head,
  ...
  list_for_each_entry(lc_opt_entry_t, ent, head, list) {
    if (entry_matches(ent, hash, name)) return ent;
  }
  return NULL;
}
...
static lc_opt_entry_t *resolve_up_to_last_str(lc_opt_entry_t *root,
                                              const char *path,
                                              const char **last_name) {
  size_t next = strspn(path, path_delim);

  /* if l != 0 we saw deliminators, so we resolve from the root */
  if (next > 0) root = lc_opt_root_grp();
...
bool lc_opt_bool_cb(void *const data, size_t const length,
                    char const *const value) {
  (void)length;
  return string_to_bool((bool *)data, value);
}

bool lc_opt_double_cb(void *const data, size_t const length,
                      char const *const value) {
  (void)length;
  return sscanf(value, "%lf", (double *)data) != 0;
}

bool lc_opt_int_cb(void *const data, size_t const length,
                   char const *const value) {
  (void)length;
  return sscanf(value, "%i", (int *)data) != 0;
}

bool lc_opt_string_cb(void *const data, size_t const length,
                      char const *const value) {
 ...
 * @return     @p buf.
 */
static char *lc_opt_value_to_string(char *buf, size_t len,
...
static char *lc_opt_values_to_string(char *buf, size_t len,
                                     const lc_opt_entry_t *ent) {
  const lc_opt_special_t *s = lc_get_opt_special(ent);
  if (s->dump_vals) s->dump_vals(buf, len, s->value);
  return buf;
}
...
bool lc_opt_add_table(lc_opt_entry_t *root, const lc_opt_table_entry_t *table) {
  ...
  for (int i = 0; table[i].name != NULL; ++i) {
    ...
    lc_opt_entry_t *entry =
        lc_opt_add_opt(grp, name, tab->desc, tab->type, tab->value, tab->len,
                       tab->cb, tab->dump, tab->dump_vals);
    if (entry == NULL) res = true;
  }

  return res;
}

static void lc_opt_print_grp_path_rec(char *buf, size_t len,
                                      const lc_opt_entry_t *ent, char separator,
                                      lc_opt_entry_t *stop_ent) {
  if (ent == stop_ent) return;
...
static char *lc_opt_print_grp_path(char *buf, size_t len,
                                   const lc_opt_entry_t *ent, char separator,
                                   lc_opt_entry_t *stop_ent) {
  if (len > 0) buf[0] = '\0';
 ...
 * @param f          output file
 */
static void lc_opt_print_help_rec(lc_opt_entry_t *ent, char separator,
                                  lc_opt_entry_t *stop_ent, FILE *f) {
...
int lc_opt_from_single_arg(const lc_opt_entry_t *root, const char *arg) {
  ...
  char *buf =
      (char *)obstack_copy0(&obst, arg, end ? end - arg : (int)strlen(arg));
