...
int check_type(const ir_type *tp) {
  switch (get_type_opcode(tp)) {
    ...
    case tpo_class:
    case tpo_segment:
      return check_compound_type(tp);
    case tpo_primitive:
      return check_primitive_type(tp);
    case tpo_pointer:
      return check_pointer_type(tp);
    case tpo_array:
    case tpo_method:
...
static bool check_visited_flag(ir_graph *irg, ir_node *n) {
  ...
  if (get_irn_visited(n) > get_irg_visited(irg)) {
    report_error(
        "visited flag of %+F is larger than that of corresponding irg %+F", n,
        irg);
    fine = false;
  }
  return fine;
...
static bool check_initializer(const ir_initializer_t *initializer,
                              const ir_type *type, const ir_entity *context) {
  bool fine = true;
  switch (get_initializer_kind(initializer)) {
    ...
    case IR_INITIALIZER_TARVAL: {
      ...
      if (get_type_mode(type) != get_tarval_mode(tv)) {
        report_error(
            "tarval initializer for entity %+F has wrong mode: %+F vs %+F",
            context, get_type_mode(type), get_tarval_mode(tv));
        fine = false;
      }
      return fine;
    }
    case IR_INITIALIZER_CONST: {
      ...
      if (get_type_mode(type) != get_irn_mode(value)) {
        report_error(
            "const initializer for entity %+F has wrong mode: %+F vs %+F",
            context, get_type_mode(type), get_irn_mode(value));
        fine = false;
      }
      if (!constant_on_correct_irg(value)) {
        report_error(
            "initializer const value %+F for entity %+F not on const-code irg",
            value, context);
        fine = false;
      }
      return fine;
    ...
    case IR_INITIALIZER_COMPOUND: {
      ...
      if (is_Array_type(type)) {
        ...
        report_error(
            "compound initiailizer for non-array/compound type in entity %+F",
            context);
        fine = false;
      }
      return fine;
...
static bool check_external_linkage(const ir_entity *entity, ir_linkage linkage,
  ...
  bool fine = true;
  if ((get_entity_linkage(entity) & linkage) == 0) return true;
  if (!is_externally_visible(entity)) {
    report_error("entity %+F has IR_LINKAGE_%s but is not externally visible",
                 entity, linkage_name);
    fine = false;
  }
  if (!entity_has_definition(entity)) {
    report_error("entity %+F has IR_LINKAGE_%s but is just a declaration",
                 entity, linkage_name);
    fine = false;
  }
  return fine;
...
int check_entity(const ir_entity *entity) {
  ...
  if (linkage & IR_LINKAGE_NO_CODEGEN) {
    if (!is_method_entity(entity)) {
      report_error("entity %+F has IR_LINKAGE_NO_CODEGEN but is not a function",
                   entity);
      fine = false;
    } else if (get_entity_irg(entity) == NULL) {
      report_error(
          "entity %+F has IR_LINKAGE_NO_CODEGEN but has no ir-graph anyway",
          entity);
      fine = false;
    }
    if (!is_externally_visible(entity)) {
  ...
  check_external_linkage(entity, IR_LINKAGE_GARBAGE_COLLECT, "GARBAGE_COLLECT");
  ...
  switch (get_entity_kind(entity)) {
    ...
    case IR_ENTITY_NORMAL: {
      ...
      if (init) fine &= check_initializer(init, type, entity);

      if (!is_data_type(type)) {
        report_error("normal entity %+F has non-data type %+F", entity, type);
        fine = false;
      }
      break;
    }
    ...
    case IR_ENTITY_LABEL:
      if (type != get_code_type()) {
        report_error("label entity %+F has non-code type %+F", entity, type);
        fine = false;
      }
      break;
    ...
    case IR_ENTITY_METHOD:
      if (!is_Method_type(type)) {
        report_error("method entity %+F has non-method type %+F", entity, type);
        fine = false;
      }
      ...
      if (irg != NULL) {
        ...
        if (irg_entity != entity) {
          report_error("entity(%+F)->irg->entity(%+F) relation invalid", entity,
                       irg_entity);
          fine = false;
        }
      }
    ...
    case IR_ENTITY_PARAMETER:
      if (!is_frame_type(owner)) {
        report_error("parameter entity %+F has non-frame owner %+F", entity,
                     owner);
        fine = false;
      }
      if (!is_data_type(type)) {
...
static void check_tore(ir_type *const type, ir_entity *const entity,
                       void *const env) {
  bool *fine = (bool *)env;
...
static bool verify_info_member(ir_type const *const segment,
  ...
  if (ld_ident[0] != '\0') {
    report_error("entity %+F in %s segment must not have an ld_name", entity,
                 get_compound_name(segment));
    fine = false;
  }
  if ((get_entity_linkage(entity) & IR_LINKAGE_HIDDEN_USER) == 0) {
    report_error("entity %+F in segment %s without LINKAGE_HIDDEN_USER", entity,
                 get_compound_name(segment));
    fine = false;
  }
  return fine;
...
int tr_verify(void) {
  ...
  for (ir_segment_t s = IR_SEGMENT_FIRST; s <= IR_SEGMENT_LAST; ++s) {
    ...
    for (size_t e = 0; e < get_compound_n_members(segment); ++e) {
      ...
      if (ld_ident == NULL &&
        ...
        report_error("public segment member %+F has no name", entity);
        fine = false;
        continue;
      }
      if (segment->flags & tf_info) fine &= verify_info_member(segment, entity);
    }
  }
  ...
  for (size_t i = 0, n = get_compound_n_members(thread_locals); i < n; ++i) {
    ...
    if (get_entity_linkage(entity) & IR_LINKAGE_CONSTANT) {
      report_error("entity %+F in thread local segment is constant", entity);
      fine = false;
    }
  }
