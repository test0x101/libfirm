...
#define set_entity_alignment(ent, alignment) \
...
typedef struct method_ent_attr {
  ...
  unsigned vtable_number; /**< For a dynamically called method, the number
                             assigned in the virtual function table. */

  ptr_access_kind *param_access; /**< the parameter access */
  unsigned *param_weight; /**< The weight of method's parameters. Parameters
                               with a high weight are good candidates for
                             procedure cloning. */
} method_ent_attr;
...
typedef struct parameter_ent_attr {
  ...
  bool is_lowered_doubleword;    /**< entity is a lowered doubleword parameter,
                                      ...
                                      calling conventions and then perform
                                    doubleword    lowering...) */
} parameter_ent_attr;
...
static inline ir_initializer_t *_get_entity_initializer(
    ir_entity const *const ent) {
...
static inline void _set_entity_bitfield_offset(ir_entity *ent,
                                               unsigned offset) {
  assert(is_entity_compound_member(ent));
...
static inline ir_graph *_get_entity_linktime_irg(const ir_entity *entity) {
  ...
  if (get_entity_linkage(entity) & IR_LINKAGE_WEAK) return NULL;
  /* only method entities have an irg field (alias etc. does not) */
  if (entity->kind != IR_ENTITY_METHOD) return NULL;
