...
ir_entity *get_unknown_entity(void) { return irp->unknown_entity; }
...
static ir_entity *new_entity_vis(ir_type *owner, ident *name, ir_type *type,
  ...
  if (is_Method_type(type)) {
    ...
    res = intern_new_entity(owner, IR_ENTITY_COMPOUND_MEMBER, name, type, vis);
...
ir_entity *new_alias_entity(ir_type *owner, ident *name, ir_entity *aliased,
  ...
  ir_entity *res =
      intern_new_entity(owner, IR_ENTITY_ALIAS, name, type, visibility);
...
ir_initializer_t *get_initializer_null(void) { return &null_initializer; }
...
ir_initializer_t *create_initializer_compound(size_t n_entries) {
  ...
  ir_initializer_t *initializer = (ir_initializer_t *)obstack_alloc(obst, size);
...
void set_initializer_compound_value(ir_initializer_t *initializer, size_t index,
...
ir_initializer_kind_t get_initializer_kind(
...
int(get_entity_offset)(const ir_entity *ent) { return _get_entity_offset(ent); }
...
size_t get_entity_n_overwrites(const ir_entity *ent) {
  if (ent->overwrites == NULL) return 0;
...
size_t get_entity_overwrites_index(const ir_entity *ent,
  ...
  for (size_t i = 0, n = get_entity_n_overwrites(ent); i < n; ++i) {
    if (get_entity_overwrites(ent, i) == overwritten) return i;
  }
  return (size_t)-1;
}
...
void remove_entity_overwrites(ir_entity *ent, ir_entity *overwritten) {
  for (size_t i = 0, n = get_entity_n_overwrites(ent); i < n; ++i) {
    if (ent->overwrites[i] == overwritten) {
      for (; i < n - 1; i++) ent->overwrites[i] = ent->overwrites[i + 1];
...
size_t get_entity_n_overwrittenby(const ir_entity *ent) {
  if (ent->overwrittenby == NULL) return 0;
...
size_t get_entity_overwrittenby_index(const ir_entity *ent,
  ...
  for (size_t i = 0, n = get_entity_n_overwrittenby(ent); i < n; ++i) {
    if (get_entity_overwrittenby(ent, i) == overwrites) return i;
  }
  return (size_t)-1;
}
...
void set_entity_overwrittenby(ir_entity *ent, size_t pos,
                              ir_entity *overwrites) {
  assert(pos < get_entity_n_overwrittenby(ent));
...
void remove_entity_overwrittenby(ir_entity *ent, ir_entity *overwrites) {
  for (size_t i = 0, n = get_entity_n_overwrittenby(ent); i < n; ++i) {
    if (ent->overwrittenby[i] == overwrites) {
      for (; i < n - 1; ++i) ent->overwrittenby[i] = ent->overwrittenby[i + 1];
...
void *(get_entity_link)(const ir_entity *ent) { return _get_entity_link(ent); }

void(set_entity_link)(ir_entity *ent, void *l) { _set_entity_link(ent, l); }
...
void(mark_entity_visited)(ir_entity *ent) { _mark_entity_visited(ent); }

int(entity_visited)(const ir_entity *ent) { return _entity_visited(ent); }
...
mtp_additional_properties get_entity_additional_properties(
    const ir_entity *ent) {
...
void set_entity_additional_properties(ir_entity *ent,
  ...
  assert((get_method_additional_properties(get_entity_type(ent)) &
          ~property_mask) == 0);
