...
static void walk_initializer(ir_initializer_t *initializer, type_walk_func *pre,
                             type_walk_func *post, void *env) {
  ...
  if (ent) {
    ...
    switch (get_entity_kind(ent)) {
      case IR_ENTITY_ALIAS: {
        ...
        if (e) do_type_walk(NULL, e, pre, post, env);
        break;
      }
      ...
      case IR_ENTITY_NORMAL: {
        ...
        if (init) walk_initializer(init, pre, post, env);
        break;
      }
    ...
    switch (get_type_opcode(tp)) {
      case tpo_class:
        for (size_t i = 0, n_types = get_class_n_supertypes(tp); i < n_types;
             ++i) {
        ...
        for (size_t i = 0, n_types = get_class_n_subtypes(tp); i < n_types;
             ++i) {
...
static void irn_type_walker(ir_node *node, type_walk_func *pre,
  ...
  if (ent) do_type_walk(NULL, ent, pre, post, env);
  ir_type *const typ = get_irn_type_attr(node);
  if (typ) do_type_walk(typ, NULL, pre, post, env);
}
...
static void class_walk_s2s_2(ir_type *tp, class_walk_func *pre,
  ...
  for (size_t i = 0, n = get_class_n_supertypes(tp); i < n; ++i) {
    if (!type_visited(get_class_supertype(tp, i))) return;
  }
...
void class_walk_super2sub(class_walk_func *pre, class_walk_func *post,
  ...
  for (size_t i = 0, n_types = get_irp_n_types(); i < n_types; i++) {
    ...
    if (is_Class_type(tp) && (get_class_n_supertypes(tp) == 0) &&
        !type_visited(tp) && (!is_frame_type(tp)) && (tp != get_glob_type())) {
