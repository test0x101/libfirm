...
static tr_inh_trans_tp *get_firm_kind_entry(const firm_kind *k) {
  ...
  tr_inh_trans_tp a;
  a.kind = k;
  tr_inh_trans_tp *found = set_find(tr_inh_trans_tp, tr_inh_trans_set, &a,
                                    sizeof(a), tr_inh_trans_hash(&a));
  if (!found) {
    a.directions[d_up] = pset_new_ptr(16);
    a.directions[d_down] = pset_new_ptr(16);
    found = set_insert(tr_inh_trans_tp, tr_inh_trans_set, &a, sizeof(a),
                       tr_inh_trans_hash(&a));
  }
  return found;
}
...
static void compute_up_closure(ir_type *tp) {
  ...
  /* Walk down. */
  for (size_t i = 0, n_subtypes = get_class_n_subtypes(tp); i < n_subtypes;
       ++i) {
    ir_type *stp = get_class_subtype(tp, i);
    if (get_type_visited(stp) < master_visited - 1) {
      compute_up_closure(stp);
...
void compute_inh_transitive_closure(void) {
  ...
  inc_master_type_visited(); /* Inc twice: one if on stack, second if values
                                computed. */
  ...
  for (size_t i = 0; i < n_types; ++i) {
    ...
    if (is_Class_type(tp) &&
        !type_visited(tp)) { /* For others there is nothing to accumulate. */
      size_t n_subtypes = get_class_n_subtypes(tp);
      ...
      if (!has_unmarked_subtype) compute_down_closure(tp);
    }
  }
  ...
  for (size_t i = 0; i < n_types; ++i) {
    ...
    if (is_Class_type(tp) &&
      ...
      /* This is a good starting point. */
      if (!has_unmarked_supertype) compute_up_closure(tp);
    }
  }
...
static int check_is_SubClass_of(const ir_type *low, const ir_type *high) {
  ...
  for (size_t i = 0, n_subtypes = get_class_n_subtypes(high); i < n_subtypes;
       i++) {
    ir_type *stp = get_class_subtype(high, i);
    if (low == stp) return 1;
    if (is_SubClass_of(low, stp)) return 1;
  }
  return 0;
}
...
int is_overwritten_by(ir_entity *high, ir_entity *low) {
  ...
  for (size_t i = 0, n_overwrittenby = get_entity_n_overwrittenby(high);
    ...
    if (low == ov) return 1;
    if (is_overwritten_by(low, ov)) return 1;
  }
  return 0;
}
 ...
 */
static ir_entity *do_resolve_ent_polymorphy(ir_type *dynamic_class,
