...
ir_type *get_code_type(void) { return irp->code_type; }

ir_type *get_unknown_type(void) { return irp->unknown_type; }
...
void set_master_type_visited(ir_visited_t val) { firm_type_visited = val; }

ir_visited_t(get_master_type_visited)(void) {
  return get_master_type_visited_();
}

void inc_master_type_visited(void) { ++firm_type_visited; }
...
void free_type_entities(ir_type *const type) {
  if (is_compound_type(type)) free_compound_entities(type);
}
...
void *(get_type_link)(const ir_type *tp) { return get_type_link_(tp); }

void(set_type_link)(ir_type *tp, void *l) { set_type_link_(tp, l); }
...
ir_mode *(get_type_mode)(const ir_type *tp) { return get_type_mode_(tp); }
...
unsigned(get_type_size)(const ir_type *tp) { return get_type_size_(tp); }

void set_type_size(ir_type *tp, unsigned size) { tp->size = size; }
...
ir_type_state(get_type_state)(const ir_type *tp) { return get_type_state_(tp); }

void set_type_state(ir_type *tp, ir_type_state state) {
  assert(is_type(tp));

  tp_opcode opcode = get_type_opcode(tp);
  if (opcode == tpo_pointer || opcode == tpo_primitive || opcode == tpo_method)
    return;

#ifndef NDEBUG
  /* Just a correctness check: */
  if (state == layout_fixed && is_compound_type(tp) && !is_segment_type(tp)) {
    for (size_t i = 0, n_mem = get_compound_n_members(tp); i < n_mem; i++) {
      ir_entity *entity = get_compound_member(tp, i);
      if (is_method_entity(entity)) continue;
...
void(mark_type_visited)(ir_type *tp) { mark_type_visited_(tp); }

int(type_visited)(const ir_type *tp) { return type_visited_(tp); }
...
void add_class_subtype(ir_type *clss, ir_type *subtype) {
  ...
  for (size_t i = 0, n_supertypes = get_class_n_supertypes(subtype);
       i < n_supertypes; i++) {
    if (get_class_supertype(subtype, i) == clss) /* Class already registered */
      return;
  }
...
size_t get_class_subtype_index(const ir_type *clss, const ir_type *subclass) {
  ...
  for (size_t i = 0, n_subtypes = get_class_n_subtypes(clss); i < n_subtypes;
       ++i) {
...
void remove_class_subtype(ir_type *clss, ir_type *subtype) {
  ...
  for (size_t i = 0; i < ARR_LEN(clss->attr.cls.subtypes); ++i) {
    if (clss->attr.cls.subtypes[i] == subtype) {
      ...
      ARR_SETLEN(ir_type *, clss->attr.cls.subtypes,
                 ARR_LEN(clss->attr.cls.subtypes) - 1);
      break;
    }
  }
}
...
void add_class_supertype(ir_type *clss, ir_type *supertype) {
  ...
  for (size_t i = 0, n = get_class_n_subtypes(supertype); i < n; ++i) {
    if (get_class_subtype(supertype, i) == clss) /* Class already registered */
      return;
  }
...
size_t get_class_supertype_index(const ir_type *clss,
  ...
  for (size_t i = 0, n_supertypes = get_class_n_supertypes(clss);
       i < n_supertypes; i++) {
    if (get_class_supertype(clss, i) == super_clss) return i;
  }
  return (size_t)-1;
}
...
void remove_class_supertype(ir_type *clss, ir_type *supertype) {
  ...
  for (size_t i = 0; i < ARR_LEN(clss->attr.cls.supertypes); ++i) {
    if (clss->attr.cls.supertypes[i] == supertype) {
      ...
      ARR_SETLEN(ir_type *, clss->attr.cls.supertypes,
                 ARR_LEN(clss->attr.cls.supertypes) - 1);
      break;
    }
  }
}

int(is_Class_type)(const ir_type *clss) { return is_class_type_(clss); }
...
size_t get_struct_member_index(ir_type const *strct,
                               ir_entity const *const mem) {
  assert(is_Struct_type(strct));
  return get_compound_member_index(strct, mem);
}

int(is_Struct_type)(const ir_type *strct) { return is_struct_type_(strct); }

ir_type *new_type_method(size_t const n_param, size_t const n_res,
                         int const is_variadic, unsigned const cc_mask,
...
ir_type *clone_type_method(ir_type *const tp, int const variadic_index,
                           mtp_additional_properties const property_mask) {
...
mtp_additional_properties(get_method_additional_properties)(
    const ir_type *method) {
...
int(is_Method_type)(const ir_type *method) { return is_method_type_(method); }
...
int(is_Union_type)(const ir_type *uni) { return is_union_type_(uni); }
...
ir_type *new_type_array(ir_type *const element_type,
                        unsigned const n_elements) {
...
int(is_Array_type)(const ir_type *array) { return is_array_type_(array); }
...
ir_type *new_type_primitive(ir_mode *mode) {
  unsigned size = get_mode_size_bytes(mode);
  unsigned align = (size > 0 && size != (unsigned)-1) ? ceil_po2(size) : 1;
...
int(is_atomic_type)(const ir_type *tp) { return is_atomic_type_(tp); }
...
size_t get_compound_member_index(ir_type const *const type,
  ...
  for (size_t i = 0, n = get_compound_n_members(type); i < n; ++i) {
    if (get_compound_member(type, i) == entity) return i;
  }
...
const char *get_compound_name(const ir_type *tp) {
  if (get_compound_ident(tp) == NULL) return NULL;
...
void remove_compound_member(ir_type *type, ir_entity *member) {
  ...
  for (size_t i = 0, n = ARR_LEN(type->attr.compound.members); i < n; ++i) {
    if (get_compound_member(type, i) != member) continue;
    for (; i < n - 1; ++i)
...
void default_layout_compound_type(ir_type *type) {
  ...
  for (size_t i = 0, n = get_compound_n_members(type); i < n; ++i) {
    ...
    if (is_Method_type(entity_type)) continue;

    if (get_entity_bitfield_size(entity) > 0) {
      panic(
          "default_layout_compound_type() cannot handle bitfield members (in "
          "%+F)",
          type);
    }

    unsigned entity_size;
    if (i + 1 == n && is_Array_type(entity_type) &&
        get_array_size(entity_type) == 0) {
      entity_size = 0;
    } else {
    ...
    if (is_Union_type(type)) {
      ...
      if (align != 0) {
        ...
        if (misalign != 0) size += align - misalign;
      }
      offset = size;
      size += entity_size;
    }
  ...
  if (misalign != 0) size += align_all - misalign;
    ...
    case tpo_code:
...

      {
        ...
        case tpo_segment:
          ...
          goto compound;
        compound:
          snprintf(buffer, buffer_size, "%s '%s'", kind_name,
