...
typedef enum iter_direction_t { down, right, all } iter_direction_t;
...
#define SP_MATRIX_INIT_LIST_HEAD(ptr) \
  do {                                \
    (ptr)->next = NULL;               \
  } while (0)
 ...
 */
unsigned matrix_get_elem_size(void) { return sizeof(entry_t); }
...
/**
 * Searches in row @p row for the matrix element m[row, col], starting at
 * element @p start.
 * @return If the element exists:
 *            Element m[row, col] and @p prev points to the sp_matrix_list_head
 * in the entry_t holding the element. Else: NULL and @p prev points to the
 * sp_matrix_list_head after which the element would be inserted.
 *         @p prev_prev always points to the previous element of @p prev
 */
static inline matrix_elem_t *m_search_in_row_from(
    const sp_matrix_t *m, int row, int col, sp_matrix_list_head_t *start,
    sp_matrix_list_head_t **prev, sp_matrix_list_head_t **prev_prev) {
  ...
  *prev = start;
  ...
  while ((*prev)->next != NULL &&
         list_entry_by_row((*prev)->next)->col <= col) {
    (*prev_prev) = (*prev);
    *prev = (*prev)->next;
  }
  ...
  if (*prev != row_start) {
    ...
    if (me->row == row && me->col == col) res = me;
  }
 ...
 *         @p prev_prev always points to the previous element of @p prev
 */
static inline matrix_elem_t *m_search_in_row(
    const sp_matrix_t *m, int row, int col, sp_matrix_list_head_t **prev,
...
/**
 * Searches in col @p col for the matrix element m[row, col], starting at @p
 * start.
 * @return If the element exists:
 *            Element m[row, col] and @p prev points to the sp_matrix_list_head
 * in the entry_t holding the element. Else: NULL and @p prev points to the
 ...
 *         @p prev_prev always points to the previous element of @p prev
 */
static inline matrix_elem_t *m_search_in_col_from(
    const sp_matrix_t *m, int row, int col, sp_matrix_list_head_t *start,
    sp_matrix_list_head_t **prev, sp_matrix_list_head_t **prev_prev) {
  ...
  *prev = start;
  ...
  while ((*prev)->next != NULL &&
         list_entry_by_col((*prev)->next)->row <= row) {
    *prev_prev = (*prev);
    *prev = (*prev)->next;
  }
 ...
 *         @p prev_prev always points to the previous element of @p prev
 */
static inline matrix_elem_t *m_search_in_col(
    const sp_matrix_t *m, int row, int col, sp_matrix_list_head_t **prev,
    sp_matrix_list_head_t **prev_prev) {
...
void del_matrix(sp_matrix_t *m) {
  ...
  for (i = 0; i < m->colc; ++i) free(m->cols[i]);
  free(m->last_col_el);
  free(m->last_row_el);
  free(m->rows);
  free(m->cols);
...
void matrix_set(sp_matrix_t *m, int row, int col, double val) {
  ...
  if (row > m->maxrow) {
    m->maxrow = row;
    if (row >= m->rowc) m_alloc_row(m, m->rowc, m_new_size(m->rowc, row));
  }
  if (col > m->maxcol) {
    m->maxcol = col;
    if (col >= m->colc) m_alloc_col(m, m->colc, m_new_size(m->colc, col));
  }
...
void matrix_set_row_bulk(sp_matrix_t *m, int row, int *cols, int num_cols,
  ...
  for (i = 0; i < num_cols; ++i) {
    ...
    /* now leftof and above are the entry_t's prior the new one in each
     * direction */
...
double matrix_get(const sp_matrix_t *m, int row, int col) {
  ...
  if (is_empty_row(row) || is_empty_col(col)) return 0.0;
  ...
  if (me) assert(me->col == col && me->row == row);

  return me ? me->val : 0.0;
}

int matrix_get_entries(const sp_matrix_t *m) { return m->entries; }

int matrix_get_rowcount(const sp_matrix_t *m) { return m->maxrow + 1; }

int matrix_get_colcount(const sp_matrix_t *m) { return m->maxcol + 1; }

const matrix_elem_t *matrix_row_first(sp_matrix_t *m, int row) {
  if (is_empty_row(row)) return NULL;
...
const matrix_elem_t *matrix_col_first(sp_matrix_t *m, int col) {
  if (is_empty_col(col)) return NULL;
...
static inline const matrix_elem_t *matrix_first_from(sp_matrix_t *m,
                                                     int startrow) {
...
const matrix_elem_t *matrix_next(sp_matrix_t *m) {
  assert(m->first &&
         "Start iteration with matrix_???_first, before calling me!");
...
void matrix_optimize(sp_matrix_t *m) {
  ...
  matrix_foreach(m, e) {
    ...
    assert(e->row != e->col &&
           "Root has itself as arg. Ok. But the arg (=root) will always have "
           "the same color as root");
  ...
  for (i = 0; i < size; ++i) {
    if (!bitset_is_set(fullrow, i)) matrix_fill_row(m, i, fullrow);
  }
}
...
void matrix_dump(sp_matrix_t *m, FILE *out, int factor) {
  ...
  for (i = 0; i <= m->maxrow; ++i) {
    ...
    matrix_foreach_in_row(m, i, e) {
      for (o = last_idx + 1; o < e->col; ++o) fprintf(out, " %4.1f", 0.0);

      fprintf(out, " %4.1f", factor * e->val);
      last_idx = e->col;
    }

    for (o = last_idx + 1; o <= m->maxcol; ++o) fprintf(out, " %4.1f", 0.0);
...
void matrix_self_test(int d) {
  ...
  for (int i = 0; i < d; ++i) {
    for (int o = 0; o < d; ++o) {
      if (matrix_get(m, i, o) != i * o) panic("matrix_get/set failed");
    }
  }

  int i = 1;
  matrix_foreach_in_row(m, 1, e) {
    if (e->val != i) panic("matrix row iter failed");
    ++i;
  }
  if (matrix_next(m)) panic("matrix_foreach ended early");

  i = d - 1;
  matrix_foreach_in_col(m, d - 1, e) {
    if (e->val != i) panic("matrix foreach col failed");
    i += d - 1;
  }
  if (matrix_next(m)) panic("matrix_foreach_col ended early");
  del_matrix(m);
  ...
  if (i != 4) panic("matrix foreach 2 failed");
  del_matrix(m);
  ...
  i = 0;
  matrix_foreach(m, e) {
    if (e->val != ++i) panic("matrix foreach failed");
  }
  if (i != 6) panic("matrix_foreach failed");
  matrix_set(m, 1, 1, 0);
  if (matrix_get_entries(m) != 5) panic("matrix get/set 3 failed");
  del_matrix(m);
}
