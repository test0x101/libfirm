...
static char gurobi_cst_encoding[4] = {0, GRB_EQUAL, GRB_LESS_EQUAL,
                                      GRB_GREATER_EQUAL};
...
static void gurobi_construct(gurobi_t *grb) {
  ...
  /* set start values */
  for (i = 0; i < sv_cnt; ++i) {
    error = GRBsetdblattrelement(grb->model, GRB_DBL_ATTR_START, indices[i],
                                 startv[i]);
    check_gurobi_error(grb, error);
  }
...
static void gurobi_solve(gurobi_t *grb) {
  ...
  if (lpp->time_limit_secs > 0.0) {
    error = GRBsetdblparam(grb->modelenv, GRB_DBL_PAR_TIMELIMIT,
                           lpp->time_limit_secs);
    check_gurobi_error(grb, error);
  }
...
void lpp_solve_gurobi(lpp_t *lpp) { (void)lpp; }
