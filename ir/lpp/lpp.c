...
static void update_stats(lpp_t *lpp) {
  ...
  lpp->density =
      (double)lpp->n_elems / (double)(lpp->cst_next * lpp->var_next) * 100.0;
}
...
double lpp_get_fix_costs(lpp_t *lpp) { return matrix_get(lpp->m, 0, 0); }
...
int lpp_add_cst(lpp_t *lpp, const char *cst_name, lpp_cst_t cst_type,
  ...
  if (cst_name && cst_name[0] == '_') return ERR_NAME_NOT_ALLOWED;
...
int lpp_add_var(lpp_t *lpp, const char *var_name, lpp_var_t var_type,
  ...
  if (var_name && var_name[0] == '_') return ERR_NAME_NOT_ALLOWED;
...
int lpp_set_factor_fast(lpp_t *lpp, int cst_idx, int var_idx, double value) {
  ...
  DBG((dbg, LEVEL_2, "%s[%d] %s[%d] %g\n", lpp->csts[cst_idx]->name, cst_idx,
       lpp->vars[var_idx]->name, var_idx, value));
...
lpp_sol_state_t lpp_get_solution(lpp_t *lpp, double *values, int begin,
  ...
  if (lpp->sol_state < lpp_feasible) return lpp->sol_state;

  /* here we are feasible or optimal */
  for (i = 0; i < end - begin + 1; ++i) values[i] = lpp->vars[begin + i]->value;
...
void lpp_check_startvals(lpp_t *lpp) {
  ...
  for (cst_idx = 1; cst_idx < lpp->cst_next; ++cst_idx) {
    ...
    for (var_idx = 1; var_idx < lpp->var_next; ++var_idx) {
      if (lpp->vars[var_idx]->value_kind != lpp_value_start) goto next;

      sum += lpp->vars[var_idx]->value * matrix_get(lpp->m, cst_idx, var_idx);
    }
...
void lpp_set_log(lpp_t *lpp, FILE *log) { lpp->log = log; }
...
void lpp_dump_plain(lpp_t *lpp, FILE *f) {
  ...
  for (i = 0; i < lpp->cst_next; ++i) {
    ...
    matrix_foreach_in_row(lpp->m, cst->nr, elm) {
      ...
      if (elm->col > 0) fprintf(f, "%+4.1f %-16s ", elm->val, var->name);
    }
    ...
    fprintf(f, "%3s %+4.1f\n", lpp_cst_op_to_str(cst->type.cst_type),
            matrix_get(lpp->m, cst->nr, 0));
  }

  fprintf(f, "Binary\n");
  for (i = 0; i < lpp->var_next; ++i) {
    lpp_name_t *var = lpp->vars[i];
    if (var->type.var_type == lpp_binary) fprintf(f, "%16s\n", var->name);
  }
  fprintf(f, "End\n");
}
