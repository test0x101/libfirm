...
static cpx_t *new_cpx(lpp_t *lpp) {
  ...
  chk_cpx_err(cpx);
  if (lpp->log && CPXsetlogfile(cpx->env, lpp->log)) lpp->log = NULL;
  return cpx;
}
...
static void cpx_construct(cpx_t *cpx) {
  ...
  cpx->status = CPXcopylpwnames(cpx->env, cpx->prob, numcols, numrows, objsen,
                                obj, rhs, sense, matbeg, matcnt, matind, matval,
                                lb, ub, NULL, colname, rowname);
  chk_cpx_err(cpx);
...
static void cpx_solve(cpx_t *cpx) {
  ...
  if (lpp->set_bound) {
    CPXsetdblparam(
        cpx->env,
        (lpp->opt_type == lpp_minimize ? CPX_PARAM_OBJLLIM : CPX_PARAM_OBJULIM),
        lpp->bound);
  }
