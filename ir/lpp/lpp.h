...
typedef enum _lpp_opt_t { lpp_minimize, lpp_maximize } lpp_opt_t;
...
typedef struct _lpp_t {
  ...
  double grow_factor;     /**< The factor by which the vars and constraints are
                             enlarged */

  /* Solving options */
  bool set_bound;         /**< IN: Boolean flag to set a bound for the objective
                             function. */
  double bound;           /**< IN: The bound. Only valid if set_bound == 1. */
  double time_limit_secs; /**< IN: Time limit to obey while solving (0.0 means
                             no time limit) */

  /* Solution stuff */
  lpp_sol_state_t sol_state; /**< State of the solution */
  double sol_time;           /**< Time in seconds */
  unsigned iterations; /**< Number of iterations CPLEX needed to solve the ILP
                          (whatever this means) */

  char *error;
  unsigned next_name_number; /**< for internal use only */
  lpp_emphasis_t emphasis;   /**< On what should CPLEX concentrate (feasibility,
                                bestbound, ...) */
 ...
 */
lpp_t *lpp_new_userdef(const char *name, lpp_opt_t opt_type, int estimated_vars,
                       int estimated_csts, double grow_factor);
 ...
 * @param cst_name The name of the constraint (1st char only alpha-numeric!). If
 * NULL, a default name will be used.
 * @param cst_type The type of constraint: objective, equality, less-or-equal,
 * greater-or-equal
 * @param rhs The right hand side value to set for this constraint.
 * @return The (new or existing) index of the constraint
 */
int lpp_add_cst(lpp_t *lpp, const char *cst_name, lpp_cst_t cst_type,
                double rhs);
 ...
 * @return The (new or existing) index of the constraint
 */
int lpp_add_cst_uniq(lpp_t *lpp, const char *cst_name, lpp_cst_t cst_type,
                     double rhs);

/**
 * Returns the internal index of a constraint.
 * @param cst_name The name of the constraint
 * @return The internal index of constraint @p cst_name or -1 if it does not
 * exist.
 */
 ...
 * exists nothing is altered, and the index of the existing entry is returned.
 * @param var_name The name of the constraint (1st char only alpha-numeric!). If
 * NULL, a default name will be used.
 ...
 */
int lpp_add_var(lpp_t *lpp, const char *var_name, lpp_var_t var_type,
                double obj);

/**
 * Same as lpp_add_var() but the user can supply a default value.
 */
int lpp_add_var_default(lpp_t *lpp, const char *var_name, lpp_var_t var_type,
                        double obj, double startval);

/**
 * Returns the internal index of a variable.
 * @param cst_name The name of the variable
 * @return The internal index of variable @p var_name or -1 if it does not
 * exist.
 */
...
/**
 * Sets the factor of the variable @p var_name in constraint @p cst_name to @p
 * value.
 * @return -1 if constraint or variable name does not exist.
 *          0 otherwise
 */
int lpp_set_factor(lpp_t *lpp, const char *cst_name, const char *var_name,
                   double value);
...
int lpp_set_factor_fast_bulk(lpp_t *lpp, int cst_idx, int *var_idx,
                             int num_vars, double value);
 ...
 */
lpp_sol_state_t lpp_get_solution(lpp_t *lpp, double *values, int begin,
                                 int end);
 ...
 * @param lpp The problem.
 */
static inline void lpp_unset_bound(lpp_t *lpp) { lpp->set_bound = false; }
