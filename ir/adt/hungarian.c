...
int hungarian_solve(hungarian_problem_t *const p, unsigned *const assignment,
  ...
  for (unsigned r = 0; r < num_rows; ++r) {
    ...
    for (unsigned c = 0; c < num_cols; ++c) {
      if (cost[r * num_cols + c] != row_minimum) continue;
      if (row_mate[c] != ~0u) continue;
  ...
  for (;;) {
    ...
    for (;;) {
      ...
      int s = INT_MAX;
      for (unsigned c = 0; c < num_cols; ++c) {
        if (slack[c] && slack[c] < s) s = slack[c];
      }
      ...
      for (unsigned c = 0; c < num_cols; ++c) {
        if (slack[c]) {
          ...
          if (slack[c] == 0) {
            ...
            DBG((dbg, LEVEL_1,
                 "Decreasing uncovered elements by %d produces zero at [%u, "
                 "%u]\n",
                 s, r, c));
            if (row_mate[c] == ~0u) {
  ...
  for (unsigned r = 0; r < num_rows; ++r) res_cost += row_dec[r];

  for (unsigned c = 0; c < num_cols; ++c) res_cost -= col_inc[c];

  DBG((dbg, LEVEL_1, "Cost is %d\n", res_cost));

ret:
  if (final_cost != NULL) *final_cost = res_cost;
