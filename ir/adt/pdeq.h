...
#define deq_pop_pointer_left(type, deq) ((type *)deq_pop_pointer_left(deq))
...
#define deq_pop_pointer_right(type, deq) ((type *)deq_pop_pointer_right(deq))
...
static inline bool deq_pointer_iter_at_end(
    deq_pointer_iter_t const *const iter) {
    ...
    for (deq_pointer_iter_t pointer##__iter; pointer##__once;       \
      ...
      for (deq_pointer_iter_init(&pointer##__iter, deq);            \
        ...
        for (bool pointer##__once2 = true; pointer##__once2;)       \
          for (type *pointer = *((type **)pointer##__iter.block_p); \
               pointer##__once2; pointer##__once2 = false)
