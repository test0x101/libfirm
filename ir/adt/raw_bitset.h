...
#define BITSET_SIZE_ELEMS(size_bits) \
  ((size_bits + BITS_PER_ELEM - 1) / BITS_PER_ELEM)
#define BITSET_SIZE_BYTES(size_bits) \
  (BITSET_SIZE_ELEMS(size_bits) * sizeof(unsigned))
...
#define rbitset_alloca(size)                              \
  ((unsigned *)memset(alloca(BITSET_SIZE_BYTES(size)), 0, \
                      BITSET_SIZE_BYTES(size)))
 ...
 * @return the new bitset
 */
static inline unsigned *rbitset_duplicate_obstack_alloc(
    struct obstack *obst, const unsigned *old_bitset, size_t size) {
 ...
 */
static inline void rbitset_and(unsigned *dst, const unsigned *src,
                               size_t size) {
  for (size_t i = 0, n = BITSET_SIZE_ELEMS(size); i < n; ++i) {
    dst[i] &= src[i];
  }
 ...
 * @param size  size of both bitsets in bits
 */
static inline void rbitset_xor(unsigned *dst, const unsigned *src,
                               size_t size) {
  for (size_t i = 0, n = BITSET_SIZE_ELEMS(size); i < n; ++i) {
    dst[i] ^= src[i];
  }
 ...
 */
static inline void rbitset_set_range(unsigned *bitset, size_t from, size_t to,
                                     bool val) {
  /*
  ...
  if (val) {
    if (from_pos == to_pos) {
      ...
      for (size_t i = from_pos + 1; i < to_pos; ++i) bitset[i] = ~0u;
    }
  } else {
    /* ... or clear them? */
    if (from_pos == to_pos) {
      ...
      bitset[from_pos] &= ~from_unit_mask;
      bitset[to_pos] &= ~to_unit_mask;
      for (size_t i = from_pos + 1; i < to_pos; ++i) bitset[i] = 0;
    }
  }
}
...
static inline bool rbitsets_have_common(const unsigned *bitset1,
                                        const unsigned *bitset2, size_t size) {
  for (size_t i = 0, n = BITSET_SIZE_ELEMS(size); i < n; ++i) {
    if ((bitset1[i] & bitset2[i]) != 0) return true;
  }
  return false;
}
...
static inline bool rbitset_contains(const unsigned *bitset1,
                                    const unsigned *bitset2, size_t size) {
  for (size_t i = 0, n = BITSET_SIZE_ELEMS(size); i < n; ++i) {
    if ((bitset1[i] & bitset2[i]) != bitset1[i]) return false;
  }
  return true;
}
...
static inline void rbitset_minus1(unsigned *bitset, size_t size) {
  for (size_t i = 0, n = BITSET_SIZE_ELEMS(size); i < n; ++i) {
    unsigned mask = i == n - 1 ? rbitset_last_mask_(size) : ~0u;
    unsigned val = bitset[i] & mask;
    unsigned val_minus1 = val - 1;
    bitset[i] = val_minus1 & mask;

    if (((val >> (BITS_PER_ELEM - 1)) ^ (val_minus1 >> (BITS_PER_ELEM - 1))) ==
        0)
      break;
  }
}
...
#define rbitset_foreach(bitset, size, elm) \
  for (size_t elm = 0;                     \
       (elm = rbitset_next_max((bitset), elm, size, 1)) != (size_t)-1; ++elm)
...
#define rbitset_foreach_clear(bitset, size, elm) \
  for (size_t elm = 0;                           \
       (elm = rbitset_next_max((bitset), elm, size, 0)) != (size_t)-1; ++elm)
