 ...
 *                                      the Null value</li>
 *  <li><b>ADDITIONAL_DATA<b>   Additional fields appended to the hashset
 * struct</li>
 * </ul>
 */
#ifdef HashSet
...
static inline FindReturnValue insert_nogrow(HashSet *self, KeyType key) {
  ...
  for (;;) {
    ...
    if (EntryIsDeleted(*entry)) {
      if (insert_pos == ILLEGAL_POS) insert_pos = bucknum;
...
static inline void resize(HashSet *self, size_t new_size) {
  ...
  for (i = 0; i < num_buckets; ++i) {
    ...
    if (EntryIsEmpty(*entry) || EntryIsDeleted(*entry)) continue;
...
static inline void maybe_grow(HashSet *self) {
  if (LIKELY(self->num_elements + 1 <= self->enlarge_threshold)) return;
...
static inline void maybe_shrink(HashSet *self) {
  ...
  if (!self->consider_shrink) return;

  self->consider_shrink = 0;
  size = hashset_size(self);
  if (size <= HT_MIN_BUCKETS) return;

  if (LIKELY(size > self->shrink_threshold)) return;

  resize_to = ceil_po2(size);

  if (resize_to < 4) resize_to = 4;
...
/**
 * Insert an element into the hashset. If no element with the given key exists
 * yet, then a new one is created and initialized with the InitData function.
 * Otherwise the existing element is returned (for hashs where key is equal to
 * value, nothing is returned.)
 *
 * @param self   the hashset
 * @param key    the key that identifies the data
 * @returns      the existing or newly created data element (or nothing in case
 * of hashs where keys are the while value)
 */
 ...
 * already allocated.
 */
void hashset_init(HashSet *self) { init_size(self, HT_MIN_BUCKETS); }
#endif
...
ValueType hashset_iterator_next(HashSetIterator *self) {
  ...
  do {
    current_bucket++;
    if (current_bucket >= end) return NullValue;
...
void hashset_remove_iterator(HashSet *self, const HashSetIterator *iter) {
  ...
  if (EntryIsDeleted(*entry)) return;
