...
bipartite_t *bipartite_new(unsigned const n_left, unsigned const n_right) {
  ...
  for (unsigned i = 0; i < n_left; ++i) gr->adj[i] = bitset_malloc(n_right);

  return gr;
}

void bipartite_free(bipartite_t *const gr) {
  for (unsigned i = 0; i < gr->n_left; ++i) free(gr->adj[i]);
  free(gr);
}
...
static int apply_alternating_path(bipartite_t const *const gr,
  ...
  for (unsigned left = 0; left < gr->n_left; ++left) {
    ...
    if (matching[left] >= 0) {
      ...
      if (bitset_contains(left_adj, matched_right)) continue;
      ...
      for (i = 0; i < gr->n_left; ++i)
        if (i != left && bitset_is_set(gr->adj[i], old_right) &&
            !bitset_is_set(matched_left, i))
          break;

      /* If no such node can be found, exit. */
      if (i >= gr->n_left) continue;
      ...
      bitset_andnot(tmp, matched_right);
      if (bitset_is_empty(tmp)) continue;
