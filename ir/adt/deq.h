...
static inline void deq_shrink_left(deq_t *const deq, unsigned size) {
  deq_block_t *const left_end = deq->left_end;
  left_end->l += size;
  if (left_end->l >= left_end->r) deq_left_end_block_empty_priv(deq);
}
...
static inline void deq_shrink_right(deq_t *const deq, unsigned size) {
  ...
  right_end->r -= size;
  if (right_end->r <= right_end->l) deq_right_end_block_empty_priv(deq);
}
...
static inline void *deq_alloc_left(deq_t *const deq, unsigned size) {
  ...
  if (left_end->l < size) left_end = deq_append_block_left_priv(deq);
  left_end->l -= size;
  return left_end->data + left_end->l;
}
...
static inline deq_block_t const *deq_get_left_end_block(
    deq_t const *const deq) {
  return deq->left_end;
}

static inline deq_block_t *deq_block_next_right(
    deq_block_t const *const block) {
