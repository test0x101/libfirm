...
static void alloc_rows(gs_matrix_t *m, unsigned c_rows, unsigned c_cols,
  ...
  for (unsigned i = begin_init; i < c_rows; ++i) {
    ...
    m->rows[i].diag = 0.0;
    m->rows[i].cols = NULL;
    if (c_cols > 0) alloc_cols(&m->rows[i], c_cols);
  }
}
...
void gs_delete_matrix(gs_matrix_t *m) {
  for (unsigned i = 0; i < m->c_rows; ++i) {
    if (m->rows[i].c_cols) free(m->rows[i].cols);
  }
  if (m->c_rows) free(m->rows);
  free(m);
}
...
double gs_matrix_get(const gs_matrix_t *m, unsigned row, unsigned col) {
  assert(row < m->c_rows);
  row_col_t *the_row = &m->rows[row];
  if (row == col) return the_row->diag != 0.0 ? 1.0 / the_row->diag : 0.0;
  ...
  if (c >= the_row->n_cols || the_row->cols[c].col_idx > col) return 0.0;
 ...
 * This solves m*x=0. You must add stuff for m*x=b. See wikipedia german and
 * english article. Should be simple. param a is the number of rows in the
 * matrix that should be considered.
 *
 * Note that the diagonal element is stored separately in this matrix
 * implementation.
 * */
