...
pmap *pmap_create(void) { return pmap_create_ex(INITIAL_SLOTS); }

void pmap_destroy(pmap *map) { del_set(M2S(map)); }

void pmap_insert(pmap *map, const void *key, void *value) {
  pmap_entry entry, *p;

  entry.key = key;
  p = set_insert(pmap_entry, M2S(map), &entry, sizeof(pmap_entry),
                 hash_ptr(key));
  p->value = value;
}
...
size_t pmap_count(pmap const *map) { return set_count(M2S(map)); }

pmap_entry *pmap_first(pmap *map) { return set_first(pmap_entry, M2S(map)); }

pmap_entry *pmap_next(pmap *map) { return set_next(pmap_entry, M2S(map)); }

void pmap_break(pmap *map) { set_break(M2S(map)); }
