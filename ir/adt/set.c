...
size_t MANGLEP(count)(SET const *table) { return table->nkey; }
...
void *(MANGLEP(next))(SET *table) {
  if (!table->iter_tail) return NULL;
...
void MANGLEP(break)(SET *table) { table->iter_tail = NULL; }
...
static void expand_table(SET *table) {
  if (table->maxp + table->p >= (DIRECTORY_SIZE << SEGMENT_SIZE_SHIFT)) return;
...
void *MANGLE(_, _search)(SET *table, void const *key,
  ...
  if (!q && (action != MANGLE(_, _find))) { /* not found, insert */
    assert(!table->iter_tail &&
           "insert an element into a set that is iterated");
...
int pset_default_ptr_cmp(void const *x, void const *y) { return x != y; }
...
void pset_insert_pset_ptr(pset *target, pset *src) {
  foreach_pset(src, void, elt) { pset_insert_ptr(target, elt); }
}
