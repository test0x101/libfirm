...
static dfs_edge_t *get_edge(dfs_t const *const self, ir_node const *const src,
...
static void dfs_perform(dfs_t *dfs, ir_node *n, dfs_node_t const *anc,
                        int level) {
  ...
  foreach_block_succ(n, edge) {
    ...
    if (!child->visited) dfs_perform(dfs, p, node, level + 1);

    /* get the maximum pre num of the subtree. needed for ancestor
     * determination. */
...
dfs_edge_kind_t dfs_get_edge_kind(dfs_t const *const dfs,
                                  ir_node const *const a,
...
static int node_level_cmp(const void *a, const void *b) {
  const dfs_node_t *p = *(const dfs_node_t **)a;
  const dfs_node_t *q = *(const dfs_node_t **)b;
  if (p->level == q->level) return p->pre_num - q->pre_num;
...
void dfs_dump(const dfs_t *dfs, FILE *file) {
  ...
  int n = 0;
  foreach_set(dfs->nodes, dfs_node_t, node) { nodes[n++] = node; }
  ...
  for (int i = 0; i < n; ++i) {
    ...
    ir_fprintf(file, "\tn%d [label=\"%d\"]\n", node->pre_num,
               get_Block_dom_tree_pre_num((ir_node *)node->node));
  }

  foreach_set(dfs->edges, dfs_edge_t, edge) dfs_dump_edge(edge, file);
