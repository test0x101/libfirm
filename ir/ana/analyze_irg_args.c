...
static ptr_access_kind analyze_arg(ir_node *arg, ptr_access_kind bits) {
  ...
  foreach_irn_out_r(arg, i, succ) {
    if (irn_visited(succ)) continue;

    /* We should not walk over the memory edge.*/
    if (get_irn_mode(succ) == mode_M) continue;
    ...
    switch (get_irn_opcode(succ)) {
      case iro_Call: {
        ...
        if (ptr == arg) {
          ...
          if (callee != NULL) {
          ...
          } else if (is_Member(ptr) && get_irp_callee_info_state() ==
            ...
            for (int c = cg_get_call_n_callees(succ); c-- > 0;) {
              ...
              /* unknown_entity is used to signal that we don't know what is
               * called */
     ...
     */
    if (!mode_is_reference(get_irn_mode(succ))) continue;
...
static void analyze_ent_args(ir_entity *ent) {
  ...
  for (size_t i = nparams; i-- > 0;) {
    ...
    ent->attr.mtd_attr.param_access[i] =
        is_Pointer_type(type) ? ptr_access_all : ptr_access_none;
  }
  ...
  for (size_t i = nparams; i-- > 0;) rw_info[i] = ptr_access_none;
...
void analyze_irg_args(ir_graph *irg) {
  if (irg == get_const_code_irg()) return;

  ir_entity *entity = get_irg_entity(irg);
  if (entity == NULL) return;

  if (!entity->attr.mtd_attr.param_access) analyze_ent_args(entity);
}
...
enum args_weight {
  ...
  binop_weight =
      1, /**< If the argument have mode_weight and take part in binop. */
  const_binop_weight = 1, /**< If the argument have mode_weight and take part in
                             binop with a constant.*/
  cmp_weight = 4,         /**< If the argument take part in cmp. */
  const_cmp_weight =
      10, /**< If the argument take part in cmp with a constant. */
  indirect_call_weight =
      125, /**< If the argument is the address of an indirect Call. */
};
...
unsigned get_method_param_weight(ir_entity *ent, size_t pos) {
  if (!ent->attr.mtd_attr.param_weight) analyze_method_params_weight(ent);
...
void analyze_irg_args_weight(ir_graph *irg) {
  ...
  if (entity->attr.mtd_attr.param_weight != NULL) return;
