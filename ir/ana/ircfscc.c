...
static inline void init_node(ir_node *n, void *env) {
  ...
  if (is_Block(n)) set_irn_link(n, new_scc_info(obst));
  clear_backedges(n);
}
...
static inline void finish_scc(void) { finish_stack(); }
  ...
  if (get_irn_dfn(n) == get_irn_uplink(n)) {
    ...
    if (tail) {
      ...
      if ((get_loop_n_elements(current_loop) > 0) ||
          (is_outermost_loop(current_loop))) {
        l = new_loop();
        close = true;
      } else {
        l = current_loop;
...
static void reset_backedges(ir_node *n) {
  if (is_possible_loop_head(n)) clear_backedges(n);
}
...
void assure_loopinfo(ir_graph *irg) {
  if (irg_has_properties(irg, IR_GRAPH_PROPERTY_CONSISTENT_LOOPINFO)) return;
