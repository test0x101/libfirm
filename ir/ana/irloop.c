...
unsigned(get_loop_depth)(const ir_loop *loop) { return _get_loop_depth(loop); }
...
void set_irn_loop(ir_node *n, ir_loop *loop) { n->loop = loop; }

ir_loop *(get_irn_loop)(const ir_node *n) { return _get_irn_loop(n); }
...
void(set_irg_loop)(ir_graph *irg, ir_loop *loop) { _set_irg_loop(irg, loop); }

ir_loop *(get_irg_loop)(const ir_graph *irg) { return _get_irg_loop(irg); }
...
static bool is_loop_variant(ir_loop *l, ir_loop *b) {
  ...
  for (size_t i = 0, n_elems = get_loop_n_elements(l); i < n_elems; ++i) {
    ...
    if (is_ir_loop(e.kind) && is_loop_variant(e.son, b)) return true;
  }
