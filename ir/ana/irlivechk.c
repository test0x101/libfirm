 ...
 * @brief
 *
 * Liveness checks as developed by Benoit Boissinot, Fabrice Rastello and
 * myself.
 *
 * The speciality here is, that nothing has to be recomputed if new nodes are
 * created or old ones deleted.
 *
 * This algo has one core routine check_live_end_internal() which performs the
 * liveness check. It only relies on the precomputation done in the constructor,
 * which in turn needs:
 * - out edges
 * - the dominance tree
  ...
  bitset_foreach(tmp, elm) {
    ...
    foreach_block_succ(si->block, edge) {
      ...
      if (kind == DFS_EDGE_BACK && !bitset_is_set(bi->red_reachable, ti->id)) {
        if (!ti->be_tgt_calc) compute_back_edge_chain(lv, tgt);
...
lv_chk_t *lv_chk_new(ir_graph *irg) {
  ...
  /* first of all, compute the transitive closure of the CFG *without* back
   * edges */
...
unsigned lv_chk_bl_xxx(lv_chk_t *lv, const ir_node *bl, const ir_node *var) {
  ...
  if (def_bl == bl) {
    ...
    foreach_out_edge(var, edge) {
      ...
      if (!is_liveness_node(user)) continue;
    ...
    while (i <= max_dom) {
      ...
      if (ti == bli && !bitset_is_set(lv->back_edge_tgt, ti->id)) {
        DBG((lv->dbg, LEVEL_2,
             "\tlooking not from a back edge target and q == t. removing use: "
             "%d\n",
             ti->id));
       ...
       */
      if (use_in_current_block) bitset_set(uses, ti->id);
