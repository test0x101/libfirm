...
int cg_call_has_callees(const ir_node *node) {
  assert(is_Call(node));
  return (
      (get_irg_callee_info_state(get_irn_irg(node)) != irg_callee_info_none) &&
...
void cg_set_call_callee_arr(ir_node *node, size_t n, ir_entity **arr) {
  ...
  if (node->attr.call.callee_arr == NULL || cg_get_call_n_callees(node) != n) {
    ...
    node->attr.call.callee_arr =
        NEW_ARR_D(ir_entity *, get_irg_obstack(irg), n);
  }
...
static ir_entity **get_impl_methods(ir_entity *method) {
  ...
  if (size > 0) {
    ...
    foreach_pset(set, ir_entity, ent) { arr[--size] = ent; }
  }
  del_pset(set);
  return arr;
}
...
static void sel_methods_walker(ir_node *node, void *env) {
  ...
  if (!is_method_entity(entity)) return;
 ...
 */
static void free_mark(ir_node *node, pset *set) {
  if (get_irn_link(node) == MARK) return; /* already visited */
 ...
 */
static void add_method_address_inititializer(
    ir_initializer_t const *const initializer, pset *const set) {
  switch (initializer->kind) {
    case IR_INITIALIZER_CONST: {
      ...
      if (is_Address(n)) {
        ...
        if (is_Method_type(get_entity_type(ent))) pset_insert_ptr(set, ent);
      }
      return;
    }
...
static void add_method_address(ir_entity *ent, pset *set) {
  if (get_entity_kind(ent) == IR_ENTITY_NORMAL) {
    ...
    if (init) add_method_address_inititializer(init, set);
  }
}
...
static size_t get_free_methods(ir_entity ***free_methods) {
  ...
  foreach_irp_irg(i, irg) {
    ...
    if (entity_is_externally_visible(ent)) pset_insert_ptr(free_set, ent);
    ...
    case iro_Proj: {
      ...
      if (get_irn_link(pred) != MARK) {
        if (is_Tuple(pred)) {
          ...
          pset_insert_ptr(methods,
                          get_unknown_entity()); /* free method -> unknown */
        }
      }
      break;
    }
...
static void callee_ana_node(ir_node *node, pset *methods) {
  ...
  switch (get_irn_opcode(node)) {
    ...
    case iro_Address: {
      ...
      if (is_method_entity(ent)) pset_insert_ptr(methods, ent);
      break;
    }

    case iro_Member: {
      ir_entity *entity = get_Member_entity(node);
      if (!is_method_entity(entity)) break;
      /* polymorphic method */
      for (size_t i = 0, n = get_member_n_methods(node); i < n; ++i) {
...
static void destruct_walker(ir_node *node, void *env) {
  (void)env;
  if (is_Call(node)) cg_remove_call_callee_arr(node);
}
...
void free_irp_callee_info(void) {
  foreach_irp_irg(i, irg) { free_callee_info(irg); }
}
