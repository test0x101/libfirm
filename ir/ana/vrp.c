...
vrp_attr *vrp_get_info(const ir_node *node) {
  ...
  if (irg->vrp.infos.data == NULL) return NULL;
...
static int vrp_update_node(ir_vrp_info *info, ir_node *node) {
  ...
  switch (get_irn_opcode(node)) {
    ...
    case iro_Add: {
      ...
      const vrp_attr *vrp_right =
          vrp_get_or_set_info(info, get_Add_right(node));
    ...
    case iro_Sub: {
      ...
      if (!mode_is_int(get_irn_mode(left))) return 0;
      ...
      if (vrp_left->range_type == VRP_RANGE &&
        ...
        tarval_set_wrap_on_overflow(false);
        ir_tarval *new_top =
            tarval_sub(vrp_left->range_top, vrp_right->range_bottom);
        ir_tarval *new_bottom =
            tarval_sub(vrp_left->range_bottom, vrp_right->range_top);
        tarval_set_wrap_on_overflow(old_wrap_on_overflow);
    ...
    case iro_Or: {
      ...
      new_bits_not_set =
          tarval_or(vrp_left->bits_not_set, vrp_right->bits_not_set);
    ...
    case iro_Shl: {
      ...
      if (is_Const(right)) {
        ...
        new_bits_not_set =
            tarval_shl(vrp_left->bits_not_set, get_Const_tarval(right));
      }
      break;
    }
    ...
    case iro_Shr: {
      ...
      if (is_Const(right)) {
        ...
        new_bits_not_set =
            tarval_shr(vrp_left->bits_not_set, get_Const_tarval(right));
      }
      break;
    }
    ...
    case iro_Shrs: {
      ...
      if (is_Const(right)) {
        ...
        new_bits_not_set =
            tarval_shrs(vrp_left->bits_not_set, get_Const_tarval(right));
      }
      break;
    }
    ...
    case iro_Eor: {
      ...
      new_bits_not_set = tarval_not(
          tarval_or(tarval_and(vrp_left->bits_set, vrp_right->bits_set),
    ...
    case iro_Conv: {
      ...
      if (!mode_is_int(old_mode)) return 0;
      ...
      new_bits_not_set =
          tarval_convert_to(get_mode_all_one(old_mode), new_mode);
      new_bits_not_set =
          tarval_and(new_bits_not_set,
                     tarval_convert_to(vrp_pred->bits_not_set, new_mode));
      ...
      if (tarval_cmp(vrp_pred->range_top, get_mode_max(new_mode)) ==
       ...
       */
      if (tarval_cmp(vrp_pred->range_bottom, get_mode_min(new_mode)) ==
    ...
    case iro_Phi: {
      ...
      for (int i = 1, num = get_Phi_n_preds(node); i < num; i++) {
        ...
        new_bits_not_set = tarval_or(new_bits_not_set, vrp_pred->bits_not_set);
      }
  ...
  */
  ...
  if (new_bits_set != tarval_bad &&
      get_tarval_mode(new_bits_set) != get_tarval_mode(vrp->bits_set)) {
    vrp->bits_set = tarval_convert_to(vrp->bits_set, get_irn_mode(node));
  }
  if (new_bits_not_set != tarval_bad &&
      get_tarval_mode(new_bits_not_set) != get_tarval_mode(vrp->bits_not_set)) {
    vrp->bits_not_set =
        tarval_convert_to(vrp->bits_not_set, get_irn_mode(node));
  }

  if (vrp->range_type != VRP_UNDEFINED && new_range_type != VRP_UNDEFINED &&
      get_tarval_mode(new_range_top) != get_tarval_mode(vrp->range_top)) {
  ...
  if (vrp->range_type == VRP_UNDEFINED && new_range_type != VRP_UNDEFINED) {
    ...
    if (new_range_type == VRP_ANTIRANGE) {
      ...
      if (tarval_cmp(vrp->range_bottom, new_range_top) == ir_relation_greater &&
          tarval_cmp(vrp->range_bottom, new_range_bottom) ==
              ir_relation_greater) {
        something_changed = true;
        vrp->range_bottom = new_range_top;

      } else if (tarval_cmp(vrp->range_top, new_range_bottom) ==
                     ir_relation_greater &&
                 tarval_cmp(vrp->range_top, new_range_top) ==
                     ir_relation_less) {
        something_changed = true;
    ...
    if (new_range_type == VRP_ANTIRANGE) {
      if (tarval_cmp(vrp->range_bottom, new_range_bottom) ==
          ir_relation_greater) {
        something_changed = true;
        vrp->range_bottom = new_range_bottom;
      }
  ...
  assert(
      tarval_is_null(tarval_and(vrp->bits_set, tarval_not(vrp->bits_not_set))));
...
static void dump_vrp_info(void *ctx, FILE *F, const ir_node *node) {
  (void)ctx;
  if (!mode_is_int(get_irn_mode(node))) return;
...
void set_vrp_data(ir_graph *irg) {
  if (irg->vrp.infos.data != NULL) free_vrp_data(irg);
...
void free_vrp_data(ir_graph *irg) {
  if (irg->vrp.infos.data == NULL) return;
  obstack_free(&irg->vrp.obst, NULL);
  ir_nodemap_destroy(&irg->vrp.infos);
}

ir_relation vrp_cmp(const ir_node *left, const ir_node *right) {
  if (!mode_is_int(get_irn_mode(left))) return ir_relation_true;

  vrp_attr *vrp_left = vrp_get_info(left);
  vrp_attr *vrp_right = vrp_get_info(right);
  if (vrp_left == NULL || vrp_right == NULL) return ir_relation_true;

  if (vrp_left->range_type == VRP_RANGE && vrp_right->range_type == VRP_RANGE) {
    if (tarval_cmp(vrp_left->range_top, vrp_right->range_bottom) ==
        ir_relation_less) {
      return ir_relation_less;
    }
    if (tarval_cmp(vrp_left->range_bottom, vrp_right->range_top) ==
        ir_relation_greater) {
      return ir_relation_greater;
    }
  }

  if (!tarval_is_null(tarval_and(vrp_left->bits_set,
                                 tarval_not(vrp_right->bits_not_set))) ||
      !tarval_is_null(tarval_and(tarval_not(vrp_left->bits_not_set),
                                 vrp_right->bits_set))) {
