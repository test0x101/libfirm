 ...
 */
static void handle_case(ir_node *block, ir_node *switchn, unsigned pn,
                        env_t *env) {
  if (!env->optimize) {
    return;
  }
  ...
  if (pn == pn_Switch_default) return;
 ...
 */
static void handle_modeb(ir_node *block, ir_node *selector, pn_Cond pnc,
                         env_t *env) {
  if (!env->optimize) {
    return;
  }
  ...
  foreach_out_edge_safe(selector, edge) {
    ...
    if (block_dominates(block, user_blk)) {
      ...
      if (con == NULL) {
        ...
        con = new_r_Const(irg,
                          pnc == pn_Cond_true ? tarval_b_true : tarval_b_false);
      }
      ...
      if (other_blk == NULL) {
        ...
        ir_node *const proj =
            get_Proj_for_pn(cond, pn_Cond_false + pn_Cond_true - pnc);
        assert(proj);
        other_blk = get_edge_src_irn(get_irn_out_edge_first(proj));

        /*
         * Note the special case here: if block is a then, there might be no
         * else block. In that case the other_block is the user_blk itself and
         * pred_block is the cond_block ...
         *
 ...
 * @param env     statistical environment
 */
static void handle_if(ir_node *block, ir_node *cmp, ir_relation rel,
                      env_t *env) {
  /* Beware of Bads */
  ir_node *left = get_Cmp_left(cmp);
  ir_node *right = get_Cmp_right(cmp);
  if (is_Bad(left) || is_Bad(right)) return;

  /* Do not create Confirm nodes for Cmp(Const, Const) constructs.
     These are removed anyway */
  if (is_Const(left) && is_Const(right)) return;
  ...
  if (env->optimize && rel == ir_relation_equal) {
    ...
    foreach_out_edge_safe(left, edge) {
      ...
      if (block_dominates(block, blk)) {
        ...
        env->num_eq += 1;
      } else if (block_dominates(blk, cond_block) && is_Const(right) &&
                 !get_irn_pinned(user)) {
        /*
    ...
    foreach_out_edge_safe(left, edge) {
      ...
      if (block_dominates(block, blk)) {
         ...
         */
        if (c == NULL) c = new_r_Confirm(block, left, right, rel);
    ...
    if (!is_Const(right)) {
      ...
      foreach_out_edge_safe(right, edge) {
        ...
        if (block_dominates(block, blk)) {
           ...
           */
          if (rc == NULL) rc = new_r_Confirm(block, right, left, rel);
...
static void insert_Confirm_in_block(ir_node *block, void *data) {
   ...
   * predecessor yet.
   */
  if (get_Block_n_cfgpreds(block) != 1) return;
  ...
  if (is_Switch(cond)) {
    ...
    if (!is_Cmp(selector)) return;

    ir_relation rel = get_Cmp_relation(selector);

    /* it's the false branch */
    if (get_Proj_num(proj) != pn_Cond_true) rel = get_negated_relation(rel);
...
void construct_confirms(ir_graph *irg) { do_construct_confirms(irg, true); }
