...
ir_disambiguator_options get_irg_memory_disambiguator_options(
  ...
  if (opt & aa_opt_inherited) return global_mem_disamgig_opt;
  return opt;
}
...
static const ir_node *find_base_addr(const ir_node *node, ir_entity **pEnt) {
  ...
  for (;;) {
    if (is_Sel(node)) {
      ...
      if (is_Proj(pred) && pred == get_irg_frame(get_irn_irg(pred))) break;
      member = node;
      node = pred;
    } else {
      break;
  ...
  if (member != NULL) *pEnt = get_Member_entity(member);
  return node;
}
 ...
 */
static bool type_contains(const ir_type *compound, const ir_type *member) {
  if (is_Array_type(compound)) {
    ir_type *elem = get_array_element_type(compound);
    return elem == member || type_contains(elem, member);
    ...
    for (size_t pos = 0; pos < n; pos++) {
      ...
      if (pos_type == member || type_contains(pos_type, member)) {
        return true;
      }
    }
    return false;
...
ir_storage_class_class_t classify_pointer(const ir_node *const addr,
   ...
   * a compound type */
  if (is_Sel(addr) || is_Member(addr)) res |= ir_sc_modifier_obj_comp;
  return res;
...
static ir_alias_relation _get_alias_relation(
  ...
  if (addr1 == addr2) return ir_sure_alias;
  ir_graph *const irg = get_irn_irg(addr1);
  unsigned const options = get_irg_memory_disambiguator_options(irg);
  if (options & aa_opt_always_alias) return ir_may_alias;
  /* The Armageddon switch */
  if (options & aa_opt_no_alias) return ir_no_alias;
  ...
  if (addr1 == addr2 && info1.sym_offset == info2.sym_offset &&
      info1.has_const_offset && info2.has_const_offset) {
    ...
    return first_offset + first_size <= last_offset ? ir_no_alias
                                                    : ir_sure_alias;
  }
  ...
  if (ent1 != NULL && ent2 != NULL) {
    if (ent1 == ent2) {
      if (base1 == base2) return ir_sure_alias;
      goto check_classes;
    }
...
check_classes:;
  ...
  pointer:
    ...
    if (other_mod & ir_sc_modifier_nottaken) return ir_no_alias;
    /* the null pointer aliases nothing */
    if (other_class == ir_sc_null) return ir_no_alias;
  ...
  if (options & aa_opt_type_based) {
    ...
    if (rel != ir_may_alias) return rel;
...
ir_alias_relation get_alias_relation(const ir_node *const addr1,
                                     const ir_type *const type1, unsigned size1,
                                     const ir_node *const addr2,
                                     const ir_type *const type2,
                                     unsigned size2) {
  ir_alias_relation rel =
      _get_alias_relation(addr1, type1, size1, addr2, type2, size2);
  ...
  for (int i = get_irn_n_outs(irn); i-- > 0;) {
    ...
    switch (get_irn_opcode(succ)) {
      case iro_Load:
        ...
        if (is_hidden_cast(mode, emode)) res |= ir_usage_reinterpret_cast;
        break;
...
static void analyse_irg_entity_usage(ir_graph *irg) {
  ...
  for (size_t i = 0, n = get_compound_n_members(frame_type); i < n; ++i) {
    ...
    if (is_method_entity(ent)) continue;
...
static void check_initializer_value(ir_node *value) {
  ...
  if (irn_visited_else_mark(value)) return;
  ...
  foreach_irn_in(value, i, op) { check_initializer_value(op); }
}
...
static void check_initializer(ir_entity *ent) {
  if (get_entity_kind(ent) == IR_ENTITY_NORMAL) {
    ...
    if (init != NULL) check_initializer_nodes(init);
  }
}
...
static void print_entity_usage_flags(const ir_type *tp) {
  for (size_t i = 0, n = get_compound_n_members(tp); i < n; ++i) {
    ...
    if (flags == 0) continue;
    ir_printf("%+F:", ent);
    if (flags & ir_usage_address_taken) printf(" address_taken");
    if (flags & ir_usage_read) printf(" read");
    if (flags & ir_usage_write) printf(" write");
    if (flags & ir_usage_reinterpret_cast) printf(" reinterp_cast");
    printf("\n");
  }
}
#endif /* DEBUG_libfirm */
...
static void analyse_irp_globals_entity_usage(void) {
  ...
  foreach_irp_irg(i, irg) {
    assure_irg_properties(
        irg, IR_GRAPH_PROPERTY_CONSISTENT_OUTS | IR_GRAPH_PROPERTY_NO_TUPLES);
...
void assure_irp_globals_entity_usage_computed(void) {
  if (irp->globals_entity_usage_state != ir_entity_usage_not_computed) return;
 ...
 */
static ir_type *clone_type_and_cache(ir_type *const tp,
                                     int const variadic_index) {
  ir_type *res = pmap_get(ir_type, mtp_map, tp);
  if (res == NULL) {
    mtp_additional_properties const props =
        get_method_additional_properties(tp);
...
void mark_private_methods(void) {
  ...
  /* first step: change the calling conventions of the local non-escaped
   * entities */
  bool changed = false;
  foreach_irp_irg(i, irg) {
  ...
  if (changed) all_irg_walk(NULL, update_calls_to_private, NULL);
...
static ir_entity *find_entity(ir_node *ptr) {
  switch (get_irn_opcode(ptr)) {
    ...
    case iro_Add: {
      ...
      if (mode_is_reference(get_irn_mode(left))) return find_entity(left);
...
static bool contains_volatile_entity(ir_type *type) {
  if (!is_compound_type(type)) return false;

  for (size_t i = 0, n = get_compound_n_members(type); i < n; ++i) {
    ir_entity *ent = get_compound_member(type, i);
    if (get_entity_volatility(ent) == volatility_is_volatile) return true;

    ir_type *ent_type = get_entity_type(ent);
    if (contains_volatile_entity(ent_type)) return true;
  }
