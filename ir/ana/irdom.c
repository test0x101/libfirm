...
ir_node *get_Block_idom(const ir_node *block) {
  assert(irg_has_properties(get_irn_irg(block),
...
int block_dominates(const ir_node *a, const ir_node *b) {
  assert(irg_has_properties(get_irn_irg(a),
                            IR_GRAPH_PROPERTY_CONSISTENT_DOMINANCE));
...
ir_node *ir_deepest_common_dominator(ir_node *block0, ir_node *block1) {
  assert(irg_has_properties(get_irn_irg(block0),
                            IR_GRAPH_PROPERTY_CONSISTENT_DOMINANCE));
  ...
  /* Shortcut. */
  if (block0 == block1) return block0;
...
void dom_tree_walk(ir_node *block, irg_walk_func *pre, irg_walk_func *post,
  ...
  if (pre != NULL) pre(block, env);

  dominates_for_each(block, p) { dom_tree_walk(p, pre, post, env); }

  if (post != NULL) post(block, env);
}

void postdom_tree_walk(ir_node *block, irg_walk_func *pre, irg_walk_func *post,
                       void *env) {
  assert(irg_has_properties(get_irn_irg(block),
                            IR_GRAPH_PROPERTY_CONSISTENT_POSTDOMINANCE));
  ...
  postdominates_for_each(block, p) { postdom_tree_walk(p, pre, post, env); }
...
void compute_doms(ir_graph *irg) {
  ...
  for (int i = n_blocks; i-- > 1;) { /* Don't iterate the root, it's done. */
    ...
    for (int j = 0, arity = get_irn_arity(block); j < arity; j++) {
      ...
      const tmp_dom_info *u =
          dom_eval(&tdi_list[get_Block_dom_pre_num(pred_block)]);
    ...
    if (block == get_irg_end_block(irg)) {
      foreach_irn_in(get_irg_end(irg), j, pred) {
        ...
        const tmp_dom_info *u =
            dom_eval(&tdi_list[get_Block_dom_pre_num(pred)]);
        if (u->semi < w->semi) w->semi = u->semi;
      }
    }
  ...
  for (int i = 1; i < n_blocks; i++) {
    tmp_dom_info *w = &tdi_list[i];
    if (w->dom == NULL) continue; /* control dead */
...
void compute_postdoms(ir_graph *irg) {
  ...
  for (int i = n_blocks; i-- > 1;) { /* Don't iterate the root, it's done. */
    ...
    foreach_irn_out(block, j, succ) {
      if (get_irn_mode(succ) != mode_X || is_Bad(succ)) continue;
      if (is_End(succ)) {
  ...
  for (int i = 1; i < n_blocks; i++) {
    ...
    set_Block_postdom_depth(w->block,
                            get_Block_postdom_depth(w->dom->block) + 1);
  }
