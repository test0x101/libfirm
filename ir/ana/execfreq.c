...
static square_matrix *mat_create(int n) {
  square_matrix *result =
      xmalloc(sizeof(square_matrix) + n * n * sizeof(double));
  result->size = n;
  return result;
}
 ...
 */
static inline void setm(square_matrix *a, unsigned row, unsigned col,
                        double val) {
...
static void exec_freq_node_info(void *ctx, FILE *f, const ir_node *irn) {
  ...
  if (freq != 0.0) fprintf(f, "execution frequency: %g\n", freq);
}
...
void exit_execfreq(void) { unregister_hook(hook_node_info, &hook); }

static bool has_path_to_end(const ir_node *block) {
  return Block_block_visited(block);
}

static bool is_kept_block(const ir_node *block) { return irn_visited(block); }

static double get_sum_succ_factors(const ir_node *block,
   ...
   * end node */
  if (is_kept_block(block) && !has_path_to_end(block)) sum += KEEP_FAC;
...
static void collect_freqs(ir_node *node, void *data) {
  ...
  if (freq > 0.0 && freq < min_non_zero) min_non_zero = freq;
...
void ir_calculate_execfreq_int_factors(ir_execfreq_int_factors *factors,
  ...
  for (size_t i = 0; i < n_freqs; ++i) {
    ...
    for (size_t j = i + 1; j < n_freqs; ++j) {
      ...
      if (!UNDEF(diff)) smallest_diff = MIN(diff, smallest_diff);
    }
  }
...
int get_block_execfreq_int(const ir_execfreq_int_factors *factors,
  ...
  int res =
      (int)(f > factors->min_non_zero ? factors->m * f + factors->b : 1.0);
  return res;
}

static void block_walk_no_keeps(ir_node *block) {
  if (Block_block_visited(block)) return;
...
void ir_estimate_execfreq(ir_graph *irg) {
  ...
  ir_reserve_resources(irg, IR_RESOURCE_BLOCK_VISITED |
                                IR_RESOURCE_IRN_VISITED | IR_RESOURCE_IRN_LINK);
  ...
  for (unsigned idx = 0; idx < size; ++idx) {
    ...
    if (bb == start_block) setm(in_fac, idx, end_idx, 1.0);
  }
  ...
  for (int i = get_Block_n_cfgpreds(end_block) - 1; i >= 0; --i) {
    ...
    double const cf_probability =
        get_cf_probability(end_block, i, inv_loop_weight);
  ...
  for (unsigned k = n_keepalives; k-- > 0;) {
    ir_node *keep = get_End_keepalive(end, k);
    if (!is_Block(keep) || has_path_to_end(keep)) continue;
...
#ifdef DEBUG
  /* Check that all values in in_fac are only given in terms of nodes with
   * backedges */
  for (int y = 0; y < size; y++) {
    if (mat_to_lgs[y] == -1) continue;
