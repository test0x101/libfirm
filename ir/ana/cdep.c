...
typedef struct cdep_info {
  pmap *cdep_map; /**< A map to find the list of all control dependence nodes
                     for a block. */
...
ir_node *(get_cdep_node)(const ir_cdep *cdep) { return _get_cdep_node(cdep); }

ir_cdep *(get_cdep_next)(const ir_cdep *cdep) { return _get_cdep_next(cdep); }
...
int is_cdep_on(const ir_node *dependee, const ir_node *candidate) {
  for (const ir_cdep *dep = find_cdep(dependee); dep != NULL; dep = dep->next) {
