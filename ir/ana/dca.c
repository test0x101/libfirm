...
static void care_for(ir_node *irn, ir_tarval *care) {
  if (care == NULL) care = tarval_b_true;
  ...
  if (mode != get_tarval_mode(care))
    care = tarval_is_null(care) ? get_mode_null(mode) : get_mode_all_one(mode);

  if (mode_is_int(mode) || mode == mode_b) care = tarval_or(care, old_care);
...
static void dca_transfer(ir_node *irn) {
  ...
  if (mode == mode_X) {
    ...
    switch (get_irn_opcode(irn)) {
      ...
      case iro_Jmp:
      default:
        foreach_irn_in(irn, i, pred) { care_for(pred, NULL); }
  ...
  if (is_Phi(irn)) {
    ...
    for (int i = 0; i < npreds; i++) care_for(get_Phi_pred(irn, i), care);
    ...
    switch (get_irn_opcode(irn)) {
      case iro_Cmp: {
        ...
        if (bl != NULL && br != NULL) {
          ...
          assert(get_mode_arithmetic(get_irn_mode(left)) ==
          ...
          if (!tarval_is_null(diff)) {
            ...
            ir_tarval *care =
                tarval_shl_unsigned(get_mode_one(cmp_mode), highest_bit);
      ...
      case iro_Conv: {
        ...
        if (pred_bits < bits && mode_is_signed(pred_mode)) {
          ...
          if (get_tarval_highest_bit(care) >= (int)pred_bits)
            care = tarval_or(
                care, tarval_shl_unsigned(get_mode_one(mode), pred_bits - 1));
        } else {
          /* Thwart sign extension as it doesn't make sense on
           * our abstract tarvals. */
          /* TODO: ugly */
          care = tarval_convert_to(care,
                                   find_unsigned_mode(get_tarval_mode(care)));
        }
      ...
      case iro_Add: {
        ...
        if (bl != NULL && br != NULL &&
            tarval_is_null(tarval_and(care_mask, tarval_and(bl->z, br->z)))) {
          care_for(left,
                   tarval_or(tarval_andnot(care_mask, bl->z),
                             tarval_and(care, tarval_ornot(bl->o, br->o))));
          care_for(right,
                   tarval_or(tarval_andnot(care_mask, br->z),
      ...
      case iro_Shr: {
          ...
          if (tarval_is_null(tarval_and(ro, oversize_mask))) {
            ...
            for (ir_tarval *shift_amount = rzero; shift_amount != rbound;
              ...
              if (tarval_is_null(
                ...
                care_left =
                    tarval_or(care_left, tarval_shl(care, shift_amount));
              }
            }
          } else {
            care_left = get_mode_null(mode);
        ...
        if (is_Shrs(irn) && shift_amount != NULL) {
          ...
          if (!tarval_is_null(
                  tarval_and(tarval_shrs(msb, shift_amount), care))) {
          ...
          if (tarval_is_null(tarval_and(ro, oversize_mask))) {
            ...
            for (ir_tarval *shift_amount = rzero; shift_amount != rbound;
              ...
              if (tarval_is_null(
                      tarval_and(rsure, tarval_eor(shift_amount, rz)))) {
                care_left =
                    tarval_or(care_left, tarval_shr(care, shift_amount));
              }
            }
            care_for(left, care_left);
          } else {
      ...
      case iro_Mul: {
        ...
        if (is_Const(right))
          care_for(left, tarval_shr_unsigned(
                             care_mask,
                             get_tarval_lowest_bit(get_Const_tarval(right))));
        else
          care_for(left, care_mask);
  ...
  if (mode == mode_M || mode == mode_T) {
    foreach_irn_in(irn, i, pred) { care_for(pred, care); }
    return;
  }
...
static void dca_init_node(ir_node *n, void *data) {
  ...
  set_irn_link(n, (void *)(mode_is_int(m) ? get_mode_null(m) : tarval_b_false));
}
