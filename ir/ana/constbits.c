 ...
 * - Implement min/max calculation for And, Eor, Or, Not, Conv, Shl, Shr, Shrs,
 * Mux
 * - Implement min/max calculation for Add, Sub, Minus, Mul, Div, Mod, Conv,
 * Shl, Shr, Shrs, Mux
 */
...
static bool set_bitinfo(ir_node const *const irn, ir_tarval *const z,
                        ir_tarval *const o) {
  ...
  DB((dbg, LEVEL_3, "Set %+F: 0:%T 1:%T%s\n", irn, z, o,
      is_undefined(b)                             ? " (bottom)"
      : tarval_is_all_one(z) && tarval_is_null(o) ? " (top)"
                                                  : ""));
  return true;
}
...
bitinfo *get_bitinfo(ir_node const *const irn) { return get_bitinfo_func(irn); }
...
static bool transfer(ir_node const *const irn) {
  ...
  if (m == mode_X) {
      ...
      switch (get_irn_opcode(irn)) {
        ...
        case iro_Proj: {
          ...
          if (is_Start(pred)) {
            ...
            if (is_undefined(b)) goto unreachable_X;
            if (b->z == b->o) {
    ...
    if (!reachable) {
      ir_graph *const irg = get_irn_irg(irn);
      reachable =
          irn == get_irg_start_block(irg) || irn == get_irg_end_block(irg);
    }
    ...
    repeatphi:
      ...
      foreach_irn_in(irn, i, pred) {
        ...
        if (pred_b != NULL && is_undefined(pred_b)) goto undefined;
      }
      ...
      switch (get_irn_opcode(irn)) {
        ...
        case iro_Confirm: {
          ...
          z = b->z;
          o = b->o;
          if ((get_Confirm_relation(irn) & ~ir_relation_unordered) ==
              ir_relation_equal) {
            bitinfo *const bound_b =
                get_bitinfo_recursive(get_Confirm_bound(irn));
        ...
        case iro_Shl: {
          ...
          if (rz == ro) {
            ...
            ir_tarval *const size_mask =
                tarval_sub(new_tarval_from_long(size_bits, rmode), rone);
            ir_tarval *const modulo_mask =
                tarval_sub(new_tarval_from_long(modulo_shift, rmode), rone);
            ir_tarval *const zero = get_mode_null(m);
            ir_tarval *const all_one = get_mode_all_one(m);
            ir_tarval *const oversize_mask =
                tarval_andnot(modulo_mask, size_mask);
            ...
            if (tarval_is_null(tarval_and(ro, oversize_mask))) {
              ...
              ir_tarval *const rsure =
                  tarval_and(tarval_not(tarval_eor(ro, rz)), rmask);
              ir_tarval *const rbound = tarval_add(rmask, rone);
              ir_tarval *const rzero = get_mode_null(rmode);
              for (ir_tarval *shift_amount = rzero; shift_amount != rbound;
                   shift_amount = tarval_add(shift_amount, rone)) {
                if (tarval_is_null(
                        tarval_and(rsure, tarval_eor(shift_amount, rz)))) {
        ...
        case iro_Sub: {
          ...
          if (l == NULL || r == NULL) goto cannot_analyse;
          ...
          switch (relation) {
            ...
            case ir_relation_less:
              ...
              if (tarval_cmp(lz, ro) & relation) {
                ...
                /* Left lower bound is not smaller(/equal) than right upper
                 * bound. */
                z = o = f;
              } else {
                goto result_unknown;
            ...
            case ir_relation_greater:
              ...
              if (!(tarval_cmp(lz, ro) & relation)) {
                /* Left upper bound is not greater(/equal) than right lower
                 * bound. */
                z = o = f;
              } else if (tarval_cmp(lo, rz) & relation) {
...
static void trigger_users(ir_node const *const irn) {
  ...
  if (is_Block(irn)) {
    ...
    foreach_out_edge(irn, e) {
      ...
      if (get_irn_mode(src) == mode_X) trigger(src, irn);
    }
    ...
    foreach_out_edge(irn, e) {
      ...
      if (get_irn_mode(src) == mode_T) {
        /* Trigger Projs of tuple nodes.  They might contain analysis
         * information, but the tuple node does not. */
        trigger_users(src);
      } else {
        trigger(src, irn);
      }
