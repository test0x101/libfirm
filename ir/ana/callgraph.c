...
static void set_irg_caller_backedge(ir_graph *irg, const ir_graph *caller) {
  ...
  if (irg->caller_isbe == NULL) irg->caller_isbe = rbitset_malloc(n_callers);
...
int has_irg_caller_backedge(const ir_graph *irg) {
  if (irg->caller_isbe != NULL) {
    for (size_t i = 0, n_callers = get_irg_n_callers(irg); i < n_callers; ++i)
      if (rbitset_is_set(irg->caller_isbe, i)) return 1;
  }
  return 0;
}
...
static size_t reverse_pos(const ir_graph *callee, size_t pos_caller) {
  ...
  for (size_t i = 0, n_callees = get_irg_n_callees(caller); i < n_callees;
       ++i) {
...
int has_irg_callee_backedge(const ir_graph *irg) {
  if (irg->callee_isbe != NULL) {
    for (size_t i = 0, n_callees = get_irg_n_callees(irg); i < n_callees; ++i)
      if (rbitset_is_set(irg->callee_isbe, i)) return 1;
  }
  return 0;
}
...
static void set_irg_callee_backedge(ir_graph *irg, size_t pos) {
  ...
  if (irg->callee_isbe == NULL) irg->callee_isbe = rbitset_malloc(n);
  assert(pos < n);
...
static void ana_Call(ir_node *n, void *env) {
  ...
  for (size_t i = 0, n_callees = cg_get_call_n_callees(n); i < n_callees; ++i) {
    ...
    if (callee) {
      ...
      cg_callee_entry *found = (cg_callee_entry *)pset_find(
          (pset *)irg->callees, &buf, hash_ptr(callee));
...
void compute_callgraph(void) {
  ...
  foreach_irp_irg(i, irg) {
    construct_cf_backedges(
        irg);  // We also find the maximal loop depth of a call.
  ...
  foreach_irp_irg(i, irg) {
    ...
    j = 0;
    foreach_pset(caller_set, ir_graph, c) { irg->callers[j++] = c; }
    del_pset(caller_set);
    assert(j == count);
  }
...
void callgraph_walk(callgraph_walk_func *pre, callgraph_walk_func *post,
  ...
  foreach_irp_irg(i, irg) {
    if (get_irg_n_callers(irg) == 0) do_walk(irg, pre, post, env);
  }

  /* in case of unreachable call loops we haven't visited some irgs yet */
  foreach_irp_irg(i, irg) { do_walk(irg, pre, post, env); }
}

static ir_graph *outermost_ir_graph; /**< The outermost graph the scc is
                                        computed for */
static ir_loop *current_loop;        /**< Current cfloop construction is working
                                          on. */
static size_t loop_node_cnt = 0;     /**< Counts the number of allocated cfloop
                                        nodes.     Each cfloop node gets a unique
                                        number.     What for? ev. remove. @@@ */
...
static void init_scc(struct obstack *obst) {
  ...
  foreach_irp_irg(i, irg) { set_irg_link(irg, new_scc_info(obst)); }
}
  ...
  if (get_irg_dfn(n) == get_irg_uplink(n)) {
    ...
    if (tail) {
      ...
      /* The current backedge has been marked, that is temporarily eliminated,
         ...
         anew on the subgraph thats left (the current cfloop without the
         backedge) in order to find more inner cfloops. */
...
void find_callgraph_recursions(void) {
  ...
  foreach_irp_irg(i, irg) {
    if (!cg_irg_visited(irg) && get_irg_n_callers(irg) == 0) cgscc(irg);
  }
  foreach_irp_irg(i, irg) {
    if (!cg_irg_visited(irg)) cgscc(irg);
  }
