...
static dfs_node_t *_dfs_get_node(dfs_t const *const self, ir_node *const node) {
  ...
  templ.node = node;
  return set_insert(dfs_node_t, self->nodes, &templ, sizeof(templ),
                    hash_ptr(node));
}

#define _dfs_int_is_ancestor(n, m) \
  ((m)->pre_num >= (n)->pre_num && (m)->pre_num <= (n)->max_pre_num)

static inline int _dfs_is_ancestor(dfs_t const *const dfs, ir_node *const a,
                                   ir_node *const b) {
