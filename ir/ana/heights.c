...
static irn_height_t *get_height_data(ir_heights_t *heights,
                                     const ir_node *node) {
  irn_height_t *height = ir_nodemap_get(irn_height_t, &heights->data, node);
  if (height == NULL) {
...
static void height_dump_cb(void *data, FILE *f, const ir_node *irn) {
  ...
  if (h != NULL) fprintf(f, "height: %u\n", h->height);
}

/**
 * Check, if we can reach a target node from a given node inside one basic
 * block.
...
static bool search(ir_heights_t *h, const ir_node *curr, const ir_node *tgt) {
  ...
  foreach_irn_in(curr, i, op) {
    if (search(h, op, tgt)) return true;
  }
 ...
 * @param bl  The block.
 */
static unsigned compute_height(ir_heights_t *h, ir_node *irn,
                               const ir_node *bl) {
  irn_height_t *ih = get_height_data(h, irn);

  /* bail out if we already visited that node. */
  if (ih->visited >= h->visited) return ih->height;
