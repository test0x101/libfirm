...
static void pack(const fp_value *value, sc_word *packed) {
  ...
  /* set sign bit */
  if (value->sign) sc_set_bit_at(packed, mantissa_size + exponent_size);
}
...
static bool normalize(fp_value *val, bool sticky) {
  ...
  int hsb = ROUNDING_BITS + effective_mantissa -
            sc_get_highest_set_bit(_mant(val)) - 1;
  ...
  if (sc_is_negative(_exp(val)) || sc_is_zero(_exp(val), value_size)) {
    /* exponent underflow */
  ...
  char lsb =
      sc_sub_bits(_mant(val), effective_mantissa + ROUNDING_BITS, 0) & 0x7;
  ...
  if (sc_is_zero(_mant(val), value_size) && (val->clss == FC_SUBNORMAL))
    val->clss = FC_ZERO;

  /* check for rounding overflow */
  hsb = ROUNDING_BITS + effective_mantissa -
        sc_get_highest_set_bit(_mant(val)) - 1;
  if ((val->clss != FC_SUBNORMAL) && (hsb < -1)) {
    sc_val_from_ulong(1, temp);
    bool carry = sc_shrI(_mant(val), 1, _mant(val));
    if (carry) exact = false;
    sc_add(_exp(val), temp, _exp(val));
...
static bool handle_NAN(const fp_value *a, const fp_value *b, fp_value *result) {
  if (a->clss == FC_NAN) {
    ...
    if (b->clss == FC_NAN && smaller_nan(b, a)) goto return_nan_b;
    if (a != result) memcpy(result, a, fp_value_size);
    fc_exact = false;
    return true;
  }
  if (b->clss == FC_NAN) {
...
static void _fadd(const fp_value *a, const fp_value *b, fp_value *result) {
  ...
  if (result != a && result != b) result->desc = a->desc;
...
void fc_mul(const fp_value *a, const fp_value *b, fp_value *result) {
  fc_exact = true;
  if (handle_NAN(a, b, result)) return;
  ...
  bool sticky = sc_shrI(
      _mant(result),
      (result->desc.mantissa_size - result->desc.explicit_one) + ROUNDING_BITS,
      _mant(result));
  fc_exact &= !sticky;
...
unsigned fc_get_value_size(void) { return fp_value_size; }
...
void fc_val_from_bytes(fp_value *result, const unsigned char *buffer,
  ...
  sc_val_from_bits(buffer, mantissa_size, mantissa_size + exponent_size,
                   _exp(result));
  result->sign =
      (buffer[sign_bit / CHAR_BIT] & (1u << (sign_bit % CHAR_BIT))) != 0;
...
void fc_val_from_ieee754(long double l, fp_value *result) {
  unsigned real_size =
      (long_double_desc.mantissa_size + long_double_desc.exponent_size + 1) /
      CHAR_BIT;
  unsigned char *buf = ALLOCAN(unsigned char, real_size);
#ifdef WORDS_BIGENDIAN
  unsigned char *from = (unsigned char *)&l;
  for (unsigned i = 0; i < real_size; ++i) buf[i] = from[real_size - 1 - i];
#else
...
void fc_cast(const fp_value *value, const float_descriptor_t *dest,
  ...
  if (desc->exponent_size == dest->exponent_size &&
    ...
    if (value != result) memcpy(result, value, fp_value_size);
    return;
  }
  ...
  switch ((value_class_t)value->clss) {
    case FC_NAN: {
      unsigned const src_mant =
          value->desc.mantissa_size - value->desc.explicit_one;
...
void fc_get_max(const float_descriptor_t *desc, fp_value *result, bool sign) {
  ...
  sc_max_from_bits(desc->mantissa_size + 1 - desc->explicit_one, 0,
                   _mant(result));
  sc_shlI(_mant(result), ROUNDING_BITS, _mant(result));
}
...
void fc_get_nan(const float_descriptor_t *desc, fp_value *result,
  ...
  if (payload != NULL) {
    if (payload != _mant(result)) memcpy(_mant(result), payload, value_size);
    ...
    sc_zero(_mant(result));
    /* Ensure that we have at least 1 bit set. */
    if (signaling) sc_set_bit_at(_mant(result), ROUNDING_BITS);
  }
...
ir_relation fc_comp(fp_value const *const val_a, fp_value const *const val_b) {
  ...
  ir_relation const mul =
      val_a->sign ? ir_relation_less_greater : ir_relation_false;

  /* infinity is bigger than the rest */
  if (val_a->clss == FC_INF) return ir_relation_greater ^ mul;
  if (val_b->clss == FC_INF) return ir_relation_less ^ mul;

  /* check first exponent, that mantissa if equal */
  ir_relation rel = sc_comp(_exp(val_a), _exp(val_b));
  if (rel == ir_relation_equal) rel = sc_comp(_mant(val_a), _mant(val_b));
  if (rel != ir_relation_equal) rel ^= mul;
  return rel;
}

bool fc_is_zero(const fp_value *a) { return a->clss == FC_ZERO; }

bool fc_is_negative(const fp_value *a) { return a->sign; }
...
bool fc_is_subnormal(const fp_value *a) { return a->clss == FC_SUBNORMAL; }
...
fc_rounding_mode_t fc_get_rounding_mode(void) { return rounding_mode; }
...
flt2int_result_t fc_flt2int(const fp_value *a, sc_word *result,
  ...
  switch ((value_class_t)a->clss) {
    case FC_ZERO:
      ...
      return FLT2INT_OK;
    case FC_INF:
      return a->sign ? FLT2INT_NEGATIVE_OVERFLOW : FLT2INT_POSITIVE_OVERFLOW;
    case FC_SUBNORMAL:
    case FC_NORMAL:
      if (a->sign && !result_signed) return FLT2INT_NEGATIVE_OVERFLOW;
      ...
      if (exp_val > (int)tgt_bits ||
          (exp_val == (int)tgt_bits &&
      ...
      if (a->sign) sc_neg(result, result);
...
bool fc_is_exact(void) { return fc_exact; }
