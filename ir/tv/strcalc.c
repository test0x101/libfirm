...
static sc_word sex_digit(unsigned x) { return (SC_MASK << (x + 1)) & SC_MASK; }

static sc_word max_digit(unsigned x) { return (1u << x) - 1; }

static sc_word min_digit(unsigned x) { return SC_MASK - max_digit(x); }
...
bool sc_divmod(const sc_word *dividend, const sc_word *divisor, sc_word *quot,
               sc_word *rem) {
  ...
  if (sc_is_zero(dividend, calc_buffer_size * SC_BITS)) return false;
  ...
  if (div_sign) sc_neg(quot, quot);

  if (rem_sign) sc_neg(rem, rem);
...
unsigned sc_get_value_length(void) { return calc_buffer_size; }
...
void sc_sign_extend(sc_word *buffer, unsigned from_bits) {
  ...
  if (sign_bit) {
    ...
    for (unsigned i = word + 1; i < calc_buffer_size; ++i) buffer[i] = SC_MASK;
...
static inline void check_ascii(void) {
  assert((('a' - 97) | ('b' - 98) | ('c' - 99) | ('d' - 100) | ('e' - 101) |
          ('f' - 102) | ('A' - 65) | ('B' - 66) | ('C' - 67) | ('D' - 68) |
          ('E' - 69) | ('F' - 70) | ('0' - 48) | ('1' - 49) | ('2' - 50) |
...
bool sc_val_from_str(bool negative, unsigned base, const char *str, size_t len,
  ...
  if (negative) sc_neg(buffer, buffer);
...
void sc_min_from_bits(unsigned num_bits, bool sign, sc_word *buffer) {
  if (!sign) {
    ...
    unsigned i = 0;
    for (; i < bits / SC_BITS; i++) *pos++ = 0;

    *pos++ = min_digit(bits % SC_BITS);

    for (i++; i < calc_buffer_size; i++) *pos++ = SC_MASK;
  }
}
...
void sc_max_from_bits(unsigned num_bits, bool sign, sc_word *buffer) {
  ...
  unsigned i = 0;
  for (; i < bits / SC_BITS; i++) *pos++ = SC_MASK;

  *pos++ = max_digit(bits % SC_BITS);

  for (i++; i < calc_buffer_size; i++) *pos++ = 0;
}
...
int sc_get_highest_set_bit(const sc_word *value) {
  for (unsigned counter = calc_buffer_size; counter-- > 0;) {
    ...
    if (word != 0) return counter * SC_BITS + (31 - nlz(word));
  }
  return -1;
}
...
int sc_get_lowest_set_bit(const sc_word *value) {
  for (unsigned counter = 0; counter < calc_buffer_size; ++counter) {
    sc_word word = value[counter];
    if (word != 0) return (counter * SC_BITS) + ntz(word);
  }
  return -1;
}
...
bool sc_is_all_one(const sc_word *value, unsigned bits) {
  unsigned i;
  for (i = 0; i < bits / SC_BITS; ++i) {
    if (value[i] != SC_MASK) return false;
  }
  sc_word mask = max_digit(bits % SC_BITS);
...
unsigned char sc_sub_bits(const sc_word *value, unsigned len,
                          unsigned byte_ofs) {
  if (byte_ofs * SC_BITS >= len) return 0;
...
void sc_val_from_bits(unsigned char const *const bytes, unsigned from,
  ...
  *(p + 1) = 0;
  ...
  uint32_t hval =
      ((uint32_t)(*high) << (32 - high_bit)) >> (32 - high_bit - bit);
  *p++ |= (hval >> 0) & SC_MASK;
  if ((hval >> SC_BITS) != 0) *p++ = (hval >> SC_BITS) & SC_MASK;
...
const char *sc_print(const sc_word *value, unsigned bits, enum base_t base,
                     bool is_signed) {
  return sc_print_buf(output_buffer, bit_pattern_size + 1, value, bits, base,
                      is_signed);
}
  ...
  *(--pos) = '\0';
  ...
  switch (base) {
    ...
    case SC_DEC: {
      ...
      unsigned counter = 0;
      for (; counter < n_full_words; ++counter) div1_res[counter] = p[counter];
      ...
      for (;;) {
        ...
        *(--pos) = digits[rem_res[0]];
        ...
        sc_word x = 0;
        for (unsigned i = 0; i < calc_buffer_size; ++i) x |= m[i];
...
unsigned sc_get_precision(void) { return bit_pattern_size; }
