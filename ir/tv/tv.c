...
static ir_tarval *get_int_tarval_overflow(const sc_word *value, ir_mode *mode) {
  ...
  assert(get_mode_arithmetic(mode) == irma_twos_complement);
  if (!wrap_on_overflow && is_overflow(value, mode)) return tarval_bad;
...
ir_tarval *new_tarval_nan(ir_mode *mode, int signaling,
  ...
  assert(payload == NULL ||
         get_mode_arithmetic(get_tarval_mode(payload)) == irma_twos_complement);
...
int tarval_is_long(ir_tarval const *tv) {
  ...
  if (get_mode_size_bytes(mode) <= sizeof(long)) return true;
  ...
  if (sc_comp(tv->value, temp) == ir_relation_greater) return false;
  if (mode_is_signed(mode)) {
    sc_word *min = ALLOCAN(sc_word, sc_value_length);
    sc_min_from_bits(long_bits, true, min);
    sc_sign_extend(min, long_bits);
    if (sc_comp(tv->value, min) == ir_relation_less) return false;
  }
  return true;
}
...
bool tarval_is_uint64(ir_tarval const *tv) {
  ir_mode *const mode = get_tarval_mode(tv);
  if (get_mode_arithmetic(mode) != irma_twos_complement) return false;
  if (get_mode_size_bytes(mode) <= sizeof(uint64_t)) return true;
...
int tarval_is_double(ir_tarval const *tv) { return mode_is_float(tv->mode); }
...
ir_mode *(get_tarval_mode)(ir_tarval const *tv) { return get_tarval_mode_(tv); }
...
int(tarval_is_constant)(ir_tarval const *tv) { return tarval_is_constant_(tv); }

ir_tarval *(get_tarval_bad)(void) { return get_tarval_bad_(); }

ir_tarval *(get_tarval_unknown)(void) { return get_tarval_unknown_(); }

ir_tarval *(get_tarval_b_false)(void) { return get_tarval_b_false_(); }

ir_tarval *(get_tarval_b_true)(void) { return get_tarval_b_true_(); }
  ...
  switch (get_mode_sort(a->mode)) {
    ...
    case irms_reference:
      if (!mode_is_signed(a->mode)) {
        ...
        return sc_comp(a->value, get_mode_null(a->mode)->value) ==
                       ir_relation_less
                   ? 1
                   : 0;
      }
...
ir_relation tarval_cmp(ir_tarval const *const a, ir_tarval const *const b) {
  if (a == tarval_unknown || b == tarval_unknown) return ir_relation_true;
  ...
  switch (get_mode_sort(a->mode)) {
    ...
    case irms_reference:
    case irms_int_number:
      if (a == b) return ir_relation_equal;
...
ir_tarval *tarval_convert_to(ir_tarval const *const src,
  ...
  if (src->mode == dst_mode) return (ir_tarval *)src;
      ...
      switch (get_mode_sort(dst_mode)) {
        ...
        case irms_float_number: {
          ...
          int len = snprintf(buffer, 100, "%s",
                             sc_print(src->value, get_mode_size_bits(src->mode),
                                      SC_DEC, mode_is_signed(src->mode)));
...
ir_tarval *tarval_bitcast(ir_tarval const *src, ir_mode *const dst_mode) {
  ...
  if (src_mode == dst_mode) return (ir_tarval *)src;
...
ir_tarval *tarval_div(ir_tarval const *const a, ir_tarval const *const b) {
  ...
  switch (get_mode_sort(a->mode)) {
    ...
    case irms_reference: {
      /* x/0 error */
      if (b == get_mode_null(mode)) return tarval_bad;
...
ir_tarval *tarval_abs(ir_tarval const *const a) {
  if (tarval_is_negative(a)) return tarval_neg(a);
  return (ir_tarval *)a;
}
...
int tarval_snprintf(char *buf, size_t len, ir_tarval const *tv) {
  switch (get_mode_sort(tv->mode)) {
    ...
    case irms_internal_boolean:
      return snprintf(buf, len, "%s", (tv == tarval_b_true) ? "true" : "false");
...
static char hexchar(unsigned val) { return "0123456789ABCDEF"[val]; }
...
const char *ir_tarval_to_ascii(char *buf, size_t len, ir_tarval const *tv) {
  ...
  switch (get_mode_sort(mode)) {
    ...
    case irms_reference:
    case irms_int_number:
      return sc_print_buf(buf, len, tv->value, get_mode_size_bits(mode), SC_HEX,
                          0);
    case irms_float_number: {
...
unsigned char get_tarval_sub_bits(ir_tarval const *tv, unsigned byte_ofs) {
  switch (get_mode_arithmetic(tv->mode)) {
    ...
    case irma_ieee754:
    case irma_x86_extended_float:
      return fc_sub_bits((const fp_value *)tv->value,
                         get_mode_size_bits(tv->mode), byte_ofs);
    default:
...
int get_tarval_highest_bit(ir_tarval const *tv) {
  ...
  for (int i = l - 1; i >= 0; --i) {
    ...
    if (v) return 8 * sizeof(unsigned) - nlz(v) + i * 8 - 1;
  }
  return -1;
}
...
unsigned get_tarval_magnitude(ir_tarval const *tv) {
  ...
  for (unsigned i = l; i-- != 0;) {
    ...
    if (v != ext) return i * SC_BITS + (32 - nlz(v ^ ext)) + 1;
  }
...
unsigned tarval_ieee754_get_exact(void) { return fc_is_exact(); }

int tarval_is_nan(ir_tarval const *tv) {
  if (!mode_is_float(tv->mode)) return 0;
  return fc_is_nan((const fp_value *)tv->value);
}

int tarval_is_quiet_nan(ir_tarval const *tv) {
  if (!tarval_is_nan(tv)) return false;
...
int tarval_get_wrap_on_overflow(void) { return wrap_on_overflow; }
...
bool tarval_in_range(ir_tarval const *const min, ir_tarval const *const val,
                     ir_tarval const *const max) {
  assert(tarval_cmp(min, max) & ir_relation_less_equal);
  assert(!tarval_is_nan(val));
  /* Shortcuts. */
  if (val == min) return true;
  if (min == max) return false;
  return tarval_cmp(min, val) & ir_relation_less_equal &&
         tarval_cmp(val, max) & ir_relation_less_equal;
}
