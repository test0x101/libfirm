...
static inline block_info_t *new_block_info(struct obstack *obst,
                                           ir_node *block) {
...
static inline spill_info_t *get_spill_info(minibelady_env_t *env,
  ...
  spill_info_t *spill_info =
      ir_nodehashmap_get(spill_info_t, &env->spill_infos, node);
  return spill_info;
}
...
static void create_reload(minibelady_env_t *env, ir_node *state,
  ...
  ir_node *reload =
      env->create_reload(env->func_env, state, spill, before, last_state);
...
static block_info_t *compute_block_start_state(minibelady_env_t *env,
  ...
  sched_foreach_phi(block, node) {
    if (arch_get_irn_register(node) != env->reg) continue;
...
static void belady(minibelady_env_t *env, ir_node *block) {
  ...
  sched_foreach_non_phi(block, node) {
    ...
    be_foreach_value(
        ...
        if (reg != env->reg) continue; current_state = value;
...
void be_assure_state(ir_graph *irg, const arch_register_t *reg, void *func_env,
  ...
  while (info != NULL) {
    ...
    be_ssa_construction_add_copies(&senv, info->reloads,
                                   ARR_LEN(info->reloads));
...
void be_init_state(void) { FIRM_DBG_REGISTER(dbg, "firm.be.state"); }
