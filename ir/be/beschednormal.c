...
static int cost_cmp(const void *a, const void *b) {
  ...
  if (ret == 0) ret = (int)get_irn_idx(a1->irn) - (int)get_irn_idx(b1->irn);
  return ret;
}

static unsigned count_result(const ir_node *irn) {
  const ir_mode *mode = get_irn_mode(irn);
  if (mode == mode_M || mode == mode_X) return 0;
  if (mode == mode_T) return 1;
...
static unsigned normal_tree_cost(ir_node *irn) {
  ...
  if (be_is_Keep(irn)) return 0;
  if (is_Proj(irn)) return normal_tree_cost(get_Proj_pred(irn));
    ...
    foreach_irn_in(irn, i, pred) {
      ...
      if (is_Phi(irn) || get_irn_mode(pred) == mode_M) {
        ...
        if (!arch_irn_is_ignore(pred)) {
          ir_node *real_pred = is_Proj(pred) ? get_Proj_pred(pred) : pred;
  ...
  for (int i = 0; i < arity; ++i) {
    ...
    if (mode == mode_M || arch_irn_is_ignore(op)) continue;
...
static void normal_cost_walker(ir_node *irn, void *data) {
  ...
  if (arch_is_irn_not_scheduled(irn)) return;
...
static int root_cmp(const void *a, const void *b) {
  ...
  if (is_cfop(a1->irn) && !is_cfop(b1->irn)) {
    ...
    if (ret == 0) {
      ...
      /* compare node idx */
      if (ret == 0) ret = get_irn_idx(a1->irn) - get_irn_idx(b1->irn);
    }
  }
