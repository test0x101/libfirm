 ...
 */
static void impl_parcopy(const arch_register_class_t *cls, ir_node *before,
                         unsigned *parcopy, unsigned *n_used, ir_node **phis,
                         ir_node **phi_args, unsigned pred_nr) {
  ...
  if (perm_size > 0) {
    ...
    for (unsigned dst = 0; dst < n_regs; ++dst) {
      const unsigned src = parcopy[dst];
      if (src == n_regs || src == dst) continue;
...
static void insert_shuffle_code_walker(ir_node *block, void *data) {
  if (!is_Phi(sched_first(block))) return;
  ...
  for (int pred_nr = 0; pred_nr < get_irn_arity(block); ++pred_nr) {
    ...
    sched_foreach_phi(block, phi) {
      ...
      if (phi_reg_idx != arg_reg_idx) need_perm = true;
      ...
      if (be_is_live_in(lv, block, arg)) {
        assert(parcopy[arg_reg_idx] == n_regs ||
