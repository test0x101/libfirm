 ...
 */
static bool has_definition(const ir_node *block) { return irn_visited(block); }
...
static void mark_iterated_dominance_frontiers(be_ssa_construction_env_t *env) {
  ...
  while (!deq_empty(&env->worklist)) {
    ...
    for (size_t i = 0, len = ARR_LEN(domfront); i < len; ++i) {
      ...
      if (Block_block_visited(y)) continue;

      if (!irn_visited(y)) deq_push_pointer_right(&env->worklist, y);
 ...
 */
static ir_node *insert_dummy_phi(be_ssa_construction_env_t *env,
                                 ir_node *block) {
 ...
 */
static ir_node *get_def_at_idom(be_ssa_construction_env_t *env,
                                ir_node *block) {
...
static void process_block(be_ssa_construction_env_t *env, ir_node *block) {
  ...
  sched_foreach(block, node) {
    ...
    if (info->is_use && !is_Phi(node)) {
      ...
      /* Create a phi if the block is in the dominance frontier. */
      if (def == NULL) def = get_def_from_preds(env, block);
...
static ir_node *search_def_end_of_block(be_ssa_construction_env_t *env,
  ...
  if (last_definition != NULL) return last_definition;

  if (has_definition(block)) {
    if (has_use(env, block)) {
      if (!block_info->already_processed) process_block(env, block);
    } else {
...
static void search_def_at_block(be_ssa_construction_env_t *const env,
  ...
  if (block_info->already_processed) return;
...
void be_ssa_construction_add_copy(be_ssa_construction_env_t *env,
  ...
  if (!has_definition(block)) deq_push_pointer_right(&env->worklist, block);
 ...
 */
static void fix_phi_arguments(be_ssa_construction_env_t *const env,
                              ir_node *const phi, constr_info *const info) {
...
void be_ssa_construction_fix_users_array(be_ssa_construction_env_t *env,
  ...
  for (size_t i = 0; i < nodes_len; ++i) {
    ...
    foreach_out_edge_safe(value, edge) {
      ...
      if (is_Anchor(use) || is_End(use)) continue;
  ...
  while (!deq_empty(&env->worklist)) {
    ...
    if (info->already_processed) continue;
...
void be_init_ssaconstr(void) { FIRM_DBG_REGISTER(dbg, "firm.be.ssaconstr"); }
