...
void be_ifg_free(be_ifg_t *self) { free(self); }
  ...
  foreach_border_head(head, b) {
    ...
    if (irn == it->irn) {
      if (b->is_def)
        ...
        break; /* if we reached the end of the node's lifetime we can safely
                  break */
    } else if (b->is_def) {
      /* if any other node than the one in question starts living, add it to the
       * set */
...
static void find_neighbours(const be_ifg_t *ifg, neighbours_iter_t *it,
                            const ir_node *irn) {
  it->env = ifg->env;
  it->irn = irn;
  it->valid = 1;
...
static inline int get_next_clique(cliques_iter_t *it) {
  ...
  for (; it->blk < it->n_blocks; it->blk++) {
    ...
    struct list_head *head =
        get_block_border_head(it->cenv, it->blocks[it->blk]);

    /* on entry to a new block set the first border ... */
    if (!it->bor) it->bor = head->prev;
    ...
    for (; it->bor != head; it->bor = it->bor->prev) {
      ...
      if (b->is_def) {
        ...
        if (b->is_real) output_on_shrink = 1;
      } else
      ...
      /* if its the last usage the irn dies */
      {
        ...
        if (output_on_shrink) {
          ...
          foreach_pset(it->living, ir_node, irn) { it->buf[count++] = irn; }

          assert(count > 0 &&
                 "We have a 'last usage', so there must be sth. in it->living");
    ...
    it->bor = NULL;
    assert(0 == pset_count(it->living) &&
           "Something has survived! (At the end of the block it->living must "
           "be empty)");
  }

  if (it->n_blocks != -1) free_clique_iter(it);
...
int be_ifg_cliques_next(cliques_iter_t *iter) { return get_next_clique(iter); }

void be_ifg_cliques_break(cliques_iter_t *iter) { free_clique_iter(iter); }
...
static bool consider_component_node(bitset_t *const seen, ir_node *const irn) {
  if (bitset_is_set(seen, get_irn_idx(irn))) return false;
...
static void int_comp_rec(be_ifg_t *ifg, ir_node *n, bitset_t *seen) {
  ...
  be_ifg_foreach_neighbour(ifg, &neigh_it, n, m) {
    if (consider_component_node(seen, m)) int_comp_rec(ifg, m, seen);
  }
}
...
void be_ifg_stat(ir_graph *irg, be_ifg_t *ifg, be_ifg_stat_t *stat) {
  ...
  be_ifg_foreach_node(ifg, n) {
    ...
    neighbours_iter_t neigh_it;
    be_ifg_foreach_neighbour(ifg, &neigh_it, n, m) { ++n_edges; }
