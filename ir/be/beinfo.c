...
void be_info_init_irn(ir_node *const node, arch_irn_flags_t const flags,
                      arch_register_req_t const **const in_reqs,
                      unsigned const n_res) {
...
static void dump_backend_info_hook(void *context, FILE *F,
                                   const ir_node *node) {
  (void)context;

  ir_graph *const irg = get_irn_irg(node);
  if (!irg_is_constrained(irg, IR_GRAPH_CONSTRAINT_BACKEND)) return;

  be_dump_reqs_and_registers(F, node);

  if (is_Block(node)) {
    be_lv_t *const lv = be_get_irg_liveness(irg);
    if (lv->sets_valid) be_dump_liveness_block(lv, F, node);
  }
...
void be_info_init(void) {
  if (initialized) panic("double initialization of be_info");
...
static void sched_edge_hook(FILE *F, const ir_node *irn) {
  ...
  if (is_Proj(irn) || is_Block(irn) || !sched_is_scheduled(irn)) return;
...
void be_info_free(void) {
  if (!initialized) panic("called without prior init");
