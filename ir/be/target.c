...
int ir_target_set_triple(ir_machine_triple_t const *machine) {
  ...
  if (ir_is_cpu_x86_32(cpu)) {
    ...
    isa = &sparc_isa_if;
    if (strstr(manufacturer, "leon") != NULL || streq(manufacturer, "invasic"))
      arch = "leon";
  ...
  if (arch != NULL) {
    ...
    if (!res) panic("Could not set backend arch");
  }
...
void ir_target_init(void) {
  ...
  unsigned const ptr_modulo_shift =
      modulo_shift == 0 ? 0 : MAX(modulo_shift, pointer_size * 8);
  ir_mode *const ptr_mode =
      new_reference_mode("P", pointer_size * 8, ptr_modulo_shift);
  set_modeP(ptr_mode);
...
void finish_target(void) { ir_platform_finish(); }
...
int(ir_target_big_endian)(void) { return ir_target_big_endian_(); }

unsigned ir_target_biggest_alignment(void) {
  return 1u << ir_target.isa->po2_biggest_alignment;
}

unsigned ir_target_pointer_size(void) { return ir_target.isa->pointer_size; }
...
int ir_target_supports_pic(void) { return ir_target.isa->pic_supported; }
