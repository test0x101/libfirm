...
ir_node **be_get_cfgpostorder(ir_graph *irg) {
  ...
  /* walk blocks */
  irg_block_edges_walk(get_irg_start_block(irg), NULL, add_to_postorder, &list);
...
ir_node *insert_Perm_before(ir_graph *irg, const arch_register_class_t *cls,
  ...
  size_t p = 0;
  foreach_ir_nodeset(&live, irn, iter) { nodes[p++] = irn; }
  ir_nodeset_destroy(&live);
...
static void remove_dead_nodes_walker(ir_node *block, void *data) {
  ...
  sched_foreach_safe(block, node) {
    if (bitset_is_set(env->reachable, get_irn_idx(node))) continue;

    if (env->lv->sets_valid) be_liveness_remove(env->lv, node);
    sched_remove(node);
    ...
    if (get_irn_mode(node) == mode_T) {
      foreach_out_edge_safe(node, edge) {
        ...
        if (env->lv->sets_valid) be_liveness_remove(env->lv, proj);
        kill_node(proj);
      }
    }
    kill_node(node);
