...
static void be_set_orig_node_rec(ir_node *const node, char const *const name) {
  ...
  foreach_irn_in(node, i, in) { be_set_orig_node_rec(in, name); }
}

static void be_set_orig_node(ir_node *const new_node,
                             ir_node const *const old_node) {
  if (!is_Proj(old_node)) {
...
bool be_is_transformed(const ir_node *node) { return irn_visited(node); }
...
static ir_node *transform_proj(ir_node *node) {
  ...
  if (!proj_transform) {
    ...
    if (is_Proj(pred)) {
      ...
      panic("no transformer for %+F (%u) -> %+F (%u) -> %+F", node, node_pn,
            pred, pred_pn, pred_pred);
    } else {
...
ir_node *be_duplicate_node(ir_node *const node) {
  ...
  foreach_irn_in(node, i, in) { ins[i] = be_transform_node(in); }
...
#ifdef DEBUG_libfirm
    if (!transform) panic("no transformer for %+F", node);
#endif
...
void be_enqueue_operands(ir_node *node) {
  /* put the preds in the worklist */
  foreach_irn_in(node, i, pred) { deq_push_pointer_right(&env.worklist, pred); }
}
...
static void transform_nodes(ir_graph *irg, arch_pretrans_nodes *pre_transform) {
  ...
  if (pre_transform) pre_transform(irg);
  ...
  /* Fix loops. */
  inc_irg_visited(irg);
  foreach_irn_in_r(new_anchor, i, n) { fix_loops(n); }
...
bool be_upper_bits_clean(const ir_node *node, ir_mode *mode) {
  ...
  if (op->ops.generic2 == NULL) return false;
...
static bool shr_upper_bits_clean(const ir_node *node, ir_mode *mode) {
  if (mode_is_signed(mode)) {
    ...
    if (is_Const(right)) {
      ...
      if (val >= 32 - (long)get_mode_size_bits(mode)) return true;
    }
...
static bool conv_upper_bits_clean(const ir_node *node, ir_mode *mode) {
  ...
  if (mode_is_float(src_mode)) return true;

  unsigned src_bits = get_mode_size_bits(src_mode);
  unsigned dest_bits = get_mode_size_bits(dest_mode);
  /* downconvs are a nop */
  if (src_bits >= dest_bits) return be_upper_bits_clean(op, mode);
...
static bool proj_upper_bits_clean(const ir_node *node, ir_mode *mode) {
  ...
  switch (get_irn_opcode(pred)) {
    case iro_Load: {
      ...
      if (load_bits > get_mode_size_bits(mode)) return false;
      if (mode_is_signed(load_mode) != mode_is_signed(mode)) return false;
      return true;
    }
    default:
      break;
...
bool be_pattern_is_rotl(ir_node const *const irn_or, ir_node **const left,
  ...
  if (!ir_is_negated_value(c1, c2)) return false;
...
void be_map_exc_node_to_runtime_call(ir_node *node, ir_mode *res_mode,
  ...
  foreach_irn_in(node, i, n) {
    if (get_irn_mode(n) == mode_M) continue;
    in[p++] = n;
  }
  assert(p == n_in);
...
static int cmp_stack_dependency(const void *c1, const void *c2) {
  ...
  if (bl1 != bl2) return get_irn_idx(bl2) - get_irn_idx(bl1);
  ...
  if (dependent_on(n1, b2)) return 1;
  if (dependent_on(n2, b1)) return -1;
...
void be_stack_record_chain(be_stack_env_t *const env, ir_node *const before,
                           unsigned const pos, ir_node *const after) {
...
void be_stack_finish(be_stack_env_t *const env) {
  ...
  if (n_changes != 0) {
    ...
    for (unsigned n = n_changes; n-- != 0;) {
      ...
      if (block == prev_block) set_irn_n(c[1].before, c[1].pos, c[0].after);
      prev_block = block;
    }
  }
...
void be_add_parameter_entity_stores_list(ir_graph *irg, unsigned n_entities,
  ...
  for (unsigned i = 0; i < n_entities; ++i) {
    ...
    if (entity->attr.parameter.is_lowered_doubleword) {
      ...
      ir_node *store0 =
          new_r_Store(start_block, mem, addr, val0, tp, cons_none);
      ...
      ir_node *store1 =
          new_r_Store(start_block, mem0, next_addr, val1, tp, cons_none);
      mem = new_r_Proj(store1, mode_M, pn_Store_M);
      if (first_store == NULL) first_store = store0;

    } else {
      ir_mode *const mode = is_compound_type(tp) ? mode_P : get_type_mode(tp);
      ir_node *const val = new_r_Proj(args, mode, arg);
      ir_node *const store =
          new_r_Store(start_block, mem, addr, val, tp, cons_none);
      mem = new_r_Proj(store, mode_M, pn_Store_M);
      if (first_store == NULL) first_store = store;
    }
  }
...
void be_add_parameter_entity_stores(ir_graph *irg) {
  ...
  for (size_t i = 0, n = get_compound_n_members(type); i < n; ++i) {
    ...
    if (get_entity_offset(entity) != INVALID_OFFSET) continue;
...
ir_node *be_skip_downconv(ir_node *node, bool const single_user) {
  ...
  for (;;) {
    if (single_user && get_irn_n_edges(node) > 1) {
      ...
      if (!be_mode_needs_gp_reg(src_mode) ||
          get_mode_size_bits(get_irn_mode(node)) > get_mode_size_bits(src_mode))
        break;
      node = op;
    } else {
      break;
...
ir_node *be_skip_sameconv(ir_node *node) {
  ...
  for (;;) {
    if (get_irn_n_edges(node) > 1) {
      ...
      ir_mode *const src_mode = get_irn_mode(op);
      if (!be_mode_needs_gp_reg(src_mode) ||
          get_mode_size_bits(get_irn_mode(node)) !=
              get_mode_size_bits(src_mode))
        break;
      node = op;
    } else {
      break;
...
bool be_match_immediate(ir_node const *const node, ir_tarval **const tarval_out,
                        ir_entity **const entity_out,
                        unsigned *reloc_kind_out) {
  unsigned reloc_kind;
  ir_entity *entity;
  ir_node const *cnst;
  if (is_Const(node)) {
  ...
  if (entity && is_tls_entity(entity)) return false;
...
ir_node *be_make_Sync(ir_node *const block, int const arity,
                      ir_node **const ins) {
  return arity == 1 ? ins[0] : new_r_Sync(block, arity, ins);
}

unsigned be_get_out_for_reg(ir_node const *const node,
                            arch_register_t const *const reg) {
  be_foreach_out(node, o) {
    arch_register_req_t const *const req =
        arch_get_irn_register_req_out(node, o);
    if (req == reg->single_req) return o;
  }
