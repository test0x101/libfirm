...
static unsigned _be_liveness_bsearch(be_lv_info_t const *const arr,
...
static be_lv_info_node_t *be_lv_get_or_set(be_lv_t *li, ir_node *bl,
  ...
  if (res->node != irn) {
    ...
    if (n_members + 1 >= n_size) {
      ...
      be_lv_info_t *const nw =
          OALLOCF(&li->obst, be_lv_info_t, nodes, new_size);
      memcpy(nw, irn_live,
             sizeof(*irn_live) + n_size * sizeof(*irn_live->nodes));
      memset(&nw->nodes[n_size], 0,
             (new_size - n_size) * sizeof(*irn_live->nodes));
      nw->n_size = new_size;
      irn_live = nw;
...
static void lv_remove_irn_walker(ir_node *const bl, void *const data) {
  ...
  be_lv_info_t *const irn_live =
      ir_nodehashmap_get(be_lv_info_t, &w->lv->map, bl);
  if (irn_live == NULL) return;
...
static void live_end_at_block(ir_node *const block, be_lv_state_t const state) {
  ...
  assert(state == be_lv_state_end ||
         state == (be_lv_state_end | be_lv_state_out));
  ...
  if (before != be_lv_state_none) return;

  /* Stop going up further, if this is the block of the definition. */
  if (re.def_block == block) return;
...
static void liveness_for_node(ir_node *irn) {
  ...
  foreach_out_edge(irn, edge) {
    ...
    if (!is_liveness_node(use)) continue;
...
static void collect_liveness_nodes(ir_node *irn, void *data) {
  ...
  if (is_liveness_node(irn)) nodes[get_irn_idx(irn)] = irn;
}

void be_liveness_compute_sets(be_lv_t *lv) {
  if (lv->sets_valid) return;
  ...
  for (unsigned i = 0; i < n; ++i) {
    if (nodes[i] != NULL) liveness_for_node(nodes[i]);
  }
...
static bool value_live_after(const ir_node *const value,
  ...
  if (be_is_live_end(lv, bb, value)) return true;
...
bool be_value_live_after(const ir_node *value, const ir_node *after) {
  assert(value != after);
  if (!value_strictly_dominates(value, after)) return false;
...
static bool live_at_user(const ir_node *const users_of,
  ...
  foreach_out_edge(users_of, edge) {
    const ir_node *const user = get_edge_src_irn(edge);
    if (is_Sync(user)) return live_at_user(user, node, bb);
    if (get_nodes_block(user) == bb && !is_Phi(user) &&
...
static const ir_node *get_highest_sync_op(const ir_node *const sync) {
  ...
  for (int i = 1, arity = get_irn_arity(sync); i < arity; ++i) {
    ...
    if (is_Sync(other)) other = get_highest_sync_op(other);
    if (value_strictly_dominates(other, best)) best = other;
  }
  return best;
}

bool be_memory_values_interfere(const ir_node *a, const ir_node *b) {
  if (is_Sync(a)) a = get_highest_sync_op(a);
  if (is_Sync(b)) b = get_highest_sync_op(b);
  ...
  const be_lv_t *const lv = be_get_irg_liveness(irg);
  if (be_is_live_end(lv, bb, a)) return true;
...
static void be_live_chk_compare(be_lv_t *lv, lv_chk_t *lvc) {
  ...
  for (unsigned j = 0; nodes[j] != NULL; ++j) {
    ...
    for (unsigned i = 0; blocks[i] != NULL; ++i) {
      ...
      if (lvr_in != lvc_in)
        ir_fprintf(stderr,
                   "live in  info for %+F at %+F differs: nml: %d, chk: %d\n",
                   irn, bl, lvr_in, lvc_in);

      if (lvr_end != lvc_end)
        ir_fprintf(stderr,
                   "live end info for %+F at %+F differs: nml: %d, chk: %d\n",
                   irn, bl, lvr_end, lvc_end);

      if (lvr_out != lvc_out)
        ir_fprintf(stderr,
                   "live out info for %+F at %+F differs: nml: %d, chk: %d\n",
