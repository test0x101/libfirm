...
static void emit_section_macho(be_gas_section_t const section) {
  ...
  static const macho_sectioninfo_t macho_sectioninfos[] = {
      ...
      [GAS_SECTION_PIC_SYMBOLS] = {"__IMPORT,__pointers",
  ...
  if (flags & GAS_SECTION_FLAG_TLS)
    panic("thread local storage not supported on macho (section 0x%X)",
          section);
  ...
  if (!info->segment_section) panic("Unsupported macho section requested");

  be_emit_irprintf("\t.section\t%s", info->segment_section);
  if (info->flags) be_emit_irprintf(",%s", info->flags);
  be_emit_char('\n');
  be_emit_write_line();
}
...
static void emit_section_sparc(be_gas_section_t section,
  ...
  /* Part1: section-name */
  if (flags & GAS_SECTION_FLAG_TLS) be_emit_char('t');
  ...
  if (flags != 0) {
    ...
    switch (base) {
      case GAS_SECTION_TEXT:
        be_emit_cstring(",#execinstr");
        break;
      case GAS_SECTION_DATA:
      case GAS_SECTION_BSS:
        be_emit_cstring(",#write");
        break;
      default: /* nothing */
        break;
    }
    if (flags & GAS_SECTION_FLAG_TLS) be_emit_cstring(",#tls");
  }
...
static void emit_section_elf_coff(be_gas_section_t const section,
  ...
  if (flags & GAS_SECTION_FLAG_TLS) be_emit_char('T');
  if (flags & GAS_SECTION_FLAG_COMDAT) be_emit_char('G');
...
static void emit_section(be_gas_section_t const section,
                         ir_entity const *const entity) {
  if (is_macho()) {
...
static bool initializer_is_string_const(const ir_initializer_t *initializer,
  ...
  if (initializer->kind != IR_INITIALIZER_COMPOUND) return false;
  ...
  for (size_t i = 0; i < len; ++i) {
    ir_initializer_t *sub_initializer = initializer->compound.initializers[i];

    ir_tarval *tv = get_initializer_tarval(sub_initializer);
    if (!tarval_is_constant(tv)) return false;

    ir_mode *mode = get_tarval_mode(tv);
    if (!mode_is_int(mode) || get_mode_size_bits(mode) != 8) return false;
...
static bool initializer_is_null(const ir_initializer_t *initializer) {
  switch (initializer->kind) {
    ...
    case IR_INITIALIZER_CONST: {
      ...
      if (!is_Const(value)) return false;
    ...
    case IR_INITIALIZER_COMPOUND: {
      for (size_t i = 0; i < initializer->compound.n_initializers; ++i) {
        ...
        if (!initializer_is_null(subinitializer)) return false;
      }
      return true;
    }
  }
...
static bool entity_is_string_const(const ir_entity *ent,
  ...
  if (!is_Array_type(type)) return false;

  /* and the array's element type is primitive */
  ir_type *element_type = get_array_element_type(type);
  if (!is_Primitive_type(element_type)) return false;
...
static bool entity_is_zero_initialized(ir_entity const *entity) {
  if (is_alias_entity(entity)) return false;
...
static bool is_comdat(const ir_entity *entity) {
  ...
  return (linkage & IR_LINKAGE_MERGE) && (linkage & IR_LINKAGE_GARBAGE_COLLECT);
}
...
static be_gas_section_t determine_basic_section(const ir_entity *entity) {
  ...
  if (get_entity_linkage(entity) & IR_LINKAGE_CONSTANT) {
    ...
    if (is_macho() && get_entity_linkage(entity) & IR_LINKAGE_NO_IDENTITY &&
    ...
    if (ir_platform.pic_style != BE_PIC_NONE) {
      ...
      switch (classify_initializer_relocs(init)) {
        case ONLY_LOCAL_RELOCATIONS:
          return GAS_SECTION_REL_RO_LOCAL;
        case ANY_RELOCATIONS:
          return GAS_SECTION_REL_RO;
        case NO_RELOCATIONS:
  ...
  if (entity_is_zero_initialized(entity)) return GAS_SECTION_BSS;

  return GAS_SECTION_DATA;
}

static be_gas_section_t determine_section(be_main_env_t const *const main_env,
                                          ir_entity const *const entity) {
  ir_type *owner = get_entity_owner(entity);

  if (owner == get_segment_type(IR_SEGMENT_GLOBAL)) {
    be_gas_section_t section = determine_basic_section(entity);
    if (is_comdat(entity)) section |= GAS_SECTION_FLAG_COMDAT;
    return section;
...
static void emit_visibility(const ir_entity *entity, bool implicit_globl) {
  ...
  if (linkage & IR_LINKAGE_WEAK) emit_weak(entity);

  bool output_global = !implicit_globl;
  const char *const directive =
      get_visibility_directive(entity, &output_global);
  if (output_global) emit_symbol_directive(".globl", entity);
  if (directive != NULL) emit_symbol_directive(directive, entity);
...
void be_gas_emit_function_prolog(const ir_entity *entity, unsigned po2alignment,
  ...
  switch (ir_platform.object_format) {
    ...
    case OBJECT_FORMAT_PE_COFF:
      ...
      be_gas_emit_entity(entity);
      char const vis =
          get_entity_visibility(entity) == ir_visibility_local ? '3' : '2';
 ...
 */
static void emit_tv(ir_tarval *const tv, unsigned const offset,
                    unsigned const n) {
  be_emit_cstring("0x");
 ...
 */
const char *be_gas_insn_label_prefix(void) { return ".LE"; }
...
static void emit_init_expression(ir_node *const init) {
  ...
  switch (get_irn_opcode(init)) {
    ...
    case iro_Mul:
      if (!mode_is_int(mode)) panic("constant must be int for '*' to work");
...
static size_t get_initializer_size(const ir_initializer_t *initializer,
  ...
  switch (get_initializer_kind(initializer)) {
    case IR_INITIALIZER_TARVAL:
      assert(get_tarval_mode(get_initializer_tarval_value(initializer)) ==
             get_type_mode(type));
    ...
    case IR_INITIALIZER_COMPOUND:
      if (is_Array_type(type)) {
        if (get_array_size(type) == 0) {
          ...
          size_t n_inits = get_initializer_compound_n_entries(initializer);
        ...
        if (n != 0) {
          ...
          if (is_Array_type(last_type) && get_array_size(last_type) == 0) {
            ir_initializer_t const *const last =
                get_initializer_compound_value(initializer, n - 1);
...
static unsigned long compute_entity_size(ir_entity const *const entity) {
  ...
  if (is_alias_entity(entity)) return size;
  ...
  if (initializer != NULL) size = get_initializer_size(initializer, type);
  return size;
}
...
static void emit_bitfield(normal_or_bitfield *vals, unsigned offset_bits,
  ...
  switch (get_initializer_kind(initializer)) {
    ...
    case IR_INITIALIZER_CONST: {
      ...
      if (!is_Const(node)) panic("bitfield initializer not a Const node");
  ...
  while (bit_offset < end) {
    ...
    normal_or_bitfield *const val =
        big_endian ? &vals[value_len - dst_offset - 1] : &vals[dst_offset];
  ...
  switch (get_initializer_kind(initializer)) {
    ...
    case IR_INITIALIZER_COMPOUND:
      if (is_Array_type(type)) {
        ...
        if (misalign != 0) skip += alignment - misalign;
...
static void emit_common(const ir_entity *entity, unsigned long size,
  ...
  switch (ir_platform.object_format) {
    case OBJECT_FORMAT_MACH_O:
      ...
      return;
    case OBJECT_FORMAT_ELF:
      if (is_local) emit_symbol_directive(".local", entity);
      be_emit_cstring("\t.comm ");
      be_gas_emit_entity(entity);
      be_emit_irprintf(",%lu,%u\n", size, alignment);
      be_emit_write_line();
...
static void emit_indirect_symbol(const ir_entity *entity,
  ...
  if (!is_macho()) panic("indirect_symbol entities only supported for ELF");
...
char const *be_gas_get_private_prefix(void) { return is_macho() ? "L" : ".L"; }

static bool check_needs_quotes(char const *const s) {
  if (is_macho()) {
    for (char const *i = s; *i != '\0'; ++i) {
      if ((unsigned char)*i >= 128) return true;
    }
  }
  return false;
}
...
static bool block_needs_label(ir_node const *const block) {
  if (get_Block_entity(block)) return true;
  ...
  if (n_cfgpreds == 0) {
    ...
    if (be_emit_get_prev_block(block) != cfgpred_block) return true;

    if (is_x_regular_Proj(cfgpred)) return false;
...
void be_gas_begin_block(ir_node const *const block) {
  if (block_needs_label(block)) {
    ...
    if (!be_options.verbose_asm) return;
    be_emit_cstring("/*");
    be_gas_emit_block_name(block);
    be_emit_cstring(":*/");
  }
...
static void emit_global(be_main_env_t const *const main_env,
  ...
  if (kind == IR_ENTITY_LABEL) return;
  ...
  if (!entity_has_definition(entity)) return;
  ...
  if (!is_po2_or_zero(alignment)) panic("alignment not a power of 2");
  if (alignment > 1) emit_align(alignment);
  if (ir_platform.object_format == OBJECT_FORMAT_ELF && be_gas_emit_types &&
      visibility != ir_visibility_private) {
    be_emit_cstring("\t.type\t");
    be_gas_emit_entity(entity);
 ...
 */
static void be_gas_emit_globals(ir_type *const gt,
                                be_main_env_t const *const main_env) {
...
void be_emit_jump_table(ir_node const *const node,
                        be_switch_attr_t const *const swtch,
                        ir_mode *const entry_mode,
                        emit_target_func const emit_target) {
  ...
  for (size_t e = 0; e < n_entries; ++e) {
    ...
    if (!tarval_is_long(max)) panic("switch case overflow (%+F)", node);
  ...
  for (size_t e = 0; e < n_entries; ++e) {
    ...
    if (min == max) {
      ...
      if (!tarval_is_long(min)) panic("switch case overflow (%+F)", node);
  ...
  if (entity) {
    if (!is_macho()) be_gas_emit_switch_section(GAS_SECTION_RODATA);
  ...
  if (entity && !is_macho()) be_gas_emit_switch_section(GAS_SECTION_TEXT);
...
void be_emit_finish_line_gas(const ir_node *node) {
  if (node && be_options.verbose_asm) {
    ...
    char const *const fmt = !loc.file       ? "/* %+F */\n"
                            : loc.line == 0 ? "/* %+F %s */\n"
                            : loc.column == 0
                                ? "/* %+F %s:%u */\n"
                                :
