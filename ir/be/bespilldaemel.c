...
static void do_spilling(ir_nodeset_t *live_nodes, ir_node *node) {
  ...
  be_foreach_definition(node, cls, value, req, (void)value;
                        assert(req->width >= 1); values_defined += req->width;);

  /* we need registers for the non-live argument values */
  size_t free_regs_needed = 0;
  be_foreach_use(
      node, cls, in_req_, use, pred_req_,
  ...
  be_add_pressure_t const add_pressure =
      arch_get_additional_pressure(node, cls);
  ...
  while (spills_needed > 0) {
    ...
    if (arch_irn_is(skip_Proj_const(cand_node), dont_spill)) continue;
...
static __attribute__((unused)) void print_nodeset(ir_nodeset_t *nodeset) {
  foreach_ir_nodeset(nodeset, node, iter) { ir_fprintf(stderr, "%+F ", node); }
  fprintf(stderr, "\n");
}
...
static void spill_block(ir_node *block, void *data) {
  ...
  sched_foreach_phi(block, node) {
    if (phi_spills_needed <= 0) break;

    if (!bitset_is_set(spilled_nodes, get_irn_idx(node))) continue;
