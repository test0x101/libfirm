...
static lc_opt_enum_mask_var_t dump_var = {&be_options.dump_flags, dump_items};

static const lc_opt_table_entry_t be_main_options[] = {
    LC_OPT_ENT_ENUM_MASK("dump", "dump irg on several occasions", &dump_var),
    LC_OPT_ENT_BOOL("omitfp", "omit frame pointer", &be_options.omit_fp),
    LC_OPT_ENT_BOOL("verify", "verify the backend irg", &be_options.do_verify),
    LC_OPT_ENT_BOOL("time", "get backend timing statistics",
                    &be_options.timing),
    LC_OPT_ENT_BOOL("profilegenerate",
                    "instrument the code for execution count profiling",
                    &be_options.opt_profile_generate),
    LC_OPT_ENT_BOOL("profileuse", "use existing profile data",
                    &be_options.opt_profile_use),
    LC_OPT_ENT_BOOL("verboseasm", "enable verbose assembler output",
                    &be_options.verbose_asm),
...
void be_set_constraint_support(asm_constraint_flags_t const flags,
...
static void be_init_default_asm_constraint_flags(void) {
  ...
  be_set_constraint_support(ASM_CONSTRAINT_FLAG_NO_SUPPORT,
                            ",<>EFGHIJKLMNOPVXgimoprs");
  /* Skip whitespace.
...
asm_constraint_flags_t be_parse_asm_constraints(const char *constraint) {
  ...
  switch (*c) {
    ...
    case '+':
      ...
      break;
    case '%':
      ++c; /* FALLTHROUGH */ /* TODO stub, should mark this operand as
                                commutative with the next. */
    default:
      flags = ASM_CONSTRAINT_FLAG_MODIFIER_READ;
      break;
  }
...
int be_is_valid_clobber(const char *clobber) {
  ...
  if (streq(clobber, "memory")) return 1;
  /* cc (condition code) is always valid */
  if (streq(clobber, "cc")) return 1;
...
static ir_graph *be_prepare_profile(const char *const cup_name) {
  ...
  if (!have_profile) {
    ...
    foreach_irp_irg(i, irg) { ir_estimate_execfreq(irg); }
...
void be_begin(FILE *file_handle, const char *cup_name) {
  ...
  if (get_irp_n_irgs() > 0 &&
      !irg_is_constrained(get_irp_irg(0), IR_GRAPH_CONSTRAINT_TARGET_LOWERED))
    be_lower_for_target();
  ...
  env.pic_trampolines_type =
      new_type_segment(NEW_IDENT("$PIC_TRAMPOLINE_TYPE"), tf_none);
  env.ent_pic_symbol_map = pmap_create();
  env.pic_symbols_type =
      new_type_segment(NEW_IDENT("$PIC_SYMBOLS_TYPE"), tf_none);
  ...
  foreach_irp_irg(i, irg) {
    ...
    if (get_entity_linkage(entity) & IR_LINKAGE_NO_CODEGEN) continue;
    initialize_birg(&birgs[num_birgs++], irg, &env);
    if (ir_target.isa->handle_intrinsics) ir_target.isa->handle_intrinsics(irg);
...
static const char *get_timer_name(be_timer_id_t id) {
  switch (id) {
    ...
    case T_CODEGEN:
      return "codegen";
    case T_RA_PREPARATION:
      return "ra_preparation";
    case T_SCHED:
      return "sched";
    case T_CONSTR:
      return "constr";
    ...
    case T_RA_SPILL:
      return "ra_spill";
    case T_RA_SPILL_APPLY:
      return "ra_spill_apply";
    case T_RA_COLOR:
      return "ra_color";
    case T_RA_IFG:
      return "ra_ifg";
...
void be_after_irp_transform(const char *name) {
  if (be_after_transform == NULL) return;

  foreach_irp_irg_r(i, irg) { be_after_transform(irg, name); }
}
...
bool be_step_first(ir_graph *irg) {
  ...
  if (get_entity_linkage(entity) & IR_LINKAGE_NO_CODEGEN) return false;
...
void be_step_last(ir_graph *irg) {
  ...
  if (be_timing) {
    if (stat_ev_enabled) {
      for (be_timer_id_t t = T_FIRST; t < T_LAST + 1; ++t) {
        char buf[128];
        snprintf(buf, sizeof(buf), "bemain_time_%s", get_timer_name(t));
...
ir_jit_function_t *be_jit_compile(ir_jit_segment_t *const segment,
                                  ir_graph *const irg) {
  if (ir_target.isa->jit_compile == NULL) return NULL;

  obstack_init(&obst);

  ir_entity *entity = get_irg_entity(irg);
  if (get_entity_linkage(entity) & IR_LINKAGE_NO_CODEGEN) return NULL;
