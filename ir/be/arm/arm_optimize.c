...
static void peephole_be_IncSP(ir_node *node) {
  ...
  arm_vals v;
  if (allowed_arm_immediate(offset, &v)) return;
  ...
  for (unsigned cnt = 1; cnt < v.ops; ++cnt) {
    ...
    ir_node *const incsp =
        be_new_IncSP(block, node, value, be_get_IncSP_no_align(node));
...
typedef ir_node *new_binop_imm(dbg_info *, ir_node *, ir_node *, unsigned char,
                               unsigned char);

static ir_node *gen_ptr(new_binop_imm *const cons, ir_node *const node,
                        ir_node *const frame, arm_vals const *const v) {
...
bool arm_is_valid_offset(int32_t const v, ir_mode *const mode,
                         bool const is_store) {
