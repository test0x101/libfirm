...
static const arch_register_t *const callee_saves[] = {
    &arm_registers[REG_R4],  &arm_registers[REG_R5],  &arm_registers[REG_R6],
    &arm_registers[REG_R7],  &arm_registers[REG_R8],  &arm_registers[REG_R9],
    &arm_registers[REG_R10], &arm_registers[REG_R11], &arm_registers[REG_LR],
};

static const arch_register_t *const caller_saves[] = {
    &arm_registers[REG_R0], &arm_registers[REG_R1], &arm_registers[REG_R2],
    &arm_registers[REG_R3], &arm_registers[REG_LR],

    &arm_registers[REG_F0], &arm_registers[REG_F1], &arm_registers[REG_F2],
    &arm_registers[REG_F3], &arm_registers[REG_F4], &arm_registers[REG_F5],
    &arm_registers[REG_F6], &arm_registers[REG_F7],
};
...
static ir_node *create_const_graph_value(dbg_info *dbgi, ir_node *block,
  ...
  if (vn.ops < v.ops) {
    ...
    for (unsigned cnt = 1; cnt < vn.ops; ++cnt) {
      result =
          new_bd_arm_Bic_imm(dbgi, block, result, vn.values[cnt], vn.rors[cnt]);
    }
  } else {
    /* add bits */
    ...
    for (unsigned cnt = 1; cnt < v.ops; ++cnt) {
      result =
          new_bd_arm_Orr_imm(dbgi, block, result, v.values[cnt], v.rors[cnt]);
    }
  }
  return result;
}
...
static ir_node *gen_zero_extension(dbg_info *dbgi, ir_node *block, ir_node *op,
  ...
  if (src_bits == 8) {
    ...
    ir_node *lshift =
        new_bd_arm_Mov_reg_shift_imm(dbgi, block, op, ARM_SHF_LSL_IMM, 16);
    ir_node *rshift =
        new_bd_arm_Mov_reg_shift_imm(dbgi, block, lshift, ARM_SHF_LSR_IMM, 16);
    return rshift;
  } else {
...
static ir_node *gen_sign_extension(dbg_info *dbgi, ir_node *block, ir_node *op,
  ...
  ir_node *lshift_node = new_bd_arm_Mov_reg_shift_imm(
      dbgi, block, op, ARM_SHF_LSL_IMM, shift_width);
  ir_node *rshift_node = new_bd_arm_Mov_reg_shift_imm(
      dbgi, block, lshift_node, ARM_SHF_ASR_IMM, shift_width);
  return rshift_node;
}
...
static ir_node *gen_Conv(ir_node *node) {
  ...
  if (src_mode == dst_mode) return new_op;
  ...
  if (mode_is_float(src_mode) || mode_is_float(dst_mode)) {
    ...
    if (be_upper_bits_clean(op, min_mode)) return new_op;
...
static bool try_encode_val_as_immediate(uint32_t val,
                                        arm_immediate_t *const res) {
  if (val <= 0xff) {
    res->imm_8 = val;
    res->rot = 0;
    return true;
...
static imm_match_t try_encode_as_immediate(ir_node const *const node,
                                           arm_immediate_t *const res,
                                           imm_match_t const match) {
  if (!is_Const(node)) return IMM_NONE;
  uint32_t const val = get_Const_long(node);
  return match & IMM_POS && try_encode_val_as_immediate(val, res)    ? IMM_POS
...
typedef struct arm_binop_factory_t {
  ...
  ir_node *(*new_binop_reg)(dbg_info *dbgi, ir_node *block, ir_node *op1,
                            ir_node *op2);
  /** normal reg op imm operation. */
  ir_node *(*new_binop_imm)(dbg_info *dbgi, ir_node *block, ir_node *op1,
                            unsigned char imm8, unsigned char imm_rot);
  /** barrel shifter reg op (reg shift reg operation. */
  ir_node *(*new_binop_reg_shift_reg)(dbg_info *dbgi, ir_node *block,
                                      ir_node *left, ir_node *right,
                                      ir_node *shift,
                                      arm_shift_modifier_t shift_modifier);
  /** barrel shifter reg op (reg shift imm operation. */
  ir_node *(*new_binop_reg_shift_imm)(dbg_info *dbgi, ir_node *block,
                                      ir_node *left, ir_node *right,
                                      arm_shift_modifier_t shift_modifier,
...
static ir_node *gen_int_binop_ops(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  if ((flags & (MATCH_COMMUTATIVE | MATCH_REVERSE)) &&
      try_encode_as_immediate(op1, &imm, IMM_POS) != IMM_NONE) {
  ...
  if (is_arm_Mov(new_op2)) {
    const arm_shifter_operand_t *attr =
        get_arm_shifter_operand_attr_const(new_op2);
    ...
    switch (attr->shift_modifier) {
      ...
      case ARM_SHF_ROR_REG:
        if (factory->new_binop_reg_shift_reg) {
          ...
          return factory->new_binop_reg_shift_reg(
              dbgi, block, new_op1, mov_op, mov_sft, attr->shift_modifier);
        }
        break;
      case ARM_SHF_REG:
      case ARM_SHF_RRX:
  ...
  if ((flags & (MATCH_COMMUTATIVE | MATCH_REVERSE)) && is_arm_Mov(new_op1)) {
    const arm_shifter_operand_t *attr =
        get_arm_shifter_operand_attr_const(new_op1);
    ...
    switch (attr->shift_modifier) {
      ...
      case ARM_SHF_ROR_IMM:
        if (factory[idx].new_binop_reg_shift_imm) {
          ...
          return factory[idx].new_binop_reg_shift_imm(
              dbgi, block, new_op2, mov_op, attr->shift_modifier,
              attr->shift_immediate);
        }
        break;
      ...
      case ARM_SHF_ROR_REG:
        if (factory[idx].new_binop_reg_shift_reg) {
          ...
          return factory[idx].new_binop_reg_shift_reg(
              dbgi, block, new_op2, mov_op, mov_sft, attr->shift_modifier);
        }
        break;
...
static ir_node *gen_Ror(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  if (is_Const(op2)) {
    ...
    if (tarval_is_long(tv) && bits == 32) {
      ...
      return new_bd_arm_Mov_reg_shift_imm(dbgi, block, new_op1, ARM_SHF_ROR_IMM,
                                          val);
    }
  }
...
static ir_node *match_pkh(ir_node *node) {
  ...
  if (!is_And(left) || !is_And(right)) return NULL;
  ir_node *left_right = get_And_right(left);
  ir_node *right_right = get_And_right(right);
  if (!is_Const(left_right) || !is_Const(right_right)) return NULL;
  ...
  if (!is_low_mask(get_Const_tarval(left_right))) return NULL;
  ...
  static const arm_binop_factory_t pkhbt_pkhtb_factory[2] = {
      {new_bd_arm_Pkhbt_reg, new_bd_arm_Pkhbt_imm,
       new_bd_arm_Pkhbt_reg_shift_reg, new_bd_arm_Pkhbt_reg_shift_imm},
      {new_bd_arm_Pkhtb_reg, new_bd_arm_Pkhtb_imm,
       new_bd_arm_Pkhtb_reg_shift_reg, new_bd_arm_Pkhtb_reg_shift_imm}};
...
static ir_node *gen_Add(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    static const arm_binop_factory_t add_factory = {
        new_bd_arm_Add_reg, new_bd_arm_Add_imm, new_bd_arm_Add_reg_shift_reg,
...
static ir_node *gen_arm_AddS_t(ir_node *node) {
  ...
  ir_node *res = gen_int_binop_ops(
      node, left, right, MATCH_COMMUTATIVE | MATCH_SIZE_NEUTRAL, &adds_factory);
...
static ir_node *gen_arm_AdC_t(ir_node *node) {
  ...
  ir_node *res =
      new_bd_arm_AdC_reg(dbgi, new_block, new_left, new_right, new_flags);
  return res;
}
...
static ir_node *gen_And(ir_node *node) {
  static const arm_binop_factory_t and_factory = {
      new_bd_arm_And_reg, new_bd_arm_And_imm, new_bd_arm_And_reg_shift_reg,
      new_bd_arm_And_reg_shift_imm};
  static const arm_binop_factory_t bic_factory = {
      new_bd_arm_Bic_reg, new_bd_arm_Bic_imm, new_bd_arm_Bic_reg_shift_reg,
...
static ir_node *gen_Or(ir_node *node) {
  ...
  static const arm_binop_factory_t or_factory = {
      new_bd_arm_Orr_reg, new_bd_arm_Orr_imm, new_bd_arm_Orr_reg_shift_reg,
...
static ir_node *gen_arm_OrPl_t(ir_node *node) {
  ...
  dbg_info *dbgi = get_irn_dbg_info(node);
  ir_node *new_block = be_transform_nodes_block(node);
  ir_node *res = new_bd_arm_OrrPl(dbgi, new_block, new_left, new_right,
...
static ir_node *gen_Eor(ir_node *node) {
  static const arm_binop_factory_t eor_factory = {
      new_bd_arm_Eor_reg, new_bd_arm_Eor_imm, new_bd_arm_Eor_reg_shift_reg,
...
static ir_node *gen_Sub(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    static const arm_binop_factory_t sub_rsb_factory[2] = {
        {new_bd_arm_Sub_reg, new_bd_arm_Sub_imm, new_bd_arm_Sub_reg_shift_reg,
         new_bd_arm_Sub_reg_shift_imm},
        {new_bd_arm_Rsb_reg, new_bd_arm_Rsb_imm, new_bd_arm_Rsb_reg_shift_reg,
         new_bd_arm_Rsb_reg_shift_imm}};
    return gen_int_binop(node, MATCH_SIZE_NEUTRAL | MATCH_REVERSE,
                         sub_rsb_factory);
  }
}
...
static ir_node *gen_arm_SubS_t(ir_node *node) {
  ...
  ir_node *res = gen_int_binop_ops(
      node, left, right, MATCH_SIZE_NEUTRAL | MATCH_REVERSE, subs_factory);
...
static ir_node *gen_arm_SbC_t(ir_node *node) {
  ...
  ir_node *new_block = be_transform_nodes_block(node);
  ir_node *res =
      new_bd_arm_SbC_reg(dbgi, new_block, new_left, new_right, new_flags);
  return res;
}
  ...
  if (is_Const(op2)) {
    ...
    if (can_use_shift_constant(val, shift_modifier)) {
      switch (shift_modifier) {
        case ARM_SHF_LSL_REG:
          shift_modifier = ARM_SHF_LSL_IMM;
          break;
        case ARM_SHF_LSR_REG:
          shift_modifier = ARM_SHF_LSR_IMM;
          break;
        case ARM_SHF_ASR_REG:
          shift_modifier = ARM_SHF_ASR_IMM;
          break;
        case ARM_SHF_ROR_REG:
          shift_modifier = ARM_SHF_ROR_IMM;
          break;
        default:
          panic("unexpected shift modifier");
      }
      return new_bd_arm_Mov_reg_shift_imm(dbgi, block, new_op1, shift_modifier,
                                          val);
    }
  }
...
static ir_node *gen_Not(ir_node *node) {
  ...
  if (is_arm_Mov(new_op)) {
    const arm_shifter_operand_t *attr =
        get_arm_shifter_operand_attr_const(new_op);
    ...
    switch (attr->shift_modifier) {
      ...
      case ARM_SHF_ROR_IMM: {
        ...
        return new_bd_arm_Mvn_reg_shift_imm(
            dbgi, block, mov_op, attr->shift_modifier, attr->shift_immediate);
      }
...
static arm_am_t transform_am(ir_node *addr, ir_mode *const mode,
                             bool const is_store) {
  long offset = 0;
  if (is_Add(addr)) {
...
static ir_node *gen_Load(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    if (arm_cg_config.fpu == ARM_FPU_FPA) {
      new_load = new_bd_arm_Ldf(dbgi, block, am.base, new_mem, mode, am.entity,
                                0, am.offset, am.is_frame_entity);
    } else {
      panic("softfloat not lowered");
    }
  } else {
    new_load = new_bd_arm_Ldr(dbgi, block, am.base, new_mem, mode, am.entity, 0,
                              am.offset, am.is_frame_entity);
  }
...
static ir_node *gen_Store(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    if (arm_cg_config.fpu == ARM_FPU_FPA) {
      new_store = new_bd_arm_Stf(dbgi, block, am.base, new_val, new_mem, mode,
                                 am.entity, 0, am.offset, am.is_frame_entity);
    } else {
      panic("softfloat not lowered");
    }
  } else {
    ...
    ir_node *const new_val = be_transform_node(val);
    new_store = new_bd_arm_Str(dbgi, block, am.base, new_val, new_mem, mode,
                               am.entity, 0, am.offset, am.is_frame_entity);
  }
...
static ir_node *gen_Switch(ir_node *node) {
  ...
  if (get_mode_size_bits(mode) != 32) panic("unexpected switch selector mode");
...
static ir_node *gen_Cmp(ir_node *node) {
  ...
  switch (try_encode_as_immediate(op2, &imm, IMM_POS | IMM_NEG)) {
    case IMM_POS:
      return new_bd_arm_Cmp_imm(dbgi, block, new_op1, imm.imm_8, imm.rot, false,
                                is_unsigned);
    case IMM_NEG:
      return new_bd_arm_Cmn_imm(dbgi, block, new_op1, imm.imm_8, imm.rot, false,
                                is_unsigned);
    default: {
      ir_node *new_op2 = be_transform_node(op2);
...
static ir_node *gen_Address(ir_node *node) {
  ...
  dbg_info *dbgi = get_irn_dbg_info(node);
  if (is_tls_entity(entity)) panic("TLS not supported yet");
...
static ir_node *ints_to_double(dbg_info *dbgi, ir_node *block, ir_node *node0,
  ...
  ir_node *str0 = new_bd_arm_Str(dbgi, block, stack, node0, nomem, arm_mode_gp,
                                 NULL, 0, 0, true);
  ir_node *str1 = new_bd_arm_Str(dbgi, block, stack, node1, nomem, arm_mode_gp,
                                 NULL, 0, 4, true);
  ...
  ir_node *ldf =
      new_bd_arm_Ldf(dbgi, block, stack, sync, mode_D, NULL, 0, 0, true);
...
static ir_node *int_to_float(dbg_info *dbgi, ir_node *block, ir_node *node) {
  ...
  ir_node *stack = get_irg_frame(irg);
  ir_node *nomem = get_irg_no_mem(irg);
  ir_node *str = new_bd_arm_Str(dbgi, block, stack, node, nomem, arm_mode_gp,
                                NULL, 0, 0, true);
  set_irn_pinned(str, false);

  ir_node *ldf =
      new_bd_arm_Ldf(dbgi, block, stack, str, mode_F, NULL, 0, 0, true);
  set_irn_pinned(ldf, false);
...
static ir_node *float_to_int(dbg_info *dbgi, ir_node *block, ir_node *node) {
  ...
  ir_node *nomem = get_irg_no_mem(irg);
  ir_node *stf =
      new_bd_arm_Stf(dbgi, block, stack, node, nomem, mode_F, NULL, 0, 0, true);
  set_irn_pinned(stf, false);

  ir_node *ldr =
      new_bd_arm_Ldr(dbgi, block, stack, stf, arm_mode_gp, NULL, 0, 0, true);
...
static void double_to_ints(dbg_info *dbgi, ir_node *block, ir_node *node,
  ...
  ir_node *nomem = get_irg_no_mem(irg);
  ir_node *stf =
      new_bd_arm_Stf(dbgi, block, stack, node, nomem, mode_D, NULL, 0, 0, true);
  set_irn_pinned(stf, false);
  ...
  ir_node *ldr1 =
      new_bd_arm_Ldr(dbgi, block, stack, stf, arm_mode_gp, NULL, 0, 4, true);
  ...
  if (reg0 != NULL) {
    ...
    if (mode_is_float(reg0->cls->mode)) {
      ...
      if (reg1 != NULL) {
        ...
        ir_node *const ldr = new_bd_arm_Ldr(
            NULL, new_block, fp, mem, arm_mode_gp, param->entity, 0, 0, true);
    ...
    ir_node *value;
    if (mode_is_float(mode)) {
      load = new_bd_arm_Ldf(NULL, new_block, fp, mem, mode, param->entity, 0, 0,
                            true);
      value = be_new_Proj(load, pn_arm_Ldf_res);
    } else {
      load = new_bd_arm_Ldr(NULL, new_block, fp, mem, mode, param->entity, 0, 0,
                            true);
...
static void create_stacklayout(ir_graph *irg,
  ...
  for (unsigned p = 0, n_params = cconv->n_parameters; p < n_params; ++p) {
    ...
    if (!param->reg0) {
      if (!ent) ent = new_parameter_entity(frame_type, p, param->type);
...
static ir_node *gen_Call(ir_node *node) {
  ...
  for (size_t p = 0; p < n_params; ++p) {
    ...
    if (mode_is_float(mode) && param->reg0 != NULL) {
      ...
      if (size_bits == 64) {
        double_to_ints(dbgi, new_block, new_value, &new_value, &new_value1);
      } else {
        assert(size_bits == 32);
  ...
  if (entity != NULL) {
    ...
    res = new_bd_arm_Bl(dbgi, new_block, in_arity, in, in_req, out_arity,
                        entity, 0);
  } else {
    /* TODO:
     ...
     */
    res = new_bd_arm_LinkMovPC(dbgi, new_block, in_arity, in, in_req, out_arity,
  ...
  for (size_t o = 0; o < n_caller_saves; ++o) {
    ...
    arch_set_irn_register_req_out(res, pn_arm_Bl_first_result + o,
                                  reg->single_req);
  }
...
static void arm_parse_constraint_letter(void const *const env,
...
static bool arm_check_immediate_constraint(long const val,
                                           char const imm_type) {
  (void)val;
  arm_immediate_t res;
  switch (imm_type) {
...
static bool arm_match_immediate(arm_asm_operand_t *const operand,
  ...
  if (offset) {
    ...
    if (!arm_check_immediate_constraint(value, imm_type)) return false;
  } else {
    value = 0;
  }
...
static ir_node *gen_ASM(ir_node *const node) {
  ...
  arm_asm_operand_t *const operands =
      NEW_ARR_DZ(arm_asm_operand_t, obst, n_constraints);
  for (size_t i = 0; i != n_constraints; ++i) {
    ...
    be_parse_asm_constraints_internal(&be_constraint, c->constraint,
                                      &arm_parse_constraint_letter, NULL);
