...
static ir_node *arm_new_reload(ir_node *value, ir_node *spill,
                               ir_node *before) {
  ...
  ir_node *load =
      new_bd_arm_Ldr(NULL, block, frame, spill, mode, NULL, false, 0, true);
...
static ir_node *arm_new_spill(ir_node *value, ir_node *after) {
  ...
  ir_mode *mode = get_irn_mode(value);
  ir_node *store = new_bd_arm_Str(NULL, block, frame, value, mem, mode, NULL,
                                  false, 0, true);
...
static void arm_create_runtime_entities(void) {
  if (divsi3 != NULL) return;

  ir_mode *mode_int =
      new_int_mode("arm_be_int", ARM_MACHINE_SIZE, true, ARM_MODULO_SHIFT);
  ir_mode *mode_uint =
      new_int_mode("arm_be_int", ARM_MACHINE_SIZE, false, ARM_MODULO_SHIFT);
...
static void arm_init(void) {
  arm_mode_gp = new_int_mode("arm_gp", ARM_MACHINE_SIZE, 0, ARM_MODULO_SHIFT);
...
static void arm_finish(void) { arm_free_opcodes(); }
...
static lc_opt_enum_int_var_t arch_fpu_var = {(int *)&arm_cg_config.fpu,
                                             arm_fpu_items};

static const lc_opt_enum_int_items_t arm_arch_items[] = {
    {"armv4", ARM_VARIANT_4}, {"armv5t", ARM_VARIANT_5T},
    {"armv6", ARM_VARIANT_6}, {"armv6t2", ARM_VARIANT_6T2},
    {"armv7", ARM_VARIANT_7}, {NULL, 0},
};
static lc_opt_enum_int_var_t arch_var = {(int *)&arm_cg_config.variant,
                                         arm_arch_items};
