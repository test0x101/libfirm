...
static void lower64_add(ir_node *const node) {
  ...
  ir_node *adc =
      new_bd_arm_AdC_t(dbgi, block, left_high, right_high, res_flags, mode);
...
static void lower64_sub(ir_node *const node) {
  ...
  ir_mode *mode = get_node_high_mode(node);
  ir_node *sbc =
      new_bd_arm_SbC_t(dbgi, block, left_high, right_high, res_flags, mode);
...
static ir_entity *make_divmod(char const *const name, ir_type *const even,
                              ir_type *const odd) {
  ir_type *const mtp =
      new_type_method(4, 4, false, cc_cdecl_set, mtp_no_property);
...
static void lower_divmod(ir_node *const node, ir_node *const left,
                         ir_node *const right, ir_node *const mem,
                         int const res_offset) {
  dbg_info *dbgi = get_irn_dbg_info(node);
  ir_node *block = get_nodes_block(node);
  ir_node *left_low = get_lowered_low(left);
  ir_node *left_high = get_lowered_high(left);
  ...
  ir_node *call = new_rd_Call(dbgi, block, mem, addr, ARRAY_SIZE(in), in, mtp);
...
static void lower64_shrs(ir_node *const node) {
  ...
  ir_node *orpl =
      new_bd_arm_OrPl_t(dbgi, block, or, shrs, or, subs_flags, umode);
