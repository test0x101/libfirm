...
static void introduce_epilog(ir_node *ret) {
  assert(arch_get_irn_register_req_in(ret, n_arm_Return_sp) ==
...
static void arm_determine_frameoffset(ir_node *node, int sp_offset) {
  if (be_is_MemPerm(node)) {
    ...
    if (arm_get_irg_data(irg)->omit_fp) be_set_MemPerm_offset(node, sp_offset);
    return;
  }
  if (!is_arm_irn(node)) return;
  ...
  if (is_arm_FrameAddr(node)) {
    ...
    if (entity != NULL) attr->fp_offset += get_entity_offset(entity);
    ...
    if (load_store_attr->is_frame_entity) {
      ...
      if (entity != NULL) load_store_attr->offset += get_entity_offset(entity);
