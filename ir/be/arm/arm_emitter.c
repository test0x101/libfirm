...
static void arm_emit_address_mode(ir_node const *const node) {
  ...
  if (offset != 0) be_emit_irprintf(", #%ld", attr->offset);
...
void arm_emitf(const ir_node *node, const char *format, ...) {
  BE_EMITF(node, format, ap, false) {
    switch (*format++) {
      ...
      case 'S': {
        if (!is_digit(*format)) goto unknown;
      ...
      case 'M':
        switch (*format++) {
          case 'L':
            arm_emit_load_mode(node);
            break;
          case 'S':
            arm_emit_store_mode(node);
            break;
          case 'A':
            arm_emit_float_arithmetic_mode(node);
            break;
          case 'F':
            arm_emit_float_load_store_mode(node);
            break;
          default:
            --format;
            goto unknown;
...
static void emit_arm_fConst(const ir_node *irn) {
  ...
  ent_or_tv_t key = {.u.tv = tv, .is_entity = false};
  ent_or_tv_t *entry = get_ent_or_tv_entry(&key);
...
static void emit_arm_Bcc(const ir_node *irn) {
  ...
  if (cmp_attr->ins_permuted) relation = get_inversed_relation(relation);
  ...
  switch (relation & (ir_relation_less_equal_greater)) {
    ...
    case ir_relation_less:
      suffix = is_signed ? "lt" : "lo";
      break;
    case ir_relation_less_equal:
      suffix = is_signed ? "le" : "ls";
      break;
    case ir_relation_greater:
      suffix = is_signed ? "gt" : "hi";
      break;
    case ir_relation_greater_equal:
      suffix = is_signed ? "ge" : "hs";
      break;
...
static void emit_arm_asm_operand(ir_node const *const node, char const modifier,
                                 unsigned const pos) {
  be_asm_attr_t const *const attr = get_be_asm_attr_const(node);
  arm_asm_operand_t const *const op =
      &((arm_asm_operand_t const *)attr->operands)[pos];
  /* modifiers:
  ...
  if (!be_is_valid_asm_operand_kind(node, modifier, pos, op->op.kind, "", "Bc",
                                    "C"))
    return;
  ...
  switch (op->op.kind) {
    ...
    case BE_ASM_OPERAND_IMMEDIATE:
      if (!be_has_modifier("Bc", modifier)) be_emit_char('#');
      if (op->ent) {
        if (modifier == 'B') be_emit_char('~');
        be_gas_emit_entity(op->ent);
        if (op->val != 0) be_emit_irprintf("%+" PRId32, op->val);
      } else {
        int32_t val = op->val;
        if (modifier == 'B') val = ~val;
...
static void emit_arm_B(const ir_node *node) { emit_jmp(node, node); }
...
static void arm_gen_block(ir_node *block) {
  ...
  sched_foreach(block, irn) { be_emit_node(irn); }
}
...
void arm_init_emitter(void) { FIRM_DBG_REGISTER(dbg, "firm.be.arm.emit"); }
