...
/**
 * @file
 * @brief       Peephole optimization framework keeps track of which registers
...
static void clear_defs(ir_node *node) {
  ...
  be_foreach_value(node, value, clear_reg_value(value););
}

static void set_uses(ir_node *node) {
  /* set values used */
  foreach_irn_in(node, i, in) { set_reg_value(in); }
}
...
static void be_peephole_before_exchange(const ir_node *old_node,
  ...
  if (current_node == old_node) {
    ...
    assert(value_strictly_dominates(skip_Proj(new_node),
                                    skip_Proj_const(old_node)));
  }

  if (!mode_is_data(get_irn_mode(old_node))) return;
...
static void process_block(ir_node *block, void *data) {
  ...
  be_lv_foreach(lv, block, be_lv_state_end, node) { set_reg_value(node); }
  DB((dbg, LEVEL_1, "\nstart processing\n"));
  ...
  for (; !sched_is_begin(current_node);
    ...
    if (peephole_node == NULL) continue;
...
bool be_has_only_one_user(ir_node *node) {
  ...
  foreach_out_edge(node, edge) {
    ...
    if (is_Anchor(src) || is_End(src)) continue;
    n_users++;
  }
  return n_users == 1;
}
...
static inline bool overlapping_regs(const arch_register_t *reg0,
  ...
  if (reg0 == NULL || reg1 == NULL) return false;
...
bool be_can_move_down(ir_heights_t *heights, const ir_node *node,
  ...
  while (schedpoint != before) {
    ...
    if (heights_reachable_in_block(heights, schedpoint, node)) return false;
    ...
    foreach_irn_in(node, i, in) {
      ...
      be_foreach_out(schedpoint, o) {
        ...
        if (overlapping_regs(reg, in_req, outreg, outreq)) return false;
      }
    }
    ...
    if (entity != NULL) {
      const ir_entity *const schedpoint_entity = get_frame_entity(schedpoint);
      if (schedpoint_entity == entity) return false;
      if (be_is_MemPerm(schedpoint)) {
        int arity = be_get_MemPerm_entity_arity(schedpoint);
        for (int i = 0; i < arity; ++i) {
          if (be_get_MemPerm_out_entity(schedpoint, i) == entity) return false;
          if (be_get_MemPerm_in_entity(schedpoint, i) == entity) return false;
        }
      }
    }
...
bool be_can_move_up(ir_heights_t *heights, const ir_node *node,
    ...
    foreach_block_succ(after_block, edge) {
      ...
      be_lv_foreach(lv, succ, be_lv_state_in, live_node) {
        ...
        be_foreach_out(node, o) {
          const arch_register_t *outreg = arch_get_irn_register_out(node, o);
          const arch_register_req_t *outreq =
              arch_get_irn_register_req_out(node, o);
          if (overlapping_regs(outreg, outreq, reg, req)) return false;
        }
      }
  ...
  while (schedpoint != after) {
    ...
    if (get_nodes_block(node) != get_nodes_block(schedpoint)) return false;
    /* node must not depend on schedpoint */
    if (heights_reachable_in_block(heights, node, schedpoint)) return false;

    int arity = get_irn_arity(schedpoint);
    be_foreach_out(node, o) {
      const arch_register_t *node_outreg = arch_get_irn_register_out(node, o);
      if (node_outreg == NULL) continue;
      const arch_register_req_t *node_outreq =
          arch_get_irn_register_req_out(node, o);
      ...
      /* schedpoint must not overwrite registers written by node (output
       * dependency) */
      be_foreach_out(schedpoint, o2) {
        const arch_register_t *outreg =
            arch_get_irn_register_out(schedpoint, o2);
        if (outreg == NULL) continue;
        const arch_register_req_t *outreq =
            arch_get_irn_register_req_out(schedpoint, o2);
...
bool be_peephole_IncSP_IncSP(ir_node *node) {
  ...
  if (!be_is_IncSP(pred)) return false;

  if (!be_has_only_one_user(pred)) return false;
