...
static bool default_try_replace(ir_node *consumers, ir_node *flags,
                                ir_node *available) {
  (void)consumers;
  (void)flags;
  (void)available;
...
static bool can_move(ir_node *node, ir_node *after) {
  ...
  foreach_out_edge(node, edge) {
    ...
    if (get_nodes_block(out) != node_block) continue;
    /* phi represents a usage at block end */
    if (is_Phi(out)) continue;
    if (arch_is_irn_not_scheduled(out)) {
      if (!can_move(out, after)) return false;
    } else {
      if (sched_get_time_step(out) <= sched_get_time_step(after)) return false;
    }
  }
...
static void move_other_uses(ir_node *node, ir_node *copy) {
  ...
  foreach_out_edge(node, out) {
    ...
    if (arch_get_irn_register_req(proj) == flags_req) continue;
...
static bool rematerialize_or_move(ir_node *flags_needed, ir_node *node,
  ...
  if (available_flags &&
      try_replace(flag_consumers, flags_needed, available_flags))
    return true;
...
static void fix_flags_walker(ir_node *block, void *env) {
  ...
  sched_foreach_non_phi_reverse(block, node) {
    ...
    ir_node *test = node;
    if (be_is_Keep(test)) test = sched_prev(test);
...
void be_sched_fix_flags(ir_graph *irg, const arch_register_class_t *flag_cls,
  ...
  changed = false;
  if (remat == NULL) remat = &default_remat;
  if (check_modify == NULL) check_modify = &default_check_modifies;
  if (try_replace == NULL) try_replace = &default_try_replace;
