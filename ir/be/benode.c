...
static int be_incsp_attrs_equal(const ir_node *a, const ir_node *b) {
  ...
  return attr_a->offset == attr_b->offset &&
         attr_a->no_align == attr_b->no_align && attrs_equal_be_node(a, b);
}
...
arch_register_req_t const **be_allocate_in_reqs(ir_graph *const irg,
                                                unsigned const n) {
 ...
 */
static void init_node_attr(ir_node *const node, unsigned const n_outputs,
...
ir_node *be_new_Perm(ir_node *const block, int const n,
...
ir_node *be_new_MemPerm(ir_node *const block, int n, ir_node *const *const in) {
  ir_graph *const irg = get_irn_irg(block);
  ir_node *const irn =
      new_ir_node(NULL, irg, block, op_be_MemPerm, mode_T, n, in);
...
static void set_copy_info(ir_node *const irn, ir_graph *const irg,
                          ir_node *const op, arch_irn_flags_t const flags) {
  ...
  arch_register_req_t *const out_req =
      be_create_cls_req(irg, cls, op_req->width);
  out_req->should_be_same = 1U << 0;
  arch_set_irn_register_req_out(irn, 0, out_req);
}

ir_node *be_new_d_Copy(dbg_info *const dbgi, ir_node *const block,
                       ir_node *const op) {
  ir_graph *const irg = get_irn_irg(block);
  ir_node *const in[] = {op};
  ir_node *const res = new_ir_node(dbgi, irg, block, op_be_Copy,
                                   get_irn_mode(op), ARRAY_SIZE(in), in);
...
ir_node *be_new_Copy_before_reg(ir_node *const val, ir_node *const before,
...
ir_node *be_new_IncSP(ir_node *bl, ir_node *old_sp, int offset, bool no_align) {
  arch_register_class_t const *const cls =
      arch_get_irn_register_req(old_sp)->cls;
  ir_graph *irg = get_irn_irg(bl);
  ir_node *in[] = {old_sp};
  ir_node *irn =
      new_ir_node(NULL, irg, bl, op_be_IncSP, cls->mode, ARRAY_SIZE(in), in);
...
ir_node *be_new_CopyKeep(ir_node *const bl, ir_node *const src, int const n,
                         ir_node *const *const in_keep) {
...
ir_node *be_new_Phi(ir_node *block, int n_ins, ir_node **ins,
  ...
  ir_graph *irg = get_irn_irg(block);
  ir_node *phi =
      new_ir_node(NULL, irg, block, op_Phi, req->cls->mode, n_ins, ins);
...
ir_node *be_new_Phi0(ir_node *const block,
                     arch_register_req_t const *const req) {
  ir_graph *const irg = get_irn_irg(block);
  ir_node *const phi =
      new_ir_node(NULL, irg, block, op_Phi, req->cls->mode, 0, NULL);
  struct obstack *const obst = be_get_be_obst(irg);
...
ir_node *be_complete_Phi(ir_node *const phi, unsigned const n_ins,
                         ir_node **const ins) {
...
ir_node *be_new_Asm(dbg_info *const dbgi, ir_node *const block, int const n_ins,
                    ir_node **const ins,
                    arch_register_req_t const **const in_reqs, int const n_outs,
                    ident *const text, void *const operands) {
  ir_graph *const irg = get_irn_irg(block);
  ir_node *const asmn =
      new_ir_node(dbgi, irg, block, op_be_Asm, mode_T, n_ins, ins);
...
ir_node *be_new_Relocation(dbg_info *const dbgi, ir_graph *const irg,
                           unsigned const kind, ir_entity *const entity,
                           ir_mode *const mode) {
  ir_node *const block = get_irg_start_block(irg);
  ir_node *const node =
      new_ir_node(dbgi, irg, block, op_be_Relocation, mode, 0, NULL);
...
ir_node *be_new_Start(ir_graph *const irg, be_start_out const *const outs) {
  ...
  for (unsigned i = 0; i != n_regs; ++i) {
    if (outs[i] != BE_START_NO) {
      ...
      arch_register_req_t const *const req =
          be_create_reg_req(irg, reg, ignore);
...
ir_node *be_get_Start_proj(ir_graph *const irg,
                           arch_register_t const *const reg) {
  ir_node *const start = get_irg_start(irg);
  /* do a naive linear search... */
  be_foreach_out(start, i) {
    arch_register_t const *const out_reg = arch_get_irn_register_out(start, i);
    if (out_reg == reg) return be_get_or_make_Proj_for_pn(start, i);
  }
  panic("tried querying undefined register '%s' at Start", reg->name);
}

ir_node *be_new_Unknown(ir_node *const block,
                        arch_register_req_t const *const req) {
  ir_graph *const irg = get_irn_irg(block);
  ir_mode *const mode = req->cls->mode;
  ir_node *const node =
      new_ir_node(NULL, irg, block, op_be_Unknown, mode, 0, NULL);
...
ir_node *be_new_Proj(ir_node *const pred, unsigned const pos) {
  arch_register_req_t const *const req =
      arch_get_irn_register_req_out(pred, pos);
  return new_r_Proj(pred, req->cls->mode, pos);
}

ir_node *be_new_Proj_reg(ir_node *const pred, unsigned const pos,
                         arch_register_t const *const reg) {
    ...
    case dump_node_info_txt:
      if (be_is_Asm(irn)) {
        ...
        for (unsigned i = 0; i < be_get_MemPerm_entity_arity(irn); ++i) {
          ...
          if (in != NULL) ir_fprintf(f, "\nin[%u]: %F\n", i, in);
          if (out != NULL) ir_fprintf(f, "\nout[%u]: %F\n", i, out);
        }
      }
      break;
  }
...
void be_copy_attr(ir_graph *const irg, ir_node const *const old_node,
...
void be_switch_attr_init(ir_node *const node, be_switch_attr_t *const attr,
...
void be_init_op(void) {
  ...
  op_be_Asm = new_be_op(o + beo_Asm, "be_Asm", op_pin_state_exc_pinned,
                        irop_flag_fragile | irop_flag_uses_memory,
                        sizeof(be_asm_attr_t));
  op_be_Copy = new_be_op(o + beo_Copy, "be_Copy", op_pin_state_floats,
                         irop_flag_none, 0);
  op_be_CopyKeep = new_be_op(o + beo_CopyKeep, "be_CopyKeep",
                             op_pin_state_floats, irop_flag_keep, 0);
  op_be_IncSP = new_be_op(o + beo_IncSP, "be_IncSP", op_pin_state_pinned,
                          irop_flag_none, sizeof(be_incsp_attr_t));
  op_be_Keep = new_be_op(o + beo_Keep, "be_Keep", op_pin_state_pinned,
                         irop_flag_keep, 0);
  op_be_MemPerm = new_be_op(o + beo_MemPerm, "be_MemPerm", op_pin_state_pinned,
                            irop_flag_none, sizeof(be_memperm_attr_t));
  op_be_Perm = new_be_op(o + beo_Perm, "be_Perm", op_pin_state_pinned,
                         irop_flag_none, 0);
  op_be_Relocation =
      new_be_op(o + beo_Relocation, "be_Relocation", op_pin_state_floats,
                irop_flag_constlike | irop_flag_start_block,
                sizeof(be_relocation_attr_t));
  op_be_Start = new_be_op(o + beo_Start, "be_Start", op_pin_state_pinned,
                          irop_flag_start_block, 0);
  op_be_Unknown = new_be_op(o + beo_Unknown, "be_Unknown", op_pin_state_floats,
                            irop_flag_constlike, 0);
...
void be_finish_op(void) {
  free_ir_op(op_be_Copy);
  op_be_Copy = NULL;
  free_ir_op(op_be_CopyKeep);
  op_be_CopyKeep = NULL;
  free_ir_op(op_be_IncSP);
  op_be_IncSP = NULL;
  free_ir_op(op_be_Keep);
  op_be_Keep = NULL;
  free_ir_op(op_be_MemPerm);
  op_be_MemPerm = NULL;
  free_ir_op(op_be_Perm);
  op_be_Perm = NULL;
}
