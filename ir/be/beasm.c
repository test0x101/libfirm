...
arch_register_req_t const *be_make_register_req(
    struct obstack *obst, be_asm_constraint_t const *const c) {
  assert(c->same_as < 0);
  assert(c->cls);

  if (c->all_registers_allowed) return c->cls->class_req;

  if (c->allowed_registers == 0)
    panic("constraint does not allow any registers");

  arch_register_req_t *const req = (arch_register_req_t *)obstack_alloc(
      obst, sizeof(*req) + sizeof(unsigned));
...
void be_parse_asm_constraints_internal(
    be_asm_constraint_t *const constraint, ident *const constraint_text,
    parse_constraint_letter_func_t *const parse_constraint_letter,
    void const *const env) {
      ...
      default:
        if (is_digit(*i)) {
          ...
          if (sscanf(i, "%d%n", &same_as, &p) == 0) {
            be_errorf(NULL, "error while reading matching constraint in \"%s\"",
                      constraint_text);
            i += 1;
          } else {
            i += p;
          ...
          if (new_cls) {
            if (cls && cls != new_cls)
              be_warningf(NULL,
                          "multiple register classes not supported in \"%s\"",
                          constraint_text);
            cls = new_cls;
          }
          ...
          if (new_imm != '\0') {
            if (immediate_type != '\0' && immediate_type != new_imm)
              be_warningf(NULL,
                          "multiple immediate types not supported in \"%s\"",
                          constraint_text);
  ...
  if (same_as >= 0) {
    if (is_output) {
      be_warningf(
          NULL, "output constraint \"%s\" has matching constraint; ignoring it",
          constraint_text);
      same_as = -1;
    } else {
      if (cls) {
        be_warningf(NULL,
                    "matching and register constraint not supported in "
                    "constraint \"%s\"; ignoring register constraint",
                    constraint_text);
        cls = NULL;
      }
      if (immediate_type != '\0') {
        be_warningf(NULL,
                    "matching and immediate constraint not supported in "
                    "constraint \"%s\"; ignoring immediate constraint",
                    constraint_text);
        immediate_type = '\0';
      }
    }
...
be_asm_info_t be_asm_prepare_info(ir_node const *const node) {
  ...
  arch_register_req_t const **const out_reqs =
      NEW_ARR_F(arch_register_req_t const *, n_outs);
  ...
  arch_register_req_t const **const in_reqs =
      NEW_ARR_F(arch_register_req_t const *, n_be_Asm_first_in);
...
static bool can_match(arch_register_req_t const *const in,
                      arch_register_req_t const *const out) {
  if (in->cls != out->cls) return false;
  if (in->limited == NULL || out->limited == NULL) return true;
...
static bool match_requirement(arch_register_req_t const **reqs,
  ...
  for (size_t i = 0; i != n_reqs; ++i) {
    if (bitset_is_set(used, i)) continue;
    if (!can_match(req, reqs[i])) continue;
    bitset_set(used, i);
    return true;
  }
  return false;
...
ir_node *be_make_asm(ir_node const *const node, be_asm_info_t const *const info,
                     void *const operands) {
  ir_node **in = info->ins;
    ...
    if (!constraint->constraint) {
      ...
      if (out_pos >= 0) {
        ...
        for (unsigned i = 0; i != orig_n_ins; ++i) {
          if (in_reqs[i]->cls == cls) different |= 1U << i;
        }
  ...
  if (n_clobbers != 0) {
    ...
    for (size_t i = n_be_Asm_first_in, n = ARR_LEN(in_reqs); i != n; ++i) {
      ...
      if (clobber != 0) {
        arch_register_req_t *const new_req =
            (arch_register_req_t *)obstack_alloc(
                obst, sizeof(*new_req) + sizeof(unsigned));
  ...
  unsigned const n_outs =
      ARR_LEN(out_reqs);  // Clobbers might add further outputs.
  ...
  arch_register_req_t const **const dup_in_reqs =
      DUP_ARR_D(arch_register_req_t const *, obst, in_reqs);
  ir_node *const new_node =
      be_new_Asm(dbgi, block, n_ins, in, dup_in_reqs, n_outs, text, operands);
...
ir_node *be_emit_asm(ir_node const *const asmn,
                     be_emit_asm_operand_func *const emit_asm_operand) {
  be_emit_cstring("#APP");
...
bool be_is_valid_asm_operand_kind(ir_node const *const node,
                                  char const modifier, unsigned const pos,
                                  be_asm_operand_kind_t const have,
                                  char const *const mod_any,
                                  char const *const mod_imm,
                                  char const *const mod_mem) {
  ...
  if (want != have) {
    ...
    be_errorf(node,
              "modifier of operand '%%%c%u' requires an operand of type '%s', "
              "but got '%s'",
              modifier, pos, name_want, name_have);
    return false;
  }
  return true;
}
...
arch_register_t const *be_parse_register_name(char const *clobber) {
  ...
  if (c == '#' || c == '%' ||
      (isa->register_prefix != '\0' && c == isa->register_prefix))
    clobber += 1;
  ...
  if (add) {
    for (be_register_name_t const *i = add; i->name; ++i) {
      if (streq(i->name, clobber)) return &isa->registers[i->index];
    }
  }
