...
static const char *get_register_name_mode(const arch_register_t *reg,
  ...
  switch (size) {
    case X86_SIZE_8:
      return get_register_name_8bit(reg);
    case X86_SIZE_16:
      return get_register_name_16bit(reg);
    case X86_SIZE_32:
      return get_register_name_32bit(reg);
    case X86_SIZE_64:
    case X86_SIZE_80:
    case X86_SIZE_128:
      return reg->name;
...
static void amd64_emit_immediate64(const amd64_imm64_t *const imm) {
  ...
  if (imm->offset != 0) be_emit_irprintf("%+" PRId64, imm->offset);
}
...
static void amd64_emit_am(const ir_node *const node, bool indirect_star) {
  ...
  switch ((amd64_op_mode_t)attr->base.op_mode) {
    ...
    case AMD64_OP_X87_ADDR_REG:
      if (indirect_star) be_emit_char('*');
      goto emit_addr;
    case AMD64_OP_ADDR_REG: {
      amd64_binop_addr_attr_t const *const binop_attr =
  ...
  end_of_mods:
    ...
    switch (*fmt++) {
      ...
      case 'A':
        switch (*fmt++) {
          case 'F': {
            x87_attr_t const *const attr = amd64_get_x87_attr_const(node);
      ...
      case 'F': {
        if (*fmt == 'M') {
          ...
          if (attr->reverse) be_emit_char('r');
        } else
          goto unknown;
        break;
      }
      ...
      emit_R:
        if (mod & EMIT_IGNORE_MODE) {
          ...
          x86_insn_size_t dest_size =
              src_size == X86_SIZE_64 ? X86_SIZE_64 : X86_SIZE_32;
...
static const char *get_register_name_ir_mode(const arch_register_t *reg,
                                             ir_mode *mode) {
  if (get_mode_arithmetic(mode) != irma_twos_complement) return reg->name;
...
static void emit_amd64_asm_register(const arch_register_t *reg, char modifier,
  ...
  switch (modifier) {
    case 'b':
      name = get_register_name_8bit(reg);
      break;
    case 'h':
    ...
    case 'w':
      ...
      break;
    case 'k':
      name = get_register_name_32bit(reg);
      break;
    case 'q':
      name = reg->name;
      break;
    ...
    default:
      name = mode ? get_register_name_ir_mode(reg, mode) : reg->name;
      break;
  }
  be_emit_char('%');
  be_emit_string(name);
...
static void emit_amd64_asm_operand(ir_node const *const node,
   ...
   *   P: like 'p' and rip-relative operands without (%rip) (but gcc doc says
   * "if PIC, print an @PLT suffix") X: no modifying effect (gcc doc: don't
   * print any sort of PIC '@' suffix for a symbol) b: 8 bit low name of
   * register c: immediate without prefix '$' l: label without prefix '$' h: 8
   * bit high name of register k: 32 bit name of register p: like 'c' and other
   * operands unmodified (gcc doc: "print raw symbol") q: 64 bit name of
   * register w: 16 bit name of register */
 ...
 * Emit a Jmp.
 */
static void emit_amd64_jmp(const ir_node *node) { emit_jmp(node, node); }
...
static void emit_amd64_jmp_switch(const ir_node *node) {
  ...
  ir_mode *entry_mode =
      ir_platform.pic_style != BE_PIC_NONE ? mode_Iu : mode_Lu;
...
static x86_condition_code_t determine_final_cc(ir_node const *const flags,
  ...
  if (is_amd64_fucomi(flags)) {
    ...
    if (attr->x87.reverse) cc = x86_invert_condition_code(cc);
  }
  return cc;
}
...
static void emit_amd64_mov_gp(const ir_node *node) {
  ...
  switch (attr->size) {
    case X86_SIZE_8:
      amd64_emitf(node, "movzbl %AM, %3D0");
      return;
    case X86_SIZE_16:
      amd64_emitf(node, "movzwl %AM, %3D0");
      return;
    case X86_SIZE_32:
      amd64_emitf(node, "movl %AM, %3D0");
      return;
    case X86_SIZE_64:
      amd64_emitf(node, "movq %AM, %^D0");
      return;
    case X86_SIZE_80:
    case X86_SIZE_128:
      break;
...
static void emit_copyB_prolog(unsigned size) {
  if (size & 1) amd64_emitf(NULL, "movsb");
  if (size & 2) amd64_emitf(NULL, "movsw");
  if (size & 4) amd64_emitf(NULL, "movsd");
}
