...
ir_type *amd64_build_va_list_type(void) {
  ...
  ir_type *const va_list_type =
      new_type_struct(new_id_from_str("builtin:va_list"));

  va_list_members.gp_offset =
      new_entity(va_list_type, new_id_from_str("gp_offset"), int_type);
  va_list_members.xmm_offset =
      new_entity(va_list_type, new_id_from_str("xmm_offset"), int_type);
  va_list_members.stack_args_ptr =
      new_entity(va_list_type, new_id_from_str("stack_args_ptr"), ptr_type);
  va_list_members.reg_save_ptr =
      new_entity(va_list_type, new_id_from_str("reg_save_ptr"), ptr_type);

  default_layout_compound_type(va_list_type);
  return va_list_type;
}

void amd64_collect_variadic_params(be_start_out *const outs,
  ...
  for (size_t i = gp_params + xmm_params, n = cconv->n_param_regs; i < n;
       i++, p++) {
    const arch_register_t *reg = cconv->parameters[p].reg;
...
void amd64_insert_reg_save_area(ir_graph *irg, x86_cconv_t *cconv) {
  ...
  for (size_t i = 0; i < max_gp_params; i++) {
    ...
    gp_save_slots[i] =
        new_entity(reg_save_type, id, get_type_for_mode(mode_Lu));
  }
  ...
  for (size_t i = 0; i < max_xmm_params; i++) {
    ...
    xmm_save_slots[i] =
        new_entity(reg_save_type, id, get_type_for_mode(amd64_mode_xmm));
  }
...
static ir_node *load_result(dbg_info *dbgi, ir_node *block, ir_node *ptr,
                            ir_type *type, ir_node **mem) {
...
static void make_store(dbg_info *dbgi, ir_node *block, ir_node *ptr,
                       ir_node *value, ir_type *type, ir_node **mem) {
...
static ir_node *get_va_stack_address(dbg_info *dbgi, ir_node *block,
                                     ir_type *restype, ir_node *ap,
                                     ir_node **mem) {
  ir_graph *irg = get_irn_irg(block);

  // Load stack_args_ptr
  ir_node *stack_args_ptr =
      new_rd_Member(dbgi, block, ap, va_list_members.stack_args_ptr);
  ir_type *stack_args_type = get_entity_type(va_list_members.stack_args_ptr);
  ir_node *stack_args =
      load_result(dbgi, block, stack_args_ptr, stack_args_type, mem);
...
static ir_node *load_va_from_stack(dbg_info *dbgi, ir_node *block,
                                   ir_type *restype, ir_node *ap,
                                   ir_node **mem) {
...
static ir_node *load_va_from_register_or_stack(dbg_info *dbgi, ir_node *block,
                                               ir_mode *resmode,
                                               ir_type *restype, ir_node *max,
                                               ir_entity *offset_entity,
                                               ir_node *stride, ir_node *ap,
                                               ir_node **mem) {
  ir_graph *irg = get_irn_irg(block);
  ...
  ir_node *reg_save_ptr =
      new_rd_Member(dbgi, true_block, ap, va_list_members.reg_save_ptr);
  ir_type *reg_save_type = get_entity_type(va_list_members.reg_save_ptr);
  ir_node *reg_save =
      load_result(dbgi, true_block, reg_save_ptr, reg_save_type, &true_mem);
  ...
  ir_node *true_result_ptr =
      new_rd_Add(dbgi, true_block, reg_save, conv_offset);
  ir_node *true_result =
      load_result(dbgi, true_block, true_result_ptr, restype, &true_mem);
  ...
  ir_node *false_result =
      load_va_from_stack(dbgi, false_block, restype, ap, &false_mem);

  // Phi both sides together
  ir_node *phiM_in[] = {true_mem, false_mem};
  ir_node *phiM =
      new_rd_Phi(dbgi, lower_block, ARRAY_SIZE(phiM_in), phiM_in, mode_M);
  ir_node *phi_in[] = {true_result, false_result};
  ir_node *phi =
      new_rd_Phi(dbgi, lower_block, ARRAY_SIZE(phi_in), phi_in, resmode);

  *mem = phiM;
  return phi;
}

static ir_node *load_aggregate_va(dbg_info *dbgi, ir_node *block,
                                  ir_type *restype, ir_node *ap,
                                  ir_node **mem) {
  ...
  ir_node *gp_offset_ptr =
      new_rd_Member(dbgi, block, ap, va_list_members.gp_offset);
  ir_node *gp_offset =
      load_result(dbgi, block, gp_offset_ptr, gp_offset_type, mem);
  ir_node *gp_max =
      new_rd_Const_long(dbgi, irg, gp_offset_mode, n_gp_args * gp_size);
  ir_node *gp_check =
      new_rd_Cmp(dbgi, block, gp_offset, gp_max, ir_relation_less);
  ...
  ir_node *xmm_check_block =
      new_r_Block(irg, ARRAY_SIZE(in_xmm_check), in_xmm_check);
  ...
  ir_node *xmm_offset_ptr =
      new_rd_Member(dbgi, xmm_check_block, ap, va_list_members.xmm_offset);
  ir_node *xmm_offset =
      load_result(dbgi, xmm_check_block, xmm_offset_ptr, xmm_offset_type, mem);
  ir_node *xmm_max = new_rd_Const_long(
      dbgi, irg, xmm_offset_mode, n_gp_args * gp_size + n_xmm_args * xmm_size);
  ir_node *xmm_check =
      new_rd_Cmp(dbgi, xmm_check_block, xmm_offset, xmm_max, ir_relation_less);
  ...
  ir_node *ent_addr =
      new_rd_Member(dbgi, true_block, get_irg_frame(irg), entity);
  ...
  for (unsigned i = 0; i < spec.length; i++) {
    ...
    ir_node *offset =
        load_result(dbgi, true_block, offset_ptr, offset_type, &true_mem);
    ...
    ir_node *true_result_ptr =
        new_rd_Add(dbgi, true_block, reg_save, conv_offset);
    ir_node *reg =
        load_result(dbgi, true_block, true_result_ptr, regtype, &true_mem);

    // Store to entity
    // Use gp_size here always, because for structs we only use 8 bytes of an
    // XMM register.
    ir_node *ent_offset =
        new_rd_Const_long(dbgi, irg, offset_mode, gp_size * i);
    ...
    ir_node *offset_inc = new_rd_Add(dbgi, true_block, offset, stride);
    make_store(dbgi, true_block, offset_ptr, offset_inc, offset_type,
               &true_mem);
  }

  // False side: Load from the stack
  ir_node *false_phiM_in[] = {gp_check_mem, xmm_check_mem};
  ir_node *false_mem = new_rd_Phi(dbgi, false_block, ARRAY_SIZE(false_phiM_in),
                                  false_phiM_in, mode_M);
  ir_node *false_result =
      get_va_stack_address(dbgi, false_block, restype, ap, &false_mem);
  ...
  ir_node *phi =
      new_rd_Phi(dbgi, lower_block, ARRAY_SIZE(phi_in), phi_in, mode_P);
...
void amd64_lower_va_arg(ir_node *const node) {
  ...
  if (is_aggregate_type(restype)) {
    ...
    result = load_va_from_register_or_stack(dbgi, block, resmode, restype, max,
                                            offset_entity, stride, ap, &mem);
  }
 ...
 */
static ir_node *make_mov_imm32_to_offset_mem(dbg_info *dbgi, ir_node *block,
                                             ir_node *mem, ir_node *base,
                                             ir_entity *offset_ent,
                                             long value) {
  ...
  return new_bd_amd64_mov_store(dbgi, block, ARRAY_SIZE(mov_in), mov_in,
                                gp_am_reqs[1], &mov_attr);
}
 ...
 */
static ir_node *make_mov_val64_to_offset_mem(dbg_info *dbgi, ir_node *block,
                                             ir_node *mem, ir_node *base,
                                             ir_entity *entity,
                                             ir_entity *offset_ent,
                                             ir_node *value) {
 ...
 */
static ir_node *make_mov_xmmval64_to_offset_mem(dbg_info *dbgi, ir_node *block,
                                                ir_node *mem, ir_node *base,
                                                ir_entity *entity,
                                                ir_entity *offset_ent,
                                                ir_node *value) {
  ...
  return new_bd_amd64_movs_store_xmm(dbgi, block, ARRAY_SIZE(mov_in), mov_in,
                                     xmm_reg_mem_reqs, &mov_attr);
}
...
ir_node *amd64_initialize_va_list(dbg_info *dbgi, ir_node *block,
  ...
  mem = make_mov_imm32_to_offset_mem(
      dbgi, block, mem, ap, va_list_members.gp_offset, initial_gp_offset);
  ...
  mem = make_mov_imm32_to_offset_mem(
      dbgi, block, mem, ap, va_list_members.xmm_offset, initial_xmm_offset);

  ir_node *const reg_save_ptr =
      make_lea_with_offset_entity(dbgi, block, frame, reg_save_area);
  mem = make_mov_val64_to_offset_mem(
      dbgi, block, mem, ap, NULL, va_list_members.reg_save_ptr, reg_save_ptr);

  ir_node *const stack_args =
      make_lea_with_offset_entity(dbgi, block, frame, stack_args_param);
  mem = make_mov_val64_to_offset_mem(
      dbgi, block, mem, ap, NULL, va_list_members.stack_args_ptr, stack_args);

  return mem;
}

void amd64_save_vararg_registers(ir_graph *const irg,
                                 x86_cconv_t const *const cconv,
                                 ir_node *const frame) {
  ...
  for (size_t p = cconv->n_parameters; reg_params != max_reg_params;
       ++p, ++reg_params) {
    ...
    if (reg->cls == &amd64_reg_classes[CLASS_amd64_gp]) {
      ...
      mem = make_mov_xmmval64_to_offset_mem(
          NULL, block, mem, frame, reg_save_area, xmm_save_slots[xmm_params++],
          reg_value);
    } else {
      panic("unexpected register class");
    }
    if (!first_mov) first_mov = mem;
  }
