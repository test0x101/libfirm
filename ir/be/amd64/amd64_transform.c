...
const x86_asm_constraint_list_t amd64_asm_constraints = {
    ...
    ['O'] = {MATCH_IMM, GP, 0},
    ['R'] = {MATCH_REG, GP,
             1 << REG_GP_RAX | 1 << REG_GP_RBX | 1 << REG_GP_RCX |
                 1 << REG_GP_RDX | 1 << REG_GP_RSI | 1 << REG_GP_RDI |
                 1 << REG_GP_RBP | 1 << REG_GP_RSP},
...
static const arch_register_req_t *xmm_xmm_reg_reg_mem_reqs[] = {
    &amd64_requirement_xmm_killed, &amd64_class_reg_req_xmm,
...
static const arch_register_req_t *reg_reg_rax_reg_mem_reqs[] = {
    &amd64_class_reg_req_gp,      &amd64_class_reg_req_gp,
    &amd64_single_reg_req_gp_rax, &amd64_class_reg_req_gp,
    &arch_memory_requirement,
};
...
ir_entity *create_float_const_entity(ir_tarval *const tv) {
  ...
  entity = new_global_entity(glob, id_unique("C"), type, ir_visibility_private,
...
void init_lconst_addr(x86_addr_t *addr, ir_entity *entity) {
  ...
  x86_immediate_kind_t kind =
      ir_platform.pic_style != BE_PIC_NONE ? X86_IMM_PCREL : X86_IMM_ADDR;
  *addr = (x86_addr_t){
      .immediate =
          {
              .entity = entity,
...
static ir_node *create_float_const(dbg_info *dbgi, ir_node *block,
  ...
  if (size == X86_SIZE_128) {
    load = new_bd_amd64_movdqa(dbgi, block, ARRAY_SIZE(in), in, mem_reqs,
                               AMD64_OP_ADDR, addr);
    pn_res = pn_amd64_movdqa_res;
  } else {
    load = new_bd_amd64_movs_xmm(dbgi, block, ARRAY_SIZE(in), in, mem_reqs,
...
static ir_node *gen_x87_Const(dbg_info *const dbgi, ir_node *const block,
                              ir_tarval *tv) {
  ...
  if (tarval_is_null(tv)) {
    ...
    ir_node *load = new_bd_amd64_fld(dbgi, block, ARRAY_SIZE(in), in, mem_reqs,
                                     size, AMD64_OP_ADDR, addr);
    set_irn_pinned(load, false);
...
static ir_node *make_const(dbg_info *const dbgi, ir_node *const block,
                           uint64_t const val) {
...
typedef ir_node *(*construct_binop_func)(
    dbg_info *dbgi, ir_node *block, int arity, ir_node *const *in,
    arch_register_req_t const **in_reqs,
    amd64_binop_addr_attr_t const *attr_init);

typedef ir_node *(*construct_rax_binop_func)(
    dbg_info *dbgi, ir_node *block, int arity, ir_node *const *in,
    arch_register_req_t const **in_reqs, x86_insn_size_t size,
...
static bool match_immediate_32(x86_imm32_t *const imm, ir_node const *const op,
                               bool const must_match_ip_relative) {
  assert(mode_needs_gp_reg(get_irn_mode(op)));
  assert(imm->offset == 0 && imm->entity == NULL && imm->kind == X86_IMM_VALUE);

  ir_tarval *tv;
  ir_entity *entity;
  unsigned reloc_kind;
  if (!be_match_immediate(op, &tv, &entity, &reloc_kind)) return false;

  int32_t val;
  if (tv) {
    if (get_tarval_magnitude(tv) > 32) return false;
    val = get_tarval_uint64(tv);
  } else {
    val = 0;
  }

  if ((entity != NULL) != must_match_ip_relative) return false;
...
static ir_node *source_am_possible(ir_node *block, ir_node *node) {
  ...
  if (get_irn_n_edges(node) != 1) return NULL;
  ...
  if (be_is_transformed(node)) return NULL;
  return load;
}

static bool needs_extension(ir_node *op) {
  ir_mode *mode = get_irn_mode(op);
  if (get_mode_size_bits(mode) >= 32) return false;
...
static bool use_address_matching(ir_mode *mode, match_flags_t flags,
  ...
  if (!(flags & match_am)) return false;

  unsigned mode_bits = get_mode_size_bits(mode);
  if (mode_bits == 8 || mode_bits == 16) return false;
...
static void perform_address_matching_flags(ir_node *const ptr, int *const arity,
                                           ir_node **const in,
...
static void perform_address_matching(ir_node *const ptr, int *const arity,
                                     ir_node **const in,
                                     x86_addr_t *const addr) {
  perform_address_matching_flags(ptr, arity, in, addr, x86_create_am_normal);
}

static void match_binop(amd64_args_t *args, ir_node *block, ir_mode *mode,
...
static ir_node *gen_binop_am(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  ir_node *const new_node =
      func(dbgi, new_block, args.arity, args.in, args.reqs, &args.attr);
...
static ir_node *gen_binop_rax(ir_node *node, ir_node *op0, ir_node *op1,
  ...
  ir_node *load;
  ir_node *op;
  bool use_am = use_address_matching(mode, flags, block, op0, op1, &load, &op);
  ...
  ir_node *const new_block = be_transform_node(block);
  ir_node *const new_node =
      make_node(dbgi, new_block, arity, in, reqs, size, op_mode, addr);
  if (mem_proj != NULL) {
...
static ir_node *gen_binop_xmm(ir_node *node, ir_node *op0, ir_node *op1,
  ...
  ir_node *const new_block = be_transform_node(block);
  ir_node *const new_node =
      make_node(dbgi, new_block, args.arity, args.in, args.reqs, &args.attr);
...
typedef ir_node *(*construct_x87_binop_func)(dbg_info *dbgi, ir_node *block,
                                             ir_node *op0, ir_node *op1);

static ir_node *gen_binop_x87(ir_node *const node, ir_node *const op0,
                              ir_node *const op1,
                              construct_x87_binop_func cons) {
  dbg_info *const dbgi = get_irn_dbg_info(node);
...
typedef ir_node *(*construct_shift_func)(dbg_info *dbgi, ir_node *block,
                                         ...
                                         arch_register_req_t const **in_reqs,
                                         amd64_shift_attr_t const *attr_init);
...
static ir_node *gen_shift_binop(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  while (is_Conv(op2) && get_irn_n_edges(op2) == 1) {
    ...
    if (get_mode_arithmetic(get_irn_mode(op)) != irma_twos_complement) break;
    op2 = op;
  }
...
static ir_node *gen_fma(ir_node *const add, ir_node *const op1,
                        ir_node *const op2) {
  if (!amd64_cg_config.use_scalar_fma3) return NULL;
  ...
  if (get_irn_mode(mul) != add_mode) return NULL;
  if (get_irn_n_edges(mul) != 1) return NULL;
  ...
  bool use_am;
  ir_node *(*fma_variant)(dbg_info *, ir_node *, const int, ir_node *const *,
                          const arch_register_req_t **, x86_insn_size_t,
                          amd64_op_mode_t, x86_addr_t);
  // try if Add operand, left Mul operand or right Mul operand can be used as AM
  // input
  if ((use_am = use_address_matching(add_mode, match_am, block, mul_op1, add_op,
                                     &load, &reg_op) &&
                (!input_depends_on_load(load, mul_op2)))) {
    source1 = mul_op2;
    source2 = reg_op;
    fma_variant = &new_bd_amd64_vfmadd213s;
  } else if ((use_am = use_address_matching(add_mode, match_am, block, add_op,
                                            mul_op1, &load, &reg_op) &&
                       (!input_depends_on_load(load, mul_op2)))) {
    source1 = mul_op2;
    source2 = reg_op;
  ...
  ir_node *const new_block = be_transform_node(block);
  ir_node *const new_node =
      fma_variant(dbgi, new_block, arity, in, reqs, size, op_mode, addr);
...
static ir_node *gen_Add(ir_node *const node) {
  ...
  if (mode_is_float(mode)) {
    ...
    if (fma) return fma;
    return gen_binop_am(node, op1, op2, new_bd_amd64_adds, pn_amd64_adds_res,
                        match_commutative | match_am);
  }

  match_flags_t flags =
      match_immediate | match_am | match_mode_neutral | match_commutative;
  ir_node *load;
  ir_node *op;
  bool use_am = use_address_matching(mode, flags, block, op1, op2, &load, &op);
  ...
  if (use_am)
    res =
        gen_binop_am(node, op1, op2, new_bd_amd64_add, pn_amd64_add_res, flags);
  else {
    int arity = 0;
    ir_node *in[2];
    x86_addr_t addr;
    perform_address_matching_flags(node, &arity, in, &addr,
                                   x86_create_am_force);

    dbg_info *const dbgi = get_irn_dbg_info(node);
    ir_node *const new_block = be_transform_node(block);
    x86_insn_size_t const size = get_size_32_64_from_mode(mode);
    res = new_bd_amd64_lea(dbgi, new_block, arity, in, amd64_reg_reg_reqs, size,
                           addr);
  }
...
static ir_node *gen_Sub(ir_node *const node) {
  ...
  if (mode_is_float(mode)) {
    ...
    return gen_binop_am(node, op1, op2, new_bd_amd64_subs, pn_amd64_subs_res,
                        match_am);
  } else {
...
static ir_node *gen_And(ir_node *const node) {
  ...
  if (is_Const(op2)) {
    ...
    movzx:;
      dbg_info *const dbgi = get_irn_dbg_info(node);
      ir_node *const block = get_nodes_block(node);
      return match_mov(dbgi, block, op1, size, &new_bd_amd64_mov_gp,
                       pn_amd64_mov_gp_res);
    }
  }

  return gen_binop_am(
      node, op1, op2, new_bd_amd64_and, pn_amd64_and_res,
      match_immediate | match_am | match_mode_neutral | match_commutative);
}
...
static ir_node *gen_Eor(ir_node *const node) {
  ...
  return gen_binop_am(
      node, op1, op2, new_bd_amd64_xor, pn_amd64_xor_res,
      match_immediate | match_am | match_mode_neutral | match_commutative);
}
...
static ir_node *gen_Or(ir_node *const node) {
  ...
  return gen_binop_am(
      node, op1, op2, new_bd_amd64_or, pn_amd64_or_res,
      match_immediate | match_am | match_mode_neutral | match_commutative);
}
...
static ir_node *gen_Mul(ir_node *const node) {
  ...
  if (get_mode_size_bits(mode) < 16) {
    ...
    ir_node *new_node = gen_binop_rax(node, op1, op2, new_bd_amd64_imul_1op,
    ...
    return gen_binop_am(node, op1, op2, new_bd_amd64_muls, pn_amd64_muls_res,
                        match_commutative | match_am);
  } else {
    return gen_binop_am(
        node, op1, op2, new_bd_amd64_imul, pn_amd64_imul_res,
        match_immediate | match_am | match_mode_neutral | match_commutative);
  }
}
...
static ir_node *create_div(ir_node *const node, ir_mode *const mode,
  ...
  return constructor(dbgi, new_block, ARRAY_SIZE(in), in, reg_rax_rdx_mem_reqs,
                     size);
}
...
typedef ir_node *(*unop_constructor)(dbg_info *, ir_node *block, ir_node *op,
                                     x86_insn_size_t size);
...
typedef ir_node *(*unop_out_constructor)(
    dbg_info *, ir_node *block, const int arity, ir_node *const *const in,
    arch_register_req_t const **const reqs, x86_insn_size_t size,
    amd64_op_mode_t opmode, x86_addr_t addr);
...
static ir_node *gen_unop_out(ir_node *const node, int op_pos,
  ...
  if (load != NULL) {
    ...
    in[mem_input] = new_mem;
    addr.mem_input = mem_input;
    new_node = gen(dbgi, new_block, npred, in, gp_am_reqs[arity], size,
                   AMD64_OP_ADDR, addr);
    ...
    new_node = gen(dbgi, new_block, ARRAY_SIZE(in), in, reg_reqs, size,
                   AMD64_OP_REG, addr);
  }
...
static ir_node *gen_float_neg(ir_node *const node) {
  ...
  ir_node *const xor = new_bd_amd64_xorp(dbgi, new_block, ARRAY_SIZE(in), in,
...
static ir_node *gen_Member(ir_node *const node) {
  ...
  if (!is_Proj(ptr) || !is_Start(get_Proj_pred(ptr))) panic("Sel not lowered");
...
static ir_node *gen_Switch(ir_node *const node) {
  ...
  if (ir_platform.pic_style != BE_PIC_NONE) {
    ...
    ir_node *const load =
        new_bd_amd64_movs(dbgi, new_block, load_arity, load_in,
                          amd64_reg_reg_reqs, X86_SIZE_32, AMD64_OP_ADDR, addr);
    ir_node *const load_res = be_new_Proj(load, pn_amd64_movs_res);

    ir_node *const add =
        create_add_lea(dbgi, new_block, X86_SIZE_64, base, load_res);
  ...
  ir_node *const out =
      new_bd_amd64_jmp_switch(dbgi, new_block, arity, in, in_reqs, n_outs,
                              op_mode, X86_SIZE_64, &addr, table, entity);
  return out;
}
...
static ir_node *gen_Start(ir_node *const node) {
  ...
  if (!cconv->omit_fp) outs[REG_RBP] = BE_START_IGNORE;
...
static int make_store_value(amd64_binop_addr_attr_t *const attr,
                            ir_mode *const mode, ir_node *val,
                            ir_node **const in) {
  int arity = 0;
  if (!mode_needs_gp_reg(mode)) {
    goto store_reg;
...
static ir_node *make_store_for_mode(ir_mode *const mode, dbg_info *const dbgi,
                                    ir_node *const block, int const arity,
                                    ir_node *const *const in,
                                    amd64_binop_addr_attr_t const *const attr,
                                    bool const pinned) {
  ...
  ir_node *const new_store =
      cons(dbgi, block, arity, in, reqs[arity - 1], attr);
  set_irn_pinned(new_store, pinned);
  return new_store;
}

static ir_node *make_lea_base_frameoffset(dbg_info *dbgi, ir_node *block,
                                          ir_node *base, int32_t offset) {
  ir_node *lea_in[] = {base};
  x86_addr_t lea_addr = {
      .immediate =
          {
  ...
  return new_bd_amd64_lea(dbgi, block, ARRAY_SIZE(lea_in), lea_in, reg_reqs,
                          X86_SIZE_64, lea_addr);
}
...
static ir_node *gen_Call(ir_node *const node) {
  ...
  ir_graph *const irg = get_irn_irg(node);
  x86_cconv_t *const cconv = amd64_decide_calling_convention(type, NULL);
  ...
  arch_register_req_t const **const in_req =
      be_allocate_in_reqs(irg, max_inputs);
...
no_call_mem:;
  ...
  for (size_t p = 0; p < n_params; ++p) {
    ...
    if (is_aggregate_type(param_type)) {
      ...
      ir_node *const lea =
          make_lea_base_frameoffset(dbgi, block, callframe, param->offset);
      ir_node *const new_value = be_transform_node(value);
      unsigned const size = get_type_size(param_type);
      ir_node *const copyb =
          new_bd_amd64_copyB_i(dbgi, block, lea, new_value, sync1, size);
      ...
      x86_insn_size_t size = x86_size_from_mode(mode);
      if (size < X86_SIZE_32) size = X86_SIZE_32;
      ...
      in[arity++] = sync1;
      sync_ins[sync_arity++] =
          make_store_for_mode(mode, dbgi, new_block, arity, in, &attr, false);
    }
  }
  ...
  /* create call node */
  ir_node *const call = new_bd_amd64_call(dbgi, new_block, in_arity, in, in_req,
                                          out_arity, &call_attr);
  ...
  arch_register_class_t const *const flags =
  ...
  for (size_t r = 0; r < n_ress; ++r) {
    ...
    if (reg != NULL) arch_set_irn_register_req_out(call, o++, reg->single_req);
  }
...
static ir_node *gen_Cmp(ir_node *const node) {
  ...
  if (mode_is_float(cmp_mode)) {
    if (cmp_mode == x86_mode_E) return match_cmp_x87(node, op1, op2);
    match_binop(&args, block, cmp_mode, op1, op2, match_am);
    new_node = new_bd_amd64_ucomis(dbgi, new_block, args.arity, args.in,
                                   args.reqs, &args.attr);
  } else {
    match_binop(&args, block, cmp_mode, op1, op2, match_immediate | match_am);
    new_node = new_bd_amd64_cmp(dbgi, new_block, args.arity, args.in, args.reqs,
                                &args.attr);
  }
...
static ir_node *gen_Phi(ir_node *const node) {
  ...
  if (mode_needs_gp_reg(mode)) {
    ...
    req = mode == x86_mode_E ? &amd64_class_reg_req_x87
                             : &amd64_class_reg_req_xmm;
  } else {
    req = arch_memory_req;
  }
...
static ir_node *match_mov(dbg_info *dbgi, ir_node *block, ir_node *value,
  ...
  ir_node *op;
  bool use_am =
      use_address_matching(mode, match_am, block, NULL, value, &load, &op);
  ...
  if (use_am) {
    ...
    in[input] = new_value;
    reqs =
        get_irn_mode(new_value) == amd64_mode_xmm ? amd64_xmm_reqs : reg_reqs;
    op_mode = AMD64_OP_REG;
  }

  assert((size_t)arity <= ARRAY_SIZE(in));
  ir_node *const new_node =
      create_mov(dbgi, block, arity, in, reqs, size, op_mode, addr);

  if (mem_proj != NULL) be_set_transformed_node(load, new_node);
...
static ir_node *gen_extend(dbg_info *const dbgi, ir_node *const block,
  ...
  create_mov_func const constructor =
      is_signed ? new_bd_amd64_movs : new_bd_amd64_mov_gp;
  unsigned const pn = is_signed ? pn_amd64_movs_res : pn_amd64_mov_gp_res;
  return match_mov(dbgi, block, value, size, constructor, pn);
}

static ir_node *extend_if_necessary(dbg_info *const dbgi, ir_node *const block,
                                    ir_node *const value) {
  if (!needs_extension(value)) return be_transform_node(value);
...
static ir_node *create_cvtsd2ss(dbg_info *dbgi, ir_node *block,
                                ir_node *value) {
...
static void store_to_temp(construct_binop_func const new_store,
                          ...
                          x86_addr_t *addr, dbg_info *dbgi, ir_node *block,
                          ir_node **in, int *n_in, ir_node *new_op,
                          x86_insn_size_t size) {
...
static ir_node *conv_int_to_x87(dbg_info *dbgi, ir_node *block, ir_node *val) {
  ...
  x86_insn_size_t const size = get_size_32_64_from_mode(mode);
  if (!mode_is_signed(mode)) panic("unsigned int -> x87 NIY");
...
static ir_node *conv_x87_to_int(dbg_info *const dbgi, ir_node *const block,
  ...
  x86_insn_size_t const insn_size_src =
      insn_size_dest > X86_SIZE_32 ? X86_SIZE_64 : X86_SIZE_32;

  ir_node *in[5];
  int n_in = 0;
  x86_addr_t addr;
  store_to_temp(new_bd_amd64_fisttp, x87K_reg_mem_reqs, &addr, dbgi, block, in,
                &n_in, new_val, insn_size_src);
  assert(n_in < (int)ARRAY_SIZE(in));

  create_mov_func new_mov =
      insn_size_dest < X86_SIZE_64 ? new_bd_amd64_movs : new_bd_amd64_mov_gp;
...
static ir_node *gen_Conv(ir_node *const node) {
  ...
  if (dst_bits == 128) {
    ...
    if (src_bits < 64) {
      ...
      ir_node *const movd = new_bd_amd64_movd(dbgi, block, n_in, in, reg_reqs,
                                              AMD64_OP_REG, addr);
  ...
  if (src_bits < dst_bits) {
    ...
    if (src_float && dst_float) return be_transform_node(op);
    min_mode = src_mode;
  }
  ...
  if (dst_mode == x86_mode_E) {
    if (!src_float) return conv_int_to_x87(dbgi, block, op);
    /* SSE to x87 */
    return conv_sse_to_x87(dbgi, block, op);
  } else if (src_mode == x86_mode_E) {
    if (!dst_float) return conv_x87_to_int(dbgi, block, op, dst_mode);
  ...
  if (src_float && dst_float) {
    ...
    if (src_bits == 32 && dst_bits == 64) {
      conv = new_bd_amd64_cvtss2sd(dbgi, block, n_in, in, amd64_xmm_reqs, size,
                                   AMD64_OP_REG, addr);
      pn_res = pn_amd64_cvtss2sd_res;
    } else if (src_bits == 64 && dst_bits == 32) {
      conv = new_bd_amd64_cvtsd2ss(dbgi, block, n_in, in, amd64_xmm_reqs,
                                   AMD64_OP_REG, addr);
    ...
    if (src_bits == 32) {
      conv = new_bd_amd64_cvttss2si(dbgi, block, n_in, in, amd64_xmm_reqs, size,
                                    AMD64_OP_REG, addr);
      pn_res = pn_amd64_cvttss2si_res;
    } else if (src_bits == 64) {
      conv = new_bd_amd64_cvttsd2si(dbgi, block, n_in, in, amd64_xmm_reqs, size,
                                    AMD64_OP_REG, addr);
    ...
    if (mode_is_signed(src_mode)) {
      if (src_bits < 32) {
        ...
        size = X86_SIZE_32;
        ir_node *const ext = new_bd_amd64_movs(dbgi, block, n_in, in, reg_reqs,
                                               move_mode, AMD64_OP_REG, addr);
        in[0] = be_new_Proj(ext, pn_amd64_movs_res);
      } else if (src_bits != 32 && src_bits != 64) {
        panic(
            "conversion of signed %d bit integer to "
      ...
      if (src_bits >= 64) {
        panic("cannot convert unsigned %d-bit to floating point", src_bits);
      }

      size = src_bits >= 32 ? X86_SIZE_64 : X86_SIZE_32;
      ir_node *const ext = new_bd_amd64_mov_gp(dbgi, block, n_in, in, reg_reqs,
                                               move_mode, AMD64_OP_REG, addr);
    ...
    if (dst_bits == 32) {
      ...
      conv = new_bd_amd64_cvtsi2ss(dbgi, block, n_in, in, reg_reqs, size,
                                   AMD64_OP_REG, addr);
      pn_res = pn_amd64_cvtsi2ss_res;
    } else if (dst_bits == 64) {
      // http://www.felixcloutier.com/x86/CVTSI2SD.html
      conv = new_bd_amd64_cvtsi2sd(dbgi, block, n_in, in, reg_reqs, size,
                                   AMD64_OP_REG, addr);
...
static ir_node *gen_Load(ir_node *const node) {
  ...
  create_mov_func const cons =
      ...
      : get_mode_size_bits(mode) < 64 && mode_is_signed(mode)
          ? &new_bd_amd64_movs
          :
          /**/ &new_bd_amd64_mov_gp;
  x86_insn_size_t const size = x86_size_from_mode(mode);
  ir_node *const new_load =
      cons(dbgi, block, arity, in, reqs, size, AMD64_OP_ADDR, addr);
...
static ir_node *gen_Alloc(ir_node *const node) {
  ...
  in[arity++] = new_mem;
  ir_node *subsp = new_bd_amd64_sub_sp(dbgi, new_block, arity, in, reqs, &attr);

  ir_node *const stack_proj =
      be_new_Proj_reg(subsp, pn_amd64_sub_sp_stack, &amd64_registers[REG_RSP]);
...
static ir_node *gen_Proj_Alloc(ir_node *const node) {
  ...
  switch ((pn_Alloc)pn) {
    case pn_Alloc_M:
      return be_new_Proj(new_alloc, pn_amd64_sub_sp_M);
    case pn_Alloc_res:
      return be_new_Proj(new_alloc, pn_amd64_sub_sp_addr);
  }
...
static ir_node *gen_clz(ir_node *const node) {
  ...
  ir_node *const bsr =
      gen_unop_out(node, n_Builtin_max + 1, new_bd_amd64_bsr, pn_amd64_bsr_res);
  ...
  ir_node * xor
      = new_bd_amd64_xor(dbgi, block, ARRAY_SIZE(in), in, reg_reqs, &attr);
...
static ir_node *gen_ffs(ir_node *const node) {
  /* bsf input, result */
  ir_node *const bsf_res =
      gen_unop_out(node, n_Builtin_max + 1, new_bd_amd64_bsf, pn_amd64_bsf_res);
  ...
  ir_node * or = new_bd_amd64_or(dbgi, block, ARRAY_SIZE(or_in), or_in,
                                 amd64_reg_reg_reqs, &or_attr);
  ...
  ir_node *inc = new_bd_amd64_add(dbgi, block, ARRAY_SIZE(inc_in), inc_in,
                                  reg_reqs, &inc_attr);
...
static ir_node *gen_saturating_increment(ir_node *const node) {
  ...
  ir_node *const inc = new_bd_amd64_add(dbgi, new_block, ARRAY_SIZE(inc_in),
                                        inc_in, reg_reqs, &inc_attr);
  arch_set_irn_register_req_out(inc, 0, &amd64_requirement_gp_same_0);
  ...
  ir_node *const sbb = new_bd_amd64_sbb(dbgi, new_block, ARRAY_SIZE(in), in,
                                        reg_flags_reqs, &sbb_attr);
...
static ir_node *gen_Bitcast(ir_node *const node) {
  ...
  if (src_float && !dst_float) {
    return new_bd_amd64_movd_xmm_gp(dbgi, be_block, be_op, X86_SIZE_64,
                                    AMD64_OP_REG, addr);
  } else if (!src_float && dst_float) {
    return new_bd_amd64_movd_gp_xmm(dbgi, be_block, be_op, X86_SIZE_64,
                                    AMD64_OP_REG, addr);
  } else {
...
void amd64_transform_graph(ir_graph *irg) {
  ...
  if (is_variadic) amd64_insert_reg_save_area(irg, current_cconv);
