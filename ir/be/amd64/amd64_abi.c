...
static ir_mode *classify_slice_for_amd64(ir_type const *tp, unsigned min,
                                         unsigned max);
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    if (min < member_end && max > member_begin) {
      ...
      ir_mode *member_class =
          classify_slice_for_amd64(member_type, min_in_member, max_in_member);
...
static ir_mode *classify_slice_for_amd64(ir_type const *const tp, unsigned min,
                                         unsigned max) {
...
static aggregate_spec_t classify_for_amd64(amd64_abi_state *s,
                                           ir_type const *const tp,
                                           unsigned max_integer,
                                           unsigned max_sse) {
...
static void notify_amd64_scalar(amd64_abi_state *s,
                                ir_type const *const param_type,
                                unsigned max_integer, unsigned max_sse) {
...
aggregate_spec_t amd64_lower_result(void *env, ir_type const *const type) {
  ...
  if (is_aggregate_type(type)) {
    aggregate_spec_t result =
        classify_for_amd64(state, type, max_integer_results, max_sse_results);
