...
static void peephole_amd64_cmp(ir_node *const node) {
  ...
  amd64_binop_addr_attr_t const *const attr =
      get_amd64_binop_addr_attr_const(node);
  if (attr->base.base.op_mode == AMD64_OP_REG_IMM) {
    ...
    if (imm->kind == X86_IMM_VALUE && imm->offset == 0) {
      ...
      ir_node *const test = new_bd_amd64_test(dbgi, block, ARRAY_SIZE(in), in,
                                              amd64_reg_reg_reqs, &test_attr);

      arch_register_t const *const oreg =
          arch_get_irn_register_out(node, pn_amd64_cmp_flags);
...
static void make_add(ir_node *const node, size_t const n_in,
...
static void peephole_amd64_lea(ir_node *const node) {
  if (be_peephole_get_value(REG_EFLAGS)) return;

  arch_register_t const *const oreg =
      arch_get_irn_register_out(node, pn_amd64_lea_res);
  ...
  } else if (addr->variant == X86_ADDR_BASE_INDEX && addr->log_scale == 0 &&
             !addr->immediate.entity && addr->immediate.offset == 0) {
    ir_node *l;
    ir_node *r;
    ir_node *const base = get_irn_n(node, addr->base_input);
...
static void peephole_amd64_mov_imm(ir_node *const node) {
  ...
  if (imm->kind == X86_IMM_VALUE && imm->offset == 0) {
    ...
    arch_register_t const *const reg =
        arch_get_irn_register_out(node, pn_amd64_mov_imm_res);
...
static void peephole_amd64_mov_gp(ir_node *const node) {
  /*
   * amd64_mov_gp instructions always perform a mov with zero extension (or the
   * mov instruction overwrites the whole quadword anyway). This optimization
   * removes unnecessary zero extensions, for example after a load: movzwl
   * (%r1), %r2 movzwl %r2, %r2    <-- remove this instruction
   */
  ...
  if (is_amd64_mov_gp(operand)) {
    if (get_amd64_attr_const(node)->size <
        get_amd64_attr_const(operand)->size) {
      return;
    }
    arch_register_t const *const operand_out =
        arch_get_irn_register_out(operand, pn_amd64_mov_gp_res);
    arch_register_t const *const node_out =
        arch_get_irn_register_out(node, pn_amd64_mov_gp_res);
    arch_register_t const *const node_in = arch_get_irn_register_in(node, 0);
    if (operand_out == node_out && node_in == node_out) {
      ir_node *const node_proj =
          be_get_or_make_Proj_for_pn(node, pn_amd64_mov_gp_res);
...
static void peephole_amd64_cvtsi2sX(ir_node *const node) {
  /**
   * cvtsi2sd / cvtsi2ss instructions have a dependency on the destination
   * register, as the upper part of the xmm register remains unmodified, but
   * this dependency is not present in the amd64 backend. XORing the register to
   * zero before the convert instruction breaks the dependency chain seen by the
   * processor -> faster out-of-order execution
   */
  arch_register_t const *const reg =
      arch_get_irn_register_out(node, pn_amd64_cvtsi2sd_res);
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = get_nodes_block(node);
