...
static void transform_sub_to_neg_add(ir_node *node,
  ...
  if (is_amd64_subs(node)) {
    ...
    ir_node *xor_in[] = {in2};
    ir_node *const xor = new_bd_amd64_xorp(dbgi, block, ARRAY_SIZE(xor_in),
                                           xor_in, amd64_xmm_reqs, &xor_attr);
    sched_add_before(node, xor);
    ir_node *const neg = be_new_Proj_reg(xor, pn_amd64_xorp_res, in2_reg);

    ir_node *in[] = {neg, in1};
    add = new_bd_amd64_adds(dbgi, block, ARRAY_SIZE(in), in, amd64_xmm_xmm_reqs,
                            attr);
    ...
    add = new_bd_amd64_add(dbgi, block, ARRAY_SIZE(in), in, amd64_reg_reg_reqs,
                           attr);
...
static void amd64_turn_back_am(ir_node *const node,
  ...
  ir_node *const load = new_bd_amd64_mov_gp(
      dbgi, block, load_arity, load_in, gp_am_reqs[load_arity - 1],
      attr->base.size, AMD64_OP_ADDR, new_addr);
...
static bool amd64_handle_2addr(ir_node *const node,
  ...
  if (attr->op_mode == AMD64_OP_REG_ADDR) {
    ...
    if ((x86_addr_variant_has_base(addr->variant) &&
         arch_get_irn_register_in(node, addr->base_input) == out_reg) ||
        (x86_addr_variant_has_index(addr->variant) &&
         arch_get_irn_register_in(node, addr->index_input) == out_reg)) {
