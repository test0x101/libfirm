...
static ir_node *create_push(ir_node *node, ir_node *schedpoint, ir_node *sp,
  ...
  ir_node *const push = new_bd_amd64_push_am(dbgi, block, ARRAY_SIZE(in), in,
                                             rsp_reg_mem_reqs, size, addr);
...
static ir_node *create_pop(ir_node *node, ir_node *schedpoint, ir_node *sp,
  ...
  ir_node *const pop = new_bd_amd64_pop_am(dbgi, block, ARRAY_SIZE(in), in,
                                           rsp_reg_mem_reqs, size, addr);
...
static void rewrite_unsigned_float_Conv(ir_node *node) {
  ...
  ir_tarval *magic0 = new_integer_tarval_from_str("4530000043300000", 16, 0, 16,
                                                  amd64_mode_xmm);
  ...
  ir_tarval *magic1 = new_integer_tarval_from_str(
      "45300000000000004330000000000000", 32, 0, 16, amd64_mode_xmm);
...
static void rewrite_float_unsigned_Conv(ir_node *node) {
  ...
  ir_node *cmp =
      new_rd_Cmp(dbgi, block, fp_x, fp_const, ir_relation_greater_equal);
  ...
  ir_node *false_res = new_rd_Conv(dbgi, false_block, false_signed, dest_mode);
...
static bool is_u64(ir_mode *const mode) {
  return !mode_is_float(mode) && !mode_is_signed(mode) &&
...
static void amd64_intrinsics_walker(ir_node *node, void *data) {
  ...
  if (is_Conv(node)) {
    if (amd64_rewrite_Conv(node)) *changed = true;
  }
}
...
static void amd64_handle_intrinsics(ir_graph *irg) {
  ...
  if (changed) {
    confirm_irg_properties(
        irg, IR_GRAPH_PROPERTY_NO_BADS | IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES |
                 IR_GRAPH_PROPERTY_MANY_RETURNS | IR_GRAPH_PROPERTY_ONE_RETURN);
  }
}
...
static void amd64_collect_frame_entity_nodes(ir_node *node, void *data) {
  ...
  if (!is_amd64_irn(node) || !amd64_loads(node)) return;
...
static void introduce_epilogue(ir_node *ret, bool omit_fp) {
  ...
  if (!omit_fp) {
    int const n_rbp =
        be_get_input_pos_for_req(ret, &amd64_single_reg_req_gp_rbp);
    assert(n_rbp >= 0);
    ...
    curr_bp =
        be_new_Proj_reg(leave, pn_amd64_leave_frame, &amd64_registers[REG_RBP]);
    curr_sp =
        be_new_Proj_reg(leave, pn_amd64_leave_stack, &amd64_registers[REG_RSP]);
    ...
    ir_node *incsp = amd64_new_IncSP(block, curr_sp, -(int)frame_size, true);
...
static void introduce_prologue(ir_graph *const irg, bool omit_fp) {
  ...
  if (!omit_fp) {
    ...
    ir_node *const push = new_bd_amd64_push_reg(NULL, block, initial_sp, mem,
                                                initial_bp, X86_SIZE_64);
    ...
    ir_node *const incsp =
        amd64_new_IncSP(block, initial_sp, frame_size, false);
...
static void amd64_finish(void) { amd64_free_opcodes(); }
...
static void amd64_lower_for_target(void) {
  ...
  lower_calls_with_compounds(LF_RETURN_HIDDEN, amd64_lower_parameter,
                             &parameter_state, amd64_lower_result,
                             &result_state, amd64_reset_abi_state);
...
static void amd64_init(void) {
  ...
  ir_target.experimental =
      "the amd64 backend is experimental and unfinished (consider the ia32 "
      "backend)";
...
static be_register_name_t const amd64_additional_reg_names[] = {
    ...
    {"sil", REG_RSI},  {"si", REG_RSI},   {"esi", REG_RSI},  {"dil", REG_RDI},
    {"di", REG_RDI},   {"edi", REG_RDI},  {"bpl", REG_RBP},  {"bp", REG_RBP},
    {"ebp", REG_RBP},  {"spl", REG_RSP},  {"sp", REG_RSP},   {"esp", REG_RSP},
    {"r8b", REG_R8},   {"r8w", REG_R8},   {"r8d", REG_R8},   {"r9b", REG_R9},
    {"r9w", REG_R9},   {"r9d", REG_R9},   {"r10b", REG_R10}, {"r10w", REG_R10},
    {"r10d", REG_R10}, {"r11b", REG_R11}, {"r11w", REG_R11}, {"r11d", REG_R11},
    {"r12b", REG_R12}, {"r12w", REG_R12}, {"r12d", REG_R12}, {"r13b", REG_R13},
    {"r13w", REG_R13}, {"r13d", REG_R13}, {"r14b", REG_R14}, {"r14w", REG_R14},
    {"r14d", REG_R14}, {"r15b", REG_R15}, {"r15w", REG_R15}, {"r15d", REG_R15},
    {NULL, ~0u}};
...
void be_init_arch_amd64(void) {
  /*static const lc_opt_table_entry_t options[] = {
          LC_OPT_ENT_BOOL("no-red-zone", "gcc compatibility",
  &amd64_use_red_zone), LC_OPT_ENT_BOOL("fma",         "support FMA3 code
  generation",     &use_scalar_fma3), LC_OPT_LAST
  };
