...
static unsigned const *get_decisive_partner_regs(be_operand_t const *const o1,
                                                 size_t const n_regs) {
...
static bool list_has_irn_else_add(ir_node **const list, size_t const n,
                                  ir_node *const irn) {
  for (ir_node *const *i = list; i != list + n; ++i) {
    if (*i == irn) return true;
  }
  list[n] = irn;
  return false;
}

static void pair_up_operands(be_chordal_env_t *const env,
                             be_insn_t *const insn) {
  ...
  for (int i = 0; i < n_ops; ++i) {
    ...
    for (int j = i + 1; j < n_ops; ++j) {
      ...
      if (!partner->partner && partner->carrier &&
          op_is_def != partner_is_def &&
          rbitsets_have_common(op->regs, partner->regs, n_regs) &&
          (partner_is_def ||
           !be_value_live_after(partner->carrier, insn->irn))) {
        op->partner = partner;
        partner->partner = op;
        break;
      }
...
static void handle_constraints(be_chordal_env_t *const env,
                               ir_node *const irn) {
  ...
  for (int i = 0, n_ops = insn->n_ops; i < n_ops; ++i) {
    ...
    if (!op->carrier) continue;
    if (op->partner && pmap_contains(partners, op->partner->carrier)) continue;

    ir_node *const partner = op->partner ? op->partner->carrier : NULL;
    pmap_insert(partners, op->carrier, partner);
    if (partner != NULL) pmap_insert(partners, partner, op->carrier);
    ...
    if (bs) {
      ...
      rbitset_foreach(bs, n_regs, col) {
        arch_register_t const *const reg =
            arch_register_for_index(env->cls, col);
  ...
  if (perm != NULL) {
    foreach_out_edge(perm, edge) {
      ...
      if (pmap_contains(partners, proj)) continue;
  ...
  for (int i = 0; i < n_alloc; ++i) {
    assert(assignment[i] >= 0 &&
           ...
           "pressure faithful?)");
    arch_register_t const *const reg =
        arch_register_for_index(env->cls, assignment[i]);
...
static void constraints(ir_node *const bl, void *const data) {
  ...
  sched_foreach_safe(bl, irn) { handle_constraints(env, irn); }
}
  ...
  foreach_border_head(head, b) {
    ...
    if (!b->is_def) {
      ...
      if (reg) {
        ...
        col = reg->index;
        assert(bitset_is_set(available, col) &&
               "pre-colored register must be free");
      } else {
...
void be_init_chordal(void) {
  static be_ra_chordal_coloring_t coloring = {be_ra_chordal_color};
