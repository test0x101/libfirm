 ...
 *
 * Split into multiple instructions if offset exceeds RISC-V 12 bit immediate
 * range.
 */
...
/**
 * The address offset immediate at load and store instructions is limited to 12
 * bit. Adds an additional lui and add nodes to support large offsets.
 */
...
/**
 * RISC-V immediates are limited. Split IncSP with bigger immediates if
 * necessary.
 */
...
/**
 * Optimize consecutive shift immediate operations by combining them in a single
 * operation
 */
static void peephole_riscv_shift(ir_node *const node) {
  ...
  if (!is_riscv_irn(pred) || !be_has_only_one_user(pred) ||
      sched_prev(node) != pred) {
    return;
  }
  if (get_riscv_irn_opcode(pred) == opcode) {
    riscv_immediate_attr_t *const node_attr_imm =
        get_riscv_immediate_attr(node);
    riscv_immediate_attr_t *const pred_attr_imm =
        get_riscv_immediate_attr(pred);
