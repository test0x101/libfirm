...
static int riscv_attrs_equal_(riscv_attr_t const *const a_attr,
                              riscv_attr_t const *const b_attr) {
#if 0
...
int riscv_cond_attrs_equal(ir_node const *const a, ir_node const *const b) {
  ...
  return riscv_attrs_equal_(&a_attr->attr, &b_attr->attr) &&
         a_attr->cond == b_attr->cond;
}

int riscv_immediate_attrs_equal(ir_node const *const a,
                                ir_node const *const b) {
  riscv_immediate_attr_t const *const a_attr =
      get_riscv_immediate_attr_const(a);
  riscv_immediate_attr_t const *const b_attr =
      get_riscv_immediate_attr_const(b);
  return riscv_attrs_equal_(&a_attr->attr, &b_attr->attr) &&
         a_attr->ent == b_attr->ent && a_attr->val == b_attr->val;
}
...
static void dump_immediate(FILE *const F, char const *const prefix,
                           ir_node const *const n) {
  ...
  if (imm->ent) {
    ...
    if (imm->val != 0) fprintf(F, "%+" PRId32, imm->val);
    if (prefix) fputc(')', F);
  } else {
...
void riscv_dump_node(FILE *const F, ir_node const *const n,
                     dump_reason_t const reason) {
  switch (reason) {
    ...
    case dump_node_opcode_txt:
      ...
      switch ((riscv_opcodes)get_riscv_irn_opcode(n)) {
        ...
        case iro_riscv_ori:
        case iro_riscv_xori: {
          riscv_immediate_attr_t const *const imm =
              get_riscv_immediate_attr_const(n);
