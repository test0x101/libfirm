...
static void emit_immediate_val(char const *const prefix, ir_entity *const ent,
                               int32_t const val) {
  if (ent) {
    if (prefix) be_emit_irprintf("%s(", prefix);
    be_gas_emit_entity(ent);
    if (val != 0) be_emit_irprintf("%+" PRId32, val);
...
static void emit_immediate(char const *const prefix,
...
static void emit_riscv_asm_operand(ir_node const *const node,
                                   char const modifier, unsigned const pos) {
  be_asm_attr_t const *const attr = get_be_asm_attr_const(node);
  riscv_asm_operand_t const *const op =
      &((riscv_asm_operand_t const *)attr->operands)[pos];
  /* modifiers:
   *   R: %lo of immediate
   *   c: plain immediate
   *   h: %hi of immediate
...
static void emit_be_MemPerm(ir_node const *const node) {
  ...
  for (int i = 0; i < memperm_arity; ++i) {
    ...
    arch_register_t const *const reg =
        arch_register_for_index(&riscv_reg_classes[CLASS_riscv_gp], i + 9);
...
static void emit_riscv_j(ir_node const *const node) { emit_jmp(node, node); }
