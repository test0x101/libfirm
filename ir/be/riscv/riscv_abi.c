...
static ir_mode *classify_slice_for_ilp32(ir_type const *const tp, unsigned min,
                                         unsigned max);

static ir_mode *classify_compound_by_members(ir_type const *const tp,
  ...
  for (unsigned i = 0; i < n; i++) {
    ...
    if (min < member_end && max > member_begin) {
      ...
      unsigned max_in_member =
          member_end < max ? member_size : max - member_begin;
      ir_mode *member_class =
          classify_slice_for_ilp32(member_type, min_in_member, max_in_member);
...
static aggregate_spec_t classify_for_ilp32(ir_type const *const type) {
  ...
  for (unsigned i = 0; i < 2; i++) {
    ir_mode *c = classify_slice_for_ilp32(type, RISCV_REGISTER_SIZE * i,
                                          RISCV_REGISTER_SIZE * (i + 1));
    result.modes[i] = c;
    if (c != mode_BAD) {
      result.length++;
    }
  ...
  /* if type has 8 byte alignment and result is two (4-byte) mode_Iu slices we
   * have to convert this into a single 8 byte mode_Lu slice to ensure correct
   * alignment when passing the argument. (Variadic arguments with 2*XLEN-bit
   * alignment and size at most 2*XLEN bits are passed in an aligned register
   * pair) */
  if (result.length == 2 && result.modes[0] == mode_Iu &&
      result.modes[1] == mode_Iu &&
