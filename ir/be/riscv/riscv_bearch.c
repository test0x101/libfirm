...
static int abi;
static lc_opt_enum_int_var_t abi_var = {&abi, abi_items};
...
/**
 * Splits a 32 bit immediate value into a 20 bit hi and a 12 bit lo part, which
 * can be realised by lui and addi instructions.
 */
...
static int riscv_ifconv(ir_node const *const sel,
...
static void riscv_init(void) {
  ...
  ir_target.experimental =
      "the RISC-V backend is highly experimental and unfinished";
  ...
  if (use_softfloat && (riscv_abi_t)abi == ilp32d) {
    ...
    panic(
        "Use of hard-float instructions with soft-float ABI not supported yet");
  }
}

static void riscv_finish(void) { riscv_free_opcodes(); }
...
static ir_node *riscv_new_spill(ir_node *const value, ir_node *const after) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ...
    ir_node *const store =
        new_bd_riscv_sw(NULL, block, nomem, frame, value, NULL, 0);
...
static ir_node *riscv_new_reload(ir_node *const value, ir_node *const spill,
                                 ir_node *const before) {
  ir_mode *const mode = get_irn_mode(value);
  if (be_mode_needs_gp_reg(mode)) {
...
static void riscv_collect_frame_entity_nodes(ir_node *const node,
                                             void *const data) {
  ...
  if (is_riscv_lw(node)) {
    ...
    ir_node *const frame = get_irg_frame(irg);
    if (base == frame) {
      riscv_immediate_attr_t const *const attr =
          get_riscv_immediate_attr_const(node);
      if (!attr->ent) {
...
static void riscv_set_frame_entity(ir_node *const node, ir_entity *const entity,
                                   unsigned const size,
...
static ir_node *riscv_new_IncSP(ir_node *const block, ir_node *const sp,
                                int const offset, unsigned const align) {
  return be_new_IncSP(block, sp, offset, align);
}

static void riscv_introduce_prologue(ir_graph *const irg, unsigned size,
                                     bool omit_fp, bool is_variadic) {
  ...
  if (!omit_fp) {
    ...
    int fp_save_offset = is_variadic
                             ? -(RISCV_N_PARAM_REGS + 1) * RISCV_REGISTER_SIZE
                             : -RISCV_REGISTER_SIZE;
    if (!is_simm12(aligned)) {
      /* if desired size is not as 12 bit immediate encodeable, first compute
       * the large offset in t0 */
      ...
      /* save fp to stack */
      ir_node *const store_fp = new_bd_riscv_sw(NULL, block, mem, add, start_fp,
                                                NULL, fp_save_offset);
      ...
      /* save fp to stack */
      ir_node *const store_fp = new_bd_riscv_sw(
          NULL, block, mem, start_sp, start_fp, NULL, aligned + fp_save_offset);
      sched_add_after(inc_sp, store_fp);
      edges_reroute_except(mem, store_fp, store_fp);

      /* set current fp */
      ir_node *const curr_fp =
          new_bd_riscv_addi(NULL, block, inc_sp, NULL, aligned);
      sched_add_after(store_fp, curr_fp);
      edges_reroute_except(start_fp, curr_fp, store_fp);
      arch_set_irn_register(curr_fp, &riscv_registers[REG_S0]);
    }
...
static void riscv_introduce_epilogue(ir_node *const ret, unsigned const size,
  ...
  if (!omit_fp) {
    ...
    int fp_save_offset = is_variadic
                             ? -(RISCV_N_PARAM_REGS + 1) * RISCV_REGISTER_SIZE
                             : -RISCV_REGISTER_SIZE;
    ir_node *const load_old_fp =
        new_bd_riscv_lw(NULL, block, curr_mem, curr_fp, NULL, fp_save_offset);
    curr_mem = be_new_Proj(load_old_fp, pn_riscv_lw_M);
    ir_node *const old_fp =
        be_new_Proj_reg(load_old_fp, pn_riscv_lw_res, &riscv_registers[REG_T0]);
...
static void riscv_introduce_prologue_epilogue(ir_graph *const irg,
  ...
  if (size == 0 && omit_fp && !is_variadic) return;
...
static void riscv_sp_sim(ir_node *const node,
                         stack_pointer_state_t *const state) {
...
static void riscv_generate_code(FILE *const output,
                                char const *const cup_name) {
  ...
  foreach_irp_irg(i, irg) {
    ...
    ir_entity *const fun_ent = get_irg_entity(irg);
    ir_type *fun_type = get_entity_type(fun_ent);
    // variadic functions copy the register arguments to the vararg save area on
    // the stack
    int begin = is_method_variadic(fun_type)
                    ? -(RISCV_REGISTER_SIZE * RISCV_N_PARAM_REGS)
                    : 0;
...
static void riscv32_lower_va_arg(ir_node *node) {
  ...
  if (round_up > RISCV_PARAM_STACK_ALIGN) {
    ...
    ir_node *const align_min_1 =
        new_rd_Const_long(dbgi, irg, offset_mode, (int)round_up - 1);
    ir_node *const align_add = new_rd_Add(dbgi, block, ap, align_min_1);
    ir_node *const align_neg =
        new_rd_Const_long(dbgi, irg, offset_mode, -round_up);
    ir_node *const align_add_conv =
        new_rd_Conv(dbgi, block, align_add, offset_mode);
    ir_node *const align_and =
        new_rd_And(dbgi, block, align_add_conv, align_neg);
    ir_node *const align_and_conv = new_rd_Conv(dbgi, block, align_and, mode_P);
    ap = align_and_conv;
  }

  if (is_compound_type(original_aptype) &&
      get_type_size(original_aptype) <= 2 * RISCV_REGISTER_SIZE) {
    ...
    ir_node *const load =
        new_rd_Load(dbgi, block, node_mem, ap, apmode, aptype, cons_none);
...
static void riscv_lower_for_target(void) {
  ...
  lower_calls_with_compounds(LF_RETURN_HIDDEN, riscv_lower_parameter, NULL,
                             riscv_lower_result, NULL, reset_stateless_abi);
  ...
  static ir_builtin_kind const supported[] = {ir_bk_saturating_increment,
                                              ir_bk_va_start};
  lower_builtins(ARRAY_SIZE(supported), supported, riscv32_lower_va_arg);
  foreach_irp_irg(i, irg) { be_after_transform(irg, "lower-builtins"); }
