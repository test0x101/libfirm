...
static inline riscv_attr_t const *get_riscv_attr_const(
    ir_node const *const node) {
  return (riscv_attr_t const *)get_irn_generic_attr_const(node);
}

static inline riscv_cond_attr_t const *get_riscv_cond_attr_const(
    ir_node const *const node) {
  return (riscv_cond_attr_t const *)get_irn_generic_attr_const(node);
}

static inline riscv_immediate_attr_t *get_riscv_immediate_attr(
    ir_node *const node) {
  return (riscv_immediate_attr_t *)get_irn_generic_attr(node);
}

static inline riscv_immediate_attr_t const *get_riscv_immediate_attr_const(
    ir_node const *const node) {
  return (riscv_immediate_attr_t const *)get_irn_generic_attr_const(node);
}

static inline riscv_switch_attr_t const *get_riscv_switch_attr_const(
    ir_node const *const node) {
