...
static unsigned const caller_saves[] = {
    REG_RA, REG_T1, REG_T2, REG_A0, REG_A1, REG_A2, REG_A3, REG_A4,
    REG_A5, REG_A6, REG_A7, REG_T3, REG_T4, REG_T5, REG_T6,
};
...
static riscv_addr make_addr(ir_node *addr) {
  ...
  if (is_Member(addr)) {
    ...
    assert(is_Proj(addr) && get_Proj_num(addr) == pn_Start_P_frame_base &&
           is_Start(get_Proj_pred(addr)));
  }
...
static ir_node *make_address(ir_node const *const node, ir_entity *const ent,
                             int32_t const val) {
...
static ir_node *make_extension(dbg_info *const dbgi, ir_node *const op,
                               unsigned const to_size) {
...
static void riscv_parse_constraint_letter(void const *const env,
...
static bool riscv_check_immediate_constraint(long const val,
                                             char const imm_type) {
  switch (imm_type) {
    case 'I':
      return is_simm12(val);
    case 'J':
      return val == 0;
    case 'K':
      return is_uimm5(val);
...
static bool riscv_match_immediate(riscv_asm_operand_t *const operand,
  ...
  if (offset) {
    ...
    if (!riscv_check_immediate_constraint(value, imm_type)) return false;
  } else {
    value = 0;
  }
...
static ir_node *gen_ASM(ir_node *const node) {
  ...
  riscv_asm_operand_t *const operands =
      NEW_ARR_DZ(riscv_asm_operand_t, obst, n_constraints);
  for (size_t i = 0; i != n_constraints; ++i) {
    ...
    be_parse_asm_constraints_internal(&be_constraint, c->constraint,
                                      &riscv_parse_constraint_letter, NULL);
...
static ir_node *gen_Alloc(ir_node *node) {
  ...
  if (is_Const(size)) {
    ...
    assert((sizel & ((1 << RISCV_PO2_STACK_ALIGNMENT) - 1)) == 0 &&
           "Found Alloc with misaligned constant");
    assert(is_simm12(sizel));
    subsp = new_bd_riscv_SubSPimm(dbgi, new_block, new_mem, stack_pred, NULL,
                                  -sizel);
  } else {
  ...
  ir_node *const stack_proj =
      be_new_Proj_reg(subsp, pn_riscv_SubSP_stack, &riscv_registers[REG_SP]);
...
static ir_node *gen_Proj_Alloc(ir_node *node) {
  ...
  switch ((pn_Alloc)pn) {
    case pn_Alloc_M:
      return be_new_Proj(new_alloc, pn_riscv_SubSP_M);
    case pn_Alloc_res:
      return be_new_Proj(new_alloc, pn_riscv_SubSP_addr);
  }
  panic("invalid Proj->Alloc");
}

typedef ir_node *cons_binop(dbg_info *, ir_node *, ir_node *, ir_node *);
typedef ir_node *cons_binop_imm(dbg_info *, ir_node *, ir_node *, ir_entity *,
                                int32_t);

static ir_node *gen_logic_op(ir_node *const node, cons_binop *const cons,
                             cons_binop_imm *const cons_imm) {
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = be_transform_nodes_block(node);
  ...
  if (is_Const(r)) {
    long const val = get_Const_long(r);
    if (is_simm12(val)) return cons_imm(dbgi, block, new_l, NULL, val);
  }
  ir_node *const new_r = be_transform_node(r);
...
static ir_node *gen_Call(ir_node *const node) {
  ...
  for (size_t i = 0; i != n_params; ++i) {
    ...
    if (param->reg) {
      ...
      mems[m++] = new_bd_riscv_sw(dbgi, block, nomem, call_frame, val, NULL,
                                  param->offset);
    }
  }
  ...
  ir_node *const jal =
      callee ? new_bd_riscv_jal(dbgi, block, p, ins, reqs, n_res, callee, 0)
  ...
  for (size_t i = 0; i != ARRAY_SIZE(caller_saves); ++i) {
    arch_set_irn_register_req_out(jal, pn_riscv_jal_first_result + i,
                                  riscv_registers[caller_saves[i]].single_req);
  }
...
static ir_node *gen_Cmp(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ir_relation const rel =
        get_Cmp_relation(node) & ir_relation_less_equal_greater;
    switch (rel) {
...
static ir_node *gen_Cond(ir_node *const node) {
  ...
  if (is_Cmp(sel)) {
    ...
    if (be_mode_needs_gp_reg(mode)) {
      ir_relation const rel =
          get_Cmp_relation(sel) & ir_relation_less_equal_greater;
...
static ir_node *gen_Const(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ...
    if (val == 0) {
      ...
      if (imm.lo != 0) res = new_bd_riscv_addi(dbgi, block, res, NULL, imm.lo);
      return res;
    }
  }
  TODO(node);
...
static ir_node *gen_Div(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode) &&
      get_mode_size_bits(mode) == RISCV_MACHINE_SIZE) {
    dbg_info *const dbgi = get_irn_dbg_info(node);
    ir_node *const block = be_transform_nodes_block(node);
...
typedef ir_node *cons_loadop(dbg_info *, ir_node *, ir_node *, ir_node *,
                             ir_entity *, int32_t);
...
static ir_node *gen_Member(ir_node *const node) {
  ...
  assert(is_Proj(ptr) && get_Proj_num(ptr) == pn_Start_P_frame_base &&
         is_Start(get_Proj_pred(ptr)));
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = be_transform_nodes_block(node);
...
static ir_node *gen_Proj_Load(ir_node *const node) {
  ...
  switch ((pn_Load)pn) {
    case pn_Load_M:
      return be_new_Proj(load, pn_riscv_lw_M);
    case pn_Load_res:
...
static ir_node *gen_Proj_Proj_Call(ir_node *const node) {
  ...
  riscv_determine_calling_convention(&cconv, fun_type,
                                     get_method_n_params(fun_type), NULL);
...
static ir_node *gen_Proj_Proj_Start(ir_node *const node) {
  ...
  if (param->reg) {
    ...
    ir_node *const base =
        cur_cconv.omit_fp ? get_Start_sp(irg) : get_Start_fp(irg);
    ir_node *const load =
...
static ir_node *gen_Proj_Start(ir_node *const node) {
  ...
  switch ((pn_Start)get_Proj_num(node)) {
    ...
    case pn_Start_P_frame_base:
      return cur_cconv.omit_fp ? get_Start_sp(irg) : get_Start_fp(irg);
...
static ir_node *gen_shift_op(ir_node *const node, cons_binop *const cons,
                             cons_binop_imm *const cons_imm,
  ...
  /* right shift operations with mode sizes < 32 Bit require correct sign/zero
   * extension. This is done by first shifting the operand to the left by
   * 32-size bits and then shifting back to the right (arithmetic/logical) by
   * the same amount. Finally the actual shift operation is performed. If the
   * shift amount is known (r is const), the two right shift operations are
...
static ir_node *gen_Start(ir_node *const node) {
  ...
  if (!cur_cconv.omit_fp) outs[REG_S0] = BE_START_IGNORE;

  return be_new_Start(irg, outs);
}

typedef ir_node *cons_storeop(dbg_info *, ir_node *, ir_node *, ir_node *,
                              ir_node *, ir_entity *, int32_t);
...
static ir_node *gen_Store(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ...
    ir_node *const store =
        cons(dbgi, block, mem, addr.base, val, addr.ent, addr.val);
...
static ir_node *gen_Switch(ir_node *const node) {
  ...
  ir_switch_table const *const table =
      ir_switch_table_duplicate(irg, get_Switch_table(node));

  ir_type *const utype = get_unknown_type();
  ident *const id = id_unique("TBL");
  ir_entity *const entity =
      new_global_entity(irp->dummy_owner, id, utype, ir_visibility_private,
...
static bool riscv_variadic_fixups(ir_graph *const irg) {
  ...
  if (!is_method_variadic(mtp)) return false;

  size_t const n_params = get_method_n_params(mtp);
  if (n_params >= ARRAY_SIZE(regs_param_gp)) return false;
  ...
  ir_type *const new_mtp =
      new_type_method(new_n_params, n_ress, true, cc_mask, props);
...
static void riscv_set_allocatable_regs(ir_graph *const irg) {
  ...
  be_cconv_rem_regs(birg->allocatable_regs, ignore_regs,
                    ARRAY_SIZE(ignore_regs));
}
...
void riscv_transform_graph(ir_graph *const irg) {
  ...
  unsigned named_params = is_method_variadic(fun_type)
                              ? (unsigned)get_method_variadic_index(fun_type)
