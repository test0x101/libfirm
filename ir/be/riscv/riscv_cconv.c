...
void riscv_determine_calling_convention(riscv_calling_convention_t *const cconv,
                                        ir_type *const fun_type,
                                        unsigned named_parameters,
                                        ir_graph *const irg) {
  ...
  if (n_params != 0) {
    ...
    for (size_t i = 0; i != n_params; ++i) {
      ...
      if (!param_mode) {
        ...
        if (i >= named_parameters && param_type->flags & tf_lowered_dw &&
            gp_param % 2 != 0) {
          // variadic arguments with 2 x XLEN size have to be passed in an
          // aligned register pair / stack slots
          ++gp_param;
        }
  ...
  unsigned n_mem_param =
      gp_param > ARRAY_SIZE(regs_param_gp) ? gp_param - offset : 0;
  ...
  if (n_results != 0) {
    ...
    for (size_t i = 0; i != n_results; ++i) {
      ...
      if (!res_mode) {
        ...
        if (gp_res == ARRAY_SIZE(regs_result_gp)) panic("too many gp results");
  ...
  if (irg != NULL) {
    ...
    if (!omit_fp) rbitset_clear(birg->allocatable_regs, REG_S0);
  }
  cconv->omit_fp = omit_fp;
}

void riscv_layout_parameter_entities(riscv_calling_convention_t *const cconv,
                                     ir_graph *const irg) {
  ...
  for (size_t i = 0; i != n_params; ++i) {
    ...
    ir_type *const param_type = get_method_param_type(fun_type, i);
    if (!is_atomic_type(param_type)) panic("unhandled parameter type");
  ...
  if (is_method_variadic(fun_type)) {
    int const offset =
        (cconv->va_arg_first_slot - RISCV_N_PARAM_REGS) * RISCV_REGISTER_SIZE;
