...
#define be_ifg_foreach_neighbour(ifg, iter, irn, pos)               \
  for (ir_node *pos = be_ifg_neighbours_begin(ifg, iter, irn); pos; \
       pos = be_ifg_neighbours_next(iter))

#define be_ifg_foreach_node(ifg, pos)                                     \
  for (bool pos##__once = true; pos##__once;)                             \
    for (nodes_iter_t pos##__iter = be_ifg_nodes_begin(ifg); pos##__once; \
         pos##__once = false)                                             \
      for (ir_node * pos; (pos = be_ifg_nodes_next(&pos##__iter));)

#define be_ifg_foreach_clique(ifg, iter, buf, count)                    \
  for (*(count) = be_ifg_cliques_begin(ifg, iter, buf); *(count) != -1; \
