...
typedef struct {
  ...
  unsigned long long
      aff_nodes; /**< number of nodes with incident affinity edges. */
  unsigned long long
      aff_int; /**< number of affinity edges whose nodes also interfere. */
  unsigned long long inevit_costs; /**< costs which cannot be evited (due to
                                      interfering affinities). */
  unsigned long long max_costs;    /**< all costs of the affinities. */
  unsigned long long costs;        /**< The costs of the current coloring. */
  unsigned long long
      unsatisfied_edges; /**< The number of unequally colored affinity edges. */
} co_complete_stats_t;
...
static const lc_opt_enum_mask_items_t dump_items[] = {{"before", DUMP_BEFORE},
                                                      {"after", DUMP_AFTER},
                                                      {"appel", DUMP_APPEL},
                                                      {"all", DUMP_ALL},
                                                      {NULL, 0}};
...
static const lc_opt_enum_mask_items_t style_items[] = {
    {"color", CO_IFG_DUMP_COLORS},       {"labels", CO_IFG_DUMP_LABELS},
    {"constr", CO_IFG_DUMP_CONSTR},      {"shape", CO_IFG_DUMP_SHAPE},
    {"full", 2 * CO_IFG_DUMP_SHAPE - 1}, {NULL, 0}};
...
static lc_opt_enum_mask_var_t dump_var = {&dump_flags, dump_items};

static lc_opt_enum_mask_var_t style_var = {&style_flags, style_items};

static lc_opt_enum_func_ptr_var_t cost_func_var = {(opt_funcptr *)&cost_func,
                                                   cost_func_items};

static const lc_opt_table_entry_t options[] = {
    LC_OPT_ENT_ENUM_FUNC_PTR("cost", "select a cost function", &cost_func_var),
    LC_OPT_ENT_ENUM_MASK("dump", "dump ifg before or after copy optimization",
                         &dump_var),
    LC_OPT_ENT_ENUM_MASK("style", "dump style for ifg dumping", &style_var),
    LC_OPT_ENT_BOOL("stats", "dump statistics after each optimization",
                    &do_stats),
    LC_OPT_LAST};
...
void be_register_copyopt(const char *name, co_algo_info *copyopt) {
  if (selected_copyopt == NULL) selected_copyopt = copyopt;
...
void be_init_copynone(void) {
  static co_algo_info copyheur = {void_algo};

  be_register_copyopt("none", &copyheur);
}

static copy_opt_t *new_copy_opt(be_chordal_env_t *chordal_env,
                                cost_fct_t get_costs) {
...
static void free_copy_opt(copy_opt_t *co) { free(co); }
...
static bool co_is_optimizable_root(const ir_node *irn) {
  ...
  if (req->ignore) return false;

  if (is_Phi(irn) || is_Perm_Proj(irn)) return true;

  if (req->should_be_same != 0) return true;
...
static int co_get_costs_loop_depth(const ir_node *root, int pos) {
  ...
  if (is_Phi(root)) block = get_Block_cfgpred_block(block, pos);
...
static int ou_max_ind_set_costs(unit_t *const ou) {
  ...
  if (unsafe_count > MIS_HEUR_TRIGGER) {
    ...
    for (int i = 0; i < unsafe_count; ++i) {
      ...
      for (int o = bitset_next_set(best, 0); o != -1 && o < i;
           o = bitset_next_set(best, o + 1))
    ...
    /* compute the weight */
    bitset_foreach(best, pos) best_weight += unsafe_costs[pos];
  } else {
    ...
    while (bitset_popcount(curr) != 0) {
      ...
      for (int i = bitset_next_set(curr, 0); i != -1;
           i = bitset_next_set(curr, i + 1))
        for (int o = bitset_next_set(curr, i + 1); o != -1;
             o = bitset_next_set(
                 curr, o + 1)) /* !!!!! difference to qnode_max_ind_set(): NOT
                                  (curr, i) */
          if (be_values_interfere(unsafe[i], unsafe[o])) goto no_stable_set;

      /* if we arrive here, we have a stable set */
      /* compute the weight of the stable set*/
      int curr_weight = 0;
      bitset_foreach(curr, pos) curr_weight += unsafe_costs[pos];
...
static void co_collect_units(ir_node *irn, void *env) {
  if (get_irn_mode(irn) == mode_T) return;
  copy_opt_t *co = (copy_opt_t *)env;
  const arch_register_req_t *req = arch_get_irn_register_req(irn);
  if (req->cls != co->cls) return;
  if (!co_is_optimizable_root(irn)) return;
  ...
  if (is_Phi(irn)) {
    ...
    foreach_irn_in(irn, i, arg) {
      assert(arch_get_irn_register_req(arg)->cls == co->cls &&
      ...
      if (arch_irn_is_ignore(arg)) continue;
    ...
    for (int i = 0; (1U << i) <= other; ++i) {
      if (other & (1U << i)) {
        ...
        if (arch_irn_is_ignore(o)) continue;
        if (be_values_interfere(irn, o)) continue;
        ++count;
      }
    }
    ...
    if (count != 0) {
      ...
      for (int i = 0; 1U << i <= other; ++i) {
        if (other & (1U << i)) {
          ir_node *o = get_irn_n(skip_Proj(irn), i);
          if (!arch_irn_is_ignore(o) && !be_values_interfere(irn, o)) {
            unit->nodes[k] = o;
...
int co_get_inevit_copy_costs(const copy_opt_t *co) {
  ...
  int res = 0;
  list_for_each_entry(unit_t, curr, &co->units, units) res +=
...
int co_get_lower_bound(const copy_opt_t *co) {
  ...
  int res = 0;
  list_for_each_entry(unit_t, curr, &co->units, units) res +=
      curr->inevitable_costs + curr->min_nodes_costs;
  return res;
}
  ...
  co_gs_foreach_aff_node(co, an) {
    ...
    co_gs_foreach_neighb(an, neigh) {
      if (!bitset_is_set(seen, get_irn_idx(neigh->irn))) {
        ...
        if (arch_get_irn_register(an->irn) !=
            arch_get_irn_register(neigh->irn)) {
...
static int compare_affinity_node_t(const void *k1, const void *k2,
                                   size_t size) {
  (void)size;
...
static inline void add_edges(copy_opt_t *co, ir_node *n1, ir_node *n2,
                             int costs) {
...
static void build_graph_walker(ir_node *irn, void *env) {
  ...
  copy_opt_t *co = (copy_opt_t *)env;
  const arch_register_req_t *req = arch_get_irn_register_req(irn);
  if (req->cls != co->cls || req->ignore) return;
...
static bool co_dump_appel_disjoint_constraints(ir_node *const a,
                                               ir_node *const b) {
  arch_register_req_t const *const reqa = arch_get_irn_register_req(a);
  if (reqa->limited == NULL) return false;

  arch_register_req_t const *const reqb = arch_get_irn_register_req(b);
  if (reqb->limited == NULL) return false;

  return !rbitsets_have_common(reqa->limited, reqb->limited, reqa->cls->n_regs);
}

/**
 * Dump the interference graph according to the Appel/George coalescing contest
 * file format. See: http://www.cs.princeton.edu/~appel/coalesce/format.html
...
static void co_dump_appel_graph(const copy_opt_t *co, FILE *f) {
  ...
  be_ifg_foreach_node(ifg, irn) {
    if (arch_irn_is_ignore(irn)) continue;
  ...
  be_ifg_foreach_node(ifg, irn) {
    ...
    be_ifg_foreach_neighbour(ifg, &nit, irn, adj) {
      if (!arch_irn_is_ignore(adj) &&
        ...
        if (idx < adj_idx) fprintf(f, "%d %d -1\n", idx, adj_idx);
      }
    }
...
void co_driver(be_chordal_env_t *cenv) {
  ...
  if (selected_copyopt->copyopt == void_algo) return;
  ...
  if (dump_flags & DUMP_BEFORE) {
    ...
    be_dump_ifg_co(f, co, style_flags & CO_IFG_DUMP_LABELS,
                   style_flags & CO_IFG_DUMP_COLORS);
    fclose(f);
  }
  ...
  if (do_stats) {
    ...
    printf("%10s %10llu%10llu%10llu", cenv->cls->name, after.max_costs,
           before.costs, after.inevit_costs);

    if (optimizable_costs > 0)
      printf("%10llu %5.2f\n", after.costs,
             (evitable * 100.0) / optimizable_costs);
    else
      printf("%10llu %5s\n", after.costs, "-");
  }
