...
static const lc_opt_table_entry_t be_spill_options[] = {
    LC_OPT_ENT_BOOL("coalesce_slots", "coalesce the spill slots",
                    &be_coalesce_spill_slots),
    LC_OPT_ENT_BOOL("remat", "try to rematerialize values instead of reloading",
                    &be_do_remats),
    LC_OPT_LAST};
...
void be_register_spiller(const char *name, be_spill_func spiller) {
  if (selected_spiller == NULL) selected_spiller = spiller;
...
void be_init_spilloptions(void) {
  ...
  be_add_module_list_opt(spill_grp, "algo", "spill algorithm", &spillers,
                         (void **)&selected_spiller);
}
