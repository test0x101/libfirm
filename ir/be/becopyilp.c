...
static const lc_opt_enum_mask_items_t dump_items[] = {{"ilp", DUMP_ILP},
                                                      {NULL, 0}};

static lc_opt_enum_mask_var_t dump_var = {&dump_flags, dump_items};

static const lc_opt_table_entry_t options[] = {
    LC_OPT_ENT_INT("limit",
                   "time limit for solving in seconds (0 for unlimited)",
                   &time_limit),
    LC_OPT_ENT_BOOL("log", "show ilp solving log", &solve_log),
    LC_OPT_ENT_ENUM_MASK("dump", "dump flags", &dump_var), LC_OPT_LAST};
...
/**
 * Checks if a node is simplicial in the graph heeding the already removed
 * nodes.
 */
static inline bool sr_is_simplicial(ilp_env_t *const ienv,
                                    ir_node const *const ifn) {
  bool res = true;
  ...
  be_ifg_foreach_neighbour(ienv->co->cenv->ifg, &iter, ifn, curr) {
    ...
    if (sr_is_removed(ienv, curr)) continue;
...
static void sr_remove(ilp_env_t *const ienv) {
  ...
  while (redo) {
    ...
    be_ifg_foreach_node(ifg, irn) {
      ...
      if (req->limited || sr_is_removed(ienv, irn)) continue;
      if (co_gs_is_optimizable(ienv->co, irn)) continue;
      if (!sr_is_simplicial(ienv, irn)) continue;
...
static void sr_reinsert(ilp_env_t *const ienv) {
  ...
  unsigned const *const allocatable_cols =
  ...
  for (size_t i = ARR_LEN(ienv->col_suff); i-- != 0;) {
    ...
    be_ifg_foreach_neighbour(ifg, &iter, irn, other) {
      ...
      if (sr_is_removed(ienv, other)) continue;
    ...
    assert(!rbitset_is_empty(possible_cols, n_regs) &&
           "No free color found. This can not be.");
    unsigned free_col = 0;
    for (;;) {
      free_col = (unsigned)rbitset_next(possible_cols, free_col, true);
      if (free_col % arch_get_irn_register_req(irn)->width == 0) break;
      ++free_col;
...
ilp_env_t *new_ilp_env(copy_opt_t *const co, ilp_callback const build,
                       ilp_callback const apply, void *const env) {
...
lpp_sol_state_t ilp_go(ilp_env_t *const ienv) {
  ...
  if (dump_flags & DUMP_ILP) {
    char buf[128];
    ir_snprintf(buf, sizeof(buf), "%F_%s-co.ilp", ienv->co->irg,
                ienv->co->cenv->cls->name);
    FILE *const f = fopen(buf, "wt");
    if (!f) panic("couldn't open '%s' for writing", buf);
    lpp_dump_plain(ienv->lp, f);
    fclose(f);
  }

  lpp_set_time_limit(ienv->lp, time_limit);
  if (solve_log) lpp_set_log(ienv->lp, stdout);
