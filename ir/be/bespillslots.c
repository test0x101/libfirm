...
static inline ir_node *get_memory_edge(const ir_node *node) {
  foreach_irn_in_r(node, i, arg) {
    if (get_irn_mode(arg) == mode_M) return arg;
  }
...
static spill_t *collect_spill(be_fec_env_t *env, ir_node *node,
  ...
  if (rbitset_is_set(env->spills_set, idx)) {
    ...
    if (new_web == NULL) {
      new_web = web;
      if (new_web == NULL) new_web = OALLOCZ(&env->obst, spillweb_t);
    } else {
  ...
  DB((dbg, LEVEL_1, "Slot %d: %+F (%+F)\n", spill->spillslot, skip_Proj(node),
      node));
  ...
  if (is_Phi(node)) {
    foreach_irn_in(node, i, arg) {
      ...
      web = arg_spill->web;
      assert(web->merged_with == NULL);
      assert(web == old_web || old_web == NULL || old_web->merged_with != NULL);
      spill->web = web;
    }
  } else if (web == NULL) {
...
static int merge_interferences(be_fec_env_t *env, bitset_t **interferences,
  ...
  for (size_t i = 0, n = ARR_LEN(env->spills); i < n; ++i) {
    ...
    if (bitset_is_set(intfs, s2)) bitset_set(intfs, s1);
  }
...
static void do_greedy_coalescing(be_fec_env_t *env) {
  ...
  for (size_t i = 0, n = ARR_LEN(env->affinity_edges); i < n; ++i) {
    ...
    DB((dbg, LEVEL_1, "Merging %d and %d because of affinity edge\n", s1, s2));
  ...
  for (size_t i = 0; i < spillcount; ++i) {
    ...
    if (s1 != (int)i) continue;

    for (size_t i2 = i + 1; i2 < spillcount; ++i2) {
      int s2 = uf_find(spillslot_unionfind, i2);
      if (s2 != (int)i2) continue;
      ...
      DB((dbg, LEVEL_1, "Merging %d and %d because it is possible\n", s1, s2));

      if (merge_interferences(env, interferences, spillslot_unionfind, s1,
                              s2) != 0) {
      ...
      foreach_irn_in(node, i, arg) {
        ...
        if (slotid != argslotid) {
          ...
          if (argslot->entity == NULL)
            argslot->entity =
                new_spillslot(frame, argslot->size, argslot->po2align);

          memperm_t *const memperm = get_memperm(env, predblock);
          memperm_entry_t *const entry = OALLOC(&env->obst, memperm_entry_t);
          entry->node = node;
          entry->pos = i;
...
void be_assign_entities(be_fec_env_t *env,
  ...
  if (stat_ev_enabled) stat_ev_dbl("spillslots", ARR_LEN(env->spills));
  ...
  if (be_coalesce_spill_slots &&
      !be_birg_from_irg(env->irg)->has_returns_twice_call)
...
void be_init_spillslots(void) { FIRM_DBG_REGISTER(dbg, "firm.be.spillslots"); }
