...
#define ASSERT_OU_AVAIL(co) \
  assert((co)->units.next && "Representation as optimization-units not built")
#define ASSERT_GS_AVAIL(co) \
  assert((co)->nodes && "Representation as graph not built")
...
typedef struct unit_t {
  ...
  ir_node **nodes; /**< [0] is the root-node, others are non interfering args of
                      it. */
  ...
  int min_nodes_costs;  /**< a lower bound for the costs in costs[], determined
                           by a max independent set */
  int sort_key; /**< maximum costs. controls the order of ou's in the struct
                   list_head units. */
...
static inline affinity_node_t *get_affinity_info(const copy_opt_t *co,
                                                 const ir_node *irn) {
  ASSERT_GS_AVAIL(co);

  affinity_node_t find;
  find.irn = irn;
  return set_find(affinity_node_t, co->nodes, &find, sizeof(find),
                  hash_irn(irn));
}

#define co_gs_foreach_aff_node(co, aff_node) \
  foreach_set((co)->nodes, affinity_node_t, (aff_node))
#define co_gs_foreach_neighb(aff_node, neighb) \
  for (neighb_t *neighb = aff_node->neighbours; neighb; neighb = neighb->next)
