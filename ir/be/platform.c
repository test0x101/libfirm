...
static void init_lc_opts(void) {
  static const lc_opt_table_entry_t platform_options[] = {
      LC_OPT_ENT_BOOL("pic", "Generate position independent code", &option_pic),
...
static void ppdef1(const char *name) { ppdef(name, "1"); }
...
void ir_platform_set(ir_machine_triple_t const *machine,
  ...
  if (ir_is_cpu_x86_32(cpu)) {
    ...
    ppdef1("__arm__");
    if (strstr(os, "eabi") != NULL) ppdef1("__ARM_EABI__");
  ...
  if (strstr(os, "linux") != NULL) {
    ...
    ppdef1("__linux__");
    ppdef1("linux");
    if (strstr(os, "gnu") != NULL) ppdef1("__gnu_linux__");
...
unsigned ir_platform_type_size(ir_platform_type_t type) {
  switch (type) {
    ...
    case IR_TYPE_LONG_DOUBLE:
      return ir_platform.long_double_size;
  }
  panic("Invalid object type");
}

unsigned ir_platform_type_align(ir_platform_type_t type) {
  if (type == IR_TYPE_LONG_DOUBLE) return ir_platform.long_double_align;
...
ir_type *ir_platform_va_list_type(void) { return ir_platform.va_list_type; }
...
static ir_mode *make_int_mode(ir_platform_type_t type, bool is_signed) {
  ...
  if (modulo_shift > 0) modulo_shift = MAX(bitsize, modulo_shift);
