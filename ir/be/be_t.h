...
struct be_main_env_t {
  ...
  pmap
      *ent_trampoline_map; /**< A map containing PIC trampolines for methods. */
  ir_type *pic_trampolines_type; /**< Class type containing all trampolines */
  pmap *ent_pic_symbol_map;
  ir_type *pic_symbols_type;
};

void be_set_constraint_support(asm_constraint_flags_t flags,
                               char const *constraints);

void be_get_allocatable_regs(ir_graph const *irg,
...
static inline void be_dump(be_dump_flags_t const mask, ir_graph *const irg,
  ...
  if (be_options.dump_flags & mask) dump_ir_graph(irg, suffix);
}
