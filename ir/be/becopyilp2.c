 ...
 *     x_nc                  =  0           n \in N, c \not\in C(n)
 *
 *     \sum x_nc            <=  1           x_nc \in Clique \in AllCliques,  c
 * \in C
 *
...
static void make_color_var_name(char *buf, size_t buf_size, const ir_node *node,
                                unsigned color) {
...
static void build_coloring_cstr(ilp_env_t *ienv) {
  ...
  unsigned const *const allocatable_colors =
      ienv->co->cenv->allocatable_regs->data;

  unsigned *const colors = rbitset_alloca(n_regs);
  be_ifg_foreach_node(ifg, irn) {
    if (sr_is_removed(ienv, irn)) continue;
    ...
    for (unsigned col = 0; col < n_regs; ++col) {
      ...
      if (lenv->first_x_var == -1) lenv->first_x_var = var_idx;
    }
...
static void build_interference_cstr(ilp_env_t *const ienv) {
  ...
  be_ifg_foreach_clique(ifg, &iter, clique, &size) {
    ...
    for (int i = 0; i < size; ++i) {
      if (!sr_is_removed(ienv, clique[i])) ++realsize;
    }

    if (realsize < 2) continue;

    /* for all colors */
    for (unsigned col = 0; col < n_colors; ++col) {
      if (!rbitset_is_set(allocatable_colors, col)) continue;
...
static void init_edge(edge_t *const edge, ir_node *const n1,
                      ir_node *const n2) {
  if (n1 < n2) {
    edge->n1 = n1;
    edge->n2 = n2;
...
static inline edge_t *add_edge(set *edges, ir_node *n1, ir_node *n2,
                               size_t *counter) {
  edge_t new_edge;
  init_edge(&new_edge, n1, n2);

  ++*counter;
  return set_insert(edge_t, edges, &new_edge, sizeof(new_edge),
                    HASH_EDGE(&new_edge));
}
...
static inline edge_t *find_edge(set *edges, ir_node *n1, ir_node *n2) {
  ...
  return set_find(edge_t, edges, &new_edge, sizeof(new_edge),
                  HASH_EDGE(&new_edge));
}

static inline void remove_edge(set *edges, ir_node *n1, ir_node *n2,
                               size_t *counter) {
  edge_t new_edge;
  init_edge(&new_edge, n1, n2);

  edge_t *const e = set_find(edge_t, edges, &new_edge, sizeof(new_edge),
                             HASH_EDGE(&new_edge));
  if (e) {
    e->n1 = NULL;
    e->n2 = NULL;
...
static void build_clique_star_cstr(ilp_env_t *ienv) {
  ...
  co_gs_foreach_aff_node(ienv->co, aff) {
    ...
    if (arch_irn_is_ignore(center)) continue;
    ...
    while (n_edges) {
      ...
      do {
        ...
        for (int i = 0; i < n_nodes; ++i) {
          ...
          if (pset_find_ptr(clique, cand)) continue;
      ...
      {
        int const cst_idx = lpp_add_cst(ienv->lp, NULL, lpp_greater_equal,
                                        pset_count(clique) - 1);
        pset_foreach(clique, member) {
          char buf[32];
...
static void extend_path(ilp_env_t *ienv, deq_t *path, const ir_node *irn) {
  ...
  if (path_contains(path, irn)) return;

  if (arch_irn_is_ignore(irn)) return;
  ...
  unsigned i = 0;
  deq_foreach_pointer(path, ir_node, n) { curr_path[i++] = n; }

  for (int i = 1; i < len; ++i) {
    if (be_values_interfere(irn, curr_path[i])) goto end;
  }
  ...
  if (be_values_interfere(irn, curr_path[0])) {
    ...
    if (len > 2) {
      ...
      for (int i = 1; i < len; ++i) {
        ...
        make_affinity_var_name(buf, sizeof(buf), curr_path[i - 1],
                               curr_path[i]);
  ...
  co_gs_foreach_neighb(aff, nbr) { extend_path(ienv, path, nbr->irn); }
...
static void ilp2_build(ilp_env_t *ienv) {
  ...
  int const lower_bound =
      co_get_lower_bound(ienv->co) - co_get_inevit_copy_costs(ienv->co);
...
static void ilp2_apply(ilp_env_t *const ienv) {
  ...
  if (lenv->first_x_var >= 0) {
    ...
    lpp_sol_state_t const state =
        lpp_get_solution(ienv->lp, sol, lenv->first_x_var, lenv->last_x_var);

    if (state != lpp_optimal) {
      ir_printf(
          "WARNING: Solution state of %F register class %s is not 'optimal': "
          "%d\n",
          irg, ienv->co->cls->name, (int)state);
      if (state < lpp_feasible) panic("copy coalescing solution not feasible");
    }

    for (int i = 0; i < count; ++i) {
      if (sol[i] <= 1 - EPSILON) continue;
      /* split variable name into components */
      char var_name[32];
      lpp_get_var_name(ienv->lp, lenv->first_x_var + i, var_name,
                       sizeof(var_name));
...
void be_init_copyilp2(void) {
  static co_algo_info copyheur = {co_solve_ilp2};
