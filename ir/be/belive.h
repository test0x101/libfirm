 ...
 * The new irn is not deleted from any block's liveness information, so it must
 * be fresh!
 ...
 * @param live     The set of nodes live before @p irn. This set gets modified
 * by updating it to the nodes live after irn.
 * @return live.
 */
...
static inline be_lv_state_t be_get_live_state(be_lv_t const *const li,
                                              ir_node const *const block,
...
static inline ir_node *be_lv_iteration_next(lv_iterator_t *iterator,
  ...
  while (iterator->i != 0) {
    ...
    if (node->flags & flags) return node->node;
  }
  return NULL;
}

static inline ir_node *be_lv_iteration_cls_next(
    lv_iterator_t *iterator, be_lv_state_t flags,
    const arch_register_class_t *cls) {
  while (iterator->i != 0) {
    be_lv_info_node_t const *const lnode =
        &iterator->info->nodes[--iterator->i];
    assert(get_irn_mode(lnode->node) != mode_T);
    if (!(lnode->flags & flags)) continue;
...
#define be_lv_foreach(lv, block, flags, node)                             \
  for (bool once = true; once;)                                           \
    for (lv_iterator_t iter = be_lv_iteration_begin((lv), (block)); once; \
         once = false)                                                    \
      for (ir_node * node;                                                \
           (node = be_lv_iteration_next(&iter, (flags))) != NULL;)
...
#define be_lv_foreach_cls(lv, block, flags, cls, node)                    \
  for (bool once = true; once;)                                           \
    for (lv_iterator_t iter = be_lv_iteration_begin((lv), (block)); once; \
         once = false)                                                    \
      for (ir_node * node;                                                \
           (node = be_lv_iteration_cls_next(&iter, (flags), (cls))) != NULL;)
