...
static inline void set_out_info_reg(reg_out_info_t *const out,
...
bool arch_reg_is_allocatable(const arch_register_req_t *req,
                             const arch_register_t *reg) {
  if (req->cls != reg->cls) return false;
  if (reg->is_virtual) return true;
  if (req->limited != NULL) return rbitset_is_set(req->limited, reg->index);
  return true;
}
...
arch_register_t const *arch_find_register(char const *const name) {
  ...
  for (size_t i = 0, n = ir_target.isa->n_registers; i < n; ++i) {
    ...
    if (streq(reg->name, name)) return reg;
  }
  return NULL;
}
...
be_add_pressure_t arch_get_additional_pressure(
    ir_node const *const node, arch_register_class_t const *const cls) {
  backend_info_t *const info = be_get_info(node);
  /* restricted to the first few classes for now */
  unsigned index = cls->index;
  if (index < ARRAY_SIZE(info->add_pressure)) return info->add_pressure[index];
  return 0;
}

void arch_copy_irn_out_info(ir_node *const dst, unsigned const dst_pos,
                            ir_node const *const src) {
  reg_out_info_t *const src_info = get_out_info(src);
  reg_out_info_t *const dst_info = get_out_info_n(dst, dst_pos);
  *dst_info = *src_info;
}

arch_register_req_t *be_create_cls_req(ir_graph *const irg,
                                       arch_register_class_t const *const cls,
                                       unsigned char const width) {
...
arch_register_req_t const *be_create_reg_req(ir_graph *const irg,
                                             arch_register_t const *const reg,
                                             bool const ignore) {
  if (!ignore) return reg->single_req;
...
int be_get_input_pos_for_req(ir_node const *const irn,
                             arch_register_req_t const *const req) {
  for (int i = 0, n = get_irn_arity(irn); i != n; ++i) {
    if (arch_get_irn_register_req_in(irn, i) == req) return i;
  }
  return -1;
}
