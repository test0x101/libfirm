...
static void print_living_values(FILE *F, const ir_nodeset_t *live_nodes) {
  ir_fprintf(F, "\t");
  foreach_ir_nodeset(live_nodes, node, iter) { ir_fprintf(F, "%+F ", node); }
  ir_fprintf(F, "\n");
}
...
/**
 * Check if number of live nodes never exceeds the number of available
 * registers.
 */
static void verify_liveness_walker(ir_node *block, void *data) {
  be_verify_register_pressure_env_t *env =
      (be_verify_register_pressure_env_t *)data;
  ir_nodeset_t live_nodes;

  /* collect register pressure info, start with end of a block */
  ir_nodeset_init(&live_nodes);
  be_liveness_end_of_block(env->lv, env->cls, block, &live_nodes);
  ...
  sched_foreach_non_phi_reverse(block, irn) {
    ...
    if (pressure > env->registers_available) {
      verify_warnf(block, "register pressure too high before %+F (%d/%d):", irn,
                   pressure, env->registers_available);
      print_living_values(stderr, &live_nodes);
      env->problem_found = true;
    }
...
static void verify_schedule_walker(ir_node *block, void *data) {
  ...
  sched_foreach(block, node) {
    ...
    if (!is_Phi(node)) {
      ...
      verify_warnf(block, "%+F scheduled after non-Phi %+F", node,
                   non_phi_found);
      env->problem_found = true;
    }
    ...
    if (is_cfop(node)) {
      ...
      if (cfchange_found != NULL) {
        verify_warnf(
            block,
            "additional control flow changing node %+F scheduled after %+F",
            node, cfchange_found);
        env->problem_found = true;
      } else {
        cfchange_found = node;
      ...
      if (!be_is_Keep(node)) {
        verify_warnf(block, "%+F scheduled after control flow changing node",
                     node);
        env->problem_found = true;
      }
    }
    ...
    if (!is_Phi(node)) {
      ...
      foreach_irn_in(node, i, arg) {
        if (get_nodes_block(arg) != block || !sched_is_scheduled(arg)) continue;

        if (sched_get_time_step(arg) >= nodetime) {
          verify_warnf(block, "%+F used by %+F before it was defined", arg,
                       node);
          env->problem_found = true;
        }
      }
    ...
    if (be_is_Keep(node) || be_is_CopyKeep(node)) {
      ...
      while (be_is_Keep(prev) || be_is_CopyKeep(prev)) prev = sched_prev(prev);

      do {
        foreach_irn_in(node, i, in) {
          if (skip_Proj(in) == prev) goto ok;
        }
...
static void check_schedule(ir_node *node, void *data) {
  ...
  if (should_be != scheduled) {
    verify_warnf(node, "%+F should%s be scheduled", node,
                 should_be ? "" : " not");
    env->problem_found = true;
  }
}
...
static void check_output_constraints(be_verify_reg_alloc_env_t *const env,
                                     const ir_node *node) {
  arch_register_req_t const *const req = arch_get_irn_register_req(node);
  if (!req->cls->regs) return;
  ...
  if (reg == NULL) {
    ...
    verify_warnf(node,
                 "register %s assigned as output of %+F not allowed (register "
                 "constraint)",
                 reg->name, node);
    env->problem_found = true;
  }
...
static void check_input_constraints(be_verify_reg_alloc_env_t *const env,
                                    ir_node *const node) {
  ...
  foreach_irn_in(node, i, pred) {
    ...
    if (!req->cls->regs) continue;

    if (req->width > pred_req->width) {
      verify_warnf(node, "%+F register width of value at input %d too small",
                   node, i);
      env->problem_found = true;
    }
    ...
    if (reg == NULL) {
      verify_warnf(pred,
                   "%+F should have a register assigned (%+F input constraint)",
                   pred, node);
      env->problem_found = true;
    } else if (!arch_reg_is_allocatable(req, reg)) {
      verify_warnf(
          node,
          "register %s as input %d of %+F not allowed (register constraint)",
          reg->name, i, node);
      env->problem_found = true;
    }
  }
  ...
  if (is_Phi(node)) {
    ...
    foreach_irn_in(node, i, pred) {
      ...
      if (reg != pred_reg && !(pred_reg->is_virtual)) {
        ...
        verify_warnf(node, "input %d of %+F uses register %s instead of %s", i,
                     node, pred_name, reg_name);
        env->problem_found = true;
      }
    }
  }
...
static bool ignore_error_for_reg(ir_graph *irg, const arch_register_t *reg) {
  ...
  if (birg->non_ssa_regs == NULL) return false;
  return rbitset_is_set(birg->non_ssa_regs, reg->global_index);
}

static void value_used(be_verify_reg_alloc_env_t *const env,
                       ir_node const **const registers,
                       ir_node const *const block, ir_node const *const node) {
  const arch_register_t *reg = arch_get_irn_register(node);
  if (reg == NULL || reg->is_virtual) return;
  ...
  for (unsigned i = 0; i < req->width; ++i) {
    ...
    if (reg_node != NULL && reg_node != node &&
      ...
      verify_warnf(block,
                   "register %s assigned more than once (nodes %+F and %+F)",
                   reg->name, node, reg_node);
      env->problem_found = true;
    }
...
static void value_def(be_verify_reg_alloc_env_t *const env,
                      ir_node const **const registers,
  ...
  for (unsigned i = 0; i < req->width; ++i) {
    ...
    if (reg_node == NULL && get_irn_n_edges(node) == 0) return;

    if (reg_node != node && !ignore_error_for_reg(get_irn_irg(node), reg)) {
      verify_warnf(node,
                   "%+F not registered as value for register %s (but %+F)",
                   node, reg->name, reg_node);
      env->problem_found = true;
    }
...
static void verify_block_register_allocation(ir_node *block, void *data) {
  ...
  sched_foreach_reverse(block, node) {
    be_foreach_value(node, value, value_def(env, registers, value);
    ...
    if (!is_Phi(node)) {
      foreach_irn_in(node, i, use) { value_used(env, registers, block, use); }
    }
  }
...
static const char *lv_flags_to_str(unsigned flags) {
  static const char *states[] = {"---", "i--", "-e-", "ie-",
                                 "--o", "i-o", "-eo", "ieo"};
...
static void lv_check_walker(ir_node *bl, void *data) {
  ...
  be_lv_info_t *const curr =
      ir_nodehashmap_get(be_lv_info_t, &w->given->map, bl);
  be_lv_info_t *const fresh =
      ir_nodehashmap_get(be_lv_info_t, &w->fresh->map, bl);
