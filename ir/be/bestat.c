...
static void check_reg_pressure_class(pressure_walker_env_t *env, ir_node *block,
                                     const arch_register_class_t *cls) {
  ...
  if (max_live > env->max_pressure) env->max_pressure = max_live;
...
static void block_count_values(ir_node *block, void *data) {
  ...
  sched_foreach(block, node) {
    be_foreach_value(
        ...
        if (!req->cls->regs) continue; ++stats->values;
        if (req->should_be_same != 0 || is_Phi(value))++ stats->should_be_sames;
        if (req->limited != NULL)++ stats->constrained_values;);
    for (int i = 0, arity = get_irn_arity(node); i < arity; ++i) {
      ...
      if (be_is_Keep(node)) {
        ...
        if (get_irn_n_edges(value) <= 1) {
          ...
          if (req->limited != NULL) ++stats->unused_constrained_values;
        }
      }
      if (req->limited != NULL) ++stats->constrained_uses;
    }
  }
}
