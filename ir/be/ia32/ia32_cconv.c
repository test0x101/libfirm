...
static const unsigned caller_saves_fp[] = {
    REG_FPCW, REG_ST0,  REG_ST1,  REG_ST2,  REG_ST3,  REG_ST4,
    REG_ST5,  REG_ST6,  REG_ST7,  REG_XMM0, REG_XMM1, REG_XMM2,
    REG_XMM3, REG_XMM4, REG_XMM5, REG_XMM6, REG_XMM7,
};
...
x86_cconv_t *ia32_decide_calling_convention(ir_type const *const function_type,
  ...
  for (unsigned i = 0; i < n_params; ++i) {
    ...
    align_stack:;
      ...
      if (misalign > 0) stack_offset += IA32_REGISTER_SIZE - misalign;
    }
  }
  ...
  cconv->sp_delta =
      (cc & cc_compound_ret) && !(cc & cc_reg_param) ? IA32_REGISTER_SIZE : 0;
  ...
  if (irg != NULL) {
    ...
    birg->allocatable_regs =
        be_cconv_alloc_all_regs(&birg->obst, N_IA32_REGISTERS);
    be_cconv_rem_regs(birg->allocatable_regs, ignore_regs,
                      ARRAY_SIZE(ignore_regs));
    if (!omit_fp) rbitset_clear(birg->allocatable_regs, REG_EBP);
  }
...
void ia32_cconv_init(void) {
  be_cconv_add_regs(default_caller_saves, caller_saves_gp,
                    ARRAY_SIZE(caller_saves_gp));
  be_cconv_add_regs(default_callee_saves, callee_saves,
                    ARRAY_SIZE(callee_saves));
  if (!ia32_cg_config.use_softfloat) {
    be_cconv_add_regs(default_caller_saves, caller_saves_fp,
                      ARRAY_SIZE(caller_saves_fp));
