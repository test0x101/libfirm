...
static bool eat_imm(x86_address_t *const addr, ir_node const *const node,
  ...
  switch (get_irn_opcode(node)) {
    case iro_Add:
      ...
      return !x86_is_non_address_mode_node(node) &&
    ...
    case iro_Address:
      ...
      if (addr->imm.entity) return false;
      addr->imm.entity = get_Address_entity(node);
      addr->imm.kind = X86_IMM_ADDR;
      if (is_tls_entity(addr->imm.entity)) addr->tls_segment = true;
      return true;

    case iro_Const: {
      /* Use the value for the offset. */
      if (addr->imm.offset != 0) return false;
      ir_tarval *const tv = get_Const_tarval(node);
      if (!tarval_possible(tv)) return false;
    ...
    default:
      if (be_is_Relocation(node)) {
        ...
        if (kind == X86_IMM_GOTPCREL || kind == X86_IMM_PCREL) {
          if (!basereg_usable) return false;
          addr->ip_base = true;
        }
        return true;
      }
...
static ir_node *eat_immediates(x86_address_t *addr, ir_node *node,
  ...
  if (is_Add(node)) {
    ...
    if (eat_immediate(addr, left, basereg_usable))
      return eat_immediates(addr, right, x86_create_am_normal, basereg_usable);
    if (eat_immediate(addr, right, basereg_usable))
      return eat_immediates(addr, left, x86_create_am_normal, basereg_usable);
...
static bool eat_shl(x86_address_t *addr, ir_node *node) {
  ...
  if (addr->scale != 0 || addr->index != NULL) return false;
  ...
  if (is_Shl(node)) {
    ...
    if (!is_Const(right)) return false;
    ir_tarval *tv = get_Const_tarval(right);
    if (!tarval_is_long(tv)) return false;

    val = get_tarval_long(tv);
    if (val < 0 || val > 3) return false;
    if (val == 0) be_warningf(node, "found unoptimized Shl x,0");
  ...
  if (x86_is_non_address_mode_node(node)) return false;
      ...
      tryit:
        if (get_And_left(and) == get_binop_left(shr)) {
          ...
          if (is_Const(and_right) && is_Const(shr_right)) {
            ...
            ir_tarval *shift_mask =
                tarval_shl(tarval_shr(all_one, shift_amount), shift_amount);
...
static bool value_last_used_here(be_lv_t *lv, ir_node *here, ir_node *value) {
  ...
  if (be_is_live_end(lv, block, value)) return false;
...
static void mark_non_address_nodes(ir_node *node, void *env) {
  ...
  if (!be_mode_needs_gp_reg(mode) && mode != mode_b) return;
  ...
  switch (get_irn_opcode(node)) {
    ...
    default:
      foreach_irn_in(node, i, in) { x86_mark_non_am(in); }
      break;
  }
}
...
void x86_free_non_address_mode_nodes(void) { free(non_address_mode_nodes); }

char const *x86_get_addr_variant_str(x86_addr_variant_t const variant) {
  switch (variant) {
    case X86_ADDR_REG:
      return "reg";
    case X86_ADDR_JUST_IMM:
      return "immediate";
    case X86_ADDR_BASE:
      return "base";
    case X86_ADDR_BASE_INDEX:
      return "base+index";
    case X86_ADDR_INDEX:
      return "index";
    case X86_ADDR_RIP:
  ...
  if (variant != X86_ADDR_JUST_IMM) {
    ...
    if (variant == X86_ADDR_RIP) {
      ...
      if (x86_addr_variant_has_index(variant)) {
        ...
        if (log_scale > 0) be_emit_irprintf(",%u", 1u << log_scale);
      }
    }
    be_emit_char(')');
  }
