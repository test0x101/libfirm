...
static bool check_immediate_constraint(long val,
  ...
  switch (immediate_constraint_type) {
    ...
    case 'e':
      return -v31 <= val && val < v31;
    case 'I':
      return 0 <= val && val < 32;
    case 'J':
      return 0 <= val && val < 64;
    case 'K':
      return -128 <= val && val < 128;
    case 'L':
      return val == 0xff || val == 0xffff;
    case 'M':
      return 0 <= val && val < 4;
    case 'N':
      return 0 <= val && val < 256;
    case 'O':
...
bool x86_match_immediate(x86_imm32_t *immediate, const ir_node *node,
  ...
  if (offset) {
    ...
    if (!check_immediate_constraint(val, constraint)) return false;
  }
  ...
  if (entity != NULL) {
    ...
    switch (constraint) {
      case 'e':
        if (get_mode_size_bits(mode_P) > 32) return false;
      case 'g':
        break;
      case 'i':
        break;
...
void x86_dump_imm32(x86_imm32_t const *const imm, FILE *const F) {
  if (imm->entity != NULL) ir_fprintf(F, "%+F", imm->entity);
