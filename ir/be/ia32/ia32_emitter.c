...
void x86_emit_condition_code(x86_condition_code_t cc) {
  switch (cc) {
    case x86_cc_overflow:
      be_emit_cstring("o");
      return;
    case x86_cc_not_overflow:
      be_emit_cstring("no");
      return;
    ...
    case x86_cc_sign:
      be_emit_cstring("s");
      return;
    case x86_cc_not_sign:
      be_emit_cstring("ns");
      return;
    case x86_cc_parity:
      be_emit_cstring("p");
      return;
    case x86_cc_not_parity:
      be_emit_cstring("np");
      return;
    case x86_cc_less:
      be_emit_cstring("l");
      return;
    case x86_cc_greater_equal:
      be_emit_cstring("ge");
      return;
    case x86_cc_less_equal:
      be_emit_cstring("le");
      return;
    case x86_cc_greater:
      be_emit_cstring("g");
    ...
    switch (*fmt++) {
      ...
      case 'A': {
        switch (*fmt++) {
          ...
          emit_AM:
          case 'M':
            if (mod & EMIT_ALTERNATE_AM) be_emit_char('*');
            ia32_emit_am(node);
            break;
      ...
      case 'F':
        if (*fmt == 'M') {
          ...
          ia32_x87_attr_t const *const attr = get_ia32_x87_attr_const(node);
          if (attr->x87.pop) be_emit_char('p');
        } else if (*fmt == 'R') {
           ...
           * permuted.  In particular it is not necessary to consider whether
           * the result is to be placed into the explicit register operand. */
          if (get_ia32_x87_attr_const(node)->x87.reverse) be_emit_char('r');
        } else if (*fmt == 'X') {
      ...
      emit_I:
        if (mod & EMIT_SHIFT_COMMA) {
          ...
          if (attr->imm.entity == NULL && attr->imm.offset == 1) break;
        }
      ...
      case 'M': {
        ...
        if (mod & EMIT_32BIT_REG) {
          ...
          if (attr->size == X86_SIZE_32) break;
      ...
      emit_R:
        ...
        if (mod & EMIT_HIGH_REG) {
          ...
          ia32_attr_t const *const attr = get_ia32_attr_const(node);
          name = get_register_name_size(reg, attr->size, attr->use_8bit_high);
        }
        be_emit_char('%');
        be_emit_string(name);
        if (mod & EMIT_SHIFT_COMMA) {
...
static ir_node *find_original_value(ir_node *node) {
  if (irn_visited(node)) return NULL;
...
x86_condition_code_t ia32_determine_final_cc(ir_node const *const node,
  ...
  if (attr->attr.ins_permuted) cc = x86_negate_condition_code(cc);

  ir_node *flags = skip_Proj(get_irn_n(node, flags_pos));

  /* Permuted operands of a test instruction do not change the result. */
  if (is_ia32_Test(flags)) return cc;

  if (is_ia32_Sahf(flags)) {
    flags = get_irn_n(flags, n_ia32_Sahf_val);
    if (!is_ia32_FucomFnstsw(flags) && !is_ia32_FucomppFnstsw(flags) &&
        !is_ia32_FtstFnstsw(flags)) {
      ir_graph *const irg = get_irn_irg(node);
      inc_irg_visited(irg);
      flags = find_original_value(flags);
      assert(is_ia32_FucomFnstsw(flags) || is_ia32_FucomppFnstsw(flags) ||
             is_ia32_FtstFnstsw(flags));
    }
  }

  ia32_attr_t const *const flags_attr = get_ia32_attr_const(flags);
  if (flags_attr->ins_permuted) cc = x86_invert_condition_code(cc);
  return cc;
}
...
static void emit_ia32_Setcc(const ir_node *node) {
  const arch_register_t *dreg =
      arch_get_irn_register_out(node, pn_ia32_Setcc_res);
 ...
 */
static void emit_ia32_Jmp(const ir_node *node) { emit_jmp(node, node); }
...
static void emit_ia32_asm_operand(ir_node const *const node,
                                  char const modifier, unsigned const pos) {
  be_asm_attr_t const *const attr = get_be_asm_attr_const(node);
  x86_asm_operand_t const *const op =
      &((x86_asm_operand_t const *)attr->operands)[pos];
  /* modifiers:
   *   A: print '*' before operand (gcc doc: print an absolute memory reference)
   *   P: like 'p' (but gcc doc says "if PIC, print an @PLT suffix")
   *   X: no modifying effect (gcc doc: don't print any sort of PIC '@' suffix
   * for a symbol) b: 8 bit low name of register c: immediate without prefix '$'
  ...
  if (!be_is_valid_asm_operand_kind(node, modifier, pos, op->op.kind,
                                    "APXbhkpw", "c", ""))
    return;

  if (modifier == 'A') be_emit_char('*');
  ...
  switch ((be_asm_operand_kind_t)op->op.kind) {
    ...
    case BE_ASM_OPERAND_IMMEDIATE:
      if (!be_has_modifier("Pcp", modifier)) be_emit_char('$');
      x86_emit_imm32(&op->u.imm32);
      return;

    case BE_ASM_OPERAND_LABEL:
      if (modifier != 'l') be_emit_char('$');
...
static void emit_ia32_Asm(const ir_node *node) {
  ...
  if (fallthrough) emit_jmp(node, fallthrough);
}
...
static void emit_CopyB_prolog(unsigned size) {
  if (size & 1) ia32_emitf(NULL, "movsb");
...
static void Copy_emitter(const ir_node *node, const ir_node *op) {
  ...
  if (in->cls == &ia32_reg_classes[CLASS_ia32_fp]) return;
...
static void emit_mov(const ir_node *node, const arch_register_t *src,
                     const arch_register_t *dst) {
...
/* helper function for emit_ia32_Minus64 */
static void emit_sbb(const ir_node *node, const arch_register_t *src,
                     const arch_register_t *dst) {
  ia32_emitf(node, "sbbl %R, %R", src, dst);
}

/* helper function for emit_ia32_Minus64 */
static void emit_xchg(const ir_node *node, const arch_register_t *src,
                      const arch_register_t *dst) {
...
static void emit_ia32_Minus64(const ir_node *node) {
  arch_register_t const *const in_lo =
      arch_get_irn_register_in(node, n_ia32_Minus64_low);
  arch_register_t const *const in_hi =
      arch_get_irn_register_in(node, n_ia32_Minus64_high);
  arch_register_t const *const out_lo =
      arch_get_irn_register_out(node, pn_ia32_Minus64_res_low);
  arch_register_t const *const out_hi =
      arch_get_irn_register_out(node, pn_ia32_Minus64_res_high);
  ...
  switch ((get_ip_style_t)get_ip_style) {
    ...
    case IA32_GET_IP_THUNK: {
      ...
      if (thunk == NULL) {
        ...
        thunk =
            new_global_entity(glob, id, tp, ir_visibility_external_private,
...
static void ia32_emit_node(ir_node *node) {
  ...
  if (is_ia32_irn(node)) {
    ...
    if (get_ia32_exc_label(node)) ia32_assign_exc_label(node);
...
bool ia32_should_align_block(ir_node const *const block) {
  ...
  if (ia32_cg_config.label_alignment_factor <= 0) return false;

  double block_freq = get_block_execfreq(block);
  if (block_freq < DELTA) return false;
  ...
  if (prev_freq < DELTA && !(jmp_freq < DELTA)) return true;
...
static bool has_fallthrough(ir_node const *const block) {
  for (int i = get_Block_n_cfgpreds(block); i-- > 0;) {
    ...
    if (be_emit_get_prev_block(block) == pred_block) return true;
  }
  return false;
}
...
static void ia32_gen_block(ir_node *block) {
  ...
  sched_foreach(block, node) { ia32_emit_node(node); }
}
...
static int cmp_exc_entry(const void *a, const void *b) {
  ...
  if (get_ia32_exc_label_id(ea->exc_instr) <
      get_ia32_exc_label_id(eb->exc_instr))
    return -1;
  return +1;
}
...
static parameter_dbg_info_t *construct_parameter_infos(ir_graph *irg) {
  ...
  for (size_t i = 0, n_members = get_compound_n_members(frame_type);
    ...
    if (!is_parameter_entity(member)) continue;
    size_t param = get_entity_parameter_number(member);
    if (param == IR_VA_START_PARAMETER_NUMBER) continue;
...
static void emit_function_text(ir_graph *const irg,
                               exc_entry **const exc_list) {
...
static const lc_opt_enum_int_items_t get_ip_syle_items[] = {
    {"pop", IA32_GET_IP_POP}, {"thunk", IA32_GET_IP_THUNK}, {NULL, 0}};

static lc_opt_enum_int_var_t get_ip_style_var = {&get_ip_style,
                                                 get_ip_syle_items};

static const lc_opt_table_entry_t ia32_emitter_options[] = {
    LC_OPT_ENT_BOOL("mark_spill_reload",
                    "mark spills and reloads with ud opcodes",
                    &mark_spill_reload),
    LC_OPT_ENT_ENUM_INT("get_ip", "method to get IP for the pic base",
                        &get_ip_style_var),
    LC_OPT_LAST};
...
static unsigned emit_jit_entity_relocation_asm(char *const buffer,
  ...
  if (offset != 0) be_emit_irprintf("%+" PRId32, offset);
  be_emit_char('\n');
  be_emit_write_line();
  return res;
}
