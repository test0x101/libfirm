...
static bool is_hl_register(arch_register_t const *const reg) {
  return reg == &ia32_registers[REG_EAX] || reg == &ia32_registers[REG_EBX] ||
         reg == &ia32_registers[REG_ECX] || reg == &ia32_registers[REG_EDX];
}

static void copy_mark(const ir_node *old, ir_node *newn) {
  if (is_ia32_is_reload(old)) set_ia32_is_reload(newn);
  if (is_ia32_is_spill(old)) set_ia32_is_spill(newn);
  if (is_ia32_is_remat(old)) set_ia32_is_remat(newn);
}
...
static ia32_immediate_attr_t const *get_op_imm_no_ent(ir_node *const node,
                                                      unsigned const n) {
  ir_node *const op = get_irn_n(node, n);
  if (!is_ia32_Immediate(op)) return NULL;

  ia32_immediate_attr_t const *const imm = get_ia32_immediate_attr_const(op);
  if (imm->imm.entity) return NULL;
...
static produces_flag_t check_produces_zero_sign(ir_node *node, unsigned pn) {
  if (!is_ia32_irn(node)) return produces_no_flag;
  ...
  switch (get_ia32_irn_opcode(node)) {
    ...
    case iro_ia32_Xor:
...

      {
        unsigned count_pos;
        case iro_ia32_ShlD:
          count_pos = n_ia32_ShlD_count;
          goto check_shift_amount;
        case iro_ia32_ShrD:
          count_pos = n_ia32_ShrD_count;
          goto check_shift_amount;
        case iro_ia32_Sar:
          count_pos = n_ia32_Sar_count;
          goto check_shift_amount;
        case iro_ia32_Shl:
          count_pos = n_ia32_Shl_count;
          goto check_shift_amount;
        case iro_ia32_Shr:
          count_pos = n_ia32_Shr_count;
          goto check_shift_amount;
        check_shift_amount:;
          /* when shift count is zero the flags are not affected, so we can only
           * do this for constants != 0 */
          ia32_immediate_attr_t const *const imm =
              get_op_imm_no_ent(node, count_pos);
          if (!imm || (imm->imm.offset & 0x1f) == 0) return produces_no_flag;
          break;
      }

    case iro_ia32_Mul:
      return pn == pn_ia32_Mul_res_high ? produces_zero_in_carry
                                        : produces_no_flag;
...
static ir_node *make_binop(ir_node *const node, cons_binop *const cons,
  ...
  ir_node *const binop =
      cons(dbgi, block, noreg, noreg, nomem, left, right, size);
  arch_set_irn_register_out(binop, pn_ia32_res, reg);
  if (get_irn_mode(node) == mode_T) set_irn_mode(binop, mode_T);
  replace(node, binop);
  return binop;
}
...
static void peephole_ia32_And(ir_node *const node) {
  if (get_ia32_attr_const(node)->size != X86_SIZE_32) return;

  ia32_immediate_attr_t const *const imm =
      get_op_imm_no_ent(node, n_ia32_And_right);
  if (!imm) return;

  /* Perform the replacement only, if the flags are unused.
   * Xor sets the flags differently than And. */
  if (is_res_used(node, pn_ia32_And_flags)) return;

  arch_register_t const *const reg =
      arch_get_irn_register_out(node, pn_ia32_And_res);
  ...
  if (val == 0xFFFF0000) {
    ...
    if (val == 0xFFFFFF00) {
      ...
      make_binop_imm_8h(node, &new_bd_ia32_And_8bit, val >> 8 & 0xFF,
                        X86_SIZE_8, reg);
    }
  }
}
...
static void peephole_ia32_Or(ir_node *const node) {
  ...
  ia32_immediate_attr_t const *const imm =
      get_op_imm_no_ent(node, n_ia32_Or_right);
  if (!imm) return;

  if (is_res_used(node, pn_ia32_Or_flags)) return;

  arch_register_t const *const reg =
      arch_get_irn_register_out(node, pn_ia32_Or_res);
...
static ir_node *make_not(ir_node *const xor,
  ...
  if (get_irn_mode(xor) == mode_T) set_irn_mode(not, mode_T);
  replace(xor, not );
  return not ;
}

static void make_not_8h(ir_node *const xor,
                        ir_node *(*const cons)(dbg_info *, ir_node *, ir_node *,
                                               x86_insn_size_t),
                        x86_insn_size_t const size,
                        arch_register_t const *const reg) {
...
static void peephole_ia32_Xor(ir_node *const node) {
  ...
  ia32_immediate_attr_t const *const imm =
      get_op_imm_no_ent(node, n_ia32_Xor_right);
  if (!imm) return;

  /* Perform the replacement only, if the flags are unused.
   * Not does not set the flags, while Xor does. */
  if (is_res_used(node, pn_ia32_Xor_flags)) return;

  arch_register_t const *const reg =
      arch_get_irn_register_out(node, pn_ia32_Xor_res);
  uint32_t const val = imm->imm.offset;
  if (val == 0x0000FFFF) {
...
static void peephole_ia32_Cmp(ir_node *const node) {
  if (get_ia32_op_type(node) != ia32_Normal) return;

  ia32_immediate_attr_t const *const imm =
      get_op_imm_no_ent(node, n_ia32_Cmp_right);
  if (!imm || imm->imm.offset != 0) return;
  ...
  ir_node *const test =
      size == X86_SIZE_8
          ? new_bd_ia32_Test_8bit(dbgi, block, noreg, noreg, nomem, op, op,
                                  size, ins_permuted)
          : new_bd_ia32_Test(dbgi, block, noreg, noreg, nomem, op, op, size,
                             ins_permuted);
  arch_set_irn_register_out(test, pn_ia32_Test_eflags,
                            &ia32_registers[REG_EFLAGS]);

  foreach_out_edge_safe(node, edge) {
    ir_node *const user = get_edge_src_irn(edge);

    if (is_Proj(user)) exchange(user, test);
  }
...
static void peephole_ia32_Test(ir_node *node) {
  ...
  if (left == right) { /* we need a test for 0 */
    ...
    if (get_nodes_block(left) != block) return;
    ...
    for (;;) {
      ...
      if (schedpoint == op) break;
      if (arch_irn_is(schedpoint, modify_flags)) return;
      if (schedpoint == block) panic("couldn't find left");
    }

    produces_flag_t produced = check_produces_zero_sign(op, pn);
    if (produced == produces_no_flag) return;

    /* make sure users only look at the sign/zero flag */
    foreach_out_edge(node, edge) {
      ir_node *user = get_edge_src_irn(edge);
      if (is_ia32_CMovcc(user) || is_ia32_Jcc(user) || is_ia32_Setcc(user) ||
          is_ia32_SetccMem(user)) {
        x86_condition_code_t cc = get_ia32_condcode(user);

        if (cc == x86_cc_equal || cc == x86_cc_not_equal) continue;
    ...
    if (op_size != get_ia32_attr_const(node)->size) return;
    ...
    if (produced == produces_zero_in_carry) {
      ...
      foreach_out_edge(node, edge) {
        ...
        switch (cc) {
          case x86_cc_equal:
            cc = x86_cc_above_equal;
            break;
          case x86_cc_not_equal:
            cc = x86_cc_below;
            break;
          default:
            panic("unexpected pn");
    ...
    if (get_irn_mode(op) != mode_T) {
      ...
      if (get_irn_n_edges(left) == 2) kill_node(left);
    }

    ir_node *const flags_proj =
        be_new_Proj_reg(op, pn_ia32_flags, &ia32_registers[REG_EFLAGS]);
    be_peephole_exchange(node, flags_proj);
  } else {
    /* A test with an entity is rather strange, but better safe than sorry */
    ia32_immediate_attr_t const *const imm =
        get_op_imm_no_ent(node, n_ia32_Test_right);
    if (!imm) return;
...
static void peephole_IncSP_Store_to_push(ir_node *irn) {
  ...
  sched_foreach_after(irn, node) {
    ...
    if (inc_ofs - 4 < offset || offset >= MAXPUSH_OPTIMIZE * 4) continue;

    int storeslot = offset >> 2;

    /* storing into the same slot twice is bad (and shouldn't happen...) */
    if (stores[storeslot] != NULL) break;

    stores[storeslot] = node;
    if (storeslot > maxslot) maxslot = storeslot;
  }

  ir_node *curr_sp = irn;
  int i;
  for (i = -1; i < maxslot; ++i) {
    if (stores[i + 1] == NULL) break;
  }
  ...
  for (; i >= 0; --i) {
    ...
    ir_node *const push = new_bd_ia32_Push(dbgi, block, noreg, noreg, mem, val,
                                           curr_sp, X86_SIZE_32);
    copy_mark(store, push);

    if (first_push == NULL) first_push = push;

    sched_add_after(skip_Proj(curr_sp), push);

    /* create stackpointer Proj */
    curr_sp =
        be_new_Proj_reg(push, pn_ia32_Push_stack, &ia32_registers[REG_ESP]);
...
static void peephole_Load_IncSP_to_pop(ir_node *irn) {
  ...
  sched_foreach_reverse_before(irn, node) {
    ...
    if (!is_ia32_Load(node)) {
      if (be_is_Copy(node)) {
        ...
        const arch_register_t *sreg =
            arch_get_irn_register(be_get_Copy_op(node));
    ...
    if (offset < 0 || offset >= MAXPUSH_OPTIMIZE * 4) continue;
    /* ignore those outside the possible windows */
    if (offset > inc_ofs - 4) continue;
    int loadslot = offset >> 2;

    /* loading from the same slot twice is bad (and shouldn't happen...) */
    if (loads[loadslot] != NULL) break;

    const arch_register_t *dreg =
        arch_get_irn_register_out(node, pn_ia32_Load_res);
    ...
    loads[loadslot] = node;
    if (loadslot > maxslot) maxslot = loadslot;
  }
  ...
  if (inc_ofs > 0) {
    pred_sp =
        ia32_new_IncSP(block, pred_sp, -inc_ofs, be_get_IncSP_no_align(irn));
  ...
  for (++i; i <= maxslot; ++i) {
    ...
    const arch_register_t *reg =
        arch_get_irn_register_out(load, pn_ia32_Load_res);

    x86_insn_size_t const size = get_ia32_attr_const(load)->size;
    ir_node *pop =
        new_bd_ia32_Pop(get_irn_dbg_info(load), block, mem, pred_sp, size);
...
static const arch_register_t *get_free_gp_reg(ir_graph *irg) {
  ...
  for (int i = 0; i < N_ia32_gp_REGS; ++i) {
    ...
    if (!rbitset_is_set(birg->allocatable_regs, reg->global_index)) continue;

    if (be_peephole_get_value(reg->global_index) == NULL) return reg;
  }
 ...
 */
static ir_node *create_pop(dbg_info *dbgi, ir_node *block, ir_node *stack,
                           ir_node *schedpoint, const arch_register_t *reg,
                           x86_insn_size_t const size) {
...
static void peephole_be_IncSP(ir_node *node) {
  ...
  if (be_peephole_IncSP_IncSP(node)) return;
...
static void peephole_ia32_Const(ir_node *node) {
  ...
  if (attr->imm.offset != 0 || attr->imm.entity != NULL) return;
  /* xor destroys the flags, so no-one must be using them */
  if (be_peephole_get_value(REG_EFLAGS) != NULL) return;
...
static ir_node *make_add(ir_node *const node, ir_node *const l,
                         ir_node *const r) {
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = get_nodes_block(node);
  ir_graph *const irg = get_irn_irg(node);
  ...
  ir_node *const nomem = get_irg_no_mem(irg);
  ir_node *const add =
      new_bd_ia32_Add(dbgi, block, noreg, noreg, nomem, l, r, X86_SIZE_32);
...
static void peephole_ia32_Lea(ir_node *node) {
  ...
  arch_register_t const *const breg =
      is_ia32_NoReg_GP(base) ? NULL : arch_get_irn_register(base);
  ir_node *const idx = get_irn_n(node, n_ia32_Lea_index);
  arch_register_t const *const ireg =
      is_ia32_NoReg_GP(idx) ? NULL : arch_get_irn_register(idx);
  unsigned const scale = attr->addr.log_scale;
  arch_register_t const *const oreg =
      arch_get_irn_register_out(node, pn_ia32_Lea_res);
#ifdef DEBUG_libfirm
  if (!breg && !ireg) be_warningf(node, "found lea which is a const");
...
static void peephole_ia32_ImulImm_split(ir_node *imul) {
  ...
  if (get_ia32_op_type(imul) != ia32_AddrModeS) return;
...
void ia32_peephole_optimization(ir_graph *irg) {
  ...
  if (!ia32_cg_config.use_imul_mem_imm32)
    register_peephole_optimization(op_ia32_IMulImm,
                                   peephole_ia32_ImulImm_split);
...
static void optimize_conv_store(ir_node *node) {
  ...
  if (!is_ia32_Conv_I2I(pred)) return;
  if (get_ia32_op_type(pred) != ia32_Normal) return;
  ...
  if (pred_attr->size < node_attr->size) return;
...
static void optimize_load_conv(ir_node *node) {
  ...
  ia32_attr_t const *const node_attr = get_ia32_attr_const(node);
  if (node_attr->size < load_attr->size) return;
  ...
  if (node_attr->size == pred_attr->size &&
    ...
    if (get_irn_n_edges(pred_proj) == 1) {
      ...
      if (node_attr->size == X86_SIZE_8) {
        arch_register_req_t const **reqs = arch_get_irn_register_reqs_in(node);
        arch_set_irn_register_reqs_in(pred, reqs);
      }
    } else {
      /* we don't want to end up with 2 loads, so we better do nothing */
      if (get_irn_mode(pred) == mode_T) return;
