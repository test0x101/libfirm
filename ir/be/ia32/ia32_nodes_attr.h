...
struct ia32_attr_t {
  ...
  unsigned am_arity : 2; /**< Indicates the address mode type supported by this
                            node. */

  ENUMBF(ia32_frame_use_t)
  frame_use : 2; /**< Whether an entity on the frame is used and its size. */
  unsigned has_except_label : 1; /**< Set if this node needs a label because of
                                    possible exception. */

  unsigned
      is_commutative : 1; /**< Indicates whether op is commutative or not. */

  unsigned ins_permuted : 1; /**< inputs of node have been permuted (for
                                commutative nodes) */
  ...
  ir_label_t exc_label; /**< the exception label iff this instruction can throw
                           an exception */
...
/* the following union is necessary to indicate to the compiler that we might
 * want to cast the structs (we use them to simulate OO-inheritance) */
...
#ifndef NDEBUG
#define CAST_IA32_ATTR(type, ptr)                                     \
  (assert(((const ia32_attr_t *)(ptr))->attr_type &IA32_ATTR_##type), \
   (type *)(ptr))
#define CONST_CAST_IA32_ATTR(type, ptr)                               \
  (assert(((const ia32_attr_t *)(ptr))->attr_type &IA32_ATTR_##type), \
   (const type *)(ptr))
#else
