 ...
 */
static unsigned x87_get_depth(const x87_state *state) { return state->depth; }
...
static unsigned x87_on_stack(x87_state const *const state,
  ...
  for (unsigned i = 0; i < state->depth; ++i) {
    if (x87_get_st_node(state, i) == node) return i;
  }
...
static unsigned x87_reg_on_stack(x87_state const *const state,
                                 unsigned vreg_idx) {
  for (unsigned i = 0; i < state->depth; ++i) {
    if (x87_get_st_reg(state, i) == vreg_idx) return i;
  }
  return X87_NOT_ON_STACK;
}
 ...
 * @param state     the x87 state
 */
static void x87_emms(x87_state *state) { state->depth = 0; }
...
static void move_to_pos(x87_state *const state, ir_node *const before,
  ...
  if (from != to) {
    if (from != 0) x87_create_fxch(state, before, from);
    if (to != 0) x87_create_fxch(state, before, to);
  }
}
...
static x87_state *x87_shuffle(ir_node *block, x87_state *state,
  ...
  for (n_cycles = 0; all_mask != 0; ++n_cycles) {
    ...
    for (i = 0; i < state->depth; ++i) {
      if (all_mask & (1 << i)) {
        ...
        if (x87_get_st_reg(state, i) != x87_get_st_reg(dst_state, i)) break;
      }
    }
  ...
  for (unsigned ri = 0; ri < n_cycles; ++ri) {
    if ((cycles[ri] & 1) == 0) x87_create_fxch(state, before, cycle_idx[ri][0]);
 ...
 */
static inline bool is_fp_live(ir_node const *const node,
...
static const arch_register_t *find_unused_vreg(x87_state const *const state) {
  ...
  if (depth >= N_X87_REGS) return NULL;
  ...
  for (unsigned i = 0; i < N_X87_REGS; ++i) {
    if ((live & (1u << i)) == 0) return &x87.regclass->regs[i];
  }
...
static void fp_dump_live(fp_liveness live) {
  ...
  for (unsigned i = 0; i < N_X87_REGS; ++i) {
    if (live & (1 << i)) DB((dbg, LEVEL_2, "vf%d ", i));
  }
  DB((dbg, LEVEL_2, "\n"));
}
#endif /* DEBUG_libfirm */
...
void x86_sim_x87_binop(x87_state *const state, ir_node *const node,
  ...
  if (pop) x87_pop(state);

  DEBUG_ONLY(char const *const st0 = get_st_reg(0)->name;
  ...
  if (!is_fp_live(val, live)) {
    ...
    if (store_bits > 64) {
      ...
      do_pop:
        ...
        if (!is_ia32_fst(n)) panic("TODO: out of stack spill case");
        ...
        ir_node *const vfld =
            new_bd_ia32_fld(NULL, block, base, idx, mem, X86_SIZE_80);
...
void x86_sim_x87_store_pop(x87_state *const state, ir_node *const node,
  ...
  DB((dbg, LEVEL_1, ">>> %+F %s ->\n", node, arch_get_irn_register(val)->name));
...
static void sim_FtstFnstsw(x87_state *state, ir_node *n) {
  ...
  if (!is_fp_live(val, live)) x86_x87_create_fpop(state, n, 0);
}
...
unsigned x86_sim_x87_fucom(x87_state *const state, ir_node *const node,
  ...
  if (pops > 0) {
    ...
    if (pops == 2) additional_pop = op_reg - 1; /* -1 due to prior pop */
  }
...
static void sim_be_Copy(x87_state *state, ir_node *n) {
  if (!is_x87_req(arch_get_irn_register_req(n))) return;
...
void x86_sim_x87_ret(x87_state *const state, ir_node *const ret) {
  ...
  for (unsigned i = 0, n = get_irn_arity(ret); i != n; ++i) {
    if (is_x87_req(arch_get_irn_register_req_in(ret, i))) ++n_float_res;
  }
...
static void sim_be_Perm(x87_state *state, ir_node *irn) {
  ...
  if (!is_x87_req(arch_get_irn_register_req_out(irn, 0))) return;
...
static void sim_Phi(x87_state *state, ir_node *node) {
  if (!is_x87_req(arch_get_irn_register_req(node))) return;
...
static void x87_kill_deads(x87_simulator *const sim, ir_node *const after,
  ...
  for (unsigned i = depth; i-- > 0;) {
    ...
    if (!is_fp_live(node, live)) kill_mask |= (1 << i);
  }
...
static bool reg_req_same_limited(arch_register_req_t const *const req,
  ...
  if (req == ref_req) return true;

  if (req->cls != ref_req->cls || !req->limited ||
...
static bool is_clobber(ir_node const *const asm_n, ir_node const *const value) {
  ...
  if (req->should_be_same != 0) return false;
  ...
  for (size_t i = 0, n = ARR_LEN(operands); i != n; ++i) {
    ...
    if (op->op.kind == BE_ASM_OPERAND_OUTPUT_VALUE &&
        (unsigned)op->op.pos == num)
      return false;
  }
...
static void sim_be_Asm(x87_state *const state, ir_node *const n) {
  ...
  ir_node *in_u = NULL;
  be_foreach_use(
      n, x87.regclass, req, value, value_req,
      if (reg_req_same_limited(req, req_t)) {
        ...
        assert(!in_u);
        in_u = value;
      } else { panic("cannot handle %+F with x87 constraints", n); });
  ...
  if (out_t) x86_x87_push(state, out_t);
...
void x86_init_x87(void) { FIRM_DBG_REGISTER(dbg, "firm.be.x86.x87"); }
