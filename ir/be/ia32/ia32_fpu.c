...
static ir_entity *create_ent(ir_entity **const dst, int value,
                             const char *name) {
  if (!*dst) {
...
static ir_node *create_fnstcw(ir_node *const block, ir_node *const frame,
                              ir_node *const noreg, ir_node *const nomem,
                              ir_node *const state) {
  ir_node *const fnstcw =
...
static ir_node *create_fpu_mode_spill(void *const env, ir_node *const state,
                                      bool const force, ir_node *const after) {
  (void)env;

  if (!force && is_ia32_ChangeCW(state)) return NULL;
...
static ir_node *create_fpu_mode_reload(void *const env, ir_node *const state,
                                       ir_node *const spill,
                                       ir_node *const before,
                                       ir_node *const last_state) {
  (void)env;
  (void)state;
  ...
  if (ia32_cg_config.use_unsafe_floatconv) {
    ...
    ir_entity *const rounding_mode =
        spill ? create_ent(&fpcw_round, 0xC7F, "_fpcw_round")
    ...
    if (spill) {
      ...
      assert(last_state);
      ir_node *const cwstore =
          create_fnstcw(block, frame, noreg, nomem, last_state);
      ...
      ir_node *const orn = new_bd_ia32_Or(NULL, block, noreg, noreg, nomem,
                                          load_res, or_const, X86_SIZE_32);
      sched_add_before(before, orn);

      ir_node *const store =
          new_bd_ia32_Store(NULL, block, frame, noreg, nomem, orn, X86_SIZE_32);
...
static void collect_fpu_mode_nodes_walker(ir_node *node, void *data) {
  if (is_Proj(node) || is_ia32_ChangeCW(node)) return;
  ...
  be_foreach_out(node, o) {
    ...
    if (reg != &ia32_registers[REG_FPCW]) continue;
    ir_node *value = node;
...
void ia32_setup_fpu_mode(ir_graph *irg) {
  ...
  be_assure_state(irg, &ia32_registers[REG_FPCW], NULL, create_fpu_mode_spill,
                  create_fpu_mode_reload);
}
