...
const x86_asm_constraint_list_t ia32_asm_constraints = {
    ...
    ['Q'] = {MATCH_REG, GP,
             1 << REG_GP_EAX | 1 << REG_GP_EBX | 1 << REG_GP_ECX |
                 1 << REG_GP_EDX},
    ['V'] = {MATCH_MEM, GP, 0},
    ['X'] = {MATCH_ANY, GP, 0},
...
typedef ir_node *construct_binop_func(dbg_info *db, ir_node *block,
                                      ir_node *base, ir_node *index,
                                      ir_node *mem, ir_node *op1, ir_node *op2,
                                      x86_insn_size_t size);

typedef ir_node *construct_binop_flags_func(dbg_info *db, ir_node *block,
                                            ir_node *base, ir_node *index,
                                            ir_node *mem, ir_node *op1,
                                            ir_node *op2, ir_node *flags,
                                            x86_insn_size_t size);
...
static ir_node *create_I2I_Conv(ir_mode *src_mode, dbg_info *dbgi,
...
static bool is_simple_x87_Const(ir_node *node) {
  ...
  if (tarval_is_null(tv) || tarval_is_one(tv)) return true;
...
static bool is_simple_sse_Const(ir_node *node) {
  ...
  if (get_mode_size_bits(mode) == 32) return true;
  ...
  if (mode == ia32_mode_float64) {
    ...
    if (val == 0) /* lower 32bit are zero, really a 32bit constant */
      return true;
  }
...
static ir_node *get_global_base(ir_graph *const irg) {
  if (ir_platform.pic_style != BE_PIC_NONE) return ia32_get_pic_base(irg);
  return noreg_GP;
}
...
ir_node *ia32_create_Immediate_full(ir_graph *const irg,
  ...
  ir_node *const immediate = new_bd_ia32_Immediate(NULL, start_block, imm);
  arch_set_irn_register(immediate, &ia32_registers[REG_GP_NOREG]);
  return immediate;
}

static void adjust_relocation(x86_imm32_t *imm) {
  if (imm->kind != X86_IMM_ADDR) return;
  ir_entity *entity = imm->entity;
  if (is_tls_entity(entity)) {
    imm->kind = entity_has_definition(entity) ? X86_IMM_TLS_LE : X86_IMM_TLS_IE;
    return;
  }
}

static ir_node *try_create_Immediate(const ir_node *node,
                                     char const constraint) {
  x86_imm32_t immediate;
  if (!x86_match_immediate(&immediate, node, constraint)) return NULL;
...
static ir_entity *create_float_const_entity(ir_tarval *tv, ident *name) {
  ...
  if (!res) {
    if (!name) name = id_unique("C");

    ir_type *const tp = get_type_for_mode(mode);
    res = new_global_entity(get_glob_type(), name, tp, ir_visibility_private,
  ...
  if (mode_is_float(mode)) {
    ...
    if (ia32_cg_config.use_sse2) {
      if (tarval_is_null(tv)) {
        ...
        ir_node *pslld = new_bd_ia32_xPslld(dbgi, block, allones, imm1, size);
        ...
        ir_node *load =
            new_bd_ia32_xLoad(dbgi, block, base, noreg_GP, nomem, size);
      ...
      negate:
        ...
        ir_node *base = get_global_base(irg);
        ir_node *load =
            new_bd_ia32_fld(dbgi, block, base, noreg_GP, nomem, size);
        set_am_const_entity(load, floatent);
        set_ia32_op_type(load, ia32_AddrModeS);
        arch_add_irn_flags(load, arch_irn_flag_rematerializable);
...
static ir_type *ia32_create_float_array(ir_type *tp) {
  ...
  ir_type **const arr = mode == ia32_mode_float32 ? &float_F
                        : mode == ia32_mode_float64
                            ? &float_D
                            :
                            /*                       */ &float_E;
  if (!*arr) *arr = new_type_array(tp, 2);
  return *arr;
}
...
ir_entity *ia32_gen_fp_known_const(ia32_known_const_t const kct) {
  static const struct {
    ...
    const char *cnst_str;
    char mode;
  } names[ia32_known_const_max] = {{"C_sfp_sign", "0x80000000", 0},
  ...
  if (ent == NULL) {
    ...
    if (kct == ia32_ULLBIAS) {
      ...
      ent =
          new_global_entity(get_glob_type(), name, atype, ir_visibility_private,
                            IR_LINKAGE_CONSTANT | IR_LINKAGE_NO_IDENTITY);

      ir_initializer_t *initializer = create_initializer_compound(2);
      set_initializer_compound_value(
          initializer, 0, create_initializer_tarval(get_mode_null(mode)));
...
static bool prevents_AM_one(ir_node *const other, ir_node *const am_candidate) {
  ...
  if (is_Proj(other) && get_Proj_pred(other) == am_candidate) return false;

  if (!heights_reachable_in_block(heights, other, am_candidate)) return false;
...
static bool prevents_AM(ir_node *const block, ir_node *const am_candidate,
                        ir_node *const other) {
  if (get_nodes_block(other) != block) return false;
...
static bool ia32_use_source_address_mode(ir_node *block, ir_node *node,
  ...
  if (is_Const(node)) {
    ...
    if (mode_is_float(mode)) {
      ...
      if (!tarval_ieee754_can_conv_lossless(tv, ia32_mode_float64))
        return false;
      if (ia32_cg_config.use_sse2) {
        if (is_simple_sse_Const(node)) return false;
      } else if (is_simple_x87_Const(node)) {
        return false;
      }
      if (get_irn_n_edges(node) > 1) return false;
      return true;
    }
    return false;
  }
  ...
  if (!is_Load(load)) return false;
  assert(get_Proj_num(node) == pn_Load_res);
  if (get_nodes_block(load) != block) return false;
  ir_mode *mode = get_irn_mode(node);
  /* we can't fold mode_E AM */
  if (mode == x86_mode_E) return false;
  ...
  if (be_is_transformed(node)) return false;

  /* don't do AM if other node inputs depend on the load (via mem-proj) */
  if (other != NULL && prevents_AM(block, load, other)) return false;

  if (other2 != NULL && prevents_AM(block, load, other2)) return false;
...
static void build_address_ptr(x86_address_t *const addr, ir_node *const ptr,
                              ir_node *const mem,
                              x86_create_am_flags_t const flags) {
...
static void build_address(ia32_address_mode_t *am, ir_node *node,
  ...
  if (is_Const(node)) {
    ...
    addr->variant = lconst_variant, adjust_relocation(&addr->imm);
...
static void set_address(ir_node *node, const x86_address_t *addr) {
  ...
  if (addr->tls_segment) attr->addr.segment = X86_SEGMENT_GS;
  if (addr->imm.kind == X86_IMM_FRAMEENT)
    set_ia32_frame_use(node, IA32_FRAME_USE_AUTO);
}

static void set_indexed_ent(ir_node *const node, unsigned const scale,
                            ir_entity *const ent) {
  x86_address_t const addr = {
      .variant = ir_platform.pic_style != BE_PIC_NONE ? X86_ADDR_BASE_INDEX
                                                      : X86_ADDR_INDEX,
      .scale = scale,
      .imm =
          {
...
static void set_am_attributes(ir_node *node, const ia32_address_mode_t *am) {
  ...
  if (am->pinned && !get_irn_pinned(node)) set_irn_pinned(node, true);
  if (am->commutative) set_ia32_commutative(node);
}

static bool is_float_downconv(const ir_node *node) {
  if (!is_Conv(node)) return false;
...
static void match_arguments(ia32_address_mode_t *am, ir_node *block,
  ...
  ir_node *new_op1;
  if (new_op2 == NULL && use_am &&
      ia32_use_source_address_mode(block, op2, op1, other_op, flags)) {
    ...
    if (!new_op2) new_op2 = transform(op2);
    am->size = size;
  }
 ...
 *         node else
 */
static ir_node *fix_mem_proj(ir_node *const node,
                             ia32_address_mode_t const *const am) {
  if (am->mem_proj == NULL) return node;
...
static ir_node *make_binop(ir_node *const node,
                           ia32_address_mode_t const *const am,
                           construct_binop_func *const func) {
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = be_transform_nodes_block(node);
  ir_node *const res = func(dbgi, block, am->addr.base, am->addr.index,
                            am->addr.mem, am->new_op1, am->new_op2, am->size);
...
COMPILETIME_ASSERT((int)n_ia32_l_binop_left == (int)n_ia32_l_Adc_left,
                   n_Adc_left)
COMPILETIME_ASSERT((int)n_ia32_l_binop_right == (int)n_ia32_l_Adc_right,
                   n_Adc_right)
COMPILETIME_ASSERT((int)n_ia32_l_binop_eflags == (int)n_ia32_l_Adc_eflags,
                   n_Adc_eflags)
COMPILETIME_ASSERT((int)n_ia32_l_binop_left == (int)n_ia32_l_Sbb_minuend,
                   n_Sbb_minuend)
COMPILETIME_ASSERT((int)n_ia32_l_binop_right == (int)n_ia32_l_Sbb_subtrahend,
                   n_Sbb_subtrahend)
COMPILETIME_ASSERT((int)n_ia32_l_binop_eflags == (int)n_ia32_l_Sbb_eflags,
                   n_Sbb_eflags)
...
static ir_node *gen_binop_flags(ir_node *node, construct_binop_flags_func *func,
  ...
  x86_address_t *addr = &am.addr;
  ir_node *new_node = func(dbgi, block, addr->base, addr->index, addr->mem,
                           am.new_op1, am.new_op2, new_eflags, am.size);
...
static ir_node *skip_float_upconv(ir_node *node) {
  ...
  while (is_Conv(node)) {
     ...
     */
    if (get_irn_n_edges(node) > 1) break;
...
static void check_x87_floatmode(ir_mode *mode) {
  if (mode != x86_mode_E) panic("x87 only supports x86 extended float mode");
}
...
static ir_node *gen_binop_x87_float(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  ir_mode *mode = is_Div(node) ? get_Div_resmode(node) : get_irn_mode(node);
  ...
  x86_address_t *addr = &am.addr;
  if (am.op_type == ia32_Normal) am.size = X86_SIZE_80;
  ir_node *new_node = func(dbgi, new_block, addr->base, addr->index, addr->mem,
                           am.new_op1, am.new_op2, fpcw, am.size);
  set_am_attributes(new_node, &am);
...
static ir_node *skip_shift_amount_conv(ir_node *n) {
  ...
  while (is_Conv(n) && get_irn_n_edges(n) == 1) {
    ...
    if (!be_mode_needs_gp_reg(mode)) break;
 ...
 * @return The constructed ia32 node.
 */
static ir_node *gen_shift_binop(ir_node *const node, ir_node *op1, ir_node *op2,
                                construct_shift_func *const func,
                                construct_shift_func *const func8,
                                match_flags_t const flags) {
  assert((flags & ~(match_mode_neutral | match_sign_ext | match_zero_ext)) ==
         0);
...
static ir_node *gen_unop(ir_node *node, ir_node *op, construct_unop_func *func,
  ...
  if (flags & match_mode_neutral) op = be_skip_downconv(op, true);
...
static ir_node *create_lea(dbg_info *const dbgi, ir_node *const block,
                           ir_node *const base, ir_node *const idx,
                           unsigned const log_scale, int32_t const offset) {
...
static ir_node *create_lea_from_address(dbg_info *dbgi, ir_node *block,
  ...
  if (addr->tls_segment) {
    ...
    if (base == noreg_GP) {
      base = tls_base;
      addr->variant =
          addr->variant == X86_ADDR_INDEX ? X86_ADDR_BASE_INDEX : X86_ADDR_BASE;
    } else {
 ...
 */
static ir_node *gen_64bit_shifts(ir_node *const node, ir_node *const high,
                                 ir_node *const low, ir_node *count,
                                 new_shiftd_func const func) {
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const block = be_transform_nodes_block(node);
...
static ir_node *match_64bit_shift(ir_node *node) {
  ...
  if (is_Shl(op1) && is_Shr(op2)) {
    ...
    if (is_complementary_shifts(shl_right, shr_right))
      return gen_64bit_shifts(node, shl_left, shr_left, shl_right,
                              new_bd_ia32_ShlD_imm);
    ...
    if (is_Shr(shr_left) && is_Not(shr_right) &&
      ...
      return gen_64bit_shifts(node, shl_left, val_h, shl_right,
                              new_bd_ia32_ShlD);
    }
    ...
    if (is_Shl(shl_left) && is_Not(shl_right) &&
      ...
      return gen_64bit_shifts(node, shr_left, val_h, shr_right,
                              new_bd_ia32_ShrD);
    }
  }
...
static ir_node *gen_Rol(ir_node *node, ir_node *op1, ir_node *op2) {
  return gen_shift_binop(node, op1, op2, &new_bd_ia32_Rol,
                         &new_bd_ia32_Rol_8bit, match_none);
}

static ir_node *gen_Ror(ir_node *node, ir_node *op1, ir_node *op2) {
  return gen_shift_binop(node, op1, op2, &new_bd_ia32_Ror,
                         &new_bd_ia32_Ror_8bit, match_none);
}
...
static ir_node *gen_Add(ir_node *node) {
  ...
  ia32_address_mode_t am;
  match_arguments(&am, block, op1, op2, NULL,
                  match_commutative | match_mode_neutral | match_am |
                      match_immediate | match_try_am);
...
static ir_node *gen_Mul(ir_node *node) {
  ...
  construct_binop_func *const func =
      is_ia32_Immediate(am.new_op2) ? new_bd_ia32_IMulImm : new_bd_ia32_IMul;
...
static ir_node *gen_Sub(ir_node *node) {
  ...
  if (is_Const(op2)) be_warningf(node, "found unoptimized Sub with Const");

  ir_node *ia32_sub =
      gen_binop(node, op1, op2, new_bd_ia32_Sub,
...
static ir_node *transform_AM_mem(ir_node *const block, ir_node *const src_val,
                                 ir_node *const src_mem,
                                 ir_node *const am_mem) {
  if (is_NoMem(am_mem)) {
    return be_transform_node(src_mem);
  } else if (is_Proj(src_val) && is_Proj(src_mem) &&
    ...
    for (int i = arity - 1; i >= 0; --i) {
      ...
      /* avoid memory loop */
      if (is_Proj(pred) && get_Proj_pred(pred) == ptr_pred) continue;
 ...
 */
static ir_node *create_sex_32_64(dbg_info *const dbgi, ir_node *const block,
                                 ir_node *const val) {
 ...
 */
static ir_node *create_Div(ir_node *const node, ir_node *const op1,
                           ir_node *const op2, ir_node *const mem,
                           ir_mode *const mode) {
  ...
  match_flags_t const flags =
      (is_Pin(mem) && is_Sync(mem_pin_skip) ? match_none : match_am) |
      (is_signed ? match_sign_ext : match_zero_ext);
  ...
  ir_node *const new_mem =
      transform_AM_mem(block, op2_skip, mem_pin_skip, addr->mem);

  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *ext;
  ir_node *(*cons)(dbg_info *db, ir_node *block, ir_node *base, ir_node *index,
                   ir_node *mem, ir_node *op1, ir_node *op2, ir_node *ext,
                   x86_insn_size_t size);
  if (is_signed) {
  ...
  ir_node *const new_node = cons(dbgi, block, addr->base, addr->index, new_mem,
                                 am.new_op2, am.new_op1, ext, X86_SIZE_32);
...
static ir_node *gen_Shl(ir_node *node) {
  ...
  return gen_shift_binop(node, left, right, &new_bd_ia32_Shl,
                         &new_bd_ia32_Shl_8bit, match_mode_neutral);
}
...
static ir_node *gen_Shr(ir_node *node) {
  ...
  return gen_shift_binop(node, left, right, &new_bd_ia32_Shr,
                         &new_bd_ia32_Shr_8bit, match_zero_ext);
}
...
static ir_node *gen_Shrs(ir_node *node) {
  ...
  return gen_shift_binop(node, left, right, &new_bd_ia32_Sar,
                         &new_bd_ia32_Sar_8bit, match_sign_ext);
}
...
static ir_node *gen_Minus(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    if (ia32_cg_config.use_sse2) {
      ...
      x86_insn_size_t const size = x86_size_from_mode(mode);
      ir_node *const new_node = new_bd_ia32_Xorp(
          dbgi, block, base, noreg_GP, nomem, new_op, noreg_xmm, size);

      ir_entity *const ent = ia32_gen_fp_known_const(
          size == X86_SIZE_32 ? ia32_SSIGN : ia32_DSIGN);
...
static ir_node *create_float_abs(dbg_info *const dbgi, ir_node *const new_block,
                                 ir_node *const op, bool const negate) {
  ...
  if (ia32_cg_config.use_sse2) {
    ...
    ir_node *const base = get_global_base(irg);
    x86_insn_size_t const size = x86_size_from_mode(mode);
    new_node = new_bd_ia32_Andp(dbgi, new_block, base, noreg_GP, nomem, new_op,
                                noreg_fp, size);

    ir_entity *ent =
        ia32_gen_fp_known_const(size == X86_SIZE_32 ? ia32_SABS : ia32_DABS);
    ...
    if (negate) new_node = new_bd_ia32_fchs(dbgi, new_block, new_node);
  }
  ...
  if (mode_is_float(mode)) {
    switch (relation) {
      case ir_relation_equal:
        return x86_cc_float_equal;
      case ir_relation_less:
        return x86_cc_float_below;
      case ir_relation_less_equal:
        return x86_cc_float_below_equal;
      case ir_relation_greater:
        return x86_cc_float_above;
      case ir_relation_greater_equal:
        return x86_cc_float_above_equal;
      case ir_relation_less_greater:
        return x86_cc_not_equal;
      case ir_relation_less_equal_greater:
        return x86_cc_not_parity;
      case ir_relation_unordered:
        return x86_cc_parity;
      case ir_relation_unordered_equal:
        return x86_cc_equal;
      case ir_relation_unordered_less:
        return x86_cc_float_unordered_below;
    ...
    switch (relation) {
      ...
      case ir_relation_unordered_less_equal:
      case ir_relation_less_equal:
        return x86_cc_less_equal;
      case ir_relation_unordered_greater:
      case ir_relation_greater:
        return x86_cc_greater;
      case ir_relation_unordered_greater_equal:
      case ir_relation_greater_equal:
    ...
    switch (relation) {
      ...
      case ir_relation_unordered_less_equal:
      case ir_relation_less_equal:
        return x86_cc_below_equal;
      case ir_relation_unordered_greater:
      case ir_relation_greater:
        return x86_cc_above;
      case ir_relation_unordered_greater_equal:
      case ir_relation_greater_equal:
        return x86_cc_above_equal;
      case ir_relation_unordered_less_greater:
      case ir_relation_less_greater:
        return x86_cc_not_equal;
      case ir_relation_less_equal_greater:
...
static bool is_bt_relation(ir_relation const relation, ir_mode *const mode) {
  return relation == ir_relation_equal ? true
         : mode_is_signed(mode)
...
static ir_node *get_flags_node(ir_node *cmp, x86_condition_code_t *cc_out) {
  ...
  if (!(relation & ir_relation_equal) && relation & ir_relation_less_greater)
    relation |= get_negated_relation(ir_get_possible_cmp_relations(l, r)) &
  ...
  *cc_out =
      ir_relation_to_x86_condition_code(relation, mode, overflow_possible);
...
static ir_node *gen_Load(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    if (ia32_cg_config.use_sse2) {
      ...
      new_node = new_bd_ia32_fld(dbgi, block, base, idx, new_mem, size);
    }
  } else {
    /* create a conv node with address mode for smaller modes */
    if (get_mode_size_bits(mode) < 32) {
      bool sign_extend = mode_is_signed(mode);
      new_node = new_bd_ia32_Conv_I2I(dbgi, block, base, idx, new_mem, noreg_GP,
                                      size, sign_extend);
      /* Conv_I2I is not mode_T by default. */
      set_irn_mode(new_node, mode_T);
    } else {
      new_node = new_bd_ia32_Load(dbgi, block, base, idx, new_mem, size, false);
    }
  }
...
static ir_node *use_dest_am(ir_node *block, ir_node *node, ir_node *mem,
  ...
  ir_node *const load = get_Proj_pred(node);
  if (!is_Load(load)) return NULL;
  if (get_nodes_block(load) != block) return NULL;

  /* store should have the same pointer as the load */
  if (get_Load_ptr(load) != ptr) return NULL;
  ...
  if (prevents_AM(block, load, mem)) return NULL;
  /* Store should be attached to the load via mem */
  assert(heights_reachable_in_block(heights, mem, load));
  return load;
}

static ir_node *start_dest_am(ia32_address_mode_t *const am,
                              ir_node *const block, ir_node *const op,
                              ir_node *const mem) {
...
static void finish_dest_am(ir_node *const new_node, ir_node *const load,
                           ia32_address_mode_t const *const am) {
...
static ir_node *dest_am_binop(ir_node *node, ir_node *op1, ir_node *op2,
  ...
  if (!load) {
    if (!(flags & match_commutative)) return NULL;
  ...
  ir_node *const new_node =
      cons(dbgi, block, am.addr.base, am.addr.index, new_mem, new_op, size);
...
static ir_node *dest_am_unop(ir_node *node, ir_node *op, ir_node *mem,
  ...
  dbg_info *const dbgi = get_irn_dbg_info(node);
  ir_node *const new_node =
      func(dbgi, block, am.addr.base, am.addr.index, new_mem, size);
  finish_dest_am(new_node, load, &am);
  return new_node;
}
...
static ir_node *try_create_SetMem(ir_node *node, ir_node *ptr, ir_node *mem) {
  ir_mode *const mode = get_irn_mode(node);
  x86_insn_size_t const size = x86_size_from_mode(mode);
  if (size != X86_SIZE_8) return NULL;
  ...
  if (cc & x86_cc_additional_float_cases) return NULL;
  if (negated) cc = x86_negate_condition_code(cc);
...
static ir_node *try_create_dest_am(ir_node *node) {
  ...
  if (!be_mode_needs_gp_reg(mode)) return NULL;

  /* store must be the only user of the val node */
  if (get_irn_n_edges(val) > 1) return NULL;

  /* value must be in the same block */
  if (get_nodes_block(node) != get_nodes_block(val)) return NULL;
  ...
  switch (get_irn_opcode(val)) {
    case iro_Add: {
      ...
      if (be_pattern_is_rotl(val, &rot_left, &rot_right)) {
        if (is_Minus(rot_right)) {
          ...
          new_node = dest_am_binop(val, rot_left, rot_right, mem, ptr, size,
                                   new_bd_ia32_RorMem, new_bd_ia32_RorMem,
      ...
      if (ia32_cg_config.use_incdec) {
        if (is_irn_one(op2)) {
          new_node = dest_am_unop(val, op1, mem, ptr, size, new_bd_ia32_IncMem);
          break;
        } else if (is_Const_Minus_1(op2)) {
          new_node = dest_am_unop(val, op1, mem, ptr, size, new_bd_ia32_DecMem);
          break;
        }
      }
    ...
    case iro_Sub: {
      ...
      if (is_Const(op2)) be_warningf(val, "found unoptimized Sub with Const");
      new_node =
          dest_am_binop(val, op1, op2, mem, ptr, size, new_bd_ia32_SubMem,
                        new_bd_ia32_SubMem_8bit, match_immediate, 'i');
      break;
    }
    case iro_And: {
    ...
    case iro_Or: {
      ...
      new_node = dest_am_binop(val, op1, op2, mem, ptr, size, new_bd_ia32_OrMem,
                               new_bd_ia32_OrMem_8bit,
                               match_commutative | match_immediate, 'i');
      break;
    }
    case iro_Eor: {
    ...
    case iro_Shl: {
      ...
      new_node =
          dest_am_binop(val, op1, op2, mem, ptr, size, new_bd_ia32_ShlMem,
                        new_bd_ia32_ShlMem, match_immediate, 'I');
      break;
    }
    case iro_Shr: {
      ...
      new_node =
          dest_am_binop(val, op1, op2, mem, ptr, size, new_bd_ia32_ShrMem,
                        new_bd_ia32_ShrMem, match_immediate, 'I');
      break;
    }
    case iro_Shrs: {
      ...
      new_node =
          dest_am_binop(val, op1, op2, mem, ptr, size, new_bd_ia32_SarMem,
                        new_bd_ia32_SarMem, match_immediate, 'I');
      break;
    }
    case iro_Mux:
...
static ir_node *gen_float_const_Store(ir_node *node, ir_node *cns) {
  ...
  do {
    ...
    ir_node *const new_node = new_bd_ia32_Store(
        dbgi, new_block, addr.base, addr.index, addr.mem, imm, size);
...
static ir_node *gen_fist(dbg_info *dbgi, ir_node *block, ir_node *base,
  ...
  if (ia32_cg_config.use_fisttp) {
    ...
    if (size == X86_SIZE_64) {
      res = new_bd_ia32_fistp(dbgi, block, base, index, mem, val, trunc_mode,
                              size);
    } else {
      assert(size == X86_SIZE_16 || size == X86_SIZE_32);
      res = new_bd_ia32_fist(dbgi, block, base, index, mem, val, trunc_mode,
                             size);
    }
  }
  return res;
 ...
 * @return the created ia32 Store node
 */
static ir_node *create_store(dbg_info *dbgi, ir_node *new_block, ir_node *value,
                             const x86_address_t *addr) {
  ...
  if (mode_is_float(mode)) {
    if (ia32_cg_config.use_sse2) {
      ...
      ir_node *new_val = be_transform_node(value);
      store = create_fst(dbgi, new_block, addr->base, addr->index, addr->mem,
                         new_val, size);
    }
    ...
    store = size == X86_SIZE_8
                ? new_bd_ia32_Store_8bit(dbgi, new_block, addr->base,
                                         addr->index, addr->mem, new_val, size)
...
static ir_node *gen_Store(ir_node *node) {
  ...
  if (destam_node != NULL) return destam_node;
...
static ir_node *gen_Switch(ir_node *node) {
  ...
  if (get_mode_size_bits(sel_mode) < 32) new_sel = transform_zext(sel);

  ir_type *const utype = get_unknown_type();
  ir_entity *const entity = new_global_entity(
      irp->dummy_owner, id_unique("TBL"), utype, ir_visibility_private,
      IR_LINKAGE_CONSTANT | IR_LINKAGE_NO_IDENTITY);
  ...
  if (pic_style == BE_PIC_NONE) {
    switchjmp = new_bd_ia32_SwitchJmp(dbgi, block, base, new_sel, n_outs, table,
                                      entity);
    table_am = switchjmp;
  } else {
    assert(pic_style == BE_PIC_ELF_PLT || pic_style == BE_PIC_ELF_NO_PLT ||
    ...
    table_am = add;
    switchjmp = new_bd_ia32_SwitchJmp(dbgi, block, add, noreg_GP, n_outs, table,
                                      entity);
  }
...
static ir_node *create_Fucom(ir_node *node) {
  ...
  if (ia32_cg_config.use_fucomi) {
    ...
    new_node = new_bd_ia32_Fucomi(dbgi, new_block, new_left, new_right, 0);
    ...
    if (is_irn_null(right)) {
      ...
      ir_node *new_right = be_transform_node(right);
      new_node =
          new_bd_ia32_FucomFnstsw(dbgi, new_block, new_left, new_right, 0);
      set_ia32_commutative(new_node);
    }
...
static ir_node *create_Ucomi(ir_node *node) {
  ...
  dbg_info *dbgi = get_irn_dbg_info(node);
  ir_node *new_node =
      new_bd_ia32_Ucomis(dbgi, new_block, addr->base, addr->index, addr->mem,
                         am.new_op1, am.new_op2, am.ins_permuted);
  set_am_attributes(new_node, &am);
  new_node = fix_mem_proj(new_node, &am);
  return new_node;
}
...
static ir_node *gen_Cmp(ir_node *node) {
  ...
  ir_node *right = get_Cmp_right(node);
  ir_node *new_node;
  if (is_irn_null(right) && is_And(left) && get_irn_n_edges(left) == 1) {
    ...
    if (am.op_type == ia32_Normal && be_upper_bits_clean(and_left, cmp_mode) &&
        be_upper_bits_clean(and_right, cmp_mode)) {
      am.size = X86_SIZE_32;
    }

    new_node = am.size == X86_SIZE_8
                   ? new_bd_ia32_Test_8bit(dbgi, new_block, addr->base,
                                           addr->index, addr->mem, am.new_op1,
                                           am.new_op2, am.size, am.ins_permuted)
                   : new_bd_ia32_Test(dbgi, new_block, addr->base, addr->index,
                                      addr->mem, am.new_op1, am.new_op2,
                                      am.size, am.ins_permuted);
  } else {
    /* Cmp(left, right) */
    ...
    match_arguments(&am, block, left, right, NULL,
                    match_commutative | match_am | match_8bit_am |
                        ...
                        match_immediate);
    /* use 32bit compare mode if possible since the opcode is smaller */
    if (am.op_type == ia32_Normal && be_upper_bits_clean(left, cmp_mode) &&
        be_upper_bits_clean(right, cmp_mode)) {
      am.size = X86_SIZE_32;
    }

    new_node = am.size == X86_SIZE_8
                   ? new_bd_ia32_Cmp_8bit(dbgi, new_block, addr->base,
                                          addr->index, addr->mem, am.new_op1,
                                          am.new_op2, am.size, am.ins_permuted)
                   : new_bd_ia32_Cmp(dbgi, new_block, addr->base, addr->index,
                                     addr->mem, am.new_op1, am.new_op2, am.size,
                                     am.ins_permuted);
  }
  set_am_attributes(new_node, &am);
  new_node = fix_mem_proj(new_node, &am);
...
static ir_node *create_CMov(ir_node *node, ir_node *flags, ir_node *new_flags,
  ...
  ir_node *block = get_nodes_block(node);
  match_arguments(
      &am, block, val_false, val_true, flags,
      match_commutative | match_am | match_16bit_am | match_mode_neutral);

  if (am.ins_permuted) cc = x86_negate_condition_code(cc);

  x86_address_t *addr = &am.addr;
  dbg_info *dbgi = get_irn_dbg_info(node);
  ir_node *new_block = be_transform_node(block);
  ir_node *new_node =
      new_bd_ia32_CMovcc(dbgi, new_block, addr->base, addr->index, addr->mem,
                         am.new_op1, am.new_op2, new_flags, X86_SIZE_32, cc);
  set_am_attributes(new_node, &am);
  new_node = fix_mem_proj(new_node, &am);
  return new_node;
}
...
static ir_node *create_doz(ir_node *psi, ir_node *a, ir_node *b) {
  ir_node *new_node = gen_binop(
      psi, a, b, new_bd_ia32_Sub,
  ...
  new_node = new_bd_ia32_And(dbgi, block, noreg_GP, noreg_GP, nomem, new_node,
                             notn, X86_SIZE_32);
  set_ia32_commutative(new_node);
  return new_node;
}
...
static ir_entity *ia32_create_const_array(ir_node *c0, ir_node *c1,
  ...
  ir_entity *ent = new_global_entity(
      get_glob_type(), id_unique("C"), tp, ir_visibility_private,
      IR_LINKAGE_CONSTANT | IR_LINKAGE_NO_IDENTITY);

  ir_initializer_t *initializer = create_initializer_compound(2);

  set_initializer_compound_value(initializer, 0,
                                 create_initializer_tarval(tv0));
  set_initializer_compound_value(initializer, 1,
                                 create_initializer_tarval(tv1));
 ...
 * tv_t and tv_f.
 */
static void find_const_transform(x86_condition_code_t cc, ir_tarval *t,
                                 ir_tarval *f, setcc_transform_t *res) {
  unsigned step = 0;
  ...
  if (!tarval_is_null(f)) {
    if (tarval_is_all_one(t)) {
      ...
      if (t == tarval_bad) panic("constant subtract failed");
    }

    if (!tarval_is_long(f)) panic("tarval is not long");
  ...
  if (tarval_is_long(t)) {
    ...
    switch (v) {
      case 9:
        if (step > 0 && res->steps[step - 1].transform == SETCC_TR_ADD) --step;
        res->steps[step].transform = SETCC_TR_LEAxx;
        res->steps[step].log_scale = 3; /* (a << 3) + a */
        break;
      case 8:
        if (step > 0 && res->steps[step - 1].transform == SETCC_TR_ADD) --step;
        res->steps[step].transform =
            res->steps[step].val == 0 ? SETCC_TR_SHL : SETCC_TR_LEA;
...
static ir_node *create_Conv_I2I(dbg_info *dbgi, ir_node *block, ir_node *base,
  ...
  func = size == X86_SIZE_8 ? new_bd_ia32_Conv_I2I_8bit : new_bd_ia32_Conv_I2I;
...
static ir_node *gen_Mux(ir_node *node) {
  ...
  /* Note: a Mux node uses a Load two times IFF it's used in the compare AND in
   * the result */
  if (mode_is_float(mode)) {
    ...
    if (is_Const(mux_true) && is_Const(mux_false)) {
      ...
      typedef ir_node *(cons_t)(dbg_info *, ir_node *, ir_node *, ir_node *,
                                ir_node *, x86_insn_size_t);

      cons_t *const cons =
          ia32_cg_config.use_sse2 ? &new_bd_ia32_xLoad : &new_bd_ia32_fld;
      ir_graph *const irg = get_irn_irg(new_block);
      ir_node *const base = get_global_base(irg);
    ...
    if (is_Const(mux_true) && is_Const(mux_false)) {
      ...
      for (unsigned step = res.num_steps; step-- != 0;) {
        ...
        switch (s->transform) {
          case SETCC_TR_ADD: {
            new_node =
                create_lea(dbgi, new_block, new_node, noreg_GP, 0, s->val);
            continue;
          }
          ...
          case SETCC_TR_LEAxx: {
            new_node = create_lea(dbgi, new_block, new_node, new_node,
                                  s->log_scale, s->val);
            continue;
          }
          ...
          case SETCC_TR_SHL: {
            ir_graph *const irg = get_irn_irg(new_block);
            ir_node *const imm = ia32_create_Immediate(irg, s->log_scale);
            new_node =
                new_bd_ia32_Shl(dbgi, new_block, new_node, imm, X86_SIZE_32);
            continue;
          }

          case SETCC_TR_NEG:
            new_node = new_bd_ia32_Neg(dbgi, new_block, new_node, X86_SIZE_32);
            continue;

          case SETCC_TR_NOT:
            new_node = new_bd_ia32_Not(dbgi, new_block, new_node, X86_SIZE_32);
            continue;

          case SETCC_TR_AND: {
            ir_graph *const irg = get_irn_irg(new_block);
            ir_node *const imm = ia32_create_Immediate(irg, s->val);
            new_node = new_bd_ia32_And(dbgi, new_block, noreg_GP, noreg_GP,
                                       nomem, new_node, imm, X86_SIZE_32);
            continue;
          }
          ...
          case SETCC_TR_OR: {
            ir_graph *const irg = get_irn_irg(new_block);
            ir_node *const imm = ia32_create_Immediate(irg, s->val);
            new_node = new_bd_ia32_Or(dbgi, new_block, noreg_GP, noreg_GP,
                                      nomem, new_node, imm, X86_SIZE_32);
            continue;
          }
        }
...
static ir_node *gen_x87_fp_to_gp(ir_node *node) {
  ...
  /* do a Load */
  ir_node *load =
      new_bd_ia32_Load(dbgi, block, frame, noreg_GP, mem, X86_SIZE_32, false);
 ...
 */
static ir_node *gen_x87_conv(dbg_info *const dbgi, x86_insn_size_t const size,
                             ir_node *const node) {
  ir_node *block = get_nodes_block(node);
  ir_graph *irg = get_irn_irg(block);
  ir_node *frame = get_irg_frame(irg);

  ir_node *store = create_fst(dbgi, block, frame, noreg_GP, nomem, node, size);
  ...
  ir_node *load =
      new_bd_ia32_fld(dbgi, block, frame, noreg_GP, store_mem, size);
  ia32_attr_t *const load_attr = get_ia32_attr(load);
  load_attr->addr.variant = X86_ADDR_BASE;
...
static void store_gp(dbg_info *dbgi, ia32_address_mode_t *am, ir_node *block,
  ...
  if (!extend_unsigned) {
    ...
    if (am->op_type == ia32_AddrModeS) return;
  ...
  ir_node *frame = get_irg_frame(irg);
  ir_node *new_block = be_transform_node(block);
  ir_node *store = new_bd_ia32_Store(dbgi, new_block, frame, noreg_GP, nomem,
                                     new_node, X86_SIZE_32);
  ...
  if (!mode_is_signed(mode) && extend_unsigned) {
    ...
    ir_node *const zero_store = new_bd_ia32_Store(
        dbgi, new_block, frame, noreg_GP, nomem, zero_const, X86_SIZE_32);
 ...
 */
static ir_node *create_I2I_Conv(ir_mode *const src_mode, dbg_info *const dbgi,
                                ir_node *const block, ir_node *op) {
#ifdef DEBUG_libfirm
  if (is_Const(op)) be_warningf(op, "unoptimized conv after constant");
#endif

  op = be_skip_downconv(op, true);

  if (be_upper_bits_clean(op, src_mode)) return be_transform_node(op);
  ...
  x86_address_t *addr = &am.addr;
  ir_node *new_block = be_transform_node(block);
  ir_node *new_node = create_Conv_I2I(dbgi, new_block, addr->base, addr->index,
                                      addr->mem, am.new_op2, size, sign_extend);
  set_am_attributes(new_node, &am);
  ...
  if (mode_is_float(src_mode)) {
    ...
    if (mode_is_float(tgt_mode)) {
      ...
      if (ia32_cg_config.use_sse2) {
        ...
        res = new_bd_ia32_Conv_FP2FP(dbgi, new_block, noreg_GP, noreg_GP, nomem,
                                     new_op, size);
      } else {
      ...
      if (ia32_cg_config.use_sse2) {
        res = new_bd_ia32_Conv_FP2I(dbgi, new_block, noreg_GP, noreg_GP, nomem,
                                    new_op, x86_size_from_mode(src_mode));
      } else {
    ...
    if (mode_is_float(tgt_mode)) {
      ...
      if (ia32_cg_config.use_sse2) {
        ir_node *new_op = be_transform_node(op);
        res = new_bd_ia32_Conv_I2FP(dbgi, new_block, noreg_GP, noreg_GP, nomem,
                                    new_op, tgt_size);
      } else {
        unsigned int_mantissa =
            get_mode_size_bits(src_mode) - (mode_is_signed(src_mode) ? 1 : 0);
      ...
      /* to int */
      if (src_bits >= tgt_bits) {
        DB((dbg, LEVEL_1, "omitting unnecessary Conv(%+F, %+F) ...", src_mode,
            tgt_mode));
        return be_transform_node(op);
      }
...
static void store_fp(dbg_info *dbgi, ia32_address_mode_t *am, ir_node *block,
  ...
  ir_node *fst =
      create_fst(dbgi, new_block, frame, noreg_GP, nomem, new_value, size);
...
static ir_node *gen_Bitcast(ir_node *const node) {
  ...
  switch (get_mode_arithmetic(dst_mode)) {
    case irma_ieee754:
    case irma_x86_extended_float:
      ld = new_bd_ia32_fld(dbgi, new_block, addr->base, addr->index, addr->mem,
                           size);
...
static ir_node *gen_Start(ir_node *node) {
  ...
  for (size_t i = 0, n = cconv->n_parameters; i != n; ++i) {
    ...
    if (reg) outs[reg->global_index] = BE_START_REG;
  }

  /* callee saves */
  for (size_t i = 0; i < N_IA32_REGISTERS; ++i) {
    if (rbitset_is_set(cconv->callee_saves, i)) outs[i] = BE_START_REG;
  }
  if (!cconv->omit_fp) outs[REG_EBP] = BE_START_IGNORE;
...
static ir_node *gen_Proj_Start(ir_node *node) {
  ...
  switch ((pn_Start)pn) {
    ...
    case pn_Start_P_frame_base:
      return current_cconv->omit_fp ? get_initial_sp(irg) : get_initial_fp(irg);
  }
...
static ir_node *gen_Return(ir_node *node) {
  ...
  for (unsigned i = 0; i < N_IA32_REGISTERS; ++i) {
    if (!rbitset_is_set(cconv->callee_saves, i)) continue;
  ...
  ir_node *const ret = new_bd_ia32_Ret(dbgi, new_block, n_ins, in, reqs,
...
static ir_node *gen_Alloc(ir_node *node) {
  ...
  ir_node *const new_node = new_bd_ia32_SubSP(
      dbgi, new_block, noreg_GP, noreg_GP, new_mem, stack, new_size);

  ir_node *const stack_proj =
      be_new_Proj_reg(new_node, pn_ia32_SubSP_stack, &ia32_registers[REG_ESP]);
...
static ir_node *gen_Proj_Alloc(ir_node *node) {
  ...
  switch ((pn_Alloc)get_Proj_num(node)) {
    case pn_Alloc_M:
      return be_new_Proj(subsp, pn_ia32_SubSP_M);
    case pn_Alloc_res:
      return be_new_Proj(subsp, pn_ia32_SubSP_addr);
  }
...
static void adjust_pc_relative_relocation(ir_node *node) {
  if (!is_ia32_Immediate(node)) return;
  ia32_immediate_attr_t *attr = get_ia32_immediate_attr(node);
  if (attr->imm.kind == X86_IMM_ADDR) attr->imm.kind = X86_IMM_PCREL;
}
...
static ir_node *gen_IJmp(ir_node *node) {
  ...
  dbg_info *dbgi = get_irn_dbg_info(node);
  ir_node *new_block = be_transform_node(block);
  ir_node *new_node = new_bd_ia32_IJmp(dbgi, new_block, addr->base, addr->index,
                                       addr->mem, am.new_op2);
  set_am_attributes(new_node, &am);
...
static ir_node *gen_ia32_l_Add(ir_node *node) {
  ...
  ir_node *lowered = gen_binop(
      node, left, right, new_bd_ia32_Add,
      match_commutative | match_am | match_immediate | match_mode_neutral);
...
static ir_node *gen_ia32_l_Adc(ir_node *node) {
  return gen_binop_flags(
      node, new_bd_ia32_Adc,
      match_commutative | match_am | match_immediate | match_mode_neutral);
}
...
static ir_node *gen_ia32_l_LLtoFloat(ir_node *node) {
  if (ia32_cg_config.use_sse2) panic("not implemented for SSE2");
  ...
  /* do a fild */
  ir_node *fild =
      new_bd_ia32_fild(dbgi, block, frame, noreg_GP, sync, X86_SIZE_64);
  ...
  if (!mode_is_signed(get_irn_mode(val_high))) {
    ...
    ir_node *const index =
        new_bd_ia32_Shr(dbgi, block, new_val_high, count, X86_SIZE_32);
    ir_node *const noreg = ia32_new_NoReg_fp(irg);
    ir_node *const fpcw = get_initial_fpcw(irg);
    res = new_bd_ia32_fadd(dbgi, block, base, index, nomem, res, noreg, fpcw,
                           X86_SIZE_32);
...
static ir_node *gen_ia32_l_FloattoLL(ir_node *node) {
  ...
  ir_node *fist =
      gen_fist(dbgi, block, frame, noreg_GP, nomem, new_val, X86_SIZE_64);
...
static ir_node *gen_Proj_Load(ir_node *node) {
  ...
  assert(is_ia32_Conv_I2I(new_pred) || is_ia32_Load(new_pred) ||
         is_ia32_fld(new_pred) || is_ia32_xLoad(new_pred));
...
static ir_node *create_proj_for_store(ir_node *store, pn_Store pn) {
  if (get_irn_mode(store) == mode_M) {
    if (pn == pn_Store_M) return store;
  } else if (is_ia32_Store(store) || is_ia32_fist(store) ||
             is_ia32_fistp(store) || is_ia32_fisttp(store) ||
             is_ia32_xStore(store) || is_ia32_fst(store) ||
             is_ia32_fstp(store)) {
    switch (pn) {
      case pn_Store_M:
        return be_new_Proj(store, pn_ia32_st_M);
      case pn_Store_X_except:
        return be_new_Proj(store, pn_ia32_st_X_except);
      case pn_Store_X_regular:
        return be_new_Proj(store, pn_ia32_st_X_regular);
    }
...
static ir_node *gen_Proj_Store(ir_node *node) {
  ...
  if (is_Proj(new_pred)) new_pred = get_Proj_pred(new_pred);
...
static ir_node *gen_CopyB(ir_node *node) {
  ...
  if (size >= 32 * 4) {
    ...
    ir_node *cnst = new_bd_ia32_Const(dbgi, block, &imm);
    ir_node *copyb =
        new_bd_ia32_CopyB(dbgi, block, new_dst, new_src, cnst, new_mem, rem);
    projm = be_new_Proj(copyb, pn_ia32_CopyB_M);
  } else {
    if (size == 0) be_warningf(node, "unoptimized CopyB with size <4");
    ir_node *copyb =
        new_bd_ia32_CopyB_i(dbgi, block, new_dst, new_src, new_mem, size);
...
static ir_node *gen_Call(ir_node *node) {
  ...
  unsigned const n_ins = has_fpcw + in_arity + cconv->n_param_regs + is_plt;
  ...
  for (unsigned p = 0; p < n_params; ++p) {
    ...
    if (is_aggregate_type(param_type)) {
      ...
      ir_node *const lea =
          create_lea(dbgi, block, callframe, noreg_GP, 0, param->offset);
      ir_node *const new_value = be_transform_node(value);
      unsigned const size = get_type_size(param_type);
      ir_node *const copyb =
          new_bd_ia32_CopyB_i(dbgi, block, lea, new_value, mem, size);
  ...
  ir_node *const call = new_bd_ia32_Call(dbgi, block, in_arity, in, in_req,
                                         n_out, cconv->sp_delta, n_reg_results);
  arch_set_additional_pressure(call, &ia32_reg_classes[CLASS_ia32_gp],
                               add_pressure);

  if (get_irn_pinned(node)) set_irn_pinned(call, true);
  ...
  for (unsigned r = 0; r < n_ress; ++r) {
    ...
    if (reg->cls == &ia32_reg_classes[CLASS_ia32_fp]) ia32_request_x87_sim(irg);
  }

  /* Caller saves. */
  unsigned const *const allocatable_regs =
      be_birg_from_irg(irg)->allocatable_regs;
  for (unsigned i = 0; i < N_IA32_REGISTERS; ++i) {
    if (!rbitset_is_set(cconv->caller_saves, i)) continue;
    arch_register_t const *const reg = &ia32_registers[i];
    unsigned const op = o++;
...
static ir_node *create_frame_load(dbg_info *const dbgi, ir_node *const block,
  ...
  ir_node *const load =
      new_bd_ia32_Load(dbgi, block, base, noreg_GP, nomem, X86_SIZE_32, false);
  set_ia32_op_type(load, ia32_AddrModeS);
  ia32_attr_t *const attr = get_ia32_attr(load);
  attr->addr.variant = X86_ADDR_BASE;
  attr->addr.immediate = imm;
...
static ir_node *climb_frame(dbg_info *const dbgi, ir_node *const block,
  ...
  for (unsigned i = 0; i < levels; ++i) {
    x86_imm32_t const imm =
        i == 0 ? (x86_imm32_t){.kind = X86_IMM_FRAMEOFFSET, .offset = -4}
...
static ir_node *gen_return_address(ir_node *node) {
  ...
  if (value > 256) panic("Enormeous level argument for return_address builtin");

  ir_node *const base = climb_frame(dbgi, block, value);
  x86_imm32_t const imm =
      value == 0 ? (x86_imm32_t){.kind = X86_IMM_FRAMEOFFSET, .offset = 0}
...
static ir_node *gen_frame_address(ir_node *node) {
  ...
  ir_node *const n_frames = get_Builtin_param(node, 0);
  unsigned long const value = get_Const_long(n_frames);
  if (value > 256) panic("Enormeous level argument for frame_address builtin");
...
static ir_node *gen_prefetch(ir_node *node) {
  ...
  if (rw == 1 && ia32_cg_config.use_3dnow_prefetch) {
    ...
    long const locality =
        n_params > 2 ? get_Const_long(get_Builtin_param(node, 2)) : 3;
 ...
 */
static ir_node *gen_unop_AM(ir_node *const node,
                            construct_binop_dest_func *const func,
                            match_flags_t const flags) {
...
static ir_node *gen_parity(ir_node *node) {
  ...
  ir_node *const shr =
      new_bd_ia32_Shr(dbgi, new_block, new_param, count, X86_SIZE_32);
  ir_node *const xorn = new_bd_ia32_Xor(dbgi, new_block, noreg_GP, noreg_GP,
                                        nomem, shr, new_param, X86_SIZE_32);
  ...
  /* setp */
  ir_node *new_node =
      new_bd_ia32_Setcc(dbgi, new_block, flags, x86_cc_not_parity);

  /* conv to 32bit */
  return new_bd_ia32_Conv_I2I_8bit(dbgi, new_block, noreg_GP, noreg_GP, nomem,
                                   new_node, X86_SIZE_8, false);
}
...
static ir_node *gen_popcount(ir_node *node) {
  ...
  return gen_unop_AM(node, new_bd_ia32_Popcnt,
                     match_am | match_16bit_am | match_zero_ext);
}
  ...
  switch (size) {
    case 32:
      if (ia32_cg_config.use_bswap) {
        ...
        ir_node *const rol1 =
            new_bd_ia32_Rol(dbgi, new_block, new_param, i8, X86_SIZE_16);
        ir_node *const i16 = ia32_create_Immediate(irg, 16);
        ir_node *const rol2 =
            new_bd_ia32_Rol(dbgi, new_block, rol1, i16, X86_SIZE_32);
        ir_node *const rol3 =
            new_bd_ia32_Rol(dbgi, new_block, rol2, i8, X86_SIZE_16);
        return rol3;
      }
...
static ir_node *gen_saturating_increment(ir_node *node) {
  ...
  ir_node *increment = new_bd_ia32_Add(dbgi, block, noreg_GP, noreg_GP, nomem,
                                       operand, one, X86_SIZE_32);
  ...
  return new_bd_ia32_Sbb(dbgi, block, noreg_GP, noreg_GP, nomem, value, zero,
                         eflags, X86_SIZE_32);
}
...
static ir_node *gen_compare_swap(ir_node *node) {
  ...
  build_address_ptr(&addr, ptr, mem, x86_create_am_normal);
  x86_insn_size_t size = x86_size_from_mode(mode);
  ir_node *new_node = new_bd_ia32_CmpXChgMem(dbgi, block, addr.base, addr.index,
                                             addr.mem, new_old, new_new, size);
  set_irn_pinned(new_node, get_irn_pinned(node));
  set_ia32_op_type(new_node, ia32_AddrModeD);
  set_address(new_node, &addr);
  return new_node;
...
static ir_node *gen_va_start(ir_node *node) {
  if (initial_va_list == NULL) {
    ...
    initial_va_list =
        create_lea_frameaddress(dbgi, block, current_cconv->va_start_addr);
  }
...
void ia32_transform_graph(ir_graph *irg) {
  ...
  ir_node *const get_eip = ia32_get_irg_data(irg)->get_eip;
  if (get_eip != NULL) set_irn_mode(get_eip, ia32_mode_gp);
