...
static ir_entity *create_trampoline(be_main_env_t *be, ir_entity *method) {
  ...
  ir_entity *ent = new_global_entity(parent, id, type, ir_visibility_private,
                                     IR_LINKAGE_DEFAULT);
...
static ir_node *get_eip_relative(dbg_info *const dbgi, ir_graph *const irg,
  ...
  ir_node *const reloc =
      be_new_Relocation(dbgi, irg, (unsigned)kind, entity, offset_mode);
...
static ir_node *get_table_load(dbg_info *const dbgi, ir_graph *const irg,
                               x86_immediate_kind_t const kind,
                               ir_entity *const entity) {
  ...
  ir_node *const block = get_irg_start_block(irg);
  ir_node *const load =
      new_rd_Load(dbgi, block, nomem, addr, mode_P, type, cons_floats);
...
static void fix_address_elf(ir_node *const node, void *const data) {
  ...
  foreach_irn_in(node, i, pred) {
    ...
    if (is_tls_entity(entity)) continue;
