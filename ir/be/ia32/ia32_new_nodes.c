...
static const char *condition_code_name(x86_condition_code_t cc) {
  switch (cc) {
    ...
    case x86_cc_float_unordered_below:
      return "float unordered or below";
    case x86_cc_below:
      return "below";
    case x86_cc_float_above_equal:
      return "float above or equal";
    case x86_cc_float_unordered_above_equal:
      return "float unordered or above or equal";
    case x86_cc_above_equal:
      return "above or equal";
    ...
    case x86_cc_float_parity_cases:
      return "float parity cases";
    case x86_cc_additional_float_cases:
      return "additional float cases";
    default:
      return NULL;
  }
}
...
static bool has_ia32_condcode_attr(const ir_node *node) {
  return is_ia32_Setcc(node) || is_ia32_SetccMem(node) ||
         is_ia32_CMovcc(node) || is_ia32_Jcc(node) || is_ia32_Adc(node) ||
         is_ia32_Sbb(node) || is_ia32_Sbb0(node) || is_ia32_Cmc(node);
}
...
static void ia32_dump_immediate(FILE *const F, ir_entity *const entity,
  ...
  if (entity) {
    ...
    if (offset != 0) fprintf(F, "%+" PRId32, offset);
  } else {
...
const ia32_immediate_attr_t *get_ia32_immediate_attr_const(
    const ir_node *node) {
  ia32_attr_t const *const attr = get_ia32_attr_const(node);
...
void init_ia32_immediate_attributes(ir_node *res,
                                    x86_imm32_t const *const imm) {
  ia32_immediate_attr_t *attr =
      (ia32_immediate_attr_t *)get_irn_generic_attr(res);
...
void init_ia32_call_attributes(ir_node *res, uint8_t pop,
...
void init_ia32_condcode_attributes(ir_node *res, x86_condition_code_t cc) {
  ia32_condcode_attr_t *attr =
      (ia32_condcode_attr_t *)get_irn_generic_attr(res);
...
static int ia32_attrs_equal_(const ia32_attr_t *a, const ia32_attr_t *b) {
  ...
  return a->tp == b->tp && x86_addrs_equal(&a->addr, &b->addr) &&
         a->size == b->size && a->frame_use == b->frame_use &&
