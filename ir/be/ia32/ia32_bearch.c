...
/**
 * Returns the admissible noreg register node for input register pos of node
 * irn.
 */
...
static void ia32_set_frame_entity(ir_node *node, ir_entity *entity,
   ...
   * a certain mode */
  if (get_ia32_frame_use(node) != IA32_FRAME_USE_AUTO || is_ia32_Cmp(node) ||
...
static void ia32_determine_frameoffset_addr(ir_node *const node,
...
static void ia32_sp_sim(ir_node *const node, stack_pointer_state_t *state) {
  ...
  if (!state->no_change) ia32_determine_frameoffset(node, state->offset);
...
int ia32_get_sp_change(ir_node *const node) {
  if (be_is_IncSP(node)) return -be_get_IncSP_offset(node);
...
/**
 * Check if irn can load its operand at position i from memory (source
 * addressmode).
...
static bool ia32_possible_memory_operand(const ir_node *irn, unsigned int i) {
  if (!is_ia32_irn(irn) || /* must be an ia32 irn */
      get_ia32_op_type(irn) !=
          ia32_Normal || /* must not already be a addressmode irn */
      get_ia32_frame_use(irn) !=
          IA32_FRAME_USE_NONE) /* must not already use frame */
    return false;
  ...
  switch (get_ia32_am_support(irn)) {
    ...
    case ia32_am_unary:
      if (i != n_ia32_unary_op) return false;
      break;

    case ia32_am_binary:
      switch (i) {
        case n_ia32_binary_left: {
          if (!is_ia32_commutative(irn)) return false;

          /* we can't swap left/right for limited registers
           * (As this (currently) breaks constraint handling copies) */
          arch_register_req_t const *const req =
              arch_get_irn_register_req_in(irn, n_ia32_binary_left);
          if (req->limited != NULL) return false;
          break;
        }
  ...
  if (!is_NoMem(get_irn_n(irn, n_ia32_mem))) return false;
  ...
  if (attr->size > X86_SIZE_64) return false;
   ...
   * exist. */
  if (is_ia32_fld(load)) return false;

  return true;
}

static void ia32_perform_memory_operand(ir_node *irn, unsigned int i) {
  if (!ia32_possible_memory_operand(irn, i)) return;
  ...
  if (load_size <= dest_op_size) attr->size = load_size;
  set_ia32_op_type(irn, ia32_AddrModeS);
  set_ia32_frame_use(irn, IA32_FRAME_USE_AUTO);

  if (i == n_ia32_binary_left && get_ia32_am_support(irn) == ia32_am_binary &&
...
static ir_node *ia32_turn_back_dest_am(ir_node *node) {
  typedef ir_node *construct_binop_func(
      dbg_info * db, ir_node * block, ir_node * base, ir_node * index,
      ir_node * mem, ir_node * op1, ir_node * op2, x86_insn_size_t size);
  ...
  switch (get_ia32_irn_opcode(node)) {
    case iro_ia32_AddMem:
      func = is_8bit ? new_bd_ia32_Add_8bit : new_bd_ia32_Add;
      break;
    case iro_ia32_AndMem:
      func = is_8bit ? new_bd_ia32_And_8bit : new_bd_ia32_And;
      break;
    case iro_ia32_OrMem:
      func = is_8bit ? new_bd_ia32_Or_8bit : new_bd_ia32_Or;
      break;
    case iro_ia32_SubMem:
      func = is_8bit ? new_bd_ia32_Sub_8bit : new_bd_ia32_Sub;
      break;
    case iro_ia32_XorMem:
      func = is_8bit ? new_bd_ia32_Xor_8bit : new_bd_ia32_Xor;
      break;
    default:
  ...
  ir_node *const load =
      new_bd_ia32_Load(dbgi, block, base, idx, mem, size, false);
  ia32_copy_am_attrs(load, node);
  if (is_ia32_is_reload(node)) set_ia32_is_reload(load);
  ...
  ir_node *const new_node =
      func(dbgi, block, noreg, noreg, nomem, load_res, operand, size);
  set_irn_mode(new_node, mode_T);

  arch_set_irn_register_out(new_node, pn_ia32_flags,
                            &ia32_registers[REG_EFLAGS]);

  ir_node *const res_proj = be_new_Proj(new_node, pn_ia32_res);
  ir_node *const store =
      is_8bit
          ? new_bd_ia32_Store_8bit(dbgi, block, base, idx, load_mem, res_proj,
                                   size)
...
ir_node *ia32_turn_back_am(ir_node *node) {
  ...
  ir_node *load = new_bd_ia32_Load(dbgi, block, base, idx, mem, attr->size,
                                   attr->sign_extend);
  ...
  if (sched_is_scheduled(node)) sched_add_before(node, load);
...
static bool ia32_try_replace_flags(ir_node *consumers, ir_node *flags,
                                   ir_node *available) {
  if (!is_ia32_Sub(flags) && !is_ia32_Cmp(flags)) return false;
  unsigned pn;
  ...
  if (flags_left != avail_right) return false;
  ...
  if (avail_left != flags_right) return false;
  ...
  ir_node *const proj = get_irn_mode(available) == mode_T
                            ? be_get_or_make_Proj_for_pn(available, pn)
                            : available;
...
static void remat_simplifier(ir_node *node, void *env) {
  ...
  if (is_ia32_Sub(node) && get_irn_mode(node) == mode_T) {
    ...
    x86_insn_size_t const size = get_ia32_attr_const(node)->size;
    bool is_8bit = size == X86_SIZE_8;
    ir_node *cmp = is_8bit ? new_bd_ia32_Cmp_8bit(dbgi, block, base, idx, mem,
                                                  minu, subt, size, false)
                           : new_bd_ia32_Cmp(dbgi, block, base, idx, mem, minu,
                                             subt, size, false);
...
static ir_node *ia32_new_spill(ir_node *value, ir_node *after) {
  ...
  if (cls == &ia32_reg_classes[CLASS_ia32_gp]) {
    ...
    if (is_ia32_Load(skip)) size = get_ia32_attr_const(skip)->size;
    ...
    if (is_ia32_fld(skip)) size = get_ia32_attr_const(skip)->size;
...
static ir_node *ia32_new_reload(ir_node *value, ir_node *spill,
                                ir_node *before) {
  ...
  if (cls == &ia32_reg_classes[CLASS_ia32_gp]) {
    ...
    load = new_bd_ia32_xxLoad(NULL, block, frame, noreg, spill, X86_SIZE_128);
  }
...
static ir_node *create_push(ir_node *node, ir_node *schedpoint, ir_node *sp,
  ...
  ir_node *const push =
      new_bd_ia32_Push(dbgi, block, frame, noreg, mem, noreg, sp, size);
...
static void transform_MemPerm(ir_node *node) {
  ...
  for (int i = 0; i < arity; ++i) {
    ...
    if (entsize2 < entsize) entsize = entsize2;
...
static void ia32_collect_frame_entity_nodes(ir_node *node, void *data) {
  if (!is_ia32_irn(node) || get_ia32_op_type(node) != ia32_AddrModeS) return;
  ia32_attr_t const *const attr = get_ia32_attr_const(node);
  if (attr->addr.immediate.kind != X86_IMM_FRAMEENT) {
    assert(get_ia32_frame_use(node) == IA32_FRAME_USE_NONE);
    return;
  }
  if (attr->addr.immediate.entity != NULL) return;
...
static void introduce_epilogue(ir_node *const ret, bool const omit_fp) {
  ...
  if (!omit_fp) {
    ...
    int const n_ebp =
        be_get_input_pos_for_req(ret, &ia32_single_reg_req_gp_ebp);
    assert(n_ebp >= 0);
    ...
    if (ia32_cg_config.use_leave) {
      ...
      /* Pop ebp. */
      restore =
          new_bd_ia32_Pop_ebp(NULL, block, curr_mem, curr_sp, X86_SIZE_32);
  ...
  /* Keep verifier happy. */
  if (get_irn_n_edges(first_sp) == 0 && is_Proj(first_sp)) kill_node(first_sp);
}
...
static void introduce_prologue(ir_graph *const irg, bool omit_fp) {
  ...
  if (!omit_fp) {
    ...
    ir_node *const push = new_bd_ia32_Push(NULL, block, noreg, noreg, mem,
                                           initial_bp, initial_sp, X86_SIZE_32);
    ...
    ir_node *const incsp = ia32_new_IncSP(block, initial_sp, frame_size, false);
...
static void ia32_before_emit(ir_graph *irg) {
  ...
  if (ia32_get_irg_data(irg)->do_x87_sim) {
    ...
    const x87_simulator_config_t config = {
        ...
        .new_bd_ffreep = ia32_cg_config.use_ffreep ? new_bd_ia32_ffreep : NULL,
...
static bool mux_is_float_min_max(ir_node const *const sel,
  ...
  if (!is_Cmp(sel)) return false;

  ir_node *cmp_l = get_Cmp_left(sel);
  ir_node *cmp_r = get_Cmp_right(sel);
  if (!mode_is_float(get_irn_mode(cmp_l))) return false;
  ...
  switch (relation) {
    ...
    case ir_relation_greater:
      /* this is a max */
      if (cmp_l == mux_true && cmp_r == mux_false) return true;
      break;
    ...
    case ir_relation_unordered_greater:
      /* this is a min */
      if (cmp_l == mux_false && cmp_r == mux_true) return true;
      break;
...
static bool mux_is_set(ir_node const *const sel, ir_node const *const mux_true,
  ...
  if (!be_mode_needs_gp_reg(mode) && mode != mode_b) return false;

  /* we can create a set plus up two 3 instructions for any combination
   * of constants */
  if (is_Const(mux_true) && is_Const(mux_false)) return true;
...
static bool mux_is_float_const_const(ir_node const *const sel,
                                     ...
                                     ir_node const *const mux_false) {
  (void)sel;
  if (!mode_is_float(get_irn_mode(mux_true))) return false;
...
static bool mux_is_doz(ir_node const *const sel, ir_node const *mux_true,
  ...
  ir_mode *mode = get_irn_mode(mux_true);
  if (mode_is_signed(mode) || mode_is_float(mode)) return false;
  ...
  if (!is_irn_null(mux_false)) return false;
  if (!is_Sub(mux_true)) return false;
  ir_node *sub_left = get_Sub_left(mux_true);
  ir_node *sub_right = get_Sub_right(mux_true);

  /* Mux(a >=u b, 0, a-b) */
  if ((relation & ir_relation_greater) && sub_left == cmp_left &&
      sub_right == cmp_right)
    return true;
  /* Mux(a <=u b, 0, b-a) */
  if ((relation & ir_relation_less) && sub_left == cmp_right &&
      sub_right == cmp_left)
    return true;
...
static int ia32_is_mux_allowed(ir_node const *const sel,
  ...
  if (ir_is_optimizable_mux(sel, mux_false, mux_true)) return true;
  /* we can handle Set for all modes and compares */
  if (mux_is_set(sel, mux_true, mux_false)) return true;
  /* SSE has own min/max operations */
  if (ia32_cg_config.use_sse2 && mux_is_float_min_max(sel, mux_true, mux_false))
    return true;
  /* we can handle Mux(?, Const[f], Const[f]) */
  if (mux_is_float_const_const(sel, mux_true, mux_false)) return true;
  ...
  if (ir_mux_is_abs(sel, mux_false, mux_true) != 0) return true;
  /* we can't handle MuxF yet */
  if (mode_is_float(mode)) return false;

  if (mux_is_doz(sel, mux_true, mux_false)) return true;
  ...
  if (is_Cmp(sel)) {
    ...
    if (get_mode_size_bits(cmp_mode) > 32) return false;

    /* we can't handle float compares */
    if (mode_is_float(cmp_mode)) return false;
  }
...
static void ia32_init(void) {
  ...
  ia32_mode_float64 =
      new_float_mode("F64", irma_ieee754, 11, 52, ir_overflow_indefinite);
  ia32_mode_float32 =
      new_float_mode("F32", irma_ieee754, 8, 23, ir_overflow_indefinite);
...
static void ia32_mark_remat(ir_node *node) {
  if (is_ia32_irn(node)) set_ia32_is_remat(node);
}
...
static bool lower_for_emit(ir_graph *const irg,
                           const unsigned *const sp_is_non_ssa) {
  if (!be_step_first(irg)) return false;
  ...
  be_sched_fix_flags(irg, &ia32_reg_classes[CLASS_ia32_flags], &flags_remat,
                     NULL, &ia32_try_replace_flags);
...
static void ia32_generate_code(FILE *output, const char *cup_name) {
  ...
  foreach_irp_irg(i, irg) {
    if (!lower_for_emit(irg, sp_is_non_ssa)) continue;
...
static ir_jit_function_t *ia32_jit_compile(ir_jit_segment_t *const segment,
  ...
  if (!lower_for_emit(irg, sp_is_non_ssa)) return NULL;
  ...
  if (ir_platform.ia32_struct_in_regs && size <= 8) {
    if (get_compound_n_members(type) == 1) {
      ...
      if (is_float(member_type)) {
        ...
        if (member_size == 4) return float_spec;
        if (member_size == 8) return double_spec;
      }
    }
...
static void ia32_lower_for_target(void) {
  ...
  lower_calls_with_compounds(LF_RETURN_HIDDEN, dont_lower_aggregates, NULL,
                             decide_compound_ret, NULL, reset_stateless_abi);
  ...
  if (ia32_cg_config.use_popcnt) supported[s++] = ir_bk_popcount;
  if (ia32_cg_config.use_cmpxchg) supported[s++] = ir_bk_compare_swap;
...
static be_register_name_t const ia32_additional_reg_names[] = {
    {"al", REG_EAX}, {"ah", REG_EAX}, {"ax", REG_EAX}, {"bl", REG_EBX},
    {"bh", REG_EBX}, {"bx", REG_EBX}, {"cl", REG_ECX}, {"ch", REG_ECX},
    {"cx", REG_ECX}, {"dl", REG_EDX}, {"dh", REG_EDX}, {"dx", REG_EDX},
    {"si", REG_ESI}, {"di", REG_EDI}, {"sp", REG_ESP}, {"bp", REG_EBP},
    {NULL, ~0u}};
