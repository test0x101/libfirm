 ...
 */
static bool ia32_transform_sub_to_neg_add(
    ir_node *const irn, arch_register_t const *const out_reg) {
  /* in case of sub and OUT == SRC2 we can transform the sequence into neg
   * src2 -- add */
  ir_node *const in2 = get_irn_n(irn, n_ia32_binary_right);
  if (out_reg != arch_get_irn_register(in2)) return false;
  ...
  if (is_ia32_Subs(irn)) {
    ...
    res =
        new_bd_ia32_Xorp(dbgi, block, noreg, noreg, nomem, in2, noreg_fp, size);
    ir_entity *entity =
        ia32_gen_fp_known_const(size == X86_SIZE_32 ? ia32_SSIGN : ia32_DSIGN);
    ...
    /* generate the add */
    res = new_bd_ia32_Adds(dbgi, block, noreg, noreg, nomem, res, in1, size);
  } else {
    ...
    if (is_ia32_Sbb(irn)) {
       ...
       * Complement Carry
       *
       * a + -b = a + (~b + 1)  would set the carry flag wrong IFF both a and b
       * are zero.
       */
    ...
    carry:;
      ...
      ir_node *adc = new_bd_ia32_Adc(dbgi, block, noreg, noreg, nomem, nnot,
                                     in1, carry, X86_SIZE_32);
      ...
      if (flags_proj != NULL) {
        ...
        ir_node *const adc_flags =
            be_new_Proj_reg(adc, pn_ia32_Adc_flags, reg_flags);
      ...
      arch_set_irn_register_out(res, pn_ia32_flags,
 ...
 */
static void fix_am_source(ir_node *const irn,
                          arch_register_t const *const out_reg) {
  /* Only need to fix operations with source address mode. */
  if (get_ia32_op_type(irn) != ia32_AddrModeS) return;
...
static bool ia32_handle_2addr(ir_node *const node,
  ...
  if (is_ia32_Lea(node) || is_ia32_Minus64(node)) return true;
  fix_am_source(node, reg);
  if (req->should_be_same ==
      (1U << n_ia32_binary_left | 1U << n_ia32_binary_right)) {
