...
static lc_opt_enum_int_var_t arch_var = {(int *)&arch_val, arch_items};

static lc_opt_enum_int_var_t opt_arch_var = {(int *)&opt_arch_val, arch_items};
...
static lc_opt_enum_int_var_t fp_unit_var = {&fpu_arch, fp_unit_items};
...
static const lc_opt_table_entry_t ia32_architecture_options[] = {
    ...
    LC_OPT_ENT_ENUM_INT("arch", "select the instruction architecture",
                        &arch_var),
    LC_OPT_ENT_ENUM_INT("tune", "optimize for instruction architecture",
                        &opt_arch_var),
    LC_OPT_ENT_ENUM_INT("fpmath", "select the floating point unit",
                        &fp_unit_var),
    LC_OPT_ENT_BOOL("optcc", "optimize calling convention", &opt_cc),
    LC_OPT_ENT_BOOL("unsafe_floatconv",
                    "do unsafe floating point controlword optimizations",
                    &opt_unsafe_floatconv),
    LC_OPT_ENT_BOOL("machcode", "output machine code instead of assembler",
                    &emit_machcode),
    LC_OPT_ENT_BOOL("soft-float", "equivalent to fpmath=softfloat",
                    &use_softfloat),
...
typedef struct insn_const {
  ...
  unsigned
      function_alignment;   /**< logarithm for alignment of function labels */
  unsigned label_alignment; /**< logarithm for alignment of loops labels */
  unsigned label_alignment_max_skip; /**< maximum skip for alignment of loops
                                        labels */
} insn_const;
...
void ia32_setup_cg_config(void) {
  ...
  if (use_softfloat) fpu_arch = IA32_FPU_SOFTFLOAT;
...
#endif
  if (opt_arch_val == cpu_autodetect) opt_arch = arch;
   ...
   * longer opcode */
  c->use_leave =
      arch_flags(opt_arch, arch_i386 | arch_all_amd | arch_core2) || opt_size;
  /* P4s don't like inc/decs because they only partially write the flags
   * register which produces false dependencies */
  c->use_incdec =
      !arch_flags(opt_arch, arch_netburst | arch_nocona | arch_core2_plus |
                                arch_atom_plus | arch_geode) ||
      opt_size;
  c->use_softfloat = (fpu_arch & IA32_FPU_SOFTFLOAT) != 0;
  c->use_sse2 =
      (fpu_arch & IA32_FPU_SSE2) != 0 && feature_flags(arch, arch_feature_sse2);
  c->use_ffreep = arch_flags(opt_arch, arch_athlon_plus);
  c->use_femms = arch_flags(opt_arch, arch_athlon_plus) &&
                 feature_flags(arch, arch_feature_3DNow);
  c->use_fucomi = feature_flags(arch, arch_feature_fcmov);
  c->use_cmov = feature_flags(arch, arch_feature_cmov) && use_cmov;
  c->use_modeD_moves = arch_flags(
      opt_arch, arch_generic | arch_athlon_plus | arch_netburst | arch_nocona |
                    arch_core2_plus | arch_atom_plus | arch_ppro | arch_geode);
  c->use_add_esp_4 =
      arch_flags(opt_arch, arch_generic | arch_athlon_plus | arch_netburst |
                               arch_nocona | arch_core2_plus | arch_atom_plus |
                               arch_geode) &&
      !opt_size;
  c->use_add_esp_8 =
      arch_flags(opt_arch, arch_generic | arch_athlon_plus | arch_netburst |
                               arch_nocona | arch_core2_plus | arch_atom_plus |
                               arch_ppro | arch_geode | arch_i386 |
                               arch_i486) &&
      !opt_size;
  c->use_sub_esp_4 =
      arch_flags(opt_arch, arch_generic | arch_athlon_plus | arch_netburst |
  ...
  c->use_pad_return =
      arch_flags(opt_arch, arch_athlon | arch_k8 | arch_k10) && !opt_size;
  c->use_bt = arch_flags(opt_arch,
                         arch_core2_plus | arch_atom_plus | arch_athlon_plus) ||
              opt_size;
  c->use_fisttp = feature_flags(opt_arch, arch_feature_sse3) &&
                  feature_flags(arch, arch_feature_sse3);
  c->use_sse_prefetch =
      feature_flags(arch, (arch_feature_3DNowE | arch_feature_sse1));
