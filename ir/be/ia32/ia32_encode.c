...
static unsigned char pnc2cc(x86_condition_code_t cc) { return cc & 0xf; }

enum OpSize {
  OP_8 = 0x00,       /* 8bit operation. */
  OP_16_32 = 0x01,   /* 16/32bit operation. */
  OP_MEM_SRC = 0x02, /* The memory operand is in the source position. */
  OP_IMM8 = 0x02,    /* 8bit immediate, which gets sign extended for 16/32bit
                        operation. */
  OP_16_32_IMM8 =
      0x03,      /* 16/32bit operation with sign extended 8bit immediate. */
...
typedef enum reg_modifier { REG_LOW = 0, REG_HIGH = 4 } reg_modifier_t;
 ...
 *
 * @param reg   content of the reg field: either a register index or an opcode
 * extension
 * @param node  the node
 */
...
static void enc_mov(arch_register_t const *const src,
                    arch_register_t const *const dst) {
  be_emit8(0x88 | OP_MEM_SRC | OP_16_32);  // movl %src, %dst
  enc_modrr(src, dst);
}

static void enc_xchg(arch_register_t const *const src,
                     arch_register_t const *const dst) {
...
static void enc_xor0(const ir_node *node) {
  ...
  arch_register_t const *const out =
      arch_get_irn_register_out(node, pn_ia32_Xor0_res);
  enc_modrr(out, out);
}

static void enc_mov_const(const ir_node *node) {
  arch_register_t const *const out =
      arch_get_irn_register_out(node, pn_ia32_Const_res);
  be_emit8(0xB8 + out->encoding);
  enc_imm32(node);
}

void ia32_enc_simple(uint8_t const opcode) { be_emit8(opcode); }
...
void ia32_enc_unop_mem(ir_node const *const node, uint8_t const code,
  ...
  if (size == X86_SIZE_16) be_emit8(0x66);
  be_emit8(size == X86_SIZE_8 ? code : code | OP_16_32);
  enc_mod_am(ext, node);
}

static bool use_eax_short_form(ir_node const *const node) {
  return get_ia32_op_type(node) == ia32_Normal &&
         arch_get_irn_register_in(node, n_ia32_binary_left)->index ==
             REG_GP_EAX;
}

static void enc_binop_reg(ir_node const *const node, unsigned char const code,
                          ir_node const *const right) {
  be_emit8(code);
  arch_register_t const *const dst =
...
void ia32_enc_binop_mem(ir_node const *const node, unsigned const code) {
  ...
  if (is_ia32_Immediate(val)) {
    ia32_immediate_attr_t const *const attr =
        get_ia32_immediate_attr_const(val);
    /* Try to use the short form with 8bit sign extended immediate. */
    if (op != OP_8 && ia32_is_8bit_imm(attr)) {
      op = OP_16_32_IMM8;
      size = X86_SIZE_8;
...
static void enc_shld(const ir_node *node) {
  const arch_register_t *in =
      arch_get_irn_register_in(node, n_ia32_ShlD_val_low);
  const arch_register_t *out =
      arch_get_irn_register_out(node, pn_ia32_ShlD_res);
...
static void enc_shrd(const ir_node *node) {
  const arch_register_t *in =
      arch_get_irn_register_in(node, n_ia32_ShrD_val_low);
  const arch_register_t *out =
      arch_get_irn_register_out(node, pn_ia32_ShrD_res);
...
static void enc_sbb0(ir_node const *const node) {
  be_emit8(0x1B);
  arch_register_t const *const out =
      arch_get_irn_register_out(node, pn_ia32_Sbb0_res);
  enc_modrr(out, out);
}
...
static void enc_bt(ir_node const *const node) {
  be_emit8(0x0F);
  arch_register_t const *const lreg =
      arch_get_irn_register_in(node, n_ia32_Bt_left);
...
static void enc_ldtls(const ir_node *node) {
  be_emit8(0x65);  // gs:
  arch_register_t const *const out =
      arch_get_irn_register_out(node, pn_ia32_LdTls_res);
...
/* helper function for enc_minus64bit */
static void enc_helper_sbb(const arch_register_t *src,
...
static void enc_pop(const ir_node *node) {
  arch_register_t const *const reg =
      arch_get_irn_register_out(node, pn_ia32_Pop_res);
...
static void enc_call(ir_node const *const node) {
  ...
  if (is_ia32_Immediate(callee)) {
    x86_imm32_t const *const imm = &get_ia32_immediate_attr_const(callee)->imm;
...
static void enc_jump(const ir_node *node) {
  if (!be_is_fallthrough(node)) enc_jmp(node);
}
...
static void enc_subsp(const ir_node *node) {
  ...
  /* mov %esp, %out */
  arch_register_t const *const out =
      arch_get_irn_register_out(node, pn_ia32_SubSP_addr);
...
static void enc_fist(const ir_node *node) {
  ...
  switch (size) {
    ...
    case X86_SIZE_64:
      ...
      goto enc;  // fistpll
    enc:
      if (get_ia32_x87_attr_const(node)->x87.pop) ++op;
...
static void assign_block_fragment_num(ir_node *const block,
                                      unsigned const num) {
...
static void gen_binary_block(ir_node *const block) {
  ...
  uint8_t p2align = 0;
  uint8_t max_skip = 0;
  if (block != get_irg_end_block(irg) && ia32_cg_config.label_alignment > 0 &&
      ia32_should_align_block(block)) {
  ...
  assert(fragment_num == (unsigned)PTR_TO_INT(ir_nodehashmap_get(
                             void, &block_fragmentnum, block)));
  (void)fragment_num;
...
static unsigned enc_relocation_callback(char *const buffer,
  ...
  if (entity == NULL) {
    ...
    if (be_kind == X86_IMM_PCREL) addr -= (intptr_t)buffer;
    value = (uint32_t)addr;
    if ((intptr_t)value != addr) panic("Overflow in relocation");
  }
