...
typedef enum { no_carry = 0, can_carry, must_carry } carry_result;
...
static carry_result lower_add_carry(ir_node *left, ir_node *right,
                                    ir_mode *mode) {
...
static carry_result lower_sub_borrow(ir_node *left, ir_node *right,
                                     ir_mode *mode) {
  assert(!mode_is_signed(mode));
...
static void ia32_lower_add64(ir_node *const node) {
  ...
  } else if (cr == must_carry &&
             (is_Const(left_high) || is_Const(right_high))) {
...
static void ia32_lower_mul64(ir_node *const node) {
  ...
  if (is_sign_extend(left_low, left_high) &&
    ...
    if (is_irn_null(left_low) || is_irn_one(right_low)) {
      ...
      ir_node *const lo =
          new_rd_Proj(dbg, ll_rl, ia32_mode_gp, pn_ia32_l_Mul_res_low);
...
static void ia32_lower_conv64(ir_node *const node) {
  ...
  if (mode_is_float(mode_from) && get_mode_size_bits(mode_to) == 64 &&
    ...
    if (mode_is_signed(mode)) {
      ...
      l_res =
          new_r_Proj(float_to_ll, ia32_mode_gp, pn_ia32_l_FloattoLL_res_low);
      h_res = new_r_Proj(float_to_ll, mode, pn_ia32_l_FloattoLL_res_high);
    } else {
      ...
      ir_node *cmp =
          new_rd_Cmp(dbg, upper_blk, opc, flt_corr, ir_relation_less);
      ir_node *cond = new_rd_Cond(dbg, upper_blk, cmp);
      ir_node *in[] = {new_r_Proj(cond, mode_X, pn_Cond_true),
      ...
      /* create to Phis */
      ir_node *phi_in[] = {new_r_Const_null(irg, mode),
                           new_r_Const_long(irg, mode, 0x80000000)};
      ir_node *int_phi = new_r_Phi(lower_blk, ARRAY_SIZE(phi_in), phi_in, mode);

      ir_node *fphi_in[] = {opc, new_rd_Sub(dbg, upper_blk, opc, flt_corr)};
      ir_node *flt_phi =
          new_r_Phi(lower_blk, ARRAY_SIZE(fphi_in), fphi_in, x86_mode_E);

      /* fix Phi links for next part_block() */
      if (is_Phi(int_phi)) add_Block_phi(lower_blk, int_phi);
      if (is_Phi(flt_phi)) add_Block_phi(lower_blk, flt_phi);
...
static void ia32_lower_ASM(ir_node *const asmn) {
  ...
  if (pos_out_cons == pos_none) return;

  size_t const new_n_constraints = n_constraints + 1;
  ir_asm_constraint *const new_constraints =
      ALLOCAN(ir_asm_constraint, new_n_constraints);
  int const pos_out = constraints[pos_out_cons].out_pos;
  for (size_t i = 0; i != n_constraints; ++i) {
    ir_asm_constraint *const c = &new_constraints[i];
    *c = constraints[i];
    if (c->out_pos > pos_out) c->out_pos += 1;
  }
  ...
  if (get_irn_pinned(asmn) == op_pin_state_floats) flags |= cons_floats;
  if (ir_throws_exception(asmn)) flags |= cons_throws_exception;
  ...
  ir_node *const new_asm =
      new_rd_ASM(dbgi, block, mem, n_ins, ins, asm_text, new_n_constraints,
                 new_constraints, n_clobber, clobbers, flags);
...
void ia32_lower64(void) {
  ...
  lwrdw_param_t lower_dw_params = {
      word_unsigned, word_signed, 64 /* doubleword size */
  };
