...
static const lc_opt_table_entry_t options[] = {
    LC_OPT_ENT_BOOL("movespills", "try to move spills out of loops",
                    &move_spills),
    LC_OPT_ENT_BOOL("respectloopdepth", "outermost loop cutting",
                    &respectloopdepth),
    LC_OPT_ENT_BOOL("improveknownpreds", "known preds cutting",
                    &improve_known_preds),
    LC_OPT_ENT_INT("rematbonus", "give bonus to rematerialisable nodes",
                   &remat_bonus),
    LC_OPT_LAST};
 ...
 */
static void workset_bulk_fill(workset_t *workset, int count,
                              const loc_t *locs) {
  workset->len = count;
 ...
 */
static void workset_clear(workset_t *workset) { workset->len = 0; }
...
static void workset_remove(workset_t *workset, ir_node *val) {
  ...
  if (loc) *loc = workset->vals[--workset->len];
}
...
static inline unsigned workset_get_time(const workset_t *workset,
                                        unsigned idx) {
...
static unsigned get_distance(ir_node *from, const ir_node *def,
  ...
  if (USES_IS_INFINITE(time)) return USES_INFINITY;
...
static void displace(workset_t *const new_vals, bool const is_usage,
                     ir_node *const instr,
                     be_add_pressure_t const add_pressure) {
...
static void decide_start_workset(ir_node *const block) {
  ...
  sched_foreach_phi(block, node) {
    ...
    available_t available = AVAILABLE_UNKNOWN;
    if (all_preds_known) {
      available = available_in_all_preds(pred_worksets, n_cfgpreds, node, true);
    }
  ...
  be_lv_foreach_cls(lv, block, be_lv_state_in, cls, node) {
    ...
    if (all_preds_known) {
      available =
          available_in_all_preds(pred_worksets, n_cfgpreds, node, false);
    }
    ...
    for (size_t i = 0; i < ARR_LEN(delayed) && free_slots > 0; ++i) {
      ...
      if (!is_Phi(loc->node)) {
        ...
        for (int p = 0; p < n_cfgpreds; ++p) {
          ...
          if (pred_info == NULL) continue;

          if (!workset_contains(pred_info->end_workset, loc->node)) {
            DB((dbg, DBG_START, "    delayed %+F not live at pred %+F\n",
                loc->node, pred_block));
            goto skip_delayed;
          }
        }
  ...
  for (size_t i = ws_count, len = ARR_LEN(starters); i < len; ++i) {
    ...
    if (!is_Phi(node) || get_nodes_block(node) != block) continue;
...
static void process_block(ir_node *block) {
  ...
  sched_foreach_non_phi(block, irn) {
    ...
    be_add_pressure_t const add_pressure =
        arch_get_additional_pressure(irn, cls);
    displace(new_vals, true, irn, MAX(add_pressure, 0));

    /* allocate all values _defined_ by this instruction */
    workset_clear(new_vals);
    be_foreach_definition(irn, cls, value, req, assert(req->width == 1);
...
static void fix_block_borders(ir_node *block, void *data) {
  ...
  for (int i = 0, n_cfgpreds = get_Block_n_cfgpreds(block); i < n_cfgpreds;
    ...
    workset_foreach(pred_end_workset, node, iter) {
      ...
      if (workset_contains(start_workset, node)) continue;
    ...
    workset_foreach(start_workset, node, iter) {
      ...
      if (pred_loc != NULL) {
        ...
        if (move_spills && !pred_loc->spilled && l->spilled) {
          ir_node *insert_point = be_get_end_of_block_insertion_point(pred);
          insert_point = sched_prev(insert_point);
          DB((dbg, DBG_SPILL, "Spill %+F after %+F\n", node, insert_point));
          be_add_spill(senv, node, insert_point);
        }
