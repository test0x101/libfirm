...
static void set_reg_free(unsigned *free_regs, ir_node const *irn,
                         bool const reg_is_free) {
  if (!mode_is_data(get_irn_mode(irn))) return;
...
static void mark_live_nodes_registers(const ir_node *irn, lower_env_t *env) {
  ...
  arch_register_class_t const *cls =
      arch_get_irn_register(get_irn_n(irn, 0))->cls;
  be_irg_t *birg = be_birg_from_irg(irg);
  unsigned n_regs = ir_target.isa->n_registers;
  unsigned *free_regs = rbitset_duplicate_obstack_alloc(
      &env->obst, birg->allocatable_regs, n_regs);
...
static void live_nodes_registers_walker(ir_node *irn, void *env) {
  if (!be_is_Perm(irn)) return;

  mark_live_nodes_registers(irn, (lower_env_t *)env);
}

static arch_register_t const *get_free_register(ir_node *const perm,
                                                lower_env_t *env) {
  if (!env->use_copies) return NULL;

  ir_node *block = get_nodes_block(perm);
  arch_register_class_t const *cls =
      arch_get_irn_register(get_irn_n(perm, 0))->cls;
  unsigned *free_regs = (unsigned *)ir_nodehashmap_get(arch_register_t const,
                                                       &env->live_regs, perm);
  ...
  sched_foreach_non_phi_reverse(block, node) {
    ...
    be_foreach_value(node, value, set_reg_free(free_regs, value, reg_is_free););

    foreach_irn_in(node, i, in) { set_reg_free(free_regs, in, false); }
 ...
 */
static void lower_perm_node(ir_node *const perm,
                            arch_register_class_t const *const cls,
                            unsigned const arity, ir_node **const projs,
                            lower_env_t *const env) {
  ...
  for (unsigned i = 0; i != n_regs; ++i) {
    if (rbitset_is_set(inregs, i)) continue;
    ...
    for (reg_pair_t const *p; (p = oregmap[k]);) {
      oregmap[k] = NULL;
      ir_node *const copy =
          be_new_Copy_before_reg(p->in_node, perm, p->out_reg);
      DBG((dbg, LEVEL_2, "%+F: inserting %+F for %+F from %s to %s\n", perm,
           copy, p->in_node, p->in_reg, p->out_reg));
  ...
  if (use_copies && free_reg != NULL) {
    ...
    for (unsigned i = 0; i != n_regs; /* empty */) {
      ...
      ir_node *const save_copy =
          be_new_Copy_before_reg(start->in_node, perm, free_reg);
      ...
      ir_node *const restore_copy =
          be_new_Copy_before_reg(save_copy, perm, start->out_reg);
    ...
    for (unsigned i = 0; i != n_regs;) {
      ...
      for (;;) {
        ...
        DBG((dbg, LEVEL_2, "%+F: inserting %+F for %+F (%s) and %+F (%s)\n",
             perm, xchg, in[0], arch_get_irn_register(in[0]), in[1],
             arch_get_irn_register(in[1])));
 ...
 */
static bool push_through_perm(ir_node *const perm,
                              arch_register_class_t const *const cls,
                              unsigned const arity, ir_node **const projs) {
  ...
  for (ir_node *prev; !sched_is_begin(node); node = prev) {
    ...
    if (be_is_Copy(node)) {
      ...
      arch_register_t const *const reg_copy =
          arch_get_irn_register_out(node, 0);
      if (reg_copy == reg_op) {
    ...
    for (unsigned pn = 0; pn != arity; ++pn) {
      ...
      if (in == node || (is_Proj(in) && get_Proj_pred(in) == node)) {
        ...
        foreach_irn_in(node, i, node_in) {
          if (node_in == in) set_irn_n(node, i, proj);
        }
      }
    }
  }
...
static void lower_nodes_after_ra_walker(ir_node *irn, void *walk_env) {
  ...
  arch_register_class_t const *const cls =
      arch_get_irn_register_req_out(perm, 0)->cls;
