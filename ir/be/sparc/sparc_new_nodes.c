...
static bool has_fp_attr(const ir_node *node) {
  return is_sparc_fadd(node) || is_sparc_fsub(node) || is_sparc_fmul(node) ||
         is_sparc_fdiv(node) || is_sparc_fftoi(node) || is_sparc_fitof(node) ||
    ...
    case dump_node_info_txt: {
      ...
      if (sparc_has_load_store_attr(n)) {
        const sparc_load_store_attr_t *attr =
            get_sparc_load_store_attr_const(n);
      ...
      if (is_sparc_cond_branch(n)) {
        const sparc_jmp_cond_attr_t *attr = get_sparc_jmp_cond_attr_const(n);
...
const sparc_jmp_cond_attr_t *get_sparc_jmp_cond_attr_const(
    const ir_node *node) {
  assert(is_sparc_cond_branch(node));
...
int sparc_jmp_cond_attrs_equal(const ir_node *a, const ir_node *b) {
  ...
  return sparc_attrs_equal(a, b) && attr_a->relation == attr_b->relation &&
...
int sparc_fp_conv_attrs_equal(const ir_node *a, const ir_node *b) {
  ...
  return sparc_attrs_equal(a, b) && attr_a->src_mode == attr_b->src_mode &&
