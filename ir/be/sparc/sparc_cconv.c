...
static const arch_register_t *const float_result_regs[] = {
    &sparc_registers[REG_F0], &sparc_registers[REG_F1],
    &sparc_registers[REG_F2], &sparc_registers[REG_F3],
    &sparc_registers[REG_F4], &sparc_registers[REG_F5],
    &sparc_registers[REG_F6], &sparc_registers[REG_F7],
};
...
static const unsigned caller_saves[] = {
    REG_G1,  REG_G2,  REG_G3,  REG_O0,  REG_O1,  REG_O2,  REG_O3,  REG_O4,
    REG_O5,  REG_F0,  REG_F1,  REG_F2,  REG_F3,  REG_F4,  REG_F5,  REG_F6,
    REG_F7,  REG_F8,  REG_F9,  REG_F10, REG_F11, REG_F12, REG_F13, REG_F14,
    REG_F15, REG_F16, REG_F17, REG_F18, REG_F19, REG_F20, REG_F21, REG_F22,
    REG_F23, REG_F24, REG_F25, REG_F26, REG_F27, REG_F28, REG_F29, REG_F30,
    REG_PSR, REG_FSR, REG_Y,
};
static unsigned default_caller_saves[BITSET_SIZE_ELEMS(N_SPARC_REGISTERS)];

static const unsigned returns_twice_saved[] = {REG_SP, REG_FP, REG_I7};
static unsigned
    default_returns_twice_saves[BITSET_SIZE_ELEMS(N_SPARC_REGISTERS)];
...
calling_convention_t *sparc_decide_calling_convention(ir_type *function_type,
  ...
  if (irg != NULL) {
     ...
     * args 0-5 in it */
    if (is_method_variadic(function_type)) omit_fp = false;
    ...
    if (get_method_calling_convention(function_type) & cc_compound_ret)
      omit_fp = false;
    if (omit_fp) irg_walk_graph(irg, check_omit_fp, NULL, &omit_fp);
  ...
  if (mtp & mtp_property_returns_twice) {
    rbitset_copy(caller_saves, default_returns_twice_saves, N_SPARC_REGISTERS);
  } else {
  ...
  for (int i = 0; i < n_params; ++i) {
    ...
    if (regnum < n_param_regs) {
      ...
      if (irg == NULL || omit_fp) reg = map_i_to_o_reg(reg);
      param->reg0 = reg;
    ...
    if (bits > 32) {
      if (bits > 64) panic("only 32 and 64bit modes supported");

      if (regnum < n_param_regs) {
        param->offset =
            SPARC_PARAMS_SPILL_OFFSET + regnum * SPARC_REGISTER_SIZE;
        arch_register_t const *reg = param_regs[regnum++];
        if (irg == NULL || omit_fp) reg = map_i_to_o_reg(reg);
        param->reg1 = reg;
  ...
  if (irg != NULL) {
    ...
    birg->allocatable_regs =
        be_cconv_alloc_all_regs(&birg->obst, N_SPARC_REGISTERS);
...
void sparc_cconv_init(void) {
  be_cconv_add_regs(default_caller_saves, caller_saves,
                    ARRAY_SIZE(caller_saves));

  rbitset_set_all(default_returns_twice_saves, N_SPARC_REGISTERS);
  be_cconv_rem_regs(default_returns_twice_saves, returns_twice_saved,
                    ARRAY_SIZE(returns_twice_saved));
  be_cconv_rem_regs(default_returns_twice_saves, ignore_regs,
                    ARRAY_SIZE(ignore_regs));
