...
static void sparc_emit_load_mode(ir_node const *const node) {
  ...
  switch (bits) {
    ...
    case 128:
      ...
      break;
    default:
      panic("invalid load/store mode %+F", mode);
  }
}
...
static bool is_no_instruction(const ir_node *node) {
  ...
  if (be_is_Copy(node) || be_is_CopyKeep(node)) {
    ...
    if (src_reg == dest_reg) return true;
  }
  if (be_is_IncSP(node) && be_get_IncSP_offset(node) == 0) return true;
  /* Ba is not emitted if it is a simple fallthrough */
  if (is_sparc_Ba(node) && be_is_fallthrough(node)) return true;

  return be_is_Keep(node) || be_is_Start(node) || be_is_Unknown(node) ||
         is_Phi(node);
}
...
static bool emits_multiple_instructions(const ir_node *node) {
  if (has_delay_slot(node)) return true;

  if (is_sparc_Call(node))
    return arch_get_irn_flags(node) & sparc_arch_irn_flag_aggregate_return;

  return is_sparc_SMulh(node) || is_sparc_UMulh(node) || is_sparc_SDiv(node) ||
         is_sparc_UDiv(node) || be_is_MemPerm(node) || be_is_Perm(node) ||
...
static bool writes_reg(const ir_node *node, unsigned reg_index,
                       unsigned width) {
  be_foreach_out(node, o) {
    const arch_register_t *out_reg = arch_get_irn_register_out(node, o);
    if (out_reg == NULL) continue;
...
static bool is_legal_delay_slot_filler(const ir_node *node) {
  if (is_no_instruction(node)) return false;
  if (emits_multiple_instructions(node)) return false;
  if (rbitset_is_set(delay_slot_fillers, get_irn_idx(node))) return false;
  if (is_sparc_Save(node)) return false;
  return true;
}

static bool can_move_down_into_delayslot(const ir_node *node,
                                         const ir_node *to) {
  if (!is_legal_delay_slot_filler(node)) return false;
  ...
  if (is_sparc_Call(to)) {
    ...
    if (!is_sparc_reg_call(to)) return true;

    check = get_irn_n(to, get_sparc_Call_dest_addr_pos(to));
    if (skip_Proj(check) == node) return false;
...
static bool can_move_up_into_delayslot(const ir_node *node, const ir_node *to) {
  if (!be_can_move_up(heights, node, to)) return false;

  /* node must not use any results of 'to' */
  foreach_irn_in(node, i, in) {
    ir_node *skipped = skip_Proj(in);
    if (skipped == to) return false;
  }
  ...
  if (is_restore(node)) {
    ...
    int dest_addr_pos =
        is_sparc_reg_call(to) ? get_sparc_Call_dest_addr_pos(to) : -1;

    int call_arity = get_irn_arity(to);
    for (int i = 0; i < call_arity; ++i) {
      if (i == dest_addr_pos) continue;
      ...
      if (writes_reg(node, reg->global_index, req->width)) return false;
    }
    ...
    for (int i = 0, arity = get_irn_arity(to); i < arity; ++i) {
      assert((unsigned)n_sparc_SDiv_dividend_high ==
             (unsigned)n_sparc_UDiv_dividend_high);
      if (i == n_sparc_SDiv_dividend_high) continue;
      const arch_register_t *reg = arch_get_irn_register_in(to, i);
      if (reg == NULL) continue;
      const arch_register_req_t *req = arch_get_irn_register_req_in(to, i);
      if (writes_reg(node, reg->global_index, req->width)) return false;
...
static ir_node *pick_delay_slot_for(ir_node *node) {
  ...
  if (is_sparc_cond_branch(node)) optimize_fallthrough(node);

  unsigned tries = 0;
  sched_foreach_reverse_before(node, schedpoint) {
    if (has_delay_slot(schedpoint)) break;
    if (tries++ >= PICK_DELAY_SLOT_MAX_DISTANCE) break;

    if (!can_move_down_into_delayslot(schedpoint, node)) continue;
  ...
  sched_foreach_after(node, schedpoint) {
    if (has_delay_slot(schedpoint)) break;
    if (tries++ >= PICK_DELAY_SLOT_MAX_DISTANCE) break;
    if (!is_legal_delay_slot_filler(schedpoint)) continue;
    if (!can_move_up_into_delayslot(schedpoint, node)) continue;
  ...
  foreach_block_succ(block, edge) {
    ...
    if (get_Block_n_cfgpreds(succ) > 1) continue;
    ...
    sched_foreach(succ, schedpoint) {
      ...
      if (is_restore(schedpoint)) continue;
      if (tries++ >= PICK_DELAY_SLOT_MAX_DISTANCE) break;
      if (!is_legal_delay_slot_filler(schedpoint)) continue;
      if (!get_irn_pinned(schedpoint) && !is_memop(schedpoint) &&
          can_move_up_into_delayslot(schedpoint, node)) {
  ...
  BE_EMITF(node, fmt, ap, emitting_delay_slot) {
    ...
    switch (*fmt++) {
      ...
      case 'E': {
        ...
        if (attr->immediate_value != 0) {
          be_emit_irprintf(plus ? "%+" PRId32 : "%" PRId32,
      ...
      case 'F': {
        ...
        switch (*fmt++) {
          case 'D':
            mode = get_sparc_fp_conv_attr_const(node)->dest_mode;
            break;
          case 'M':
            mode = get_sparc_fp_attr_const(node)->fp_mode;
            break;
          case 'S':
            mode = get_sparc_fp_conv_attr_const(node)->src_mode;
            break;
          default:
            goto unknown;
        }
      ...
      case 'S': {
        ...
        if (!is_digit(*fmt)) goto unknown;
        unsigned const pos = *fmt++ - '0';
        if (imm && arch_get_irn_flags(node) &
...
static void emit_sparc_asm_operand(ir_node const *const node,
                                   char const modifier, unsigned const pos) {
  be_asm_attr_t const *const attr = get_be_asm_attr_const(node);
  sparc_asm_operand_t const *const op =
      &((sparc_asm_operand_t const *)attr->operands)[pos];
  /* modifiers:
   *   c: plain immediate
  ...
  if (!be_is_valid_asm_operand_kind(node, modifier, pos, op->op.kind, "r", "c",
                                    "fm"))
    return;
...
static void memperm_emit_spill_registers(const ir_node *node, int n_spilled,
  ...
  if (n_to_spill == 2) {
    sparc_emitf(node, "st %%l1, [%%sp%+d]",
                SPARC_MIN_STACKSIZE + SPARC_REGISTER_SIZE);
  }
}
...
static void emit_be_MemPerm(const ir_node *node) {
  ...
  for (int i = 0; i < memperm_arity; ++i) {
    ...
    if (get_index(entities, n, out) == -1) entities[n++] = out;
    if (get_index(entities, n, in) == -1) entities[n++] = in;
...
static void emit_sparc_Return(const ir_node *node) {
  ...
  char const *const destreg =
      delay_slot && is_restore(delay_slot) ? "%i7" : "%o7";
  char const *const offset =
      get_method_calling_convention(type) & cc_compound_ret ? "12" : "8";
...
static const arch_register_t *map_i_to_o_reg(const arch_register_t *reg) {
  ...
  if (idx < REG_I0 || idx > REG_I7) return reg;
  idx += REG_O0 - REG_I0;
...
static void emit_sparc_Ba(const ir_node *node) {
  BE_EMIT_JMP(sparc, node, "ba", node) { fill_delay_slot(node); }
}
...
static void sparc_emit_block(ir_node *block) {
  ...
  sched_foreach(block, node) {
    if (rbitset_is_set(delay_slot_fillers, get_irn_idx(node))) continue;
    be_emit_node(node);
  }
}
...
static void pick_delay_slots(size_t n_blocks, ir_node **blocks) {
  ...
  for (size_t i = 0; i < n_blocks; ++i) {
    sched_foreach(sorted_blocks[i], node) {
      if (!has_delay_slot(node)) continue;
...
void sparc_init_emitter(void) { FIRM_DBG_REGISTER(dbg, "firm.be.sparc.emit"); }
