...
ir_entity *sparc_get_frame_entity(const ir_node *node) {
  if (be_is_MemPerm(node)) return be_get_MemPerm_in_entity(node, 0);
...
static void sparc_determine_frameoffset(ir_node *const node,
  ...
  if (is_sparc_FrameAddr(node)) {
    ...
    ir_entity const *const entity = attr->base.immediate_value_entity;
    if (entity != NULL) {
...
static void sparc_sp_sim(ir_node *const node, stack_pointer_state_t *state) {
  ...
  if (is_sparc_Save(node)) {
    sparc_attr_t *const attr = get_sparc_attr(node);
    if (get_irn_arity(node) == 3) panic("no support for _reg variant yet");

    /* Adjust for alignment */
    assert(state->misalign == 0);
    int const prev_offset = state->offset;
    int const new_offset =
        prev_offset - state->align_padding - attr->immediate_value;
...
void sparc_fix_stack_bias(ir_graph *irg) {
  ...
  be_sim_stack_pointer(irg, misalign, SPARC_PO2_STACK_ALIGNMENT, sparc_sp_sim);
}
...
static void sparc_layout_param_entities(ir_graph *const irg,
                                        calling_convention_t *const cconv,
                                        ir_type *const non_lowered) {
