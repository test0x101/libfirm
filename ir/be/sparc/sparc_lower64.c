...
static void lower64_add(ir_node *const node) {
  ...
  ir_node *addcc = new_bd_sparc_AddCC_t(dbgi, block, left_low, right_low);
  ...
  ir_node *addx =
      new_bd_sparc_AddX_t(dbgi, block, left_high, right_high, res_flags, mode);
...
static void lower64_sub(ir_node *const node) {
  ...
  ir_node *right_low = get_lowered_low(right);
  ir_node *right_high = get_lowered_high(right);
  ir_node *subcc = new_bd_sparc_SubCC_t(dbgi, block, left_low, right_low);
  ...
  ir_mode *mode = get_node_high_mode(node);
  ir_node *subx =
      new_bd_sparc_SubX_t(dbgi, block, left_high, right_high, res_flags, mode);
