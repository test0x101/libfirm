...
static const arch_register_t *const omit_fp_callee_saves[] = {
    &sparc_registers[REG_L0], &sparc_registers[REG_L1],
    &sparc_registers[REG_L2], &sparc_registers[REG_L3],
    &sparc_registers[REG_L4], &sparc_registers[REG_L5],
    &sparc_registers[REG_L6], &sparc_registers[REG_L7],
    &sparc_registers[REG_I0], &sparc_registers[REG_I1],
    &sparc_registers[REG_I2], &sparc_registers[REG_I3],
    &sparc_registers[REG_I4], &sparc_registers[REG_I5],
};
...
static ir_node *gen_sign_extension(dbg_info *dbgi, ir_node *block, ir_node *op,
  ...
  ir_node *lshift_node =
      new_bd_sparc_Sll_imm(dbgi, block, op, NULL, shift_width);
  ir_node *rshift_node =
      new_bd_sparc_Sra_imm(dbgi, block, lshift_node, NULL, shift_width);
  return rshift_node;
}
...
typedef ir_node *(*new_binop_reg_func)(dbg_info *dbgi, ir_node *block,
                                       ir_node *op1, ir_node *op2);
typedef ir_node *(*new_binop_fp_func)(dbg_info *dbgi, ir_node *block,
                                      ir_node *op1, ir_node *op2,
                                      ir_mode *mode);
typedef ir_node *(*new_binop_imm_func)(dbg_info *dbgi, ir_node *block,
                                       ir_node *op1, ir_entity *entity,
                                       int32_t immediate);
typedef ir_node *(*new_unop_fp_func)(dbg_info *dbgi, ir_node *block,
                                     ir_node *op1, ir_mode *mode);

/**
 * checks if a node's value can be encoded as a immediate
 */
static bool is_imm_encodeable(const ir_node *node) {
  if (!is_Const(node)) return false;
...
static bool needs_extension(ir_node *op) {
  ...
  if (get_mode_size_bits(mode) >= gp_bits) return false;
  return !be_upper_bits_clean(op, mode);
}

static void sparc_parse_constraint_letter(void const *const env,
                                          be_asm_constraint_t *const c,
                                          char const l) {
  (void)env;
...
void sparc_init_asm_constraints(void) {
  ...
  be_set_constraint_support(ASM_CONSTRAINT_FLAG_SUPPORTS_IMMEDIATE,
                            "AIJKLMOPin");
...
static bool sparc_check_immediate_constraint(long const val,
                                             char const imm_type) {
  switch (imm_type) {
    case 'A':
      return -16 <= val && val < 16;
    case 'I':
      return sparc_is_value_imm_encodeable(val);
    case 'J':
      return val == 0;
    case 'K':
      return (val & 0x03FF) == 0;
    case 'L':
      return -1024 <= val && val < 1024;
    case 'M':
      return -512 <= val && val < 512;
    case 'O':
      return val == 4096;
    case 'P':
      return val == -1;
...
static bool sparc_match_immediate(sparc_asm_operand_t *const operand,
                                  ir_node *const node, char const imm_type) {
  ir_tarval *offset;
  ir_entity *entity;
  unsigned reloc_kind;
  if (!be_match_immediate(node, &offset, &entity, &reloc_kind)) return false;

  if (entity && imm_type != 'g' && imm_type != 'i') return false;

  long value;
  if (offset) {
    value = get_tarval_long(offset);
    if (!sparc_check_immediate_constraint(value, imm_type)) return false;
  } else {
    value = 0;
  }
...
static ir_node *gen_ASM(ir_node *node) {
  ...
  sparc_asm_operand_t *const operands =
      NEW_ARR_DZ(sparc_asm_operand_t, obst, n_constraints);
  for (size_t i = 0; i != n_constraints; ++i) {
    ir_asm_constraint const *const c = &constraints[i];

    be_asm_constraint_t be_constraint;
    be_parse_asm_constraints_internal(&be_constraint, c->constraint,
                                      &sparc_parse_constraint_letter, NULL);
    ...
    if (in_pos >= 0) {
      ...
      if (imm != '\0' && sparc_match_immediate(op, in, imm)) {
        ...
        arch_register_req_t const *const ireq =
            be_make_register_req(obst, &be_constraint);
        be_asm_add_inout(&info, &op->op, obst, in, ireq, c->out_pos);
      } else {
        ir_node *const new_in = be_transform_node(in);
        arch_register_req_t const *const ireq =
            arch_get_irn_register_req(new_in)->cls->class_req;
  ...
  for (size_t c = 0; c < get_ASM_n_clobbers(node); ++c) {
    ...
    if (streq(clobber, "cc")) {
      ARR_APP1(arch_register_req_t const *, info.out_reqs,
               &sparc_single_reg_req_flags_psr);
      ARR_APP1(arch_register_req_t const *, info.out_reqs,
               &sparc_single_reg_req_fpflags_fsr);
    }
  }
...
static ir_node *gen_helper_binop_left(ir_node *left, dbg_info *dbgi,
  ...
  if (flags & MATCH_MODE_NEUTRAL) return new_left;

  int bits = get_mode_size_bits(mode);
  int gp_bits = get_mode_size_bits(mode_gp);

  if (bits >= gp_bits) return new_left;
 ...
 */
static ir_node *gen_helper_binop_args(ir_node *node, ir_node *op1, ir_node *op2,
                                      match_flags_t flags,
...
static ir_node *gen_Add(ir_node *node) {
  ...
  if (is_Const(right)) {
    ...
    if (is_Address(left) && get_irn_n_edges(left) == 1) {
      ...
      return new_bd_sparc_Or_imm(dbgi, block, address.ptr, address.entity,
                                 address.offset);
    }
...
static ir_node *gen_AddCC_t(ir_node *node) {
  ...
  ir_node *new_node = gen_helper_binop_args(
      node, left, right, MATCH_COMMUTATIVE | MATCH_MODE_NEUTRAL,
      new_bd_sparc_AddCC_reg, new_bd_sparc_AddCC_imm);
  arch_set_irn_register_out(new_node, pn_sparc_AddCC_flags,
...
static ir_node *gen_Sub(ir_node *node) {
  ...
  return gen_helper_binop(node, MATCH_MODE_NEUTRAL, new_bd_sparc_Sub_reg,
...
static ir_node *gen_SubCC_t(ir_node *node) {
  ...
  ir_node *new_node =
      gen_helper_binop_args(node, left, right, MATCH_MODE_NEUTRAL,
                            new_bd_sparc_SubCC_reg, new_bd_sparc_SubCC_imm);
  arch_set_irn_register_out(new_node, pn_sparc_SubCC_flags,
                            &sparc_registers[REG_PSR]);
...
static ir_node *gen_SubX_t(ir_node *node) {
  return gen_helper_binopx(node, MATCH_MODE_NEUTRAL, new_bd_sparc_SubX_reg,
                           new_bd_sparc_SubX_imm);
}

ir_node *create_ldf(dbg_info *dbgi, ir_node *block, ir_node *ptr, ir_node *mem,
                    ir_mode *mode, ir_entity *entity, long offset,
                    bool is_frame_entity) {
  unsigned bits = get_mode_size_bits(mode);
  assert(mode_is_float(mode));
  if (bits == 32) {
    return new_bd_sparc_Ldf_s(dbgi, block, ptr, mem, mode, entity, offset,
                              is_frame_entity);
  } else if (bits == 64) {
    return new_bd_sparc_Ldf_d(dbgi, block, ptr, mem, mode, entity, offset,
                              is_frame_entity);
  } else {
    assert(bits == 128);
    return new_bd_sparc_Ldf_q(dbgi, block, ptr, mem, mode, entity, offset,
                              is_frame_entity);
  }
}
...
ir_node *create_stf(dbg_info *dbgi, ir_node *block, ir_node *value,
                    ...
                    ir_entity *entity, long offset, bool is_frame_entity) {
  unsigned bits = get_mode_size_bits(mode);
  assert(mode_is_float(mode));
  if (bits == 32) {
...
static ir_node *gen_Load(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    if (address.ptr2 != NULL) {
      ...
      new_load = new_bd_sparc_Ld_reg(dbgi, block, new_mem, address.ptr,
                                     address.ptr2, mode);
    } else {
      new_load = new_bd_sparc_Ld_imm(dbgi, block, new_mem, address.ptr, mode,
                                     address.entity, address.offset, false);
    }
  }
...
static ir_node *gen_Store(ir_node *node) {
  ...
  if (mode_is_float(mode)) {
    ...
    address_t address;
    match_address(ptr, &address, false);
    new_store = create_stf(dbgi, block, new_val, address.ptr, new_mem, mode,
                           address.entity, address.offset, false);
  } else {
    ...
    if (address.ptr2 != NULL) {
      assert(address.entity == NULL && address.offset == 0);
      new_store = new_bd_sparc_St_reg(dbgi, block, new_mem, new_val,
                                      address.ptr, address.ptr2, mode);
    } else {
      new_store =
          new_bd_sparc_St_imm(dbgi, block, new_mem, new_val, address.ptr, mode,
                              address.entity, address.offset, false);
    }
  }
...
static ir_node *gen_Mulh(ir_node *node) {
  ir_mode *mode = get_irn_mode(node);
  if (mode_is_float(mode)) panic("FP not supported yet");

  if (mode_is_signed(mode)) {
    return gen_helper_binop(node, MATCH_COMMUTATIVE, new_bd_sparc_SMulh_reg,
                            new_bd_sparc_SMulh_imm);
  } else {
    return gen_helper_binop(node, MATCH_COMMUTATIVE, new_bd_sparc_UMulh_reg,
...
static ir_node *gen_helper_bitop(ir_node *node, new_binop_reg_func new_reg,
                                 new_binop_imm_func new_imm,
  ...
  ir_node *op2 = get_binop_right(node);
  if (is_Not(op1)) {
    return gen_helper_binop_args(node, op2, get_Not_op(op1), flags, new_not_reg,
                                 new_not_imm);
  }
  if (is_Not(op2)) {
    return gen_helper_binop_args(node, op1, get_Not_op(op2), flags, new_not_reg,
                                 new_not_imm);
  }
  if (is_Const(op2) && get_irn_n_edges(op2) == 1) {
    ...
    if (!sparc_is_value_imm_encodeable(value)) {
      ...
      if ((notvalue & 0x3ff) == 0) {
        ir_node *new_block = be_transform_nodes_block(node);
        dbg_info *dbgi = get_irn_dbg_info(node);
        ir_node *new_op2 = new_bd_sparc_SetHi(NULL, new_block, NULL, notvalue);
        ir_node *new_op1 = be_transform_node(op1);
        ir_node *result = new_not_reg(dbgi, new_block, new_op1, new_op2);
        return result;
      }
    }
  }
  return gen_helper_binop_args(node, op1, op2, flags | MATCH_COMMUTATIVE,
                               new_reg, new_imm);
}

static ir_node *gen_And(ir_node *node) {
  return gen_helper_bitop(node, new_bd_sparc_And_reg, new_bd_sparc_And_imm,
                          new_bd_sparc_AndN_reg, new_bd_sparc_AndN_imm,
                          MATCH_MODE_NEUTRAL);
}

static ir_node *gen_Or(ir_node *node) {
  return gen_helper_bitop(node, new_bd_sparc_Or_reg, new_bd_sparc_Or_imm,
                          new_bd_sparc_OrN_reg, new_bd_sparc_OrN_imm,
                          MATCH_MODE_NEUTRAL);
}

static ir_node *gen_Eor(ir_node *node) {
  return gen_helper_bitop(node, new_bd_sparc_Xor_reg, new_bd_sparc_Xor_imm,
                          new_bd_sparc_XNor_reg, new_bd_sparc_XNor_imm,
                          MATCH_MODE_NEUTRAL);
}
...
static ir_node *gen_Shl(ir_node *node) {
  ...
  return gen_helper_binop(node, MATCH_NONE, new_bd_sparc_Sll_reg,
...
static ir_node *gen_Shr(ir_node *node) {
  ...
  if (get_mode_modulo_shift(mode) != 32)
    panic("modulo_shift!=32 not supported");
  return gen_helper_binop(node, MATCH_ZERO_EXT_LEFT, new_bd_sparc_Srl_reg,
                          new_bd_sparc_Srl_imm);
}
...
static ir_node *gen_Shrs(ir_node *node) {
  ...
  if (get_mode_modulo_shift(mode) != 32)
    panic("modulo_shift!=32 not supported");
  return gen_helper_binop(node, MATCH_SIGN_EXT_LEFT, new_bd_sparc_Sra_reg,
...
static ir_node *gen_float_const(dbg_info *dbgi, ir_node *block, ir_tarval *tv) {
  ...
  ir_mode *mode = get_tarval_mode(tv);
  ir_node *new_op = create_ldf(dbgi, block, hi, mem, mode, entity, 0, false);
...
static ir_node *gen_Switch(ir_node *node) {
  ...
  /* load from jumptable */
  ir_node *load = new_bd_sparc_Ld_reg(dbgi, new_block, get_irg_no_mem(irg),
                                      table_address, idx, mode_gp);
  ir_node *address = be_new_Proj(load, pn_sparc_Ld_res);

  unsigned n_outs = get_Switch_n_outs(node);
  return new_bd_sparc_SwitchJmp(dbgi, new_block, address, n_outs, table,
                                entity);
}
...
static ir_node *gen_Cmp(ir_node *node) {
  ...
  if (bits == 32 && is_irn_null(op2) && get_irn_n_edges(op1) == 1) {
    if (is_And(op1)) {
      ir_node *new_node = gen_helper_bitop(
          op1, new_bd_sparc_AndCCZero_reg, new_bd_sparc_AndCCZero_imm,
          new_bd_sparc_AndNCCZero_reg, new_bd_sparc_AndNCCZero_imm, MATCH_NONE);
      arch_set_irn_register(new_node, &sparc_registers[REG_PSR]);
      return new_node;
    } else if (is_Or(op1)) {
      ir_node *new_node = gen_helper_bitop(
          op1, new_bd_sparc_OrCCZero_reg, new_bd_sparc_OrCCZero_imm,
          new_bd_sparc_OrNCCZero_reg, new_bd_sparc_OrNCCZero_imm, MATCH_NONE);
      arch_set_irn_register(new_node, &sparc_registers[REG_PSR]);
      return new_node;
    } else if (is_Eor(op1)) {
      ir_node *new_node = gen_helper_bitop(
          op1, new_bd_sparc_XorCCZero_reg, new_bd_sparc_XorCCZero_imm,
          new_bd_sparc_XNorCCZero_reg, new_bd_sparc_XNorCCZero_imm, MATCH_NONE);
      arch_set_irn_register(new_node, &sparc_registers[REG_PSR]);
      return new_node;
    } else if (is_Mul(op1)) {
      ir_node *new_node =
          gen_helper_binop(op1, MATCH_COMMUTATIVE, new_bd_sparc_SMulCCZero_reg,
  ...
  /* integer compare */
  return gen_helper_binop_args(node, op1, op2, MATCH_NONE, new_bd_sparc_Cmp_reg,
...
static ir_node *create_ftoi(dbg_info *dbgi, ir_node *block, ir_node *op,
  ...
  ir_node *stf =
      new_bd_sparc_Stf_s(dbgi, block, ftoi, sp, nomem, mode_fp, NULL, 0, true);
  arch_add_irn_flags(stf, arch_irn_flag_spill);
  ir_node *ld =
      new_bd_sparc_Ld_imm(dbgi, block, stf, sp, mode_gp, NULL, 0, true);
...
static ir_node *create_itof(dbg_info *dbgi, ir_node *block, ir_node *op,
  ...
  ir_node *nomem = get_irg_no_mem(irg);
  ir_node *st =
      new_bd_sparc_St_imm(dbgi, block, nomem, op, sp, mode_gp, NULL, 0, true);
  arch_add_irn_flags(st, arch_irn_flag_spill);
  ir_node *ldf =
      new_bd_sparc_Ldf_s(dbgi, block, sp, st, mode_fp, NULL, 0, true);
...
static ir_node *gen_Conv(ir_node *node) {
  ...
  if (src_mode == dst_mode) return be_transform_node(op);
  ...
  if (mode_is_float(src_mode) || mode_is_float(dst_mode)) {
    ...
    if (mode_is_float(src_mode)) {
      if (mode_is_float(dst_mode)) {
        ...
        /* float -> int conv */
        if (!mode_is_signed(dst_mode)) panic("float to unsigned not lowered");
...
static ir_node *gen_Start(ir_node *node) {
  ...
  for (size_t i = 0, n = cconv->n_parameters; i != n; ++i) {
    ...
    if (reg0) outs[reg0->global_index] = BE_START_REG;
    arch_register_t const *const reg1 = param->reg1;
    if (reg1) outs[reg1->global_index] = BE_START_REG;
  }
...
static ir_node *gen_Return(ir_node *node) {
  ...
  if (current_cconv->omit_fp) n_ins += ARRAY_SIZE(omit_fp_callee_saves);
  ...
  ir_node *const ret =
      new_bd_sparc_Return_reg(dbgi, new_block, n_ins, in, reqs);
...
static ir_node *bitcast_int_to_float(dbg_info *dbgi, ir_node *block,
  ...
  ir_node *sp = get_irg_frame(irg);
  ir_node *nomem = get_irg_no_mem(irg);
  ir_node *st = new_bd_sparc_St_imm(dbgi, block, nomem, value0, sp, mode_gp,
                                    NULL, 0, true);
  arch_add_irn_flags(st, arch_irn_flag_spill);
  set_irn_pinned(st, false);
  ...
  ir_node *mem;
  if (value1 != NULL) {
    ir_node *st1 = new_bd_sparc_St_imm(dbgi, block, nomem, value1, sp, mode_gp,
                                       NULL, 4, true);
...
static void bitcast_float_to_int(dbg_info *dbgi, ir_node *block, ir_node *value,
                                 ir_mode *float_mode, ir_node **result) {
  ...
  if (is_Const(value)) {
    ...
    ir_node *stf = create_stf(dbgi, block, new_value, stack, nomem, float_mode,
                              NULL, 0, true);
    arch_add_irn_flags(stf, arch_irn_flag_spill);
    set_irn_pinned(stf, false);

    ir_node *ld =
        new_bd_sparc_Ld_imm(dbgi, block, stf, stack, mode_gp, NULL, 0, true);
    set_irn_pinned(ld, false);
    result[0] = be_new_Proj(ld, pn_sparc_Ld_res);

    if (bits == 64) {
      ir_node *ld2 =
          new_bd_sparc_Ld_imm(dbgi, block, stf, stack, mode_gp, NULL, 4, true);
      set_irn_pinned(ld, false);
      result[1] = be_new_Proj(ld2, pn_sparc_Ld_res);

      arch_add_irn_flags(
          ld, (arch_irn_flags_t)sparc_arch_irn_flag_needs_64bit_spillslot);
      arch_add_irn_flags(
          ld2, (arch_irn_flags_t)sparc_arch_irn_flag_needs_64bit_spillslot);
    } else {
      assert(bits == 32);
      result[1] = NULL;
    }
...
static ir_node *gen_Call(ir_node *node) {
  ...
  calling_convention_t *cconv = sparc_decide_calling_convention(type, NULL);
  ...
  int in_arity = 0;
  int sync_arity = 0;
  int n_caller_saves = rbitset_popcount(cconv->caller_saves, N_SPARC_REGISTERS);
  ir_entity *entity = NULL;
  bool aggregate_return = get_method_calling_convention(type) & cc_compound_ret;
  ...
  ir_node *const callframe =
      be_new_IncSP(new_block, new_frame, cconv->param_stack_size, false);
  ...
  for (size_t p = 0; p < n_params; ++p) {
    ...
    ir_node *const str =
        mode_is_float(mode)
            ? create_stf(dbgi, new_block, partial_value, callframe, new_mem,
                         mode, NULL, offset, true)
            : new_bd_sparc_St_imm(dbgi, new_block, new_mem, partial_value,
                                  callframe, mode, NULL, offset, true);
  ...
  /* create call node */
  ir_node *const res =
      entity ? new_bd_sparc_Call_imm(dbgi, new_block, in_arity, in, in_req,
                                     out_arity, entity, 0, aggregate_return)
             : new_bd_sparc_Call_reg(dbgi, new_block, in_arity, in, in_req,
                                     out_arity, aggregate_return);
  ...
  ir_node *const incsp =
      be_new_IncSP(new_block, call_stack, -cconv->param_stack_size, false);
...
static ir_node *gen_Alloc(ir_node *node) {
  ...
  if (is_Const(size)) {
    ...
    assert((sizel & (SPARC_STACK_ALIGNMENT - 1)) == 0 &&
           "Found Alloc with misaligned constant");
    subsp = new_bd_sparc_SubSP_imm(dbgi, new_block, new_mem, stack_pred, NULL,
                                   sizel);
  } else {
    ir_node *new_size = be_transform_node(size);
    subsp =
        new_bd_sparc_SubSP_reg(dbgi, new_block, new_mem, stack_pred, new_size);
  }

  ir_node *const stack_proj =
      be_new_Proj_reg(subsp, pn_sparc_SubSP_stack, &sparc_registers[REG_SP]);
...
static ir_node *gen_Proj_Alloc(ir_node *node) {
  ...
  switch ((pn_Alloc)pn) {
    case pn_Alloc_M:
      return be_new_Proj(new_alloc, pn_sparc_SubSP_M);
    case pn_Alloc_res:
      return be_new_Proj(new_alloc, pn_sparc_SubSP_addr);
  }
...
static ir_node *gen_compare_swap(ir_node *node) {
  ...
  ir_node *cas =
      new_bd_sparc_Cas(dbgi, block, new_ptr, new_old, new_new, stbar);
...
static ir_node *gen_Bitcast(ir_node *node) {
  ...
  switch (get_mode_arithmetic(src_mode)) {
    case irma_twos_complement: {
      ...
      ir_node *st = new_bd_sparc_St_imm(dbgi, new_block, nomem, new_op, sp,
                                        src_mode, NULL, 0, true);
      arch_add_irn_flags(st, arch_irn_flag_spill);
      ir_node *const ldf =
          create_ldf(dbgi, new_block, sp, st, dst_mode, NULL, 0, true);
    ...
    case irma_ieee754: {
      ...
      ir_node *stf = create_stf(dbgi, new_block, new_op, sp, nomem, src_mode,
                                NULL, 0, true);
      arch_add_irn_flags(stf, arch_irn_flag_spill);
      ir_node *const ld = new_bd_sparc_Ld_imm(dbgi, new_block, stf, sp,
                                              dst_mode, NULL, 0, true);
...
static ir_node *gen_Proj_Store(ir_node *node) {
  ...
  switch (get_sparc_irn_opcode(new_store)) {
    case iro_sparc_St:
    case iro_sparc_Stf:
      if (pn == pn_Store_M) return new_store;
      break;
    default:
      break;
  }
  ...
  if (reg0) {
    ...
    if (is_float) {
      ...
      if (reg1 != NULL) {
        ...
        ir_node *mem = be_get_Start_mem(irg);
        ir_node *ld = new_bd_sparc_Ld_imm(NULL, new_block, mem, fp, mode_gp,
                                          param->entity, 0, true);
    ...
    if (mode_is_float(mode)) {
      load =
          create_ldf(NULL, new_block, base, mem, mode, param->entity, 0, true);
      value = be_new_Proj(load, pn_sparc_Ldf_res);
    } else {
      load = new_bd_sparc_Ld_imm(NULL, new_block, mem, base, mode,
                                 param->entity, 0, true);
...
static ir_node *gen_IJmp(ir_node *node) {
  ...
  if (address.ptr2) {
    ...
    return new_bd_sparc_IJmp_imm(dbgi, block, address.ptr, address.entity,
...
void sparc_transform_graph(ir_graph *irg) {
  assure_irg_properties(
      irg, IR_GRAPH_PROPERTY_NO_TUPLES | IR_GRAPH_PROPERTY_NO_BADS);
  ...
  for (size_t i = 0, n = current_cconv->n_parameters; i < n; ++i) {
    ...
    if (param->already_stored) continue;
