...
static int cpu;
static lc_opt_enum_int_var_t cpu_var = {&cpu, cpu_items};

sparc_codegen_config_t sparc_cg_config;

typedef enum { use_fpu_auto, use_fpu_yes, use_fpu_no } sparc_use_fpu_t;
/* fpu set architectures. */
static const lc_opt_enum_int_items_t fpu_items[] = {{"auto", use_fpu_auto},
                                                    {"fpu", use_fpu_yes},
                                                    {"softfloat", use_fpu_no},
                                                    {NULL, 0}};

static int fpu;
static lc_opt_enum_int_var_t arch_fpu_var = {&fpu, fpu_items};
...
static bool sparc_modifies_flags(const ir_node *node) {
  be_foreach_out(node, o) {
    ...
    if (req->cls == &sparc_reg_classes[CLASS_sparc_flags]) return true;
  }
  return false;
}
...
static bool sparc_modifies_fp_flags(const ir_node *node) {
  be_foreach_out(node, o) {
    const arch_register_req_t *req = arch_get_irn_register_req_out(node, o);
    if (req->cls == &sparc_reg_classes[CLASS_sparc_fpflags]) return true;
  }
  return false;
}
...
static void rewrite_float_unsigned_Conv(ir_node *node) {
  ...
  ir_node *cmp =
      new_rd_Cmp(dbgi, block, float_x, limitc, ir_relation_greater_equal);
  ir_node *cond = new_rd_Cond(dbgi, block, cmp);
  ir_node *proj_true = new_r_Proj(cond, mode_X, pn_Cond_true);
  ir_node *proj_false = new_r_Proj(cond, mode_X, pn_Cond_false);
  ir_node *in_true[1] = {proj_true};
...
static bool sparc_rewrite_Conv(ir_node *node) {
  ...
  if (mode_is_float(to_mode) && mode_is_int(from_mode) &&
      get_mode_size_bits(from_mode) == 32 && !mode_is_signed(from_mode)) {
    rewrite_unsigned_float_Conv(node);
    return true;
  }
  if (mode_is_float(from_mode) && mode_is_int(to_mode) &&
      get_mode_size_bits(to_mode) <= 32 && !mode_is_signed(to_mode)) {
...
static void handle_intrinsic(ir_node *node, void *data) {
  ...
  if (is_Mod(node)) {
    ...
    if (sparc_rewrite_Conv(node)) *changed = true;
  }
}

static ir_type *make_mod_type(ir_type *const tp) {
  ir_type *const mtp =
      new_type_method(2, 1, false, cc_cdecl_set, mtp_no_property);
...
static void sparc_setup_cg_config(void) {
  ...
  if (use_softfloat) fpu = use_fpu_no;
...
static void sparc_finish(void) { sparc_free_opcodes(); }
...
static ir_node *sparc_new_spill(ir_node *value, ir_node *after) {
  ...
  if (mode_is_float(mode)) {
    ...
    store = new_bd_sparc_St_imm(NULL, block, mem, value, frame, mode, NULL, 0,
                                true);
  }
...
static void sparc_generate_code(FILE *output, const char *cup_name) {
  ...
  foreach_irp_irg(i, irg) {
    ...
    be_sched_fix_flags(irg, &sparc_reg_classes[CLASS_sparc_flags], NULL,
                       sparc_modifies_flags, NULL);
    be_sched_fix_flags(irg, &sparc_reg_classes[CLASS_sparc_fpflags], NULL,
                       sparc_modifies_fp_flags, NULL);
...
static void sparc_lower_for_target(void) {
  ...
  lower_calls_with_compounds(LF_RETURN_HIDDEN, lower_aggregates_as_pointers,
                             NULL, lower_aggregates_as_pointers, NULL,
                             reset_stateless_abi);
  ...
  if (sparc_cg_config.use_cas) supported[s++] = ir_bk_compare_swap;
...
static unsigned sparc_get_op_estimated_cost(const ir_node *node) {
  /* TODO: refine */
  if (sparc_has_load_store_attr(node)) return 5;
  return 1;
}
