...
static void kill_unused_stacknodes(ir_node *node) {
  if (get_irn_n_edges(node) > 0) return;
...
static void sparc_introduce_prolog_epilog(ir_graph *irg, bool omit_fp) {
  ...
  if (!omit_fp) {
    ir_node *const save = new_bd_sparc_Save_imm(
        NULL, block, initial_sp, NULL, -frame_size - SPARC_MIN_STACKSIZE);
...
static void finish_be_IncSP(ir_node *node) {
   ...
   */
  if (!sparc_is_value_imm_encodeable(offset) &&
      !sparc_is_value_imm_encodeable(-offset)) {
...
static void finish_load_store(ir_node *const old, ir_node *const nw,
                              sparc_load_store_attr_t const *const ldst_attr) {
...
static void finish_sparc_Ld(ir_node *node) {
  ...
  if (!load_store_attr->is_frame_entity) return;
  ...
  if (!sparc_is_value_imm_encodeable(offset)) {
    ...
    ir_node *constant = create_constant_from_immediate(node, offset);
    ir_node *new_load =
        new_bd_sparc_Ld_reg(dbgi, block, mem, ptr, constant, load_store_mode);
...
static void split_sparc_ldf(ir_node *node) {
  ...
  if (bits == 128) {
    ...
    ir_node *new_load = new_bd_sparc_Ldf_d(
        dbgi, block, ptr, mem, mode_D, attr->base.immediate_value_entity,
        attr->base.immediate_value + 8, attr->is_frame_entity);
...
static void finish_sparc_Ldf(ir_node *node) {
  ...
  if (!sparc_is_value_imm_encodeable(offset)) {
    ...
    ir_node *new_load = new_bd_sparc_Ldf_s(dbgi, block, new_ptr, mem,
                                           load_store_mode, NULL, 0, true);
    finish_load_store(node, new_load, load_store_attr);
  }
}
...
static void finish_sparc_St(ir_node *node) {
  ...
  if (!sparc_is_value_imm_encodeable(offset)) {
    ...
    ir_mode *load_store_mode = load_store_attr->load_store_mode;
    ir_node *constant = create_constant_from_immediate(node, offset);
    ir_node *new_load = new_bd_sparc_St_reg(dbgi, block, mem, value, ptr,
                                            constant, load_store_mode);
    finish_load_store(node, new_load, load_store_attr);
  }
}
...
static void finish_sparc_Stf(ir_node *node) {
  ...
  if (!sparc_is_value_imm_encodeable(offset)) {
    ...
    ir_node *constant = create_constant_from_immediate(node, offset);
    ir_node *new_ptr = new_bd_sparc_Add_reg(dbgi, block, ptr, constant);
    ir_node *new_load = new_bd_sparc_Stf_s(dbgi, block, value, new_ptr, mem,
                                           load_store_mode, NULL, 0, true);
    finish_load_store(node, new_load, load_store_attr);
  }
}
...
static void replace_with_restore(ir_node *const restore, ir_node *const node,
                                 ir_node *const replaced) {
...
static void replace_with_restore_reg(ir_node *node, ir_node *replaced,
  ...
  ir_node *block = get_nodes_block(node);
  ir_node *new_node =
      new_bd_sparc_Restore_reg(dbgi, block, stack_in, fp, op0, op1);
...
static void replace_with_restore_imm(ir_node *node, ir_node *replaced,
  ...
  ir_node *fp = get_irn_n(node, n_sparc_RestoreZero_frame_pointer);
  ir_node *block = get_nodes_block(node);
  ir_node *new_node = new_bd_sparc_Restore_imm(dbgi, block, stack_in, fp, op,
                                               imm_entity, immediate);
  replace_with_restore(new_node, node, replaced);
}
...
static void peephole_sparc_RestoreZero(ir_node *node) {
  ...
  sched_foreach_reverse_before(node, schedpoint) {
    ...
    if (arch_get_irn_n_outs(schedpoint) == 0) continue;

    arch_register_t const *const reg = arch_get_irn_register_out(schedpoint, 0);
    if (!reg || !is_restorezeroopt_reg(reg)) continue;
    ...
    if (be_is_Copy(schedpoint) && can_move_down(schedpoint, node)) {
      ...
      replace_with_restore_imm(node, schedpoint, op, NULL, 0);
               ...
               arch_get_irn_flags(schedpoint) &
                   ((arch_irn_flags_t)sparc_arch_irn_flag_immediate_form) &&
               arch_get_irn_register_in(schedpoint, 0) ==
                   &sparc_registers[REG_G0] &&
    ...
    } else if (is_sparc_Add(schedpoint) && can_move_down(schedpoint, node)) {
      if (arch_get_irn_flags(schedpoint) &
          ((arch_irn_flags_t)sparc_arch_irn_flag_immediate_form)) {
