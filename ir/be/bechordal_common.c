...
static inline border_t *border_add(be_chordal_env_t *const env,
                                   struct list_head *const head,
                                   ir_node *const irn, unsigned const is_def,
                                   unsigned const is_real) {
...
#define border_def(irn, real) border_add(env, head, irn, 1, real)

/* Convenience macro for a use */
#define border_use(irn, real) border_add(env, head, irn, 0, real)
  ...
  sched_foreach_reverse(block, irn) {
    ...
    if (!is_Phi(irn)) {
      be_foreach_use(
          irn, env->cls, in_req_, op, op_req_,
          DEBUG_ONLY(char msg = '-';)
...
ir_node *pre_process_constraints(be_chordal_env_t *env, be_insn_t **the_insn) {
  ...
  /*
   * We also have to re-build the insn since the input operands are now the
   * Projs of the Perm. Recomputing liveness is also a good idea if a Perm is
   * inserted, since the live sets may change.
   */
  ...
  *the_insn = be_scan_insn(env, irn);
  ...
  foreach_irn_in(irn, i, proj) {
    ...
    if (!is_Proj(proj) || get_Proj_pred(proj) != perm) continue;
