...
static spill_info_t *get_spillinfo(spill_env_t *env, ir_node *value) {
  spill_info_t *info = ir_nodehashmap_get(spill_info_t, &env->spillmap, value);
  if (info == NULL) {
...
static void spill_irn(spill_env_t *env, spill_info_t *spillinfo) {
  ...
  for (spill_t *spill = spillinfo->spills; spill != NULL; spill = spill->next) {
...
static void spill_node(spill_env_t *env, spill_info_t *spillinfo) {
  ...
  if (spillinfo->spills != NULL && spillinfo->spills->spill != NULL) return;
...
static bool is_value_available(spill_env_t *env, const ir_node *arg) {
  if (is_Unknown(arg) || is_NoMem(arg)) return true;
  if (arch_irn_is(skip_Proj_const(arg), spill)) return true;
  if (arg == get_irg_frame(env->irg)) return true;
  if (get_irn_mode(arg) == mode_T) return false;
  /* "Ignore registers" are always available */
  if (arch_irn_is_ignore(arg)) return true;
...
static int check_remat_conditions_costs(spill_env_t *env,
  ...
  foreach_irn_in(insn, i, arg) {
    ...
    costs +=
        check_remat_conditions_costs(env, arg, reloader, parentcosts + costs);
    if (parentcosts + costs >= spillcosts) return REMAT_COST_INFINITE;
  }
 ...
 */
static ir_node *do_remat(spill_env_t *env, ir_node *spilled,
                         ir_node *reloader) {
  ...
  if (env->regif.mark_remat) env->regif.mark_remat(res);

  DBG((dbg, LEVEL_1, "Insert remat %+F of %+F before reloader %+F\n", res,
       spilled, reloader));

  if (!is_Proj(res)) sched_add_before(reloader, res);

  return res;
}

double be_get_spill_costs(spill_env_t *env, ir_node *to_spill,
                          ir_node *before) {
  (void)to_spill;
...
unsigned be_get_reload_costs_no_weight(spill_env_t *env,
  ...
  if (be_do_remats) {
    ...
    if (costs < (unsigned)env->regif.reload_cost) return costs;
  }
...
double be_get_reload_costs(spill_env_t *env, ir_node *to_spill,
  ...
  if (be_do_remats) {
    ...
    if (costs < (int)env->regif.reload_cost) return costs * freq;
  }
...
static void determine_spill_costs(spill_env_t *env, spill_info_t *spillinfo) {
  /* already calculated? */
  if (spillinfo->spill_costs >= 0) return;
  ...
  for (spill_info_t *si = env->spills; si != NULL; si = si->next) {
    ...
    if (be_do_remats) {
      ...
      for (reloader_t *rld = si->reloaders; rld != NULL; rld = rld->next) {
        ...
        int remat_cost =
            check_remat_conditions_costs(env, to_spill, reloader, 0);
        ...
        DBG((dbg, LEVEL_2,
             "\tremat costs delta before %+F: "
             "%d (rel %f)\n",
             reloader, remat_cost_delta, remat_cost_delta * freq));
      }
    ...
    if (si->spills != NULL && si->spills->next != NULL) {
      ...
      for (spill_t *spill = si->spills; spill != NULL; spill = spill->next) {
        /* maybe we rematerialized the value and need no spill */
        if (spill->spill == NULL) continue;
...
static void be_new_Copy_for_input(ir_node *const val, ir_node *const before,
                                  int const pos) {
...
static void prepare_constr_insn(ir_node *const node) {
  ...
  foreach_irn_in(node, i, op) {
    ...
    if (rbitset_is_set(req->limited, reg->index)) continue;
  ...
  for (int i = 0, arity = get_irn_arity(node); i < arity; ++i) {
    ...
    for (int i2 = i + 1; i2 < arity; ++i2) {
      ...
      arch_register_req_t const *const req2 =
          arch_get_irn_register_req_in(node, i2);
      if (!req2->limited ||
          rbitsets_equal(req->limited, req2->limited, req->cls->n_regs))
        continue;
  ...
  be_foreach_value(
      ...
      if (req->limited == NULL) continue;
      if (def_constr == NULL) def_constr =
          rbitset_alloca(ir_target.isa->n_registers);
  ...
  for (int i = 0, arity = get_irn_arity(node); i < arity; ++i) {
    ...
    if (req->kills_value) {
      ...
      if (def_constr == NULL || req->limited == NULL) continue;
      const arch_register_req_t *const in_req = arch_get_irn_register_req(in);
      if (in_req->ignore) continue;
    ...
    if (be_is_Copy(in)) continue;
    if (!be_value_live_after(in, node)) continue;
...
static void add_missing_copies_in_block(ir_node *block, void *data) {
  (void)data;
  sched_foreach(block, node) { prepare_constr_insn(node); }
}
...
static void gen_assure_different_pattern(ir_node *const value,
                                         ir_node *const irn,
                                         ir_node *const other_different,
                                         constraint_env_t *const env) {
  arch_register_req_t const *const req =
      arch_get_irn_register_req(other_different);
  if (req->ignore) {
    DB((dbg_constr, LEVEL_1,
        "ignore constraint for %+F because other_irn is ignore\n", value));
    return;
  }
 ...
 */
static void assure_different_constraints(ir_node *const value,
                                         ir_node *const irn,
                                         constraint_env_t *const env) {
  ...
  if (same != 0 && is_po2_or_zero(other) && is_po2_or_zero(same)) {
    ...
    if (get_irn_n(irn, idx_other) == get_irn_n(irn, idx_same)) return;
  }
...
static bool has_real_user(const ir_node *node) {
  foreach_out_edge(node, edge) {
    ...
    if (!is_End(user) && !is_Anchor(user)) return true;
  }
  return false;
}

static void add_missing_keep_walker(ir_node *node, void *data) {
  if (!sched_is_scheduled(node)) return;
...
void be_spill_prepare_for_constraints(ir_graph *irg) {
  ...
  foreach_ir_nodehashmap(&cenv.op_set, map_entry, map_iter) {
    op_copy_assoc_t const *const entry =
        (op_copy_assoc_t const *)map_entry.data;
    ...
    for (size_t i = 0; i != n_copies; ++i) {
      ...
      if (be_is_CopyKeep(cp) && get_irn_n_edges(cp) == 0) {
        ...
        /* Set all ins (including the block) of the CopyKeep BAD to keep the
         * verifier happy. */
        kill_node(cp);
      }
    }
...
void be_init_spill(void) { FIRM_DBG_REGISTER(dbg, "firm.be.spill"); }
