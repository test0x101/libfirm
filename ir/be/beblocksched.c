 ...
 * execution frequencies and tries to transform them to fallthroughs in this
 * order
 */
...
static void remove_empty_block(ir_node *block) {
  if (irn_visited_else_mark(block)) return;

  if (get_Block_n_cfgpreds(block) != 1) goto check_preds;
  ...
  sched_foreach(block, node) {
    ...
    if (jump) panic("found 2 jumps in a block");
    jump = node;
  }
  if (jump == NULL) goto check_preds;
...
static void remove_empty_blocks(ir_graph *irg) {
  ...
  foreach_irn_in(get_irg_end(irg), i, pred) {
    if (is_Block(pred)) remove_empty_block(pred);
  }
...
static void collect_egde_frequency(ir_node *block, void *data) {
  ...
  if (block == get_irg_end_block(env->irg)) return;
...
static void coalesce_blocks(blocksched_env_t *env) {
  ...
  for (size_t i = 0; i < edge_count; ++i) {
    ...
    if (!edge->highest_execfreq) continue;

    /* the block might have been removed already... */
    ir_node *const block = edge->block;
    if (is_Bad(get_Block_cfgpred(block, 0))) continue;
    ...
    if (pred_entry->next != NULL || entry->prev != NULL) continue;

    /* only coalesce jumps */
    if (get_block_succ_next(pred_block, get_block_succ_first(pred_block)) !=
        NULL)
      continue;

    /* schedule the two blocks behind each other */
    DB((dbg, LEVEL_1, "Coalesce (Jump) %+F -> %+F (%.3g)\n", pred_entry->block,
        entry->block, edge->execfreq));
  ...
  for (size_t i = 0; i < edge_count; ++i) {
    ...
    if (edge->outedge_penalty_freq == 0) break;
    ...
    if (get_loop_link(loop) != NULL) continue;
...
static void pick_block_successor(blocksched_entry_t *entry,
  ...
  foreach_block_succ(block, edge) {
    ...
    if (irn_visited(succ_block)) continue;
    ...
    if (irn_visited(succ_entry->block)) continue;
  ...
  foreach_block_succ(block, edge) {
    ...
    if (succ_entry->prev != NULL) continue;
...
void be_init_blocksched(void) { FIRM_DBG_REGISTER(dbg, "firm.be.blocksched"); }
