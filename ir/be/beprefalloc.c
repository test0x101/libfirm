 ...
 */
static void check_defs(ir_nodeset_t const *const live_nodes, float const weight,
                       ir_node *const node,
  ...
  if (req->should_be_same != 0) {
    ...
    foreach_irn_in(insn, i, op) {
      if (!rbitset_is_set(&req->should_be_same, i)) continue;
      ...
      if (ir_nodeset_contains(live_nodes, op)) continue;
...
static void congruence_def(ir_nodeset_t *const live_nodes,
...
static void create_congruence_class(ir_node *block, void *data) {
  ...
  sched_foreach_phi(block, phi) {
    if (!arch_irn_consider_in_reg_alloc(cls, phi)) continue;
...
static void set_congruence_prefs(ir_node *node, void *data) {
  ...
  if (node_set == node_idx) return;

  if (!arch_irn_consider_in_reg_alloc(cls, node)) return;
...
static void register_assignment(ir_node *const node, unsigned const reg_idx,
...
static void use_reg_idx(ir_node *const node, unsigned const reg_idx,
                        unsigned const width) {
...
static bool try_optimistic_split(ir_node *to_split, ir_node *before, float pref,
                                 float pref_delta, unsigned *forbidden_regs,
                                 int recursion) {
  (void)pref;
  unsigned r = 0;
  ...
  if (pref_delta < split_threshold * 0.5) return false;
  ...
  for (i = 0; i < n_regs; ++i) {
    ...
    r = prefs[i].num;
    if (!rbitset_is_set(normal_regs, r)) continue;
    if (rbitset_is_set(forbidden_regs, r)) continue;
    if (r == from_r) continue;
    ...
    if (assignments[r] == NULL) break;

    /* otherwise we might try recursively calling optimistic_split */
    if (recursion + 1 > MAX_OPTIMISTIC_SPLIT_RECURSION) continue;
    ...
    /* try recursive split */
    bool res = try_optimistic_split(assignments[r], before, apref, apref_delta,
                                    forbidden_regs, recursion + 1);
  ...
  if (i >= n_regs) return false;

  ir_node *const copy = be_new_Copy(block, to_split);
  unsigned const width = 1;
  mark_as_copy_of(copy, to_split);
  /* hacky, but correct here */
  if (assignments[from_reg->index] == to_split) free_reg_of_value(to_split);
  use_reg_idx(copy, r, width);
  sched_add_before(before, copy);

  DB((dbg, LEVEL_3,
      "Optimistic live-range split %+F move %+F(%s) -> %s before %+F (win %f, "
      "depth %d)\n",
      copy, to_split, from_reg->name, arch_get_irn_register(copy)->name, before,
      delta, recursion));
  return true;
}
 ...
 */
static void assign_reg(ir_node const *const block, ir_node *const node,
                       arch_register_req_t const *const req,
                       unsigned *const forbidden_regs) {
  assert(!is_Phi(node));
  ...
  if (req->should_be_same != 0) {
    ...
    foreach_irn_in(in_node, i, in) {
       ...
       * should_be_same info */
      if (assignments[reg_index] == in) continue;
  ...
  for (unsigned r = 0; r < n_regs; ++r) {
    ...
    if (!rbitset_is_set(normal_regs, num)) continue;
  ...
  for (r = 0; r < n_regs; ++r) {
    ...
    if (!rbitset_is_set(allowed_regs, final_reg_index)) continue;
    /* alignment constraint? */
    if (width > 1) {
      if (final_reg_index % width != 0) continue;
      bool fine = true;
      for (unsigned r0 = r + 1; r0 < r + width; ++r0) {
        if (assignments[r0] != NULL) fine = false;
      }
      /* TODO: attempt optimistic split here */
      if (!fine) continue;
    }

    if (assignments[final_reg_index] == NULL) break;
    float pref = reg_prefs[r].pref;
    float delta = r + 1 < n_regs ? pref - reg_prefs[r + 1].pref : 0;
    ir_node *before = skip_Proj(node);
    bool res = try_optimistic_split(assignments[final_reg_index], before, pref,
...
static void permute_values(ir_nodeset_t *live_nodes, ir_node *before,
  ...
  for (unsigned r = 0; r < n_regs; /* empty */) {
    ...
    DB((dbg, LEVEL_2, "Copy %+F (from %+F, before %+F) -> %s\n", copy, src,
        before, arch_get_irn_register(copy)->name));
...
static void free_last_uses(ir_nodeset_t *live_nodes, ir_node *node) {
  ...
  foreach_irn_in(node, i, op) {
    ...
    if (!rbitset_is_set(last_uses, i)) continue;
...
static void determine_live_through_regs(unsigned *bitset, ir_node *node) {
  ...
  for (unsigned r = 0; r < n_regs; ++r) {
    if (assignments[r] == NULL) continue;
    if (!rbitset_is_set(normal_regs, r)) continue;
  ...
  foreach_irn_in(node, i, op) {
    if (!rbitset_is_set(info->last_uses, i)) continue;
...
static void solve_lpp(ir_nodeset_t *live_nodes, ir_node *node,
  ...
  be_foreach_use(
      node, cls, req, op, op_req, if (req->limited == NULL) continue;

      const unsigned *limited = req->limited;
      const arch_register_t *reg = arch_get_irn_register(op);
      unsigned current_reg = reg->index; for (unsigned r = 0; r < n_regs; ++r) {
        if (rbitset_is_set(limited, r)) continue;
  ...
  for (unsigned l = 0; l < n_regs; ++l) {
    ...
    for (unsigned r = 0; r < n_regs; ++r) {
      if (!rbitset_is_set(normal_regs, r)) continue;
      if (rbitset_is_set(forbidden_edges, l * n_regs + r)) continue;
      ...
      lpp_vars[l * n_regs + r] = lpp_add_var(lpp, name, lpp_binary, costs);
  ...
  if (out == NULL) panic("couldn't open lppdump.txt");
  lpp_dump_plain(lpp, out);
  fclose(out);

  /* solve lpp */
  lpp_solve(lpp, be_options.ilp_solver);
  if (!lpp_is_sol_valid(lpp)) panic("ilp solution not valid");
...
static void enforce_constraints(ir_nodeset_t *live_nodes, ir_node *node,
  ...
  bool good = true;
  be_foreach_use(
      node, cls, req, op, op_req,
      /* are there any limitations for the i'th operand? */
      const arch_register_t *reg = arch_get_irn_register(op);
      unsigned reg_index = reg->index; if (req->width != 1) {
        double_width = true;
  ...
  be_foreach_definition(
      node, cls, value, req, (void)value;
      if (req->width > 1) double_width = true;
      if (req->limited == NULL) continue; if (live_through_regs == NULL) {
      ...
      if (rbitsets_have_common(req->limited, live_through_regs, n_regs)) good =
          false;);
...
static bool is_copy_of(ir_node *value, ir_node *test_value) {
  if (value == test_value) return true;
...
static int find_value_in_block_info(block_info_t *info, ir_node *value) {
  ...
  for (unsigned r = 0; r < n_regs; ++r) {
    ...
    if (is_copy_of(a_value, value)) return (int)r;
  }
...
static void adapt_phi_prefs(ir_node *phi) {
  ...
  foreach_irn_in(phi, i, op) {
    ...
    if (!pred_block_info->processed) continue;
...
static void propagate_phi_register(ir_node *phi, unsigned assigned_r) {
  ...
  for (int i = 0; i < arity; ++i) {
    ...
    ir_node *pred_block = get_Block_cfgpred_block(block, i);
    float weight = (float)get_block_execfreq(pred_block) * AFF_PHI;

    if (info->prefs[assigned_r] >= weight) continue;
    ...
    if (is_Phi(op)) propagate_phi_register(op, assigned_r);
  }
}
...
static void assign_phi_registers(ir_node *block) {
  ...
  sched_foreach_phi(block, node) {
    if (!arch_irn_consider_in_reg_alloc(cls, node)) continue;
    ++n_phis;
  }
...
static void allocate_coalesce_block(ir_node *block, void *data) {
  ...
  be_lv_foreach(lv, block, be_lv_state_in, node) {
    ...
    if (need_phi) {
      ...
      if (req->width > 1) phi_req = be_create_cls_req(irg, cls, req->width);
...
static void determine_block_order(void) {
  ...
  for (size_t p = 0; p < n_blocks; ++p) {
    ir_node *block = blocklist[p];
    if (Block_block_visited(block)) continue;
    ...
    do {
      ...
      for (int i = 0; i < n_cfgpred; ++i) {
        ...
        /* ignore backedges */
        if (pred_info->dfs_num > info->dfs_num) continue;
...
static void free_block_order(void) { free(block_order); }
