...
static inline void be_set_asm_operand(be_asm_operand_t *const op,
                                      be_asm_operand_kind_t const kind,
                                      int const pos) {
  assert(pos == -1
             ? kind == BE_ASM_OPERAND_IMMEDIATE || kind == BE_ASM_OPERAND_MEMORY
             : kind != BE_ASM_OPERAND_IMMEDIATE);
  op->kind = kind;
  op->pos = pos;
}
...
arch_register_req_t const *be_make_register_req(struct obstack *obst,
                                                be_asm_constraint_t const *c);

typedef void parse_constraint_letter_func_t(void const *env,
                                            be_asm_constraint_t *, char l);

void be_parse_asm_constraints_internal(
    be_asm_constraint_t *constraint, ident *constraint_text,
    parse_constraint_letter_func_t *parse_constraint_letter, void const *env);
...
static inline size_t be_asm_append_in(be_asm_info_t *const info,
                                      ir_node *const in,
...
static inline void be_asm_add_in(be_asm_info_t *const info,
                                 be_asm_operand_t *const op,
                                 be_asm_operand_kind_t const kind,
                                 ir_node *const in,
                                 arch_register_req_t const *const req) {
...
static inline arch_register_req_t const *be_asm_make_same_req(
    struct obstack *const obst, arch_register_req_t const *const req,
    unsigned const pos) {
...
static inline void be_asm_add_inout(be_asm_info_t *const info,
                                    be_asm_operand_t *const op,
                                    struct obstack *const obst,
                                    ir_node *const in,
                                    arch_register_req_t const *const orig_ireq,
                                    int const opos) {
  ...
  if (opos >= 0) {
    arch_register_req_t const *const oreq =
        be_asm_make_same_req(obst, orig_ireq, ARR_LEN(info->in_reqs) - 1);
    info->out_reqs[opos] = oreq;
  }
}

static inline void be_asm_add_out(
    be_asm_info_t *const info, be_asm_operand_t *const op,
    struct obstack *const obst, be_asm_constraint_t const *const be_constraint,
    int const opos) {
  be_asm_operand_kind_t const kind =
      be_constraint->cls == &arch_exec_cls
          ? BE_ASM_OPERAND_LABEL
          :
          /*                                  */ BE_ASM_OPERAND_OUTPUT_VALUE;
...
ir_node *be_make_asm(ir_node const *node, be_asm_info_t const *info,
                     void *operands);

typedef void be_emit_asm_operand_func(ir_node const *asmn, char modifier,
                                      unsigned pos);

ir_node *be_emit_asm(ir_node const *asmn,
                     be_emit_asm_operand_func *emit_asm_operand);

bool be_is_valid_asm_operand_kind(ir_node const *node, char modifier,
                                  unsigned pos, be_asm_operand_kind_t have,
                                  char const *mod_any, char const *mod_imm,
                                  char const *mod_mem);
...
static inline bool be_has_modifier(char const *const candidates,
                                   char const modifier) {
