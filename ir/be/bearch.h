...
static inline reg_out_info_t *get_out_info_n(ir_node const *const node,
                                             unsigned const pos) {
 ...
 */
void arch_set_irn_register_out(ir_node *irn, unsigned pos,
                               const arch_register_t *r);

const arch_register_t *arch_get_irn_register_out(const ir_node *irn,
                                                 unsigned pos);
...
static inline void arch_set_irn_register_req_out(
    ir_node *node, unsigned pos, const arch_register_req_t *req) {
  reg_out_info_t *const out = get_out_info_n(node, pos);
  out->req = req;
}

static inline void arch_set_irn_register_reqs_in(
    ir_node *node, const arch_register_req_t **reqs) {
...
static inline const arch_register_req_t *arch_get_irn_register_req(
    const ir_node *node) {
...
static inline void arch_add_irn_flags(ir_node *const node,
...
be_add_pressure_t arch_get_additional_pressure(
    ir_node const *node, arch_register_class_t const *cls);
...
#define arch_irn_is(irn, flag) \
  ((arch_get_irn_flags(irn) & arch_irn_flag_##flag) != 0)
...
static inline bool reg_reqs_equal(const arch_register_req_t *req1,
  ...
  if (req1->cls != req2->cls || req1->should_be_same != req2->should_be_same ||
      req1->must_be_different != req2->must_be_different ||
      req1->width != req2->width || req1->ignore != req2->ignore ||
...
static inline bool reg_req_has_constraint(const arch_register_req_t *req) {
  return req->limited || req->must_be_different != 0 || req->ignore ||
         req->width != 1;
}
...
#define be_foreach_definition(node, ccls, value, req, code)                 \
  be_foreach_definition_(node, ccls, value, req, if (req->ignore) continue; \
                         code)
...
arch_register_req_t *be_create_cls_req(ir_graph *irg,
                                       arch_register_class_t const *cls,
                                       unsigned char width);

arch_register_req_t const *be_create_reg_req(ir_graph *irg,
                                             arch_register_t const *reg,
                                             bool ignore);

static inline void arch_set_irn_register_idx(ir_node *const irn,
                                             unsigned const idx) {
...
int be_get_input_pos_for_req(ir_node const *irn,
