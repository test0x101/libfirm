...
enum { LEVEL_NONE, LEVEL_BASIC, LEVEL_LOCATIONS, LEVEL_FRAMEINFO };
...
static void emit_ref(const ir_entity *entity) {
  const char *directive =
      ir_target_pointer_size() == 8 ? "\t.quad " : "\t.long ";
...
static void emit_pubnames(void) {
  ...
  for (size_t i = 0; i < ARR_LEN(env.pubnames_list); ++i) {
    ...
    be_emit_irprintf("\t.long %sE%ld - %sinfo_begin\n",
                     be_gas_get_private_prefix(), get_entity_nr(entity),
...
void be_dwarf_location(dbg_info *dbgi) {
  if (debug_level < LEVEL_LOCATIONS) return;
...
void be_dwarf_callframe_register(const arch_register_t *reg) {
  if (debug_level < LEVEL_FRAMEINFO) return;
...
void be_dwarf_function_before(const ir_entity *entity,
  ...
  if (debug_level < LEVEL_BASIC) return;
...
static void emit_subroutine_type_abbrev(void) {
  begin_abbrev(abbrev_subroutine_type, DW_TAG_subroutine_type, DW_CHILDREN_yes);
  register_attribute(DW_AT_prototyped, DW_FORM_flag);
  register_attribute(DW_AT_type, DW_FORM_ref4);
  end_abbrev();

  begin_abbrev(abbrev_void_subroutine_type, DW_TAG_subroutine_type,
               DW_CHILDREN_yes);
  register_attribute(DW_AT_prototyped, DW_FORM_flag);
  end_abbrev();

  begin_abbrev(abbrev_unnamed_formal_parameter, DW_TAG_formal_parameter,
               DW_CHILDREN_no);
  register_attribute(DW_AT_type, DW_FORM_ref4);
  end_abbrev();
}
...
static void emit_type(ir_type *type) {
  if (!pset_new_insert(&env.emitted_types, type)) return;
  ...
  switch (get_type_opcode(type)) {
    case tpo_primitive:
      ...
      return;
    case tpo_pointer:
      emit_pointer_type(type);
      return;
    case tpo_array:
      emit_array_type(type);
      return;
    case tpo_class:
    case tpo_struct:
    case tpo_union:
      emit_compound_type(type);
      return;
    case tpo_method:
      emit_subroutine_type(type);
      return;
    case tpo_segment:
    case tpo_code:
    case tpo_unknown:
...
void be_dwarf_variable(const ir_entity *entity) {
  if (debug_level < LEVEL_BASIC) return;
  if (get_entity_ld_name(entity)[0] == '\0') return;
...
static void emit_compile_unit_abbrev(void) {
  ...
  register_attribute(DW_AT_name, DW_FORM_string);
  if (language != 0) register_attribute(DW_AT_language, DW_FORM_data2);
  if (comp_dir != NULL) register_attribute(DW_AT_comp_dir, DW_FORM_string);
  end_abbrev();
}
...
void be_dwarf_unit_begin(const char *filename) {
  ...
  if (language != 0) emit_int16(language);
  if (comp_dir != NULL) be_gas_emit_cstring(comp_dir);
...
void be_init_dwarf(void) {
  static const lc_opt_enum_int_items_t level_items[] = {
      ...
      {NULL, 0}};
  static lc_opt_enum_int_var_t debug_level_opt = {&debug_level, level_items};
