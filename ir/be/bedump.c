...
static void dump_ifg_nodes(FILE *F, const be_ifg_t *ifg) {
  be_ifg_foreach_node(ifg, node) { dump_node(F, node); }
}
...
static void dump_ifg_edges(FILE *F, const be_ifg_t *ifg) {
  be_ifg_foreach_node(ifg, node) {
    ...
    be_ifg_foreach_neighbour(ifg, &neigh_iter, node, neighbour) {
       ...
       * direction */
      if (get_irn_node_nr(node) >= get_irn_node_nr(neighbour)) continue;
...
static void dump_affinity_edges(FILE *F, const copy_opt_t *co, bool dump_costs,
                                bool dump_colors) {
  co_gs_foreach_aff_node(co, a) {
    co_gs_foreach_neighb(a, n) {
      /* edges are bidirection, dumping one direction is enough */
      if (get_irn_node_nr(a->irn) >= get_irn_node_nr(n->irn)) continue;
      ...
      if (dump_costs) fprintf(F, " label:\"%d\"", n->costs);
      if (dump_colors) {
 ...
 */
static void dump_register_req(FILE *const F,
  ...
  if (req->limited != NULL) {
    ...
    for (unsigned i = 0, n_regs = cls->n_regs; i < n_regs; ++i) {
      if (rbitset_is_set(req->limited, i)) fprintf(F, " %s", cls->regs[i].name);
    }
  }
  ...
  if (req->width > 1) fprintf(F, " width:%d", req->width);
  if (req->ignore) fputs(" ignore", F);
  if (req->kills_value) fputs(" kills_value", F);
}

static void dump_req_reg(FILE *const F, char const *const ctx,
                         unsigned const idx,
                         arch_register_req_t const *const req,
...
static void dump_req_reg_out(FILE *const F, ir_node const *const node,
                             unsigned const pos) {
  arch_register_req_t const *const req =
      arch_get_irn_register_req_out(node, pos);
...
static const char *get_flag_name(arch_irn_flags_t flags) {
  ...
  switch (flags) {
    case arch_irn_flags_none:
      return "none";
    case arch_irn_flag_dont_spill:
      return "dont_spill";
    case arch_irn_flag_rematerializable:
      return "rematerializable";
    case arch_irn_flag_modify_flags:
      return "modify_flags";
    case arch_irn_flag_simple_jump:
      return "simple_jump";
    case arch_irn_flag_not_scheduled:
      return "not_scheduled";
    case arch_irn_flag_schedule_first:
      return "schedule_first";
    case arch_irn_flag_spill:
      return "spill";
    case arch_irn_flag_reload:
      return "reload";
    case arch_irn_flag_fallthrough:
      return "fallthrough";
...
void be_dump_reqs_and_registers(FILE *const F, ir_node const *const node) {
  ...
  if (info->in_reqs) {
    foreach_irn_in(node, i, op) {
      ...
      reg_out_info_t const *const info =
          be_get_info(skip_Proj_const(op))->out_infos;
      arch_register_t const *const reg =
          info ? arch_get_irn_register(op) : NULL;
  ...
  if (info->out_infos) {
    be_foreach_out(node, o) { dump_req_reg_out(F, node, o); }
  } else {
