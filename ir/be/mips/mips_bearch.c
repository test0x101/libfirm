...
static void mips_finish(void) { mips_free_opcodes(); }
...
static ir_node *mips_new_spill(ir_node *const value, ir_node *const after) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ...
    ir_node *const store =
        new_bd_mips_sw(NULL, block, nomem, frame, value, NULL, 0);
...
static ir_node *mips_new_reload(ir_node *const value, ir_node *const spill,
...
static void mips_collect_frame_entity_nodes(ir_node *const node,
                                            void *const data) {
  ...
  if (is_mips_lw(node)) {
    ...
    ir_node *const frame = get_irg_frame(irg);
    if (base == frame) {
      mips_immediate_attr_t const *const attr =
          get_mips_immediate_attr_const(node);
      if (!attr->ent) {
...
static void mips_set_frame_entity(ir_node *const node, ir_entity *const entity,
                                  unsigned const size,
...
static ir_node *mips_new_IncSP(ir_node *const block, ir_node *const sp,
...
static void mips_sp_sim(ir_node *const node,
