...
static int mips_attrs_equal_(mips_attr_t const *const a_attr,
                             mips_attr_t const *const b_attr) {
#if 0
...
int mips_cond_attrs_equal(ir_node const *const a, ir_node const *const b) {
  ...
  return mips_attrs_equal_(&a_attr->attr, &b_attr->attr) &&
...
void mips_dump_node(FILE *const F, ir_node const *const n,
  ...
  switch (reason) {
    ...
    case dump_node_opcode_txt:
      ...
      switch ((mips_opcodes)get_mips_irn_opcode(n)) {
        ...
        case iro_mips_ori:
        case iro_mips_xori: {
          mips_immediate_attr_t const *const imm =
