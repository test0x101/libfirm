...
static void emit_mips_asm_operand(ir_node const *const node,
                                  char const modifier, unsigned const pos) {
  be_asm_attr_t const *const attr = get_be_asm_attr_const(node);
  mips_asm_operand_t const *const op =
      &((mips_asm_operand_t const *)attr->operands)[pos];
  /* modifiers:
   *   c: plain immediate
   *   l: label only
...
static void emit_jmp(ir_node const *const node, ir_node const *const target) {
  BE_EMIT_JMP(mips, node, "b", target) { mips_emitf(NULL, "nop"); }
}
...
static void emit_mips_b(ir_node const *const node) { emit_jmp(node, node); }

static void emit_mips_bcc(ir_node const *const node) {
  mips_cond_t const cond = get_mips_cond_attr_const(node)->cond;
  char const *const fmt = cond == mips_cc_eq || cond == mips_cc_ne
                              ? "b%C\t%S0, %S1, %L\nnop"
                              : "b%C\t%S0, %L\nnop";
