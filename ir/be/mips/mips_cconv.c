...
void mips_determine_calling_convention(mips_calling_convention_t *const cconv,
  ...
  if (n_params != 0) {
    ...
    for (size_t i = 0; i != n_params; ++i) {
      ...
      if (!param_mode) {
        ...
        if (param_type->flags & tf_lowered_dw && gp_param % 2 != 0) ++gp_param;
  ...
  cconv->param_stack_size =
      MAX(n_reg_params, gp_param) * (MIPS_MACHINE_SIZE / 8);
...
void mips_layout_parameter_entities(mips_calling_convention_t *const cconv,
                                    ir_graph *const irg) {
