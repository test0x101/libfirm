...
static unsigned const caller_saves[] = {
    REG_V0, REG_V1, REG_A0, REG_A1, REG_A2, REG_A3, REG_T0, REG_T1, REG_T2,
    REG_T3, REG_T4, REG_T5, REG_T6, REG_T7, REG_T8, REG_T9, REG_RA,
};
...
static inline bool is_uimm5(long const val) { return 0 <= val && val < 32; }

static inline bool is_simm16(long const val) {
  return -32768 <= val && val < 32768;
}

static inline bool is_uimm16(long const val) { return 0 <= val && val < 65536; }

static inline bool is_imm_lui(long const val) { return (val & 0xFFFFU) == 0; }
...
static void mips_parse_constraint_letter(void const *const env,
...
static bool mips_check_immediate_constraint(long const val,
                                            char const imm_type) {
  switch (imm_type) {
    case 'I':
      return is_simm16(val);
    case 'J':
      return val == 0;
    case 'K':
      return is_uimm16(val);
    case 'L':
      return is_imm_lui(val);
    case 'M':
      return !is_simm16(val) && !is_uimm16(val) && !is_imm_lui(val);
    case 'N':
      return -65535 <= val && val < 0;
    case 'O':
      return -16384 <= val && val < 16384;
    case 'P':
...
static bool mips_match_immediate(mips_asm_operand_t *const operand,
  ...
  if (offset) {
    ...
    if (!mips_check_immediate_constraint(value, imm_type)) return false;
  } else {
    value = 0;
  }
...
static ir_node *gen_ASM(ir_node *const node) {
  ...
  mips_asm_operand_t *const operands =
      NEW_ARR_DZ(mips_asm_operand_t, obst, n_constraints);
  for (size_t i = 0; i != n_constraints; ++i) {
    ...
    be_parse_asm_constraints_internal(&be_constraint, c->constraint,
                                      &mips_parse_constraint_letter, NULL);
...
static ir_node *gen_logic_op(ir_node *const node, cons_binop *const cons,
  ...
  if (is_Const(r)) {
    long const val = get_Const_long(r);
    if (is_uimm16(val)) return cons_imm(dbgi, block, new_l, NULL, val);
  }
  ir_node *const new_r = be_transform_node(r);
...
static ir_node *gen_Call(ir_node *const node) {
  ...
  for (size_t i = 0; i != n_params; ++i) {
    ...
    if (param->reg) {
      ...
      mems[m++] = new_bd_mips_sw(dbgi, block, nomem, call_frame, val, NULL,
                                 param->offset);
    }
  }
  ...
  ir_node *const jal =
      callee ? new_bd_mips_jal(dbgi, block, p, ins, reqs, n_res, callee, 0)
  ...
  for (size_t i = 0; i != ARRAY_SIZE(caller_saves); ++i) {
    arch_set_irn_register_req_out(jal, pn_mips_jal_first_result + i,
                                  mips_registers[caller_saves[i]].single_req);
  }
    ...
    if (be_mode_needs_gp_reg(mode)) {
      ...
      if (is_irn_null(r)) {
        if (mode_is_signed(mode)) {
          ...
          if (rel == ir_relation_greater) rel = ir_relation_less_greater;
          goto normal;
        }
      } else {
      normal:
...
static ir_node *gen_Const(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode)) {
    ...
    if (val == 0) {
      ...
      if (lo != 0) res = new_bd_mips_ori(dbgi, block, res, NULL, lo);
      return res;
    }
  }
  TODO(node);
...
static ir_node *gen_Div(ir_node *const node) {
  ...
  if (be_mode_needs_gp_reg(mode) &&
      get_mode_size_bits(mode) == MIPS_MACHINE_SIZE) {
    dbg_info *const dbgi = get_irn_dbg_info(node);
    ir_node *const block = be_transform_nodes_block(node);
...
static ir_node *gen_Proj_Load(ir_node *const node) {
  ...
  switch ((pn_Load)pn) {
    case pn_Load_M:
      return be_new_Proj(load, pn_mips_lw_M);
    case pn_Load_res:
...
static ir_node *gen_Proj_Proj_Start(ir_node *const node) {
  ...
  if (param->reg) {
    ...
    ir_node *const load =
        new_bd_mips_lw(dbgi, block, mem, base, param->entity, 0);
    return be_new_Proj(load, pn_mips_lw_res);
  }
}
...
static ir_node *gen_shift_op(ir_node *const node, cons_binop *const cons,
  ...
  if (is_Const(r)) {
    long const val = get_Const_long(r);
    if (is_uimm5(val)) return cons_imm(dbgi, block, new_l, NULL, val);
  }
  ir_node *const new_r = be_transform_node(r);
  return cons(dbgi, block, new_l, new_r);
}
