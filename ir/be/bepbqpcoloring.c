...
#define insert_edge(pbqp, src_node, trg_node, template_matrix)        \
  (add_edge_costs(pbqp, get_irn_idx(src_node), get_irn_idx(trg_node), \
                  pbqp_matrix_copy(pbqp, template_matrix)))
#define get_free_regs(restr_nodes, cls, irn) \
  ((cls)->n_regs - restr_nodes[get_irn_idx(irn)])

static const lc_opt_table_entry_t options[] = {
    LC_OPT_ENT_BOOL("exec_freq", "use exec_freq", &use_exec_freq),
    LC_OPT_ENT_BOOL("late_decision",
                    "use late decision for register allocation",
                    &use_late_decision),
    LC_OPT_LAST};
...
static void create_pbqp_node(be_pbqp_alloc_env_t *pbqp_alloc_env,
                             ir_node *irn) {
...
static void insert_ife_edge(be_pbqp_alloc_env_t *pbqp_alloc_env,
                            ir_node *src_node, ir_node *trg_node) {
  ...
  if (get_edge(pbqp, get_irn_idx(src_node), get_irn_idx(trg_node)) == NULL) {
    ...
    /* do useful optimization to speed up pbqp solving (we can do this because
     * we know our matrix) */
    if (get_free_regs(restr_nodes, cls, src_node) == 1 &&
        get_free_regs(restr_nodes, cls, trg_node) == 1) {
      assert(
          vector_get_min_index(get_node(pbqp, get_irn_idx(src_node))->costs) !=
              vector_get_min_index(
                  get_node(pbqp, get_irn_idx(trg_node))->costs) &&
    ...
    if (get_free_regs(restr_nodes, cls, src_node) == 1 ||
      ...
      if (get_free_regs(restr_nodes, cls, src_node) == 1) {
        unsigned idx =
            vector_get_min_index(get_node(pbqp, get_irn_idx(src_node))->costs);
        vector_set(get_node(pbqp, get_irn_idx(trg_node))->costs, idx,
                   INF_COSTS);
      } else {
        unsigned idx =
            vector_get_min_index(get_node(pbqp, get_irn_idx(trg_node))->costs);
        vector_set(get_node(pbqp, get_irn_idx(src_node))->costs, idx,
                   INF_COSTS);
      }
      return;
    }
...
static void insert_afe_edge(be_pbqp_alloc_env_t *pbqp_alloc_env,
                            ir_node *src_node, ir_node *trg_node, int pos) {
  pbqp_t *pbqp = pbqp_alloc_env->pbqp_inst;
  const arch_register_class_t *cls = pbqp_alloc_env->cls;
  unsigned *restr_nodes = pbqp_alloc_env->restr_nodes;
  ...
  if (get_edge(pbqp, get_irn_idx(src_node), get_irn_idx(trg_node)) == NULL) {
    if (use_exec_freq) {
      ...
      ir_node *copy_bl =
          is_Phi(src_node) ? get_Block_cfgpred_block(root_bl, pos) : root_bl;
      int res =
          get_block_execfreq_int(&pbqp_alloc_env->execfreq_factors, copy_bl);

      /* create afe-matrix */
      unsigned row, col;
      for (row = 0; row < colors_n; row++) {
        for (col = 0; col < colors_n; col++) {
          if (row != col) pbqp_matrix_set(afe_matrix, row, col, (num)res);
        }
      }
    } else {
    ...
    if (get_free_regs(restr_nodes, cls, src_node) == 1 ||
      ...
      if (get_free_regs(restr_nodes, cls, src_node) == 1) {
        unsigned regIdx =
            vector_get_min_index(get_node(pbqp, get_irn_idx(src_node))->costs);
        vector_add_matrix_col(get_node(pbqp, get_irn_idx(trg_node))->costs,
                              afe_matrix, regIdx);
      } else {
        unsigned regIdx =
            vector_get_min_index(get_node(pbqp, get_irn_idx(trg_node))->costs);
        vector_add_matrix_col(get_node(pbqp, get_irn_idx(src_node))->costs,
                              afe_matrix, regIdx);
      }
      return;
    }
...
static void create_affinity_edges(ir_node *irn, void *env) {
  ...
  if (is_Phi(irn)) { /* Phis */
    foreach_irn_in(irn, pos, arg) {
      if (!arch_irn_consider_in_reg_alloc(cls, arg)) continue;
    ...
    if (!arch_irn_consider_in_reg_alloc(cls, arg)) return;
    ...
    for (i = 0; 1U << i <= other; ++i) {
      if (other & (1U << i)) {
        ...
        if (!arch_irn_consider_in_reg_alloc(cls, other)) continue;
...
static void create_pbqp_coloring_instance(ir_node *block, void *data) {
  ...
  /* (this is only for compatibility with copymin optimization, it's not needed
   * for pbqp coloring) */
  ...
  sched_foreach_reverse(block, irn) {
    be_foreach_value(
        irn, value, if (!arch_irn_consider_in_reg_alloc(cls, value)) continue;
    ...
    if (get_irn_mode(irn) == mode_T) {
      ...
      foreach_out_edge(irn, edge) {
        ...
        if (!arch_irn_consider_in_reg_alloc(cls, proj)) continue;

        /* insert pbqp node into temp rpeo list of this block */
        ARR_APP1(pbqp_node_t *, temp_list,
                 get_node(pbqp_inst, get_irn_idx(proj)));
      ...
      if (clique_size > 0) {
        for (size_t i = ARR_LEN(temp_list); i-- > 0;) {
          ...
          for (idx = 0; idx < clique_size; idx++) {
            ...
            if (get_edge(pbqp_inst, member->index, clique_candidate->index) ==
                    NULL &&
                get_edge(pbqp_inst, clique_candidate->index, member->index) ==
                    NULL) {
              isMember = false;
              break;
            }
          ...
          /* goto next list element if current node is not a member of the
           * clique */
          if (!isMember) {
            continue;
          }
      ...
      for (nodeIdx = 0; nodeIdx < clique_size; nodeIdx++) {
        ...
        assert(assignment[nodeIdx] >= 0 &&
               "there must have been a register assigned (node not register "
               "pressure faithful?)");
      }
    ...
    if (get_irn_mode(irn) == mode_T) {
      ...
      foreach_out_edge(irn, edge) {
        ...
        /* insert proj node into priority queue (descending by the number of
         * interference edges) */
        if (get_free_regs(restr_nodes, cls, proj) <= 4) {
          pqueue_put(restr_nodes_queue, proj,
                     pbqp_alloc_env->ife_edge_num[get_irn_idx(proj)]);
        } else {
          pqueue_put(queue, proj,
                     pbqp_alloc_env->ife_edge_num[get_irn_idx(proj)]);
        }

        /* skip last step if there is no last_element */
        if (last_element == NULL) continue;

        /* check if proj has an interference edge to last_element (at this time
         * pbqp contains only interference edges) */
        if (get_edge(pbqp_inst, proj->node_idx, last_element->node_idx) ==
                NULL &&
            get_edge(pbqp_inst, last_element->node_idx, proj->node_idx) ==
                NULL) {
          allHaveIFEdges = false; /* there is no interference edge between proj
                                     and last_element */
        }
      }
      ...
      if (last_element != NULL && allHaveIFEdges) {
        if (get_free_regs(restr_nodes, cls, last_element) <= 4) {
          pqueue_put(restr_nodes_queue, last_element,
                     pbqp_alloc_env->ife_edge_num[get_irn_idx(last_element)]);
        } else {
          pqueue_put(queue, last_element,
                     pbqp_alloc_env->ife_edge_num[get_irn_idx(last_element)]);
        }
      ...
      while (!pqueue_empty(restr_nodes_queue)) {
        ...
        ARR_APP1(pbqp_node_t *, sorted_list,
                 get_node(pbqp_inst, get_irn_idx(node)));
      }
      ...
      if (arch_irn_consider_in_reg_alloc(cls, irn)) {
        ...
        last_element = irn;
        ARR_APP1(pbqp_node_t *, temp_list,
                 get_node(pbqp_inst, get_irn_idx(irn)));
      } else {
...
static void insert_perms(ir_node *block, void *data) {
  ...
  sched_foreach_safe(block, irn) {
    ...
    if (insn) pre_process_constraints(env, &insn);
  }
}
...
static void be_pbqp_coloring(be_chordal_env_t *env) {
  ...
  pbqp_alloc_env.pbqp_inst =
      alloc_pbqp(get_irg_last_idx(irg)); /* initialize pbqp instance */
  ...
  pbqp_matrix_t *const ife_matrix =
      pbqp_matrix_alloc(pbqp_alloc_env.pbqp_inst, colors_n, colors_n);
  /* set costs */
  ...
  if (!use_exec_freq) {
    ...
    pbqp_matrix_t *afe_matrix =
        pbqp_matrix_alloc(pbqp_alloc_env.pbqp_inst, colors_n, colors_n);
    /* set costs */
    for (unsigned row = 0; row < colors_n; row++) {
      for (unsigned col = 0; col < colors_n; col++) {
        if (row != col) pbqp_matrix_set(afe_matrix, row, col, 2);
      }
    }
  ...
  if (solution == INF_COSTS) panic("no PBQP solution found");
...
void be_init_pbqp_coloring(void) {
  ...
  static be_ra_chordal_coloring_t coloring = {be_pbqp_coloring};
