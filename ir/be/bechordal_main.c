...
static lc_opt_enum_int_var_t lower_perm_var = {&options.lower_perm_opt,
                                               lower_perm_items};

static lc_opt_enum_mask_var_t dump_var = {&options.dump_flags, dump_items};

static const lc_opt_table_entry_t be_chordal_options[] = {
    LC_OPT_ENT_ENUM_INT("perm", "perm lowering options", &lower_perm_var),
    LC_OPT_ENT_ENUM_MASK("dump", "select dump phases", &dump_var), LC_OPT_LAST};

static be_module_list_entry_t *colorings = NULL;
static const be_ra_chordal_coloring_t *selected_coloring = NULL;

void be_register_chordal_coloring(const char *name,
                                  be_ra_chordal_coloring_t *coloring) {
  if (selected_coloring == NULL) selected_coloring = coloring;
...
void be_chordal_dump(unsigned const mask, ir_graph *const irg,
...
static void memory_operand_walker(ir_node *irn, void *env) {
  ...
  foreach_irn_in(irn, i, in) {
    if (!arch_irn_is(skip_Proj(in), reload)) continue;
    if (get_nodes_block(in) != get_nodes_block(irn)) continue;
    /* only use memory operands, if the reload is only used by 1 node */
    if (get_irn_n_edges(in) > 1) continue;
...
void check_for_memory_operands(ir_graph *irg, const regalloc_if_t *regif) {
  if (regif->perform_memory_operand == NULL) return;
...
static void be_ra_chordal_main(ir_graph *irg, const regalloc_if_t *regif) {
  ...
  if (stat_ev_enabled) be_collect_node_stats(&last_node_stats, irg);
  ...
  for (int j = 0, m = ir_target.isa->n_register_classes; j < m; ++j) {
    ...
    stat_ev_dbl("bechordal_spillcosts",
                be_estimate_irg_costs(irg) - pre_spill_cost);
