...
static unsigned be_compute_block_pressure(
    ir_node *const block, arch_register_class_t const *const cls) {
...
static unsigned be_compute_loop_pressure(be_loopana_t *loop_ana, ir_loop *loop,
  ...
  key.cls = cls;
  key.max_pressure = 0;
  be_loop_info_t *entry = set_insert(be_loop_info_t, loop_ana->data, &key,
                                     sizeof(key), HASH_LOOP_INFO(&key));
  entry->max_pressure = MAX(entry->max_pressure, pressure);

  return pressure;
}

be_loopana_t *be_new_loop_pressure(ir_graph *const irg,
...
unsigned be_get_loop_pressure(be_loopana_t *loop_ana,
  ...
  key.loop = loop;
  key.cls = cls;
  be_loop_info_t *entry = set_find(be_loop_info_t, loop_ana->data, &key,
                                   sizeof(key), HASH_LOOP_INFO(&key));
...
void be_init_loopana(void) { FIRM_DBG_REGISTER(dbg, "firm.be.loopana"); }
