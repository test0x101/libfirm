...
static void introduce_prologue(ir_graph *const irg) {
  ...
  ir_node *const initial_sp =
      be_get_Start_proj(irg, &TEMPLATE_registers[REG_SP]);
...
static void TEMPLATE_finish(void) { TEMPLATE_free_opcodes(); }
...
static unsigned TEMPLATE_get_op_estimated_cost(const ir_node *node) {
  if (is_TEMPLATE_Load(node)) return 5;
  if (is_TEMPLATE_Store(node)) return 7;
  return 1;
}
...
void be_init_arch_TEMPLATE(void) { TEMPLATE_init_transform(); }
