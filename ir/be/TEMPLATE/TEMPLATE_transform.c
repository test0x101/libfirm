...
static ir_node *transform_const(ir_node *const node, ir_entity *const entity,
...
static ir_node *transform_unop(ir_node *node, int op_index,
                               new_unop_func new_func) {
  ir_node *new_block = be_transform_nodes_block(node);
  dbg_info *dbgi = get_irn_dbg_info(node);
...
static void setup_calling_convention(ir_graph *irg) {
  ...
  unsigned *allocatable_regs =
      rbitset_obstack_alloc(obst, N_TEMPLATE_REGISTERS);
