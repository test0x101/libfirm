...
#define DBG_AFF_CHUNK(env, level, chunk)                                 \
  do {                                                                   \
    if (firm_dbg_get_mask(dbg) & (level)) dbg_aff_chunk((env), (chunk)); \
  } while (0)
#define DBG_COL_COST(env, level, cost)                                 \
  do {                                                                 \
    if (firm_dbg_get_mask(dbg) & (level)) dbg_col_cost((env), (cost)); \
  } while (0)
...
typedef struct co_mst_env_t {
  bitset_t const
      *allocatable_regs; /**< set containing all global ignore registers */
...
typedef struct co_mst_irn_t {
  ...
  ir_node **int_neighs;  /**< array of all interfering neighbours (cached for
                            speed reasons) */
...
static co_mst_irn_t *co_mst_irn_init(co_mst_env_t *env, const ir_node *irn) {
  ...
  res->tmp_col = -1;
  res->int_neighs = NULL;
  /* set the number of interfering affinity neighbours to -1, they are
   * calculated later */
  res->int_aff_neigh = -1;
  ...
  if (req->limited != NULL) rbitset_and(adm->data, req->limited, n_regs);
...
static void dbg_aff_chunk(const co_mst_env_t *env, const aff_chunk_t *c) {
  (void)env;
  if (c->weight_consistent) ir_fprintf(stderr, " $%d ", c->weight);
  ir_fprintf(stderr, "{");
...
static void dbg_admissible_colors(const co_mst_env_t *env,
  ...
  if (bitset_popcount(node->adm_colors) < 1) {
    ...
    bitset_foreach(node->adm_colors, idx) { ir_fprintf(stderr, " %zu", idx); }
  }
}
...
static bool nodes_insert(const ir_node ***arr, const ir_node *irn) {
  ...
  idx = ~idx;
  const ir_node **l = *arr;
  for (int i = ARR_LEN(*arr) - 2; i >= idx; --i) l[i + 1] = l[i];
  l[idx] = irn;
  return true;
}
...
static inline void aff_chunk_add_node(aff_chunk_t *c, co_mst_irn_t *node) {
  if (!nodes_insert(&c->n, node->irn)) return;
...
static inline bool aff_chunks_interfere(const aff_chunk_t *c1,
  ...
  for (size_t i = ARR_LEN(c2->n); i-- > 0;) {
    ...
    if (node_contains(c1->interfere, irn)) return true;
  }
  return false;
}
 ...
 */
static inline aff_chunk_t *get_aff_chunk(co_mst_env_t *env,
                                         const ir_node *irn) {
 ...
 */
static void aff_chunk_absorb(co_mst_env_t *const env, ir_node const *const src,
                             ir_node const *const tgt) {
  ...
  if (c1 == NULL) {
    if (c2 == NULL) {
      ...
      for (i = mirn->n_neighs; i-- > 0;) {
        if (mirn->int_neighs[i] == tgt) break;
      }
      if (i < 0) {
  ...
  if (!c->weight_consistent) {
    ...
    for (unsigned idx = 0, len = ARR_LEN(c->n); idx < len; ++idx) {
      ...
      if (node->constr_factor > REAL(0.0)) {
        bitset_foreach(node->adm_colors, col) c->color_affinity[col].cost +=
            node->constr_factor;
      }
      ...
      if (an != NULL) {
        co_gs_foreach_neighb(an, neigh) {
          ...
          if (arch_irn_is_ignore(m)) continue;
...
static unsigned count_interfering_aff_neighs(co_mst_env_t *env,
  ...
  co_gs_foreach_neighb(an, neigh) {
    ...
    if (arch_irn_is_ignore(n)) continue;
...
static void build_affinity_chunks(co_mst_env_t *env) {
  ...
  for (size_t i = 0, len = ARR_LEN(edges); i < len; ++i) {
    DBG((dbg, LEVEL_1, "edge (%u,%u) %f\n", edges[i].src->node_idx,
         edges[i].tgt->node_idx, edges[i].weight));
  ...
  for (size_t pn = 0, n = ARR_LEN(env->map.data); pn < n; ++pn) {
    ...
    if (mirn == NULL) continue;
    if (mirn->chunk != NULL) continue;
...
/**
 * Greedy collect affinity neighbours into thew new chunk @p chunk starting at
 * node @p node.
 */
  ...
  while (!deq_empty(&nodes)) {
    ...
    if (an != NULL) {
      co_gs_foreach_neighb(an, neigh) {
        ...
        co_mst_irn_t *n2 = get_co_mst_irn(env, m);
        unsigned m_idx = get_irn_idx(m);
        if (!bitset_is_set(visited, m_idx) && decider(n2, col) && !n2->fixed &&
...
static aff_chunk_t *fragment_chunk(co_mst_env_t *env, unsigned col,
  ...
  for (unsigned idx = 0, len = ARR_LEN(c->n); idx < len; ++idx) {
    ...
    if (bitset_is_set(visited, get_irn_idx(irn))) continue;
...
static void determine_color_costs(co_mst_env_t *env, co_mst_irn_t *node,
  ...
  for (unsigned i = 0; i < n_regs; ++i) {
    ...
    costs[i].col = i;
    costs[i].cost =
        bitset_is_set(node->adm_colors, i) ? node->constr_factor : REAL(0.0);
  }
...
static bool change_node_color_excluded(co_mst_env_t *env, co_mst_irn_t *node,
  ...
  if (col != exclude_col) {
    if (is_loose(node)) set_temp_color(node, col, changed);
    return true;
  }
  ...
  if (is_loose(node)) {
    ...
    /* Since the node must not have the not_col, set the costs for that color to
     * "infinity" */
...
static bool recolor_nodes(co_mst_env_t *env, co_mst_irn_t *node,
  ...
  for (unsigned i = 0, n = env->n_regs; i < n; ++i) {
    ...
    /* If the costs for that color (and all successive) are infinite, bail out
    ...
    for (unsigned j = node->n_neighs; j-- != 0;) {
      ...
      DB((dbg, LEVEL_4,
          "\tHandling neighbour %+F, at position %d (fixed: %d, tmp_col: %d, "
          "col: %d)\n",
          neigh, j, nn->fixed, nn->tmp_col, nn->col));

      /*
              Try to change the color of the neighbor and record all nodes which
              get changed in the tmp list. Add this list to the "changed" list
         for that color. If we did not succeed to change the color of the
         neighbor, we bail out and try the next color.
      */
    ...
    /*
            We managed to assign the target color to all neighbors, so from the
       perspective of the current node, every thing was ok and we can return
       safely.
    */
    if (neigh_ok) {
...
static bool change_node_color(co_mst_env_t *env, co_mst_irn_t *node,
  ...
  if (col == tgt_col) {
    DBG((dbg, LEVEL_4, "\t\tCNC: %+F has already color %d, fix temporary\n",
         node->irn, tgt_col));
    if (is_loose(node)) set_temp_color(node, tgt_col, changed);
    return true;
  }
  ...
  */
  ...
  if (firm_dbg_get_mask(dbg) & LEVEL_4) {
    if (is_loose(node)) {
      DB((dbg, LEVEL_4, "\t\tCNC: color %d not admissible for %+F (", tgt_col,
          node->irn));
...
static void color_aff_chunk(co_mst_env_t *env, aff_chunk_t *c) {
  ...
  for (unsigned i = 0, n = env->n_regs; i < n; ++i) {
    real_t dislike =
        n_int_chunks > 0 ? REAL(1.0) - order[i].cost / n_int_chunks : REAL(0.0);
    order[i].col = i;
    order[i].cost =
        (REAL(1.0) - dislike_influence) * c->color_affinity[i].cost +
        dislike_influence * dislike;
  }
  ...
  for (unsigned i = 0, n = env->n_regs; i < n; ++i) {
    ...
    if (!bitset_is_set(env->allocatable_regs, col)) continue;
    ...
    for (size_t idx = 0, len = ARR_LEN(c->n); idx < len; ++idx) {
      ...
      DB((dbg, LEVEL_4, "\t\t... %+F attempt from %d to %d %s\n", irn,
          node->col, col, good ? "succeeded" : "failed"));
    }
    ...
    if (local_best) {
      ...
      DB((dbg, LEVEL_3,
          "\t\tlocal best chunk (id %u) for color %d: ", local_best->id, col));
  ...
  while (!deq_empty(&tmp_chunks)) {
    ...
    if (tmp != best_chunk) delete_aff_chunk(tmp);
  }
  deq_free(&tmp_chunks);

  /* return if coloring failed */
  if (!best_chunk) return;
  ...
  /* we have to get the nodes back into the original chunk because they are
   * scattered over temporary chunks */
  for (size_t idx = 0, len = ARR_LEN(c->n); idx < len; ++idx) {
    const ir_node *n = c->n[idx];
  ...
  for (size_t idx = 0, len = ARR_LEN(c->n); idx < len; ++idx) {
    ...
    if (!bitset_is_set(visited, get_irn_idx(irn))) {
      ...
      expand_chunk_from(env, node, visited, new_chunk, c, decider_always_yes,
                        0);
...
static int co_solve_heuristic_mst(copy_opt_t *co) {
  ...
  for (size_t pn = 0; pn < ARR_LEN(mst_env.map.data); ++pn) {
    ...
    if (mirn->init_col == mirn->col) continue;
...
static const lc_opt_table_entry_t options[] = {
    ...
    LC_OPT_ENT_DBL("di", "dislike influence", &dislike_influence), LC_OPT_LAST};
...
void be_init_copyheur4(void) {
  ...
  static co_algo_info copyheur = {co_solve_heuristic_mst};
