...
void normalize_one_return(ir_graph *irg) {
  ...
  endbl_in[last_idx++] =
      new_rd_Return(combined_dbgi, block, in[0], n_ret_vals - 1, &in[1]);
...
static bool can_move_ret(ir_node *ret) {
  ...
  if (n <= 1 || get_Block_entity(retbl) != NULL) return false;
  for (int i = 0; i < n; ++i) {
...
void normalize_n_returns(ir_graph *irg) {
  ...
  while (list != NULL) {
    ...
    for (int i = 0, n = get_Block_n_cfgpreds(block); i < n; ++i) {
      ...
      ir_node *new_ret =
          new_rd_Return(dbgi, new_bl, in[0], n_ret_vals - 1, &in[1]);
      if (!is_Bad(new_ret)) {
        /*
  ...
  confirm_irg_properties(irg, IR_GRAPH_PROPERTY_NO_TUPLES |
                                  IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES |
                                  IR_GRAPH_PROPERTY_CONSISTENT_ENTITY_USAGE);
  add_irg_properties(irg, IR_GRAPH_PROPERTY_MANY_RETURNS);
}
