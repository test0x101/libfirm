...
static bool is_Or_Eor_Add(const ir_node *node) {
  if (is_Or(node) || is_Eor(node) || is_Add(node)) {
     ...
     */
    if (bl && br && tarval_is_null(tarval_and(bl->z, br->z))) return true;
  }
  return false;
}
...
static bool is_Eor_Add(const ir_node *node) {
  if (is_Or_Eor_Add(node)) return true;
  if (!is_Eor(node) && !is_Add(node)) return false;

  const ir_mode *mode = get_irn_mode(node);
  if (get_mode_arithmetic(mode) != irma_twos_complement) return false;
...
static ir_tarval *is_shl_const_like(const ir_node *node) {
  if (is_Shl(node)) {
    ...
    if (is_Const(right)) return get_Const_tarval(right);
  } else if (is_Mul(node)) {
...
static bool is_size_minus_1(ir_node const *const irn, ir_mode *const mode) {
  if (is_Const(irn)) {
    ...
    if (tarval_is_long(tv) &&
        get_tarval_long(tv) == (long)get_mode_size_bits(mode) - 1)
      return true;
  }
  return false;
}
...
int value_not_null(const ir_node *n, const ir_node **confirm) {
  ...
  for (;;) {
    ...
    switch (get_Confirm_relation(n)) {
      case ir_relation_equal: /* n == C && C != 0 ==> n != 0 */
        if (relation != ir_relation_equal) goto confirmed;
        break;
      ...
      case ir_relation_less_equal: /* n <= C /\ C <  0 ==> n != 0 */
        if (relation == ir_relation_less) goto confirmed;
        break;
      case ir_relation_greater_equal: /* n >= C /\ C >  0 ==> n != 0 */
        if (relation == ir_relation_greater) goto confirmed;
        break;
  ...
  if (is_Alloc(n)) return true;

  const bitinfo *bi = get_bitinfo(n);
  if (bi != NULL && !tarval_is_null(bi->o)) return true;
...
static bool complement_values(const ir_node *a, const ir_node *b) {
  ...
  if ((is_Not(a) && get_Not_op(a) == b) || (is_Not(b) && get_Not_op(b) == a))
    return true;
...
static ir_tarval *computed_value_Add(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_add(ta, tb);

  /* x+~x => -1 */
  if (complement_values(a, b)) return get_mode_all_one(get_irn_mode(n));
  /* x + -x => 0 */
...
static ir_tarval *computed_value_Sub(const ir_node *n) {
  ...
  ir_tarval *ta = value_of(a);
  ir_tarval *tb = value_of(b);
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_sub(ta, tb);
...
static ir_tarval *computed_value_Minus(const ir_node *n) {
  ...
  if (ta != tarval_unknown) return tarval_neg(ta);
...
static ir_tarval *computed_value_Mul(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_mul(ta, tb);
...
static ir_tarval *computed_value_And(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_and(ta, tb);

  if (tarval_is_null(ta)) return ta;
  if (tarval_is_null(tb)) return tb;

  /* x&~x => 0 */
  if (complement_values(a, b)) return get_mode_null(get_irn_mode(n));
...
static ir_tarval *computed_value_Or(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_or(ta, tb);
...
static ir_tarval *computed_value_Eor(const ir_node *n) {
  ...
  /* a ^ a == 0 */
  if (a == b) return get_mode_null(get_irn_mode(n));
  ...
  ir_tarval *ta = value_of(a);
  ir_tarval *tb = value_of(b);
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_eor(ta, tb);
...
static ir_tarval *computed_value_Not(const ir_node *n) {
  ...
  if (ta != tarval_unknown) return tarval_not(ta);
...
static ir_tarval *computed_value_Shl(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_shl(ta, tb);
  if (is_oversize_shift(n)) return get_mode_null(get_irn_mode(n));
...
static ir_tarval *computed_value_Shr(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_shr(ta, tb);
  if (a == b || is_oversize_shift(n)) return get_mode_null(get_irn_mode(n));
  return tarval_unknown;
}
...
static ir_tarval *computed_value_Shrs(const ir_node *n) {
  ...
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_shrs(ta, tb);
  return tarval_unknown;
}
...
bool ir_zero_when_converted(const ir_node *node, ir_mode *dest_mode) {
  ...
  if (tv != NULL && tarval_is_long(tv)) {
    ...
    if (shiftval >= destbits && shiftval < (long)get_mode_modulo_shift(mode))
      return true;
  }
  if (is_And(node)) {
...
static ir_tarval *computed_value_Conv(const ir_node *n) {
  ...
  if (ta != tarval_unknown) return tarval_convert_to(ta, mode);
  if (ir_zero_when_converted(a, mode)) return get_mode_null(mode);
...
static ir_tarval *computed_value_Bitcast(const ir_node *n) {
  ...
  if (ta == tarval_unknown) return tarval_unknown;
...
static ir_tarval *computed_value_Confirm(const ir_node *n) {
  ...
  switch (possible & relation) {
    ...
    case ir_relation_equal: {
      ...
      if (tarval_is_constant(tv)) return tv;
      break;
    }
  }
...
static ir_node *get_commutative_other_op(const ir_node *const node,
                                         const ir_node *const op) {
...
ir_relation ir_get_possible_cmp_relations(const ir_node *left,
  ...
  if (tarval_is_nan(tv_l) || tarval_is_nan(tv_r)) return ir_relation_unordered;

  /* a == a is never less or greater (but might be equal or unordered) */
  if (left == right) possible &= ~ir_relation_less_greater;
  /* unordered results only happen for float compares */
  const ir_mode *mode = get_irn_mode(left);
  if (!mode_is_float(mode)) possible &= ~ir_relation_unordered;
  ...
  if (tv_l == min) possible &= ~ir_relation_greater;
  if (tv_l == max) possible &= ~ir_relation_less;
  if (tv_r == max) possible &= ~ir_relation_greater;
  if (tv_r == min) possible &= ~ir_relation_less;
  ...
  if (is_Alloc(left) && tarval_is_null(tv_r)) possible &= ~ir_relation_equal;
  if (is_And(left) && !mode_is_signed(mode) &&
      get_commutative_other_op(left, right))
    possible &= ~ir_relation_greater;
  if (is_And(right) && !mode_is_signed(mode) &&
      get_commutative_other_op(right, left))
    possible &= ~ir_relation_less;
 ...
 */
static bool is_relation(ir_relation relation, ir_relation cmp_relation,
 ...
 *         possible.
 */
static ir_relation get_complementary_relations(ir_relation const relation,
                                               ir_relation const cmp_relation,
                                               ir_relation const possible) {
  assert(possible != ir_relation_false);
  if (is_relation(relation, cmp_relation, possible)) return relation;
  ir_relation const compl = relation ^ ir_relation_less_equal_greater;
  if (is_relation(compl, cmp_relation, possible)) return compl ;
...
static ir_tarval *computed_value_Cmp(const ir_node *cmp) {
  ...
  if ((possible & relation) == ir_relation_false) return tarval_b_false;
  /* if possible relations are a subset of the requested ones return true */
  if ((possible & ~relation) == ir_relation_false) return tarval_b_true;

  /* we have some special rules for == 0 and != 0 */
  ir_relation const rel_eq =
      get_complementary_relations(ir_relation_equal, relation, possible);
  if (rel_eq != ir_relation_false && is_irn_null(right) &&
      value_not_null(left, NULL))
...
static ir_tarval *compute_cmp_ext(const ir_node *cmp) {
  if (!get_opt_constant_folding()) return tarval_unknown;
  ...
  switch (get_Builtin_kind(builtin)) {
    ...
    case ir_bk_ctz: {
      ...
      if (b) {
        ...
        if (val != -1 && tarval_get_bit(b->o, val)) goto make_val;
      }
      break;
    }
...
static ir_tarval *do_computed_value_Div(const ir_node *a, const ir_node *b) {
  ...
  ir_tarval *tb = value_of(b);
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_div(ta, tb);
  return tarval_unknown;
}
...
static ir_tarval *do_computed_value_Mod(const ir_node *a, const ir_node *b) {
  ...
  /* constant folding */
  ir_tarval *ta = value_of(a);
  if (ta != tarval_unknown && tb != tarval_unknown) return tarval_mod(ta, tb);

  /* 0 % b == 0 if b != 0 */
  /* a % a == 0 if a != 0 */
  if (tarval_is_null(ta) || a == b) {
    assert(get_mode_arithmetic(mode) == irma_twos_complement);
    if (value_not_null(b, NULL)) return get_mode_null(mode);
  }
...
static ir_tarval *computed_value_Proj_Div(const ir_node *n) {
  ...
  if (proj_nr != pn_Div_res) return tarval_unknown;
...
ir_tarval *computed_value(const ir_node *n) {
  ...
  if (vrp != NULL && vrp->bits_set == vrp->bits_not_set) return vrp->bits_set;

  if (n->op->ops.computed_value) return n->op->ops.computed_value(n);
  return tarval_unknown;
}
...
static ir_node *equivalent_node_Add(ir_node *n) {
  ...
  if (mode_is_float(mode)) {
    ...
    if (n != oldn) return n;
    if (!ir_imprecise_float_transforms_allowed()) return n;
  }
...
static ir_node *equivalent_node_Bitcast(ir_node *n) {
  ...
  while (is_Bitcast(op)) {
    ...
    if (get_irn_mode(prev_op) == get_irn_mode(n)) return prev_op;
    op = prev_op;
  }
  return n;
}
...
static ir_node *equivalent_node_Phi(ir_node *n) {
  ...
  if (is_Dummy(first_val)) return n;
...
static ir_node *equivalent_node_Pin(ir_node *n) {
  ...
  /* Pin(Pin(x)) -> Pin(x) */
  if (is_Pin(op) && get_nodes_block(op) == get_nodes_block(n)) return op;
  return n;
}
...
static ir_node *equivalent_node_Mux(ir_node *n) {
  ...
  if (!tarval_is_constant(ts) && is_Cmp(sel)) ts = compute_cmp_ext(sel);
  ...
  if (is_Cmp(sel) && !mode_has_signed_zero(get_irn_mode(n))) {
    ...
    const bool is_relation_equal =
        is_relation(ir_relation_equal, relation, possible);
    const bool is_relation_less_greater =
        is_relation(ir_relation_less_greater, relation, possible);
    const bool is_relation_unordered_less_greater =
        is_relation(ir_relation_unordered_less_greater, relation, possible);
    ...
    if (cmp_l == t && tarval_is_null(value_of(cmp_r))) {
      ...
      if (is_Minus(f) && get_Minus_op(f) == t) {
        ...
        if (is_relation_equal) {
          ...
          n = f;
          DBG_OPT_ALGSIM0(oldn, n);
        } else if (is_relation_less_greater ||
                   is_relation_unordered_less_greater) {
          /* Mux(t != 0, -t, t)  ==> t */
          n = t;
          DBG_OPT_ALGSIM0(oldn, n);
        }
    ...
    /* Mux(a == 0, 0, -a) ==> 0 */
    if (is_irn_null(f) && is_Minus(t) && is_relation_equal &&
        is_irn_null(cmp_r) && cmp_l == get_Minus_op(t)) {
      return f;
    }
  }
...
static ir_node *equivalent_node_Confirm(ir_node *n) {
  ...
  if (relation == ir_relation_equal) return pred;
...
ir_node *equivalent_node(ir_node *n) {
  if (n->op->ops.equivalent_node) return n->op->ops.equivalent_node(n);
  return n;
}
...
static bool only_one_user(const ir_node *node) {
  ...
  if (!edges_activated(irg)) return false;
...
static bool is_const_Phi(ir_node *n) {
  if (!is_Phi(n) || get_irn_arity(n) == 0) return false;
 ...
 * @param mode   the mode of the result, may be different from the mode of the
 * Phi!
...
static ir_node *apply_binop_on_phi(ir_node *phi, ir_tarval *other,
  ...
  if (left) {
    foreach_irn_in(phi, i, pred) {
      ...
      if (!tarval_is_constant(evaluated)) return NULL;
      tvs[i] = evaluated;
    }
  } else {
    foreach_irn_in(phi, i, pred) {
 ...
 * @return a new Phi node if the conversion was successful, NULL else
 */
static ir_node *apply_binop_on_2_phis(ir_node *a, ir_node *b, tarval_binop eval,
                                      ir_mode *mode) {
  if (get_nodes_block(a) != get_nodes_block(b)) return NULL;
  ...
  for (int i = 0; i < n; ++i) {
    ...
    /* abort if folding failed */
    if (!tarval_is_constant(tv)) return NULL;
    tvs[i] = tv;
  }
...
static ir_node *apply_binop_on_mux(ir_node *mux, ir_tarval *other,
                                   tarval_binop eval, bool left) {
  if (!only_one_user(mux)) return NULL;
 ...
 * @return a new Mux node if the conversion was successful, NULL else
 */
static ir_node *apply_binop_on_2_muxs(ir_node *a, ir_node *b,
                                      tarval_binop eval) {
  if (!only_one_user(a) || !only_one_user(b)) return NULL;
  if (get_nodes_block(a) != get_nodes_block(b)) return NULL;
 ...
 */
static bool is_cmp_equality_zero(const ir_node *left, const ir_node *right,
                                 ir_relation relation) {
  if (!is_irn_null(right)) return false;

  ir_relation possible = ir_get_possible_cmp_relations(left, right);

  return get_complementary_relations(ir_relation_equal, relation, possible) !=
         ir_relation_false;
}

static bool is_binop_const(ir_node *const irn, ir_node *const left,
                           ir_tarval *const val) {
  if (!irn || get_binop_left(irn) != left) return false;
...
static ir_node *transform_bitop_chain(ir_node *const irn) {
  ...
  for (;;) {
    if (is_Not(current)) {
      ...
      if (!is_Const(right)) goto chain_end;
      ...
      switch (get_irn_opcode(current)) {
        ...
        case iro_Add:
          if (!is_Eor_Add(current)) goto chain_end;
          /* FALLTHROUGH */
        case iro_Eor:
...
static ir_node *new_binop(ir_node *const old, ir_node *const block,
                          ir_node *const l, ir_node *const r) {
  ...
  ir_node *const n =
      new_ir_node(dbgi, irg, block, op, mode, ARRAY_SIZE(in), in);
...
static ir_node *transform_bitwise_distributive(ir_node *n) {
  ...
  if (op != get_irn_op(b)) return n;

  if (!only_one_user(a) && !only_one_user(b)) return n;
  ...
  if (op == op_Eor) return n; /* nothing to gain here */

  if (op == op_Shrs || op == op_Shr || op == op_Shl || op == op_And ||
      op == op_Or || op == op_Eor) {
...
static ir_node *transform_node_Or_(ir_node *n) {
  ...
  if (is_Cmp(a) && is_Cmp(b)) {
    ...
    if (is_cmp_unequal(a) && is_cmp_unequal(b) &&
        !mode_is_float(cmp_mode_left) && !mode_is_float(cmp_mode_right)) {
...
static ir_node *transform_node_Eor_(ir_node *n) {
  ...
  if (is_And(tb) || is_Or(tb)) {
    ...
    if (ta == l && only_one_user(tb)) {
      ...
      if (nots) t = new_rd_Not(dbgi, block, t);
      return t;
    }
  }
...
static ir_node *transform_node_Add(ir_node *n) {
  ...
  /* these optimizations are imprecise for floating-point ops */
  if (mode_is_float(mode) && !ir_imprecise_float_transforms_allowed()) return n;
    ...
    if (get_mode_arithmetic(mode) == irma_twos_complement) {
      ...
      if (is_Conv(b)) {
        ...
        normalize_p:;
          ...
          dbg_info *const dbgi = get_irn_dbg_info(n);
          ir_node *const block = get_nodes_block(n);
          ir_mode *const offset_mode = get_reference_offset_mode(ref_mode);
          ir_node *const conv_o = new_r_Conv(block, other_op, offset_mode);
...
static ir_node *transform_node_Sub(ir_node *n) {
  ...
  if (is_Sub(b)) {
    ...
    if (mode_is_reference(s_mode)) {
      ...
      if (s_mode != mode) s_right = new_r_Conv(lowest_block, s_right, mode);
    ...
    if (mode == get_irn_mode(b)) {
      ...
      if (mode_is_reference(ma) && ma == get_irn_mode(op_b)) {
        ...
        if (ma_size == mode_size) {
          ...
          dbg_info *const dbgi = get_irn_dbg_info(n);
          ir_node *const block = get_nodes_block(n);
          ir_node *const new_sub = new_rd_Sub(dbgi, block, op_a, op_b);
          ir_node *const conv = new_rd_Conv(dbgi, block, new_sub, mode);
          return conv;
        }
      }
    }
  ...
  if (get_mode_arithmetic(mode) == irma_twos_complement) {
    ...
    if (is_Conv(a) && mode_is_int(mode)) {
      ...
      if (mode_is_reference(ref_mode) &&
        ...
        ir_node *const block = get_nodes_block(n);
        ir_mode *const offset_mode = get_reference_offset_mode(ref_mode);
        ir_node *const conv_b = new_r_Conv(block, b, offset_mode);
...
static ir_node *can_negate_cheaply(dbg_info *const dbgi, ir_node *const node) {
  ...
  /* -(-X) => X */
  if (is_Minus(node)) return get_Minus_op(node);
  /* -(a-b) => b-a */
...
static bool is_zero_or_all_one(ir_node const *const n) {
  if (is_Shrs(n)) {
    ...
    if (is_size_minus_1(r, get_irn_mode(n))) return true;
  }
  return false;
}
...
static ir_node *transform_node_Mul(ir_node *n) {
  ...
  if (is_Const(b)) {
    ...
    } else if (arith == irma_twos_complement && get_tarval_popcount(c1) == 1) {
  ...
  if (arith == irma_twos_complement) {
    ...
    if (is_Proj(a)) {
      ...
      if (is_Div(div)) {
        ...
        if (div_right == b) goto transform_div_mul;
      }
    }
    if (is_Proj(b)) {
      div_proj = b;
      ...
      if (is_Div(div)) {
        ...
        transform_div_mul:
          if (only_one_user(div_proj)) {
            ...
            ir_node *mod = new_rd_Mod(dbgi, div_block, div_mem, div_left,
                                      div_right, pinned);
...
static ir_node *transform_node_Div(ir_node *n) {
  ...
  if (mode_is_int(mode)) {
    ...
    if (tarval_is_constant(tv)) {
      ...
      if (tarval_is_constant(tv) && (tarval_ieee754_get_exact() ||
...
static ir_node *transform_node_Mod(ir_node *n) {
  ...
  make_tuple:;
    ...
    /* skip a potential Pin */
    mem = skip_Pin(mem);
    ir_node *in[pn_Mod_max + 1] = {[pn_Mod_M] = mem, [pn_Mod_res] = value};
    int n_in = 2;
...
static ir_node *transform_node_Cond(ir_node *n) {
  ...
  if (get_irg_pinned(irg) == op_pin_state_floats) return n;

  ir_node *a = get_Cond_selector(n);
  ir_tarval *ta = value_of(a);
  /* try again with a direct call to compute_cmp, as we don't care
   * about the MODEB_LOWERED flag here */
  if (!tarval_is_constant(ta) && is_Cmp(a)) ta = compute_cmp_ext(a);
  ...
  if (tarval_is_constant(ta)) {
    ...
    clear_irg_properties(
        irg, IR_GRAPH_PROPERTY_NO_UNREACHABLE_CODE | IR_GRAPH_PROPERTY_NO_BADS |
...
static ir_node *transform_node_shift_bitop(ir_node *n) {
  ...
  if (!is_Const(right)) return n;

  ir_node *left = get_binop_left(n);
  ir_op *op_left = get_irn_op(left);
  if (op_left != op_And && op_left != op_Or && op_left != op_Eor) return n;

  /* doing it with Shrs is not legal if the Or/Eor affects the topmost bit */
  /* TODO: test if sign bit is affected */
  if (is_Shrs(n) && (op_left == op_Or || op_left == op_Eor)) return n;

  ir_node *bitop_right = get_binop_right(left);
  if (!is_Const(bitop_right)) return n;
...
static ir_node *transform_node_And(ir_node *n) {
  ...
  n = transform_bitwise_distributive(n);
  if (is_And(n)) n = transform_node_bitop_shift(n);
...
static ir_node *transform_node_Minus(ir_node *n) {
  ...
  if (negated_op != NULL) return negated_op;
...
static ir_node *transform_node_Proj_Builtin(ir_node *proj) {
  ...
  if (get_Builtin_kind(builtin) != ir_bk_saturating_increment) return proj;
  ...
  if (value_not_null(b, &confirm)) {
    ...
    switch (proj_nr) {
      ...
      case pn_Div_M: {
        ...
        if (confirm) new_mem = new_r_Pin(get_nodes_block(confirm), new_mem);
...
static bool is_single_bit(const ir_node *node) {
  ...
  if (is_Shl(node)) {
    ...
    if (is_irn_one(shl_l) && 0 < modulo &&
        modulo <= (int)get_mode_size_bits(mode)) {
      return true;
    }
...
static ir_node *flips_bit(const ir_node *node, ir_tarval *tv) {
  if (is_Not(node)) return get_Not_op(node);
  if (is_Eor(node)) {
    ir_node *right = get_Eor_right(node);
    if (is_Const(right)) {
      ir_tarval *right_tv = get_Const_tarval(right);
      if (!tarval_is_null(tarval_and(right_tv, tv))) return get_Eor_left(node);
    }
  }
  return NULL;
}
...
restart:
   ...
   */
  if (!mode_overflow_on_unary_Minus(mode) && is_Minus(left) &&
      is_Minus(right)) {
  ...
  ir_relation rel_eq =
      get_complementary_relations(ir_relation_equal, relation, possible);
  /* The following operations are NOT safe for floating point operations, for
   * instance 1.0 + inf == 2.0 + inf, =/=> x == y */
  if (rel_eq != ir_relation_false &&
      (mode_is_int(mode) || mode_is_reference(mode))) {
    ...
    if (is_Add(left) || is_Eor(left) || is_Sub(left) || is_Or_Eor_Add(left)) {
      ...
      if (x) goto cmp_x_eq_0;
    }
    if (is_Add(right) || is_Eor(right) || is_Sub(right) ||
        is_Or_Eor_Add(right)) {
      if (is_Sub(right)) {
      ...
      if (is_Shr(ll) && is_Const(lr)) {
        ...
        if (is_Const(llr)) {
          ...
          if (l1 + h2 < bits && l1 + h3 < bits) {
            ...
            left = new_rd_And(dbg, block, get_Shr_left(ll),
                              new_r_Const(irg, mask));
  ...
  if (tarval_is_constant(tv)) {
    ...
    if (is_Conv(left) && tarval_is_null(tv)) {
      ...
      if (get_mode_size_bits(mode) > get_mode_size_bits(op_mode) &&
          (rel_eq != ir_relation_false || mode_is_signed(mode) ||
           !mode_is_signed(op_mode)) &&
          !mode_is_float(op_mode)) {
        if (!mode_is_signed(mode) && mode_is_signed(op_mode)) relation = rel_eq;
    ...
    if (mode_is_int(mode) && !is_Const(left)) {
      if (!is_relation(ir_relation_false, relation, possible) &&
        ...
        if ((relation == ir_relation_less ||
          ...
          if (bl) {
            ir_tarval *const uneq =
                tarval_or(tarval_andnot(tv, bl->z), tarval_andnot(bl->o, tv));
            ...
            if (hi >= 0) {
              ...
              tv = tarval_or(tarval_andnot(tarval_sub(tv, hibit), mask),
                             tarval_and(bl->z, mask));
              goto reduced_tv;
            }
          }
          ...
          if (bl) {
            ...
            if (hi >= 0) {
              ...
              ir_tarval *const hibit = tarval_shl_unsigned(one, hi);
              ir_tarval *const mask = tarval_sub(hibit, one);
              tv = tarval_or(tarval_andnot(tarval_add(tv, hibit), mask),
                             tarval_and(bl->o, mask));
              goto reduced_tv;
            }
          }
    ...
    if (rel_eq != ir_relation_false) {
      switch (get_irn_opcode(left)) {
        ...
        case iro_And:
          ...
          if (is_Const(c1)) {
            ...
            if (mode_is_signed(mode) &&
              ...
              relation = rel_eq != ir_relation_equal
                             ? ir_relation_less
        ...
        case iro_Shl:
          /*
           * optimize x << c1 == c into x & (-1 >>u c1) == c >> c1  if  c & (-1
           * << c1) == c FALSE                       else optimize x << c1 != c
           * into x & (-1 >>u c1) != c >> c1  if  c & (-1 << c1) == c TRUE else
           */
          ...
          if (is_Const(c1)) {
            ...
            left = new_rd_And(get_irn_dbg_info(left), blk, sl,
                              new_r_Const(irg, amask));
            right = new_r_Const(irg, tv);
            relation = rel_eq;
            changed = true;
            DBG_OPT_ALGSIM0(n, n);
        ...
        case iro_Shr:
          /*
           * optimize x >>u c1 == c into x & (-1 << c1) == c << c1  if  c & (-1
           * >>u c1) == c FALSE                       else optimize x >>u c1 !=
           * c into x & (-1 << c1) != c << c1  if  c & (-1 >>u c1) == c TRUE
           * else
           */
        ...
        case iro_Shrs:
          /*
           * optimize x >>s c1 == c into x & (-1 << c1) == c << c1  if  (c >>s
           * ((BITS - 1) - c1)) \in {0,-1} FALSE                       else
           * optimize x >>s c1 != c into x & (-1 << c1) != c << c1  if  (c >>s
           * ((BITS - 1) - c1)) \in {0,-1} TRUE                        else
           */
          c1 = get_Shrs_right(left);
          if (is_Const(c1)) {
            ir_tarval *tv1 = get_Const_tarval(c1);
            ir_tarval *all_one = get_mode_all_one(mode);
            ir_tarval *cond = new_tarval_from_long(get_mode_size_bits(mode) - 1,
                                                   get_tarval_mode(tv1));
    ...
    if (is_Bad(pred)) {
      ...
      if (is_Address(target)) {
        ...
        if (entity == get_Block_entity(block)) {
           ...
           * ...) */
        bad:
          if (!bad) bad = new_r_Bad(irg, mode_X);
...
static ir_node *transform_node_Phi(ir_node *phi) {
  ...
  for (int i = 0; i < n; ++i) {
    if (!is_Bad(get_Phi_pred(phi, i))) {
      ...
      if (is_Bad(pred) || is_block_unreachable(get_nodes_block(pred))) {
        if (bad == NULL) bad = new_r_Bad(irg, mode);
        set_irn_n(phi, i, bad);
      }
    }
  }
  ...
  if (mode_is_reference(mode)) {
    ...
    for (int i = 1; i < n; ++i) {
      ...
      if (is_Confirm(pred) && get_Confirm_bound(pred) == bound &&
...
static ir_node *transform_node_shift(ir_node *n) {
  ...
  if (get_irn_op(left) != get_irn_op(n)) return n;

  ir_node *right = get_binop_right(n);
  ir_tarval *tv1 = value_of(right);
  if (!tarval_is_constant(tv1)) return n;

  ir_tarval *tv2 = value_of(get_binop_right(left));
  if (!tarval_is_constant(tv2)) return n;

  ir_mode *count_mode = get_tarval_mode(tv1);
  /* TODO: search bigger mode or something and convert... */
  if (get_tarval_mode(tv2) != count_mode) return n;
  ...
  if (!(tarval_cmp(res, mode_size) & ir_relation_less)) {
    if (is_Shrs(n)) {
      ...
      ir_node *cnst =
          new_r_Const_long(irg, smode, get_mode_size_bits(mode) - 1);
...
static ir_node *transform_node_shl_shr(ir_node *n) {
  ...
  if (tv_shl != NULL && (is_Shr(left) || is_Shrs(left))) {
    ...
    if (!is_Const(shr_right)) return n;
    ...
    if (is_Shrs(left)) {
      ...
      if (!(tarval_cmp(tv_shl, tv_shr) & ir_relation_greater_equal)) return n;
  ...
  if (relation != ir_relation_equal && !only_one_user(left)) return n;
...
static ir_node *transform_node_shift_modulo(ir_node *n,
  ...
  if (get_mode_arithmetic(mode) != irma_twos_complement) return n;
  if (!is_po2_or_zero(modulo)) return n;
  ...
  if (is_Const(right)) {
    ...
    if (!tarval_is_constant(tv_mod) || tv_mod == tv) return n;
...
static ir_node *transform_node_Shr(ir_node *n) {
  ...
  if (is_Shr(n)) n = transform_node_shift_modulo(n, new_rd_Shr);
  if (is_Shr(n)) n = transform_node_shl_shr(n);
  if (is_Shr(n)) n = transform_node_shift_bitop(n);
  if (is_Shr(n)) n = transform_node_shift_and(n, new_rd_Shr);
...
static ir_node *transform_node_Shl(ir_node *n) {
  ...
  if (is_Shl(n)) n = transform_node_shift_modulo(n, new_rd_Shl);
  if (is_Shl(n)) n = transform_node_shl_shr(n);
  if (is_Shl(n)) n = transform_node_shift_bitop(n);
  if (is_Shl(n)) n = transform_node_shift_and(n, new_rd_Shl);
...
bool may_leave_out_middle_conv(ir_mode *m0, ir_mode *m1, ir_mode *m2) {
  ...
  if (size1 < size2 && size0 >= size1) return false;
...
static ir_node *transform_node_End(ir_node *n) {
  ...
  for (int i = j = 0; i < n_keepalives; ++i) {
    ...
    /* no need to keep Bad */
    if (is_Bad(ka)) continue;
    /* do not keep unreachable code */
    ir_node *const block = get_block(ka);
    if (is_block_unreachable(block)) continue;
    in[j++] = ka;
  }
  if (j != n_keepalives) set_End_keepalives(n, j, in);
  return n;
}

int ir_is_negated_value(const ir_node *a, const ir_node *b) {
  if (is_Minus(a) && get_Minus_op(a) == b) return true;
  if (is_Minus(b) && get_Minus_op(b) == a) return true;
...
static const ir_node *skip_upconv(const ir_node *node) {
  while (is_Conv(node)) {
    ...
    ir_mode *op_mode = get_irn_mode(op);
    if (!smaller_mode(op_mode, mode)) break;
    node = op;
  }
  return node;
}
...
int ir_mux_is_abs(const ir_node *sel, const ir_node *mux_false,
  ...
  if (mode_has_signed_zero(mode)) return 0;

  /* must be <, <=, >=, > */
  ir_relation relation = get_Cmp_relation(sel);
  if ((relation & ir_relation_less_greater) == 0) return 0;
  ...
  if (!is_irn_null(cmp_right)) return 0;
...
static bool ir_is_optimizable_mux_set(const ir_node *cond, ir_relation relation,
  ...
  if (rel_eq != ir_relation_false) {
    if (rel_eq == ir_relation_less_greater) {
      ...
      if (bl != NULL && br != NULL) {
        ...
        /* Check that there is only one position where the operands may have a
         * bit set. */
        if (left_low_bit == left_high_bit && right_low_bit == right_high_bit &&
            (left_low_bit == -1 || right_low_bit == -1 ||
             left_low_bit == right_low_bit)) {
          /* (a & (1 << c)) != (b & (1 << c)) <=> (a ^ b) >> c */
          return true;
        }
      }
  ...
  ir_relation const rel_lt =
      get_complementary_relations(ir_relation_less, relation, possible);
  ir_relation const rel_gt =
      get_complementary_relations(ir_relation_greater, relation, possible);

  if (get_mode_size_bits(mode) >= get_mode_size_bits(dest_mode)) {
    /* Due to possible overflows, we can only transform compares with special
     * constants. */
    if (!mode_is_signed(mode) || !is_Const(right)) return false;
...
int ir_is_optimizable_mux(const ir_node *sel, const ir_node *mux_false,
  ...
  if (is_Cmp(sel)) {
    ...
    if (is_irn_null(f) && get_mode_arithmetic(mode) == irma_twos_complement &&
        get_mode_arithmetic(cmp_mode) == irma_twos_complement) {
      if (is_Const(t)) {
        if (is_Const_one(t)) {
          if (ir_is_optimizable_mux_set(sel, relation, mode)) return true;
        } else if (is_Const_all_one(t) && is_irn_null(cmp_r)) {
          ir_relation possible = ir_get_possible_cmp_relations(cmp_l, cmp_r);
          if (get_complementary_relations(ir_relation_less, relation,
                                          possible) != ir_relation_false) {
      ...
      if (mode_is_int(mode) && is_Const(cmp_r) &&
          (is_Const_null(cmp_r) || is_Const_one(cmp_r))) {
    ...
    if (mode_is_int(mode) && is_cmp_equality_zero(cmp_l, cmp_r, relation) &&
        is_And(cmp_l) && f == cmp_r) {
...
static ir_node *transform_Mux_set(ir_node *n, ir_relation relation) {
    ...
    if (rel_eq == ir_relation_less_greater) {
      ...
      if (bl != NULL && br != NULL) {
        ...
        if (left_low_bit == left_high_bit && right_low_bit == right_high_bit &&
          ...
          ir_node *shift_cnt =
              new_rd_Const_long(dbgi, irg, mode_Iu, shift_amount);
    ...
    if (is_irn_null(right) && is_And(left)) {
      ...
      if (is_Const(and_right)) {
        ...
        if (get_tarval_popcount(tv) == 1) {
          ...
          if (rel_eq == ir_relation_equal) a = new_rd_Not(dbgi, block, a);
  ...
  if (get_mode_size_bits(mode) >= get_mode_size_bits(dest_mode)) {
    /* Due to possible overflows, we can only transform compares with special
     * constants. */
    if (!mode_is_signed(mode) || !is_Const(right)) return n;
  ...
  if (need_not) sub = new_rd_Not(dbgi, block, sub);
...
static ir_node *transform_node_Mux(ir_node *n) {
  ...
  if (is_Cmp(sel)) {
    ...
    if (is_irn_null(f) && get_mode_arithmetic(mode) == irma_twos_complement &&
      ...
      if (is_Const(t)) {
        if (is_Const_one(t)) {
          ...
          if (n != oldn) return n;
                   ...
                   mode_is_signed(cmp_mode)) {
          ir_relation const possible =
              ir_get_possible_cmp_relations(cmp_l, cmp_r);
          ir_relation const rel_lt =
              get_complementary_relations(ir_relation_less, relation, possible);
          if (rel_lt != ir_relation_false) {
            ir_node *block = get_nodes_block(n);
            ...
            if (cmp_mode != mode) shrs = new_rd_Conv(dbgi, block, shrs, mode);
      ...
      if (mode_is_int(mode) && is_Const(cmp_r) &&
        ...
        if (bl && tarval_is_one(bl->z)) {
          ir_relation possible = ir_get_possible_cmp_relations(cmp_l, cmp_r);
          ir_relation rel_eq = get_complementary_relations(ir_relation_equal,
                                                           relation, possible);
          ir_node *block = get_nodes_block(n);
          dbg_info *dbgi = get_irn_dbg_info(n);
          assert(rel_eq != ir_relation_false);

          if (mode != cmp_mode) cmp_l = new_rd_Conv(dbgi, block, cmp_l, mode);

          if (rel_eq == ir_relation_equal ? is_Const_one(cmp_r)
                                          : is_Const_null(cmp_r)) {
            /* Mux((a & 1) != 0, 0, b) => -a & b */
    ...
    ir_relation const rel_eq =
        get_complementary_relations(ir_relation_equal, relation, possible);
    if (rel_eq != ir_relation_false && is_irn_null(cmp_r) && is_And(cmp_l) &&
        f == cmp_r) {
  ...
  if (!irg_is_constrained(irg, IR_GRAPH_CONSTRAINT_MODEB_LOWERED)) {
    if (is_Mux(t)) {
      ...
      if (t == t1) {
        /* Mux(cond0, x, Mux(cond1, x, y)) -> typical if (cond0 || cond1) x else
         * y */
...
static bool sim_store_bitfield(unsigned char *buf, ir_mode *mode, long offset,
                               unsigned bitfield_offset, unsigned bitfield_size,
                               const ir_type *type,
                               const ir_initializer_t *initializer) {
  if (get_initializer_kind(initializer) == IR_INITIALIZER_NULL) return true;
  if (get_initializer_kind(initializer) != IR_INITIALIZER_TARVAL) return false;
  ir_tarval *tv = get_initializer_tarval_value(initializer);
  if (tv == tarval_unknown) return false;
  ...
  mask =
      tarval_shr_unsigned(mask, get_mode_size_bits(type_mode) - bitfield_size);
  ...
  for (unsigned b = (unsigned)MAX(0, offset); b < initializer_size; ++b) {
    if (b > (unsigned)offset + mode_size) continue;
    unsigned idx = ir_target_big_endian() ? (initializer_size - 1 - b) : b;
...
static bool sim_store(unsigned char *buf, ir_mode *mode, long offset,
                      ...
                      const ir_initializer_t *initializer) {
  unsigned mode_size = get_mode_size_bytes(mode);
  if (offset + (int)mode_size <= 0) return true;
  unsigned initializer_size = get_type_size(type);
  assert(initializer_size > 0);
  if (offset >= (long)initializer_size) return true;
  ...
  switch (get_initializer_kind(initializer)) {
    ...
    case IR_INITIALIZER_COMPOUND:
      if (is_Array_type(type)) {
        ...
        size_t last_idx =
            (size_t)((unsigned)(offset + get_mode_size_bytes(mode) - 1) /
                     el_size);
        size_t n_entries = get_initializer_compound_n_entries(initializer);
        if (last_idx >= n_entries) last_idx = n_entries - 1;
        for (size_t i = first_idx; i <= last_idx; ++i) {
          const ir_initializer_t *sub_initializer =
              get_initializer_compound_value(initializer, i);
          long new_offset = offset - (i * el_size);
          bool res = sim_store(buf, mode, new_offset, el_type, sub_initializer);
          if (!res) return false;
        }
      } else {
        ...
        for (size_t i = 0, n_members = get_compound_n_members(type);
          ...
          if (member_offs >= offset + (long)get_mode_size_bytes(mode) ||
            ...
            continue;
          if (i > get_initializer_compound_n_entries(initializer)) continue;
          const ir_initializer_t *sub_initializer =
              get_initializer_compound_value(initializer, i);
          ...
          bool res;
          if (bitfield_size > 0) {
            unsigned bitfield_offset = get_entity_bitfield_offset(member);
            res =
                sim_store_bitfield(buf, mode, new_offset, bitfield_offset,
                                   bitfield_size, member_type, sub_initializer);
          } else {
            res =
                sim_store(buf, mode, new_offset, member_type, sub_initializer);
          }
          if (!res) return false;
        }
      }
...
static ir_node *sim_store_load(const ir_type *type,
  ...
  if (!sim_store(storage, mode, offset, type, initializer)) return NULL;
  if (ir_target_big_endian()) reverse_bytes(storage, storage_size);
  ir_tarval *tv = new_tarval_from_bytes(storage, mode);
  if (tv == tarval_unknown) return NULL;
  return new_r_Const(irg, tv);
}
...
static ir_node *extract_from_initializer(const ir_type *type,
  ...
  if ((unsigned long)offset >= size) return NULL;
  ...
  switch (get_initializer_kind(initializer)) {
    ...
    handle_tv:;
      ...
      if (tv_mode != get_type_mode(type)) return NULL;
      /* the easy case */
      if (offset == 0 &&
    ...
    case IR_INITIALIZER_CONST: {
      ...
      if (offset == 0 &&
        ...
        if (val_mode != mode) res = new_rd_Conv(dbgi, start_block, res, mode);
        return res;
      }
    ...
    case IR_INITIALIZER_COMPOUND: {
      if (is_Array_type(type)) {
        ...
        return extract_from_initializer(el_type, sub_initializer, new_offset,
                                        mode, dbgi, irg);
      } else {
        assert(is_compound_type(type));
        /* in case of unions there is no order in the compound offsets,
        ...
        for (size_t i = get_compound_n_members(type); i-- > 0;) {
          ...
          if (member_offs >= offset + (long)get_mode_size_bytes(mode) ||
              offset >= member_offs + (long)get_type_size(member_type))
            continue;
          if (get_entity_bitfield_size(member) > 0) return NULL;
          if (i > get_initializer_compound_n_entries(initializer)) continue;
          const ir_initializer_t *sub_initializer =
              get_initializer_compound_value(initializer, i);
          if (get_initializer_kind(sub_initializer) == IR_INITIALIZER_NULL)
            continue;
          long new_offset = offset - member_offs;
...
ir_node *predict_load(ir_node *ptr, ir_mode *mode) {
  ...
  if (is_Add(ptr)) {
    ...
    if (!is_Const(right)) return NULL;
    ir_tarval *tv = get_Const_tarval(right);
    if (!tarval_is_long(tv)) return NULL;
  ...
  if (is_Address(ptr)) {
    ...
    if (get_type_state(type) != layout_fixed) return NULL;
    const ir_initializer_t *initializer = get_entity_initializer(entity);
    if (initializer == NULL) initializer = get_initializer_null();
    dbg_info *dbgi = get_irn_dbg_info(ptr);
    ir_graph *irg = get_irn_irg(ptr);
    /* attempt 1 */
    ir_node *val =
        extract_from_initializer(type, initializer, offset, mode, dbgi, irg);
...
static ir_node *transform_node_Load(ir_node *n) {
  ...
  if (get_Load_volatility(n) == volatility_is_volatile) return n;
  ...
  if (val != NULL) return create_load_replacement_tuple(n, mem, val);
  ...
  if (is_Load(mem_pred)) {
    ...
    if (get_Load_ptr(pred_load) != ptr) return n;
    ir_mode *const pred_mode = get_Load_mode(pred_load);
    if (get_mode_size_bits(pred_mode) != get_mode_size_bits(mode)) return n;
    ...
    if (get_Store_ptr(pred_store) != ptr) return n;
    ...
    if (value_mode != mode) {
       ...
       * than the load.*/
      if (get_mode_size_bits(value_mode) != get_mode_size_bits(mode)) return n;
      ir_node *const block = get_nodes_block(n);
...
static ir_node *transform_node_Store(ir_node *n) {
  ...
  if (get_Store_volatility(n) == volatility_is_volatile) return n;
...
static bool always_optimize(unsigned const iro) {
  return iro == iro_Block || iro == iro_Id || iro == iro_Phi || iro == iro_Proj;
}
...
restart:;
  ...
  if (get_opt_constant_folding() || always_optimize(iro)) {
    n = equivalent_node(n);
    if (n != old_n) goto restart;
  }

  /* Some more constant expression evaluation. */
  if (get_opt_algebraic_simplification() || (iro == iro_Cond) ||
...
static int identities_cmp(const void *elt, const void *key) {
  ...
  if ((get_irn_op(a) != get_irn_op(b)) || (get_irn_mode(a) != get_irn_mode(b)))
    return 1;

  /* compare if a's in and b's in are of equal length */
  int irn_arity_a = get_irn_arity(a);
  if (irn_arity_a != get_irn_arity(b)) return 1;

  /* blocks are never the same */
  if (is_Block(a)) return 1;

  if (get_irn_pinned(a)) {
    /* for pinned nodes, the block inputs must be equal */
    if (get_nodes_block(a) != get_nodes_block(b)) return 1;
  } else {
    ...
    if (!get_opt_global_cse()) {
       ...
       * However doing GCSE optimistically produces a lot of partially dead code
       * which appears to be worse in practice than the missed opportunities. So
       * we use a very conservative variant here and only CSE if one value
       * dominates the other or one value postdominates the common dominator. */
...
unsigned ir_node_hash(const ir_node *node) { return node->op->ops.hash(node); }
...
void del_identities(ir_graph *irg) {
  if (irg->value_table != NULL) del_pset(irg->value_table);
}
...
void visit_all_identities(ir_graph *irg, irg_walk_func visit, void *env) {
  foreach_pset(irg->value_table, ir_node, node) { visit(node, env); }
}
...
ir_node *optimize_node(ir_node *n) {
   ...
   */
  if (get_opt_cse()) n = identify_cons(n);
...
ir_node *optimize_in_place(ir_node *n) {
  ...
  if (get_opt_global_cse()) set_irg_pinned(irg, op_pin_state_floats);
