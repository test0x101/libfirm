...
struct block_t {
  ...
  list_head
      nodes; /**< Wait-queue of nodes that must be checked for congruence. */
...
struct node_t {
  ...
  ir_node
      *node; /**< Pointer to the associated IR-node or NULL for block inputs. */
...
static unsigned opcode_hash(const opcode_key_t *entry) {
  /* assume long >= int */
  return (unsigned)(PTR_TO_INT(entry->mode) * 9 + entry->code +
                    entry->u.proj * 3 + hash_ptr(entry->u.addr) + entry->arity);
}
...
static int cmp_opcode(const void *elt, const void *key, size_t size) {
  ...
  return o1->code != o2->code || o1->mode != o2->mode ||
         o1->arity != o2->arity || o1->u.proj != o2->u.proj ||
         o1->u.addr != o2->u.addr;
}
 ...
 * @param env        the environment
 */
static block_t *create_block(ir_node *block, int meet_input,
                             partition_t *partition, environment_t *env) {
 ...
 * @param env    the environment
 */
static void add_pair(block_t *block, ir_node *irn, int idx,
                     environment_t *env) {
...
static opcode_key_t *opcode(const node_t *node, environment_t *env) {
  ...
  entry = set_insert(opcode_key_t, env->opcode2id_map, &key, sizeof(key),
                     opcode_hash(&key));
  return entry;
}
...
static int is_input_node(ir_node *pred, ir_node *irn, int index) {
  ...
  if (index != 1) return 1;
  if (!is_Address(pred)) return 1;
  if (!is_Call(irn)) return 1;
  return 0;
}
...
static void propagate_blocks(partition_t *part, environment_t *env) {
  ...
  /* Let map be an empty mapping from the range of Opcodes to (local) list of
   * blocks. */
  listmap_init(&map);
      ...
      foreach_irn_in_r(irn, i, pred) {
        ...
        if (block != bl->block) {
          ...
          if (is_input_node(pred, irn, i)) {
            ...
            p_node->is_input = 1;
            if (!is_Phi(irn)) add_pair(bl, irn, i, env);
          } else if (is_Phi(pred)) {
            /* update the Phi list */
            add_phi(bl, pred, env);
          }
...
static void propagate(environment_t *env) {
  list_for_each_entry_safe(partition_t, part, next, &env->partitions,
                           part_list) {
    if (part->n_blocks < 2) {
...
static void *live_throughs(const block_t *bl, const ir_node *phi) {
  ...
  if (get_nodes_block(input) == bl->block) return NULL;
  return input;
}
 ...
 * @param env   the environment
 */
static void propagate_blocks_live_troughs(partition_t *part,
                                          environment_t *env) {
  ...
  for (phi = get_Block_phis(meet_block); phi != NULL; phi = get_Phi_next(phi)) {
    ...
    /* Let map be an empty mapping from the range of live-troughs to (local)
     * list of blocks. */
    listmap_init(&map);
    list_for_each_entry_safe(block_t, bl, next, &part->blocks, block_list) {
      opcode_key_t *id;
...
static void apply(ir_graph *irg, partition_t *part) {
  ...
  for (repr_phi = repr->phis; repr_phi != NULL; repr_phi = repr_phi->next) {
    ...
    for (i = 0; i < n; ++i) repr_phi->ins[i] = get_Phi_pred(repr_phi->phi, i);
  }

  /* ... and all newly created Phis */
  for (repr_pair = repr->input_pairs; repr_pair != NULL;
       repr_pair = repr_pair->next) {
    ir_node *input = get_irn_n(repr_pair->irn, repr_pair->index);

    repr_pair->ins = NEW_ARR_F(ir_node *, n);
    for (i = 0; i < n; ++i) repr_pair->ins[i] = input;
  }
  ...
  list_for_each_entry(block_t, bl, &part->blocks, block_list) {
    ...
    for (i = get_End_n_keepalives(end) - 1; i >= 0; --i) {
      ...
      if (is_Block(ka)) {
        if (ka == block) remove_End_keepalive(end, ka);
      } else {
        if (get_nodes_block(ka) == block) remove_End_keepalive(end, ka);
      }
    }
    ...
    for (repr_phi = repr->phis, phi = bl->phis; repr_phi != NULL;
    ...
    for (repr_pair = repr->input_pairs, pair = bl->input_pairs;
         repr_pair != NULL; repr_pair = repr_pair->next, pair = pair->next) {
      ir_node *input = get_irn_n(pair->irn, pair->index);

      for (i = 0; i < n; ++i) ARR_APP1(ir_node *, repr_pair->ins, input);
    }
  }
  ...
  for (repr_pair = repr->input_pairs; repr_pair != NULL;
    ...
    if (is_Phi(phi)) add_Block_phi(block, phi);
  }
  ...
  for (i = j = 0; i < n; ++i) {
    ...
    list_for_each_entry(block_t, bl, &part->blocks, block_list) {
      if (bl->cf_root->node == pred) goto continue_outer;
    }
    ins[j] = pred;

    for (k = 0, p = get_Block_phis(meet_block); p != NULL;
         p = get_Phi_next(p), ++k) {
 ...
 * @param env      the environment
 */
static void partition_for_block(ir_node *block, pred_t preds[], int n_preds,
                                environment_t *env) {
...
static void check_for_cf_meet(ir_node *block, void *ctx) {
  ...
  for (i = n - 1; i >= 0; --i) {
    ...
    if (!is_Jmp(pred) && !is_Raise(pred) && !is_Bad(pred)) continue;
  ...
  if (k > 1) partition_for_block(block, preds, k, env);
}
...
static int cmp_nodes(const void *a, const void *b) {
  ...
  if (mode_a != mode_b) return mode_a < mode_b ? -1 : +1;
...
void shape_blocks(ir_graph *irg) {
  ...
  while (!list_empty(&env.partitions)) propagate(&env);
