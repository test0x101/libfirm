...
static bool can_inline(ir_node *call, ir_graph *called_graph) {
  ...
  if (props & mtp_property_noinline) return false;
  ...
  if (is_method_variadic(called_type)) return false;
  ...
  for (size_t i = 0; i < n_params; ++i) {
    ...
    if (param_tp != arg_tp) {
      ...
      if (pmode == NULL || amode == NULL) return false;
      if (get_mode_size_bits(pmode) != get_mode_size_bits(amode)) return false;
  ...
  for (size_t i = 0; i < n_res; ++i) {
    ...
    if (decl_res_tp != used_res_tp) {
      ...
      if (decl_mode == NULL || used_mode == NULL) return false;
...
static void copy_parameter_entities(ir_node *call, ir_graph *called_graph) {
  ...
  for (size_t i = 0, n_entities = get_compound_n_members(called_frame);
    ...
    if (!is_parameter_entity(old_entity)) continue;
    ...
    if (is_aggregate_type(old_type)) {
      ...
      ir_node *store =
          new_rd_Store(dbgi, block, call_mem, sel, param, old_type, cons_none);
...
static bool inline_method(ir_node *const call, ir_graph *called_graph) {
  ...
  */
  ...
  for (int i = 0; i < irn_arity; i++) {
    ...
    if (!is_Bad(ka)) add_End_keepalive(get_irg_end(irg), ka);
  }
  ...
  ir_node *call_mem = n_mem_phi > 0
                          ? new_r_Phi(post_bl, n_mem_phi, cf_pred, mode_M)
  ...
  if (n_res > 0) {
    for (int j = 0; j < n_res; j++) {
      ...
      ir_mode *res_mode = is_aggregate ? mode_P : get_type_mode(res_type);
      int n_ret = 0;
      for (int i = 0; i < arity; i++) {
        ir_node *ret = get_Block_cfgpred(end_bl, i);
        if (is_Return(ret)) {
      ...
      ir_node *const phi = n_ret == 0 ? new_r_Bad(irg, res_mode)
                           : n_ret == 1
                               ? cf_pred[0]
      ...
      if (is_aggregate) {
        ...
        bool is_volatile =
            is_partly_volatile(phi) || is_partly_volatile(member);
        call_mem = new_r_CopyB(post_bl, call_mem, member, phi, res_type,
                               is_volatile ? cons_volatile : cons_none);
        res_pred[j] = member;
      } else {
        res_pred[j] = phi;
  ...
  if (ir_throws_exception(call)) {
    ...
    foreach_irn_in(main_end_bl, i, pred) end_preds[i] = pred;
...
typedef struct {
  ...
  unsigned *local_weights; /**< Once allocated, the beneficial weight for
                              transmitting local addresses. */
  unsigned n_nodes; /**< Number of nodes in graph except Id, Tuple, Proj, Start,
                       End. */
  unsigned
      n_blocks; /**< Number of Blocks in graph without Start and End block. */
  ...
  unsigned got_inline : 1; /**< Set, if at least one call inside this graph was
                              inlined. */
...
static void collect_calls2(ir_node *node, void *ctx) {
  ...
  if (callee != NULL) {
    ...
    if (callee == current_ir_graph) x->recursive = 1;
  ...
  foreach_irn_out_r(arg, i, succ) {
    switch (get_irn_opcode(succ)) {
      ...
      case iro_Store:
        ...
        break;
      case iro_Sel:
        if (!is_Const(get_Sel_index(succ))) return 0;
        /* FALLTHROUGH */
      case iro_Member: {
...
static unsigned get_method_local_adress_weight(ir_graph *callee, size_t pos) {
  ...
  if (env->local_weights == NULL) analyze_irg_local_weights(env, callee);
...
static int calc_inline_benefice(call_entry *entry, ir_graph *callee) {
  ...
  for (size_t i = 0; i < n_params; ++i) {
    ...
    if (is_Const(param)) {
      ...
      all_const = false;
      if (is_Address(param) || is_Align(param) || is_Offset(param) ||
          is_Size(param))
        weight += get_method_param_weight(ent, i);
  ...
  if (callee_env->n_blocks == 1) weight = weight * 3 / 2;

  /* and one for small non-recursive functions: we want them to be inlined in
   * mostly every case */
  if (callee_env->n_nodes < 30 && !callee_env->recursive) weight += 2000;

  /* and finally for leafs: they do not increase the register pressure
     because of callee safe registers */
  if (callee_env->n_call_nodes == 0) weight += 400;
...
static void maybe_push_call(pqueue_t *pqueue, call_entry *call,
  ...
  mtp_additional_properties caller_props =
      get_entity_additional_properties(caller_ent);
  ir_graph *callee = call->callee;
  ir_entity *callee_ent = get_irg_entity(callee);
  mtp_additional_properties callee_props =
      get_entity_additional_properties(callee_ent);
  ...
  if (!(callee_props & mtp_property_always_inline) &&
    ...
    DB((dbg, LEVEL_2,
        "Do not inline %+F into %+F to prevent endless inlining\n", call->call,
        caller));
    return;
  }
 ...
 */
static void inline_into(ir_graph *irg, unsigned maxsize, int inline_threshold,
                        pmap *copied_graphs) {
  inline_irg_env *env = (inline_irg_env *)get_irg_link(irg);
  if (env->n_call_nodes == 0) return;
  ...
  while (!pqueue_empty(pqueue)) {
    ...
    if (calleee != NULL) {
       ...
       */
      if (!curr_call->all_const) benefice -= 2000;
      if (benefice < inline_threshold) continue;
  ...
  for (size_t i = 0; i < n_irgs; ++i) {
    ...
    if (env->got_inline || (env->n_callers_orig != env->n_callers)) {
      DB((dbg, LEVEL_1,
          ...
          env->n_nodes_orig, env->n_nodes, env->n_call_nodes_orig,
          env->n_call_nodes, env->n_callers_orig, env->n_callers,
...
void firm_init_inline(void) { FIRM_DBG_REGISTER(dbg, "firm.opt.inline"); }
