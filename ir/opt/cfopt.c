...
static unsigned optimize_pointless_forks(ir_node *block, unsigned n_cfgpreds,
  ...
  for (unsigned i = 0; i < n_cfgpreds; ++i) {
    ...
    if (new_cfgpreds != NULL && new_cfgpreds[i] == NULL) continue;
    if (!is_Proj(pred0)) continue;
    ir_node *cfop = get_Proj_pred(pred0);
    if (!is_Cond(cfop) && !is_Switch(cfop)) continue;

    for (unsigned j = i + 1; j < n_cfgpreds; ++j) {
      ir_node *pred1 = cfgpreds[j];
      if (new_cfgpreds != NULL && new_cfgpreds[j] == NULL) continue;
      if (!is_Proj(pred1)) continue;
      if (get_Proj_pred(pred1) != cfop) continue;
  ...
  if (new_n_cfgpreds != n_cfgpreds) {
    ...
    for (ir_node *phi = get_Block_phis(block); phi != NULL;
      ...
      for (unsigned i = 0; i < n_cfgpreds; ++i) {
        if (new_cfgpreds[i] == NULL) continue;
...
static bool is_default_switch(const ir_node *node) {
  ...
  for (size_t i = 0, n_entries = ir_switch_table_get_n_entries(table);
       i < n_entries; ++i) {
    if (ir_switch_table_get_pn(table, i) != pn_Switch_default) return false;
  }
  return true;
}
#endif
...
static bool try_merge_blocks(ir_node *block, unsigned pred_pos) {
  ...
  if (!is_Block_removable(block)) set_Block_removable(pred_block, false);
...
static void merge_empty_predecessors(ir_node *block, unsigned new_n_cfgpreds) {
  ...
  if (new_n_cfgpreds == 1) set_Block_phis(block, NULL);
  ...
  for (unsigned i = 0; i < n_cfgpreds; ++i) {
    ...
    if (is_Bad(pred)) continue;
    ir_node *predb = get_nodes_block(pred);
    if (predb == block || !is_Block_removable(predb)) continue;
    ...
    for (ir_node *phi = get_Block_phis(predb), *next_phi; phi != NULL;
      ...
      for (unsigned j = 0; j < n_cfgpreds; ++j) {
        ...
        for (unsigned k = 0, n_predb2_cfgpreds = get_Block_n_cfgpreds(predb2);
             k < n_predb2_cfgpreds; ++k) {
          ir_node *predpred = get_Block_cfgpred(predb2, k);
          if (is_Bad(predpred)) continue;
...
static bool optimize_block(ir_node *block, bool *changed) {
  if (irn_visited_else_mark(block)) return false;
...
again:;
  ...
  for (unsigned i = 0; i < n_cfgpreds; ++i) {
    ...
    if (bail_out || is_Id(block) || is_Id(predb)) return false;
  ...
  if (!can_opt) {
    ...
    unsigned new_n_cfgpreds = optimize_pointless_forks(block, n_cfgpreds, NULL);
    if (new_n_cfgpreds != n_cfgpreds) {
      *changed = true;
      goto again;
    }
...
void optimize_cf(ir_graph *irg) {
  ...
  do {
    ...
    foreach_irn_in(end, i, kept) {
      if (is_Block(kept)) optimize_block(kept, &changed);
    }
