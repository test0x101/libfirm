...
#define MAX_PROJ \
  MAX(MAX((unsigned)pn_Load_max, (unsigned)pn_Store_max), (unsigned)pn_Call_max)
...
static void collect_nodes(ir_node *node, void *env) {
  ...
  if (opcode == iro_Proj) {
    ...
    for (int i = get_Block_n_cfgpreds(node); i-- > 0;) {
      ...
      opcode = get_irn_opcode(pred);
      if (is_exc &&
          (opcode == iro_Load || opcode == iro_Store || opcode == iro_Call)) {
        ldst_info_t *ldst_info = get_ldst_info(pred, &wenv->obst);
...
static void handle_load_update(ir_node *load) {
  ...
  if (get_Load_volatility(load) == volatility_is_volatile) return;
 ...
 */
static void reduce_node_usage(ir_node *ptr) {
  if (!is_Proj(ptr)) return;
  if (get_irn_n_edges(ptr) > 0) return;
...
static void get_base_and_offset(ir_node *ptr, base_offset_t *base_offset) {
  ...
  for (;;) {
    if (is_Add(ptr)) {
      ...
      if (get_irn_mode(l) != mode || !is_Const(r)) break;
      ...
      if (!is_Const(index)) break;

      ir_type *type = get_Sel_type(ptr);
      ir_type *element_type = get_array_element_type(type);
      if (get_type_state(element_type) != layout_fixed) break;
      ...
      if (get_type_state(owner) != layout_fixed) break;
 ...
 */
static bool is_contained_in(ir_mode *const load_mode,
                            const base_offset_t *const load_bo,
                            ir_mode *const prev_mode,
                            const base_offset_t *const prev_bo) {
  if (load_bo->base != prev_bo->base) return false;
  ...
  if (delta + load_mode_len > prev_mode_len) return false;

  /* simple case: previous value has the same mode */
  if (load_mode == prev_mode) return true;
  ...
  return (prev_arithmetic == irma_twos_complement &&
         ...
         (prev_arithmetic != load_arithmetic && load_mode_len == prev_mode_len);
}
 ...
 * value.
 */
static ir_node *transform_previous_value(
    ir_mode *const load_mode, const long load_offset, ir_mode *const prev_mode,
    const long prev_offset, ir_node *const prev_value, ir_node *const block,
    ir_node *const load) {
  /* simple case: previous value has the same mode */
  if (load_mode == prev_mode) return prev_value;
  ...
  if (prev_arithmetic == irma_twos_complement &&
    ...
    unsigned const shift =
        ir_target_big_endian() ? prev_mode_len - load_mode_len - delta : delta;
    ...
    assert(prev_arithmetic != load_arithmetic &&
...
static changes_t try_load_after_store(track_load_env_t *env, ir_node *store) {
  ...
  if (!on_regular_path(load, store)) return NO_CHANGES;
  ...
  if (is_contained_in(load_mode, base_offset, store_mode, &prev_base_offset)) {
    ...
    ir_node *const new_value = transform_previous_value(
        load_mode, base_offset->offset, store_mode, prev_base_offset.offset,
        store_value, block, load);
...
static changes_t try_load_after_load(track_load_env_t *env,
                                     ir_node *prev_load) {
  ir_node *const load = env->load;
  if (!on_regular_path(load, prev_load)) return NO_CHANGES;
  ...
  if (prev_value == NULL) return NO_CHANGES;
  ...
  if (is_contained_in(load_mode, base_offset, prev_mode, &prev_base_offset)) {
    ir_node *const block = get_nodes_block(load);
    ir_node *const new_value = transform_previous_value(
        load_mode, base_offset->offset, prev_mode, prev_base_offset.offset,
        prev_value, block, load);
...
static bool try_update_ptr_CopyB(track_load_env_t *env, ir_node *copyb) {
  ...
  if (dst_base_offset.base != env->base_offset.base) return false;

  /* see if bytes loaded are fully contained in the CopyB */
  if (env->base_offset.offset < dst_base_offset.offset) return false;
  ...
  if (delta + load_size > n_copy) return false;
  ...
  ir_node *cnst =
      new_r_Const_long(irg, mode_offset, src_base_offset.offset + delta);
...
static changes_t follow_load_mem_chain(track_load_env_t *env, ir_node *start) {
  ...
  for (;;) {
    ...
    if (is_Store(node)) {
      ...
      if (changes != NO_CHANGES) return changes | res;
      ...
      ir_alias_relation rel = get_alias_relation(
          ptr, store_type, store_size, env->ptr, load_type, load_size);
      /* if the might be an alias, we cannot pass this Store */
      if (rel != ir_no_alias) break;
      ...
      if (updated) {
        ...
        if (new_value != NULL) return replace_load(load, new_value) | res;
      }
      ...
      ir_alias_relation rel =
          get_alias_relation(dst, type, size, env->ptr, load_type, load_size);
    ...
    /* check for cycles */
    if (NODE_VISITED(node_info)) break;
    MARK_NODE(node_info);
  }
  ...
  if (is_Sync(node)) {
    ...
    foreach_irn_in(node, i, in) {
      ...
      if ((res & ~NODES_CREATED) != NO_CHANGES) break;
    }
  }
  return res;
}
...
static ir_entity *find_entity(ir_node *ptr) {
  switch (get_irn_opcode(ptr)) {
    ...
    case iro_Add: {
      ...
      if (mode_is_reference(get_irn_mode(right))) return find_entity(right);
    } /* FALLTHROUGH */
    case iro_Sub:
...
static changes_t optimize_load(ir_node *load) {
  ...
  /* do NOT touch volatile loads for now */
  if (get_Load_volatility(load) == volatility_is_volatile) return NO_CHANGES;
...
static changes_t follow_store_mem_chain(ir_node *store, ir_node *start,
  ...
  while (node != store) {
    ...
    if (is_Store(node)) {
      ...
      ir_alias_relation rel = get_alias_relation(store_ptr, store_type,
                                                 store_size, ptr, type, size);
      /* if the might be an alias, we cannot pass this Store */
      if (rel != ir_no_alias) break;
      node = skip_Proj(get_Store_mem(node));
    } else if (is_Load(node)) {
      ...
      ir_alias_relation rel =
          get_alias_relation(load_ptr, load_type, load_size, ptr, type, size);
      if (rel != ir_no_alias) break;
      ...
      ir_alias_relation src_rel = get_alias_relation(
          copyb_src, copyb_type, copyb_size, ptr, type, size);
      if (src_rel != ir_no_alias) break;
      ir_node *copyb_dst = get_CopyB_dst(node);
      ir_alias_relation dst_rel = get_alias_relation(
          copyb_dst, copyb_type, copyb_size, ptr, type, size);
      if (dst_rel != ir_no_alias) break;
    } else {
...
static changes_t optimize_store(ir_node *store) {
  if (get_Store_volatility(store) == volatility_is_volatile) return NO_CHANGES;

  /* Check, if the address of this Store is used more than once.
   * If not, this Store cannot be removed in any case. */
  ir_node *ptr = get_Store_ptr(store);
  if (get_irn_n_edges(ptr) <= 1) return NO_CHANGES;
...
static changes_t follow_copyb_mem_chain(ir_node *copyb, ir_node *start,
  ...
  while (node != copyb) {
    ...
    if (is_Store(node) &&
      ...
      ir_alias_relation src_rel = get_alias_relation(src, type, size, store_ptr,
    ...
    if (is_Store(node)) {
      ...
      /* check if we can pass through this store */
      ir_alias_relation src_rel = get_alias_relation(
          store_ptr, store_type, store_size, src, type, size);
      if (src_rel != ir_no_alias) break;
      ir_alias_relation dst_rel = get_alias_relation(
          store_ptr, store_type, store_size, dst, type, size);
      if (dst_rel != ir_no_alias) break;
      ...
      unsigned load_size = get_mode_size_bytes(get_Load_mode(node));
      ir_alias_relation rel =
          get_alias_relation(load_ptr, load_type, load_size, dst, type, size);
      if (rel != ir_no_alias) break;
      ...
      if (src == pred_dst && size == pred_size &&
        ...
        ir_alias_relation dst_dst_rel =
            get_alias_relation(pred_dst, pred_type, pred_size, dst, type, size);
        if (dst_dst_rel != ir_no_alias) break;
        ir_alias_relation src_dst_rel =
            get_alias_relation(pred_src, pred_type, pred_size, dst, type, size);
        if (src_dst_rel != ir_no_alias) break;
        ir_alias_relation dst_src_rel =
            get_alias_relation(pred_dst, pred_type, pred_size, src, type, size);
        if (dst_src_rel != ir_no_alias) break;
      }
...
static changes_t optimize_copyb(ir_node *copyb) {
  if (get_CopyB_volatility(copyb) == volatility_is_volatile) return NO_CHANGES;
...
static ir_cons_flags get_store_cons_flags(ir_node *const store) {
  ...
  if (get_Store_volatility(store) != volatility_non_volatile)
    cons |= cons_volatile;
  if (get_Store_unaligned(store) != align_is_aligned) cons |= cons_unaligned;
  if (get_irn_pinned(store) == op_pin_state_floats) cons |= cons_floats;
  if (ir_throws_exception(store)) cons |= cons_throws_exception;
  return cons;
}
...
static changes_t optimize_phi(ir_node *phi, walk_env_t *wenv) {
  ...
  if (get_irn_mode(phi) != mode_M) return NO_CHANGES;
  ...
  if (n <= 1) return NO_CHANGES;

  /* must be only one user */
  ir_node *projM = get_Phi_pred(phi, 0);
  if (has_multiple_users(projM)) return NO_CHANGES;

  ir_node *store = skip_Proj(projM);
  if (!is_Store(store)) return NO_CHANGES;
  ...
  if (bl_info->flags & BLOCK_HAS_EXC) return NO_CHANGES;

  ir_node *phi_block = get_nodes_block(phi);
  if (!block_strictly_postdominates(phi_block, block)) return NO_CHANGES;
  ...
  for (int i = 1; i < n; ++i) {
    ...
    if (has_multiple_users(pred)) return NO_CHANGES;

    pred = skip_Proj(pred);
    if (!is_Store(pred)) return NO_CHANGES;

    if (ptr != get_Store_ptr(pred) ||
        mode != get_irn_mode(get_Store_value(pred)))
      return NO_CHANGES;
    if (cons != get_store_cons_flags(pred)) return NO_CHANGES;

    /* check, if all stores have the same exception flow */
    info = (ldst_info_t *)get_irn_link(pred);
    if (exc != info->exc_block) return NO_CHANGES;
  ...
  if (get_Phi_loop(phi)) remove_keep_alive(phi);
  exchange(phi, projM);
...
static changes_t optimize_conv_load(ir_node *conv) {
  ...
  if (!is_Proj(op)) return NO_CHANGES;
  if (has_multiple_users(op)) return NO_CHANGES;
  /* shrink mode of load if possible. */
  ir_node *load = get_Proj_pred(op);
  if (!is_Load(load)) return NO_CHANGES;
  ...
  ir_mode *load_mode = get_Load_mode(load);
  int bits_diff = get_mode_size_bits(load_mode) - get_mode_size_bits(mode);
  if (mode_is_float(load_mode) || mode_is_float(mode) || bits_diff < 0)
    return NO_CHANGES;

  if (ir_target_big_endian()) {
    if (bits_diff % 8 != 0) return NO_CHANGES;
    ...
    if (is_Load(load)) {
      ...
      for (other = pscc->head; other != NULL; other = next_other) {
        ...
        if (is_Store(other)) {
          ...
          ir_alias_relation rel = get_alias_relation(
              store_ptr, store_type, store_size, ptr, load_type, load_size);
          /* if the might be an alias, we cannot pass this Store */
          if (rel != ir_no_alias) break;
        }
      ...
      if (other == NULL) {
        ...
        for (phi_entry *pe = phi_list; pe != NULL; pe = pe->next) {
          ...
          avail_entry_t *res =
              set_find(avail_entry_t, avail, &entry, sizeof(entry),
                       hash_cache_entry(&entry));
          ir_node *irn;
          if (res != NULL) {
            irn = res->load;
          } else {
            irn = new_rd_Load(db, pred, get_Phi_pred(phi, pos), ptr, load_mode,
                              load_type, cons_none);
            entry.load = irn;
            (void)set_insert(avail_entry_t, avail, &entry, sizeof(entry),
                             hash_cache_entry(&entry));
...
static void do_dfs(ir_graph *irg, loop_env *env) {
  ...
  for (int i = get_End_n_keepalives(end); i-- > 0;) {
    ...
    if (is_Phi(ka) && !irn_visited(ka)) dfs(ka, env);
  }
}
...
static ir_node *get_ptr(const ir_node *node) {
  if (is_Store(node)) return get_Store_ptr(node);
  if (is_Load(node)) return get_Load_ptr(node);
  return NULL;
}
...
static void combine_memop(ir_node *sync, void *data) {
  ...
  again:;
    ...
    if (is_Store(store0)) {
      ...
      if (get_mode_arithmetic(store_mode) != irma_twos_complement) continue;
      unsigned store_size = get_mode_size_bytes(store_mode);
      if (store_size >= pointer_size || !is_po2_or_zero(store_size)) continue;
      ir_mode *mode_unsigned = find_unsigned_mode(store_mode);
      if (mode_unsigned == NULL) continue;
      ir_mode *double_mode = find_double_bits_int_mode(mode_unsigned);
      if (double_mode == NULL) continue;
      ...
      if (!is_Const(store_val1)) {
        if (!is_Shr(store_val1)) continue;
        ir_node *shiftval = get_Shr_right(store_val1);
        if (!is_Const(shiftval)) continue;
        ir_tarval *tv = get_Const_tarval(shiftval);
        if (!tarval_is_long(tv) || get_tarval_long(tv) != (long)store_size * 8)
          continue;
      }
  ...
  if (changed) {
    ...
    for (int i = 0; i < n_preds; ++i) {
      ir_node *in = new_in[i];
      if (in != NULL) new_in[new_arity++] = in;
    }
...
void combine_memops(ir_graph *irg) {
   ...
   * stores. */
  if (!ir_target.fast_unaligned_memaccess) return;
...
void optimize_load_store(ir_graph *irg) {
  ...
  confirm_irg_properties(
      irg, env.changes == NO_CHANGES         ? IR_GRAPH_PROPERTIES_ALL
           : (env.changes & CF_CHANGED) != 0 ? IR_GRAPH_PROPERTIES_NONE
           : (env.changes & DF_CHANGED) != 0
