...
static interval_t *get_interval(interval_t *iv, ir_node *bound,
  ...
  if (iv->min != tarval_unknown && iv->max != tarval_unknown) return iv;
  return NULL;
}
...
static ir_tarval *(compare_iv)(const interval_t *l_iv, const interval_t *r_iv,
  ...
  if (l_iv == NULL || r_iv == NULL) return tarval_unknown;
  ...
  switch (relation) {
    case ir_relation_equal: {
      ...
      /* b < c ==> [a,b] != [c,d] */
      if (res == ir_relation_less) return tv_false;
    ...
    case ir_relation_less_greater:
      /* two intervals can be compared for not equality only if they are a
       * single value */
      if (l_iv->min == l_iv->max && r_iv->min == r_iv->max)
        return l_iv->min != r_iv->min ? tv_true : tv_false;
      break;
    ...
    case ir_relation_less: {
      ...
      /* [a, b] < [c, d]  <==> b < c */
      if (res == ir_relation_less) return tv_true;
      ...
      if (res == ir_relation_greater) return tv_false;
    ...
    case ir_relation_less_equal: {
      ...
      if (flags != 0) {
        ir_relation res = tarval_cmp(l_iv->max, r_iv->min);
        if (res == ir_relation_less || res == ir_relation_equal) return tv_true;
      }
...
ir_tarval *computed_value_Cmp_Confirm(ir_node *left, ir_node *right,
  ...
  if (is_Confirm(right)) {
    ...
    if (r_bound == l_bound) {
      if (is_transitive(l_relation)) {
         ...
         */
        if ((l_relation & ~ir_relation_equal) ==
            (r_inc_relation & ~ir_relation_equal)) {
          ir_relation res_relation =
              (l_relation & ~ir_relation_equal) |
              (l_relation & r_inc_relation & ir_relation_equal);

          if ((relation == res_relation) ||
              ((relation & ~ir_relation_equal) == res_relation)) {
            return tarval_b_true;
          } else {
            ir_relation neg_relation = get_negated_relation(relation);

            if ((neg_relation == res_relation) ||
                ((neg_relation & ~ir_relation_equal) == res_relation)) {
    ...
    if (left == r_bound) {
       ...
       */
      if ((r_relation == relation) ||
          (r_relation == (relation & ~ir_relation_equal))) {
        return tarval_b_true;
      } else {
        /*
         * l == bound(r) AND relation(r) != relation:
        ...
        if ((r_relation == neg_relation) ||
            (r_relation == (neg_relation & ~ir_relation_equal))) {
          return tarval_b_false;
        }
      }
    }
    ...
    ir_tarval *tv =
        compare_iv(get_interval(&l_iv, l_bound, l_relation),
                   get_interval(&r_iv, r_bound, r_relation), relation);
  ...
  if (right == l_bound) {
     ...
     * We know that a CMP b and check for that
     */
    if ((l_relation == relation) ||
        (l_relation == (relation & ~ir_relation_equal))) {
      return tarval_b_true;
    } else {
      /*
      ...
      if ((l_relation == neg_relation) ||
          (l_relation == (neg_relation & ~ir_relation_equal))) {
        return tarval_b_false;
      }
    }
  }
  ...
  if (tv != tarval_unknown) {
    tv = compare_iv(get_interval(&l_iv, l_bound, l_relation),
                    get_interval_from_tv(&r_iv, tv), relation);
  }
