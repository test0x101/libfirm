...
static void walk_critical_cf_edges(ir_node *block, void *env) {
  ...
  foreach_irn_in(block, i, pre) {
    ...
    if (is_unknown_jump(pre)) {
       ...
       * this... */
      fprintf(stderr,
              "libfirm warning: Couldn't split all critical edges (compiler "
              "will probably fail now)\n");
      continue;
    }
...
void remove_critical_cf_edges_ex(ir_graph *irg, int ignore_exception_edges) {
  ...
  if (env.changed) {
    ...
    clear_irg_properties(
        irg, IR_GRAPH_PROPERTIES_ALL & ~(IR_GRAPH_PROPERTY_ONE_RETURN |
