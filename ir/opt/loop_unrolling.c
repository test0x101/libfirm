...
static bool block_is_inside_loop(ir_node *const block, ir_loop *const loop) {
  ...
  if (block_loop == NULL) return false;
...
static bool block_dominates_loop(ir_node *const block, ir_loop *const loop) {
  ...
  for (size_t i = 0; i < n_elements; ++i) {
    ...
    if (*element.kind == k_ir_node) {
      ...
      if (!block_dominates(block, element.node)) return false;
    } else if (*element.kind == k_ir_loop) {
      if (!block_dominates_loop(block, element.son)) return false;
    }
  }
  return true;
}
...
static void duplicate_block(ir_node *const block) {
  ...
  for (unsigned i = 0; i < n_outs; ++i) {
    ...
    assert(!is_Block(node));
    if (get_nodes_block(node) != block) continue;
...
/**
 * Analyzes loop and decides whether it should be unrolled or not and chooses a
 * suitable unroll factor.
 *
 * Currently only loops featuring a counter variable with constant start, step
 * and limit known at compile time are considered for unrolling. Tries to find a
 * divisor of the number of loop iterations which is smaller than the maximum
 * unroll factor and is a power of two. In this case, additional optimizations
 * are possible.
 *
...
static unsigned find_suitable_factor(ir_node *const header, unsigned max,
  ...
  for (unsigned i = 0; i < n_outs; ++i) {
    ...
    assert(!is_Block(node));
    if (get_nodes_block(node) != header) continue;

    if (is_Cmp(node)) {
      ir_relation cmp_rel = get_Cmp_relation(node);
      if (cmp_rel == ir_relation_less_greater || cmp_rel == ir_relation_equal ||
      ...
      for (int j = 0; j < phi_preds; j++) {
        ...
        if (is_Const(phi_pred) && mode_is_int(get_irn_mode(cmp_right))) {
          ...
          if (tv_init == NULL ||
              tarval_cmp(tv_init, const_tv) == ir_relation_equal) {
            tv_init = const_tv;
            continue;
          }
        }
        phi_pred = skip_trivial_phis(phi_pred);
        // is_binop() would find more cases, but we currently can only optimize
        // further if we have an Add here
          ...
          if (is_Const(right) && is_Phi(left)) {
            ...
            do {
              ...
              left = get_Phi_pred(left, 0);
                     ...
                     (get_Phi_n_preds(left) == 1 || left == header_phi));
      ...
      // calculate loop iterations; add one iteration to count the first
      // iteration
      ir_tarval *tv_loop_count =
          (tarval_add(tarval_div(tv_interval, tv_step), tv_one));
  ...
  if (n == 1) {
    ...
    for (k = 0; k < get_irn_n_outs(block); k++) {
      ...
      if (is_Phi(phi)) {
        ...
        // idx is either 0 or 1; exchange phi with phi input with index we want
        // to keep
...
static void rewire_fully_unrolled(ir_loop *const loop, ir_node *header) {
  ...
  for (unsigned i = 0; i < header_n_outs; ++i) {
    ...
    if (is_Proj(succ) && get_irn_mode(succ) == mode_X) {
      ...
      for (unsigned j = 0; j < proj_outs; j++) {
        ...
        if (get_irn_link(cf_succ) == NULL && is_Block(cf_succ) &&
            !block_is_inside_loop(cf_succ, loop)) {
  ...
  for (int i = 0; i < n_header_preds; i++) {
    ...
    for (unsigned j = 0; j < n_outs; ++j) {
      ...
      if (is_Phi(phi)) {
        ...
        if (is_Phi(pred)) {
          ...
          // note: if there are no nodes except the phi on the memory path
          // within the loop header, the case above already handled the memory
          // phi correctly.
          new_pred = pred;
          // walk memory path until phi is found
          while (!is_Phi(new_pred)) {
            new_pred = is_memop(new_pred) ? get_memop_mem(new_pred)
                                          : get_irn_n(new_pred, 0);
          }
    ...
    // 5. remove input of loop header which represents jump from the last loop
    // iteration
  ...
  for (int i = get_End_n_keepalives(end); i-- > 0;) {
    ...
    if (is_Block(el) && (block_is_inside_loop(el, loop) ||
                         pset_new_contains(&loop_blocks, el))) {
...
static bool duplicate_innermost_loops(ir_loop *const loop,
  ...
  // all loops directly in this loop were unrolled. reanalyze later to possibly
...
void unroll_loops(ir_graph *const irg, unsigned factor, unsigned maxsize) {
  ...
  assure_irg_properties(
      irg, IR_GRAPH_PROPERTY_CONSISTENT_OUTS | IR_GRAPH_PROPERTY_NO_BADS);
  do {
    reanalyze = false;
