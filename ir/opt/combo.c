...
struct node_t {
  ...
  bool
      on_touched : 1; /**< Set, if this node is on the partition.touched set. */
  ...
  unsigned
      flagged : 2; /**< 2 Bits, set if this node was visited by race 1 or 2. */
};
...
struct partition_t {
  ...
  partition_t
      *touched_next; /**< Points to the next partition in the touched set. */
  partition_t
      *cprop_next; /**< Points to the next partition in the cprop list. */
  partition_t *split_next; /**< Points to the next partition in the list that
                              must be split by split_by(). */
  ...
  bool type_is_B_or_C : 1; /**< Set, if all nodes in this partition have type
                              Bottom or Constant. */
#ifdef DEBUG_libfirm
...
typedef struct environment_t {
  ...
  bool commutative : 1; /**< Set, if commutation nodes should be handled
                           specially. */
#ifdef DEBUG_libfirm
...
static int cmp_irn_opcode(const ir_node *a, const ir_node *b) {
  ...
  if (arity != get_irn_arity(b)) return 1;
...
static void check_all_partitions(environment_t *env) {
  ...
  for (partition_t *P = env->dbg_list; P != NULL; P = P->dbg_next) {
    check_partition(P);
    if (!P->type_is_B_or_C) check_opcode(P);
...
static void dump_partition(const char *msg, const partition_t *part) {
  ...
  DB((dbg, LEVEL_2, "%s part%u%s (%u, %+F) {\n  ", msg, part->nr,
      part->type_is_B_or_C ? "*" : "", part->n_leaders, type));
 ...
 */
static int dump_partition_hook(FILE *F, const ir_node *n,
                               const ir_node *local) {
...
static void verify_type(const lattice_elem_t old_type, node_t *node) {
  ...
  panic("wrong translation from %+F to %+F on node %+F", old_type, node->type,
        node->node);
}
 ...
 * @param map  the listmap
 */
static void listmap_term(listmap_t *map) { del_set(map->map); }
...
static listmap_entry_t *listmap_find(listmap_t *map, void *id) {
  ...
  listmap_entry_t *entry =
      set_insert(listmap_entry_t, map->map, &key, sizeof(key), hash_ptr(id));
...
static void add_to_cprop(node_t *y, environment_t *env) {
  ...
  if (get_irn_mode(irn) == mode_T) {
    ...
    /* Due to the way we handle Phi's, we must place all Phis of a block on the
     * list if someone placed the block. The Block is only placed if the
     * reachability changes, and this must be re-evaluated in compute_Phi(). */
    for (ir_node *phi = get_Block_phis(irn); phi != NULL;
 ...
 * @param env      the environment
 */
static void update_worklist(partition_t *Z, partition_t *Z_prime,
                            environment_t *env) {
    ...
    for (unsigned j = 0; j < pred->n_followers; ++j) {
      ...
      if (edge.pos == i && edge.use == irn) {
        ...
        for (k = pred->n_followers + 1; k < n; ++k) {
          if (p->o.out->edges[k].pos >= edge.pos) break;
 ...
 */
static partition_t *split_no_followers(partition_t *Z, node_t *g,
                                       environment_t *env) {
  ...
  for (node_t *node = g; node != NULL; node = node->next) {
    ...
    node->part = Z_prime;
    if (node->max_user_input > max_input) max_input = node->max_user_input;
  }
...
static bool is_real_follower(const ir_node *irn, int input) {
  switch (get_irn_opcode(irn)) {
    ...
    case iro_Phi: {
      ...
      if (pred->type.tv == tarval_bottom) return false;
      break;
    }
    case iro_Sub:
    case iro_Shr:
...
static partition_t *split(partition_t **pX, node_t *gg, environment_t *env) {
  ...
  int max_input = 0;
  unsigned n = 0;
  for (node_t *node = senv[winner].walked; node != NULL;
       node = node->race_next) {
    list_del(&node->node_list);
    node->part = X_prime;
    if (node->is_follower) {
...
static bool type_is_neither_bottom_nor_const(const lattice_elem_t type) {
  if (is_tarval(type.tv)) {
    if (type.tv == tarval_bottom) return false;
    if (tarval_is_constant(type.tv)) return false;
  } else {
    /* is an entity */
    return false;
  }
...
static void collect_touched(list_head *list, int idx, environment_t *env) {
  ...
  list_for_each_entry(node_t, x, list, node_list) {
    ...
    while (x->next_edge < num_edges) {
      ...
      if (env->commutative && (idx == 0 || idx == 1) &&
          is_op_commutative(get_irn_op(succ)))
        continue;

      /* ignore the "control input" for non-pinned nodes
      if we are running in GCSE mode */
      if (idx < end_idx && !get_irn_pinned(succ)) continue;

      node_t *y = get_irn_node(succ);
      assert(get_irn_n(succ, idx) == x->node);

      /* ignore block edges touching followers */
      if (idx == -1 && y->is_follower) continue;

      if (tarval_is_constant(y->type.tv)) {
        unsigned code = get_irn_opcode(succ);
        if (code == iro_Sub || code == iro_Cmp) add_to_cprop(y, env);
      }

      /* Partitions of constants should not be split simply because their Nodes
         have unequal functions or incongruent inputs. */
...
static void collect_commutative_touched(list_head *list, environment_t *env) {
  list_for_each_entry(node_t, x, list, node_list) {
    ...
    while (x->next_edge < num_edges) {
      ...
      ++x->next_edge;
      if (edge->pos < 0) continue;

      succ = edge->use;

      /* only commutative nodes */
      if (!is_op_commutative(get_irn_op(succ))) continue;

      node_t *y = get_irn_node(succ);
      if (tarval_is_constant(y->type.tv)) {
        unsigned code = get_irn_opcode(succ);
        if (code == iro_Eor) add_to_cprop(y, env);
      }
...
static partition_t *split_by_what(partition_t *X, what_func What,
  ...
  /* Let map be an empty mapping from the range of What to (local) list of
   * Nodes. */
  listmap_t map;
  listmap_init(&map);
...
static void *lambda_opcode(const node_t *node, environment_t *env) {
  ...
  opcode_key_t *entry = set_insert(opcode_key_t, env->opcode2id_map, &key,
                                   sizeof(key), opcode_hash(&key));
  return entry;
}
...
static void *lambda_partition(const node_t *node, environment_t *env) {
  ...
  if (i < env->end_idx && !get_irn_pinned(skipped)) return NULL;
...
static void *lambda_commutative_partition(const node_t *node,
                                          environment_t *env) {
  int i = env->lambda_input;
...
static bool is_con(const lattice_elem_t type) {
  /* be conservative */
  if (is_tarval(type.tv)) return tarval_is_constant(type.tv);
            ...
            for (int input = arity - 1; input >= -1; --input) {
              do {
                ...
                if (Z_prime->n_leaders > 1) {
                  ...
                  DEBUG_ONLY(snprintf(buf, sizeof(buf),
                                      "lambda n.(n[%d].partition)", input);)
...
static void default_compute(node_t *node) {
  ir_node *irn = node->node;
  if (get_irn_mode(irn) == mode_X) node->type.tv = tarval_top; /* reachable */
...
static void join(lattice_elem_t *const res, lattice_elem_t const *const a,
                 lattice_elem_t const *const b) {
  if (a->tv == b->tv) {
    *res = *a;
...
static void compute_Phi(node_t *node) {
  ...
  for (int i = get_Phi_n_preds(phi); i-- > 0;) {
    ...
    if (pred_X->type.tv == tarval_bottom) continue;
...
static void compute_Sub(node_t *node) {
  ...
  if (a.tv == tarval_bottom || b.tv == tarval_bottom) {
    ...
    if (node->type.tv != tv) tv = tarval_top;
    node->type.tv = tv;
  } else {
    node->type.tv = tarval_top;
  }
...
static void compute_Cmp(node_t *node) {
  ...
  if (a.tv == tarval_bottom || b.tv == tarval_bottom) {
    ...
    ir_tarval *tv =
        relation & ir_relation_equal ? tarval_b_true : tarval_b_false;
...
static void compute_Proj_Switch(node_t *node, ir_node *switchn) {
  ...
  if (selector->type.tv == tarval_bottom) {
    ...
    node->type.tv = pn == pn_Switch_default ? tarval_top : tarval_bottom;
  }
}
...
static void compute(node_t *node) {
   ...
   */
  if (node->type.tv == tarval_top) return;
#endif
 ...
 * synonym for equivalent_node(). While this is true, we cannot use it for the
 * algorithm here, because it expects that the identity node is one of the
 * inputs, which is NOT always true for equivalent_node() which can handle (and
 * does sometimes) DAGs. So, we have our own implementation, which copies some
 * parts of equivalent_node()
 */
...
static node_t *identity_comm_zero_binop(node_t *node) {
  ...
  /* for FP these optimizations are only allowed if fp_strict_algebraic is
   * disabled */
  ...
  if (a->type.tv == zero) return b;
  if (b->type.tv == zero) return a;
  return node;
}
...
static node_t *identity_shift(node_t *node) {
  ...
  /* node: no input should be tarval_bottom, else the binop would be also
   * Bottom and not being split. */
  if (b->type.tv == zero) return get_irn_node(get_binop_left(op));
  return node;
}
...
static node_t *identity_Mul(node_t *node) {
  ...
  if (a->type.tv == one) return b;
  if (b->type.tv == one) return a;
  return node;
}
...
static node_t *identity_Sub(node_t *node) {
  ...
  if (b->type.tv == get_mode_null(mode)) return get_irn_node(get_Sub_left(sub));
  return node;
}
...
static node_t *identity_And(node_t *node) {
   ...
   * Bottom and not being split. */
  if (a->type.tv == neutral) return b;
  if (b->type.tv == neutral) return a;
  return node;
}
  ...
  while (env->cprop != NULL) {
    ...
    for (;;) {
      ...
      if (x->type.tv != old_type.tv) {
        ...
        if (!x->on_fallen) {
          /* Add x to fallen. Nodes might fall from T -> const -> _|_, so check
             that they are not already on the list. */
          x->next = fallen;
          x->on_fallen = true;
          fallen = x;
          ++n_fallen;
    ...
    for (node_t *x = fallen; x != NULL; x = x->next) x->on_fallen = false;
...
static void find_kept_memory(ir_node *irn, void *ctx) {
  ...
  node_t *block = get_irn_node(get_nodes_block(irn));
  if (!is_reachable(block)) return;

  node_t *node = get_irn_node(irn);
  if (node->type.tv == tarval_bottom) return;
...
static void apply_result(ir_node *irn, void *ctx) {
  ...
  if (is_Block(irn) || is_End(irn) || is_Bad(irn)) return;
...
void combo(ir_graph *irg) {
  ...
  do {
    propagate(&env);
    if (env.worklist != NULL) cause_splits(&env);
  ...
  if (len > 0) add_memory_keeps(irg, env.kept_memory, len);
