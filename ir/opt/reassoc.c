...
static ir_mode *get_mode_from_ops(ir_node *op1, ir_node *op2) {
  ...
  if (mode_is_reference(m1)) return m1;

  ir_mode *m2 = get_irn_mode(op2);
  if (mode_is_reference(m2)) return m2;
...
static int reassoc_commutative(ir_node **node) {
  ...
  if (get_irn_op(t1) == op) {
    ...
    if (c_c1 != NO_CONSTANT) {
      ...
      DBG((dbg, LEVEL_5,
           "Applied: %n .%s. (%n .%s. %n) => %n .%s. (%n .%s. %n)\n", c1,
           get_irn_opname(n), c2, get_irn_opname(n), t2, t2, get_irn_opname(n),
           c1, get_irn_opname(n), c2));
  ...
  if (get_irn_op(t1) == op) {
    ...
    if (l == c1 && r != c1) {
      ...
      ir_mode *mode_c1 = get_irn_mode(c1);
      ir_node *in[] = {c1, c1};
      ir_node *irn0 = create_node(dbgi, block, op, mode_c1, ARRAY_SIZE(in), in);

      ir_node *in1[] = {r, irn0};
      ir_node *irn1 =
          create_node(dbgi, block, op, mode_res, ARRAY_SIZE(in1), in1);

      DBG((dbg, LEVEL_5,
           "Applied: %n .%s. (%n .%s. %n) => %n .%s. (%n .%s. %n)\n", c1,
           get_irn_opname(n), l, get_irn_opname(n), r, r, get_irn_opname(n), c1,
           get_irn_opname(n), c1));
...
static ir_node *earliest_block(ir_node *a, ir_node *b, ir_node *curr_blk) {
  ...
  if (res == get_irg_start_block(get_irn_irg(curr_blk))) return curr_blk;
  return res;
}
...
static int reverse_rule_distributive(ir_node **node) {
  ...
  if (op != get_irn_op(right)) return 0;
...
static int move_consts_up(ir_node **node) {
  ...
  if (is_constant_expr(l) || is_constant_expr(r)) return 0;
...
transform:;
  ...
  if (ma != mb && mode_is_int(ma) && mode_is_int(mb)) return 0;

  /* check if (a .op. b) can be calculated in the same block is the old
   * instruction */
  if (!block_dominates(get_nodes_block(a), blk)) return 0;
...
static void reverse_rules(ir_node *node, void *env) {
  ...
  if (mode_is_float(mode) && !ir_imprecise_float_transforms_allowed()) return;
 ...
 */
static void find_path_to_top_node(ir_node *current, ir_node *other_node,
                                  ir_node *base_node, ir_node *middle_node,
                                  ir_node *top_node, shannon_data *shdata) {
  ir_node *top_node2;
  ...
  if (current == top_node2 && ((middle_node && !only_one_user(middle_node)) ||
  ...
  if ((counter - shdata->walk_base) == (unsigned)get_irn_n_edges(current) &&
      is_bitop(current)) {
    foreach_irn_in(current, i, n) {
      find_path_to_top_node(n, current, base_node, middle_node, top_node,
                            shdata);
    }
  }
}
...
/**
 * If given node is a middle_node, return the top_node. Else return the node
 * itself.
 */
...
static void try_basenode(ir_node *base_node, void *env) {
  ...
  for (int i = 0; i < 2; i++) {
    ...
    find_path_to_top_node(r, base_node, base_node, middle_node, top_node,
                          shdata);
...
/**
 * Replace top_node from given other_node by constant. base_node could be And or
 * Or and is used to decide if the constant will be (replacement Eor -1) or
 * (replacement Eor 0).
 */
static void replace_node(ir_node *top_node, ir_node *base_node,
                         ir_node *other_node, ir_tarval *replacement) {
  ...
  ir_tarval *base_val = is_And(base_node) ? get_mode_all_one(other_mode)
                                          : get_mode_null(other_mode);
...
static ir_tarval *get_Eor_tarval(ir_node *node) {
  ...
  if (is_Const(l)) return get_Const_tarval(l);
...
static void do_shannon(ir_graph *irg) {
  shannon_data shdata = {
      .irg = irg, .walk_counter = pmap_create(), .walk_base = 0, .walk_max = 0};
  ...
  deq_foreach_pointer(&shdata.optimizations, optimization_t, optimization) {
    ...
    DBG((dbg, LEVEL_4,
         "base_node: %+F, middle_node: %+F, top_node: %+F, other_node: %+F\n",
    ...
    if (base_node == other_node) continue;

    /* check if optimization is still valid */
    if (middle_node) {
      if (!has_operand(middle_node, top_node) ||
          !has_operand(base_node, middle_node)) {
        continue;
      }
 ...
 * @param need_not  Whether we should replace a with ~b (instead of b), or vice
 * versa.
 * @param replace   Whether we already found one node and thus can replace the
 * other.
 *
 * @return match result that indicates whether we found a or b first, or none of
 * them.
 */
static match_result_t replace_until_other_user(ir_node *node, ir_node *a,
                                               ir_node *b, bool need_not,
                                               bool replace) {
...
/**
 * If one of the following cases is matched, replace_until_other_user will be
 * called: (a ^ b) & f(a, b) ->  (a ^ b) & f(a, ~a)
 * ~(a ^ b) & f(a, b) -> ~(a ^ b) & f(a,  a)
 *  (a ^ b) | f(a, b) ->  (a ^ b) | f(a,  a)
 * ~(a ^ b) | f(a, b) -> ~(a ^ b) | f(a, ~a)
 */
...
typedef struct {
  ...
  pmap *edge_value;   /**< Edge-based multiplier, used to compute the multiplier
                         above */
  pset *multi_operands; /**< Multiops of all operands */
  pset *multi_users;    /**< Multiops of all users */
  bool other_op; /**< Whether a node has a user that does not belong to any
                    multiop in multi_users (because the operation is not
                    supported by multiops) */
...
static ir_op *get_multi_op_op(multi_op *multi_op) {
  ...
  return is_Sub(base_node) || is_Mul(base_node) ? op_Add
                                                : get_irn_op(base_node);
}
...
static bool is_scalar_Mul(ir_node *node) {
  return is_Mul(node) &&
         (is_Const(get_Mul_left(node)) || is_Const(get_Mul_right(node)));
}

/**
 * Adds a node to a Eor set.
 */
static void add_to_Eor_set(multi_op_env *multi_env, ir_node *node,
                           multi_op *set) {
 ...
 */
static void add_to_And_Or_set(multi_op_env *multi_env, ir_node *node,
                              multi_op *set) {
  if (get_irn_op(node) == get_multi_op_op(set)) {
    pset_insert_ptr(set->nodes, node);
    pmap_insert(multi_env->set_map, node, set);
...
/**
 * Adds a node to a Add set, with given entry. The entry is the user of the node
 * and indicates it's value. Example: In 3*(x + y) is 3 the value of (x+y) and *
 * is the entry.
 */
static void add_to_Add_set(multi_op_env *multi_env, ir_node *node,
                           multi_op *set, ir_node *entry) {
  ir_mode *mode = get_irn_mode(node);
  ir_op *op = get_irn_op(node);
  ...
  if (is_scalar_Mul(node) && mode == base_mode) {
    ...
    pmap_insert(set->edge_value, node, val);
              ...
              (op == op_Sub &&
               ...
               !mode_is_reference(get_irn_mode(get_Sub_left(node))))) &&
             mode == base_mode) {
 ...
 */
static void add_to_set(multi_op_env *multi_env, ir_node *node, multi_op *set,
                       ir_node *entry) {
 ...
 */
static void connect_Add(multi_op *operand, multi_op *user, ir_node *op_node,
                        ir_node *entry) {
  assert(entry);
...
/**
 * Connects a operand multi_op to a user multi_op. Connect means operand is a
 * whole operand of user.
 */
static void connect_sets(multi_op *operand, multi_op *user, ir_node *op_node,
                         ir_node *user_node) {
...
/**
 * Returns true iff the given node is supported by setsort, else it would be
...
static void walk_sets(ir_node *node, void *env) {
  ...
  if (is_supported_node(node)) {
    ...
    ir_op *op = get_irn_op(node);
    if (is_same_set && (get_multi_op_op(user_set) == op ||
                        (get_multi_op_op(user_set) == op_Add &&
                         (op == op_Sub || is_scalar_Mul(node))))) {
...
static void merge(multi_op_env *multi_env, multi_op *a, multi_op *b) {
  ...
  if (get_multi_op_op(b) == op_Add) {
    ...
    foreach_pset(a->operands, ir_node, n) { add_to_set(multi_env, n, b, NULL); }
 ...
 */
static void merge_trivial_multi_op(multi_op_env *multi_env, multi_op *a,
                                   multi_op *b) {
...
static multi_op *get_user(multi_op *o) {
  ...
  multi_op *user = NULL;
  foreach_pset(o->multi_users, multi_op, n) { user = n; }
...
/**
 * Rebuild the graph according to the multi_ops. Only sets with change flag set,
 * will bereplaced.
 */
  ...
  foreach_pset(multi_env->sets, multi_op, o) {
    ...
    if (get_multi_op_op(o) == op_Add) {
      ...
      foreach_pmap(dic, entry) {
        ...
        if (is_max_val || pmap_contains(dic, neg_val) ||
            !tarval_is_negative(curr_val)) {
          assert(!(mode_is_signed(get_irn_mode(o->base_node)) &&
                   pset_find_ptr(done, neg_val)));
      ...
      deq_foreach_pointer(&negatives, ir_tarval, negativ_val) {
        ...
        if (!tarval_is_one(positiv_val)) {
          ...
          ir_node *c = new_rd_Const(dbgi, get_irn_irg(inner),
                                    tarval_convert_to(positiv_val, node_mode));
          inner = new_rd_Mul(dbgi, block, c, inner);
        }
...
static void do_Setsort(ir_graph *irg) {
  ...
  foreach_pset(multi_env->sets, multi_op, o) { destroy_multi_op(o); }
...
/**
 * This function checks if chaining could be applied and swaps nodes if
 * possible.
 */
static bool walk_chains(ir_node *node) {
  ...
  if (node_op == op_Or || node_op == op_And || node_op == op_Eor) {
    ...
    if (!changed && get_irn_op(b) == node_op && is_bitop(a) &&
        only_one_user(b)) {
