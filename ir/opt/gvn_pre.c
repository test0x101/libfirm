...
typedef struct block_info {
  ...
  ir_valueset_t
      *antic_done; /* keeps elements of antic_in after insert nodes phase */
...
typedef struct pre_env {
  ...
  ir_nodeset_t
      *keeps; /* a list of to be removed phis to kill their keep alive edges */
  unsigned last_idx; /* last node index of input graph */
  char changes;      /* flag for fixed point iterations - non-zero if changes
                        occurred */
...
static void init_stats(void) { gvnpre_stats = XMALLOCZ(gvnpre_statistics); }
 ...
 */
static void dump_value_set(ir_valueset_t *set, const char *txt,
                           ir_node *block) {
...
static void dump_all_expgen_sets(block_info *list) {
  for (block_info *block_info = list; block_info != NULL;
...
static int compare_gvn_identities(const void *elt, const void *key) {
  ...
  if (is_Block(a) || is_Block(b)) return 1;
 ...
 * values/leaders, a true value tree is built. Phis kill their predecessor
 * value, so no circular dependencies need to be resolved.
 *
 * TODO Improvement:
...
static void free_block_info(block_info *block_info) {
  ...
  if (block_info->new_set) ir_valueset_del(block_info->new_set);
}
...
static void infinite_loop_walker(ir_node *block, void *env) {
  ...
  if (block == environment->start_block) return;
...
static unsigned is_in_infinite_loop(ir_node *block) {
  ...
  if (loop) return (get_loop_link(loop) != NULL);
...
static unsigned is_nice_value(ir_node *n) {
  ...
  if (is_Phi(n)) return 1;

#if LOADS || DIVMODS
  if (is_Proj(n) && mode != mode_X && mode != mode_T) return 1;
#else
  if (is_Proj(n)) return 0;
#endif

#if LOADS
  if (is_Load(n)) return get_Load_volatility(n) == volatility_non_volatile;
  if (is_Store(n)) return get_Store_volatility(n) == volatility_non_volatile;
#endif

  if (get_irn_pinned(n)) return 0;

  if (!mode_is_data(mode)) {
    if (!is_Div(n) && !is_Mod(n)) return 0;
  }
  return 1;
}
 ...
 */
static unsigned is_clean_in_block(ir_node *n, ir_node *block,
                                  ir_valueset_t *valueset) {
  if (is_Phi(n)) return 1;

  if (!is_nice_value(n)) return 0;

#if LOADS
  /* filter loads with no phi predecessor from antic_in */
  if (is_Load(n) && !is_Phi(get_Load_mem(n))) return 0;
  if (is_Store(n) && !is_Phi(get_Store_mem(n))) return 0;
#endif
  ...
  if (is_Div(n)) {
    ...
    if (!is_Phi(mem) && !is_NoMem(mem)) return 0;
  }

  if (is_Mod(n) && !is_Phi(get_Mod_mem(n))) return 0;
#endif

  foreach_irn_in(n, i, pred) {
    if (is_Phi(pred)) continue;

    /* we only handle current block */
    if (get_nodes_block(pred) != block) continue;

    if (!is_nice_value(pred)) return 0;

    ir_node *const value = identify(pred);
    if (!ir_valueset_lookup(valueset, value)) return 0;
  }
  return 1;
}
...
static void topo_walker(ir_node *irn, void *ctx) {
  ...
  if (is_irn_constlike(irn)) return;
...
static ir_node *get_translated(ir_node *block, ir_node *node) {
  if (is_irn_constlike(node)) return node;
 ...
 *
 */
static void set_translated(ir_nodehashmap_t *map, ir_node *node,
                           ir_node *trans) {
...
static ir_node *phi_translate(ir_node *node, ir_node *block, int pos,
                              ir_valueset_t *leaderset) {
  ir_node *pred_block = get_Block_cfgpred_block(block, pos);

  if (is_Phi(node)) {
    if (get_nodes_block(node) == block) return get_Phi_pred(node, pos);
  ...
  foreach_irn_in(node, i, pred) {
    ...
    if (is_Div(node)) {
      ...
      if (!is_Phi(mem)) pred_trans = get_Div_mem(node);
    }
#endif
    ...
    if (pred_trans == NULL) {
      ...
      if (is_Proj(pred_trans) && get_irn_mode(pred_trans) == mode_M) {
        ...
        if (identify(new_pred) != identify(pred)) needed |= 1;
      }
    }
  ...
  if (is_Proj(node)) pred_block = get_nodes_block(in[0]);
  ...
  DB((dbg, LEVEL_3, "New node %+F in %+F origin %+F\n", nn,
      get_Block_cfgpred_block(block, pos), node));
  return nn;
}
...
static void compute_antic(ir_node *block, void *ctx) {
  ...
  if (n_succ == 1 && get_irn_arity(get_Block_cfg_out(block, 0)) > 1) {
    ...
    foreach_valueset(succ_info->antic_in, value, expr, iter) {
      ...
      if (is_clean_in_block(expr, block, info->antic_in)) {
        ...
        if (env->iteration <= 2 ||
            (is_backedge(succ, pos) && !is_in_infinite_loop(succ))) {
  ...
  if (size != ir_valueset_size(info->antic_in)) env->changes |= 1;
}
 ...
 */
static ir_mode *is_partially_redundant(ir_node *block, ir_node *expr,
                                       ir_node *value) {
  ...
  for (int pos = 0; pos < arity; ++pos) {
    ...
    if (is_Const(trans_expr))
      ...
      avail_expr =
          (ir_node *)ir_valueset_lookup(pred_info->avail_out, trans_value);
    ...
    if (avail_expr == NULL && is_Const(trans_expr)) {
      ...
      /* tarval within range? */
      if (tarval_in_range(lower, c, upper)) avail_expr = trans_expr;
    }
  ...
  if (!partially_redundant || fully_redundant) return NULL;
  return mode;
}
...
static unsigned is_hoisting_greedy(ir_node *irn, ir_node *block) {
  ...
  for (int pos = 0; pos < block_arity; ++pos) {
    ...
    foreach_irn_in(irn, i, pred) {
      ...
      if (is_Phi(pred) && get_irn_n_edges(pred) != 1) return 1;
#endif

      if (is_Phi(pred) && get_nodes_block(pred) == block) continue;
      ...
      if (is_Address(trans_val) || is_Align(trans_val) || is_Const(trans_val) ||
          is_Offset(trans_val) || is_Size(trans_val)) {
        /* existing constant */
      ...
      if (is_irn_constlike(trans_val)) continue;
      ...
      if (ir_valueset_lookup(info->antic_done, value) == NULL) return 1;
#endif
    }
  }
  return 0;
...
static void insert_nodes_walker(ir_node *block, void *ctx) {
  ...
  block_info *info = get_block_info(block);
  if (info->new_set) ir_valueset_del(info->new_set);
  info->new_set = ir_valueset_new(16);

  if (block == env->start_block) return;
  ...
  foreach_valueset(info->antic_in, value, expr, iter) {
    /* already done? */
    if (ir_valueset_lookup(info->antic_done, value)) continue;

    /* filter phi nodes from antic_in */
    if (is_Phi(expr)) continue;
...
#ifdef DEBUG_libfirm
    if (!is_Proj(expr)) {
      if (env->first_iter) inc_stats(gvnpre_stats->first_iter_found);
    ...
    for (int pos = 0; pos < arity; ++pos) {
      ...
      if (!pred_info->found) {
        ...
        if (is_Proj(expr)) target_block = get_nodes_block(in[0]);
...
static void hoist_high(ir_node *block, void *ctx) {
  ...
  if (curr_info->new_set) ir_valueset_del(curr_info->new_set);
  ...
  foreach_valueset(curr_info->antic_done, value, expr, iter) {
    ...
    if (is_memop(expr) || is_Proj(expr)) continue;
    ...
    for (pos = 0; pos < arity; ++pos) {
      ...
      if (new_target) {
        ...
        foreach_irn_in(avail, i, pred) {
          ir_node *avail_pred =
              ir_valueset_lookup(target_info->avail_out, identify(pred));
          assert(avail_pred);
          in[i] = avail_pred;
        }
...
static void eliminate_nodes(elim_pair *pairs, ir_nodeset_t *keeps) {
  ...
  foreach_ir_nodeset(keeps, m_phi, iter) { remove_End_keepalive(end, m_phi); }
}
...
void do_gvn_pre(ir_graph *irg) {
  ...
  for (block_info *block_info = env.list; block_info != NULL;
       block_info = block_info->next) {
