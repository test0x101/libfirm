...
static void place_floats_early(ir_node *n, deq_t *worklist) {
  ...
  if (get_irn_pinned(n)) {
    ...
    foreach_irn_in(n, i, pred) { deq_push_pointer_right(worklist, pred); }
    if (!is_Block(n)) deq_push_pointer_right(worklist, get_nodes_block(n));
    return;
  }
  ...
  foreach_irn_in(n, i, pred) { place_floats_early(pred, worklist); }
  ...
  /* Set the new block */
  if (new_block != NULL) set_nodes_block(n, new_block);
}
...
static void place_early(ir_graph *irg, deq_t *worklist) {
  ...
  while (!deq_empty(worklist)) {
    ...
    if (!irn_visited(n)) place_floats_early(n, worklist);
  }
...
static ir_node *calc_dom_dca(ir_node *dca, ir_node *block) {
  assert(block != NULL);
  if (dca == NULL || dca == block) return block;
...
static void move_out_of_loops(ir_node *n, ir_node *early) {
  ...
  if (best != get_nodes_block(n)) set_nodes_block(n, best);
}
...
static void place_floats_late(ir_node *n, deq_t *worklist) {
  ...
  if (get_irn_mode(n) == mode_T) set_projs_block(n, get_nodes_block(n));
}
...
static void place_late(ir_graph *irg, deq_t *worklist) {
  ...
  while (!deq_empty(worklist)) {
    ir_node *n = deq_pop_pointer_left(ir_node, worklist);
    if (!irn_visited(n)) place_floats_late(n, worklist);
  }
}
