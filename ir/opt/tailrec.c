...
static void do_opt_tail_rec(ir_graph *irg, tr_env *env) {
  ...
  if (n_parameters > 0) {
    ...
    for (unsigned p = 0; p < n_parameters; ++p) {
      ...
      if (parameter_proj == NULL) continue;
  ...
  if (n_locs > 0) {
    ...
    for (ir_node *next, *ret = env->rets; ret != NULL; ret = next) {
      ...
      ir_node *in[pn_Call_max + 1] = {
          [pn_Call_M] = mem, [pn_Call_T_result] = tuple};
      int n_in = 2;
      ...
      for (size_t i = 0; i < env->n_ress; ++i) {
        ...
        if (env->variants[i] != TR_DIRECT) set_r_value(irg, i, res);
      }
...
static bool check_lifetime_of_locals(ir_graph *irg) {
  ...
  foreach_irn_out_r(irg_frame, i, succ) {
    if (is_Member(succ)) {
      ...
      if (get_entity_owner(get_Member_entity(succ)) != frame_tp) return false;

      if (is_address_taken(succ)) return false;
    }
  }
  return true;
}
  ...
  switch (get_irn_opcode(irn)) {
    case iro_Add: {
      ...
      if (get_nodes_block(b) != call_block) {
        ...
        if (vb == TR_BAD) return TR_BAD;
      }
      tail_rec_variants res = va == vb           ? va
                              : va == TR_UNKNOWN ? vb
                              : vb == TR_UNKNOWN ? va
                                                 : TR_BAD;
      if (res == TR_DIRECT || res == TR_ADD) return TR_ADD;
      /* not compatible */
      return TR_BAD;
    }
    ...
    case iro_Sub: {
      ...
      if (get_nodes_block(b) == get_nodes_block(call)) {
        ...
        if (vb != TR_UNKNOWN) return TR_BAD;
      }
      tail_rec_variants res = va;
      if (res == TR_DIRECT || res == TR_ADD) return res;
      /* not compatible */
      return TR_BAD;
    }
    ...
    case iro_Mul: {
      ...
      tail_rec_variants res = va == vb           ? va
                              ...
                              : vb == TR_UNKNOWN ? va
                                                 : TR_BAD;
      if (res == TR_DIRECT || res == TR_MUL) return TR_MUL;
      /* not compatible */
      return TR_BAD;
    }
    ...
    case iro_Minus: {
      ...
      if (res == TR_DIRECT) return TR_MUL;
      if (res == TR_MUL || res == TR_UNKNOWN) return res;
      /* not compatible */
      return TR_BAD;
    }
...
static unsigned find_tail_rec_possibilities(tr_env *env, ir_graph *irg) {
  ...
  if (n_ress > 0) {
    ...
    for (size_t i = 0; i < n_ress; ++i) env->variants[i] = TR_DIRECT;
  }
  ...
  for (int i = get_Block_n_cfgpreds(end_block); i-- > 0;) {
    ...
    if (get_nodes_block(call) != get_nodes_block(ret)) continue;

    /* check if it's a recursive call */
    ir_entity *callee = get_Call_callee(call);
    if (callee == NULL || get_entity_linktime_irg(callee) != irg) continue;
    ...
    if (mtd_type != call_type) {
       ...
       */
      DB((dbg, LEVEL_3,
          "  tail recursion fails because of call type mismatch: %+F != %+F\n",
          mtd_type, call_type));
      continue;
    }
...
void opt_tail_rec_irg(ir_graph *irg) {
  ...
  assure_irg_properties(irg, IR_GRAPH_PROPERTY_MANY_RETURNS |
  ...
  if (n_tail_calls > 0) {
    ...
    DB((dbg, LEVEL_2, "  Performing tail recursion for graph %s and %u Calls\n",
