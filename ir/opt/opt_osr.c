...
static ir_node *search(unsigned code, ir_node *op1, ir_node *op2, iv_env *env) {
  ...
  quadruple_t *entry =
      set_find(quadruple_t, env->quad_map, &key, sizeof(key), hash);
  if (entry != NULL) return entry->res;
  return NULL;
}
...
static ir_node *find_location(ir_node *block1, ir_node *block2) {
  if (block_dominates(block1, block2)) return block2;
  ...
  do {
    ...
    foreach_irn_in_r(irn, i, pred) {
      ...
      if (pe->header == header && pe->pscc == NULL) {
        /* set the pscc here to ensure that the node is NOT enqueued another
         * time */
        pe->pscc = pscc;
...
static bool is_counter_iv(ir_node *iv, iv_env *env) {
  ...
  for (ir_node *irn = pscc->head; irn != NULL; irn = e->next) {
    if (is_Add(irn) || is_Sub(irn)) {
      if (have_incr != NULL) return false;
      ...
      for (int i = get_Phi_n_preds(irn) - 1; i >= 0; --i) {
        ...
        if (ne->header == e->header) continue;
        if (have_init != NULL) return false;
        have_init = pred;
  ...
  switch (code) {
    ...
    case iro_Sub: {
      ...
      if (liv && is_rc(right, liv)) {
        iv = left;
        rc = right;
      } else if (riv && is_op_commutative(op) && is_rc(left, riv)) {
        iv = right;
        rc = left;
      } else {
        return false;
...
static void process_phi_only_scc(scc *pscc, iv_env *env) {
  ...
  if (e->next != NULL) remove_phi_cycle(pscc, env);
}
...
static void dfs(ir_node *irn, iv_env *env) {
  ...
  if (is_Block(irn)) {
    foreach_irn_in(irn, i, pred) {
      if (!irn_visited(pred)) dfs(pred, env);
    }
  } else {
    ir_node *block = get_nodes_block(irn);
    ...
    /* handle the block */
    if (!irn_visited(block)) dfs(block, env);
  ...
  if (is_Add(irn)) {
    ...
    if (mode_is_int(mode)) {
      ...
      if (mode_is_int(l_mode)) {
         ...
         * However, it is correct in this case, so add Conv to make verifier
         * happy. */
        ir_node *block = get_nodes_block(right);
...
void opt_osr(ir_graph *irg, unsigned flags) {
  ...
  if (env.replaced) {
    if (env.need_postpass) irg_walk_graph(irg, NULL, fix_adds_and_subs, &env);
