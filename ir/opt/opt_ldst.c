...
struct memop_t {
  ...
  memop_t
      *next; /**< links to the next memory op in the block in forward order. */
  memop_t *prev;  /**< links to the previous memory op in the block in forward
                     order. */
...
struct block_t {
  memop_t *memop_forward;  /**< topologically sorted list of memory ops in this
                              block */
  ...
  memop_t **trans_results;    /**< used to cached translated nodes due antic
                                 calculation. */
};
...
static void dump_curr(block_t *bl, const char *s) {
  ...
  i = 0;
  for (pos = rbitset_next(env.curr_set, 0, 1); pos < end;
       pos = rbitset_next(env.curr_set, pos + 1, 1)) {
...
#else
static void dump_block_list(ldst_env *env) { (void)env; }
static void dump_curr(block_t *bl, const char *s) {
  (void)bl;
  (void)s;
}
 ...
 */
static void walk_memory(ir_node *irn, irg_walk_func *pre, irg_walk_func *post,
                        void *ctx) {
  ir_mode *mode;
  ...
  if (mode == mode_M) {
    ...
    foreach_irn_out_r(irn, i, succ) {
      if (!irn_visited(succ)) walk_memory(succ, pre, post, ctx);
    }
  } else if (mode == mode_T) {
 ...
 * @param ctx   context parameter for the walker functions
 */
static void walk_memory_irg(ir_graph *irg, irg_walk_func pre,
                            irg_walk_func post, void *ctx) {
  inc_irg_visited(irg);
...
static void prepare_blocks(ir_node *irn, void *ctx) {
  ...
  if (is_Block(irn)) {
    ...
    if (n > env.max_cfg_preds) env.max_cfg_preds = n;
  } else {
...
static void inverse_post_order(ir_node *block, void *ctx) {
  ...
  if (env.backward == NULL) env.backward = entry;
}
...
static memop_t *alloc_memop(ir_node *irn) {
  ...
  if (irn != NULL) set_irn_link(irn, m);
  return m;
}
...
static ir_entity *find_constant_entity(ir_node *ptr) {
  for (;;) {
    if (is_Address(ptr)) {
      ...
      if (is_Class_type(owner) && ((get_entity_n_overwrites(entity) > 0) ||
      ...
      if (is_Const(r))
        ...
        return NULL;
      /* for now, we support only one subtraction, reassoc should fold all
       * others */
      if (!is_Address(ptr) && !is_Align(ptr) && !is_Offset(ptr) &&
          !is_Sel(ptr) && !is_Size(ptr))
        return NULL;
...
static void collect_memops(ir_node *irn, void *ctx) {
  ...
  if (is_Proj(irn)) {
    ...
    if (irn != get_irg_initial_mem(irg)) return;
  }
  ...
  if (is_Phi(irn)) {
    ...
    if (entry->memop_backward == NULL) entry->memop_backward = op;
  } else {
...
static memop_t *find_address(const value_t *value) {
  if (rbitset_is_set(env.curr_set, value->id)) {
    ...
    if (res->value.mode == value->mode) return res;
    /* allow hidden casts */
 ...
 * @param bl     the block
 */
static memop_t *find_address_avail(const block_t *bl, unsigned id,
                                   const ir_mode *mode) {
  if (rbitset_is_set(bl->avail_out, id)) {
    memop_t *res = bl->id_2_memop_avail[id];

    if (res->value.mode == mode) return res;
    /* allow hidden casts */
    if (get_mode_arithmetic(res->value.mode) == irma_twos_complement &&
...
/**
 * Kill memops that are not alias free due to a Store value from the current
 * set.
 *
...
static void kill_memops(const value_t *value) {
  ...
  for (pos = rbitset_next(env.curr_set, 0, 1); pos < end;
    ...
    if (ir_no_alias != get_alias_relation(value->address, value_type,
                                          value_size, op->value.address,
                                          op_type, op_size)) {
...
static ir_node *conv_to(ir_node *irn, ir_mode *mode) {
  ...
  if (other != mode) {
    /* different modes: check if conversion is possible without changing the
     * bits */
...
static void update_address(value_t *value) {
  if (is_Proj(value->address)) {
    ...
    if (is_Load(load)) {
      ...
      if (op->flags & FLAG_KILLED_NODE) value->address = op->replace;
    }
  }
}
  ...
  for (op = bl->memop_forward; op != NULL; op = op->next) {
    switch (get_irn_opcode(op->node)) {
      ...
      case iro_Store:
          ...
          if (other != NULL) {
            if (is_Store(other->node)) {
              if (op != other && !(other->flags & FLAG_IGNORE) &&
                 ...
                 * This is a shortcut: we know that the second Store will be
                 * anticipated then in an case.
                 */
        ...
        break;
      default:
        if (op->flags & FLAG_KILL_ALL) kill_all();
    }
  }
}
...
static int backward_antic(block_t *bl) {
  ...
  if (n == 1) {
    ...
    for (pos = rbitset_next(succ_bl->anticL_in, 0, 1); pos < end;
      ...
      if (op == NULL) {
        ...
        if (trans_adr != adr) {
          ...
          new_op->node =
              op->node; /* we need the node to decide if Load/Store */
...
static void replace_load(memop_t *op) {
  ...
  if (def != NULL) {
    DB((dbg, LEVEL_1, "Replacing %+F by definition %+F\n", load,
        is_Proj(def) ? get_Proj_pred(def) : def));
  } else {
 ...
 */
static void reroute_mem_through(ir_node *omem, ir_node *nmem,
                                ir_node *pass_bl) {
...
static int insert_Load(block_t *bl) {
  ...
  if (n > 1) {
    ...
    /* More than one predecessors, calculate the join for all avail_outs
       ignoring unevaluated Blocks. These put in Top anyway. */
    for (i = n - 1; i >= 0; --i) {
    ...
    for (pos = 0; pos < env.rbs_size - 1; ++pos) {
      if (!rbitset_is_set(env.curr_set, pos)) {
        ...
        if (need_phi) {
          ...
          DB((dbg, LEVEL_3,
              "Created new %+F on merging value for address %+F\n", phi,
              first->value.address));
        }
      }
    }
      ...
      if (have_some && !all_same) {
        ...
        for (i = n - 1; i >= 0; --i) {
          ...
          if (pred_bl->avail == NULL) {
            ...
            DB((dbg, LEVEL_1,
                "Created new %+F in %+F for party redundant %+F\n", load, pred,
                op->node));
            ...
            if (pred_bl->memop_forward == NULL) pred_bl->memop_forward = new_op;
  ...
  /* always update the map after gen/kill, as values might have been changed due
   * to RAR/WAR/WAW */
...
void opt_ldst(ir_graph *irg) {
  ...
  assure_irg_properties(
      irg, IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES /* we need landing pads */
  ...
  /* first step: allocate block entries. Note that some blocks might be
     unreachable here. Using the normal walk ensures that ALL blocks are
     initialized. */
  ...
  /* create address sets: for now, only the existing addresses are allowed plus
     one needed for the sentinel */
