...
static void unreachable_to_bad(ir_node *node, void *env) {
  ...
  if (is_Block(node)) {
     ...
     * code */
    if (is_block_unreachable(node)) return;
    ...
    if (is_block_unreachable(block)) return;

    ir_graph *const irg = get_irn_irg(node);
    foreach_irn_in(node, i, phi_pred) {
      if (is_Bad(phi_pred)) continue;
...
void remove_unreachable_code(ir_graph *irg) {
  ...
  confirm_irg_properties(irg, changed ? IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES |
                                            IR_GRAPH_PROPERTY_NO_TUPLES |
