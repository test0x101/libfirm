...
static bool is_loop_nested_inside(ir_loop const *inner,
                                  ir_loop const *const outer) {
...
static bool cf_has_loop_exit(ir_node const *const from_block,
                             ir_node const *const to_block) {
  DB((dbg, LEVEL_2, "Checking whether %+F --> %+F exits a loop: ", from_block,
      to_block));
  ...
  // Check if to_block's loop is an inner loop (or the same) of from_block's
  // loop.
...
/* Returns true if control flow from pred_block to block is an exit of any loop
 * containing target. */
static bool is_nested_loop_exit(ir_node const *const block,
                                ir_node const *const pred_block,
                                ir_node const *const target) {
...
static ir_node *insert_phis_recursive(ir_node *const target,
                                      ir_node *const block) {
  ...
  if (block == get_nodes_block(target)) {
...
#ifdef DEBUG_libfirm
    DB((dbg, LEVEL_2, "\tChecking if %+F can be optimized away (preds are",
        phi));
    for (unsigned i = 0; i < arity; i++) {
    ...
    if (phi_has_single_pred) {
      // single_pred dominates phi because it dominates every CF predecessor of
      // block
...
static void insert_phis_for_edge(ir_node *node, int n) {
  ...
  if (!(mode_is_data(mode) || mode == mode_M)) return;

  ir_node *block = get_nodes_block(node);
  ir_node *const pred_block = get_nodes_block(pred);
  if (!cf_has_loop_exit(pred_block, block)) return;
...
static void insert_phis_for_node(ir_node *const node, void *const env) {
  ...
  if (is_Block(node) || is_End(node)) return;
