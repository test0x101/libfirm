  ...
  foreach_irn_out_r(node, i, succ) {
    switch (get_irn_opcode(succ)) {
      case iro_Load: {
        ...
        if (get_Load_volatility(succ) == volatility_is_volatile) return true;
        ...
        if (emode == NULL || !conv_is_bitcast(emode, mode)) return true;
        break;
      }
      ...
      case iro_Store: {
        ...
        /* do not remove volatile variables */
        if (get_Store_volatility(succ) == volatility_is_volatile) return true;
        /* check if this Store is not a hidden conversion */
        ir_mode *mode = get_irn_mode(value);
        ir_type *type = get_addr_type(node);
        ir_mode *emode = get_type_mode(type);
        if (emode == NULL || !conv_is_bitcast(mode, emode)) return true;
        break;
      }
      ...
      case iro_Member: {
        ...
        if (is_Union_type(get_entity_owner(entity))) return true;
      }
...
static leaf_state_t link_all_leaf_members(ir_entity *ent, ir_node *sel) {
  ...
  if (link == ADDRESS_TAKEN) return HAS_CHILD_LOAD_STORE | HAS_CHILD_SELS;
...
static bool find_possible_replacements(ir_graph *irg) {
  ...
  foreach_irn_out_r(irg_frame, i, succ) {
    ...
    if (get_entity_owner(ent) != frame_tp) continue;
    if (get_entity_link(ent) == ADDRESS_TAKEN) continue;
    ...
    if (is_aggregate_type(ent_type) || is_atomic_type(ent_type)) {
      if (is_address_taken(succ)) {
        /* killing one */
        if (get_entity_link(ent)) --res;
        set_entity_link(ent, ADDRESS_TAKEN);
      } else {
        /* possible found one */
        if (get_entity_link(ent) == NULL) ++res;
        link_all_leaf_members(ent, succ);
      }
    }
  }
...
static path_t *find_path(ir_node *node, size_t len) {
  ...
  if (is_Sel(node)) {
    ...
    if (!is_Const(index)) goto found_root;
...
static unsigned allocate_value_numbers(pset *members, ir_entity *ent,
  ...
  for (ir_node *member = (ir_node *)get_entity_link(ent), *next; member != NULL;
       member = next) {
    ...
    path_t *path =
        set_find(path_t, pathes, key, path_size(key), path_hash(key));
    ...
    if (path != NULL) {
      ...
      DB((dbg, SET_LEVEL_2, " = %u (%s)\n", PTR_TO_INT(get_irn_link(member)),
...
static void walker(ir_node *node, void *ctx) {
  ...
  if (is_Load(node)) {
    ...
    if (!is_Member(addr)) return;
    if (!pset_find_ptr(env->members, addr)) return;
...
void scalar_replacement_opt(ir_graph *irg) {
  ...
  if (find_possible_replacements(irg)) {
    ...
    foreach_irn_out_r(irg_frame, i, succ) {
      ...
      scalars_t key;
      key.ent = ent;
      (void)set_insert(scalars_t, set_ent, &key, sizeof(key),
                       hash_ptr(key.ent));
      ...
      if (is_Array_type(ent_type)) {
        ...
        panic(
            "neither an array nor a struct or atomic value found in scalar "
            "replace");
      }
#endif
    ...
    if (nvals > 0) {
      ...
      foreach_set(set_ent, scalars_t, value) { free_entity(value->ent); }
  ...
  confirm_irg_properties(
      irg, changed ? IR_GRAPH_PROPERTIES_NONE : IR_GRAPH_PROPERTIES_ALL);
}
