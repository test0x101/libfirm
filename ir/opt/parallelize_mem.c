...
static void parallelize_load(parallelize_info *pi, ir_node *irn) {
  ...
  if (ir_nodeset_contains(&pi->all_visited, irn)) return;
  ...
  if (get_nodes_block(irn) == pi->origin_block) {
    if (is_Proj(irn)) {
      ...
      if (is_Load(pred) &&
        ...
        unsigned store_size =
            get_mode_size_bytes(get_irn_mode(get_Store_value(pred)));
        ir_node *store_ptr = get_Store_ptr(pred);
        if (get_alias_relation(org_ptr, org_type, org_size, store_ptr,
                               store_type, store_size) == ir_no_alias) {
      ...
      if (get_alias_relation(org_ptr, org_type, org_size, copyB_dst, copyB_type,
                             copyB_size) == ir_no_alias) {
  ...
  if (get_nodes_block(irn) == pi->origin_block) {
    if (is_Proj(irn)) {
      ...
      if (is_Load(pred) &&
        ...
        if (get_alias_relation(org_ptr, org_type, org_size, load_ptr, load_type,
                               load_size) == ir_no_alias) {
          ir_node *mem = get_Load_mem(pred);
          ir_nodeset_insert(&pi->user_mem, irn);
      ...
      ir_type *copyB_type = get_CopyB_type(irn);
      unsigned copyB_size = get_type_size(copyB_type);
      if (get_alias_relation(org_ptr, org_type, org_size, copyB_src, copyB_type,
                             copyB_size) == ir_no_alias &&
          get_alias_relation(org_ptr, org_type, org_size, copyB_dst, copyB_type,
                             copyB_size) == ir_no_alias) {
        ir_node *mem = get_CopyB_mem(irn);
        ir_nodeset_insert(&pi->user_mem, irn);
        parallelize_store(pi, mem);
...
static void parallelize_copyB(parallelize_info *pi, ir_node *origin,
                              ir_node *irn) {
  /* There is no point in investigating the same subgraph twice */
  if (ir_nodeset_contains(&pi->all_visited, irn)) return;
  ...
  if (get_nodes_block(irn) == pi->origin_block) {
    if (is_Proj(irn)) {
      ...
      if (is_Load(pred) &&
        ...
        unsigned store_size =
            get_mode_size_bytes(get_irn_mode(get_Store_value(pred)));
        if (get_alias_relation(org_src, org_type, org_size, store_ptr,
                               store_type, store_size) == ir_no_alias &&
            get_alias_relation(org_dst, org_type, org_size, store_ptr,
                               store_type, store_size) == ir_no_alias) {
          ir_node *mem = get_Store_mem(pred);
          ir_nodeset_insert(&pi->user_mem, irn);
          parallelize_copyB(pi, origin, mem);
      ...
      ir_type *copyB_type = get_CopyB_type(irn);
      unsigned copyB_size = get_type_size(copyB_type);
      if (get_alias_relation(org_src, org_type, org_size, copyB_dst, copyB_type,
                             copyB_size) == ir_no_alias &&
          get_alias_relation(org_dst, org_type, org_size, copyB_src, copyB_type,
                             copyB_size) == ir_no_alias &&
          get_alias_relation(org_dst, org_type, org_size, copyB_dst, copyB_type,
                             copyB_size) == ir_no_alias) {
        ir_node *mem = get_CopyB_mem(irn);
        ir_nodeset_insert(&pi->user_mem, irn);
        parallelize_copyB(pi, origin, mem);
...
static void walker(ir_node *proj, void *env) {
  ...
  if (is_Load(mem_op)) {
    ...
    pi.origin_block = block, pi.origin_ptr = get_Load_ptr(mem_op);
    ...
    pi.origin_block = block, pi.origin_ptr = get_Store_ptr(mem_op);
  ...
  if (n > 0) { /* nothing happened otherwise */
    ...
    size_t i = 0;
    in[i++] = proj;
    foreach_ir_nodeset(&pi.user_mem, node, iter) { in[i++] = node; }
    assert(i == n + 1);
    ...
    if (n == 1) {
      ...
      i = 0;
      foreach_ir_nodeset(&pi.this_mem, node, iter) { in[i++] = node; }
      assert(i == n);
...
static void dfs_by_edges_from(ir_node *irn, irg_walk_func *pre,
  ...
  dfs_env_t dfs_env = {pre, post, again, &visited, env};
...
static void simplify_Sync(ir_node *irn, void *e) {
  ...
  if (irn != new_Sync) exchange(irn, new_Sync);
