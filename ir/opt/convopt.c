...
static bool is_optimizable_node(const ir_node *node, ir_mode *dest_mode) {
  switch (get_irn_opcode(node)) {
    ...
    case iro_Shrs: {
      ...
      if (dest_size != size) return false;
    } /* FALLTHROUGH */
    case iro_Shl: {
...
static int get_conv_costs(const ir_node *const node, ir_mode *const dest_mode) {
  ...
  if (ir_zero_when_converted(node, dest_mode)) return -1;

  /* TODO: Phi nodes */

  if (!is_downconv(mode, dest_mode)) return 1;
  ...
  if (!is_optimizable_node(node, dest_mode)) return 1;
...
static ir_node *conv_transform(ir_node *node, ir_mode *dest_mode) {
  ...
  if (get_irn_n_edges(node) > 1) return place_conv(node, dest_mode);

  if (!is_downconv(mode, dest_mode)) return place_conv(node, dest_mode);
  ...
  if (!is_optimizable_node(node, dest_mode)) return place_conv(node, dest_mode);
  ...
  ir_node *const new_node =
      new_ir_node(dbg, irg, block, op, dest_mode, arity, ins);
...
static void conv_opt_walker(ir_node *node, void *data) {
  ...
  ir_mode *const mode = get_irn_mode(node);
  if (mode_is_reference(mode)) return;

  ir_node *const pred = get_Conv_op(node);
  ir_mode *const pred_mode = get_irn_mode(pred);
  if (mode_is_reference(pred_mode)) return;

  if (!is_Phi(pred) && !is_downconv(pred_mode, mode)) return;
...
void conv_opt(ir_graph *irg) {
  ...
  do {
    ...
    if (changed) local_optimize_graph(irg);
