...
static int entry_cmp(const void *elt, const void *key) {
  ...
  return (e1->q.ent != e2->q.ent) || (e1->q.pos != e2->q.pos) ||
         (e1->q.tv != e2->q.tv);
}
...
static unsigned hash_entry(const entry_t *entry) {
  return hash_ptr(entry->q.ent) ^ hash_ptr(entry->q.tv) ^
         (unsigned)(entry->q.pos * 9);
}
...
static void copy_irn_to_irg(ir_node *n, ir_graph *irg) {
  ...
  nn = new_ir_node(
      get_irn_dbg_info(n), irg, NULL, /* no block yet, will be set later */
      get_irn_op(n), get_irn_mode(n), get_irn_arity(n), get_irn_in(n));
 ...
 * @param hmap    The quadruple-set containing the calls with constant
 * parameters
 */
static void process_call(ir_node *call, ir_entity *callee, q_set *hmap) {
  ...
  for (size_t i = n_params; i-- > 0;) {
    ...
    if (is_Const(call_param)) {
       ...
       * we need.*/
      if (!hmap->map) hmap->map = new_pset(entry_cmp, 8);
      ...
      entry_t *const entry =
          (entry_t *)pset_insert(hmap->map, key, hash_entry(key));
      /* Free memory if entry already is in set */
      if (entry != key) obstack_free(&hmap->obst, key);
...
static void copy_nodes(ir_node *irn, void *env) {
  ...
  if (irn != arg) copy_irn_to_irg(irn, clone_irg);
  ...
  if (is_Proj(irn) && get_Proj_pred(irn) == irg_args) {
    ...
    if (get_Proj_num(arg) < proj_nr) set_Proj_num(irn_copy, proj_nr - 1);
  }
}
...
static ir_node *get_irg_arg(ir_graph *irg, size_t pos) {
  ...
  foreach_irn_out_r(irg_args, i, proj) {
    if (pos == get_Proj_num(proj)) {
      if (arg != NULL) panic("multiple projs for the same argument");
      arg = proj;
    }
  }
  ...
  for (size_t i = 0, n = get_compound_n_members(src_frame); i != n; ++i) {
    ...
    if (is_parameter_entity(src_ent)) {
      ...
      if (pos == param_pos) {
        ...
        if (pos > param_pos) set_entity_parameter_number(dst_ent, pos - 1);
      }
    } else {
      dst_ent = clone_entity(src_ent, name, dst_frame);
    }
...
static ir_node *new_cl_Call(ir_node *call, ir_entity *new_entity, size_t pos) {
  ...
  for (size_t i = 0; i < n_params; ++i) {
    if (pos != i) in[new_params++] = get_Call_param(call, i);
  }
...
void proc_cloning(float threshold) {
  ...
  /* We have the "Call" nodes to optimize in set "set_entries". Our algorithm
     replace one constant parameter and make a new "Call" node for all found
     "Calls". It exchange the old one with the new one and the algorithm is
     called with the new "Call".
   */
