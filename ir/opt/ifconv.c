 ...
 * otherwise returns a copy of the passed node created in the i-th predecessor
 * of src_block.
 *
...
static ir_node *copy_or_move(ir_node *node, ir_node *src_block, int i) {
  ...
  if (can_move) {
    ...
    DB((dbg, LEVEL_1,
        "copy_or_move: copying %+F to predecessor %+F, copy is %+F\n", node,
        dst_block, copy));
    return copy;
  }
}
...
/**
 * Remove the j-th predecessor from the i-th predecessor of block and add it to
 * block
 */
static void split_block(ir_node *block, int i, int j) {
  ...
  for (ir_node *next, *phi = get_Block_phis(pred_block); phi != NULL;
       phi = next) {
...
restart:;
  ...
  for (int i = 0; i < arity; ++i) {
    ...
    for (ir_cdep *cdep = find_cdep(pred0); cdep != NULL;
         cdep = get_cdep_next(cdep)) {
      ...
      for (int j = i + 1; j < arity; ++j) {
        ...
        for (ir_node *p = phi; p != NULL; p = get_Phi_next(p)) {
          ...
          if (mux_true == mux_false) continue;
          ir_mode *mode = get_irn_mode(mux_true);
          if (mode == mode_M || !env->allow_ifconv(sel, mux_false, mux_true)) {
            supported = false;
            break;
          }
        }
        ...
        do { /* generate Mux nodes in mux_block for Phis in block */
          ...
          if (val_i == val_j) {
            mux = val_i;
            DB((dbg, LEVEL_2,
                "Generating no Mux for %+F, because both values are equal\n",
                phi));
          } else {
            ir_node *t, *f;
...
void opt_if_conv_cb(ir_graph *irg, arch_allow_ifconv_func callback) {
  ...
  confirm_irg_properties(
      irg, IR_GRAPH_PROPERTY_NO_CRITICAL_EDGES | IR_GRAPH_PROPERTY_ONE_RETURN);
}

void opt_if_conv(ir_graph *irg) { opt_if_conv_cb(irg, ir_target.allow_ifconv); }
