...
static void add_pred(ir_node *node, ir_node *x) {
  ...
  foreach_irn_in(node, i, pred) { ins[i] = pred; }
  ins[n] = x;
...
static ir_node *search_def_and_create_phis(ir_node *block, ir_mode *mode,
  ...
  if (block == ssa_second_def_block && !first) return ssa_second_def;
...
static void copy_and_fix(const jumpthreading_env_t *env, ir_node *block,
  ...
  foreach_out_edge(block, edge) {
    ...
    if (mode == mode_X || is_Cond(node) || is_Switch(node)) continue;
#ifdef AVOID_PHIB
    ...
    if (get_irn_visited(copy) >= env->visited_nr) {
      ...
      if (prev_copy != NULL) set_irn_link(node, prev_copy);
    }
  }
  ...
  for (int i = 0, arity = get_End_n_keepalives(end); i < arity; ++i) {
    ...
    if (get_irn_visited(keep) < env->visited_nr || is_Block(keep)) continue;
    ir_node *copy = get_irn_link(keep);
    /* exact copy does not reproduce the keep alive edges */
    if (is_Phi(copy) && get_Phi_loop(copy)) add_End_keepalive(end, copy);
  }
}
...
static int eval_cmp_tv(ir_relation relation, ir_tarval *tv_left,
  ...
  if (cmp_result == ir_relation_false) return -1;
  if ((cmp_result & relation) != 0) return 1;
...
static int eval_cmp(jumpthreading_env_t *env, ir_node *cand) {
  if (is_Const(cand)) {
    ...
    ir_tarval *res = computed_value_Cmp_Confirm(cand, env->cnst, env->relation);
    if (!tarval_is_constant(res)) return -1;
...
static bool is_Const_or_Confirm(const ir_node *node) {
  if (is_Confirm(node)) node = get_Confirm_bound(node);
...
static ir_node *find_const_or_confirm(jumpthreading_env_t *env, ir_node *jump,
                                      ir_node *value) {
  if (irn_visited_else_mark(value)) return NULL;
...
static ir_node *find_candidate(jumpthreading_env_t *env, ir_node *jump,
  ...
  if (is_Cmp(value)) {
    ...
    if (get_nodes_block(left) != block) return NULL;
    /* negate condition when we're looking for the false block */
    if (env->tv == tarval_b_false) relation = get_negated_relation(relation);
