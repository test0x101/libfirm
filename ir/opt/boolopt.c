 ...
 */
static bool find_cond_pair(ir_node *const l, ir_node *const r,
                           cond_pair *const res) {
  if (is_Cmp(l) && is_Cmp(r)) {
    ...
    if (is_irn_null(lor) && is_irn_null(ror) && pnc_l == pnc_r &&
...
static ir_node *make_Cmp(ir_node *const block, ir_node *const cmp,
...
static ir_node *bool_and(cond_pair *const cpair, ir_node *dst_block) {
  ...
  if (is_Const(cmp_lo)) return is_Const_null(cmp_lo) ? cmp_hi : cmp_lo;

  if (is_Const(cmp_hi)) return is_Const_null(cmp_hi) ? cmp_lo : cmp_hi;
  ...
  if ((rel_lo & ~ir_relation_unordered) == ir_relation_equal &&
      (rel_hi & ~ir_relation_unordered) == rel_lo && tarval_is_null(tv_lo) &&
      tarval_is_null(tv_hi) && mode == get_tarval_mode(tv_hi)) {
    ...
    if (mode_is_reference(mode)) {
      ...
      if (tv_lo == tarval_bad) return NULL;
    }
    if (mode_is_int(mode)) {
  ...
  if (get_Cmp_left(cmp_lo) != get_Cmp_left(cmp_hi)) return NULL;

  /* TODO: for now reject float modes */
  if (!mode_is_int(mode)) return NULL;
  ...
  /* Beware of NaN's, we can only check for (ordered) != here (which is Lg, not
   * Ne) */
  if ((rel_lo == ir_relation_less || rel_lo == ir_relation_less_equal ||
       rel_lo == ir_relation_equal) &&
      (rel_hi == ir_relation_equal || rel_hi == ir_relation_greater_equal ||
       rel_hi == ir_relation_greater)) {
    /* x <|<=|== lo && x ==|>=|> hi ==> false */
    ir_node *const t = new_r_Const(irg, tarval_b_false);
    return t;
             ...
             (rel_hi == ir_relation_less || rel_hi == ir_relation_less_equal ||
              rel_hi == ir_relation_less_greater)) {
    /* x <|<=|== lo && x <|<=|!= hi ==> x <|<=|== lo */
    return cmp_lo;
  } else if ((rel_lo == ir_relation_greater_equal ||
              rel_lo == ir_relation_greater ||
              rel_lo == ir_relation_less_greater) &&
             (rel_hi == ir_relation_equal ||
              rel_hi == ir_relation_greater_equal ||
              rel_hi == ir_relation_greater)) {
    /* x >=|>|!= lo && x ==|>=|> hi ==> x ==|>=|> hi */
    return cmp_hi;
    ...
    } else if (rel_lo == ir_relation_less_greater &&
               rel_hi == ir_relation_less) {
      /* x != c && c < c + 1 ==> x < c */
    ...
    if (rel_lo == ir_relation_greater_equal) {
      ...
      if (mode_is_signed(mode)) {
        ...
        if (tv_lo == tarval_bad || tv_hi == tarval_bad) return NULL;
      }
      c = new_r_Const(irg, tv_lo);
...
static ir_node *bool_or(cond_pair *const cpair, ir_node *dst_block) {
  ...
  /* Beware of NaN's, we can only check for (ordered) != here (which is Lg, not
   * Ne) */
  if ((rel_lo == ir_relation_greater_equal || rel_lo == ir_relation_greater ||
       rel_lo == ir_relation_less_greater) &&
      (rel_hi == ir_relation_less || rel_hi == ir_relation_less_equal ||
       rel_hi == ir_relation_less_greater)) {
    /* x >=|>|!= lo | x <|<=|!= hi ==> true */
    ...
    } else if (rel_lo == ir_relation_equal &&
               rel_hi == ir_relation_greater_equal) {
      /* x == c || x >= c + 1 ==> x >= c */
...
static void bool_walk(ir_node *n, void *ctx) {
  ...
  if (is_And(n)) {
    ...
    ir_node *replacement;
    cond_pair cpair;
    if (!find_cond_pair(l, r, &cpair)) return;
...
static ir_node *skip_empty_blocks(ir_node *node) {
  while (is_Jmp(node)) {
    ...
    if (get_Block_n_cfgpreds(block) != 1) break;

    if (get_Block_mark(block)) break;
...
static int can_fuse_block_inputs(const ir_node *block, int idx_i, int idx_j) {
  ...
  for (phi = get_Block_phis(block); phi != NULL; phi = get_Phi_next(phi)) {
    if (get_Phi_pred(phi, idx_i) != get_Phi_pred(phi, idx_j)) return 0;
  }
  return 1;
}
...
static void remove_block_input(ir_node *block, int idx) {
  ...
  if (n == 1) {
    ...
    for (phi = get_Block_phis(block); phi != NULL; phi = get_Phi_next(phi)) {
      for (i = j = 0; i <= n; ++i) {
        if (i != idx) ins[j++] = get_Phi_pred(phi, i);
      }
      set_irn_in(phi, n, ins);
    }
  }
  for (i = j = 0; i <= n; ++i) {
    if (i != idx) ins[j++] = get_Block_cfgpred(block, i);
  }
...
static void move_nodes_to_block(ir_node *jmp, ir_node *to_block) {
  ...
  if (new_jmp) exchange(jmp, new_jmp);
}

static void normalize_cmp(ir_node **io_cmp, ir_relation *io_rel,
                          ir_tarval **io_tv) {
  ir_node *cmp = *io_cmp;
...
static void find_cf_and_or_walker(ir_node *block, void *ctx) {
   ...
   */
  if (is_Id(block)) return;

  n_cfgpreds = get_Block_n_cfgpreds(block);
restart:
  if (n_cfgpreds < 2) return;
  ...
  for (low_idx = 0; low_idx < n_cfgpreds; ++low_idx) {
    ...
    for (up_idx = 0; up_idx < n_cfgpreds; ++up_idx) {
      ...
      if (is_Bad(upper_cf)) continue;
      upper_block = get_nodes_block(upper_cf);
      if (upper_block != lower_pred) continue;
      if (!block_dominates(upper_block, block)) continue;

      /* we have found the structure */
      /* check Phis: There must be NO Phi in block that
         depends on the existence of low block */
      if (!can_fuse_block_inputs(block, low_idx, up_idx)) continue;

      /* all fine, try it */
      ir_node *const upper_cond = get_Proj_pred(upper_cf);
      if (!is_Cond(upper_cond)) continue;

      ir_node *const upper_cond_selector = get_Cond_selector(upper_cond);
      if (!find_cond_pair(cond_selector, upper_cond_selector, &cpair)) continue;
...
void opt_bool(ir_graph *const irg) {
  ...
  confirm_irg_properties(
      irg, env.changed ? IR_GRAPH_PROPERTIES_NONE : IR_GRAPH_PROPERTIES_ALL);
}
