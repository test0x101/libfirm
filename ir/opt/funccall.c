...
static bool method_type_contains_aggregate(const ir_type *type) {
  for (size_t i = 0, n_params = get_method_n_params(type); i < n_params; ++i) {
    ir_type *param = get_method_param_type(type, i);
    if (is_aggregate_type(param)) return true;
  }
  for (size_t i = 0, n_ress = get_method_n_ress(type); i < n_ress; ++i) {
    ir_type *res = get_method_res_type(type, i);
    if (is_aggregate_type(res)) return true;
  }
  return false;
}
...
static void fix_const_call_lists(ir_graph *irg, ir_node **pure_call_list) {
  ...
  for (size_t i = ARR_LEN(pure_call_list); i-- > 0;) {
    ...
    if (method_type_contains_aggregate(type)) continue;
...
static mtp_additional_properties check_termination(ir_graph *irg) {
  ...
  ir_reserve_resources(irg,
                       IR_RESOURCE_BLOCK_VISITED | IR_RESOURCE_IRN_VISITED);
  ...
  if (!found_return) return mtp_property_noreturn;
  if (!found_loop)
 ...
 */
static mtp_additional_properties follow_mem(
    ir_node *node, const mtp_additional_properties min_prop,
    mtp_additional_properties max_prop) {
  do {
    if (irn_visited_else_mark(node)) return max_prop;
    ...
    switch (get_irn_opcode(node)) {
      ...
      case iro_Sync:
        ...
        foreach_irn_in_r(node, i, pred) {
          ...
          if ((max_prop & ~min_prop) == mtp_no_property) return max_prop;
        }
        return max_prop;
      ...
      case iro_Call: {
        ...
        if ((max_prop & callprops) != max_prop) {
          ir_entity *callee = get_Call_callee(node);
          if (callee == NULL) return mtp_no_property;
          ir_graph *irg = get_entity_linktime_irg(callee);
          if (irg == NULL) return mtp_no_property;
...
static mtp_additional_properties analyze_irg(ir_graph *irg) {
  ...
  /* already checked? */
  if (IS_IRG_READY(irg)) return prop;
  ...
  const mtp_additional_properties min_prop =
      prop &
      (mtp_property_no_write | mtp_property_pure | mtp_property_terminates);
  ...
  if ((max_prop & ~min_prop) == mtp_no_property) goto early_finish;
  ...
  foreach_irn_in(endbl, i, pred) {
    ...
    if ((max_prop & ~min_prop) == mtp_no_property) goto finish;
  }
  ...
  for (int j = get_End_n_keepalives(end); j-- > 0;) {
    ...
    if (get_irn_mode(kept) != mode_M && !is_memop(kept)) continue;
...
static mtp_additional_properties update_property(
    mtp_additional_properties orig_prop, mtp_additional_properties call_prop) {
  ...
  foreach_out_edge(n, edge) {
    ...
    switch (get_irn_opcode(succ)) {
      ...
      case iro_Cmp:
        ...
        break;
      case iro_Store:
        if (get_Store_value(succ) == n) return true;
        /* ok if its only the address input */
        break;
      case iro_Sel:
      case iro_Confirm:
        if (is_stored(succ)) return true;
        break;
      case iro_Call: {
 ...
 */
static mtp_additional_properties check_nothrow_or_malloc(ir_graph *irg,
                                                         bool top) {
  ...
  if (get_method_n_ress(mtp) <= 0) curr_prop &= ~mtp_property_malloc;
    ...
    if (is_Return(pred)) {
      if (curr_prop & mtp_property_malloc) {
        ...
        for (size_t j = get_Return_n_ress(pred); j-- > 0;) {
          ...
          /* skip Proj's */
          while (is_Proj(res)) res = get_Proj_pred(res);
...
void optimize_funccalls(void) {
  ...
  foreach_irp_irg(i, irg) {
    const mtp_additional_properties prop = check_nothrow_or_malloc(irg, true);
  ...
  foreach_irp_irg(i, irg) { analyze_irg(irg); }
  handle_const_Calls();
...
void firm_init_funccalls(void) { FIRM_DBG_REGISTER(dbg, "firm.opt.funccalls"); }
