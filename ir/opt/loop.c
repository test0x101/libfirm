...
#define for_each_phi_safe(block, phi, next)                  \
  for (ir_node *phi = get_Block_phis((block)), *next = NULL; \
       phi ? next = get_Phi_next(phi), true : false; phi = next)

/* Currently processed loop. */
static ir_loop *cur_loop;

/* Flag for kind of unrolling. */
typedef enum unrolling_kind_flag { constant, invariant } unrolling_kind_flag;
...
/* Set stats to sero */
static void reset_stats(void) { memset(&stats, 0, sizeof(loop_stats_t)); }
...
typedef struct loop_info_t {
  ...
  unsigned latest_value : 1; /* 1 if condition is checked against latest counter
                                value */
...
static void get_loop_info(ir_node *const node, void *const env) {
  ...
  if (node_in_loop) {
    switch (get_irn_opcode(node)) {
      ...
      case iro_Phi:
        if (opt_params.count_phi) goto count;
        break;
  ...
  foreach_irn_in_r(node, i, pred) {
    ...
    if (is_Block(node)) {
      ...
      foreach_block_succ(node, edge) {
        ...
        if (is_Block(succ) && is_in_loop(succ)) ++outs_n;
      }
      if (outs_n > 1) ++loop_info.branches;
      ...
      if (node_in_loop && !pred_in_loop && loop_head_valid) {
        ...
        if (!is_in_loop(cfgpred)) {
          DB((dbg, LEVEL_5,
              "potential head %+F because inloop and pred %+F not inloop\n",
              node, pred));
 ...
 */
static ir_node *search_def_and_create_phis(ir_node *const block,
                                           ir_mode *const mode,
                                           int const first) {
  ...
  for (int i = 0; i < n_cfgpreds; ++i) in[i] = new_r_Dummy(irg, mode);
 ...
 */
static void construct_ssa(ir_node *const orig_block, ir_node *const orig_val,
                          ir_node *const second_block,
                          ir_node *const second_val) {
  assert(orig_block && orig_val && second_block && second_val &&
         "no parameter of construct_ssa may be NULL");

  if (orig_val == second_val) return;
  ...
  foreach_out_edge_safe(orig_val, edge) {
    ...
    if (newval != user && !is_Bad(newval)) set_irn_n(user, j, newval);
  }
...
static void reset_block_mark(ir_node *const node, void *const env) {
  ...
  if (is_Block(node)) set_Block_mark(node, 0);
}
...
static void extend_irn(ir_node *const n, ir_node *const newnode,
                       bool const new_is_backedge) {
...
static int get_backedge_n(ir_node *const block, bool const with_alien) {
  ...
  for (int i = 0; i < arity; ++i) {
    ...
    if (is_backedge(block, i) && (with_alien || is_in_loop(pred))) ++be_n;
  }
  return be_n;
}
...
static ir_node *copy_node(ir_node *const node) {
  ...
  /* Keep backedge info */
  for (int i = 0; i < arity; ++i) {
    if (is_backedge(node, i)) set_backedge(cp, i);
  }

  if (is_Block(cp)) set_Block_mark(cp, 0);
 ...
 */
static void copy_walk(ir_node *const node,
                      walker_condition *const walk_condition,
                      ir_loop *const set_loop) {
  /**
  ...
  if (!is_Block(node)) {
    ...
    if (walk_condition(pred)) DB((dbg, LEVEL_5, "walk block %N\n", pred));
  ...
  if (!is_Block(node)) {
    ...
    if (is_Phi(cp)) add_Block_phi(cpblock, cp);
  }
 ...
 */
static void copy_walk_n(ir_node *const node,
                        walker_condition *const walk_condition,
                        int const copy_index) {
  /* break condition and cycle resolver, creating temporary node copies */
  if (irn_visited(node)) {
    /* Here we rely on nodestate's copy being initialized with NULL */
    DB((dbg, LEVEL_5, "copy_walk: We have already visited %N\n", node));
  ...
  if (!is_Block(node)) {
    ...
    if (walk_condition(block)) DB((dbg, LEVEL_5, "walk block %N\n", block));
  ...
  if (is_Phi(node) && get_nodes_block(node) == loop_head) {
    foreach_irn_in(node, i, pred) {
      if (is_own_backedge(loop_head, i) && is_nodes_block_marked(pred)) {
        ...
        DB((dbg, LEVEL_5,
            "Found incc assignment node %N @%d is pred %N, graph %N %N\n", node,
            i, entry.pred, get_irn_irg(node),
...
static void find_condition_chain(ir_node *const block) {
  ...
  /* Get node count */
  unsigned nodes_n = 0;
  foreach_out_edge(block, edge) { ++nodes_n; }
  ...
  foreach_block_succ(block, edge) {
    ir_node *const src = get_edge_src_irn(edge);
    if (is_in_loop(src) && !irn_visited(src)) find_condition_chain(src);
  }
}
 ...
 * Copy of loop_head must have phi list and old (unfixed) backedge info of the
 * loop head. (loop_head is already fixed, we cannot rely on it.)
 */
static void fix_copy_inversion(void) {
  ...
  foreach_irn_in(head_cp, i, pred) {
    if (!is_backedge(head_cp, i)) ins[pos++] = pred;
  }
  ...
  pos = 0;
  for_each_phi_safe(head_cp, phi, next) { exchange(phi, phis[pos++]); }
...
static void fix_head_inversion(void) {
  ...
  foreach_irn_in(loop_head, i, pred) {
    if (is_own_backedge(loop_head, i)) ins[pos++] = pred;
  }
...
static void inversion_walk(ir_graph *const irg,
                           entry_edge *const head_entries) {
  /* The order of rewiring bottom-up is crucial.
   * Any change of the order leads to lost information that would be needed
   * later.
   */
  ...
  for (size_t i = 0; i < ARR_LEN(head_entries); ++i) {
    ...
    /* Ignore keepalives */
    if (is_End(head_out.node)) continue;
...
static void loop_inversion(ir_graph *const irg) {
  ...
  DB((dbg, LEVEL_1,
      ...
      max_loop_nodes, max_loop_nodes_adapted, loop_depth,
      opt_params.depth_adaption));

  if (loop_info.nodes == 0) return;

  if (loop_info.nodes > max_loop_nodes) {
    /* Only for stats */
    DB((dbg, LEVEL_1, "Nodes %d > allowed nodes %d\n", loop_info.nodes,
        loop_depth, max_loop_nodes));
    ++stats.too_large;
    /* no RETURN */
  ...
  if (inversion_blocks_in_cc < 1) {
    do_inversion = false;
    DB((dbg, LEVEL_3,
        "Loop contains %d (less than 1) invertible blocks => No Inversion "
        "done.\n",
        inversion_blocks_in_cc));
  }
...
static void place_copies(int const copies) {
  ...
  for (int c = 0; c < copies; ++c) {
    ...
    ir_node *const topmost_be_block =
        get_nodes_block(get_Block_cfgpred(loophead, be_src_pos));
    ...
    if (loop_info.unroll_kind == constant) {
      ...
      ir_node *proj =
          new_r_Proj(loop_info.duff_cond, mode_X, unroll_nr - c - 1);
      ...
      for_each_phi(loophead, phi) {
        ...
        ir_node *const upper_phi_pred =
            is_in_loop(topmost_phi_pred) ? get_unroll_copy(topmost_phi_pred, c)
                                         : topmost_phi_pred;
  ...
  if (loop_info.unroll_kind == constant) {
    ...
    for_each_phi(loop_head, phi) {
      ...
      ir_node *const last_pred =
          is_in_loop(pred) ? get_unroll_copy(pred, copies) : pred;
...
static void copy_loop(ir_graph *const irg, entry_edge *const cur_loop_outs,
                      int const copies) {
  ir_reserve_resources(irg, IR_RESOURCE_IRN_VISITED);
...
static ir_node *clone_phis_sans_bes(ir_node *const phi, ir_node *const be_block,
                                    ir_node *const dest_block) {
  ...
  foreach_irn_in(phi, i, pred) {
    if (!is_own_backedge(be_block, i)) ins[c++] = pred;
  }
 ...
 * using be_block as supplier of backedge informations. */
static ir_node *clone_block_sans_bes(ir_graph *const irg, ir_node *const node,
                                     ir_node *const be_block) {
...
static void create_duffs_block(ir_graph *const irg) {
  ...
  ir_node *const ems =
      new_r_Sub(block1, loop_info.end_val, loop_info.start_val);
  ...
  ir_node *const count_block =
      new_r_Block(irg, ARRAY_SIZE(count_block_ins), count_block_ins);
  ...
  ir_node *const correction =
      new_r_Phi(count_block, ARRAY_SIZE(correction_ins), correction_ins, mode);
  ...
  ir_relation const rel =
      loop_info.decreasing == 1 ? ir_relation_less : ir_relation_greater;
  ...
  ir_node *const duff_block =
      new_r_Block(irg, ARRAY_SIZE(duff_block_ins), duff_block_ins);
  ...
  ir_node *const count_phi =
      new_r_Phi(duff_block, ARRAY_SIZE(count_phi_ins), count_phi_ins, mode);

  /* count % unroll_nr */
  ir_node *const unroll_c = new_r_Const_long(irg, mode, (long)unroll_nr);
  ir_node *const duff_mod =
      new_r_Mod(duff_block, nomem, count_phi, unroll_c, true);
...
static unsigned is_loop_invariant_def(ir_node *const node) {
  ...
  if (is_Phi(node)) {
    ...
    if (block != loop_head) return 0;

    foreach_irn_in(node, i, pred) {
      /* Check if all bes are just loopbacks. */
      if (is_own_backedge(block, i) && pred != node) return 0;
    }
...
static unsigned get_invariant_pred(ir_node *const node,
                                   ir_node **const invar_pred,
                                   ir_node **const other) {
...
static unsigned get_start_and_add(unrolling_kind_flag const role) {
  ...
  foreach_irn_in(iteration_phi, i, found_start_val) {
    ...
    if (!is_backedge(block, i)) {
      ...
      switch (role) {
        case constant:
          if (!is_const(found_start_val)) return 0;
          break;
        case invariant:
          if (!is_loop_invariant_def(found_start_val)) return 0;
          break;
      }
     ...
     * Follow all own backedges. Every value supplied from these preds of the
     * phi needs to origin from the same add. */
    if (is_own_backedge(block, i)) {
      DB((dbg, LEVEL_4, "is add? %N\n", found_start_val));

      if (!is_Add(found_start_val) && !is_Sub(found_start_val)) return 0;
      if (found_add && found_add != found_start_val) return 0;
...
static unsigned get_const_pred(ir_node *const node, ir_node **const const_pred,
                               ir_node **const other) {
...
static unsigned simulate_next(ir_tarval **const count_tar,
                              ir_tarval *const stepped,
                              ir_tarval *const step_tar,
                              ir_tarval *const end_tar,
                              ir_relation const norm_proj) {
  DB((dbg, LEVEL_4, "Loop taken if (stepped)%ld %s (end)%ld ",
      get_tarval_long(stepped), get_relation_string((norm_proj)),
  ...
  if (!(tarval_cmp(stepped, end_tar) & norm_proj)) return 1;
  ...
  /* next step */
  ir_tarval *const next = is_Add(loop_info.add) ? tarval_add(stepped, step_tar)
                                                : tarval_sub(stepped, step_tar);

  DB((dbg, LEVEL_4, "Loop taken if %ld %s %ld ", get_tarval_long(next),
      get_relation_string(norm_proj), get_tarval_long(end_tar)));
  DB((dbg, LEVEL_4, "comparing latest value %d\n", loop_info.latest_value));

  /* Increase steps. */
  *count_tar =
      tarval_add(*count_tar, get_mode_one(get_tarval_mode(*count_tar)));
...
static ir_node *is_simple_loop(void) {
  ...
  if (loop_info.cf_outs != 1) return NULL;

  DB((dbg, LEVEL_4, "1 loop exit\n"));

  /* Calculate maximum unroll_nr keeping node count below limit. */
  loop_info.max_unroll = (int)((double)opt_params.max_unrolled_loop_size /
                               (double)loop_info.nodes);
  ...
  if (exit_block != loop_block) return NULL;
...
static unsigned are_mode_I(ir_node *const n1, ir_node *const n2,
                           ir_node *const n3) {
  ir_mode *const m1 = get_irn_mode(n1);
  ir_mode *const m2 = get_irn_mode(n2);
  ir_mode *const m3 = get_irn_mode(n3);

  if (m1 != m2 || m1 != m3) return 0;
  if (m1 != mode_Iu && m1 != mode_Is) return 0;
  return 1;
}
...
static unsigned get_unroll_decision_invariant(ir_graph *const irg) {
  ...
  if (!loop_condition) return 0;

  /* Use a minimal size for the invariant unrolled loop,
   * as duffs device produces overhead */
  if (loop_info.nodes < opt_params.invar_unrolling_min_size) return 0;

  ir_node *iteration_path;
  unsigned const success =
      get_invariant_pred(loop_condition, &loop_info.end_val, &iteration_path);
  ...
  if (is_Add(iteration_path) || is_Sub(iteration_path)) {
    ...
    if (!get_const_pred(loop_info.add, &loop_info.step,
                        &loop_info.iteration_phi))
      return 0;

    DB((dbg, LEVEL_4, "Got step %N\n", loop_info.step));

    if (!is_Phi(loop_info.iteration_phi)) return 0;

    DB((dbg, LEVEL_4, "Got phi %N\n", loop_info.iteration_phi));

    /* Find start_val.
     * Does necessary sanity check of add, if it is already set.  */
    if (!get_start_and_add(invariant)) return 0;
    ...
    if (loop_info.iteration_phi != new_iteration_phi) return 0;
  } else {
    return 0;
  }

  DB((dbg, LEVEL_4, "start %N, end %N, step %N\n", loop_info.start_val,
      loop_info.end_val, loop_info.step));

  ir_mode *const mode = get_irn_mode(loop_info.end_val);
  if (mode != mode_Is && mode != mode_Iu) return 0;
...
static unsigned get_preferred_factor_constant(ir_tarval *const count_tar) {
   ...
   * we either might reduce the preferred factor and prevent a duffs device
   * block, or create a duffs device block, from which in this case (constants
   * only) we know the startloop at compiletime. The latter yields the following
   * graphs. but for code generation we would want to use graph A. The graphs
   * are equivalent. So, we can only reduce the preferred factor. A) B) PreHead
   * PreHead |      ,--.         |   ,--.
   *         \ Loop1   \        Loop2   \
   *          \  |     |       /  |     |
   *           Loop2   /      / Loop1   /
...
static unsigned get_unroll_decision_constant(void) {
  ...
  ir_node *iteration_path;
  if (!get_const_pred(cmp, &loop_info.end_val, &iteration_path)) return 0;
  ...
  if (is_Add(iteration_path) || is_Sub(iteration_path)) {
    ...
    /* Find start_val.
     * Does necessary sanity check of add, if it is already set.  */
    if (!get_start_and_add(constant)) return 0;
  ...
  if (tarval_is_null(step_tar)) /* TODO Might be worth a warning. */
    return 0;
  ...
  ir_relation const norm_relation =
      loop_info.exit_cond == 1 ? get_negated_relation(relation) : relation;
  ...
  if (norm_relation == ir_relation_equal) /* TODO Might be worth a warning. */
    return 0;
...
static void unroll_loop(ir_graph *const irg) {
  ...
  if (unroll_nr > 1) {
    ...
    if (unroll_nr > 1) loop_info.unroll_kind = invariant;
  }
...
static void find_innermost_loop(ir_loop *const loop) {
  ...
  if (!had_sons) ARR_APP1(ir_loop *, loops, loop);
}
...
static void loop_optimization(ir_graph *const irg, loop_op_t const loop_op) {
  ...
  for (size_t i = 0; i < n_elements; ++i) {
    ...
    if (*element.kind != k_ir_loop) continue;
    find_innermost_loop(element.son);
  }
...
void firm_init_loop_opt(void) { FIRM_DBG_REGISTER(dbg, "firm.opt.loop"); }
