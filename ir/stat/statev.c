...
static void stat_ev_vprintf(char ev, const char *key, const char *fmt,
                            va_list ap) {
...
void(stat_ev_ctx_pop)(const char *key) { stat_ev_ctx_pop_(key); }
...
void(stat_ev_dbl)(const char *name, double value) { stat_ev_dbl_(name, value); }
...
void(stat_ev_int)(const char *name, int value) { stat_ev_int_(name, value); }
...
void(stat_ev)(const char *name) { stat_ev_(name); }
...
void stat_ev_begin(const char *prefix, const char *filt) {
  ...
  if (filt != NULL && filt[0] != '\0') {
    ...
    if (regcomp(&regex, filt, REG_EXTENDED) == 0) {
      ...
      fprintf(stderr, "Warning: Couldn't parse statev filter expression '%s'\n",
              filt);
    }
  }
