...
static void collect_needs_lowering(ir_node *node, void *env) {
  ...
  if (get_irn_mode(node) == mode_b) {
    assert(is_And(node) || is_Or(node) || is_Eor(node) || is_Phi(node) ||
           is_Not(node) || is_Mux(node) || is_Cmp(node) || is_Const(node) ||
           is_Unknown(node) || is_Bad(node));
    return;
  }

  foreach_irn_in(node, i, in) {
    if (get_irn_mode(in) != mode_b) continue;
    if (is_Cmp(in) && needs_mode_b_input(node, i)) continue;
...
void ir_lower_mode_b(ir_graph *const irg, ir_mode *const nlowered_mode) {
  ...
  for (size_t i = 0; i < n; ++i) {
    ...
    if (needs_mode_b_input(node, input)) lowered = convert_to_modeb(lowered);
  ...
  confirm_irg_properties(
      irg, n > 0 ? IR_GRAPH_PROPERTIES_NONE : IR_GRAPH_PROPERTIES_ALL);
}
