...
static void lower_small_copyb_node(ir_node *irn) {
  ...
  unsigned mode_bytes =
      allow_misalignments ? native_mode_bytes : get_type_alignment(tp);
  ...
  while (offset < size) {
    ...
    for (; offset + mode_bytes <= size; offset += mode_bytes) {
      ...
      ir_node *store =
          new_rd_Store(dbgi, block, load_mem, add2, load_res, tp, flags);
...
static void lower_large_copyb_node(ir_node *irn) {
  ...
  ir_node *call =
      new_rd_Call(dbgi, block, mem, callee, ARRAY_SIZE(in), in, call_tp);
...
static void find_copyb_nodes(ir_node *irn, void *ctx) {
  if (!is_CopyB(irn)) return;

  ir_type *tp = get_CopyB_type(irn);
  if (get_type_state(tp) != layout_fixed) return;

  unsigned size = get_type_size(tp);
  bool medium_sized = max_small_size < size && size < min_large_size;
  if (medium_sized) return; /* Nothing to do for medium-sized CopyBs. */
