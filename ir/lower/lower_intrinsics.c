...
ir_intrinsics_map *ir_create_intrinsics_map(i_record *list, size_t length,
  ...
  for (size_t i = 0; i < length; ++i) {
    if (list[i].kind == INTRINSIC_CALL) {
      pmap_insert(c_map, list[i].i_call.i_ent, (void *)list[i].i_call.i_mapper);
    } else {
 ...
 * @param exc_jmp  new exception control flow, if reg_jmp == NULL, a Bad will be
 * used
 */
...
int i_mapper_abs(ir_node *call) {
  ...
  if (!allow_ifconv(cmp, op, minus_op)) return 0;
...
int i_mapper_sqrt(ir_node *call) {
  ...
  if (!is_Const(op)) return 0;

  tv = get_Const_tarval(op);
  if (!tarval_is_null(tv) && !tarval_is_one(tv)) return 0;
...
int i_mapper_exp2(ir_node *call) { return i_mapper_exp(call); }

int i_mapper_exp10(ir_node *call) { return i_mapper_exp(call); }
...
static ir_tarval *get_initializer_value(ir_initializer_t *const init,
                                        ir_mode *const mode) {
  switch (get_initializer_kind(init)) {
    ...
    case IR_INITIALIZER_CONST: {
      ...
      if (is_Const(irn)) return get_Const_tarval(irn);
      break;
    }
...
static bool initializer_val_is_null(ir_initializer_t *init) {
  ...
  if (get_initializer_kind(init) == IR_INITIALIZER_NULL) return true;
...
static ir_node *eval_strlen(ir_graph *irg, ir_entity *ent, ir_type *res_tp) {
  ...
  if (!is_Primitive_type(tp)) return NULL;
  mode = get_type_mode(tp);

  /* FIXME: This is too restrict, as the type char might be more the 8bits */
  if (!mode_is_int(mode) || get_mode_size_bits(mode) != 8) return NULL;

  initializer = get_entity_initializer(ent);
  if (get_initializer_kind(initializer) != IR_INITIALIZER_COMPOUND) return NULL;
...
static ir_node *eval_strcmp(ir_graph *irg, ir_entity *left, ir_entity *right,
  ...
  for (i = 0; i != size; ++i) {
    ...
    if (!tarval_is_constant(tv_l) || !tarval_is_constant(tv_r)) return NULL;
...
static int is_empty_string(ir_entity *ent) {
  ...
  if (!is_Array_type(tp)) return 0;
  tp = get_array_element_type(tp);
  if (!is_Primitive_type(tp)) return 0;
  mode = get_type_mode(tp);

  /* FIXME: This is too restrict, as the type char might be more the 8bits */
  if (!mode_is_int(mode) || get_mode_size_bits(mode) != 8) return 0;

  initializer = get_entity_initializer(ent);
  if (get_initializer_kind(initializer) != IR_INITIALIZER_COMPOUND) return 0;

  if (get_initializer_compound_n_entries(initializer) < 1) return 0;
...
int i_mapper_strcmp(ir_node *call) {
  ...
  if (!is_Primitive_type(res_tp)) return 0;
  char_tp = get_method_param_type(call_tp, 0);
  if (char_tp != get_method_param_type(call_tp, 1)) return 0;
  if (!is_Pointer_type(char_tp)) return 0;
