...
static void fix_parameter_entities(ir_graph *irg, unsigned *arg_map) {
  ...
  for (size_t i = 0; i < n_members; ++i) {
    ...
    if (num == IR_VA_START_PARAMETER_NUMBER) continue;
    set_entity_parameter_number(member, arg_map[num]);
  }
}

static void remove_compound_param_entities(ir_graph *irg, unsigned *arg_map,
                                           mtp_info_t *mtp_info) {
  ir_type *frame_type = get_irg_frame_type(irg);
  size_t n_members = get_compound_n_members(frame_type);
  ...
  for (size_t i = n_members; i-- > 0;) {
    ...
    if (is_aggregate_type(type) && how_to_pass(&spec) != PASS_ON_STACK) {
...
static void grow_aggregate_spec(struct obstack *obst,
                                aggregate_spec_t const *const spec) {
...
static ir_type *lower_mtp(lowering_env_t const *const env, ir_type *mtp) {
  if (!is_Method_type(mtp)) return mtp;
  ...
  ir_type **params =
      ALLOCANZ(ir_type *, n_params * MAX_REGS_PER_AGGREGATE + n_ress);
  ...
  for (size_t i = 0; i < n_ress; ++i) {
    ...
    aggregate_spec_t const spec =
        env->lower_return(env->lower_return_env, res_tp);
    ...
    if (is_aggregate_type(res_tp)) {
      switch (how_to_pass(&spec)) {
        ...
        case PASS_AS_POINTER: {
          ...
          if (env->flags & LF_RETURN_HIDDEN) results[nn_ress++] = ptr_tp;
          break;
        }
      }
    } else {
  ...
  for (size_t i = 0; i < nn_params; i++) {
    ...
    aggregate_spec_t const spec =
        env->lower_parameter(env->lower_parameter_env, ptr_tp);
    grow_aggregate_spec(obst, &spec);
  }
  ...
  for (size_t i = 0; i < n_params; ++i) {
    ...
    aggregate_spec_t const spec =
        env->lower_parameter(env->lower_parameter_env, param_tp);
    DBG((dbg, LEVEL_1, "Aggregate spec for %+F: length=%d, %+F, %+F\n",
         param_tp, spec.length, spec.modes[0],
         spec.length > 1 ? spec.modes[1] : NULL));
    grow_aggregate_spec(obst, &spec);
  ...
  if (nn_params > n_params) cconv |= cc_compound_ret;
  ...
  lowered =
      new_type_method(nn_params, nn_ress, is_variadic, cconv, mtp_properties);
...
typedef struct wlk_env {
  ...
  ir_type
      *lowered_mtp; /**< The lowered method type of the current irg if any. */
...
static void check_ptr(ir_node *ptr, wlk_env *env) {
  ...
  ir_storage_class_class_t sc = get_base_sc(classify_pointer(ptr, base_ptr));
...
static bool is_self_recursive_Call(const ir_node *call) {
  ...
  if (callee != NULL) {
    ...
    if (irg == get_irn_irg(call)) return true;
  }
  return false;
}
  ...
  switch (get_irn_opcode(n)) {
    ...
    case iro_CopyB: {
      ...
      if (env->only_local_mem) {
        ...
        if (env->only_local_mem) check_ptr(get_CopyB_dst(n), env);
      }
      /* check for compound returns */
      if (is_Proj(src)) {
        ...
        if (is_Proj(proj) && get_Proj_num(proj) == pn_Call_T_result) {
          ...
          if (is_Call(call)) {
            ...
            if (is_aggregate_type(
                    get_method_res_type(ctp, get_Proj_num(src)))) {
    ...
    case iro_Member: {
      ...
      if (!is_parameter_entity(entity)) break;
...
static void get_dest_addrs(const cl_entry *entry, ir_node **ins,
  ...
  for (ir_node *next, *copyb = entry->copyb; copyb != NULL; copyb = next) {
    ...
    if (!block_dominates(dst_block, call_block)) continue;
    ...
    if (dst_block == call_block) {
       ...
       * call. */
      if (heights_reachable_in_block(heights, dst, call)) continue;
    }

    ir_graph *irg = get_irn_irg(dst);
    ir_node *frame = get_irg_frame(irg);
    if (!is_Member(dst) || get_Member_ptr(dst) != frame) continue;

    ir_entity *dst_ent = get_Member_entity(dst);
    if (get_entity_usage(dst_ent) & ir_usage_address_taken) continue;
  ...
  if (n_args < n_compound_ret) {
    for (size_t i = 0, j = 0, n_ress = get_method_n_ress(orig_ctp); i < n_ress;
         ++i) {
...
static void fix_int_return(cl_entry const *const entry, ir_type *const type,
  ...
  if (proj_mem == NULL) proj_mem = new_r_Proj(call, mode_M, pn_Call_M);
  ir_node *proj_res = entry->proj_res;
  if (proj_res == NULL) proj_res = new_r_Proj(call, mode_T, pn_Call_T_result);
  /* reroute old users */
  ir_node *const res_user = get_Proj_for_pn(proj_res, orig_pn);
  if (res_user != NULL) edges_reroute(res_user, base_addr);
  ...
  for (unsigned i = 0; i < length; ++i) {
    ...
    while (spec_size > 0 && type_size > 0) {
      ...
      // The value was classified as SSE, so we need a Lu bitcast before conv to
      // the target mode
      ...
      if (shift_offset > 0) {
        ...
        ir_node *const shr_count =
            new_r_Const_long(irg, const_mode, shift_offset);
      ...
      if (offset > 0) {
        addr = new_r_Add(block, base_addr,
                         new_r_Const_long(irg, mode_offset, offset));
      }

      ir_type *const type = get_type_for_mode(mode);
      ir_node *const store =
          new_r_Store(block, proj_mem, addr, node_value, type, cons_none);
...
static void fix_call_compound_ret(const cl_entry *entry,
  ...
  for (size_t i = 0, c = 0, n_ress = get_method_n_ress(orig_ctp); i < n_ress;
       ++i) {
    ...
    switch (how_to_pass(&ret_spec)) {
      ...
      case PASS_AS_POINTER:
        ...
        if (env->flags & LF_RETURN_HIDDEN) ++pn;
        break;
    }
  }
...
static ir_node *get_compound_slice(ir_node *block, ir_node *ptr, int offset,
                                   ir_type *compound_type,
                                   ir_type *lower_param_type, ir_node **mem) {
  ir_graph *irg = get_irn_irg(block);
  ...
  ir_node *load =
      new_rd_Load(dbgi, block, *mem, ptr, mode, compound_type, cons_none);
  *mem = new_rd_Proj(dbgi, load, mode_M, pn_Load_M);
  return new_rd_Proj(dbgi, load, mode, pn_Load_res);
}

static void fix_call_compound_params(const cl_entry *entry,
                                     const ir_type *ctp) {
  ir_node *call = entry->call;
    ...
    switch (how_to_pass(&spec)) {
      case PASS_AS_VALUES: {
        ...
        for (size_t v = 0; v < spec.length; v++) {
          ir_type *lower_arg_type =
              get_method_param_type(lower, INPUT_TO_PARAM(i));
          new_in[i++] = get_compound_slice(block, arg, offset, arg_type,
                                           lower_arg_type, &mem);
...
static void fix_calls(wlk_env *env) {
  for (const cl_entry *entry = env->cl_list; entry; entry = entry->next) {
    if (!entry->has_compound_param && entry->n_compound_ret == 0) continue;
    ir_node *call = entry->call;
    ir_type *ctp = get_Call_type(call);
...
static void transform_return(ir_node *ret, size_t n_ret_com, wlk_env *env) {
  ...
  for (size_t i = 0, k = 0; i < n_ress; ++i) {
    ...
    if (how_to_pass(&ret_spec) == PASS_AS_VALUES) {
      ...
      if (is_Unknown(pred)) {
        ...
        for (unsigned i = 0; i < length; ++i) {
          ...
          ir_node *const load =
              new_r_Load(block, mem, addr, mode, type, cons_none);
    ...
    if (env->flags & LF_RETURN_HIDDEN) new_in[n_in++] = arg;
  ...
  if (n_cr_opt > 0) {
    ...
    for (size_t c = 0; c < n_cr_opt; ++c) free_entity(cr_opt[c].ent);
  }
...
static ir_node *build_compound_from_arguments(ir_node *irn, wlk_env *env,
                                              unsigned pn, ir_type *tp,
  ...
  for (unsigned i = 0; i < spec->length; i++) {
    ...
    while (reg_offset < arg_size && offset < tp_size) {
      ...
      if (reg_offset != 0) {
        ir_node *shift_amount =
            new_rd_Const_long(dbgi, irg, arg_mode, reg_offset * 8);
      ...
      ir_node *store =
          new_rd_Store(dbgi, block, mem, ptr, value, tp, cons_none);
 ...
 * parameters.
 */
static void transform_irg(lowering_env_t const *const env,
                          ir_graph *const irg) {
  ...
  for (size_t i = 0; i < n_ress; ++i) {
    ...
    if (is_aggregate_type(type)) {
      ...
      if (how_to_pass(&ret_spec) == PASS_AS_POINTER) ++arg;
    }
  }
  for (size_t i = 0; i < n_params; ++i) {
    arg_map[i] = arg;
  ...
  for (size_t i = 0, n = ARR_LEN(walk_env.param_members); i < n; ++i) {
    ...
    aggregate_spec_t spec =
        mtp_info->parameter_specs[h_num + mtp_info->n_hidden];
  ...
  if (n_param_com > 0) remove_compound_param_entities(irg, arg_map, mtp_info);
  ...
  if (walk_env.heights != NULL) heights_free(walk_env.heights);
...
void lower_calls_with_compounds(compound_call_lowering_flags flags,
  ...
  foreach_irp_irg(i, irg) { transform_irg(&env, irg); }
