...
static void widen_builtin(ir_node *node) {
  ...
  if (get_type_size(arg1) >= ir_target_pointer_size()) return;

  // Only touch builtins with no 32-bit version.
  ir_builtin_kind kind = get_Builtin_kind(node);
  if (kind != ir_bk_clz && kind != ir_bk_ctz && kind != ir_bk_ffs &&
...
static void replace_with_call(ir_node *node) {
  ...
  ir_entity *const entity = create_compilerlib_entity(get_id_str(id), mtp);
  ...
  ir_node *const call =
      new_rd_Call(dbgi, block, mem, callee, n_params, params, mtp);
...
static void replace_may_alias(ir_node *node) {
  ...
  ir_graph *const irg = get_irn_irg(node);
  ir_node *const result =
      (alias != ir_no_alias ? new_r_Const_one : new_r_Const_null)(irg, rmode);
  ir_node *const in[] = {
...
static void lower_builtin(ir_node *node, void *env) {
  ...
  if (!is_Builtin(node)) return;

  ir_builtin_kind const kind = get_Builtin_kind(node);
  if (dont_lower[kind]) return;
...
static void irg_walk_builtin_nodes_post(ir_graph *irg, irg_walk_func *post,
                                        void *env) {
