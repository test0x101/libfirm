...
static void lower_sel(ir_node *sel) {
  ...
  if (element_size == 0) {
    ...
    if (element_size == 1) {
      ...
      ir_node *const el_size =
          new_rd_Const_long(dbg, irg, offset_mode, element_size);
...
static void lower_member(ir_node *member) {
  ...
  if (get_type_state(owner) != layout_fixed) return;
  ...
  if (offset != 0) {
    ...
    ir_node *const cnst = new_r_Const_long(irg, mode_offset, offset);
 ...
 */
void lower_const_code(void) { walk_const_code(NULL, lower_irnode, NULL); }

void lower_highlevel() {
  foreach_irp_irg(i, irg) { lower_highlevel_graph(irg); }
  lower_const_code();
}
