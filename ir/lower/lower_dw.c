...
static bool type_needs_lowering(ir_type const *const tp) {
  if (is_Primitive_type(tp)) {
    ...
    if (needs_lowering(mode)) return true;
  }
  return false;
}

static void lower_mode(void (*const set)(ir_type *, size_t, ir_type *),
                       ir_type *const mtd, ir_mode *const mode) {
  size_t n = 0;
...
static ir_type *get_conv_type(ir_mode *imode, ir_mode *omode) {
  ...
  size_t n_param = 1;
  size_t n_res = 1;
  if (needs_lowering(imode)) n_param = 2;
  if (needs_lowering(omode)) n_res = 2;
...
static void add_block_cf_input_nr(ir_node *block, int nr, ir_node *cf) {
  ...
  for (int i = 0; i < n_cfgpreds; ++i) in[i] = get_Block_cfgpred(block, i);
  in[n_cfgpreds] = cf;
  ...
  foreach_out_edge(block, edge) {
    ...
    foreach_irn_in(phi, i, pred) in[i] = pred;
    in[n_cfgpreds] = in[nr];
...
static void prepare_links(ir_node *node, void *data) {
  ...
  if (needs_lowering(mode)) {
    ...
    for (size_t i = 0, n_res = get_method_n_ress(tp); i < n_res; ++i) {
      ...
      if (type_needs_lowering(rtp)) env.flags |= MUST_BE_LOWERED;
    }
  }
}
...
ir_mode *ir_get_low_unsigned_mode(void) { return env.p.word_unsigned; }
...
static void lower_Store(ir_node *node) {
  ...
  high = new_rd_Store(dbg, block, proj_m, high, entry->high_word, type,
                      volatility);
...
static ir_entity *create_libgcc_entity(ir_type *const method,
                                       ir_op const *const op,
                                       ir_mode const *const imode,
                                       ir_mode const *const omode) {
  const char *name;
  if (op == op_Mul) {
    name = "__muldi3";
  } else if (op == op_Div) {
...
static void move(ir_node *node, ir_node *from_bl, ir_node *to_bl) {
  ...
  foreach_irn_in(node, i, pred) {
    ...
    if (get_nodes_block(pred) == from_bl) move(pred, from_bl, to_bl);
    if (needs_lowering(pred_mode)) {
      ir_node *pred_low = get_lowered_low(pred);
      ir_node *pred_high = get_lowered_high(pred);
      if (get_nodes_block(pred_low) == from_bl) move(pred_low, from_bl, to_bl);
...
void set_dw_control_flow_changed(void) { env.flags |= CF_CHANGED; }

typedef ir_node *(*new_rd_shr_func)(dbg_info *dbgi, ir_node *block,
                                    ir_node *left, ir_node *right);

static void lower_shr_helper(ir_node *const node,
                             new_rd_shr_func const new_rd_shrs) {
  ir_node *right = get_binop_right(node);
  ir_node *left = get_binop_left(node);
  ...
  ir_node *tconv = create_conv(block_true, left_high, low_unsigned);
  ...
  ir_node *phi_low =
      new_r_Phi(lower_block, ARRAY_SIZE(phi_low_in), phi_low_in, low_unsigned);
  ir_node *phi_high =
      new_r_Phi(lower_block, ARRAY_SIZE(phi_high_in), phi_high_in, mode);
...
static void lower_Minus(ir_node *const node) {
  ...
  ir_node *const cmp_carry =
      new_rd_Cmp(dbgi, block, res_low, zero_low, ir_relation_greater);
...
static void lower_binop_additive(ir_node *const node,
                                 ir_node *(*const cons)(dbg_info *, ir_node *,
                                                        ir_node *, ir_node *),
                                 ir_relation const relation) {
  ...
  ir_node *const res_low =
      cons(dbg, block, left_entry->low_word, right_entry->low_word);
  ir_node *const cmp_carry =
      new_rd_Cmp(dbg, block, res_low, left_entry->low_word, relation);
  ir_graph *const irg = get_irn_irg(node);
  ir_mode *const mode = get_node_high_mode(node);
  ir_node *const one = new_r_Const(irg, get_mode_one(mode));
  ir_node *const zero = new_r_Const(irg, get_mode_null(mode));
  ir_node *const carry = new_rd_Mux(dbg, block, cmp_carry, zero, one);
  ir_node *const sum_high =
      cons(dbg, block, left_entry->high_word, right_entry->high_word);
 ...
 */
static void lower_binop_logical(ir_node *const node,
                                ir_node *(*const constr_rd)(dbg_info *db,
                                                            ir_node *block,
                                                            ir_node *op1,
                                                            ir_node *op2)) {
  ir_node *left = get_binop_left(node);
...
static void lower_Cond(ir_node *const node) {
  ...
  ir_node *irn = new_rd_Cmp(dbg, block, lentry->high_word, rentry->high_word,
  ...
  irn = new_rd_Cmp(dbg, newbl_l, lentry->low_word, rentry->low_word, relation);
  dbg = get_irn_dbg_info(node);
...
static void lower_Conv_to_Ll(ir_node *node) {
  ...
  if (get_mode_arithmetic(imode) == irma_twos_complement) {
    if (needs_lowering(imode)) {
      ...
      if (imode != low_unsigned) op = new_rd_Conv(dbg, block, op, low_unsigned);
...
static void lower_Conv_from_Ll(ir_node *node) {
  ...
  if (get_mode_arithmetic(omode) == irma_twos_complement) {
    ...
    ir_mode *imode = get_irn_mode(op);
    ir_type *mtp = get_conv_type(imode, omode);
    ir_node *adr = get_intrinsic_address(mtp, get_irn_op(node), imode, omode);
...
static void lower_Cmp(ir_node *const cmp) {
  ...
  if (!needs_lowering(cmp_mode)) return;
  ...
  /* a rel b <==> a_h REL b_h || (a_h == b_h && a_l rel b_l) */
  ir_node *high1 = new_rd_Cmp(dbg, block, lentry->high_word, rentry->high_word,
                              relation & ~ir_relation_equal);
  ir_node *low =
      new_rd_Cmp(dbg, block, lentry->low_word, rentry->low_word, relation);
  ir_node *high = new_rd_Cmp(dbg, block, lentry->high_word, rentry->high_word,
                             ir_relation_equal);
...
transform:
  ...
  ir_entity *entity = new_entity(frame_type, id, get_type_for_mode(other_mode));
  ir_node *block = get_nodes_block(node);
  ...
  if (needs_lowering(src_mode)) {
    ...
    ir_node *store0 = new_rd_Store(dbgi, block, nomem, high, entry->high_word,
                                   src_type, cons_floats);
    ir_node *mem0 = new_r_Proj(store0, mode_M, pn_Store_M);
    ir_node *store1 = new_rd_Store(dbgi, block, nomem, low, entry->low_word,
                                   src_type, cons_floats);
    ...
    ir_node *load =
        new_rd_Load(dbgi, block, sync, addr, dst_mode, src_type, cons_floats);
    ir_node *res = new_r_Proj(load, dst_mode, pn_Load_res);
    exchange(node, res);
  } else {
    assert(needs_lowering(dst_mode));
    ir_node *store =
        new_rd_Store(dbgi, block, nomem, addr, op, src_type, cons_floats);
    ir_node *mem = new_r_Proj(store, mode_M, pn_Store_M);
    ir_node *load_low = new_rd_Load(dbgi, block, mem, low, env.p.word_unsigned,
                                    src_type, cons_floats);
    ir_node *res_low = new_r_Proj(load_low, env.p.word_unsigned, pn_Load_res);
    ir_mode *mode = get_node_high_mode(node);
    ir_node *load_high =
        new_rd_Load(dbgi, block, mem, high, mode, src_type, cons_floats);
...
static void fix_parameter_entities(ir_graph *irg, ir_type *orig_mtp) {
  ...
  for (i = 0; i < n; ++i) {
    ...
    size_t p;
    if (!is_parameter_entity(entity)) continue;
    p = get_entity_parameter_number(entity);
    if (p == IR_VA_START_PARAMETER_NUMBER) continue;
  ...
  for (i = 0; i < orig_n_params; ++i, ++n_param) {
    ...
    if (entity != NULL) set_entity_parameter_number(entity, n_param);
...
static size_t lower_type(void (*const set)(ir_type *, size_t, ir_type *),
                         ir_type *const mtp, size_t n, ir_type *const tp) {
  if (is_Primitive_type(tp)) {
...
static ir_type *lower_mtp(ir_type *mtp) {
  ...
  if (type_visited(mtp)) return mtp;
...
static void lower_Return(ir_node *const node) {
  ...
  for (int i = 0, n = get_Return_n_ress(node); i < n; ++i) {
    ...
    if (needs_lowering(rmode)) need_conv = true;
  }
  if (!need_conv) return;
...
static void lower_types_builtin(size_t const n, ir_type *const res,
                                ir_type *const mtp, ir_type *const tp_s,
                                ir_type *const tp_u, get_type *const get,
                                set_type *const set) {
...
static ir_type *lower_Builtin_type(pmap *const type_map, ir_type *const mtp,
                                   ir_type *const tp_s_l, ir_type *const tp_u) {
  ...
  /* set param types and result types */
  lower_types_builtin(n_params, res, mtp, tp_s_l, tp_u, &get_method_param_type,
                      &set_method_param_type);
  lower_types_builtin(n_results, res, mtp, tp_s_l, tp_u, &get_method_res_type,
                      &set_method_res_type);
...
static void lower_reduce_builtin(ir_node *const builtin) {
  ...
  if (!needs_lowering(operand_mode)) return;
  ...
  switch (kind) {
    case ir_bk_ffs: {
      ir_node *number_of_bits = new_r_Const_long(
          irg, result_mode, get_mode_size_bits(env.p.word_unsigned));
      ir_node *zero_high = new_rd_Const_null(dbgi, irg, high_mode);
      ir_node *zero_unsigned =
          new_rd_Const_null(dbgi, irg, env.p.word_unsigned);
      ir_node *zero_result = new_rd_Const_null(dbgi, irg, result_mode);
      ir_node *cmp_low = new_rd_Cmp(dbgi, block, entry->low_word, zero_unsigned,
                                    ir_relation_equal);
      ir_node *cmp_high = new_rd_Cmp(dbgi, block, entry->high_word, zero_high,
                                     ir_relation_equal);
      ir_node *ffs_high =
          new_rd_Builtin(dbgi, block, mem, 1, in_high, kind, lowered_type_high);
      ir_node *high_proj =
          new_r_Proj(ffs_high, result_mode, pn_Builtin_max + 1);
      ir_node *high = new_rd_Add(dbgi, block, high_proj, number_of_bits);
      ir_node *ffs_low =
          new_rd_Builtin(dbgi, block, mem, 1, in_low, kind, lowered_type_low);
    ...
    case ir_bk_clz: {
      ...
      ir_node *cmp_high =
          new_rd_Cmp(dbgi, block, entry->high_word, zero, ir_relation_equal);
      ir_node *clz_high =
          new_rd_Builtin(dbgi, block, mem, 1, in_high, kind, lowered_type_high);
      ...
      ir_node *number_of_bits =
          new_r_Const_long(irg, result_mode, get_mode_size_bits(mode));
    ...
    case ir_bk_popcount: {
      ir_node *popcount_high =
          new_rd_Builtin(dbgi, block, mem, 1, in_high, kind, lowered_type_high);
      ir_node *popcount_low =
          new_rd_Builtin(dbgi, block, mem, 1, in_low, kind, lowered_type_low);
      ir_node *high =
          new_r_Proj(popcount_high, result_mode, pn_Builtin_max + 1);
    ...
    case ir_bk_parity: {
      ...
      ir_node *const parity =
          new_rd_Builtin(dbgi, block, mem, 1, in, kind, lowered_type_high);
...
static void lower_arithmetic_builtin(ir_node *const builtin) {
  ...
  switch (kind) {
    case ir_bk_bswap: {
      ...
      ir_node *low =
          new_r_Proj(swap_low, env.p.word_unsigned, pn_Builtin_max + 1);
  ...
  if (proj) ir_set_dw_lowered(proj, res_low, res_high);
}
...
static int cmp_op_mode(const void *elt, const void *key, size_t size) {
  ...
  return (e1->op != e2->op) | (e1->imode != e2->imode) |
         (e1->omode != e2->omode);
}
...
static void enqueue_preds(ir_node *node) {
  foreach_irn_in(node, i, pred) { deq_push_pointer_right(&env.waitq, pred); }
}

static void lower_node(ir_node *node) {
  if (irn_visited_else_mark(node)) return;
  ...
  if (!is_Cond(node)) {
    foreach_irn_in(node, i, pred) { lower_node(pred); }
  }
...
static void lower_irg(ir_graph *irg) {
  ...
  confirm_irg_properties(
      irg, !(env.flags & MUST_BE_LOWERED) ? IR_GRAPH_PROPERTIES_ALL
...
static ir_type *make_type_4_2(ir_type *const even, ir_type *const odd) {
  ir_type *const tp =
      new_type_method(4, 2, false, cc_cdecl_set, mtp_no_property);
...
static ir_type *make_type_2_2(ir_type *const even, ir_type *const odd) {
  ir_type *const tp =
      new_type_method(2, 2, false, cc_cdecl_set, mtp_no_property);
  set_method_param_type(tp, 0, even);
  set_method_param_type(tp, 1, odd);
  set_method_res_type(tp, 0, even);
  set_method_res_type(tp, 1, odd);
...
void ir_lower_dw_ops(void) {
  ...
  env.tv_mode_bytes =
      new_tarval_from_long(env.p.doubleword_size / (2 * 8), offset_mode);
  deq_init(&env.waitq);

  irp_reserve_resources(irp,
                        IRP_RESOURCE_TYPE_LINK | IRP_RESOURCE_TYPE_VISITED);
  ...
  foreach_irp_irg(i, irg) {
    ...
    if (ir_nodeset_size(&created_mux_nodes) > 0) lower_mux(irg, lower_mux_cb);
