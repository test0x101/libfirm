...
static void analyse_switch0(switch_info_t *info, ir_node *switchn) {
  ...
  for (size_t e = 0; e < n_entries; ++e) {
    ...
    if (entry->pn == pn_Switch_default) continue;
...
static int compare_entries(const void *a, const void *b) {
  ...
  if (entry0->pn == pn_Switch_default) return 1;
  if (entry1->pn == pn_Switch_default) return -1;

  if (tarval_cmp(entry0->max, entry1->min) == ir_relation_less) return -1;
...
static void normalize_table(ir_node *switchn, ir_mode *new_mode,
  ...
  for (size_t e = 0, n_entries = ir_switch_table_get_n_entries(table);
    ...
    if (delta != NULL) min = tarval_sub(min, delta);
    ...
    if (entry->min == entry->max) {
      ...
      if (delta != NULL) max = tarval_sub(max, delta);
      entry->min = min;
      entry->max = max;
    }
  }
...
static void create_out_of_bounds_check(switch_info_t *info) {
  ...
  ir_node *cmp =
      new_rd_Cmp(dbgi, block, selector, max_const, ir_relation_less_equal);
...
static bool normalize_switch(switch_info_t *info, ir_mode *selector_mode) {
  ...
  if (selector_mode != NULL) {
    ...
    info->switch_max = tarval_convert_to(info->switch_max, mode);
    if (delta != NULL) delta = tarval_convert_to(delta, mode);
    set_Switch_selector(switchn, selector);
  }
...
static void find_switch_nodes(ir_node *block, void *ctx) {
  ...
  if (!is_Switch(switchn)) return;
  if (!ir_nodeset_insert(&env->processed, switchn)) return;
  ...
  bool lower_switch =
      info.num_cases <= env->small_switch ||
