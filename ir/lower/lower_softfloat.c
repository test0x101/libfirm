...
static ir_mode *get_lowered_mode(ir_mode *mode) {
  if (!mode_is_float(mode)) return mode;
...
static ir_node *create_softfloat_address(const ir_node *n, const char *name) {
  ...
  ident *const id =
      float_types + double_types > 1
          ? new_id_fmt("__%s%s%s%s%u", name, first_param, second_param, result,
                       float_types + double_types)
...
static ir_node *make_softfloat_call(ir_node *const n, char const *const name,
  ...
  ir_node *const call =
      new_rd_Call(dbgi, block, nomem, callee, arity, in, type);
...
static ir_type *lower_type(ir_type *const tp) {
  if (!is_Primitive_type(tp)) return tp;
...
static bool lower_Builtin(ir_node *node) {
  if (get_Builtin_kind(node) != ir_bk_va_arg) return false;
...
static bool lower_Cmp(ir_node *const n) {
  ...
  if (!mode_is_float(op_mode)) return false;
...
static bool lower_Div_mode(ir_node *n) {
  ...
  ir_mode *mode = get_irn_mode(n);
  ir_mode *lowered_mode = get_lowered_mode(mode);
  if (lowered_mode == mode && lowered_res_mode == res_mode) return false;
...
static bool lower_Load(ir_node *n) {
  ...
  ir_mode *mode = get_irn_mode(n);
  ir_mode *lowered_mode = get_lowered_mode(mode);
  if (ls_mode == lowered_ls_mode && mode == lowered_mode) return false;
...
static void make_unop_type(ir_type **const memoized, ir_type *const op,
  ...
  if (!*memoized) {
    ir_type *const type = *memoized =
        new_type_method(1, 1, false, cc_cdecl_set, mtp_no_property);
...
static void ir_prepare_softfloat_lowering(void) {
  if (!lowered_type) lowered_type = pmap_create();
...
void lower_floating_point(void) {
  ...
  foreach_irp_irg(i, irg) {
    ...
    if (lowered_mtp != mtp) set_entity_type(ent, lowered_mtp);
