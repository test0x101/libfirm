 ...
 */
static ir_node *adjust_alloc_size(dbg_info *dbgi, ir_node *size,
                                  ir_node *block) {
...
static void lower_node(ir_node *node, void *data) {
  ...
  ir_node *const new_node = new_rd_Alloc(dbgi, block, mem, new_size, 1);

  if (new_node != node) exchange(node, new_node);
  *changed = true;
}

void lower_alloc(ir_graph *irg, unsigned new_po2_stack_alignment) {
  if (new_po2_stack_alignment == 0) return;
