...
static void test(const unsigned char *bytes, unsigned from, unsigned to,
  ...
  for (sc_word *c = val0; c < val0 + val_len; ++c) *c = 0xa5 ^ (c - val0);
  sc_val_from_bits(bytes, from, to, val0);

  char print_buf[32];
  char *res =
      sc_print_buf(print_buf, sizeof(print_buf), val0, 32, SC_HEX, false);
  if (!streq(res, expected)) {
    printf("Failed: 0x%02X%02X%02X%02X [%u:%u) => %s (expected %s)\n", bytes[3],
           bytes[2], bytes[1], bytes[0], from, to, res, expected);
    fine = false;
  }
}
