...
static void compare_int(const char *file, unsigned line, const char *expr0,
                        int v0, const char *expr1, int v1) {
  if (v0 == v1) return;
  fprintf(stderr, "%s:%d [%s%s]: Test failed %s != %s  (%d != %d)\n", file,
          line, context, op_name, expr0, expr1, v0, v1);
  result = 1;
}
#define COMPARE_INT(val0, val1) \
  compare_int(__FILE__, __LINE__, #val0, val0, #val1, val1)
#define TEST(expr) COMPARE_INT(expr, 1)

static void compare_tv(const char *file, unsigned line, const char *expr0,
                       ir_tarval *tv0, const char *expr1, ir_tarval *tv1) {
  if (tv0 == tv1) return;
  ir_fprintf(stderr, "%s:%d [%s%s]: Test failed %s != %s (%T != %T)\n", file,
             line, context, op_name, expr0, expr1, tv0, tv1);
  result = 1;
}
#define TVS_EQUAL(val0, val1) \
  compare_tv(__FILE__, __LINE__, #val0, val0, #val1, val1)
...
static ir_tarval *safe_div(ir_tarval const *op0, ir_tarval const *op1) {
  if (tarval_is_null(op1)) return (ir_tarval *)op1;
...
static ir_tarval *tarval_id(ir_tarval const *tv) { return (ir_tarval *)tv; }
...
static void test_neutral_(binop op, const char *new_op_name,
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ...
    if (commutative) TVS_EQUAL(op(neutral_element, value), value);
  }
...
static void test_zero_(binop op, const char *new_op_name,
                       ir_tarval *zero_element, bool left_zero, bool right_zero,
                       ir_tarval *zero) {
  op_name = new_op_name;

  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ir_tarval *value = tarvals[i];
    if (left_zero) TVS_EQUAL(op(zero_element, value), zero);
    if (right_zero) TVS_EQUAL(op(value, zero_element), zero);
  }
...
static void test_binop_nan_(binop op, const char *new_op_name, ir_tarval *nan) {
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ir_tarval *value = tarvals[i];
    if (tarval_is_nan(value)) continue;
...
static void test_bitcast(ir_mode *mode) {
  for (unsigned m = 0, n = n_modes; m < n; ++m) {
    ...
    if (get_mode_size_bits(other_mode) != get_mode_size_bits(mode)) continue;
...
static void test_compare(ir_tarval *minus_zero, ir_tarval *zero) {
  ...
  for (unsigned a = 0; a < n_tarvals; ++a) {
    ...
    for (unsigned b = 0; b < n_tarvals; ++b) {
      ir_tarval *val_b = tarvals[b];
      TEST(tarval_cmp(val_a, val_b) ==
           get_inversed_relation(tarval_cmp(val_b, val_a)));
      ...
      if (relation != expected) {
        ir_fprintf(stderr,
                   "Comparing %+F with %+F failed: got %s, expected %s\n",
                   val_a, val_b, get_relation_string(relation),
                   get_relation_string(expected));
      }
    }
  }
}
...
static void test_int_tarvals(ir_mode *mode) {
  ...
  n_tarvals = 0;
  while (tarvals[n_tarvals] != NULL) ++n_tarvals;
  ...
  if (bits % 8 == 0) test_neutral(tarval_shrs, zero, false);
...
static void test_float_tarvals(ir_mode *mode) {
  ir_tarval *nan_payload0 =
      new_tarval_from_str("0xCAFEBABE12345678", 10, mode_Lu);
  ir_tarval *nan_payload1 =
      new_tarval_from_str("0x12345678DEADBEEF", 10, mode_Lu);
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ir_tarval *value = tarvals[i];
    if (value == inf || value == minus_inf || value == qnan || value == snan) {
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ...
    if (value == inf || value == minus_inf) {
      ...
      TVS_EQUAL(tarval_mul(value, zero),
                tarval_is_negative(value) ? minus_zero : zero);
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ...
    if (tarval_is_nan(value) || value == inf || value == minus_inf ||
      ...
      continue;
    TVS_EQUAL(tarval_div(value, zero),
              tarval_is_negative(value) ? minus_inf : inf);
    TVS_EQUAL(tarval_div(value, minus_zero),
              tarval_is_negative(value) ? inf : minus_inf);
  }
  ...
  for (unsigned i = 0, n = n_tarvals; i < n; ++i) {
    ir_tarval *value = tarvals[i];
    if (!tarval_is_finite(value)) continue;
    TVS_EQUAL(tarval_div(value, inf),
              tarval_is_negative(value) ? minus_zero : zero);
    TVS_EQUAL(tarval_div(value, minus_inf),
              tarval_is_negative(value) ? zero : minus_zero);
  }
...
int main(void) {
  ...
  ir_mode *const new_modes[] = {
      ...
      mode_F,
      mode_D,
      new_float_mode("E", irma_x86_extended_float, 15, 64,
  ...
  for (size_t i = 0, n = n_modes; i < n; ++i) {
    ...
    if (mode_is_float(mode)) test_float_tarvals(mode);
  }
