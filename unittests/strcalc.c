...
static const unsigned precision =
    72; /* some random non-po2 number (but a multiple of SC_BITS),
...
static bool is_zero(const sc_word *val) { return sc_is_zero(val, precision); }
...
int main(void) {
  ...
  const sc_word *const vals[] = {zero, all_one, one, alt0,  alt1,    v2,
                                 v4,   v8,      v16, v2048, highbit, cafebabe};
